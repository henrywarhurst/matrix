...
struct file_info {
  ...
  explicit file_info(boost::shared_ptr<file_info> const &parent_,
      ...
      : parent(parent_), document(parent->document),
        compatibility_version(compatibility_version_), depth(parent->depth + 1),
  ...
  explicit file_info(boost::shared_ptr<file_info> const &parent_,
      ...
      : parent(parent_), document(document_),
        compatibility_version(compatibility_version_), depth(0),
        override_depth(0), override_id(0), doc_id_1_1(doc_id_1_1_.to_s()) {}
};
...
struct section_info {
  ...
  explicit section_info(boost::shared_ptr<section_info> const &parent_,
      ...
      : parent(parent_),
        ...
        file_depth(current_file_->depth), level(parent ? parent->level + 1 : 1),
        explicit_id(explicit_id_), id_1_1(id_1_1_.to_s()),
        placeholder_1_6(placeholder_1_6_), source_mode(source_mode_) {}
};
...
void document_state::start_file(unsigned compatibility_version_,
...
std::string document_state::start_file_with_docinfo(
    unsigned compatibility_version_, quickbook::string_view include_doc_id,
    quickbook::string_view id, value const &title_) {
  return state
      ->start_file(compatibility_version_, true, include_doc_id, id, title_)
      ->to_string();
}

void document_state::end_file() { state->end_file(); }

std::string document_state::begin_section(value const &explicit_id_,
                                          quickbook::string_view id,
...
std::string document_state::old_style_id(quickbook::string_view id,
                                         id_category category) {
  return state->old_style_id(id, category)->to_string();
}

std::string document_state::add_id(quickbook::string_view id,
                                   id_category category) {
  return state->add_id(id, category)->to_string();
}

std::string document_state::add_anchor(quickbook::string_view id,
                                       id_category category) {
...
std::string
document_state::replace_placeholders(quickbook::string_view xml) const {
...
id_placeholder::id_placeholder(std::size_t index_, quickbook::string_view id_,
                               id_category category_,
                               id_placeholder const *parent_)
    : index(index_), id(id_.begin(), id_.end()),
      unresolved_id(parent_ ? parent_->unresolved_id + '.' + id : id),
      parent(parent_), category(category_),
...
id_placeholder const *
document_state_impl::add_placeholder(quickbook::string_view id,
                                     id_category category,
...
id_placeholder const *
document_state_impl::get_placeholder(quickbook::string_view value) const {
...
id_placeholder const *
document_state_impl::start_file(unsigned compatibility_version,
                                bool document_root,
                                quickbook::string_view include_doc_id,
                                quickbook::string_view id, value const &title) {
  ...
  if (document_root || compatibility_version < 106u) {
    ...
    doc_id_1_1 = !initial_doc_id.empty()
                     ? initial_doc_id.to_s()
  ...
  if (document_root) {
    ...
    if (!initial_doc_id.empty()) {
      ...
      return create_new_section(
          empty_value(), detail::make_identifier(title.get_quickbook()),
          id_category::generated_doc, default_source_mode);
    } else if (compatibility_version >= 106u) {
      return create_new_section(empty_value(), "doc", id_category::numbered,
                                default_source_mode);
    } else {
      return create_new_section(empty_value(), "", id_category::generated_doc,
                                default_source_mode);
    }
  } else {
    ...
    if (!initial_doc_id.empty() && compatibility_version >= 106u) {
      ...
      override_id = add_id_to_section(
          initial_doc_id, id_category::explicit_section_id, null_section);
    }

    // Create new file

    current_file = boost::make_shared<file_info>(parent, compatibility_version,
...
void document_state_impl::end_file() { current_file = current_file->parent; }

id_placeholder const *document_state_impl::add_id(quickbook::string_view id,
                                                  id_category category) {
...
id_placeholder const *document_state_impl::add_id_to_section(
  ...
  if (!section || section->compatibility_version >= 106u) {
    ...
    if (!new_id.empty() && !qualified_id.empty())
      new_id += '.';
    new_id += qualified_id;
    if (!new_id.empty() && !id_part.empty())
      new_id += '.';
    new_id += id_part;
...
id_placeholder const *
document_state_impl::old_style_id(quickbook::string_view id,
                                  id_category category) {
  return current_file->compatibility_version < 103u
             ? add_placeholder(current_file->document->section_id_1_1 + "." +
                                   id.to_s(),
                               category)
             : add_id(id, category);
}
...
id_placeholder const *document_state_impl::begin_section(
    value const &explicit_id, quickbook::string_view id, id_category category,
    source_mode_info const &source_mode) {
...
id_placeholder const *document_state_impl::create_new_section(
  ...
  current_file->document->current_section =
      boost::make_shared<section_info>(parent, current_file.get(), explicit_id,
                                       id_1_1, placeholder_1_6, source_mode);
