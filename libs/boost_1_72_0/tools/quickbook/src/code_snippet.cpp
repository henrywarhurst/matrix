...
struct code_snippet_actions {
  code_snippet_actions(std::vector<template_symbol> &storage_,
                       file_ptr source_file_, char const *source_type_)
      : last_code_pos(source_file_->source().begin()), in_code(false),
        snippet_stack(), storage(storage_), source_file(source_file_),
        source_type(source_type_), error_count(0) {
...
struct python_code_snippet_grammar : cl::grammar<python_code_snippet_grammar> {
...
int load_snippets(
  ...
  code_snippet_actions a(storage, load(filename, qbk_version_n),
                         is_python ? "[python]" : "[c++]");
  ...
  if (is_python) {
    info = boost::spirit::classic::parse(first, last,
...
void code_snippet_actions::append_code(string_iterator first,
...
void code_snippet_actions::pass_thru(string_iterator first,
                                     string_iterator last) {
  if (!snippet_stack)
    return;
...
void code_snippet_actions::escaped_comment(string_iterator first,
                                           string_iterator last) {
  append_code(first, last);
  close_code();
...
void code_snippet_actions::start_snippet(string_iterator first,
                                         string_iterator last) {
  append_code(first, last);
  start_snippet_impl(std::string(mark_begin, mark_end), first);
}

void code_snippet_actions::end_snippet(string_iterator first,
                                       string_iterator last) {
  append_code(first, last);
  ...
  while (snippet_stack) {
    if (qbk_version_n >= 106u) {
      detail::outerr(source_file->path)
          << "Unclosed snippet '" << snippet_stack->id << "'" << std::endl;
      ++error_count;
    } else {
...
void code_snippet_actions::end_snippet_impl(string_iterator position) {
  ...
  storage.push_back(
      template_symbol(snippet->id, params,
                      qbk_value(body, body->source().begin(),
                                body->source().end(), template_tags::snippet)));
}
} // namespace quickbook
