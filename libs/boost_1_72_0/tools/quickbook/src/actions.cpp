...
std::string
add_anchor(quickbook::state &state, quickbook::string_view id,
...
std::string get_attribute_value(quickbook::state &state,
  ...
  std::string x =
      value.is_encoded() ? value.get_encoded() : value.get_quickbook().to_s();
...
std::string validate_id(quickbook::state &state,
  ...
  if (qbk_version_n >= 107u) {
    ...
    QUICKBOOK_FOR(char c, id) {
      if (!std::isalnum(c) && !std::strchr(allowed_punctuation, c))
        valid = false;
    }
  }
...
bool quickbook_strict::is_strict_checking() const { return state.strict_mode; }
...
void element_action::operator()(parse_iterator first, parse_iterator) const {
  ...
  if (!values.check() || !state.conditional)
    return;
...
void error_message_action::operator()(parse_iterator first,
                                      parse_iterator last) const {
...
void error_action::operator()(parse_iterator first,
                              parse_iterator /*last*/) const {
  file_position const pos = state.current_file->position_of(first.base());
...
void role_action(quickbook::state &state, value role_list) {
  ...
  detail::print_string(get_attribute_value(state, role), state.phrase.get());
...
void paragraph_action::operator()() const {
  ...
  if (pos != end) {
    detail::markup markup =
        state.in_list ? detail::get_markup(block_tags::paragraph_in_list)
...
void explicit_list_action::operator()() const { state.explicit_list = true; }
...
void write_bridgehead(quickbook::state &state, int level,
                      std::string const &str, std::string const &id,
                      bool self_link) {
...
void header_action(quickbook::state &state, value heading_list) {
  ...
  if (!element_id.empty()) {
    ...
    std::string anchor = state.document.add_id(validate_id(state, element_id),
                                               id_category::explicit_id);

    write_bridgehead(state, level, content.get_encoded(), anchor,
                     self_linked_headers);
  } else if (state.document.compatibility_version() >= 106u) {
    // Generate ids for 1.6+

    std::string anchor =
        state.document.add_id(detail::make_identifier(content.get_quickbook()),
    ...
    if (generic || state.document.compatibility_version() >= 103) {
      ...
      write_bridgehead(state, level, content.get_encoded(), anchor, false);
    }
  }
}
...
bool cond_phrase_push::start() {
  ...
  if (saved_conditional) {
    ...
    state.conditional = (bool)find(state.macro, macro.c_str()) == positive;
...
void raw_char_action::operator()(parse_iterator first,
                                 parse_iterator last) const {
  while (first != last)
    state.phrase << *first++;
}
...
void code_action(quickbook::state &state, value code_block) {
  ...
  if (block) {
    ...
    if (qbk_version_n >= 107u)
      state.phrase << state.end_callouts();
...
void plain_char_action::operator()(parse_iterator first,
                                   parse_iterator last) const {
  write_anchors(state, state.phrase);
...
void escape_unicode_action::operator()(parse_iterator first,
                                       parse_iterator last) const {
  write_anchors(state, state.phrase);
  ...
  if (hex_digits.size() == 2 && *first > '0' && *first <= '7') {
    using namespace std;
    detail::print_char((char)strtol(hex_digits.c_str(), 0, 16),
                       state.phrase.get());
  } else {
...
void image_action(quickbook::state &state, value image) {
  ...
  if (fileref.find('\\') != std::string::npos) {
    (qbk_version_n >= 106u
         ? detail::outerr(attributes["fileref"].get_file(),
                          attributes["fileref"].get_position())
         : detail::outwarn(attributes["fileref"].get_file(),
                           attributes["fileref"].get_position()))
        << "Image path isn't portable: '" << fileref << "'" << std::endl;
  ...
  quickbook::value alt_text =
      alt_pos != attributes.end()
          ? alt_pos->second
          : qbk_version_n < 106u ? encoded_value(stem) : quickbook::value();
  ...
  if (extension == "svg") {
    ...
    if (a != std::string::npos) {
      attributes.insert(std::make_pair(
          "contentwidth",
          encoded_value(std::string(boost::next(svg_text.begin(), a + 1),
...
void macro_definition_action(quickbook::state &state,
  ...
  std::string phrase;
  if (phrase_value.check())
    phrase = phrase_value.get_encoded();
  values.finish();
...
void template_body_action(quickbook::state &state,
  ...
  BOOST_ASSERT(values.check(template_tags::block) ||
               values.check(template_tags::phrase));
  value body = values.consume();
  BOOST_ASSERT(!values.check());

  if (!state.templates.add(template_symbol(identifier, template_values, body,
...
namespace {
string_iterator find_first_seperator(string_iterator begin,
                                     string_iterator end) {
...
std::pair<string_iterator, string_iterator>
find_seperator(string_iterator begin, string_iterator end) {
...
void break_arguments(std::vector<value> &args,
  ...
  if (qbk_version_n < 105 ? args.size() : args.size() == 1) {
    ...
    while (args.size() < params.size()) {
      ...
      if (pos.second == end)
        break;
      value new_arg(qbk_value(last_arg.get_file(), pos.second, end,
                              template_tags::phrase));

      args.back() =
          qbk_value(last_arg.get_file(), begin, pos.first, last_arg.get_tag());
...
get_arguments(std::vector<value> const &args,
              std::vector<std::string> const &params,
              template_scope const &scope, string_iterator first,
...
bool parse_template(value const &content, quickbook::state &state,
...
void call_template(quickbook::state &state, template_symbol const *symbol,
                   std::vector<value> const &args, string_iterator first,
                   bool is_attribute_template = false) {
...
void call_code_snippet(quickbook::state &state, template_symbol const *symbol,
                       string_iterator first) {
  assert(symbol->params.size() == 0);
  std::vector<value> args;

  // Create a fake symbol for call_template
  template_symbol t(symbol->identifier, symbol->params, symbol->content,
...
void do_template_action(quickbook::state &state, value template_list,
  ...
  QUICKBOOK_FOR(value arg, values) { args.push_back(arg); }
  ...
  case template_tags::phrase:
    ...
    if (args.size() != symbol->params.size()) {
      detail::outerr(state.current_file, first)
          ...
          << symbol->params.size() << " argument(s), got: " << args.size()
...
void begin_section_action(quickbook::state &state, value begin_section_list) {
  ...
  std::string full_id = state.document.begin_section(
      element_id,
      element_id.empty() ? detail::make_identifier(content.get_quickbook())
...
void end_section_action(quickbook::state &state, value end_section_list,
                        string_iterator first) {
  ...
  if (!element_id.empty() && !(element_id == state.document.explicit_id())) {
    ...
    if (section_element_id.empty()) {
      detail::outerr(state.current_file->path, pos.line)
          << "Endsect has unexpected id '" << element_id.get_quickbook()
          << "' in section with no explicit id, near column " << pos.column
          << ".\n";
    } else {
      detail::outerr(state.current_file->path, pos.line)
          << "Endsect has incorrect id '" << element_id.get_quickbook()
          << "', expected '" << state.document.explicit_id().get_quickbook()
...
void element_id_warning_action::operator()(parse_iterator first,
                                           parse_iterator) const {
...
void xinclude_action(quickbook::state &state, value xinclude) {
  ...
  if (x.type == path_parameter::path) {
    ...
    state.out << "\n<xi:include href=\"";
    detail::print_string(file_path_to_url(path.abstract_file_path),
                         state.out.get());
    state.out << "\" />\n";
  }
}

void load_quickbook(quickbook::state &state, quickbook_path const &path,
                    value::tag_type load_type,
                    value const &include_doc_id = value()) {
  assert(load_type == block_tags::include || load_type == block_tags::import);
...

  {
    ...
    // file.
    state_save save(state, load_type == block_tags::import
    ...
    if (keep_inner_source_mode)
      save.source_mode = state.source_mode;
  }
...
void load_source_file(quickbook::state &state, quickbook_path const &path,
                      value::tag_type load_type, string_iterator first,
                      value const & /* include_doc_id */ = value()) {
  assert(load_type == block_tags::include || load_type == block_tags::import);
  ...
  // Throws load_error
  state.error_count += load_snippets(path.file_path, storage, ext, load_type);
...
void include_action(quickbook::state &state, value include,
                    string_iterator first) {
  write_anchors(state, state.out);
  ...
  std::set<quickbook_path> search = include_search(parameter, state, first);
    ...
    try {
      if (qbk_version_n >= 106) {
        ...
        if (ext == ".qbk" || ext == ".quickbook") {
          ...
          load_source_file(state, path, include.get_tag(), first,
                           include_doc_id);
        }
      } else {
      ...
      detail::outerr(state.current_file, first)
          << "Loading file " << path.file_path << ": " << e.what() << std::endl;
    }
  }
}
...
void to_value_scoped_action::success(parse_iterator first,
                                     parse_iterator last) {
  std::string value;
