...
struct document_state {
  ...
  std::string start_file_with_docinfo(unsigned compatibility_version,
                                      quickbook::string_view include_doc_id,
                                      quickbook::string_view id,
                                      value const &title);

  void start_file(unsigned compatibility_version,
                  quickbook::string_view include_doc_id,
                  quickbook::string_view id, value const &title);

  void end_file();

  std::string begin_section(value const &, quickbook::string_view, id_category,
  ...
  std::string
      replace_placeholders_with_unresolved_ids(quickbook::string_view) const;
