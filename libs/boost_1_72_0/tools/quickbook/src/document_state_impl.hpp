...
struct document_state_impl {
  ...
  get_id_placeholder(boost::shared_ptr<section_info> const &section) const;

  // Events

  id_placeholder const *start_file(unsigned compatibility_version,
                                   bool document_root,
  ...
  id_placeholder const *add_id(quickbook::string_view id, id_category category);
  id_placeholder const *old_style_id(quickbook::string_view id,
                                     id_category category);
  id_placeholder const *begin_section(value const &explicit_id,
                                      quickbook::string_view id,
                                      id_category category,
...
private:
  id_placeholder const *
  add_id_to_section(quickbook::string_view id, id_category category,
                    boost::shared_ptr<section_info> const &section);
  id_placeholder const *create_new_section(value const &explicit_id,
                                           quickbook::string_view id,
                                           id_category category,
                                           source_mode_info const &);
};

std::string replace_ids(document_state_impl const &state,
                        quickbook::string_view xml,
                        std::vector<std::string> const * = 0);
std::vector<std::string> generate_ids(document_state_impl const &,
