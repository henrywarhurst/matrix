...
public:
  ...
  bool has_attribute(quickbook::string_view name) {
    for (auto it = attributes_.begin(), end = attributes_.end(); it != end;
         ++it) {
      if (name == it->first) {
        return true;
      }
  ...
  string_view set_attribute(quickbook::string_view name,
    ...
    attributes_.push_back(std::make_pair(name.to_s(), value.to_s()));
...
struct xml_parse_error {
  ...
  xml_parse_error(char const *m, string_iterator p) : message(m), pos(p) {}
};
