...
public:
  ...
  file(fs::path const &path_, quickbook::string_view source_view,
      ...
      : path(path_), source_(source_view.begin(), source_view.end()),
        is_code_snippets(false), qbk_version(qbk_version_), ref_count(0) {}

  explicit file(file const &f, quickbook::string_view s)
      : path(f.path), source_(s.begin(), s.end()),
        is_code_snippets(f.is_code_snippets), qbk_version(f.qbk_version),
        ref_count(0) {}
