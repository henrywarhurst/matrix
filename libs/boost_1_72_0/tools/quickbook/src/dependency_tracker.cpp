...
static std::string get_path(fs::path const &path, dependency_tracker::flags f) {
...
dependency_tracker::dependency_tracker()
    : dependencies(), glob_dependencies(), last_glob(glob_dependencies.end()) {}
...
void dependency_tracker::add_glob(fs::path const &f) {
  std::pair<glob_list::iterator, bool> r =
      glob_dependencies.insert(std::make_pair(f, glob_list::mapped_type()));
  last_glob = r.first;
}
...
void dependency_tracker::write_dependencies(fs::path const &file_out, flags f) {
  fs::ofstream out(file_out);

  if (out.fail()) {
    throw std::runtime_error("Error opening dependency file " +
...
void dependency_tracker::write_dependencies(std::ostream &out, flags f) {
  if (f & checked) {
    QUICKBOOK_FOR(dependency_list::value_type const &d, dependencies) {
      out << (d.second ? "+ " : "- ") << get_path(d.first, f) << std::endl;
    }
    ...
    QUICKBOOK_FOR(std::string const &p, paths) { out << p << std::endl; }
  }
}
} // namespace quickbook
