...
#else
std::string command_line_to_utf8(command_line_string const &x) { return x; }
#endif
...
std::string path_to_generic(fs::path const &x) { return x.generic_string(); }
#endif
...
stream_string path_to_stream(fs::path const &path) { return path.string(); }
#else
stream_string path_to_stream(fs::path const &path) { return path.native(); }
#endif
...
path_or_url::operator bool() const { return type_ != path_or_url_empty; }
