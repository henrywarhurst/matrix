...
static void write_document_title(collector &out, value const &title,
                                 value const &version);
std::string write_boostbook_header(quickbook::state &state,
                                   doc_info_values const &info,
                                   bool nested_file);
...
value consume_list(value_consumer &c, value::tag_type tag,
  ...
  if (count > 1)
    duplicates->push_back(doc_info_attribute_name(tag));
...
std::vector<value> consume_multiple_values(value_consumer &c,
                                           value::tag_type tag) {
...
unsigned get_version(quickbook::state &state, bool using_docinfo,
  ...
  if (!version.empty()) {
    ...
    bool before_docinfo =
        version_values.optional_consume(doc_info_tags::before_docinfo).check();
    ...
    if (before_docinfo || using_docinfo) {
      result = ((unsigned)major_verison * 100) + (unsigned)minor_verison;

      if (classify_version(result) == version_unknown) {
        detail::outerr(state.current_file->path)
            << "Unknown version: " << major_verison << "." << minor_verison
            << std::endl;
        ++state.error_count;
      }
    }
...
std::string pre(quickbook::state &state, parse_iterator pos,
  ...
  if (values.check(doc_info_tags::type)) {
    ...
    if (!nested_file) {
      ...
      info.doc_title = qbk_value(state.current_file, pos.base(), pos.base(),
                                 doc_info_tags::type);
      use_doc_info = true;
    }
  }
  ...
  info.compatibility_mode =
      consume_list(values, doc_attributes::compatibility_mode, &duplicates);
  ...
  info.dirname =
      consume_value_in_list(values, doc_info_attributes::dirname, &duplicates);
  info.last_revision = consume_value_in_list(
      values, doc_info_attributes::last_revision, &duplicates);
  info.purpose =
      consume_value_in_list(values, doc_info_attributes::purpose, &duplicates);
  info.categories =
      consume_multiple_values(values, doc_info_attributes::category);
  info.lang =
      consume_value_in_list(values, doc_info_attributes::lang, &duplicates);
  info.version =
      consume_value_in_list(values, doc_info_attributes::version, &duplicates);
  ...
  info.license =
      consume_value_in_list(values, doc_info_attributes::license, &duplicates);
  info.biblioids =
      consume_multiple_values(values, doc_info_attributes::biblioid);
  info.xmlbase =
      consume_value_in_list(values, doc_info_attributes::xmlbase, &duplicates);
  ...
  if (!compatibility_version) {
    compatibility_version =
        use_doc_info ? qbk_version_n : state.document.compatibility_version();
  }

  // Start file, finish here if not generating document info.

  if (!use_doc_info) {
    state.document.start_file(compatibility_version, info.include_doc_id_,
                              info.id_, info.doc_title);
    return "";
  }
...
std::string write_boostbook_header(quickbook::state &state,
  ...
  if (!info.last_revision.empty()) {
    ...
    char strdate[64];
    strftime(strdate, sizeof(strdate),
  ...
  QUICKBOOK_FOR(value_consumer copyright, info.copyrights) {
    while (copyright.check()) {
      ...
      while (copyright.check(doc_info_tags::copyright_year)) {
        ...
        int year_end = copyright.check(doc_info_tags::copyright_year_end)
        ...
        if (year_end < year_start) {
          ...
          detail::outerr(state.current_file, copyright.begin()->get_position())
              << "Invalid year range: " << year_start << "-" << year_end << "."
              << std::endl;
        }
      ...
      tmp << "      <holder>"
          << doc_info_output(copyright.consume(doc_info_tags::copyright_name),
                             106)
          << "</holder>\n"
          << "    </copyright>\n"
          << "\n";
  ...
  QUICKBOOK_FOR(value_consumer category_values, info.categories) {
    ...
    if (!category.empty()) {
      tmp << "    <" << info.doc_type
          << "category name=\"category:" << doc_info_output(category, 106)
...
void post(quickbook::state &state, std::string const &doc_type) {
  ...
  if (!doc_type.empty() && state.document.section_level() > 1) {
    if (state.strict_mode) {
      ...
      detail::outwarn(state.current_file->path)
          << "Missing [endsect] detected at end of file." << std::endl;
    }
