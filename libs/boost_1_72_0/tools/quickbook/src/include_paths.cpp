  ...
  if (qbk_version_n >= 107u) {
    ...
    try {
      if (check_glob(path_text)) {
        ...
        return path_parameter(glob_unescape(path_text), path_parameter::path);
      }
    } catch (glob_error &e) {
      detail::outerr(path.get_file(), path.get_position())
          << "Invalid path (" << e.what() << "): " << path_text << std::endl;
      ++state.error_count;
      return path_parameter(path_text, path_parameter::invalid);
    }
  } else {
    ...
    if (path_text.find('\\') != std::string::npos) {
      ...
      *err << "Path isn't portable: '" << path_text << "'" << std::endl;
...
void include_search_glob(std::set<quickbook_path> &result,
                         quickbook_path const &location, std::string path,
  ...
  quickbook::string_view glob(path.data() + glob_begin, glob_end - glob_begin);

  fs::path base_dir =
      new_location.file_path.empty() ? fs::path(".") : new_location.file_path;
  if (!fs::is_directory(base_dir))
    return;

  // Walk through the dir for matches.
  for (fs::directory_iterator dir_i(base_dir), dir_e; dir_i != dir_e; ++dir_i) {
    fs::path f = dir_i->path().filename();
    std::string generic_path = detail::path_to_generic(f);

    // Skip if the dir item doesn't match.
    if (!quickbook::glob(glob, generic_path))
      continue;
...
std::set<quickbook_path> include_search(path_parameter const &parameter,
  ...
  case path_parameter::glob:
    ...
    // we do a discovery of all the matches..
    {
      ...
      QUICKBOOK_FOR(fs::path dir, include_path) {
        ...
        include_search_glob(result, quickbook_path(dir, count, fs::path()),
                            parameter.value, state);
      }
  ...
  case path_parameter::path: {
    ...
    if (!path.has_root_directory() && !path.has_root_name()) {
      quickbook_path path2 = state.current_path.parent_path() / parameter.value;
...
quickbook_path resolve_xinclude_path(std::string const &x,
                                     quickbook::state &state, bool is_file) {
