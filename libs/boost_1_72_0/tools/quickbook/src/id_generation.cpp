...
placeholder_index index_placeholders(document_state_impl const &,
                                     quickbook::string_view);

void generate_id_block(placeholder_index::iterator, placeholder_index::iterator,
                       std::vector<std::string> &generated_ids);

std::vector<std::string> generate_ids(document_state_impl const &state,
...
placeholder_index index_placeholders(document_state_impl const &state,
                                     quickbook::string_view xml) {
  ...
  if (order[p.index])
    sorted_placeholders.push_back(&p);
...
std::string
generate_id_block_type::generate_id(id_placeholder const *p,
  ...
  if (child_start == std::string::npos) {
    ...
    base_id = normalize_id(resolved_id.substr(child_start + 1), max_size - 1);
  }
...
struct replace_ids_callback : xml_processor::callback {
  ...
  void id_value(quickbook::string_view value) {
    if (id_placeholder const *p = state.get_placeholder(value)) {
      quickbook::string_view id = ids ? (*ids)[p->index] : p->unresolved_id;
