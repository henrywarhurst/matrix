...
struct syntax_highlight_actions {
  ...
  syntax_highlight_actions(quickbook::state &state_, bool is_block_)
      : state(state_), do_macro_impl(state_),
...
void syntax_highlight_actions::span(parse_iterator first, parse_iterator last,
                                    char const *name) {
...
void syntax_highlight_actions::pre_escape_back(parse_iterator, parse_iterator) {
...
void syntax_highlight_actions::callout(parse_iterator, parse_iterator) {
  state.phrase << state.add_callout(
      qbk_value(state.current_file, marked_text.begin(), marked_text.end()));
  marked_text.clear();
}
...
struct cpp_highlight : public cl::grammar<cpp_highlight> {
  ...
  template <typename Scanner> struct definition {
    definition(cpp_highlight const &self) : g(self.actions.state.grammar()) {
      // clang-format off
    ...
    cl::rule<Scanner> program, line_start, rest_of_line, macro, preprocessor,
        inline_callout, line_callout, comment, special, string_, char_, number,
        identifier, keyword, escape, string_char;
...
struct python_highlight : public cl::grammar<python_highlight> {
  ...
  template <typename Scanner> struct definition {
    definition(python_highlight const &self) : g(self.actions.state.grammar()) {
      // clang-format off
    ...
    cl::rule<Scanner> program, macro, inline_callout, line_callout, comment,
        special, string_, string_prefix, short_string, long_string, number,
        identifier, keyword, escape, string_char;
...
struct teletype_highlight : public cl::grammar<teletype_highlight> {
  teletype_highlight(syntax_highlight_actions &actions_) : actions(actions_) {}
  ...
  template <typename Scanner> struct definition {
    definition(teletype_highlight const &self)
      ...
      member_action<syntax_highlight_actions> plain_char(
          ...
          pre_escape_back(self.actions,
                          &syntax_highlight_actions::pre_escape_back),
          post_escape_back(self.actions,
...
void syntax_highlight(parse_iterator first, parse_iterator last,
                      quickbook::state &state, source_mode_type source_mode,
                      bool is_block) {
