...
struct pretty_printer {
  ...
  bool line_is_empty() const {
    for (iter_type i = out.end() - (column - current_indent); i != out.end();
         ++i) {
      if (*i != ' ')
        return false;
    }
...
char const *block_tags_[] = {
    "author",       "blockquote",   "bridgehead", "callout",   "calloutlist",
    "caution",      "copyright",    "entry",      "important", "informaltable",
    "itemizedlist", "legalnotice",  "listitem",   "note",      "orderedlist",
    "para",         "row",          "section",    "simpara",   "table",
    "tbody",        "textobject",   "tgroup",     "thead",     "tip",
    "variablelist", "varlistentry", "warning",    "xml",       "xi:include"};
...
struct tidy_grammar : cl::grammar<tidy_grammar> {
  ...
  template <typename Scanner> struct definition {
    ...
    cl::rule<Scanner> tidy, tag, start_tag, start_end_tag, content, end_tag,
        markup, code, escape;
  };
  ...
  void do_content(iter_type f, iter_type l) const { state.printer.print(f, l); }
