...
namespace quickbook {
namespace {
char const *id_attributes_[] = {"id", "linkend", "linkends", "arearefs"};
}
    ...
    default:
      if ((*it >= 'a' && *it <= 'z') || (*it >= 'A' && *it <= 'Z') ||
        ...
        for (;;) {
          ...
          if (boost::find(id_attributes, name.to_s()) != id_attributes.end()) {
            c.id_value(value);
          }
        }
      } else {
...
namespace detail {
std::string linkify(quickbook::string_view source,
