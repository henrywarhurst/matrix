...
bool check_bom(InputIterator &begin, InputIterator end, OutputIterator out,
               char const *chars, int length) {
...
file_ptr load(fs::path const &filename, unsigned qbk_version) {
  boost::unordered_map<fs::path, file_ptr>::iterator pos = files.find(filename);
  ...
  if (pos == files.end()) {
    ...
    std::string source;
    normalize(std::istream_iterator<char>(in), std::istream_iterator<char>(),
    ...
    boost::tie(pos, inserted) =
        files.emplace(filename, new file(filename, source, qbk_version));
...
file_position relative_position(string_iterator begin,
...
struct mapped_file_section {
  enum section_types { normal, empty, indented };

  std::string::size_type original_pos;
  std::string::size_type our_pos;
  section_types section_type;

  explicit mapped_file_section(std::string::size_type original_pos_,
...
struct mapped_section_original_cmp {
  bool operator()(mapped_file_section const &x, mapped_file_section const &y) {
...
struct mapped_file : file {
  explicit mapped_file(file_ptr original_)
      : file(*original_, std::string()), original(original_),
        mapped_sections() {}
  ...
  void add_empty_mapped_file_section(string_iterator pos) {
    std::string::size_type original_pos = pos - original->source().begin();

    if (mapped_sections.empty() ||
        mapped_sections.back().section_type != mapped_file_section::empty ||
  ...
  void add_mapped_file_section(string_iterator pos) {
    mapped_sections.push_back(
        mapped_file_section(pos - original->source().begin(), source().size()));
  }
  ...
  to_original_pos(std::vector<mapped_file_section>::const_iterator section,
    ...
    case mapped_file_section::indented: {
      ...
      for (quickbook::string_view::size_type i = section->our_pos; i != pos;
           ++i) {
        if (source()[i] == '\n') {
          our_line = i + 1;
          ++newline_count;
      ...
      quickbook::string_view::size_type original_line = section->original_pos;
      ...
      original_line = skip_indentation(original->source(), original_line);
  ...
  std::vector<mapped_file_section>::const_iterator
  find_section(string_iterator pos) const {
...
private:
  static std::string::size_type skip_indentation(quickbook::string_view src,
...
void mapped_file_builder::add(mapped_file_builder const &x, pos_type begin,
                              pos_type end) {
  ...
  if (begin != end) {
    std::vector<mapped_file_section>::const_iterator i =
        x.data->new_file->find_section(x.data->new_file->source().begin() +
                                       begin);

    std::string::size_type size = data->new_file->source_.size();

    data->new_file->mapped_sections.push_back(mapped_file_section(
        x.data->new_file->to_original_pos(i, begin), size, i->section_type));

    for (++i; i != x.data->new_file->mapped_sections.end() && i->our_pos < end;
         ++i) {
      data->new_file->mapped_sections.push_back(mapped_file_section(
          i->original_pos, i->our_pos - begin + size, i->section_type));
    }

    data->new_file->source_.append(x.data->new_file->source_.begin() + begin,
...
void mapped_file_builder::unindent_and_add(quickbook::string_view x) {
  ...
  quickbook::string_view::size_type full_indent =
      indentation_count(quickbook::string_view(&program[text_start], indent));
  ...
  while (std::string::npos != (pos = program.find_first_of("\r\n", pos))) {
    ...
    full_indent = (std::min)(
        full_indent,
        indentation_count(quickbook::string_view(&program[pos], n - pos)));
  }
  ...
  while (std::string::npos != (pos = program.find_first_of("\r\n", pos))) {
    ...
    std::string::size_type n = program.find_first_not_of(" \t", pos);
    if (n == std::string::npos || n - pos < indent)
      continue;
  ...
  do {
    ...
    unindented_program.append(program.begin() + copy_start,
                              program.begin() + pos);
    copy_start = pos;
    ...
    if (mixed_indentation) {
      string_view::size_type length =
          indentation_count(quickbook::string_view(&program[pos], next - pos));
