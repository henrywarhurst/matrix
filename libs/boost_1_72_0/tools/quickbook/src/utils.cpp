...
static std::string escape_uri_impl(quickbook::string_view uri_param,
                                   char const *mark) {
  ...
  for (std::string::size_type n = 0; n < uri_param.size(); ++n) {
    if (static_cast<unsigned char>(uri_param[n]) > 127 ||
        ...
        (uri_param[n] == '%' &&
         !(n + 2 < uri_param.size() && std::strchr(hex, uri_param[n + 1]) &&
           std::strchr(hex, uri_param[n + 2])))) {
      char escape[] = {'%', hex[uri_param[n] / 16], hex[uri_param[n] % 16],
                       '\0'};
      uri += escape;
    } else {
      uri += uri_param[n];
