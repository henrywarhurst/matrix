...
public:
  ...
  friend void intrusive_ptr_add_ref(value_node *ptr) { ++ptr->ref_count_; }
...
protected:
  explicit value_base(value_node *base) : value_(base) { assert(value_); }
...
public:
  ...
  string_iterator get_position() const { return value_->get_position(); }
...
struct value_base::iterator
    : public boost::forward_iterator_helper<iterator, value, int, value_proxy,
                                            value_ref> {
public:
  iterator();
...
inline value_base::iterator value_base::end() const { return iterator(); }
...
value encoded_value(std::string const &, value::tag_type = value::default_tag);
...
public:
  struct iterator : public boost::input_iterator_helper<
                        iterator, boost::iterator_value<value::iterator>::type,
  ...
  value_consumer(value const &x) : list_(x), pos_(x.begin()), end_(x.end()) {}
