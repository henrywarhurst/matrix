...
inline quickbook_strict qbk_strict(quickbook::state &state,
                                   unsigned lower = 999u) {
...
struct error_message_action {
  ...
  explicit error_message_action(quickbook::state &state_,
...
template <typename T, typename Arg1> struct member_action1 {
  ...
  struct impl {
    ...
    explicit impl(member_action1 &a_, Arg1 value_) : a(a_), value(value_) {}
