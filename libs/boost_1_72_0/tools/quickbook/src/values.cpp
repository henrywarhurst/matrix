...
#define UNDEFINED_ERROR()                                                      \
  throw value_undefined_method(std::string(BOOST_CURRENT_FUNCTION) +           \
                               " not defined for " + this->type_name() +       \
                               " values.");
...
private:
  ...
  virtual bool equals(value_node *other) const { return this == other; }
...
private:
  ...
  virtual value_node *clone() const { return new empty_value_impl(tag_); }

  virtual bool empty() const { return true; }

  virtual bool check() const { return false; }

  virtual bool equals(value_node *other) const { return !other->check(); }
...
public:
  explicit qbk_value_impl(file_ptr const &, string_iterator begin,
                          string_iterator end, value::tag_type);
...
private:
  ...
  encoded_qbk_value_impl(file_ptr const &, string_iterator, string_iterator,
  ...
  friend quickbook::value
  quickbook::encoded_qbk_value(file_ptr const &, string_iterator,
                               string_iterator, std::string const &,
                               quickbook::value::tag_type);
};

// encoded_value_impl

encoded_value_impl::encoded_value_impl(std::string const &val,
                                       value::tag_type tag)
...
qbk_value_impl::qbk_value_impl(file_ptr const &f, string_iterator begin,
                               string_iterator end, value::tag_type tag)
...
encoded_qbk_value_impl::encoded_qbk_value_impl(file_ptr const &f,
...
value_node *encoded_qbk_value_impl::clone() const {
  return new encoded_qbk_value_impl(file_, begin_, end_, encoded_value_, tag_);
}

file_ptr encoded_qbk_value_impl::get_file() const { return file_; }

string_iterator encoded_qbk_value_impl::get_position() const { return begin_; }
...
bool encoded_qbk_value_impl::empty() const { return encoded_value_.empty(); }
...
value qbk_value(file_ptr const &f, string_iterator x, string_iterator y,
                value::tag_type t) {
...
value encoded_qbk_value(file_ptr const &f, string_iterator x, string_iterator y,
...
value_node **merge_sort(value_node **l, int recurse_limit) {
  ...
  for (int count = 0;
       count < recurse_limit && *p != &value_list_end_impl::instance; ++count) {
    p = merge(l, p, merge_sort(p, count));
  }
  return p;
}

value_node **merge(value_node **first, value_node **second,
                   value_node **third) {
  for (;;) {
    for (;;) {
      if (first == second)
        return third;
      if ((*second)->tag_ < (*first)->tag_)
        break;
      first = &(*first)->next_;
    }
...
void value_list_builder::swap(value_list_builder &other) {
  ...
  if (back_ == &other.head_)
    back_ = &head_;
  if (other.back_ == &head_)
    other.back_ = &other.head_;
}
...
void value_builder::insert(value const &item) { current.append(item.value_); }

void value_builder::extend(value const &list) {
  for (value::iterator begin = list.begin(), end = list.end(); begin != end;
       ++begin) {
    insert(*begin);
  }
}
