...
void write_xml_tree_impl(std::string &out, xml_element *node,
                         unsigned int depth);
...
void read_tag(xml_tree_builder &, string_iterator &it, string_iterator start,
              string_iterator end);
void read_close_tag(xml_tree_builder &, string_iterator &it,
                    string_iterator start, string_iterator end);
void skip_question_mark_tag(string_iterator &it, string_iterator start,
                            string_iterator end);
void skip_exclamation_mark_tag(string_iterator &it, string_iterator start,
                               string_iterator end);
quickbook::string_view read_tag_name(string_iterator &it, string_iterator start,
                                     string_iterator end);
quickbook::string_view read_attribute_value(string_iterator &it,
...
xml_tree xml_parse(quickbook::string_view source) {
  ...
  while (true) {
    ...
    if (start != it) {
      builder.add_element(
          xml_element::text_node(quickbook::string_view(start, it - start)));
    }
...
void read_tag(xml_tree_builder &builder, string_iterator &it,
              string_iterator start, string_iterator end) {
  ...
  while (true) {
    ...
    node->set_attribute(attribute_name,
                        quickbook::detail::decode_string(attribute_value));
  }
}

void read_close_tag(xml_tree_builder &builder, string_iterator &it,
                    string_iterator start, string_iterator end) {
