...
struct scoped_parser_impl
    : public cl::unary<
          ParserT, cl::parser<scoped_parser_impl<Impl, Arguments, ParserT>>> {
  typedef scoped_parser_impl<Impl, Arguments, ParserT> self_t;
  typedef cl::unary<ParserT,
                    cl::parser<scoped_parser_impl<Impl, Arguments, ParserT>>>
      base_t;

  template <typename ScannerT> struct result { typedef cl::match<> type; };

  scoped_parser_impl(Impl const &impl, Arguments const &arguments,
                     ParserT const &p)
  ...
  struct scoped {
    ...
    bool start(phoenix::tuple<Arg1, Arg2> const &x) {
      in_progress_ =
          phoenix::bind(&Impl::start)(phoenix::var(impl_), x[t0()], x[t1()])();
      return in_progress_;
    }
  ...
  typename result<ScannerT>::type parse(ScannerT const &scan) const {
    ...
    if (success) {
      ...
      if (r) {
        ...
        return scan.create_match(scan.first.base() - save.base(), cl::nil_t(),
                                 save, scan.first);
      }
    } else {
      scope.failure();
...
template <typename Impl, typename Arguments> struct scoped_parser_gen {
  ...
  scoped_parser_impl<Impl, Arguments, typename cl::as_parser<ParserT>::type>
  operator[](ParserT const &p) const {
    typedef cl::as_parser<ParserT> as_parser_t;
    typedef typename as_parser_t::type parser_t;

    return scoped_parser_impl<Impl, Arguments, parser_t>(impl_, arguments_, p);
  }
...
struct u8_codepoint_parser : public cl::parser<u8_codepoint_parser> {
  ...
  template <typename Scanner> struct result { typedef cl::match<> type; };
  ...
  typename result<Scanner>::type parse(Scanner const &scan) const {
    ...
    do {
      ++scan.first;
    } while (!scan.at_end() && ((unsigned char)*scan.first & 0xc0) == 0x80);

    return scan.create_match(scan.first.base() - save.base(), cl::nil_t(), save,
                             scan.first);
  }
};
