...
template <typename T, typename CharT> struct tst_node {
  tst_node(CharT value_)
      : reference_count(0), left(), middle(), right(), data(), value(value_) {}

  tst_node(tst_node const &other)
      : reference_count(0), left(other.left), middle(other.middle),
        right(other.right), data(other.data ? new T(*other.data) : 0),
        value(other.value) {}
...
public:
  ...
  template <typename ScannerT> search_info find(ScannerT const &scan) const {
...
typedef boost::spirit::classic::symbols<std::string, char,
