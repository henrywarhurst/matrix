...
struct docinfo_node_parser {
  ...
  enum docinfo_node_category { docinfo_general = 0, docinfo_author };

  docinfo_node_category category;
  parser_type parser;
};
typedef boost::unordered_map<quickbook::string_view, docinfo_node_parser>
    docinfo_node_pasers_type;
static docinfo_node_pasers_type docinfo_node_parsers;

void generate_chunked_documentation(chunk *, ids_type const &,
                                    html_options const &);
...
void generate_toc_subtree(html_gen &gen, chunk *page, chunk *,
...
void number_calloutlist_children(html_gen &gen, unsigned &count,
                                 xml_element *x);
...
void write_file(html_state &, std::string const &path,
                std::string const &content);
std::string get_link_from_path(html_gen &, quickbook::string_view,
...
std::string relative_path_from_url_paths(quickbook::string_view,
                                         quickbook::string_view);
...
void tag_start_with_id(html_gen &gen, quickbook::string_view name,
                       xml_element *x);
void open_tag_with_id(html_gen &gen, quickbook::string_view name,
                      xml_element *x);
void tag_self_close(html_gen &gen, quickbook::string_view name, xml_element *x);
void graphics_tag(html_gen &gen, quickbook::string_view path,
...
struct html_gen {
  ...
  explicit html_gen(html_state &state_, chunk_state &chunk_, string_view p)
      : printer(), state(state_), chunk(chunk_), path(p), in_toc(false) {}

  html_gen(html_gen const &x)
      : printer(), state(x.state), chunk(x.chunk), path(x.path), in_toc(false) {
  }
};
...
int boostbook_to_html(quickbook::string_view source,
                      html_options const &options) {
  xml_tree tree;
  try {
    ...
    ::quickbook::detail::outerr()
        << "converting boostbook at line " << p.line << " char " << p.column
        << ": " << e.message << "\n"
...
void gather_chunk_ids(chunk_state &c_state, chunk *x) {
  ...
  for (chunk *it = x->children(); it && it->inline_; it = it->next()) {
    gather_chunk_ids(c_state, it);
  }
}

string_view generate_id(chunk_state &c_state, xml_element *x, string_view name,
                        string_view base) {
  std::string result;
  ...
  for (unsigned count = 1;; ++count) {
    ...
    result += num;
    if (c_state.fragment_ids.find(result) == c_state.fragment_ids.end()) {
...
void generate_toc_subtree(html_gen &gen, chunk *page, chunk *x,
                          unsigned section_depth) {
  ...
  for (chunk *it = x->children(); it; it = it->next()) {
    ...
    if (link != gen.state.ids.end()) {
      ...
      gen.printer.html += encode_string(
          get_link_from_path(gen, link->second.path(), page->path_));
      gen.printer.html += "\">";
...
void generate_footnotes_html(html_gen &gen) {
  if (!gen.chunk.footnotes.empty()) {
    ...
    gen.printer.html += "<br/>";
    gen.printer.html += "<hr/>";
    for (std::vector<xml_element *>::iterator it = gen.chunk.footnotes.begin();
         it != gen.chunk.footnotes.end(); ++it) {
      auto footnote_id = (*it)->get_attribute("(((footnote-id)))");
      tag_start(gen.printer, "div");
  ...
  if (x->type_ == xml_element::element_node) {
    if (x->name_ == "calloutlist") {
      ...
      if (x->has_attribute("linkends")) {
        ...
        gen.chunk.callout_numbers[linkends].link_id = x->get_attribute("id");
      }
    }
  }
  ...
  for (xml_element *it = x->children(); it; it = it->next()) {
    if (it->type_ == xml_element::element_node && it->name_ == "callout") {
      if (it->has_attribute("id")) {
        gen.chunk.callout_numbers[it->get_attribute("id")].number = ++count;
      }
    }
    ...
    if (it->type_ == xml_element::element_node) {
      ...
      if (parser != docinfo_node_parsers.end()) {
        ...
        quickbook::detail::out()
            << "Unsupported docinfo tag: " << x->name_ << std::endl;
...
void generate_docinfo_html(html_gen &gen, xml_element *x) {
  ...
  if (!d.authors.empty() || !d.editors.empty() || !d.collabs.empty()) {
...
void write_file(html_state &state, std::string const &generic_path,
                std::string const &content) {
  fs::path path =
      state.options.home_path.parent_path() / generic_to_path(generic_path);
...
std::string get_link_from_path(html_gen &gen, quickbook::string_view link,
  ...
  if (boost::starts_with(link, "boost:")) {
    ...
    if (!gen.state.options.boost_root_path) {
      ...
      return relative_path_or_url(gen, gen.state.options.boost_root_path /
...
std::string relative_path_from_fs_paths(fs::path const &p,
                                        fs::path const &base) {
  return path_to_generic(path_difference(base.parent_path(), p));
}

std::string relative_path_from_url_paths(quickbook::string_view path,
                                         quickbook::string_view base) {
  ...
  auto up_count =
      std::count(base_diff_start, std::find(base_it, base.end(), '#'), '/');
...
void tag_self_close(html_gen &gen, quickbook::string_view name,
                    xml_element *x) {
  tag_start_with_id(gen, name, x);
  tag_end_self_close(gen.printer);
}
...
void graphics_tag(html_gen &gen, quickbook::string_view path,
  ...
  if (gen.state.options.graphics_path) {
    ...
    tag_attribute(
        gen.printer, "src",
        relative_path_or_url(gen, gen.state.options.graphics_path / path));
...
#define DOCINFO_NODE_RULE(tag_name, category, gen, x)                          \
  ...
  static struct BOOST_PP_CAT(register_docinfo_parser_type_, tag_name) {        \
    BOOST_PP_CAT(register_docinfo_parser_type_, tag_name)() {                  \
      docinfo_node_parser p = {docinfo_node_parser::category,                  \
  ...
  void BOOST_PP_CAT(docinfo_parser_, tag_name)(docinfo_gen & gen,              \
                                               xml_element * x)
  ...
  if (x->has_attribute("linkend")) {
    ...
    if (it == gen.state.ids.end()) {
      ...
      detail::outwarn(docbook)
          << "link not found: " << x->get_attribute("linkend") << std::endl;
    }
  }
...
NODE_RULE(inlinemediaobject, gen, x) {
  ...
  if (has_image) {
    ...
    tag_attribute(gen.printer, "src", get_link_from_path(gen, image, gen.path));
...
NODE_RULE(callout, gen, x) {
  boost::unordered_map<string_view, callout_data>::const_iterator data =
      gen.chunk.callout_numbers.end();
  ...
  graphics_tag(
      gen,
      "/callouts/" + boost::lexical_cast<std::string>(data->second.number) +
          ".png",
...
NODE_RULE(footnote, gen, x) {
  ...
  auto footnote_id = generate_id(gen.chunk, x, "(((footnote-id)))", "footnote");
  if (!x->has_attribute("id")) {
...
std::string docinfo_get_author(docinfo_gen &d, xml_element *x) {
  ...
  std::size_t const length = sizeof(name_parts) / sizeof(name_parts[0]);
  ...
  for (auto child = x->children(); child; child = child->next()) {
    if (child->type_ == xml_element::element_node) {
      if (child->name_ == "year") {
        ...
        quickbook::detail::out()
            << "Unsupported copyright tag: " << x->name_ << std::endl;
      }
    }
  }
