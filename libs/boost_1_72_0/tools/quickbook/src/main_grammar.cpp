...
struct list_stack_item {
  ...
  explicit list_stack_item(char mark_, unsigned int indent_,
                           unsigned int indent2_)
      : type(syntactic_list), indent(indent_), indent2(indent2_), mark(mark_) {}
};

struct block_types {
  enum values { none, code, list, paragraph };
};
...
struct main_grammar_local {
  ...
  cl::rule<scanner> template_phrase, top_level, indent_check,
      paragraph_separator, inside_paragraph, code, code_line, blank_line, hr,
      inline_code, skip_inline_code, template_, attribute_template,
      template_body, code_block, skip_code_block, macro, template_args,
      template_args_1_4, template_arg_1_4, template_inner_arg_1_4, brackets_1_4,
      template_args_1_5, template_arg_1_5, template_arg_1_5_content,
      template_inner_arg_1_5, brackets_1_5, template_args_1_6, template_arg_1_6,
      template_arg_1_6_content, break_, command_line_macro_identifier,
  ...
  main_grammar_local(quickbook::state &state)
      : list_stack(), list_indent(0), no_eols(true),
        context(element_info::in_top_level), mark('\0'), state_(state) {}
};
...
struct process_element_impl : scoped_action_base {
  ...
  bool start() {
    ...
    if (l.state_.source_mode_next && info_.type != element_info::maybe_block) {
  ...
  bool result(ResultT r, ScannerT const &scan) {
    if (element_context_error_) {
      ...
      return true;
    } else if (qbk_version_n < 107u && info_.type & element_info::in_phrase) {
...
struct scoped_paragraph : scoped_action_base {
  scoped_paragraph(quickbook::state &state_) : state(state_), pushed(false) {}
...
void main_grammar_local::check_indentation_impl(parse_iterator first_,
...
void main_grammar_local::check_code_block_impl(parse_iterator first,
                                               parse_iterator last) {
  unsigned int new_indent = indent_length(first.base(), last.base());

  block_type = (new_indent > list_stack.top().indent2) ? block_types::code
                                                       : block_types::none;
}

void main_grammar_local::plain_block(string_iterator first,
                                     string_iterator last) {
  if (qbk_version_n >= 106u) {
    ...
    if (list_stack.top().type != list_stack_item::nested_block && last != first)
      block_type = block_types::code;
    else
      block_type = block_types::paragraph;
  }
...
void main_grammar_local::list_block(string_iterator first,
  ...
  if (list_stack.top().type == list_stack_item::top_level && new_indent > 0) {
    block_type = block_types::code;
    return;
  }
