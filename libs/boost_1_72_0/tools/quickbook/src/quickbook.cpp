...
static void set_macros(quickbook::state &state) {
  QUICKBOOK_FOR(quickbook::string_view val, preset_defines) {
    ...
    if (!info.full) {
      detail::outerr() << "Error parsing command line definition: '" << val
                       << "'" << std::endl;
      ++state.error_count;
    }
  }
...
//
///////////////////////////////////////////////////////////////////////////
void parse_file(quickbook::state &state, value include_doc_id,
                bool nested_file) {
...
struct parse_document_options {
  ...
  parse_document_options()
      : format(boostbook), style(output_file), output_path(), indent(-1),
        linewidth(-1), pretty_print(true), strict_mode(false),
...
static int parse_document(fs::path const &filein_,
  ...
  try {
    ...
    if (state.error_count == 0) {
      ...
      if (state.error_count) {
        detail::outerr() << "Error count: " << state.error_count << ".\n";
      }
    }
  ...
  if (options_.style) {
    ...
    if (options_.pretty_print) {
      try {
        stage2 = post_process(stage2, options_.indent, options_.linewidth);
      } catch (quickbook::post_process_failure &) {
        ::quickbook::detail::outerr() << "Post Processing Failed." << std::endl;
    ...
    if (options_.format == parse_document_options::html) {
      ...
      if (fileout.fail()) {
        ::quickbook::detail::outerr() << "Error opening output file "
      ...
      if (fileout.fail()) {
        ::quickbook::detail::outerr() << "Error writing to output file "
                                      << options_.output_path << std::endl;
...
#define PO_VALUE po::value
    ...
    store(wcommand_line_parser(wide_argc, wide_argv)
              .options(all)
              .positional(p)
              .run(),
          vm);
    ...
    if (vm.count("output-format")) {
      ...
      if (format == "html") {
        ...
        options.style = quickbook::parse_document_options::output_chunked;
    ...
    if (vm.count("define")) {
      boost::transform(vm["define"].as<std::vector<command_line_string>>(),
    ...
    if (vm.count("input-file")) {
      ...
      if (vm.count("output-deps-format")) {
        std::string format_flags = quickbook::detail::command_line_to_utf8(
        ...
        QUICKBOOK_FOR(std::string const &flag, flag_names) {
          if (flag == "checked") {
            ...
            quickbook::detail::outerr()
                << "Unknown dependency format flag: " << flag << std::endl;

            ++error_count;
          }
        }

        options.deps_out_flags = quickbook::dependency_tracker::flags(flags);
      }
      ...
      if (vm.count("css-path")) {
        options.html_ops.css_path = vm["css-path"].as<command_line_string>();
      ...
      if (vm.count("output-file")) {
        ...
        case quickbook::parse_document_options::output_file: {
          ...
          if (!parent.empty() && !fs::is_directory(parent)) {
            quickbook::detail::outerr()
                << "parent directory not found for output file" << std::endl;
            ++error_count;
          }
          break;
        }
      ...
      if (!vm.count("output-file") && !vm.count("output-dir")) {
        if (!output_specified && alt_output_specified) {
          options.style = quickbook::parse_document_options::output_none;
        } else {
          fs::path path = filein;
          switch (options.style) {
          case quickbook::parse_document_options::output_chunked:
      ...
      if (!error_count) {
        ...
        case parse_document_options::output_file:
          quickbook::detail::out()
              << "Generating output file: " << options.output_path << std::endl;
          break;
        case parse_document_options::output_chunked:
          quickbook::detail::out()
              << "Generating output path: " << options.output_path << std::endl;
          break;
      ...
      if (expect_errors) {
        if (!error_count)
          quickbook::detail::outerr()
              << "No errors detected for --expect-errors." << std::endl;
        return !error_count;
      } else {
        return error_count;
      }
