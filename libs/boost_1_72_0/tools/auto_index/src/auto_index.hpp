...
struct index_entry {
  ...
  std::string
      category; // The category of this entry (function, class, macro etc).
  ...
  index_entry(const std::string &k) : key(k), sort_key(k), preferred(false) {
    boost::to_upper(sort_key);
  }
  index_entry(const std::string &k, const std::string &i)
      : key(k), sort_key(k), id(i), preferred(false) {
    boost::to_upper(sort_key);
  }
  index_entry(const std::string &k, const std::string &i, const std::string &c)
      : key(k), sort_key(k), id(i), category(c), preferred(false) {
    boost::to_upper(sort_key);
  }
};
...
struct id_rewrite_rule {
  bool base_on_id; // rewrite the title if "id" matches the section id,
                   // otherwise rewrite if title matches "id".
  boost::regex id; // regex for the id or title to match
  std::string
      new_name; // either literal string or format string for the new name.
...
inline void
add_file_scanner(const std::string &type, const std::string &scanner,
                 const std::string &format, const std::string &term_formatter,
                 const std::string &id_filter, const std::string &file_filter) {
  file_scanner s;
  s.type = type;
  s.scanner = scanner;
