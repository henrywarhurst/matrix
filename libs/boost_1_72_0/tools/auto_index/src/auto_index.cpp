...
int help() {
  std::cout
      << "Please refer to the documentation for the correct command line syntax"
      << std::endl;
  return 1;
}
...
//
const std::string *find_attr(boost::tiny_xml::element_ptr node,
                             const char *name) {
  for (boost::tiny_xml::attribute_list::const_iterator i =
...
const std::string *get_current_block_id(node_id const *id) {
  ...
  if (!id->id)
    BOOST_THROW_EXCEPTION(std::runtime_error(
        "Current XML block has no enclosing ID: XML is not valid Boostbook?"));
  return id->id;
}
//
...
std::string rewrite_title(const std::string &title, const std::string &id) {
  for (std::list<id_rewrite_rule>::const_iterator i = id_rewrite_list.begin();
       i != id_rewrite_list.end(); ++i) {
    if (i->base_on_id) {
...
// https://sourceforge.net/tracker/?func=detail&aid=3325153&group_id=21935&atid=373747
//
bool can_contain_title(const char *name) {
  static const boost::array<const char *, 103> names = {{"abstract",
                                                         "appendix",
                                                         "appendixinfo",
                                                         "article",
                                                         "articleinfo",
...
bool should_flatten_node(const char *name) {
  ...
  // http://www.docbook.org/tdg/en/html/section.html. In other words basically
  // anything at the level of a paragraph/table/listing etc.
  //
  static const boost::array<const char *, 57> names = {{
      ...
      "authorblurb",
      "epigraph"
      /*"biblioentry", "bibliomixed", "callout", "glossentry", "listitem",
      "seg", "seglistitem", "member", "term", */
  }};
...
std::string unescape_xml(const std::string &s) {
  ...
  return regex_replace(s, e, "(?1\")(?2&)(?3\')(?4<)(?5>)",
...
bool is_section(const std::string &name) {
  static const boost::array<const char *, 19> data = {
      {"dedication", "toc", "lot", "glossary", "bibliography", "preface",
       "chapter", "reference", "part", "article", "appendix", "index",
       "setindex", "colophon", "sect1", "refentry", "simplesect", "section",
       "partintro"}};
...
bool is_block(const std::string &name) {
  static const boost::array<const char *, 58> data = {{"calloutlist",
                                                       "glosslist",
                                                       "bibliolist",
                                                       "itemizedlist",
                                                       "orderedlist",
...
//
void process_node(boost::tiny_xml::element_ptr node, node_id *prev,
                  title_info *pt, bool seen);
bool recurse_through_children(boost::tiny_xml::element_ptr node, node_id *id,
                              title_info *pt, bool seen) {
  try {
    for (boost::tiny_xml::element_list::const_iterator i =
             node->elements.begin();
         i != node->elements.end(); ++i) {
...
void process_node(boost::tiny_xml::element_ptr node, node_id *prev,
  ...
  if (node->name.size() && node->name[0] == '?') {
    ...
    if (verbose)
      std::cout << "Indexing refentry: " << title.prev->prev->title
                << std::endl;
  }
  if (node->name == "anchor") {
    if (node->parent.lock()->name == "title") {
      // We have a title with a nested anchor ID, change the ID of our parents
      // parent to match:
    ...
    std::cerr << "WARNING: <see> in XML source will be ignored for the index "
                 "generation"
              << std::endl;
  } else if ((node->name == "seealso") && internal_indexes) {
    std::cerr << "WARNING: <seealso> in XML source will be ignored for the "
                 "index generation"
              << std::endl;
  }
  ...
  if (flatten) {
    ...
    //
    // Recurse through children here if we're going to flatten the text, that
    // way we see any processing instructions first:
    //
  ...
  // and the content is not whitespace alone, and we haven't already searched
  // this text in one of our parent nodes that got flattened.
  //
    ...
    for (std::set<index_info>::const_iterator i = index_terms.begin();
      ...
      if (regex_search(*ptext, i->search_text)) {
        ...
        if (((no_duplicates == false) ||
             (0 == found_terms.count(item_index))) &&
            (i->search_id.empty() || regex_match(*pid, i->search_id))) {
          // We have something to index!
          found_terms.insert(item_index);

          if (!debug.empty() &&
              (regex_match(i->term, debug) || regex_match(rtitle, debug) ||
               regex_match(simple_title, debug))) {
            std::cout << "Debug term found, in block with ID: " << *pid
                      << std::endl;
            std::cout << "Current section title is: " << rtitle << std::endl;
            std::cout << "The main index entry will be : " << simple_title
                      << std::endl;
            std::cout << "The indexed term is: " << i->term << std::endl;
            std::cout << "The search regex is: " << i->search_text << std::endl;
            std::cout << "The section constraint is: " << i->search_id
                      << std::endl;
            std::cout << "The index type for this entry is: " << i->category
                      << std::endl;
          }
          ...
          if (use_section_names && (simple_title != i->term)) {
            ...
            if (internal_indexes == false) {
              ...
              prim->name = "primary";
              prim->elements.push_front(
                  boost::tiny_xml::element_ptr(new boost::tiny_xml::element()));
              ...
              try {
                ...
                std::cerr << "Unable to find location to insert <indexterm>"
                          << std::endl;
              }
            }
          ...
          if (internal_indexes == false) {
            ...
            p2->name = "indexterm";
            if (i->category.size()) {
              p2->attributes.push_back(
                  boost::tiny_xml::attribute("type", i->category));
            }
            boost::tiny_xml::element_ptr prim2(new boost::tiny_xml::element());
            prim2->name = "primary";
            prim2->elements.push_front(
                boost::tiny_xml::element_ptr(new boost::tiny_xml::element()));
            ...
            try {
              ...
              boost::tiny_xml::element_ptr parent(node->parent);
              while (!can_contain_indexterm(parent->name.c_str())) {
                // If the search text was found in a title then make it a
                // preferred term:
...
int main(int argc, char *argv[]) {
  try {
    ...
    desc.add_options()("help", "Print help message")(
        ...
        "scan", po::value<std::string>(),
        "Scan the specified file for terms to try and index.")(
        "script", po::value<std::string>(),
        "Specifies the script file to use.")(
        "no-duplicates",
        "Prevents duplicate index entries within the same section.")(
        "no-section-names",
        "Suppresses use of section names as index entries.")(
        "internal-index",
        "Causes AutoIndex to generate the index itself, rather than relying on "
        "the XSL stylesheets.")("verbose", "Turns on verbose mode.")(
        "prefix", po::value<std::string>(),
        "Sets the prefix to be prepended to all file names and paths in the "
        "script file.")("index-type", po::value<std::string>(),
                        "Sets the XML container type to use the index.");
    ...
    if (vm.count("scan")) {
      ...
      if (!exists(boost::filesystem::path(f)))
        throw std::runtime_error(
            "Error the file requested for scanning does not exist: " + f);
      scan_file(f);
    }
    if (vm.count("script")) {
    ...
    std::cout << index_entries.size() << " Index entries were created."
              << std::endl;
