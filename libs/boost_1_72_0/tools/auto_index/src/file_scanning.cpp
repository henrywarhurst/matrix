...
void install_default_scanners() {
  ...
  if (file_scanner_set.find(s) == file_scanner_set.end()) {
    add_file_scanner(
        ...
        "(?:class|struct)[^;{]+\\\\<\\5\\\\>[^;{]+\\\\{", // Format string to
                                                          // create indexing
                                                          // regex.
  ...
  if (file_scanner_set.find(s) == file_scanner_set.end()) {
    add_file_scanner(
        ...
        "typedef[^;]+\\\\<\\1\\\\>\\\\s*;", // Format string to create indexing
                                            // regex.
  ...
  s.type = "macro_name";
  if (file_scanner_set.find(s) == file_scanner_set.end()) {
    add_file_scanner("macro_name",                 // Index type
  ...
  if (file_scanner_set.find(s) == file_scanner_set.end()) {
    add_file_scanner(
        ...
        "\\w++(?:\\s*+<[^>]++>)?[\\s&*]+?(\\w+)\\s*(?:BOOST_[[:upper:]_]+\\s*)?"
        "\\([^;{}]*\\)\\s*[;{]", // scanner regex
        "\\\\<\\\\w+\\\\>(?:\\\\s+<[^>]*>)*[\\\\s&*]+\\\\<\\1\\\\>\\\\s*\\\\([^"
        ";{]*\\\\)", // Format string to create indexing regex.
        "\\1",       // Format string to create index term.
        "",          // Filter regex for section id's.
        ""           // Filter regex for filenames.
    );
...
void scan_file(const std::string &file) {
  ...
  for (file_scanner_set_type::iterator pscan = file_scanner_set.begin();
       pscan != file_scanner_set.end(); ++pscan) {
    bool need_debug = false;
    if (!debug.empty() && regex_match(pscan->type, ::debug)) {
      need_debug = true;
      std::cout << "Processing scanner " << pscan->type << " on file " << file
                << std::endl;
      std::cout << "Scanner regex:" << pscan->scanner << std::endl;
      std::cout << "Scanner formatter (search regex):" << pscan->format_string
                << std::endl;
      std::cout << "Scanner formatter (index term):" << pscan->term_formatter
                << std::endl;
      std::cout << "Scanner file name filter:" << pscan->file_name_filter
                << std::endl;
      std::cout << "Scanner section id filter:" << pscan->section_filter
                << std::endl;
    }
    if (!pscan->file_name_filter.empty()) {
      if (!regex_match(file, pscan->file_name_filter)) {
        if (need_debug) {
          std::cout << "File failed to match file name filter, this file will "
                       "be skipped..."
                    << std::endl;
        }
    ...
    if (verbose && !need_debug)
      std::cout << "Scanning for type \"" << (*pscan).type << "\" ... "
                << std::endl;
    ...
    while (i != j) {
      try {
        ...
        std::pair<std::set<index_info>::iterator, bool> pos =
            index_terms.insert(info);
        if (pos.second) {
          if (verbose || need_debug)
            std::cout << "Indexing " << info.term << " as type "
          ...
          if (pos.first->search_text != info.search_text) {
            ...
            const_cast<boost::regex &>(pos.first->search_text) =
                "(?:" + pos.first->search_text.str() +
                ")|(?:" + info.search_text.str() + ")";
          }
        ...
        std::cerr
            ...
            << i->format(pscan->term_formatter) << "\" In file " << file
            << std::endl;
...
void process_script(const std::string &script) {
  static const boost::regex comment_parser("\\s*(?:#.*)?$");
  ...
  static const boost::regex define_scanner_parser(
      ...
      "([^\"[:space:]]+|\"(?:[^\"\\\\]|\\\\.)+\")" // format string for name,
                                                   // index 4
      "(?:"
      "\\s+([^\"[:space:]]+|\"(?:[^\"\\\\]|\\\\.)+\")" // id-filter, index 5
      "(?:"
      "\\s+([^\"[:space:]]+|\"(?:[^\"\\\\]|\\\\.)+\")" // filename-filter, index
                                                       // 6
      ")?"
      ")?"
      "\\s*");
  ...
  if (is.bad() || !exists(boost::filesystem::path(script))) {
    throw std::runtime_error(std::string("Could not open script file: ") +
                             script);
  }
  while (std::getline(is, line).good()) {
    if (regex_match(line, what, comment_parser)) {
      ...
      add_file_scanner(unquote(what.str(1)), unquote(what.str(2)),
                       unquote(what.str(3)), unquote(what.str(4)),
                       unquote(what.str(5)), unquote(what.str(6)));
      ...
      if (!exists(boost::filesystem::path(d)))
        throw std::runtime_error(
            "Error the path requested for scanning does not exist: " + d);
      scan_dir(d, m, r);
      ...
      try {
        ...
        if (s.size())
          info.search_text =
              boost::regex(s, boost::regex::icase | boost::regex::perl);
        else
          info.search_text =
              boost::regex("\\<" + what.str(1) + "\\>",
