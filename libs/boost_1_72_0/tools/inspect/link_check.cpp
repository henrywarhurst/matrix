...
boost::regex
    html_url_regex("<([^\\s<>]*)\\s*[^<>]*\\s+(?:HREF|SRC)" // HREF or SRC
                   "\\s*=\\s*(['\"])\\s*(.*?)\\s*\\2"
                   "|<!--.*?-->",
                   boost::regbase::normal | boost::regbase::icase);
boost::regex css_url_regex("(\\@import\\s*[\"']|url\\s*\\(\\s*[\"']?)([^\"')]*)"
                           "|/\\*.*?\\*/",
                           boost::regbase::normal | boost::regbase::icase);
...
bool is_css(const path &p) { return p.extension() == ".css"; }
    ...
    const string &contents) // contents of file to be inspected
{
  ...
  if (!is_css(full_path)) {
    ...
    while (boost::regex_search(a_start, a_end, a_what, html_bookmark_regex,
                               a_flags)) {
      ...
      if (a_what[4].matched) {
        ...
        if (tag != "meta" && !name_following_id) {
          ...
          if (!result.second) {
            ...
            error(library_name, full_path, "Duplicate bookmark: " + bookmark,
                  ln);
          }
        }
      }
  ...
  if (!is_css(full_path)) {
    while (boost::regex_search(start, end, what, html_url_regex, flags)) {
      ...
      if (what[3].matched) {
        ...
        do_url(string(what[3].first, what[3].second), library_name, full_path,
               no_link_errors, type == "a" || type == "link", contents.begin(),
               what[3].first);
      }
  ...
  while (boost::regex_search(start, end, what, css_url_regex, flags)) {
    ...
    if (what[2].matched) {
      do_url(string(what[2].first, what[2].second), library_name, full_path,
             no_link_errors, false, contents.begin(), what[3].first);
    }
...
// precondition: source_path.is_complete()
{
  ...
  if (scheme_matched) {
    ...
    } else if (scheme == "mailto" || scheme == "ftp" || scheme == "news" ||
               scheme == "javascript") {
  ...
  if (fragment_matched) {
    if (is_css(source_path)) {
      if (!no_link_errors) {
        ++m_invalid_errors;
        int ln = std::count(contents_begin, url_start, '\n') + 1;
        error(library_name, source_path, "Fragment link in CSS: " + decoded_url,
              ln);
      }
    } else {
      if (!no_link_errors && fragment.find('#') != string::npos) {
        ++m_bookmark_errors;
        int ln = std::count(contents_begin, url_start, '\n') + 1;
        error(library_name, source_path, "Invalid bookmark: " + decoded_url,
              ln);
        ...
        ++m_broken_errors;
        int ln = std::count(contents_begin, url_start, '\n') + 1;
        error(library_name, source_path, "Unknown bookmark: " + decoded_url,
              ln);
      }
    }
  ...
  if (!no_link_errors &&
    ...
    ++m_invalid_errors;
    int ln = std::count(contents_begin, url_start, '\n') + 1;
    error(library_name, source_path, "Invalid character in URL: " + decoded_url,
          ln);
  }
  ...
  path target_path;
  try {
    target_path = source_path.branch_path() /= path(decoded_path);
...
void link_check::close() {
  for (m_path_map::const_iterator itr = m_paths.begin(); itr != m_paths.end();
       ++itr) {
