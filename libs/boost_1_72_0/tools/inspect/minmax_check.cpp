...
minmax_check::minmax_check() : m_errors(0) {
  // C/C++ source code...
    ...
    const string &contents) // contents of file to be inspected
{
  if (contents.find("boostinspect:"
                    "nominmax") != string::npos)
    return;

  boost::sregex_iterator cur(contents.begin(), contents.end(), minmax_regex),
      end;
  ...
  for (; cur != end; ++cur /*, ++m_errors*/) {
    ...
    if (!(*cur)[3].matched) {
      ...
      error(library_name, full_path,
            string(name()) + " violation of Boost min/max guidelines on line " +
