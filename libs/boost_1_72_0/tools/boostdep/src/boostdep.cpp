...
static void build_header_map() { scan_submodules("libs"); }

static void
scan_header_dependencies(std::string const &header, std::istream &is,
...
static void
scan_module_path(fs::path const &dir, bool remove_prefix,
                 std::map<std::string, std::set<std::string>> &deps,
                 std::map<std::string, std::set<std::string>> &from) {
  size_t n = dir.generic_string().size();
...
static void scan_module_dependencies(std::string const &module,
                                     module_primary_actions &actions,
                                     bool track_sources, bool track_tests,
                                     bool include_self) {
  ...
  for (std::map<std::string, std::set<std::string>>::iterator i = deps.begin();
       i != deps.end(); ++i) {
    ...
    for (std::set<std::string>::iterator j = i->second.begin();
      ...
      for (std::set<std::string>::const_iterator k = f.begin(); k != f.end();
           ++k) {
...
struct build_mdmap_actions : public module_primary_actions {
  ...
  void heading(std::string const &module) { module_ = module; }
  ...
  void header_start(std::string const &header) { header_ = header; }
...
static void build_module_dependency_map(bool track_sources, bool track_tests) {
  for (std::set<std::string>::iterator i = s_modules.begin();
       i != s_modules.end(); ++i) {
...
static void output_module_primary_report(std::string const &module,
                                         ...
                                         bool track_sources, bool track_tests) {
  try {
    scan_module_dependencies(module, actions, track_sources, track_tests,
                             false);
  } catch (fs::filesystem_error const &x) {
    std::cout << x.what() << std::endl;
  }
...
static void output_module_secondary_report(std::string const &module,
                                           ...
                                           module_secondary_actions &actions) {
  actions.heading(module);
  ...
  for (;;) {
    ...
    for (std::set<std::string>::iterator i = deps.begin(); i != deps.end();
         ++i) {
      ...
      for (std::set<std::string>::iterator j = deps3.begin(); j != deps3.end();
           ++j) {
...
struct header_inclusion_actions {
  virtual void heading(std::string const &header,
                       std::string const &module) = 0;
...
static void output_header_inclusion_report(std::string const &header,
                                           header_inclusion_actions &actions) {
  ...
  for (std::map<std::string, std::set<std::string>>::iterator i = from2.begin();
       i != from2.end(); ++i) {
    actions.module_start(i->first);
...
struct module_primary_html_actions : public module_primary_actions {
  void heading(std::string const &module) {
    std::cout
        << "\n\n<h1 id=\"primary-dependencies\">Primary dependencies for <em>"
        << module << "</em></h1>\n";
  }

  void module_start(std::string const &module) {
    std::cout << "  <h2 id=\"" << module << "\"><a href=\"" << module
              << ".html\"><em>" << module << "</em></a></h2>\n";
  }
...
struct module_secondary_html_actions : public module_secondary_actions {
  ...
  void heading(std::string const &module) {
    std::cout << "\n\n<h1 id=\"secondary-dependencies\">Secondary dependencies "
                 "for <em>"
              << module << "</em></h1>\n";
  }

  void module_start(std::string const &module) {
    std::cout << "  <h2><a href=\"" << module << ".html\"><em>" << module
              << "</em></a></h2><ul>\n";
    m2_ = module;
  }
...
struct header_inclusion_html_actions : public header_inclusion_actions {
  void heading(std::string const &header, std::string const &module) {
    std::cout << "<h1>Inclusion report for <code>&lt;" << header
              << "&gt;</code> (in module <em>" << module << "</em>)</h1>\n";
  }

  void module_start(std::string const &module) {
    std::cout << "  <h2>From <a href=\"" << module << ".html\"><em>" << module
              << "</em></a></h2><ul>\n";
  }
...
static void output_module_reverse_report(std::string const &module,
                                         module_reverse_actions &actions) {
  actions.heading(module);

  std::set<std::string> const from = s_reverse_deps[module];

  for (std::set<std::string>::const_iterator i = from.begin(); i != from.end();
       ++i) {
    actions.module_start(*i);

    for (std::map<std::string, std::set<std::string>>::iterator j =
             s_header_deps.begin();
         j != s_header_deps.end(); ++j) {
      if (s_header_map[j->first] == module) {
        bool header_started = false;

        for (std::set<std::string>::iterator k = j->second.begin();
             k != j->second.end(); ++k) {
          if (s_header_map[*k] == *i) {
            if (!header_started) {
...
struct module_reverse_html_actions : public module_reverse_actions {
  void heading(std::string const &module) {
    std::cout
        << "\n\n<h1 id=\"reverse-dependencies\">Reverse dependencies for <em>"
        << module << "</em></h1>\n";
  }

  void module_start(std::string const &module) {
    std::cout << "  <h2 id=\"reverse-" << module << "\"><a href=\"" << module
              << ".html\"><em>" << module << "</em></a></h2>\n";
  }
...
static void output_module_level_report(module_level_actions &actions) {
  ...
  for (std::map<std::string, int>::iterator i = level_map.begin();
       i != level_map.end(); ++i) {
  ...
  for (std::map<int, std::set<std::string>>::iterator i =
           reverse_level_map.begin();
       i != reverse_level_map.end(); ++i) {
    ...
    for (std::set<std::string>::iterator j = i->second.begin();
      ...
      for (std::set<std::string>::iterator k = mdeps.begin(); k != mdeps.end();
           ++k) {
...
struct module_level_txt_actions : public module_level_actions {
  ...
  void begin() { std::cout << "Module Levels:\n\n"; }
  ...
  void level_end(int /*level*/) { std::cout << "\n"; }
...
struct module_level_html_actions : public module_level_actions {
  ...
  void module_start(std::string const &module) {
    std::cout << "    <h3 id='" << module << "'><a href=\"" << module
              << ".html\">" << module << "</a></h3><p class='primary-list'>";
  }

  void module_end(std::string const & /*module*/) { std::cout << "</p>\n"; }
...
static void output_module_overview_report(module_overview_actions &actions) {
  ...
  for (std::set<std::string>::iterator i = s_modules.begin();
    ...
    for (std::set<std::string>::const_iterator j = mdeps.begin();
         j != mdeps.end(); ++j) {
      actions.module2(*j);
    }
...
struct module_overview_txt_actions : public module_overview_actions {
  ...
  void begin() { std::cout << "Module Overview:\n\n"; }
...
struct module_overview_html_actions : public module_overview_actions {
  ...
  void module2(std::string const &module) { std::cout << " " << module; }
};
...
static void output_module_weight_report(module_weight_actions &actions) {
  ...
  struct build_secondary_deps : public module_secondary_actions {
    ...
    build_secondary_deps(std::map<std::string, std::set<std::string>> *pm)
        : pm_(pm) {}
  ...
  for (std::set<std::string>::const_iterator i = s_modules.begin();
       i != s_modules.end(); ++i) {
  ...
  for (std::map<int, std::set<std::string>>::const_iterator i =
           modules_by_weight.begin();
       i != modules_by_weight.end(); ++i) {
    ...
    for (std::set<std::string>::const_iterator j = i->second.begin();
      ...
      if (!s_module_deps[*j].empty()) {
        ...
        for (std::set<std::string>::const_iterator k =
                 s_module_deps[*j].begin();
...
struct module_weight_txt_actions : public module_weight_actions {
  void begin() { std::cout << "Module Weights:\n\n"; }

  void end() {}

  void weight_start(int weight) { std::cout << "Weight " << weight << ":\n"; }
...
struct module_weight_html_actions : public module_weight_actions {
  ...
  void module_primary_start() { std::cout << "<p class='primary-list'>"; }
  ...
  void module_primary_end() { std::cout << "</p>"; }

  void module_secondary_start() { std::cout << "<p class='secondary-list'>"; }

  void module_secondary(std::string const &module, int /*weight*/) {
    std::cout << " " << module;
  }

  void module_secondary_end() { std::cout << "</p>"; }
};
...
static void add_module_headers(fs::path const &dir,
...
static void output_module_subset_report_(std::string const &module,
                                         std::set<std::string> const &headers,
                                         module_subset_actions &actions) {
  ...
  for (std::set<std::string>::const_iterator i = headers.begin();
    ...
    for (std::set<std::string>::const_iterator j = s.begin(); j != s.end();
         ++j) {
  ...
  for (;;) {
    ...
    for (std::map<std::string, std::set<std::string>>::iterator i =
      ...
      for (std::set<std::string>::const_iterator j = s2.begin(); j != s2.end();
           ++j) {
        std::set<std::string> const &s = s_header_includes[*j];

        for (std::set<std::string>::const_iterator k = s.begin(); k != s.end();
             ++k) {
          if (s2.count(*k) == 0) {
            s2.insert(*k);
  ...
  for (std::map<std::string,
    ...
    for (std::map<std::string, std::vector<std::string>>::const_iterator j =
             i->second.begin();
         j != i->second.end() && k < 4; ++j, ++k) {
...
struct module_subset_html_actions : public module_subset_actions {
  ...
  void module_start(std::string const &module) {
    std::cout << "  <h2 id=\"subset-" << module << "\"><a href=\"" << module
              << ".html\"><em>" << module << "</em></a></h2><ul>\n";
  }
...
static void list_exceptions() {
  ...
  for (std::map<std::string, std::set<std::string>>::const_iterator i =
           s_module_headers.begin();
       i != s_module_headers.end(); ++i) {
...
struct module_test_secondary_actions : public module_secondary_actions {
  ...
  void module_start(std::string const &module) { m2_ = module; }
...
static void output_module_cmake_report(std::string module) {
  ...
  if (!fs::exists(module_source_path(module))) {
    for (std::set<std::string>::const_iterator i = a1.set_.begin();
         i != a1.set_.end(); ++i) {
      std::cout << "boost_declare_dependency(INTERFACE "
                << module_cmake_name(*i) << ")\n";
    }
  } else {
    ...
    for (std::set<std::string>::const_iterator i = a2.set_.begin();
         i != a2.set_.end(); ++i) {
...
struct missing_header_actions : public module_primary_actions {
  ...
  void module_start(std::string const &module) { module2_ = module; }
...
static void output_pkgconfig(std::string const &module,
                             std::string const &version, int argc,
                             char const *argv[]) {
...
static void output_directory_subset_report(std::string const &module,
                                           std::set<std::string> const &headers,
                                           bool html) {
  for (std::set<std::string>::const_iterator i = headers.begin();
       i != headers.end(); ++i) {
    ...
    for (std::map<std::string, std::set<std::string>>::const_iterator j =
             from.begin();
         j != from.end(); ++j) {
      for (std::set<std::string>::const_iterator k = j->second.begin();
           k != j->second.end(); ++k) {
...
struct list_buildable_dependencies_actions : public module_overview_actions {
  ...
  void module_end(std::string const &module) {
    if (buildable_.count(module)) {
      ...
      for (std::set<std::string>::iterator i = deps_.begin(); i != deps_.end();
           ++i) {
        std::cout << " " << *i;
      }
...
public:
  ...
  ~save_cout_rdbuf() { std::cout.rdbuf(sb_); }
};
...
int main(int argc, char const *argv[]) {
  if (argc < 2) {
    std::cout <<
        ...
        "    boostdep --pkgconfig <module> <version> [<var>=<value>] "
        "[<var>=<value>]...\n"
        ...
        "               [--html-stylesheet <stylesheet>] [--html-prefix "
        "<prefix>]\n"
        "               [--html]\n";
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (option == "--boost-root") {
      ...
      if (i + 1 < argc) {
        enable_secondary(secondary, track_sources, track_tests);
        output_module_subset_report(argv[++i], track_sources, track_tests,
                                    html);
      }
    } else if (option == "--test") {
      if (i + 1 < argc) {
