...
void bcp_implementation::copy_path(const fs::path &p) {
  ...
  if ((p.leaf() == "Jamroot") && m_namespace_name.size()) {
    ...
    std::copy(std::istreambuf_iterator<char>(is),
              std::istreambuf_iterator<char>(), std::back_inserter(v1));
    ...
    regex_replace(std::back_inserter(v2), v1.begin(), v1.end(), libname_matcher,
                  m_namespace_name + "_");
    std::swap(v1, v2);
    v2.clear();
    ...
    if (libname_matcher.empty()) {
      ...
      for (std::set<std::string>::const_iterator i = ++m_lib_names.begin();
           i != m_lib_names.end(); ++i) {
        re += "|" + *i;
      }
      re += "\\>";
    ...
    regex_replace(std::back_inserter(v2), v1.begin(), v1.end(), libname_matcher,
                  get_new_library_name(m_namespace_name));
    std::swap(v1, v2);
    v2.clear();
    ...
    static const boost::regex namespace_matcher(
        ...
        "|"
        "(namespace\\s+\\w+\\s*=\\s*(?:::\\s*)?)boost(_\\w+)?(?:(\\s*::\\s*)"
        "phoenix)?"
        "|"
        "(namespace\\s+\\w+\\s*=\\s*(?:::\\s*)?(?:\\w+\\s*::\\s*)?)(adstl|"
        "phoenix|rapidxml)\\>"
        "|"
        ...
        "|"
        "()\\<(lw_thread_routine|at_thread_exit|on_process_enter|on_process_"
        "exit|on_thread_enter|on_thread_exit|tss_cleanup_implemented)\\>"
        "|"
    ...
    regex_replace(
        std::back_inserter(v2), v1.begin(), v1.end(), namespace_matcher,
        "$1" + m_namespace_name + "$2(?3$3" + m_namespace_name + "phoenix?4$4)",
    ...
    if (m_namespace_alias) {
      static const boost::regex namespace_alias(
          ...
          */
          ...
          ")"
          "(adstl|phoenix|rapidxml)?(\\s*\\{)");
      regex_replace(std::back_inserter(v2), v1.begin(), v1.end(),
                    namespace_alias,
                    "$1 $3$4 {} $1 (?4$4:boost) = $3$4; $1$2$3$4$5",
                    boost::regex_constants::format_all);
      std::swap(v1, v2);
      v2.clear();
    }
    ...
    boost::filesystem::ofstream os((m_dest_path / p),
