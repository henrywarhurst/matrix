...
//
// This file contains the code required to work out whether the source/header
// file being scanned is actually dependent upon some library's source code or
// not.
//
...
static void init_library_scanner(const fs::path &p, bool cvs_mode,
  ...
  */
  ...
  while (i != j) {
    ...
    if (bcp_implementation::is_source_file(*i)) {
      ...
      fileview view(*i);
      boost::regex_iterator<const char *> a(view.begin(), view.end(),
                                            function_scanner);
  ...
  if (recurse == false) {
    ...
    const char *e3 = ")\\s*\\([^;()]*\\)\\s*(?:BOOST[_A-Z]+\\s*)?;)";

    std::string class_name_list;
    std::set<std::string>::const_iterator i = class_names[libname].begin(),
                                          j = class_names[libname].end();
    if (i != j) {
      class_name_list = *i;
      ++i;
      while (i != j) {
    ...
    scanner[libname] =
        boost::regex(e1 + class_name_list + e2 + function_name_list + e3);
  }
}
      ...
      if (regex_search(view.begin(), view.end(), what, scanner[libname])) {
        ...
        if (fs::exists(m_boost_path / "libs" / libname / "build")) {
          if (!m_dependencies.count(fs::path("libs") / libname / "build")) {
            m_dependencies[fs::path("libs") / libname / "build"] =
                p; // set up dependency tree
        ...
        if (fs::exists(m_boost_path / "libs" / libname / "config")) {
          if (!m_dependencies.count(fs::path("libs") / libname / "config")) {
            m_dependencies[fs::path("libs") / libname / "config"] =
                p; // set up dependency tree
