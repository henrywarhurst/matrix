...
void show_usage() {
  std::cout
      << "Usage:\n"
         ...
         "   --unix-lines     make sure that all copied files use Unix style "
         "line endings\n"
         "   --namespace=name rename the boost namespace to name (also changes "
         "library names).\n"
         "   --namespace-alias Makes namespace boost an alias of the namespace "
         "set with --namespace.\n"
         "\n"
         "module-list:         a list of boost files or library names to copy\n"
         "html-file:           the name of a html file to which the report "
         "will be written\n"
         "output-path:         the path to which files will be copied\n";
}

bool filesystem_name_check(const std::string &) { return true; }
...
int cpp_main(int argc, char *argv[]) {
  ...
  //
  if (argc < 2) {
    std::cout << "Error: insufficient arguments, don't know what to do."
              << std::endl;
    show_usage();
    return 1;
  }
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (0 == std::strcmp("-v", argv[i]) ||
      ...
      std::cout << "bcp " << (BOOST_VERSION / 100000) << "."
