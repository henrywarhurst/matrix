...
bcp_implementation::bcp_implementation()
    : m_list_mode(false), m_list_summary_mode(false), m_license_mode(false),
      m_cvs_mode(false), m_svn_mode(false), m_unix_lines(false),
      m_scan_mode(false), m_bsl_convert_mode(false), m_bsl_summary_mode(false),
      m_namespace_alias(false), m_list_namespaces(false) {}

bcp_implementation::~bcp_implementation() {}

bcp_application::~bcp_application() {}

void bcp_implementation::enable_list_mode() { m_list_mode = true; }
...
void bcp_implementation::enable_cvs_mode() { m_cvs_mode = true; }

void bcp_implementation::enable_svn_mode() { m_svn_mode = true; }

void bcp_implementation::enable_scan_mode() { m_scan_mode = true; }

void bcp_implementation::enable_license_mode() { m_license_mode = true; }
...
void bcp_implementation::enable_unix_lines() { m_unix_lines = true; }
...
void bcp_implementation::set_boost_path(const char *p) {
  ...
  if (!fs::exists(check)) {
    std::string s = "The Boost path appears to have been incorrectly set: "
                    "could not find boost/version.hpp in ";
...
void bcp_implementation::set_namespace_alias(bool b) { m_namespace_alias = b; }
...
int bcp_implementation::run() {
  ...
  if (m_cvs_mode) {
    std::cerr << "CAUTION: Boost is no longer in CVS, cvs mode may not work "
                 "anymore!!!"
              << std::endl;
  ...
  if (m_list_namespaces) {
    ...
    std::cout << "\n\nThe top level namespaces found for header and source "
                 "files were:\n";
    while (i != j) {
    ...
    i = m_top_namespaces.begin();
    std::cout << "\n\nThe top level namespaces found for all other source "
                 "files were:\n";
    while (i != j) {
      if (!regex_match(i->second.generic_string(), important_file))
        std::cout << i->first << " (from " << i->second << ")" << std::endl;
...
pbcp_application bcp_application::create() {
  pbcp_application result(
      static_cast<bcp_application *>(new bcp_implementation()));
  return result;
}
