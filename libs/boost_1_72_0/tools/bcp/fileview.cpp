...
// construct:
fileview::fileview() { pimpl.reset(new implementation()); }
...
// capacity:
fileview::size_type fileview::size() const { return pimpl->m_data.size(); }

fileview::size_type fileview::max_size() const {
  return pimpl->m_data.max_size();
}

bool fileview::empty() const { return pimpl->m_data.empty(); }
...
void fileview::swap(fileview &that) { pimpl.swap(that.pimpl); }
