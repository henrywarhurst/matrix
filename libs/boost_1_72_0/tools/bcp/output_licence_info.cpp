...
struct split_path {
  const fs::path &root;
  const fs::path &file;
  split_path(const fs::path &r, const fs::path &f) : root(r), file(f) {}
...
std::ostream &operator<<(std::ostream &os, const split_path &p) {
  os << "<a href=\"" << (p.root / p.file).string() << "\">" << p.file.string()
     << "</a>";
  return os;
}
...
std::string make_link_target(const std::string &s) {
  ...
  for (unsigned i = 0; i < s.size(); ++i) {
    result.append(1, static_cast<std::string::value_type>(
                         std::isalnum(s[i]) ? s[i] : '_'));
  }
  return result;
}
...
void bcp_implementation::output_license_info() {
  ...
  while (i != j) {
    // title:
    os << "   <A href=\"#"
       << make_link_target(licenses.first[i->first].license_name) << "\">"
  ...
  if (!m_bsl_summary_mode) {
    os << "Moving to the Boost Software License...\n"
          "  <a href=\"#bsl-converted\">Files that can be automatically "
          "converted to the Boost Software License</a>\n"
          "  <a href=\"#to-bsl\">Files that can be manually converted to the "
          "Boost Software License</a>\n"
          "  <a href=\"#not-to-bsl\">Files that can <b>NOT</b> be moved to the "
          "Boost Software License</a>\n"
          "  <a href=\"#need-bsl-authors\">Authors we need to move to the "
          "Boost Software License</a>\n"
  ...
  os << "</p><p>The Boost path was: <code>" << m_boost_path.string()
     << "</code></P>";
  //
  ...
  static const boost::regex version_regex(
      "^[[:blank:]]*#[[:blank:]]*define[[:blank:]]+BOOST_VERSION[[:blank:]]+("
      "\\d+)");
  boost::cmatch what;
  if (boost::regex_search(version_file.begin(), version_file.end(), what,
                          version_regex)) {
    int version = boost::lexical_cast<int>(what.str(1));
    os << "<p>The Boost version is: " << version / 100000 << "."
       << version / 100 % 1000 << "." << version % 100 << "</P>\n";
  }
  ...
  if (!m_bsl_summary_mode) {
    ...
    while (i != j) {
      // title:
      os << "<H3>" << licenses.first[i->first].license_name << "</H3>\n";
      // license text:
      os << "<BLOCKQUOTE>" << licenses.first[i->first].license_text
         << "</BLOCKQUOTE>";
      // Copyright holders:
      os << "<P>This license is used by " << i->second.authors.size()
         << " authors and " << i->second.files.size() << " files <a href=\"#"
         << make_link_target(licenses.first[i->first].license_name)
         << "\">(see details)</a>";
      os << "</P></BLOCKQUOTE>\n";
      ++i;
  ...
  while (i != j) {
    // title:
    os << "<H3><A name=\""
       << make_link_target(licenses.first[i->first].license_name) << "\"></a>"
       << licenses.first[i->first].license_name << "</H3>\n";
    // license text:
    os << "<BLOCKQUOTE>" << licenses.first[i->first].license_text
       << "</BLOCKQUOTE>";
    if (!m_bsl_summary_mode || (license_index >= 3)) {
      // Copyright holders:
      os << "<P>This license is used by the following "
         << i->second.authors.size()
         << " copyright holders:</P>\n<BLOCKQUOTE><P>";
      ...
      os << "<P>This license applies to the following "
         << i->second.files.size() << " files:</P>\n<BLOCKQUOTE><P>";
      ...
      os << "<P>This license is used by " << i->second.authors.size()
         << " authors (list omitted for brevity).</P>\n";
      os << "<P>This license applies to " << i->second.files.size()
         << " files (list omitted for brevity).</P>\n";
    }
    ++license_index;
    ++i;
  }
  ...
  os << "<h2><a name=\"files\"></a>Files With No Recognisable Licence</h2>\n"
        "<P>The following "
     << m_unknown_licenses.size()
     << " files had no recognisable license information:</P><BLOCKQUOTE><P>\n";
  ...
  //
  os << "<h2><a name=\"authors\"></a>Files With No Recognisable Copyright "
        "Holder</h2>\n"
        "<P>The following "
     << m_unknown_authors.size()
     << " files had no recognisable copyright holder:</P>\n<BLOCKQUOTE><P>";
  ...
  if (!m_bsl_summary_mode) {
    ...
    // verification.
    //
    os << "<h2><a name=\"bsl-converted\"></a>Files that can be automatically "
          "converted to the Boost Software License</h2>\n"
       << "<P>The following " << m_converted_to_bsl.size()
       << " files can be automatically converted to the Boost Software "
          "License, but require manual verification before they can be "
          "committed to CVS:</P>\n";
    if (!m_converted_to_bsl.empty()) {
      typedef std::map<fs::path, std::pair<std::string, std::string>,
                       path_less>::const_iterator conv_iterator;
      conv_iterator i = m_converted_to_bsl.begin(),
                    ie = m_converted_to_bsl.end();
      int file_num = 1;
      while (i != ie) {
        os << "<P>[" << file_num << "] File: <tt>"
           << split_path(m_boost_path, i->first)
           << "</tt><br>\n<table border=\"1\">\n  <tr>\n    <td><pre>"
    ...
    //
    // Output list of files that could be moved over to the Boost Software
    // License
    //
    os << "<h2><a name=\"to-bsl\"></a>Files that could be converted to the "
          "Boost Software License</h2>\n"
          "<P>The following "
       << m_can_migrate_to_bsl.size()
       << " files could be manually converted to the Boost Software License, "
          "but have not yet been:</P>\n<BLOCKQUOTE><P>";
    ...
    os << "</p></BLOCKQUOTE>";
    //
    // Output list of files that can not be moved over to the Boost Software
    // License
    //
    os << "<h2><a name=\"not-to-bsl\"></a>Files that can NOT be converted to "
          "the Boost Software License</h2>\n"
          "<P>The following "
       << m_cannot_migrate_to_bsl.size()
       << " files cannot be converted to the Boost Software License because we "
          "need the permission of more authors:</P>\n<BLOCKQUOTE><P>";
    ...
    //
    os << "<h2><a name=\"need-bsl-authors\"></a>Authors we need for the "
          "BSL</h2>\n"
          "<P>Permission of the following authors is needed before we can "
          "convert to the Boost Software License. The list of authors that "
          "have given their permission is contained in "
          "<code>more/blanket-permission.txt</code>.</P>\n<BLOCKQUOTE><P>";
    std::copy(m_authors_for_bsl_migration.begin(),
              m_authors_for_bsl_migration.end(),
              std::ostream_iterator<std::string>(os, "<br>\n"));
    os << "</p></BLOCKQUOTE>";
    //
    // output a table of copyright information:
    //
    os << "<H2><a name=\"copyright\"></a>Copyright Holder "
          "Information</H2><table border=\"1\">\n";
    std::map<std::string, std::set<fs::path, path_less>>::const_iterator ad,
        ead;
  ...
  //
  os << "<H2><a name=\"depend\"></a>File Dependency "
        "Information</H2><BLOCKQUOTE><pre>\n";
  ...
  if (m_bsl_summary_mode) {
    ...
    bad_paths.insert(m_cannot_migrate_to_bsl.begin(),
                     m_cannot_migrate_to_bsl.end());
    typedef std::map<fs::path, std::pair<std::string, std::string>,
                     path_less>::const_iterator conv_iterator;
    conv_iterator i = m_converted_to_bsl.begin(), ie = m_converted_to_bsl.end();
    while (i != ie) {
  ...
  while (fi != efi) {
    ...
    if (dep != last_dep)
      while (true) {
        ...
        if ((dep == m_dependencies.end()) ||
            (0 == compare_paths(dep->second, last_dep->second)))
          break;
      }
    os << "\n";
    ++fi;
