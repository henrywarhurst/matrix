...
void bcp_implementation::scan_cvs_path(const fs::path &p) {
  ...
  static const int file_subs[] = {
      1,
      2,
  };

  for (std::size_t entry = 0; entry < sizeof(file_list) / sizeof(file_list[0]);
       ++entry) {
    fs::path entries(m_boost_path / p / file_list[entry]);
    if (fs::exists(entries)) {
      fileview view(entries);
      boost::regex_token_iterator<const char *> i(view.begin(), view.end(),
                                                  dir_expression, 1);
...
void bcp_implementation::scan_svn_path(const fs::path &p) {
  ...
  if (fs::exists(entries)) {
    fileview view(entries);
    boost::cregex_token_iterator i(view.begin(), view.end(), entry_expression,
                                   1),
        j;
    ...
    while (i != j) // entries
    {
      ...
      boost::sregex_token_iterator atr_it(entr.begin(), entr.end(),
                                          entry_line_expression, 1),
          atr_j;
      ...
      if (atr_it != atr_j) {
        ...
        if (++atr_it != atr_j) {
              ...
              if (atr_it->str() == "has-props") {
                ...
                if (fs::exists(properties)) {
                  ...
                  static const boost::regex mime_type(
                      "svn:mime-type[[:blank:]]*(?:\\n|\\r|\\r\\n)[^\\r\\n]*(?:"
                      "\\n|\\r|\\r\\n)[[:blank:]]*text/");
                  binary = regex_search(prop.begin(), prop.end(), mime_type)
                               ? false
                               : true;
                }
              }
          ...
          //       else
          //         std::cerr << "WARNING: unknown entry kind for entry " <<
          //         name
          //              << "in " << entries << std::endl;
        }
      }
