 ...
 * const fs::path& p, const fileview& view)
 */
...
void bcp_implementation::add_path(const fs::path &p) {
  ...
  if (fs::exists(m_boost_path / normalized_path)) {
    ...
    std::cerr << "CAUTION: dependent file " << p.string() << " does not exist."
              << std::endl;
    std::cerr << "   Found while scanning file " << m_dependencies[p].string()
              << std::endl;
  }
}
...
void bcp_implementation::add_file(const fs::path &p) {
  ...
  if (is_jam_file(p) && m_namespace_name.size() &&
      ((std::distance(p.begin(), p.end()) < 3) || (*p.begin() != "tools") ||
       (*++p.begin() != "build"))) {
    //
    // We're doing a rename of namespaces and library names
    // so scan for names of libraries:
    //
    static const boost::regex e("\\<lib\\s+(boost\\w+)\\s+[:;]");
    ...
    static const std::pair<fs::path, std::string> specials_library_names[] = {
        std::pair<fs::path, std::string>("libs/python/build/Jamfile.v2",
                                         "boost_python"),
        std::pair<fs::path, std::string>("libs/python/build/Jamfile.v2",
                                         "boost_python3"),
    };

    for (unsigned int n = 0; n < (sizeof(specials_library_names) /
                                  sizeof(specials_library_names[0]));
         ++n) {
  ...
  //
  static const std::pair<fs::path, fs::path> specials[] = {
      std::pair<fs::path, fs::path>("tools/build/src/kernel/modules.jam",
                                    "libs/predef/check"),
      std::pair<fs::path, fs::path>("tools/build/src/kernel/modules.jam",
                                    "libs/predef/tools"),
      std::pair<fs::path, fs::path>("tools/build/src/kernel/modules.jam",
      ...
      std::pair<fs::path, fs::path>(
          "tools/build/src/kernel/modules.jam",
          "tools/boost_install/boost-install-dirs.jam"),
      std::pair<fs::path, fs::path>("tools/build/src/kernel/modules.jam",
                                    "tools/boost_install/Jamfile"),
      std::pair<fs::path, fs::path>("tools/build/src/kernel/modules.jam",
                                    "libs/headers"),
      std::pair<fs::path, fs::path>("libs/test/build/Jamfile.v2",
                                    "libs/timer/src"),
      std::pair<fs::path, fs::path>("libs/test/build/Jamfile.v2",
                                    "libs/timer/build"),
      std::pair<fs::path, fs::path>("boost/atomic/capabilities.hpp",
                                    "boost/atomic/detail"),
      std::pair<fs::path, fs::path>("boost/chrono/chrono.hpp",
                                    "libs/chrono/src"),
      std::pair<fs::path, fs::path>("boost/chrono/chrono.hpp",
                                    "libs/chrono/build"),
      std::pair<fs::path, fs::path>("boost/context/execution_context.hpp",
                                    "libs/context/src"),
      std::pair<fs::path, fs::path>("boost/context/execution_context.hpp",
                                    "libs/context/build"),
      std::pair<fs::path, fs::path>("boost/context/fcontext.hpp",
                                    "libs/context/src"),
      std::pair<fs::path, fs::path>("boost/context/fcontext.hpp",
                                    "libs/context/build"),
      std::pair<fs::path, fs::path>("boost/context/fixedsize_stack.hpp",
                                    "libs/context/src"),
      std::pair<fs::path, fs::path>("boost/context/fixedsize_stack.hpp",
                                    "libs/context/build"),
      std::pair<fs::path, fs::path>(
          "boost/context/protected_fixedsize_stack.hpp", "libs/context/src"),
      std::pair<fs::path, fs::path>(
          "boost/context/protected_fixedsize_stack.hpp", "libs/context/build"),
      std::pair<fs::path, fs::path>("boost/context/segmented_stack.hpp",
                                    "libs/context/src"),
      std::pair<fs::path, fs::path>("boost/context/segmented_stack.hpp",
                                    "libs/context/build"),
      std::pair<fs::path, fs::path>("boost/context/stack_context.hpp",
                                    "libs/context/src"),
      std::pair<fs::path, fs::path>("boost/context/stack_context.hpp",
                                    "libs/context/build"),
      std::pair<fs::path, fs::path>("boost/context/stack_traits.hpp",
                                    "libs/context/src"),
      std::pair<fs::path, fs::path>("boost/context/stack_traits.hpp",
                                    "libs/context/build"),
      ...
      std::pair<fs::path, fs::path>("boost/filesystem/fstream.hpp",
                                    "boost/filesystem.hpp"),
      std::pair<fs::path, fs::path>("boost/filesystem/operations.hpp",
                                    "boost/filesystem.hpp"),
      std::pair<fs::path, fs::path>("boost/filesystem/path.hpp",
                                    "boost/filesystem.hpp"),
      std::pair<fs::path, fs::path>("boost/filesystem.hpp",
      ...
      std::pair<fs::path, fs::path>("boost/function.hpp",
                                    "boost/function/detail"),
      std::pair<fs::path, fs::path>("boost/regex/config.hpp",
                                    "boost/regex/user.hpp"),
      std::pair<fs::path, fs::path>("boost/signals/signal_template.hpp",
                                    "boost/function"),
      ...
      std::pair<fs::path, fs::path>("boost/mpl/vector_c.hpp",
                                    "boost/mpl/vector"),
      ...
      std::pair<fs::path, fs::path>("boost/mpl/aux_/include_preprocessed.hpp",
      ...
      std::pair<fs::path, fs::path>("libs/graph/src/python/visitor.hpp",
                                    "libs/graph/src/python"),
      std::pair<fs::path, fs::path>("boost/test/detail/config.hpp",
                                    "libs/test/src"),
      std::pair<fs::path, fs::path>("boost/test/detail/config.hpp",
                                    "libs/test/build"),
      std::pair<fs::path, fs::path>("boost/test/detail/config.hpp",
                                    "libs/predef/build.jam"),
      std::pair<fs::path, fs::path>("boost/test/detail/config.hpp",
                                    "libs/predef/check"),
      std::pair<fs::path, fs::path>("boost/typeof.hpp",
                                    "boost/typeof/incr_registration_group.hpp"),
      std::pair<fs::path, fs::path>("boost/function_types/detail/pp_loop.hpp",
                                    "boost/function_types/detail/pp_cc_loop"),
      std::pair<fs::path, fs::path>(
          "boost/function_types/components.hpp",
          "boost/function_types/detail/components_impl"),
      std::pair<fs::path, fs::path>("boost/function_types/detail/pp_loop.hpp",
                                    "boost/function_types/detail"),
      std::pair<fs::path, fs::path>("boost/math/tools/rational.hpp",
                                    "boost/math/tools/detail"),
      std::pair<fs::path, fs::path>("boost/proto/repeat.hpp",
      ...
      std::pair<fs::path, fs::path>(
          ...
          "libs/graph_parallel"),
      std::pair<fs::path, fs::path>("libs/coroutine/build/Jamfile.v2",
                                    "libs/context/src"),
      std::pair<fs::path, fs::path>("libs/coroutine/build/Jamfile.v2",
                                    "libs/context/build"),
      std::pair<fs::path, fs::path>("libs/fiber/build/Jamfile.v2",
                                    "libs/context/src"),
      std::pair<fs::path, fs::path>("libs/fiber/build/Jamfile.v2",
                                    "libs/context/build"),
  };
...
void bcp_implementation::add_file_dependencies(const fs::path &p,
                                               bool scanfile) {
  static const boost::regex e("^[[:blank:]]*(?://"
                              "@bcp[[:blank:]]+([^\\n]*)\n)?#[[:blank:]]*"
                              "include[[:blank:]]*[\"<]([^\">]+)[\">]");
  ...
  if (m_license_mode && !scanfile)
    scan_license(p, view);
  const int subs[] = {1, 2};
  boost::regex_token_iterator<const char *> i(view.begin(), view.end(), e,
                                              subs);
  boost::regex_token_iterator<const char *> j;
  while (i != j) {
    //
    ...
    try {
      ...
      if (discart_message.size()) {
        ...
        std::cout << "Optional functionality won't be copied: "
                  << discart_message << std::endl;
        std::cout << "Add the file " << *i
                  << " to the list of dependencies to extract to copy this "
                     "functionality."
                  << std::endl;
        ++i;
        continue;
      ...
      std::cerr << "Can't parse filename " << *i << " included by file "
                << p.string() << std::endl;
      ++i;
      continue;
    }
  ...
  //
  static const boost::regex ppfiles(
      "^[[:blank:]]*#[[:blank:]]*define[[:blank:]]+(?:BOOST_PP_FILENAME|BOOST_"
      "PP_ITERATION_PARAMS|BOOST_PP_INDIRECT_SELF)(?:[^\\n]|\\\\\\n)+?[\"<]([^"
      "\">]+)[\">]");
  i = boost::regex_token_iterator<const char *>(view.begin(), view.end(),
                                                ppfiles, 1);
  while (i != j) {
    //
    // *i contains the name of the include file,
    ...
    if (fs::exists(test_file) && !fs::is_directory(test_file) &&
      ...
      std::cerr << "CAUTION: Boost.Preprocessor iterated file "
                << include_file.string() << " does not exist." << std::endl;
    }
    ++i;
  }
  ...
  static const std::string known_macros[] = {
      ...
      "BOOST_PP_STRINGIZE(boost/mpl/aux_/preprocessed/"
  ...
  static const boost::regex indirect_includes(
      "^[[:blank:]]*#[[:blank:]]*include[[:blank:]]+([^\"<][^\n]*?)[[:blank:]]*"
      "$");
  i = boost::regex_token_iterator<const char *>(view.begin(), view.end(),
                                                indirect_includes, 1);
  while (i != j) {
    const std::string *known_macros_end =
        known_macros + sizeof(known_macros) / sizeof(known_macros[0]);
    if (known_macros_end ==
        std::find(known_macros, known_macros_end, i->str())) {
      std::cerr
          << "CAUTION: don't know how to trace depenencies through macro: \""
          << *i << "\" in file: " << p.string() << std::endl;
    }
    ++i;
  }
  //
  ...
  //
  static const boost::regex m(
      "^\\s*int\\s+(?:cpp_main|test_main|unit_test_main)");
  boost::cmatch what;
