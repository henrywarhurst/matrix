...
fileview::const_iterator context_before_license(const fileview &v,
                                                fileview::const_iterator start,
                                                int context_lines = 3) {
  char last_char = '\0';
...
fileview::const_iterator context_after_license(const fileview &v,
                                               fileview::const_iterator end,
                                               int context_lines = 3) {
  char last_char = '\0';
...
static std::string find_prefix(const fileview &v,
                               fileview::const_iterator start_of_line) {
  while (start_of_line != v.begin() && *start_of_line != '\n' &&
         *start_of_line != '\r')
    --start_of_line;
  ...
  while (*first_noncomment_char == '/' || *first_noncomment_char == '*' ||
         *first_noncomment_char == ' ' || *first_noncomment_char == '#')
    ++first_noncomment_char;

  return std::string(start_of_line, first_noncomment_char);
}

static std::string html_escape(fileview::const_iterator first,
                               fileview::const_iterator last) {
  std::string result;
  while (first != last) {
    switch (*first) {
    case '<':
...
static bool is_non_bsl_license(int index) { return index > 2; }
...
void bcp_implementation::scan_license(const fs::path &p, const fileview &v) {
  ...
  for (int i = 0; i < licenses.second; ++i) {
    ...
    if (boost::regex_search(v.begin(), v.end(), m,
                            licenses.first[i].license_signature)) {
      ...
      //
      boost::regex_iterator<const char *> cpy(
          v.begin(), v.end(), licenses.first[i].copyright_signature);
      ...
      while (cpy != ecpy) {
        ...
        std::string author_list = cpy->format(
            licenses.first[i].copyright_formatter, boost::format_all);
        // now enumerate that list for all the names:
        static const boost::regex author_separator(
            "(?:\\s*,(?!\\s*(?:inc|ltd)\\b)\\s*|\\s+(,\\s*)?(and|&)\\s+)|"
            "by\\s+",
            boost::regex::perl | boost::regex::icase);
        boost::regex_token_iterator<std::string::const_iterator> atr(
            author_list.begin(), author_list.end(), author_separator, -1);
        ...
        while (atr != eatr) {
          ...
          if (name.size() && name[0] != '-') {
            ...
            // author hasn't given blanket permission, note this for the report.
      ...
      while (start_of_license != v.begin() && *start_of_license != '\r' &&
             *start_of_license != '\n' && *start_of_license != '.')
        --start_of_license;
      ...
      while (end_of_license != v.end() && *end_of_license != '\r' &&
             *end_of_license != '\n')
        ++end_of_license;
    }
  }
  ...
  if (has_non_bsl_license) {
    ...
    if (nonbsl_author_count == 0 && license_count == 1) {
    ...
    if (!converted) {
      if (nonbsl_author_count > 0)
        m_cannot_migrate_to_bsl.insert(p);
      else
        m_can_migrate_to_bsl.insert(p);
    }
  }
}
