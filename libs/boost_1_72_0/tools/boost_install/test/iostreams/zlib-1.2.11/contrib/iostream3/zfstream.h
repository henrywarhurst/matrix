 ...
 * access (tricky). Otherwise, it attempts to be a drop-in replacement for the
 * standard file streambuf.
 */
...
public:
   ...
   *  @param  comp_strategy  Compression strategy (see zlib.h for allowed
   * values)
...
public:
   ...
   */
  explicit gzifstream(const char *name,
...
public:
   ...
   */
  explicit gzofstream(const char *name,
 ...
 */
template <typename T1, typename T2> class gzomanip2 {
public:
...
inline gzofstream &setcompression(gzofstream &gzs, int l,
...
inline gzomanip2<T1, T2>::gzomanip2(gzofstream &(*f)(gzofstream &, T1, T2),
                                    T1 v1, T2 v2)
