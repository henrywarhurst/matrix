...
std::streambuf *gzfilebuf::setbuf(char_type *p, std::streamsize n) {
  ...
  if (this->sync() == -1)
    return NULL;
  // If buffering is turned off on purpose via setbuf(0,0), still allocate
  // one... "Unbuffered" only really refers to put [27.8.1.4.10], while get
  // needs at least a buffer of size 1 (very inefficient though, therefore make
  // it bigger?) This follows from [27.5.2.4.3]/12 (gptr needs to point at
  // something, it seems)
  if (!p || !n) {
...
int gzfilebuf::sync() {
  return traits_type::eq_int_type(this->overflow(), traits_type::eof()) ? -1
                                                                        : 0;
}
...
void gzfilebuf::enable_buffer() {
  ...
  if (own_buffer && !buffer) {
    ...
    if (buffer_size > 0) {
      ...
      // The one-past-end pointer actually points to the last element of the
      // buffer, so that overflow(c) can safely add the extra character c to the
      // sequence. These pointers remain in place for the duration of the buffer
