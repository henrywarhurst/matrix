...
template <class A, class T> struct sp_bind_allocator {
...
public:
  ...
  A &allocator() BOOST_SP_NOEXCEPT { return allocator_; }

  std::size_t size() const BOOST_SP_NOEXCEPT { return size_; }
...
public:
  ...
  BOOST_CONSTEXPR std::size_t size() const BOOST_SP_NOEXCEPT { return N; }
...
inline U *sp_array_start(T *base) BOOST_SP_NOEXCEPT {
...
template <class A, class T> class sp_array_creator {
  ...
  typedef typename boost::type_with_alignment<
      sp_array_alignment<T, element>::value>::type type;
...
public:
  ...
  sp_array_base(const A &other, type *start, std::size_t size)
      : state_(other, size) {
    boost::alloc_construct_n(state_.allocator(), boost::first_scalar(start),
  ...
  sp_array_base(const A &other, type *start, std::size_t size, const U &list)
    ...
    boost::alloc_construct_n(state_.allocator(), boost::first_scalar(start),
                             state_.size() * count, boost::first_scalar(&list),
                             count);
  }

  T &state() BOOST_SP_NOEXCEPT { return state_; }
...
public:
  ...
  T *get() const BOOST_SP_NOEXCEPT { return result_; }

  void release() BOOST_SP_NOEXCEPT { result_ = 0; }
...
allocate_shared(const A &allocator, std::size_t count) {
  ...
  result.release();
  return shared_ptr<T>(
      detail::sp_internal_constructor_tag(), start,
