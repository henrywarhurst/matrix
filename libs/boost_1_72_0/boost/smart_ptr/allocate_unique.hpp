...
template <class T> struct sp_alloc_result { typedef T type; };
...
template <class T> struct sp_alloc_value {
  typedef
      typename boost::remove_cv<typename boost::remove_extent<T>::type>::type
          type;
};

#if !defined(BOOST_NO_CXX11_ALLOCATOR)
template <class A, class T> struct sp_alloc_to {
...
#if !defined(BOOST_NO_CXX11_ALLOCATOR)
template <class A> struct sp_alloc_type {
  typedef typename std::allocator_traits<A>::pointer type;
};
#else
template <class A> struct sp_alloc_type { typedef typename A::pointer type; };
#endif
...
public:
  ...
  sp_alloc_ptr() BOOST_SP_NOEXCEPT : p_() {}
  ...
  sp_alloc_ptr(std::size_t, P p) BOOST_SP_NOEXCEPT : p_(p) {}

#if !defined(BOOST_NO_CXX11_NULLPTR)
  sp_alloc_ptr(detail::sp_nullptr_t) BOOST_SP_NOEXCEPT : p_() {}
#endif

  T &operator*() const { return *p_; }

  T *operator->() const BOOST_SP_NOEXCEPT { return boost::to_address(p_); }

#if !defined(BOOST_NO_CXX11_EXPLICIT_CONVERSION_OPERATORS)
  explicit operator bool() const BOOST_SP_NOEXCEPT { return !!p_; }
#endif

  bool operator!() const BOOST_SP_NOEXCEPT { return !p_; }

  P ptr() const BOOST_SP_NOEXCEPT { return p_; }

  BOOST_STATIC_CONSTEXPR std::size_t size() BOOST_SP_NOEXCEPT { return 1; }

#if defined(BOOST_MSVC) && BOOST_MSVC < 1910
  static sp_alloc_ptr pointer_to(T &v) {
    return sp_alloc_ptr(
        1, std::pointer_traits<P>::pointer_to(
               const_cast<typename boost::remove_cv<T>::type &>(v)));
  }
#endif
...
public:
  ...
  sp_alloc_ptr(std::size_t n, P p) BOOST_SP_NOEXCEPT : p_(p), n_(n) {}
  ...
  std::size_t size() const BOOST_SP_NOEXCEPT { return n_; }

#if defined(BOOST_MSVC) && BOOST_MSVC < 1910
  static sp_alloc_ptr pointer_to(T &v) {
    return sp_alloc_ptr(
        n_, std::pointer_traits<P>::pointer_to(
                const_cast<typename boost::remove_cv<T>::type &>(v)));
  }
#endif
...
public:
  ...
  BOOST_STATIC_CONSTEXPR std::size_t size() BOOST_SP_NOEXCEPT { return N; }

#if defined(BOOST_MSVC) && BOOST_MSVC < 1910
  static sp_alloc_ptr pointer_to(T &v) {
    return sp_alloc_ptr(
        N, std::pointer_traits<P>::pointer_to(
               const_cast<typename boost::remove_cv<T>::type &>(v)));
  }
#endif
...
inline bool operator==(const sp_alloc_ptr<T, P> &lhs,
                       const sp_alloc_ptr<T, P> &rhs) {
...
template <class T, class P>
inline bool operator==(detail::sp_nullptr_t,
...
template <class A>
inline void sp_alloc_clear(A &a, typename sp_alloc_type<A>::type p, std::size_t,
                           boost::false_type) {
  boost::alloc_destroy(a, boost::to_address(p));
}

template <class A>
inline void sp_alloc_clear(A &a, typename sp_alloc_type<A>::type p,
...
class alloc_deleter : empty_value<typename detail::sp_alloc_to<
                          A, typename detail::sp_alloc_value<T>::type>::type> {
  typedef typename detail::sp_alloc_to<
      A, typename detail::sp_alloc_value<T>::type>::type allocator;
  typedef empty_value<allocator> base;

public:
  typedef detail::sp_alloc_ptr<T,
                               typename detail::sp_alloc_type<allocator>::type>
      pointer;
...
public:
  typedef
      typename sp_alloc_to<A, typename sp_alloc_value<T>::type>::type allocator;
...
public:
  ...
  allocator &state() BOOST_SP_NOEXCEPT { return a_; }
...
allocate_unique(const A &alloc, std::size_t size,
  ...
  boost::alloc_construct_n(
      c.state(), boost::first_scalar(c.get()),
