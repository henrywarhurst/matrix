...
public:
  ...
  // aliasing
  template <class Y>
  weak_ptr(shared_ptr<Y> const &r, element_type *p) BOOST_SP_NOEXCEPT
      : px(p),
        pn(r.pn) {}

  template <class Y>
  weak_ptr(weak_ptr<Y> const &r, element_type *p) BOOST_SP_NOEXCEPT : px(p),
                                                                      pn(r.pn) {
  }
  ...
  template <class Y>
  weak_ptr(weak_ptr<Y> &&r, element_type *p) BOOST_SP_NOEXCEPT
      : px(p),
        pn(std::move(r.pn)) {}
  ...
  template <class Y> weak_ptr &operator=(weak_ptr<Y> &&r) BOOST_SP_NOEXCEPT {
  ...
  bool expired() const BOOST_SP_NOEXCEPT { return pn.use_count() == 0; }
