...
public:
  ...
  void add_ref_copy() { BOOST_SP_INTERLOCKED_INCREMENT(&use_count_); }
  ...
  bool add_ref_lock() // true on success
  {
    for (;;) {
      ...
      long tmp2 = tmp + 1;
      if (BOOST_SP_INTERLOCKED_COMPARE_EXCHANGE(&use_count_, tmp2, tmp) ==
          tmp2 - 1)
        return true;

#else

      if (BOOST_SP_INTERLOCKED_COMPARE_EXCHANGE(&use_count_, tmp + 1, tmp) ==
          tmp)
        return true;
