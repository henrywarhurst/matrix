...
inline int atomic_decrement(int *pw) {
  ...
  int r =
      static_cast<int>(_Asm_fetchadd(_FASZ_W, _SEM_REL, pw, -1, _LDHINT_NONE));
  if (1 == r) {
    _Asm_mf();
  }
...
inline int atomic_conditional_increment(int *pw) {
  ...
  for (;;) {
    ...
    int r = static_cast<int>(
        _Asm_cmpxchg(_SZ_W, _SEM_ACQ, pw, v + 1, _LDHINT_NONE));
    if (r == v) {
      return r + 1;
    }
...
public:
  ...
  {
    return static_cast<int const volatile &>(
        use_count_); // TODO use ld.acq here
  }
};
