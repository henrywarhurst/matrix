...
template <class D> struct sp_convert_reference { typedef D type; };
...
public:
  ...
  BOOST_CONSTEXPR explicit shared_count(sp_counted_base *pi) BOOST_SP_NOEXCEPT
      : pi_(pi)
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
      ,
        id_(shared_count_id)
...
#endif
  {
    ...
    typedef typename std::allocator_traits<A>::template rebind_alloc<impl_type>
        A2;
...
#endif
  {
    ...
    D2 d2(r.get_deleter());
    pi_ = new sp_counted_impl_pd<typename std::unique_ptr<Y, D>::pointer, D2>(
        r.get(), d2);
...
#endif
  {
    ...
    pi_ = new sp_counted_impl_pd<
        typename boost::movelib::unique_ptr<Y, D>::pointer, D2>(r.get(), d2);
  ...
  shared_count(weak_count const &r, sp_nothrow_tag)
      BOOST_SP_NOEXCEPT; // constructs an empty *this when r.use_count() == 0
  ...
  bool unique() const BOOST_SP_NOEXCEPT { return use_count() == 1; }

  bool empty() const BOOST_SP_NOEXCEPT { return pi_ == 0; }

  friend inline bool operator==(shared_count const &a,
                                shared_count const &b) BOOST_SP_NOEXCEPT {
    return a.pi_ == b.pi_;
  }
...
public:
  ...
  friend inline bool operator==(weak_count const &a,
                                weak_count const &b) BOOST_SP_NOEXCEPT {
    return a.pi_ == b.pi_;
  }
...
inline shared_count::shared_count(weak_count const &r,
                                  sp_nothrow_tag) BOOST_SP_NOEXCEPT
    : pi_(r.pi_)
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
    ,
      id_(shared_count_id)
