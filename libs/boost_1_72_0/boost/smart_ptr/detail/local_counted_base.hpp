...
public:
  ...
  BOOST_CONSTEXPR
  local_counted_base(local_counted_base const &) BOOST_SP_NOEXCEPT
      : local_use_count_(initial_) {}
  ...
  virtual boost::detail::shared_count
  local_cb_get_shared_count() const BOOST_SP_NOEXCEPT = 0;
  ...
  long local_use_count() const BOOST_SP_NOEXCEPT { return local_use_count_; }
};
...
public:
  explicit local_counted_impl(shared_count const &pn) BOOST_SP_NOEXCEPT
      : pn_(pn) {}

#if !defined(BOOST_NO_CXX11_RVALUE_REFERENCES)

  explicit local_counted_impl(shared_count &&pn) BOOST_SP_NOEXCEPT
      : pn_(std::move(pn)) {}

#endif

  virtual void local_cb_destroy() BOOST_SP_NOEXCEPT { delete this; }

  virtual boost::detail::shared_count
  local_cb_get_shared_count() const BOOST_SP_NOEXCEPT {
    return pn_;
  }
};
