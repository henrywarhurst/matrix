...
class array_iterator
    : public iterator_facade<
          ...
          typename associated_types<T, NumDims>::value_type, IteratorCategory,
          Reference>,
...
public:
  ...
  array_iterator(index idx, TPtr base, const size_type *extents,
      ...
      : idx_(idx), base_(base), extents_(extents), strides_(strides),
        index_base_(index_base) {}

  template <typename OPtr, typename ORef, typename Cat>
  array_iterator(const array_iterator<T, OPtr, NumDims, ORef, Cat> &rhs,
  ...
  reference dereference() const {
    ...
    return accessor::access(boost::type<reference>(), idx_, base_, extents_,
                            strides_, index_base_);
  }

  void increment() { ++idx_; }
  void decrement() { --idx_; }

  template <class IteratorAdaptor> bool equal(IteratorAdaptor &rhs) const {
  ...
  template <class DifferenceType> void advance(DifferenceType n) { idx_ += n; }
