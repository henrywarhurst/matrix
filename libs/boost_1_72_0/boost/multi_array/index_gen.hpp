...
template <int NumRanges, int NumDims> struct index_gen {
private:
...
public:
  template <int Dims, int Ranges> struct gen_type {
  ...
  template <int ND>
  explicit index_gen(const index_gen<NumRanges - 1, ND> &rhs, const range &r) {
  ...
  static index_gen<0, 0> indices() { return index_gen<0, 0>(); }
};
