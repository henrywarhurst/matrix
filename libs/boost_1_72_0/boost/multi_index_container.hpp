...
class multi_index_container
    : private ::boost::base_from_member<typename detail::rebind_alloc_for<
          Allocator, typename detail::multi_index_node_type<
          ...
          detail::header_holder<
              typename detail::allocator_traits<
                  typename detail::rebind_alloc_for<
                      Allocator, typename detail::multi_index_node_type<
                                     Value, IndexSpecifierList,
                                     Allocator>::type>::type>::pointer,
...
public:
  ...
  explicit multi_index_container(
      ...
      const allocator_type &al =
          typename mpl::identity<multi_index_container>::type::allocator_type())
      :
#else
      ...
      :
        ...
        bfm_allocator(al), super(args_list, bfm_allocator::member),
        node_count(0) {
    BOOST_MULTI_INDEX_CHECK_INVARIANT;
  }
  ...
  multi_index_container(
      ...
      const ctor_args_list &args_list =
          typename mpl::identity<multi_index_container>::type::ctor_args_list(),
      const allocator_type &al =
          typename mpl::identity<multi_index_container>::type::allocator_type())
      :
#else
  ...
  multi_index_container(std::initializer_list<Value> list,
    ...
    BOOST_TRY {
      ...
      iterator hint = super::end();
      for (init_iterator first = list.begin(), last = list.end(); first != last;
           ++first) {
  ...
  multi_index_container(
      ...
      : bfm_allocator(x.bfm_allocator::member), bfm_header(), super(x),
        node_count(0) {
  ...
  template <int N> typename nth_index<N>::type &get() BOOST_NOEXCEPT {
    BOOST_STATIC_ASSERT(N >= 0 && N < mpl::size<index_type_list>::type::value);
    return *this;
  }
  ...
  template <typename Tag> typename index<Tag>::type &get() BOOST_NOEXCEPT {
    return *this;
  }
  ...
  node_type *header() const { return &*bfm_header::member; }
  ...
  void deallocate_node(node_type *x) {
    node_alloc_traits::deallocate(bfm_allocator::member,
                                  static_cast<node_pointer>(x), 1);
  }

  void construct_value(node_type *x, const Value &v) {
    node_alloc_traits::construct(bfm_allocator::member,
                                 boost::addressof(x->value()), v);
  }

  void construct_value(node_type *x, BOOST_RV_REF(Value) v) {
    node_alloc_traits::construct(bfm_allocator::member,
                                 boost::addressof(x->value()), boost::move(v));
  }
  ...
  void destroy_value(node_type *x) {
    node_alloc_traits::destroy(bfm_allocator::member,
  ...
  template <typename T> std::pair<node_type *, bool> insert_ref_(T &t) {
  ...
  std::pair<node_type *, bool> insert_ref_(value_type &x) { return insert_(x); }
  ...
  std::pair<node_type *, bool> insert_ref_(T &t, node_type *position) {
    ...
    BOOST_TRY {
      ...
      BOOST_TRY {
        node_type *res =
            super::insert_(x->value(), position, x, detail::emplaced_tag());
        if (res == x) {
          ++node_count;
          return std::pair<node_type *, bool>(res, true);
        } else {
  ...
  emplace_hint_(node_type *position, BOOST_MULTI_INDEX_FUNCTION_PARAM_PACK) {
    node_type *x = allocate_node();
    BOOST_TRY {
      construct_value(x, BOOST_MULTI_INDEX_FORWARD_PARAM_PACK);
      BOOST_TRY {
  ...
  template <typename Modifier> bool modify_(Modifier &mod, node_type *x) {
    BOOST_TRY { mod(const_cast<value_type &>(x->value())); }
    BOOST_CATCH(...) {
      this->erase_(x);
      BOOST_RETHROW;
    }
  ...
  bool modify_(Modifier &mod, Rollback &back_, node_type *x) {
    ...
    bool b;
    BOOST_TRY { b = super::modify_rollback_(x); }
...
private:
template <BOOST_MULTI_INDEX_TEMPLATE_PARAM_PACK>
void vartempl_construct_value_impl(node_type *x,
...
template <typename MultiIndexContainer, int N> struct nth_index {
  ...
  typedef
      typename mpl::at_c<typename MultiIndexContainer::index_type_list, N>::type
          type;
};
...
project(multi_index_container<Value, IndexSpecifierList, Allocator> &m,
  ...
  BOOST_STATIC_ASSERT((
      mpl::contains<BOOST_DEDUCED_TYPENAME multi_index_type::iterator_type_list,
                    IteratorType>::value));
#endif
  ...
  typedef detail::converter<multi_index_type,
                            BOOST_DEDUCED_TYPENAME IteratorType::container_type>
      converter;
