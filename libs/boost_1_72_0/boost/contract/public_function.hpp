...
specify_precondition_old_postcondition_except<> public_function(Class *obj) {
  ...
  return specify_precondition_old_postcondition_except<>(
      new boost::contract::detail::public_function<
          boost::contract::detail::none, boost::contract::detail::none,
          boost::contract::detail::none,
          Class BOOST_CONTRACT_DETAIL_NO_TVARIADIC_COMMA(
              BOOST_CONTRACT_MAX_ARGS)
              BOOST_CONTRACT_DETAIL_NO_TVARIADIC_ENUM_Z(
                  1, BOOST_CONTRACT_MAX_ARGS, boost::contract::detail::none)>(
          static_cast<boost::contract::virtual_ *>(0), obj,
          boost::contract::detail::none::value()
              BOOST_CONTRACT_DETAIL_NO_TVARIADIC_COMMA(BOOST_CONTRACT_MAX_ARGS)
                  BOOST_CONTRACT_DETAIL_NO_TVARIADIC_ENUM_Z(
...
#define BOOST_CONTRACT_PUBLIC_FUNCTION_VIRTUAL_NO_OVERRIDE_(                   \
    has_virtual_result)                                                        \
  template <BOOST_PP_EXPR_IIF(has_virtual_result, typename VirtualResult)      \
                BOOST_PP_COMMA_IF(has_virtual_result) class Class>             \
  specify_precondition_old_postcondition_except<BOOST_PP_EXPR_IIF(             \
      has_virtual_result, VirtualResult)>                                      \
  public_function(virtual_ *v BOOST_PP_COMMA_IF(has_virtual_result)            \
                      BOOST_PP_EXPR_IIF(has_virtual_result, VirtualResult &r), \
                  Class *obj) {                                                \
    BOOST_PP_IIF(                                                              \
        BOOST_CONTRACT_PUBLIC_FUNCTIONS_IMPL_, /* no F... so cannot enforce    \
                                                  contracted F returns         \
                                                  VirtualResult */             \
        return (specify_precondition_old_postcondition_except<                 \
                BOOST_PP_EXPR_IIF(has_virtual_result, VirtualResult)>(         \
            new boost::contract::detail::public_function<                      \
                boost::contract::detail::none,                                 \
                BOOST_PP_IIF(has_virtual_result, VirtualResult,                \
                             boost::contract::detail::none),                   \
                boost::contract::detail::none,                                 \
                Class BOOST_CONTRACT_DETAIL_NO_TVARIADIC_COMMA(                \
                    BOOST_CONTRACT_MAX_ARGS)                                   \
                    BOOST_CONTRACT_DETAIL_NO_TVARIADIC_ENUM_Z(                 \
                        ...
                        boost::contract::detail::none)>(                       \
                v, obj,                                                        \
                BOOST_PP_IIF(has_virtual_result, r,                            \
                             boost::contract::detail::none::value())           \
...
*/
template <class Class>
specify_precondition_old_postcondition_except<> public_function(virtual_ *v,
                                                                Class *obj);
...
#define BOOST_CONTRACT_PUBLIC_FUNCTION_VIRTUAL_OVERRIDE_Z_(                                           \
  ...
  BOOST_CONTRACT_DETAIL_DECL_OVERRIDING_PUBLIC_FUNCTION_Z(                                            \
      z, arity, /* is_friend = */ 0, has_virtual_result, Override,                                    \
      VirtualResult, F, Class, Args, v, r, /* f */ BOOST_PP_EMPTY(), obj,                             \
      args) {                                                                                         \
    BOOST_PP_IIF(                                                                                     \
        ...
        { /* extra scope paren to expand STATIC_STATIC emu on same line */                            \
          ...
          BOOST_STATIC_ASSERT_MSG(/* -2 for both `this` and `virtual_*` extra                         \
                                     parameters */                                                    \
                                  (boost::function_types::function_arity<                             \
                                       F>::value -                                                    \
                                       2 ==                                                           \
                                   BOOST_CONTRACT_DETAIL_TVARIADIC_SIZEOF(                            \
                                       arity, Args)),                                                 \
                                  "missing one or more arguments for "                                \
                                  "specified function");                                              \
        } { /* extra scope paren to expand STATIC_STATIC emu on same line */                          \
            /* assert consistency of F's result type and VirtualResult */                             \
            BOOST_PP_IIF(has_virtual_result, BOOST_STATIC_ASSERT_MSG,                                 \
                         BOOST_PP_TUPLE_EAT(2))                                                       \
            ((boost::is_same<typename boost::remove_reference<                                        \
                                 typename boost::function_types::result_type<                         \
                                     F>::type>::type,                                                 \
                             typename boost::contract::detail::                                       \
                                 remove_value_reference_if_optional<                                  \
                                     VirtualResult>::type>::value),                                   \
        ...
        } return (specify_precondition_old_postcondition_except<BOOST_PP_EXPR_IIF(has_virtual_result, \
                                                                                  VirtualResult)>(    \
            new boost::contract::detail::public_function<                                             \
                Override,                                                                             \
                BOOST_PP_IIF(has_virtual_result, VirtualResult,                                       \
                             boost::contract::detail::none),                                          \
                F,                                                                                    \
                Class BOOST_CONTRACT_DETAIL_TVARIADIC_COMMA(arity)                                    \
                ...
                BOOST_PP_IIF(has_virtual_result, r,                                                   \
                    ...
                    BOOST_CONTRACT_DETAIL_TVARIADIC_COMMA(                                            \
                        arity) BOOST_CONTRACT_DETAIL_TVARIADIC_ARGS_Z(z,                              \
                                                                      arity,                          \
                                                                      args)                           \
                        BOOST_CONTRACT_DETAIL_NO_TVARIADIC_COMMA(arity_compl)                         \
                            BOOST_CONTRACT_DETAIL_NO_TVARIADIC_ENUM_Z(                                \
...
public_function(virtual_ *v, F f, Class *obj, Args &... args);
...
template <class Override, typename VirtualResult, typename F, class Class,
          typename... Args>
specify_precondition_old_postcondition_except<VirtualResult>
public_function(virtual_ *v, VirtualResult &r, F f, Class *obj, Args &... args);
...
#define BOOST_CONTRACT_PUBLIC_FUNCTION_VIRTUAL_OVERRIDE_ARITY_(z, arity,       \
                                                               unused)         \
  BOOST_CONTRACT_PUBLIC_FUNCTION_VIRTUAL_OVERRIDES_(                           \
      z, arity, BOOST_PP_SUB(BOOST_CONTRACT_MAX_ARGS, arity), ~)

#define BOOST_CONTRACT_PUBLIC_FUNCTION_VIRTUAL_OVERRIDES_(z, arity,            \
                                                          arity_compl, unused) \
  BOOST_CONTRACT_PUBLIC_FUNCTION_VIRTUAL_OVERRIDE_Z_(                          \
      z, arity, arity_compl, /* has_virtual_result = */ 0)                     \
  BOOST_CONTRACT_PUBLIC_FUNCTION_VIRTUAL_OVERRIDE_Z_(                          \
      z, arity, arity_compl, /* has_virtual_result = */ 1)
