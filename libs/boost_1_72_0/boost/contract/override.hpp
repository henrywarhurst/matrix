...
#define BOOST_CONTRACT_OVERRIDE_CALL_BASE_(z, arity, arity_compl, func_name)   \
  template <class BOOST_CONTRACT_DETAIL_NAME1(B),                              \
            class BOOST_CONTRACT_DETAIL_NAME1(C)                               \
                BOOST_CONTRACT_DETAIL_TVARIADIC_COMMA(arity)                   \
                    BOOST_CONTRACT_DETAIL_TVARIADIC_TPARAMS_Z(                 \
                        z, arity, BOOST_CONTRACT_DETAIL_NAME1(Args))>          \
  static void BOOST_CONTRACT_DETAIL_NAME1(call_base)(                          \
      ...
      BOOST_CONTRACT_DETAIL_NAME1(C) *                                         \
          BOOST_CONTRACT_DETAIL_NAME1(                                         \
              obj) BOOST_CONTRACT_DETAIL_TVARIADIC_COMMA(arity)                \
              BOOST_CONTRACT_DETAIL_TVARIADIC_FPARAMS_Z(                       \
                  ...
                  BOOST_CONTRACT_DETAIL_NO_TVARIADIC_COMMA(arity_compl)        \
                      BOOST_CONTRACT_DETAIL_NO_TVARIADIC_ENUM_Z(               \
                          z, arity_compl, boost::contract::detail::none &)) {  \
    BOOST_CONTRACT_DETAIL_NAME1(obj)                                           \
        ->BOOST_CONTRACT_DETAIL_NAME1(B)::func_name(                           \
            BOOST_CONTRACT_DETAIL_TVARIADIC_ARGS_Z(                            \
                z, arity, BOOST_CONTRACT_DETAIL_NAME1(args))                   \
                BOOST_CONTRACT_DETAIL_TVARIADIC_COMMA(arity)                   \
...
#define BOOST_CONTRACT_OVERRIDE_CALL_BASE_ARITY_(z, arity, func_name)          \
  BOOST_CONTRACT_OVERRIDE_CALL_BASE_(                                          \
      z, arity, BOOST_PP_SUB(BOOST_CONTRACT_MAX_ARGS, arity), func_name)
#endif
...
#define BOOST_CONTRACT_NAMED_OVERRIDE(type_name, func_name)                    \
  struct type_name {                                                           \
    BOOST_CONTRACT_DETAIL_MIRROR_HAS_MEMBER_FUNCTION(                          \
        BOOST_CONTRACT_DETAIL_NAME1(has_member_function), func_name)           \
