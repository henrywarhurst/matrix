...
#define BOOST_CONTRACT_DETAIL_DECL_OVERRIDING_PUBLIC_FUNCTION_Z(               \
    z, arity, is_friend, has_result, O, VR, F, C, Args, v, r, f, obj, args)    \
  template <class O BOOST_PP_COMMA_IF(has_result)                              \
                BOOST_PP_EXPR_IIF(has_result, typename VR),                    \
            typename F,                                                        \
            class C BOOST_CONTRACT_DETAIL_TVARIADIC_COMMA(arity)               \
  ...
  boost::contract::specify_precondition_old_postcondition_except<              \
      BOOST_PP_EXPR_IIF(                                                       \
          has_result, VR)> /* no boost::contract:: here for friends (otherwise \
                              need fwd decl) */                                \
                           public_function(                                    \
                               boost::contract::virtual_ *v BOOST_PP_COMMA_IF( \
                                   has_result)                                 \
                                   BOOST_PP_EXPR_IIF(has_result, VR &r),       \
                               F f,                                            \
                               C *obj BOOST_CONTRACT_DETAIL_TVARIADIC_COMMA(   \
                                   arity)                                      \
                                   BOOST_CONTRACT_DETAIL_TVARIADIC_FPARAMS_Z(  \
                                       z, arity, Args, &, args))

#if BOOST_CONTRACT_DETAIL_TVARIADIC
#define BOOST_CONTRACT_DETAIL_DECL_FRIEND_OVERRIDING_PUBLIC_FUNCTIONS_Z(       \
    z, O, VR, F, C, Args, v, r, f, obj, args)                                  \
  BOOST_CONTRACT_DETAIL_DECL_OVERRIDING_PUBLIC_FUNCTION_Z(                     \
      z, ~, /* is_friend = */ 1, /* has_result = */ 0, O, VR, F, C, Args, v,   \
      r, f, obj, args);                                                        \
  BOOST_CONTRACT_DETAIL_DECL_OVERRIDING_PUBLIC_FUNCTION_Z(                     \
      z, ~, /* is_friend = */ 1, /* has_result = */ 1, O, VR, F, C, Args, v,   \
      r, f, obj, args);
#else
/* PRIVATE */
...
#define BOOST_CONTRACT_DETAIL_DECL_DETAIL_COND_SUBCONTRACTING_Z(               \
  ...
  template <class O, typename VR, typename F,                                  \
            class C BOOST_CONTRACT_DETAIL_TVARIADIC_COMMA(                     \
                BOOST_CONTRACT_MAX_ARGS)                                       \
                BOOST_CONTRACT_DETAIL_TVARIADIC_TPARAMS_Z(                     \
                    z, BOOST_CONTRACT_MAX_ARGS, Args)>                         \
  BOOST_PP_IIF(is_friend, friend class boost::contract::detail::, class)       \
  cond_subcontracting
