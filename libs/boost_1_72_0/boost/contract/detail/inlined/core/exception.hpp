...
template <failure_key Key> void default_handler() {
  std::string k = "";
  switch (Key) {
...
template <failure_key Key> void default_from_handler(from) {
  default_handler<Key>();
}
...
set_invariant_failure(from_failure_handler const &f) BOOST_NOEXCEPT_OR_NOTHROW {
