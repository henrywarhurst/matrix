...
#define BOOST_CONTRACT_DETAIL_COND_POST_DEF_(result_type, result_param,        \
                                             ftor_type, ftor_var, ftor_call)   \
public:                                                                        \
  template <typename F> void set_post(F const &f) { ftor_var = f; }            \
                                                                               \
protected:                                                                     \
  void check_post(result_type const &result_param) {                           \
    ...
    try {                                                                      \
      ...
      fail(&boost::contract::postcondition_failure);                           \
    }                                                                          \
  }                                                                            \
                                                                               \
private:                                                                       \
...
private:
  typedef typename boost::mpl::if_<
      ...
      boost::optional<typename boost::remove_reference<
          typename optional_value_type<VR>::type>::type const &> const &,
      VR const &>::type r_type;

  BOOST_CONTRACT_DETAIL_COND_POST_DEF_(
      r_type, r, void(r_type),
...
template <> class cond_post<none> : public cond_base { // Non-copyable base.
public:
  explicit cond_post(boost::contract::from from) : cond_base(from) {}
