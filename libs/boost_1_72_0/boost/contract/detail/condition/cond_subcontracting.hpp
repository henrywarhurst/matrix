...
BOOST_CONTRACT_DETAIL_DECL_DETAIL_COND_SUBCONTRACTING_Z(1,
                                                        /* is_friend = */ 0, O,
                                                        VR, F, C, Args)
    : public cond_inv<VR, C> { // Non-copyable base.
#ifndef BOOST_CONTRACT_NO_CONDITIONS
  template <class Class, typename Result = boost::mpl::vector<>>
  class overridden_bases_of {
    struct search_bases {
      typedef typename boost::mpl::fold<
          typename boost::contract::access::base_types_of<Class>::type, Result,
          ...
          boost::mpl::eval_if<
              ...
              boost::mpl::eval_if<
                  typename O::template BOOST_CONTRACT_DETAIL_NAME1(
                      has_member_function) < boost::mpl::_2,
                  typename member_function_types<C, F>::result_type,
                  typename member_function_types<C, F>::virtual_argument_types,
                  typename member_function_types<C, F>::property_tag>,
              boost::mpl::push_back<
  ...
  public:
    typedef typename boost::mpl::eval_if<
        boost::contract::access::has_base_types<Class>, search_bases,
        boost::mpl::identity<Result> // Return result (stop recursion).
        >::type type;
  };

  typedef typename boost::mpl::eval_if<
      boost::is_same<O, none>, boost::mpl::vector<>,
      overridden_bases_of<C>>::type overridden_bases;
...
public:
  explicit cond_subcontracting(
      boost::contract::from from, boost::contract::virtual_ * v, C * obj,
      VR &
          ...
          r
              ...
              BOOST_CONTRACT_DETAIL_TVARIADIC_COMMA(BOOST_CONTRACT_MAX_ARGS)
                  BOOST_CONTRACT_DETAIL_TVARIADIC_FPARAMS_Z(
                      1, BOOST_CONTRACT_MAX_ARGS, Args, &, args))
...
#ifndef BOOST_CONTRACT_NO_CONDITIONS
            BOOST_CONTRACT_DETAIL_TVARIADIC_COMMA(BOOST_CONTRACT_MAX_ARGS)
                BOOST_CONTRACT_DETAIL_TVARIADIC_TUPLE_INIT_Z(
                    1, BOOST_CONTRACT_MAX_ARGS, args_, args)
#endif
  {
#ifndef BOOST_CONTRACT_NO_CONDITIONS
    if (v) {
      ...
      if (!boost::mpl::empty<overridden_bases>::value) {
        v_ =
            new boost::contract::virtual_(boost::contract::virtual_::no_action);
#ifndef BOOST_CONTRACT_NO_POSTCONDITIONS
        v_->result_ptr_ = &r_;
...
protected:
  ...
  void check_subcontracted_pre() {
    exec_or(boost::contract::virtual_::check_pre,
...
private:
  ...
  void check_virtual_post() {
    ...
    typedef typename boost::remove_reference<
        typename optional_value_type<VR>::type>::type r_type;
    boost::optional<r_type const &> r; // No result copy in this code.
    if (!base_call_)
      r = optional_get(r_);
    else if (v_->result_optional_) {
      try {
        r = **boost::any_cast<boost::optional<r_type> *>(v_->result_ptr_);
      ...
      try {
        ...
        try {
          throw boost::contract::bad_virtual_result_cast(v_->result_type_name_,
                                                         typeid(r_type).name());
        } catch (...) {
          this->fail(&boost::contract::postcondition_failure);
        }
  ...
  void exec_or( // Execute action in short-circuit logic-or with bases.
    ...
    if (!base_call_ || v_->action_ == a) {
      ...
      bool checked =
          f ? (this->*f)(/* throw_on_failure = */ base_call_) : false;
      if (base_call_) {
        if (!checked) {
  ...
  exec_or_bases() {
    if (boost::mpl::empty<Bases>::value)
      return false;
    try {
      call_base (*this)(typename boost::mpl::front<Bases>::type());
    } catch (cond_subcontracting_::signal_not_checked const &) {
      return exec_or_bases<typename boost::mpl::pop_front<Bases>::type>();
    } catch (...) {
      bool checked = false;
      try {
        checked = exec_or_bases<typename boost::mpl::pop_front<Bases>::type>();
      } catch (...) {
        checked = false;
      }
      if (!checked)
  ...
  public:
    ...
    template <class B> void operator()(B *) {
      ...
      try {
        call<B>(BOOST_CONTRACT_DETAIL_TVARIADIC_TUPLE_INDEXES_OF(Args));
  ...
  private:
    ...
    void call(BOOST_CONTRACT_DETAIL_TVARIADIC_TUPLE_INDEXES_FPARAM(I)) {
      O::template BOOST_CONTRACT_DETAIL_NAME1(call_base)<B>(
          me_.v_, me_.object() BOOST_CONTRACT_DETAIL_TVARIADIC_COMMA(
                      BOOST_CONTRACT_MAX_ARGS)
                      BOOST_CONTRACT_DETAIL_TVARIADIC_TUPLE_ELEMS_Z(
                          1, BOOST_CONTRACT_MAX_ARGS, I, me_.args_));
    }
  ...
  bool base_call_;
  BOOST_CONTRACT_DETAIL_TVARIADIC_TUPLE_Z(1, BOOST_CONTRACT_MAX_ARGS, Args, &,
                                          args_)
#endif
};
