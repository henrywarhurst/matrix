...
#define BOOST_CONTRACT_DETAIL_MIRROR_END_(tparam)                              \
  template <typename> static boost::contract::detail::mirror::no &apply(...);  \
                                                                               \
public:                                                                        \
...
#define BOOST_CONTRACT_DETAIL_MIRROR_HAS_MEMBER_FUNCTION_(is_static, trait,    \
                                                          func_name)           \
  template <typename BOOST_CONTRACT_DETAIL_NAME1(T),                           \
            typename BOOST_CONTRACT_DETAIL_NAME1(R),                           \
            class BOOST_CONTRACT_DETAIL_NAME1(P),                              \
            class BOOST_CONTRACT_DETAIL_NAME1(G) =                             \
                boost::function_types::null_tag>                               \
  class trait {                                                                \
    template <class BOOST_CONTRACT_DETAIL_NAME1(C)>                            \
    static boost::contract::detail::mirror::yes &                              \
    apply(boost::contract::detail::mirror::check_function<                     \
              typename BOOST_PP_IIF(                                           \
                  is_static, boost::function_types::function_pointer,          \
                  ...
                  typename boost::mpl::push_front<                             \
                      BOOST_PP_IIF(is_static,                                  \
                          ...
                          typename boost::mpl::push_front<                     \
                              BOOST_CONTRACT_DETAIL_NAME1(P),                  \
                              BOOST_CONTRACT_DETAIL_NAME1(C)>::type),          \
                      BOOST_CONTRACT_DETAIL_NAME1(R)>::type,                   \
              BOOST_CONTRACT_DETAIL_NAME1(G)>::type,                           \
          &BOOST_CONTRACT_DETAIL_NAME1(C)::func_name > *);                     \
    BOOST_CONTRACT_DETAIL_MIRROR_END_(BOOST_CONTRACT_DETAIL_NAME1(T))          \
  };

/* PUBLIC */

#define BOOST_CONTRACT_DETAIL_MIRROR_HAS_TYPE(trait, type_name)                \
  template <typename BOOST_CONTRACT_DETAIL_NAME1(T)> class trait {             \
    template <class BOOST_CONTRACT_DETAIL_NAME1(C)>                            \
    static boost::contract::detail::mirror::yes &                              \
    apply(typename BOOST_CONTRACT_DETAIL_NAME1(C)::type_name *);               \
    BOOST_CONTRACT_DETAIL_MIRROR_END_(BOOST_CONTRACT_DETAIL_NAME1(T))          \
  };

#define BOOST_CONTRACT_DETAIL_MIRROR_HAS_MEMBER_FUNCTION(trait, func_name)     \
  BOOST_CONTRACT_DETAIL_MIRROR_HAS_MEMBER_FUNCTION_(/* is_static = */ 0,       \
                                                    trait, func_name)

#define BOOST_CONTRACT_DETAIL_MIRROR_HAS_STATIC_MEMBER_FUNCTION(trait,         \
                                                                func_name)     \
  BOOST_CONTRACT_DETAIL_MIRROR_HAS_MEMBER_FUNCTION_(/* is_static = */ 1,       \
                                                    trait, func_name)
