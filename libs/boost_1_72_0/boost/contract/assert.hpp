...
// away). It cannot an empty code block `{}`, etc. otherwise code like
// `if(...) ASSERT(...); else ASSERT(...);` won't work when NO_ALL.
#define BOOST_CONTRACT_ASSERT(cond) BOOST_CONTRACT_DETAIL_NOOP
#endif
...
#define BOOST_CONTRACT_ASSERT_AUDIT(cond) BOOST_CONTRACT_ASSERT(cond)
#else
#define BOOST_CONTRACT_ASSERT_AUDIT(cond) BOOST_CONTRACT_DETAIL_NOEVAL(cond)
#endif
...
*/
#define BOOST_CONTRACT_ASSERT_AXIOM(cond) BOOST_CONTRACT_DETAIL_NOEVAL(cond)
