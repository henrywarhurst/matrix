...
#define BOOST_CONTRACT_BASE_TYPES_REMOVE_VIRTUAL_(base)                        \
  BOOST_PP_EXPAND(                                                             \
      BOOST_PP_IIF(BOOST_CONTRACT_DETAIL_PP_KEYWORD_IS_VIRTUAL(base),          \
                   ...
                   BOOST_PP_TUPLE_REM(1))(base))

#define BOOST_CONTRACT_BASE_TYPES_PUSH_BACK_IF_(is_public, types_nilseq, base) \
  (is_public,                                                                  \
   BOOST_PP_IIF(is_public, BOOST_PP_SEQ_PUSH_BACK,                             \
                types_nilseq BOOST_PP_TUPLE_EAT(2))(types_nilseq, base))
...
#define BOOST_CONTRACT_BASE_TYPES_ACCESS_(is_public, types_nilseq, base)       \
  BOOST_PP_IIF(                                                                \
      BOOST_CONTRACT_DETAIL_PP_KEYWORD_IS_PUBLIC(base),                        \
      BOOST_CONTRACT_BASE_TYPES_PUSH_BACK_PUBLIC_,                             \
      BOOST_PP_IIF(                                                            \
          BOOST_CONTRACT_DETAIL_PP_KEYWORD_IS_PROTECTED(base),                 \
...
#define BOOST_CONTRACT_BASE_TYPES_RETURN_(types_nilseq)                        \
  BOOST_PP_IIF(BOOST_PP_EQUAL(BOOST_PP_SEQ_SIZE(types_nilseq), 1),             \
               BOOST_PP_TUPLE_EAT(1), BOOST_CONTRACT_BASE_TYPES_RETURN_YES_)   \
  (types_nilseq)

#define BOOST_CONTRACT_BASE_TYPES_OK_(base_tuple, bases_seq)                   \
  boost::mpl::vector<BOOST_CONTRACT_BASE_TYPES_RETURN_(BOOST_PP_TUPLE_ELEM(    \
      2, 1,                                                                    \
      BOOST_PP_SEQ_FOLD_LEFT(BOOST_CONTRACT_BASE_TYPES_, (0, (BOOST_PP_NIL)),  \
                             bases_seq)))>

#define BOOST_CONTRACT_BASE_TYPES_ERR_(bases_tuple, bases_seq)                       \
  BOOST_CONTRACT_ERROR_all_bases_must_explicitly_specify_public_protected_or_private \
      base_tuple

#define BOOST_CONTRACT_BASE_TYPES_IS_ACCESS_(base)                             \
  BOOST_PP_IIF(                                                                \
      BOOST_CONTRACT_DETAIL_PP_KEYWORD_IS_PUBLIC(base), 1,                     \
      BOOST_PP_IIF(                                                            \
          BOOST_CONTRACT_DETAIL_PP_KEYWORD_IS_PROTECTED(base), 1,              \
          BOOST_PP_IIF(BOOST_CONTRACT_DETAIL_PP_KEYWORD_IS_PRIVATE(base), 1,   \
                       0)))
...
#define BOOST_CONTRACT_BASE_TYPES_CHECK_(bases_tuple, bases_seq)               \
  BOOST_PP_IIF(BOOST_CONTRACT_BASE_TYPES_IS_ACCESS_(                           \
                   BOOST_CONTRACT_BASE_TYPES_REMOVE_VIRTUAL_(                  \
                       BOOST_PP_SEQ_HEAD(bases_seq))),                         \
               BOOST_CONTRACT_BASE_TYPES_OK_, BOOST_CONTRACT_BASE_TYPES_ERR_)  \
