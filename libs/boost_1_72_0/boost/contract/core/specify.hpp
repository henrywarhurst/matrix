...
template <typename VR> class specify_precondition_old_postcondition_except;

template <typename VR> class specify_old_postcondition_except;

template <typename VR> class specify_postcondition_except;
...
private:
  ...
  template <typename VR> friend class specify_old_postcondition_except;

  template <typename VR> friend class specify_postcondition_except;
...
public:
  ...
  */
  template <typename F>
  specify_nothing except(F const &
...
public:
  ...
  */
  template <typename F>
  specify_except postcondition(F const &
...
private:
  BOOST_CONTRACT_SPECIFY_CLASS_IMPL_(
      ...
      boost::contract::detail::cond_post<
          typename boost::contract::detail::none_if_void<VirtualResult>::type>)
...
#if !defined(BOOST_CONTRACT_NO_OLDS) || defined(BOOST_CONTRACT_DETAIL_DOXYGEN)
                                                      f
#endif // Else, no name (avoid unused param warning).
  ) {
...
private:
  ...
  template <class C> friend specify_old_postcondition_except<> constructor(C *);

  template <class C> friend specify_old_postcondition_except<> destructor(C *);
  /** @endcond */
};
...
@tparam VirtualResult   Return type of the enclosing function declaring the
                        ...
                        specified by programmers.)
*/
template <typename VirtualResult /* = void (already in fwd decl from decl.hpp)
                                  */
...
private:
  ...
  public_function(virtual_ *, C *);

  template <typename VR, class C>
  friend specify_precondition_old_postcondition_except<VR>
  public_function(virtual_ *, VR &, C *);

  BOOST_CONTRACT_DETAIL_DECL_FRIEND_OVERRIDING_PUBLIC_FUNCTIONS_Z(1, O, VR, F,
                                                                  C, Args, v, r,
                                                                  f, obj, args)
  /** @endcond */
};
