...
*/
template <typename T> class old_ptr { /* copyable (as *) */
public:
...
*/
template <typename T> class old_ptr_if_copyable { /* copyable (as *) */
public:
  /** Pointed old value type. */
  typedef T element_type;
...
public:
  ...
  */
  ...
  /* implicit */ old_value(
      ...
      ,
      typename boost::enable_if<boost::contract::is_old_value_copyable<T>>::type
          * = 0)
#ifndef BOOST_CONTRACT_NO_OLDS
...
private:
  ...
  friend BOOST_CONTRACT_DETAIL_DECLSPEC old_pointer make_old(old_value const &);

  friend BOOST_CONTRACT_DETAIL_DECLSPEC old_pointer make_old(virtual_ *,
                                                             old_value const &);
  /** @endcond */
};
