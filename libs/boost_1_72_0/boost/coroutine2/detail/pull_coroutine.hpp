...
private:
  template <typename X> friend class push_coroutine;
...
public:
  ...
  public:
    ...
    explicit iterator(pull_coroutine<T> *c) noexcept : c_{c} { fetch_(); }
    ...
    void operator++(int) { increment_(); }

    reference_t operator*() const noexcept { return c_->cb_->get(); }
...
template <typename T> class pull_coroutine<T &> {
private:
  template <typename X> friend class push_coroutine;
...
template <typename T>
typename pull_coroutine<T>::iterator begin(pull_coroutine<T> &c) {
  return typename pull_coroutine<T>::iterator(&c);
}

template <typename T>
typename pull_coroutine<T>::iterator end(pull_coroutine<T> &) {
