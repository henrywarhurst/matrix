...
enum period_mark {
  ...
  day_of_week,   ///< Day of week, Sunday=1, Monday=2,..., Saturday=7.
               ///< Note that updating this value respects local day of week, so
               ///< for example, If first day of week is Monday and the current
               ///< day is Tuesday then setting the value to Sunday (1) would
               ///< forward the date by 5 days forward and not backward by two
               ///< days as it could be expected if the numbers were taken as
               ///< is.
  day_of_week_in_month, ///< Original number of the day of the week in month.
                        ///< For example 1st Sunday, 2nd Sunday, etc. in
                        ///< Gregorian [1..5]
  day_of_week_local, ///< Local day of week, for example in France Monday is 1,
  ...
  first_day_of_week, ///< First day of week, constant, for example Sunday in US
...
public:
  ...
  ///
  typedef enum {
    absolute_minimum, ///< Absolute possible minimum for the value, for example
                      ///< for day is 1
    ...
    least_maximum, ///< The last maximal value for this period, For example for
                   ///< Gregorian calendar day it is 28
    actual_maximum,   ///< Actual maximum, for it can be 28, 29, 30, 31 for day
  ...
  ///
  /// After call of set_value you may want to call normalize() function to make
  /// sure vall periods are updated, if you set sereral fields that are part of
  /// single date/time representation you should call set_value several times
  /// and then call normalize().
  ///
  ...
  ///
  /// Recalculate all periods after setting them, should be called after use of
  /// set_value() function.
  ///
