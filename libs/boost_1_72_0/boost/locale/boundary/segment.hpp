...
int compare_text(LeftIterator l_begin, LeftIterator l_end,
                 RightIterator r_begin, RightIterator r_end) {
...
///
/// \brief a segment object that represents a pair of two iterators that define
/// the range where this segment exits and a rule that defines it.
///
/// This type of object is dereferenced by the iterators of segment_index. Using
/// a rule() member function you can get a specific rule this segment was
/// selected with. For example, when you use word boundary analysis, you can
/// check if the specific word contains Kana letters by checking (rule() & \ref
/// word_kana)!=0 For a sentence analysis you can check if the sentence is
/// selected because a sentence terminator is found (\ref sentence_term) or
/// there is a line break (\ref sentence_sep).
///
/// This object can be automatically converted to std::basic_string with the
/// same type of character. It is also valid range that has begin() and end()
/// member functions returning iterators on the location of the segment.
///
/// \see
///
/// - \ref segment_index
...
public:
  ...
  segment(iterator b, iterator e, rule_type r)
      : std::pair<IteratorType, IteratorType>(b, e), rule_(r) {}
  ///
...
template <typename CharType, typename Traits, typename Alloc,
          typename IteratorR>
bool operator==(std::basic_string<CharType, Traits, Alloc> const &l,
...
bool operator==(segment<Iterator> const &l,
...
typedef segment<std::wstring::const_iterator>
    wssegment; ///< convenience typedef
#ifdef BOOST_LOCALE_ENABLE_CHAR16_T
typedef segment<std::u16string::const_iterator>
    u16ssegment; ///< convenience typedef
#endif
#ifdef BOOST_LOCALE_ENABLE_CHAR32_T
typedef segment<std::u32string::const_iterator>
    u32ssegment; ///< convenience typedef
#endif
