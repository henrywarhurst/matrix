...
public:
  ...
  ///
  /// Set a flags that define a way for format data like number, spell, currency
  /// etc.
  ///
  ...
  ///
  /// Get a flags that define a way for format data like number, spell, currency
  /// etc.
  ///
...
private:
  ...
  public:
    ...
    template <typename Char> void set(Char const *s) {
      delete[] ptr;
      ptr = 0;
      type = &typeid(Char);
      Char const *end = s;
    ...
    template <typename Char> std::basic_string<Char> get() const {
...
/// \cond INTERNAL
namespace details {
template <typename CharType> struct add_ftime {
