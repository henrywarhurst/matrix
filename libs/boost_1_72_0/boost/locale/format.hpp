...
template <typename CharType> struct formattible {
  ...
  template <typename Type> formattible(Type const &value) {
    pointer_ = static_cast<void const *>(&value);
    writer_ = &write<Type>;
  }

  template <typename Type> formattible const &operator=(Type const &other) {
...
///
template <typename CharType> class basic_format {
public:
