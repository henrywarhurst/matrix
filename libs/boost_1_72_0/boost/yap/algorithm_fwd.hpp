...
template <long long I> struct placeholder : hana::llong<I> {};
...
template <typename Expr, typename = detail::void_t<>,
...
struct is_expr<Expr,
    ...
    : std::integral_constant<
          bool, std::is_same<std::remove_cv_t<decltype(
                                 detail::remove_cv_ref_t<Expr>::kind)>,
                             expr_kind>::value &&
                    hana::is_a<hana::tuple_tag,
...
template <template <expr_kind, class> class expr_template, typename T>
using expression_ref = expr_template<expr_kind::expr_ref,
...
template <typename Expr> decltype(auto) value(Expr &&expr);
