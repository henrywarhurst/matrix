...
template <typename T> struct partial_decay { using type = T; };

template <typename T> struct partial_decay<T[]> { using type = T *; };
template <typename T, std::size_t N> struct partial_decay<T[N]> {
  using type = T *;
};
...
template <typename R, typename... A> struct partial_decay<R(A...)> {
  using type = R (*)(A...);
};
template <typename R, typename... A> struct partial_decay<R(A..., ...)> {
  using type = R (*)(A..., ...);
...
template <typename T, typename U = typename detail::partial_decay<T>::type,
          bool AddRValueRef =
              std::is_same<T, U>::value && !std::is_const<U>::value>
...
template <template <expr_kind, class> class ExprTemplate, typename T,
          typename U, bool RemoveRefs, bool IsLRef>
struct operand_type<ExprTemplate, T, U, RemoveRefs, true, IsLRef> {
  using type = remove_cv_ref_t<T>;
};

template <template <expr_kind, class> class ExprTemplate, typename T,
          typename U, bool IsLRef>
...
template <typename T> struct make_operand {
  template <typename U> auto operator()(U &&u) {
...
template <template <expr_kind, class> class ExprTemplate, expr_kind OpKind,
...
struct ternary_op_result<ExprTemplate, T, U, V, true> {
  ...
  using type = ExprTemplate<expr_kind::if_else,
...
template <template <expr_kind, class> class ExprTemplate, typename T,
          typename U, typename V, template <class> class UdtTrait,
...
template <template <expr_kind, class> class ExprTemplate, expr_kind OpKind,
          typename T, template <class> class UdtTrait,
          bool Valid =
...
template <typename T, template <class> class UdtTrait> struct is_udt_arg {
  static bool const value =
      !is_expr<T>::value && UdtTrait<remove_cv_ref_t<T>>::value;
};

template <template <expr_kind, class> class ExprTemplate, expr_kind OpKind,
          typename T, typename U, template <class> class TUdtTrait,
...
template <template <expr_kind, class> class ExprTemplate, expr_kind OpKind,
          typename T, typename U, template <class> class TUdtTrait,
          template <class> class UUdtTrait>
struct udt_udt_binary_op_result<ExprTemplate, OpKind, T, U, TUdtTrait,
                                UUdtTrait, true> {
  using lhs_type = operand_type_t<ExprTemplate, T>;
...
using udt_udt_binary_op_result_t =
    typename udt_udt_binary_op_result<ExprTemplate, OpKind, T, U, TUdtTrait,
                                      UUdtTrait>::type;

// udt_any_binary_op_result

template <template <expr_kind, class> class ExprTemplate, expr_kind OpKind,
          typename T, typename U, template <class> class UdtTrait,
          bool Valid = !is_expr<T>::value && !is_expr<U>::value &&
                       (UdtTrait<remove_cv_ref_t<T>>::value ||
                        UdtTrait<remove_cv_ref_t<U>>::value)>
...
using udt_any_binary_op_result_t =
    typename udt_any_binary_op_result<ExprTemplate, OpKind, T, U,
                                      UdtTrait>::type;
...
template <typename T> struct copy_or_move<T, T const &> : std::true_type {};

template <typename T> struct copy_or_move<T, T &> : std::true_type {};
...
template <expr_kind Kind> constexpr expr_arity arity_of() {
  switch (Kind) {
