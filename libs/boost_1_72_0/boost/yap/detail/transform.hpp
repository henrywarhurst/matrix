...
template <int I, typename T, typename... Ts> struct nth_element_impl {
...
template <typename T> struct rvalue_ref_to_value<T, true> {
  using type = typename std::remove_reference<T>::type;
};

template <typename T> struct rvalue_ref_to_value<T, false> { using type = T; };

template <typename T>
using rvalue_ref_to_value_t = typename rvalue_ref_to_value<T>::type;

template <bool IsRvalueRef> struct rvalue_mover {
  template <typename T> constexpr decltype(auto) operator()(T &&t) const {
    return static_cast<T &&>(t);
  }
};

template <> struct rvalue_mover<true> {
  template <typename T>
...
template <typename... PlaceholderArgs> struct placeholder_transform_t {
  ...
  constexpr decltype(auto) operator()(expr_tag<expr_kind::terminal>,
                                      boost::yap::placeholder<I>) const {
    static_assert(I <= decltype(hana::size(std::declval<tuple_t>()))::value,
...
template <typename... PlaceholderArgs> struct evaluation_transform_t {
  using tuple_t = hana::tuple<rvalue_ref_to_value_t<PlaceholderArgs>...>;
...
#define BOOST_YAP_UNARY_OPERATOR_CASE(op, op_name)                             \
  template <typename T>                                                        \
  constexpr decltype(auto) operator()(expr_tag<expr_kind::op_name>, T &&t)     \
      const {                                                                  \
  ...
  template <typename T>
  constexpr decltype(auto) operator()(expr_tag<expr_kind::post_inc>,
                                      T &&t) const {
    return transform(as_expr<minimal_expr>(static_cast<T &&>(t)), *this)++;
  }
  template <typename T>
  constexpr decltype(auto) operator()(expr_tag<expr_kind::post_dec>,
                                      T &&t) const {
    return transform(as_expr<minimal_expr>(static_cast<T &&>(t)), *this)--;
  }
...
//]
  ...
  template <typename T, typename U>
  constexpr decltype(auto) operator()(expr_tag<expr_kind::subscript>, T &&t,
                                      U &&u) const {
    return transform(
...
#undef BOOST_YAP_BINARY_OPERATOR_CASE
  ...
  constexpr decltype(auto) operator()(expr_tag<expr_kind::if_else>, T &&t,
                                      U &&u, V &&v) const {
    return transform(as_expr<minimal_expr>(static_cast<T &&>(t)), *this)
...
template <bool Strict, int I, bool IsExprRef> struct transform_impl;

template <bool Strict, typename Expr, typename TransformTuple, int I,
          expr_arity Arity, typename = void_t<>>
...
struct default_transform {
  ...
  constexpr decltype(auto) operator()(Expr &&expr,
                                      TransformTuple transforms) const {
...
template <expr_kind Kind, template <expr_kind, class> class ExprTemplate,
          typename OldTuple, typename NewTuple>
constexpr auto make_expr_from_tuple(ExprTemplate<Kind, OldTuple> const &expr,
                                    NewTuple &&tuple) {
...
constexpr decltype(auto) transform_nonterminal(Expr const &expr, Tuple &&tuple,
  ...
  auto transformed_tuple =
      hana::transform(static_cast<Tuple &&>(tuple), [&](auto &&element) {
        ...
        ::boost::yap::detail::transform_impl<false, 0,
...
template <> struct default_transform<true, false, false> {
  template <typename Expr, typename TransformTuple>
  constexpr decltype(auto) operator()(Expr &&expr,
                                      TransformTuple transforms) const {
...
template <> struct default_transform<false, false, false> {
  template <typename Expr, typename TransformTuple>
...
struct next_or_default_transform<Strict, Expr, TransformTuple, I, false> {
  constexpr decltype(auto) operator()(Expr &&expr,
    ...
    constexpr expr_kind kind = remove_cv_ref_t<Expr>::kind;
    return default_transform<std::is_lvalue_reference<Expr>::value,
                             kind == expr_kind::terminal, Strict>{}(
        static_cast<Expr &&>(expr), transforms);
  }
};
...
template <typename T> decltype(auto) terminal_value(T &&x) {
  return value_impl<true>(static_cast<T &&>(x));
}

template <bool Strict, typename Expr, typename TransformTuple, int I>
struct transform_expression_tag<
    Strict, Expr, TransformTuple, I, expr_arity::one,
    void_t<decltype((*std::declval<TransformTuple>()[hana::llong<I>{}])(
...
template <bool Strict, typename Expr, typename TransformTuple, int I>
struct transform_expression_tag<
    Strict, Expr, TransformTuple, I, expr_arity::two,
    void_t<decltype((*std::declval<TransformTuple>()[hana::llong<I>{}])(
        expr_tag<remove_cv_ref_t<Expr>::kind>{},
...
template <bool Strict, typename Expr, typename TransformTuple, int I>
struct transform_expression_tag<
    Strict, Expr, TransformTuple, I, expr_arity::three,
    void_t<decltype((*std::declval<TransformTuple>()[hana::llong<I>{}])(
        expr_tag<remove_cv_ref_t<Expr>::kind>{},
...
template <typename Expr, typename Transform> struct transform_call_unpacker {
  template <long long... I>
  constexpr auto operator()(Expr &&expr, Transform &transform,
                            std::integer_sequence<long long, I...>) const
      -> decltype(
          transform(expr_tag<expr_kind::call>{},
                    terminal_value(::boost::yap::get(static_cast<Expr &&>(expr),
                                                     hana::llong_c<I>))...)) {
    return transform(expr_tag<expr_kind::call>{},
                     terminal_value(::boost::yap::get(
                         static_cast<Expr &&>(expr), hana::llong_c<I>))...);
  }
};

template <typename Expr> constexpr auto indices_for(Expr const &expr) {
...
template <bool Strict, typename Expr, typename TransformTuple, int I>
struct transform_expression_tag<
    Strict, Expr, TransformTuple, I, expr_arity::n,
    void_t<decltype(transform_call_unpacker<
                    Expr, decltype(*std::declval<
                                   TransformTuple>()[hana::llong<I>{}])>{}(
        std::declval<Expr>(), *std::declval<TransformTuple>()[hana::llong<I>{}],
  ...
  constexpr decltype(auto) operator()(Expr &&expr,
    ...
    return transform_call_unpacker<Expr, transform_t>{}(
        static_cast<Expr &&>(expr), *transforms[hana::llong<I>{}],
        indices_for(expr));
  }
};
...
template <bool Strict, int I, bool IsExprRef> struct transform_impl {
  ...
  constexpr decltype(auto) operator()(Expr &&expr,
    ...
    return detail::transform_expression_tag<Strict, Expr, TransformTuple, I,
                                            detail::arity_of<kind>()>{}(
        static_cast<Expr &&>(expr), transforms);
  }
};

template <bool Strict, int I> struct transform_impl<Strict, I, true> {
  template <typename Expr, typename TransformTuple>
  constexpr decltype(auto) operator()(Expr &&expr,
                                      TransformTuple transforms) const {
