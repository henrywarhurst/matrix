...
template <typename Expr, bool MutableRvalueRef> struct deref_impl {
  decltype(auto) operator()(Expr &&expr) {
    return std::move(*expr.elements[hana::llong_c<0>]);
  }
};

template <typename Expr> struct deref_impl<Expr, false> {
  decltype(auto) operator()(Expr &&expr) {
...
/** "Dereferences" a reference-expression, forwarding its referent to
   the caller. */
template <typename Expr> decltype(auto) deref(Expr &&expr) {
  static_assert(is_expr<Expr>::value,
                "deref() is only defined for expressions.");

  static_assert(detail::remove_cv_ref_t<Expr>::kind == expr_kind::expr_ref,
  ...
  if constexpr (std::is_rvalue_reference<Expr>::value &&
                !std::is_const<std::remove_reference_t<Expr>>::value) {
...
struct lvalue_ref_ith_element : std::is_lvalue_reference<decltype(
...
template <typename T, bool IsExprRef, bool ValueOfTerminalsOnly, bool TakeValue,
          bool IsLvalueRef>
struct value_expr_impl;

template <typename T, bool ValueOfTerminalsOnly, bool TakeValue,
          bool IsLvalueRef>
...
struct value_expr_impl<T, false, ValueOfTerminalsOnly, true, true> {
  decltype(auto) operator()(T &&x) { return x.elements[hana::llong_c<0>]; }
};
  ...
  decltype(auto) operator()(T &&x) {
    ...
    return value_expr_impl < T, kind == expr_kind::expr_ref,
           ...
           std::is_lvalue_reference<T>::value ||
               detail::lvalue_ref_ith_element<decltype(x.elements), 0>::value >
                   {}(static_cast<T &&>(x));
  }
};
...
struct get_impl<I, Expr, true, IsLvalueRef> {
  decltype(auto) operator()(Expr &&expr, hana::llong<I> i) {
    return ::boost::yap::get(::boost::yap::deref(static_cast<Expr &&>(expr)),
                             i);
  }
};

template <long long I, typename Expr> struct get_impl<I, Expr, false, true> {
  decltype(auto) operator()(Expr &&expr, hana::llong<I> i) {
    return expr.elements[i];
  }
};

template <long long I, typename Expr> struct get_impl<I, Expr, false, false> {
  decltype(auto) operator()(Expr &&expr, hana::llong<I> i) {
...
decltype(auto) get(Expr &&expr, I const &i) {
  ...
  static_assert(kind == expr_kind::expr_ref ||
                    (0 <= I::value &&
...
#ifdef BOOST_NO_CONSTEXPR_IF
  return detail::get_impl<I::value, Expr, kind == expr_kind::expr_ref,
...
template <long long I, typename Expr> decltype(auto) get_c(Expr &&expr) {
...
/** Returns the left operand in a binary operator expression.
    ...
    operator expression.
*/
template <typename Expr> decltype(auto) left(Expr &&expr) {
  using namespace hana::literals;
...
/** Returns the right operand in a binary operator expression.
    ...
    operator expression.
*/
template <typename Expr> decltype(auto) right(Expr &&expr) {
  using namespace hana::literals;
...
*/
template <typename Expr> decltype(auto) cond(Expr &&expr) {
  using namespace hana::literals;
  return ::boost::yap::get(static_cast<Expr &&>(expr), 0_c);
  constexpr expr_kind kind = detail::remove_cv_ref_t<Expr>::kind;
  static_assert(kind == expr_kind::expr_ref || kind == expr_kind::if_else,
...
/** Returns the then-expression in an if_else expression.
    ...
    <code>expr_kind::if_else</code> expression.
*/
template <typename Expr> decltype(auto) then(Expr &&expr) {
  using namespace hana::literals;
  return ::boost::yap::get(static_cast<Expr &&>(expr), 1_c);
  constexpr expr_kind kind = detail::remove_cv_ref_t<Expr>::kind;
  static_assert(kind == expr_kind::expr_ref || kind == expr_kind::if_else,
...
/** Returns the else-expression in an if_else expression.
    ...
    <code>expr_kind::if_else</code> expression.
*/
template <typename Expr> decltype(auto) else_(Expr &&expr) {
  using namespace hana::literals;
...
*/
template <typename Expr> decltype(auto) callable(Expr &&expr) {
...
auto make_expression(T &&... t) {
  constexpr detail::expr_arity arity = detail::arity_of<Kind>();
  static_assert((arity == detail::expr_arity::one && sizeof...(T) == 1) ||
...
template <typename Expr> struct expression_function {
  template <typename... U> decltype(auto) operator()(U &&... u) {
...
template <expr_kind Kind, typename Tuple> struct expression_function_expr {
...
*/
template <typename Expr> auto make_expression_function(Expr &&expr) {
  static_assert(is_expr<Expr>::value,
...
*/
template <typename... T> auto replacements(T &&... t) {
...
/** Returns a transform object that evaluates an expression using the
    ...
    given values.
*/
template <typename... T> inline auto evaluation(T &&... t) {
...
template <bool Strict> struct transform_ {
  template <typename Expr, typename Transform, typename... Transforms>
  constexpr decltype(auto) operator()(Expr &&expr, Transform &transform,
...
*/
template <typename Expr, typename Transform, typename... Transforms>
constexpr decltype(auto) transform(Expr &&expr, Transform &&transform,
                                   Transforms &&... transforms) {
  static_assert(is_expr<Expr>::value,
                "transform() is only defined for expressions.");
  return detail::transform_<false>{}(static_cast<Expr &&>(expr), transform,
                                     transforms...);
}
...
*/
template <typename Expr, typename Transform, typename... Transforms>
decltype(auto) transform_strict(Expr &&expr, Transform &&transform,
                                Transforms &&... transforms) {
  static_assert(is_expr<Expr>::value,
                "transform() is only defined for expressions.");
  return detail::transform_<true>{}(static_cast<Expr &&>(expr), transform,
                                    transforms...);
}
