...
/** Terminal expression specialization of the reference expression
    ...
    \endcode
*/
template <typename T> struct expression<expr_kind::terminal, hana::tuple<T>> {
...
BOOST_YAP_USER_BINARY_OPERATOR(shift_right_assign, expression,
                               expression)                                // >>=
...
template <typename T> constexpr auto make_terminal(T &&t) {
  return make_terminal<expression>(static_cast<T &&>(t));
}

/** Returns <code>as_expr<boost::yap::expression>(t)</code>. */
template <typename T> constexpr decltype(auto) as_expr(T &&t) {
