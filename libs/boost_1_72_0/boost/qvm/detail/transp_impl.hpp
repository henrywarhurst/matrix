...
template <class OriginalMatrix> class transposed_ {
...
public:
  template <class T>
  BOOST_QVM_INLINE_TRIVIAL transposed_ &operator=(T const &x) {
    assign(*this, x);
    return *this;
  }
...
struct mat_traits<qvm_detail::transposed_<OriginalMatrix>> {
  ...
  read_element_idx(int row, int col, this_matrix const &x) {
    ...
    return mat_traits<OriginalMatrix>::read_element_idx(
        col, row, reinterpret_cast<OriginalMatrix const &>(x));
  }
  ...
  write_element_idx(int row, int col, this_matrix &x) {
    ...
    return mat_traits<OriginalMatrix>::write_element_idx(
        col, row, reinterpret_cast<OriginalMatrix &>(x));
  }
};
...
template <class OriginalMatrix, int R, int C>
struct deduce_mat2<qvm_detail::transposed_<OriginalMatrix>,
                   qvm_detail::transposed_<OriginalMatrix>, R, C> {
  typedef mat<typename mat_traits<OriginalMatrix>::scalar_type, R, C> type;
};
} // namespace qvm
} // namespace boost
