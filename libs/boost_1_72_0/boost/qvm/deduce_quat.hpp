...
template <class Q> struct deduce_quat {
  BOOST_QVM_STATIC_ASSERT(is_quat<Q>::value);
  typedef Q type;
};

namespace qvm_detail {
template <class A, class B, bool QA = is_quat<A>::value,
          bool QB = is_quat<B>::value>
struct deduce_quat2_default {
  typedef quat<typename deduce_scalar<typename scalar<A>::type,
                                      typename scalar<B>::type>::type>
      type;
};

template <class Q> struct deduce_quat2_default<Q, Q, true, true> {
  typedef Q type;
};
} // namespace qvm_detail

template <class A, class B> struct deduce_quat2 {
