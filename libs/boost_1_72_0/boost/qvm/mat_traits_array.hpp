...
namespace boost {
namespace qvm {
template <class T, int R, int CR, int C> struct mat_traits<T[R][CR][C]> {
  static int const rows = 0;
  static int const cols = 0;
  typedef void scalar_type;
};

template <class T, int Rows, int Cols> struct mat_traits<T[Rows][Cols]> {
...
template <int Rows, int Cols, class T> T (&ptr_mref(T *ptr))[Rows][Cols] {
