...
template <typename T> struct single_view : sequence_base<single_view<T>> {
  ...
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED single_view() : val() {}

  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED explicit single_view(
      typename detail::call_param<T>::type in_val)
      : val(in_val) {}
...
template <typename T>
BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED inline single_view<
