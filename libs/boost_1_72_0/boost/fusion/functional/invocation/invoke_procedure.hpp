...
struct invoke_procedure<
    Function, Sequence,
    typename enable_if_has_type<typename detail::invoke_procedure_impl<
...
struct invoke_procedure_impl<Function, Sequence, N, false, true> {
  ...
  BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED static inline void
  call(Function &f, Sequence &s) {
    f(BOOST_PP_ENUM(N, M, ~));
  }
...
#define M(z, j, data)                                                          \
  typedef                                                                      \
      typename result_of::next<BOOST_PP_CAT(I, BOOST_PP_DEC(j))>::type I##j;   \
