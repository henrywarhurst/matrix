...
#define FUSION_MAX_ZIP_SEQUENCES_STR                                           \
  BOOST_PP_STRINGIZE(BOOST_FUSION_PP_ROUND_UP(FUSION_MAX_ZIP_SEQUENCES))
...
#pragma wave option(preserve : 2, line : 0, output                             \
                    : "detail/preprocessed/zip" FUSION_MAX_ZIP_SEQUENCES_STR   \
                      ".hpp")
#endif
...
template <BOOST_PP_ENUM_PARAMS_WITH_A_DEFAULT(
    BOOST_PP_INC(FUSION_MAX_ZIP_SEQUENCES), typename T, fusion::void_)>
struct zip;
}

#define FUSION_TEXT(z, n, text) , text

#define BOOST_PP_FILENAME_1 <boost/fusion/algorithm/transformation/zip.hpp>
...
struct zip<BOOST_PP_ENUM_PARAMS(ZIP_ITERATION, T) BOOST_PP_REPEAT_FROM_TO(
    BOOST_PP_DEC(ZIP_ITERATION), FUSION_MAX_ZIP_SEQUENCES, FUSION_TEXT,
    void_)> {
...
BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED inline
    typename result_of::zip<BOOST_PP_ENUM_PARAMS(ZIP_ITERATION, const T)>::type
  ...
  return typename result_of::zip<BOOST_PP_ENUM_PARAMS(ZIP_ITERATION,
                                                      const T)>::type(seqs);
}
