...
struct compute_erase_last // put this in detail!!!
{
  ...
  typedef
      typename mpl::if_<result_of::equal_to<first_type, seq_last_type>,
                        first_type,
                        typename result_of::next<first_type>::type>::type type;

  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static type call(First const &first,
                                                            mpl::false_) {
...
struct fusion_default_help : mpl::if_<is_same<T, use_default>, Default, T> {};
...
struct erase {
  ...
  typedef typename fusion_default_help<
      Last, typename compute_erase_last<Sequence, First>::type>::type LastType;
...
template <typename Sequence, typename First>
BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED inline typename lazy_enable_if<
...
erase(Sequence const &seq, First const &first) {
  ...
  left_type left(fusion::begin(seq), convert_iterator<First>::call(first));
  right_type right(
