...
namespace fusion {
namespace detail {
template <typename Fun> struct segmented_for_each_fun {
  BOOST_CONSTEXPR
      BOOST_FUSION_GPU_ENABLED explicit segmented_for_each_fun(Fun &f)
      : fun(f) {}
  ...
  template <typename Sequence, typename State, typename Context> struct apply {
    ...
    call(Sequence &seq, State const &, Context const &,
         segmented_for_each_fun const &fun) {
