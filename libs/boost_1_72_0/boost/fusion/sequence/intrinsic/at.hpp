...
template <> struct at_impl<sequence_facade_tag> {
...
template <> struct at_impl<boost_array_tag>;

template <> struct at_impl<mpl_sequence_tag>;
...
template <typename Sequence, int N> struct at_c : at<Sequence, mpl::int_<N>> {};
} // namespace result_of
