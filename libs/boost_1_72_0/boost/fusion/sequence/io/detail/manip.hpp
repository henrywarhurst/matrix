...
#define FUSION_STRING_OF_STREAM(Stream)                                        \
  std::basic_string<FUSION_GET_CHAR_TYPE(Stream),                              \
...
namespace fusion {
namespace detail {
template <typename Tag> int get_xalloc_index(Tag * = 0) {
  // each Tag will have a unique index
  static int index = std::ios::xalloc();
  return index;
}

template <typename Stream, typename Tag, typename T> struct stream_data {
  struct arena {
    ~arena() {
...
template <typename Tag, typename Stream> class string_ios_manip {
public:
  ...
  void set(string_type const &s) { stream_data_t::attach(stream, s); }
...
private:
  template <typename Char> void check_delim(Char c) const {
    using namespace std;
    if (!isspace(c)) {
      if (stream.get() != c) {
        stream.unget();
...
#define STD_TUPLE_DEFINE_MANIPULATOR_FUNCTIONS(name)                           \
  ...
  inline detail::name##_type<Char, Traits> name(                               \
      const std::basic_string<Char, Traits> &s) {                              \
    return detail::name##_type<Char, Traits>(s);                               \
  }                                                                            \
                                                                               \
  inline detail::name##_type<char> name(char const *s) {                       \
    return detail::name##_type<char>(std::basic_string<char>(s));              \
  }                                                                            \
                                                                               \
  inline detail::name##_type<wchar_t> name(wchar_t const *s) {                 \
    return detail::name##_type<wchar_t>(std::basic_string<wchar_t>(s));        \
  }                                                                            \
                                                                               \
  inline detail::name##_type<char> name(char c) {                              \
    return detail::name##_type<char>(std::basic_string<char>(1, c));           \
  }                                                                            \
                                                                               \
  inline detail::name##_type<wchar_t> name(wchar_t c) {                        \
...
#define STD_TUPLE_DEFINE_MANIPULATOR_FUNCTIONS(name)                           \
  ...
  inline detail::name##_type<Char, Traits> name(                               \
    ...
    return detail::name##_type<Char, Traits>(s);                               \
                                                                               ...
                                                                               \
  template <typename Char> inline detail::name##_type<Char> name(Char s[]) {   \
    return detail::name##_type<Char>(std::basic_string<Char>(s));              \
  }                                                                            \
                                                                               \
  ...
  inline detail::name##_type<Char> name(Char const s[]) {                      \
    return detail::name##_type<Char>(std::basic_string<Char>(s));              \
                                                                               ...
                                                                               \
  template <typename Char> inline detail::name##_type<Char> name(Char c) {     \
    return detail::name##_type<Char>(std::basic_string<Char>(1, c));           \
  }
