...
template <typename> struct value_of_data_impl;

template <typename> struct key_of_impl;

template <typename> struct deref_data_impl;
} // namespace extension
...
struct basic_iterator
  ...
  template <typename It> struct next : advance<It, mpl::int_<1>> {};

  template <typename It> struct prior : advance<It, mpl::int_<-1>> {};
  ...
  struct equal_to
      : mpl::and_<is_same<typename remove_const<typename It1::seq_type>::type,
