...
#define BOOST_FUSION_ADAPT_ADT_FILLER(...)                                     \
  BOOST_PP_IIF(                                                                \
      BOOST_PP_OR(BOOST_MPL_PP_TOKEN_EQUAL(                                    \
                      auto, BOOST_PP_VARIADIC_ELEM(0, __VA_ARGS__)),           \
                  BOOST_MPL_PP_TOKEN_EQUAL(                                    \
                      auto, BOOST_PP_VARIADIC_ELEM(1, __VA_ARGS__))),          \
                                                                               \
...
#define BOOST_FUSION_WORKAROUND_VARIADIC_EMPTINESS_LAST_ELEM(...)              \
  BOOST_PP_SEQ_HEAD(                                                           \
      BOOST_PP_SEQ_REST_N(BOOST_PP_DEC(BOOST_PP_VARIADIC_SIZE(__VA_ARGS__)),   \
...
#define BOOST_FUSION_ADAPT_ADT_WRAP_ATTR(A, B, C, D)                           \
  BOOST_PP_IIF(BOOST_MPL_PP_TOKEN_EQUAL(auto, A), ((2, (C, D))),               \
               ((4, (A, B, C, D))))
