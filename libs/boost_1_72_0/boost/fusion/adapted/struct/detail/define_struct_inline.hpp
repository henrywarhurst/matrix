...
// For MSVC, the bug was reported at
// https://connect.microsoft.com/VisualStudio/feedback/details/757891/c-compiler-error-involving-partial-specializations-of-nested-templates
...
#define BOOST_FUSION_MAKE_COPY_CONSTRUCTOR(NAME, ATTRIBUTES_SEQ)               \
  ...
  NAME(BOOST_PP_SEQ_FOR_EACH_I(BOOST_FUSION_MAKE_CONST_REF_PARAM, ~,           \
                               ATTRIBUTES_SEQ))                                \
      : BOOST_PP_SEQ_FOR_EACH_I(BOOST_FUSION_MAKE_INIT_LIST_ENTRY, ~,          \
                                ATTRIBUTES_SEQ) {}

#define BOOST_FUSION_MAKE_CONST_REF_PARAM(R, DATA, N, ATTRIBUTE)               \
  BOOST_PP_COMMA_IF(N)                                                         \
  BOOST_PP_TUPLE_ELEM(2, 0, ATTRIBUTE)                                         \
  const &BOOST_PP_TUPLE_ELEM(2, 1, ATTRIBUTE)
...
#define BOOST_FUSION_MAKE_ITERATOR_VALUE_OF_SPECS(Z, N, NAME)                  \
  template <typename boost_fusion_detail_Sq>                                   \
      ...
      : boost::mpl::identity<typename boost_fusion_detail_Sq::t##N##_type> {};

#define BOOST_FUSION_MAKE_ITERATOR_DEREF_SPEC(SPEC_TYPE, CALL_ARG_TYPE,        \
                                              TYPE_QUAL, ATTRIBUTE, N)         \
                                                                               \
  template <typename boost_fusion_detail_Sq> struct deref<SPEC_TYPE, N>> {     \
    typedef typename boost_fusion_detail_Sq::t##N##_type TYPE_QUAL &type;      \
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static type                       \
    call(CALL_ARG_TYPE, N > const &iter) {                                     \
...
#define BOOST_FUSION_MAKE_ITERATOR_DEREF_SPECS(R, NAME, N, ATTRIBUTE)          \
  BOOST_FUSION_MAKE_ITERATOR_DEREF_SPEC(                                       \
      ...
      BOOST_FUSION_ITERATOR_NAME(NAME) < boost_fusion_detail_Sq, , ATTRIBUTE,  \
      N)                                                                       \
  BOOST_FUSION_MAKE_ITERATOR_DEREF_SPEC(                                       \
      BOOST_FUSION_ITERATOR_NAME(NAME) < const boost_fusion_detail_Sq,         \
      BOOST_FUSION_ITERATOR_NAME(NAME) < const boost_fusion_detail_Sq, const,  \
      ATTRIBUTE, N)                                                            \
  BOOST_FUSION_MAKE_ITERATOR_DEREF_SPEC(                                       \
...
#define BOOST_FUSION_MAKE_AT_SPECS(R, DATA, N, ATTRIBUTE)                      \
  ...
  struct at<boost_fusion_detail_Sq, boost::mpl::int_<N>> {                     \
    typedef typename boost::mpl::if_<                                          \
        ...
        typename boost_fusion_detail_Sq::t##N##_type &>::type type;            \
                                                                               \
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static type                       \
    call(boost_fusion_detail_Sq &sq) {                                         \
...
#define BOOST_FUSION_DEFINE_ITERATOR_VALUE_OF(NAME, ATTRIBUTE_SEQ_SIZE)        \
  ...
  struct value_of                                                              \
      : boost::fusion::result_of::at_c<                                        \
            ref_vec_t, boost_fusion_detail_Iterator::index::value> {};

#define BOOST_FUSION_DEFINE_ITERATOR_DEREF(NAME, ATTRIBUTES_SEQ)               \
  template <typename boost_fusion_detail_Iterator> struct deref {              \
    typedef typename boost::remove_const<boost_fusion_detail_Iterator>::type   \
        iterator_raw_type;                                                     \
                                                                               \
    static const int index = iterator_raw_type::index::value;                  \
                                                                               \
    typedef typename boost::fusion::result_of::at_c<ref_vec_t, index>::type    \
        result_raw_type;                                                       \
                                                                               \
    typedef typename boost::mpl::if_<                                          \
        boost::is_const<typename iterator_raw_type::sequence_type>,            \
        typename boost::add_const<result_raw_type>::type,                      \
        result_raw_type>::type type;                                           \
                                                                               \
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static type                       \
    call(iterator_raw_type const &iter) {                                      \
...
#define BOOST_FUSION_DEFINE_ITERATOR_WKND_INIT_LIST_ENTRIES(ATTRIBUTES_SEQ)    \
  , ref_vec(                                                                   \
        BOOST_PP_SEQ_FOR_EACH_I(BOOST_FUSION_MAKE_ITERATOR_WKND_FIELD_NAME, ~, \
...
#define BOOST_FUSION_MAKE_ITERATOR_WKND_REF(Z, N, DATA)                        \
  ...
  typename boost::mpl::if_<                                                    \
      ...
      typename boost::add_const<                                               \
          typename boost_fusion_detail_Seq::t##N##_type>::type,                \
      typename boost_fusion_detail_Seq::t##N##_type>::type &

#define BOOST_FUSION_DEFINE_ITERATOR_WKND_MEMBERS(ATTRIBUTES_SEQ_SIZE)         \
  typedef boost::fusion::vector<BOOST_PP_REPEAT(                               \
      ATTRIBUTES_SEQ_SIZE, BOOST_FUSION_MAKE_ITERATOR_WKND_REF, ~)>            \
      ref_vec_t;                                                               \
                                                                               \
  ref_vec_t ref_vec;
...
#define BOOST_FUSION_DEFINE_ITERATOR_VALUE_OF(NAME, ATTRIBUTES_SEQ_SIZE)       \
  ...
  BOOST_PP_REPEAT(ATTRIBUTES_SEQ_SIZE,                                         \
                  BOOST_FUSION_MAKE_ITERATOR_VALUE_OF_SPECS, NAME)

#define BOOST_FUSION_DEFINE_ITERATOR_DEREF(NAME, ATTRIBUTES_SEQ)               \
  template <typename boost_fusion_detail_T> struct deref;                      \
  BOOST_PP_SEQ_FOR_EACH_I(BOOST_FUSION_MAKE_ITERATOR_DEREF_SPECS, NAME,        \
                          ATTRIBUTES_SEQ)
...
#define BOOST_FUSION_DEFINE_STRUCT_INLINE_IMPL(NAME, ATTRIBUTES)               \
  ...
  struct NAME : boost::fusion::sequence_facade<                                \
                    NAME, boost::fusion::random_access_traversal_tag> {        \
    BOOST_FUSION_DEFINE_STRUCT_INLINE_MEMBERS(NAME, ATTRIBUTES)                \
  };

#define BOOST_FUSION_DEFINE_TPL_STRUCT_INLINE_IMPL(TEMPLATE_PARAMS_SEQ, NAME,  \
                                                   ATTRIBUTES)                 \
                                                                               \
  BOOST_FUSION_DEFINE_STRUCT_INLINE_ITERATOR(NAME, ATTRIBUTES)                 \
                                                                               \
  ...
  struct NAME : boost::fusion::sequence_facade<                                \
                    NAME<BOOST_PP_SEQ_ENUM(TEMPLATE_PARAMS_SEQ)>,              \
...
#define BOOST_FUSION_DEFINE_STRUCT_INLINE_ITERATOR_IMPL_IMPL(                  \
  ...
  struct BOOST_FUSION_ITERATOR_NAME(NAME)                                      \
    ...
    (boost_fusion_detail_Seq & seq)                                            \
        : seq_(seq) BOOST_FUSION_DEFINE_ITERATOR_WKND_INIT_LIST_ENTRIES(       \
              (0)ATTRIBUTES_SEQ) {}                                            \
                                                                               \
    ...
    BOOST_FUSION_DEFINE_ITERATOR_DEREF(NAME, ATTRIBUTES_SEQ)                   \
                                                                               \
    template <typename boost_fusion_detail_It> struct next {                   \
      typedef BOOST_FUSION_ITERATOR_NAME(                                      \
          NAME)<typename boost_fusion_detail_It::sequence_type,                \
                boost_fusion_detail_It::index::value + 1> type;                \
                                                                               \
      BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static type                     \
      call(boost_fusion_detail_It const &it) {                                 \
        return type(it.seq_);                                                  \
      }                                                                        \
    };                                                                         \
                                                                               \
    template <typename boost_fusion_detail_It> struct prior {                  \
      typedef BOOST_FUSION_ITERATOR_NAME(                                      \
          NAME)<typename boost_fusion_detail_It::sequence_type,                \
                boost_fusion_detail_It::index::value - 1> type;                \
                                                                               \
      ...
      call(boost_fusion_detail_It const &it) {                                 \
        return type(it.seq_);                                                  \
                                                                               ...
                                                                               \
    template <typename boost_fusion_detail_It1,                                \
              typename boost_fusion_detail_It2>                                \
    struct distance {                                                          \
      typedef typename boost::mpl::minus<                                      \
          typename boost_fusion_detail_It2::index,                             \
          typename boost_fusion_detail_It1::index>::type type;                 \
                                                                               \
      BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static type                     \
      call(boost_fusion_detail_It1 const & /* it1 */,                          \
           boost_fusion_detail_It2 const & /* it2 */) {                        \
        return type();                                                         \
      }                                                                        \
    };                                                                         \
                                                                               \
    template <typename boost_fusion_detail_It, typename boost_fusion_detail_M> \
    struct advance {                                                           \
      typedef BOOST_FUSION_ITERATOR_NAME(                                      \
          NAME)<typename boost_fusion_detail_It::sequence_type,                \
                boost_fusion_detail_It::index::value +                         \
                    boost_fusion_detail_M::value> type;                        \
                                                                               \
      BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static type                     \
      call(boost_fusion_detail_It const &it) {                                 \
        return type(it.seq_);                                                  \
...
#define BOOST_FUSION_DEFINE_STRUCT_INLINE_MEMBERS_IMPL_IMPL(                   \
    NAME, ATTRIBUTES_SEQ, ATTRIBUTES_SEQ_SIZE)                                 \
                                                                               \
  NAME()                                                                       \
  BOOST_PP_IF(ATTRIBUTES_SEQ_SIZE, BOOST_FUSION_MAKE_DEFAULT_INIT_LIST,        \
              BOOST_PP_EMPTY)                                                  \
  (ATTRIBUTES_SEQ) {}                                                          \
                                                                               \
  BOOST_PP_IF(ATTRIBUTES_SEQ_SIZE, BOOST_FUSION_MAKE_COPY_CONSTRUCTOR,         \
              BOOST_FUSION_IGNORE_2)                                           \
  (NAME, ATTRIBUTES_SEQ)                                                       \
                                                                               \
      template <typename boost_fusion_detail_Seq>                              \
      BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED NAME(                     \
          const boost_fusion_detail_Seq &rhs) {                                \
    boost::fusion::copy(rhs, *this);                                           \
  }                                                                            \
                                                                               \
  template <typename boost_fusion_detail_Seq>                                  \
  BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED NAME &operator=(              \
      const boost_fusion_detail_Seq &rhs) {                                    \
    boost::fusion::copy(rhs, *this);                                           \
    return *this;                                                              \
  }                                                                            \
                                                                               \
  template <typename boost_fusion_detail_Sq> struct begin {                    \
    typedef BOOST_FUSION_ITERATOR_NAME(NAME)<boost_fusion_detail_Sq, 0> type;  \
                                                                               \
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static type                       \
    call(boost_fusion_detail_Sq &sq) {                                         \
      return type(sq);                                                         \
                                                                               ...
                                                                               \
  template <typename boost_fusion_detail_Sq> struct end {                      \
    typedef BOOST_FUSION_ITERATOR_NAME(                                        \
        NAME)<boost_fusion_detail_Sq, ATTRIBUTES_SEQ_SIZE> type;               \
                                                                               \
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static type                       \
    call(boost_fusion_detail_Sq &sq) {                                         \
      return type(sq);                                                         \
  ...
  struct empty : boost::mpl::bool_<ATTRIBUTES_SEQ_SIZE == 0> {};               \
                                                                               \
  template <typename boost_fusion_detail_Sq, typename boost_fusion_detail_N>   \
  struct value_at : value_at<boost_fusion_detail_Sq,                           \
                             boost::mpl::int_<boost_fusion_detail_N::value>> { \
  };                                                                           \
                                                                               \
  BOOST_PP_REPEAT(ATTRIBUTES_SEQ_SIZE, BOOST_FUSION_MAKE_VALUE_AT_SPECS, ~)    \
                                                                               \
  template <typename boost_fusion_detail_Sq, typename boost_fusion_detail_N>   \
  struct at : at<boost_fusion_detail_Sq,                                       \
                 boost::mpl::int_<boost_fusion_detail_N::value>> {};           \
                                                                               \
  ...
  BOOST_PP_SEQ_FOR_EACH_I(BOOST_FUSION_MAKE_TYPEDEF, ~, ATTRIBUTES_SEQ)        \
                                                                               \
  BOOST_PP_SEQ_FOR_EACH_I(BOOST_FUSION_MAKE_DATA_MEMBER, ~, ATTRIBUTES_SEQ)
