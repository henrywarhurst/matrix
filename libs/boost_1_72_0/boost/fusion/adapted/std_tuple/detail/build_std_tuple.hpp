...
template <typename T, typename Rest> struct push_front_std_tuple;
...
struct push_front_std_tuple<T, std::tuple<Rest...>> {
  ...
  indexed_call(T const &first, std::tuple<Rest...> const &rest,
               index_sequence<I...>) {
...
struct build_std_tuple<First, Last, false> {
  typedef build_std_tuple<typename result_of::next<First>::type, Last>
      next_build_std_tuple;

  typedef push_front_std_tuple<typename result_of::value_of<First>::type,
  ...
  call(First const &f, Last const &l) {
    ...
    return push_front::call(v, next_build_std_tuple::call(fusion::next(f), l));
  }
};
} // namespace detail
} // namespace fusion
