...
template <typename First, typename Last> struct build_deque<First, Last, true> {
  typedef deque<> type;
...
struct build_deque<First, Last, false> {
  typedef build_deque<typename result_of::next<First>::type, Last>
      next_build_deque;

  typedef push_front_deque<typename result_of::value_of<First>::type,
  ...
  call(First const &f, Last const &l) {
    ...
    return push_front::call(v, next_build_deque::call(fusion::next(f), l));
  }
};
} // namespace detail
} // namespace fusion
