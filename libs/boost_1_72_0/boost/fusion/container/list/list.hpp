...
template <> struct list<> : detail::list_to_cons<>::type {
private:
...
public:
  ...
  BOOST_FUSION_GPU_ENABLED list(Sequence const &rhs) : inherited_type(rhs) {}
...
#else
  template <typename Sequence>
  BOOST_FUSION_GPU_ENABLED list(Sequence &&rhs)
...
template <typename... T> struct list : detail::list_to_cons<T...>::type {
private:
...
public:
  ...
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED explicit list(
      typename detail::call_param<T>::type... args)
