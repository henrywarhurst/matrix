...
#pragma wave option(preserve : 2, line : 0, output                             \
                    : "preprocessed/make_set" FUSION_MAX_SET_SIZE_STR ".hpp")
#endif
...
// XXX:
#if defined(__WAVE__) && defined(BOOST_FUSION_CREATE_PREPROCESSED_FILES)
FUSION_HASH if defined(BOOST_CLANG) BOOST_CXX14_CONSTEXPR FUSION_HASH
    else BOOST_CONSTEXPR FUSION_HASH endif
#else
...
#endif
#endif
    BOOST_FUSION_GPU_ENABLED inline set<> make_set() {
  return set<>();
}
...
struct make_set<BOOST_PP_ENUM_PARAMS(N, T) BOOST_PP_REPEAT_FROM_TO(
    BOOST_PP_DEC(N), FUSION_MAX_SET_SIZE, TEXT, void_)>
#undef TEXT
{
