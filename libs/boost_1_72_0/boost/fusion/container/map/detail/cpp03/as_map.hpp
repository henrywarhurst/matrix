...
template <int size, bool is_assoc> struct as_map {
  BOOST_STATIC_ASSERT_MSG(size <= FUSION_MAX_MAP_SIZE,
                          "FUSION_MAX_MAP_SIZE limit is too low");
};

template <bool is_assoc> struct as_map<0, is_assoc> {
  template <typename Iterator> struct apply { typedef map<> type; };

  template <typename Iterator>
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static typename apply<Iterator>::type
  call(Iterator) {
    return map<>();
  }
};
...
#pragma wave option(preserve : 2, line : 0, output                             \
                    : "preprocessed/as_map" FUSION_MAX_MAP_SIZE_STR ".hpp")
#endif
...
#define BOOST_FUSION_NEXT_ITERATOR(z, n, data)                                 \
  typedef                                                                      \
      typename fusion::result_of::next<BOOST_PP_CAT(I, n)>::type BOOST_PP_CAT( \
          I, BOOST_PP_INC(n));
...
template <bool is_assoc> struct as_map<N, is_assoc> {
