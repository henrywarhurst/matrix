...
#pragma wave option(preserve : 2, line : 0, output                             \
                    : "preprocessed/vector_chooser" FUSION_MAX_VECTOR_SIZE_STR \
                      ".hpp")
#endif
...
struct vector_n_chooser {
  typedef BOOST_PP_CAT(vector, FUSION_MAX_VECTOR_SIZE)<
      BOOST_PP_ENUM_PARAMS(FUSION_MAX_VECTOR_SIZE, T)> type;
};

template <>
struct vector_n_chooser<BOOST_PP_ENUM_PARAMS(FUSION_MAX_VECTOR_SIZE,
...
struct vector_n_chooser<BOOST_PP_ENUM_PARAMS(
    N, T) BOOST_PP_ENUM_TRAILING_PARAMS(BOOST_PP_SUB(FUSION_MAX_VECTOR_SIZE, N),
                                        void_ BOOST_PP_INTERCEPT)> {
