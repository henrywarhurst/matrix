...
#define BOOST_FUSION_VECTOR_CTOR_HELPER()                                      \
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static vector_n const &ctor_helper( \
      vector const &rhs, mpl::true_) {                                         \
    return rhs.vec;                                                            \
  }                                                                            \
                                                                               \
  template <typename T>                                                        \
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static T const &ctor_helper(        \
      T const &rhs, mpl::false_) {                                             \
    return rhs;                                                                \
  }
...
#pragma wave option(preserve : 2, line : 0, output                             \
                    : "preprocessed/vvector" FUSION_MAX_VECTOR_SIZE_STR        \
                      ".hpp")
#endif
...
private:
  typedef typename detail::vector_n_chooser<BOOST_PP_ENUM_PARAMS(
      FUSION_MAX_VECTOR_SIZE, T)>::type vector_n;
