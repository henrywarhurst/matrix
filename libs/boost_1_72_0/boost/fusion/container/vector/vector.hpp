...
template <std::size_t, typename T> struct store {
...
template <typename I, typename... T> struct vector_data;
...
public:
  ...
  BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED
      typename store_at<J::value>::elem_type &
      at_impl(J) {
  ...
  template <typename J>
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED
      typename store_at<J::value>::elem_type const &
      at_impl(J) const {
    return store_at<J::value>::elem;
  }
};
                ...
                typename = typename boost::enable_if_c<
                    (sizeof...(U) >= 1 &&
                     ...
                     !fusion::detail::and_<is_base_of<
                         vector,
                         typename remove_reference<U>::type>...>::value)>::type>
      // XXX: constexpr become error due to pull-request #79, booooo!!
      //      In the (near) future release, should be fixed.
      /* BOOST_CONSTEXPR */ BOOST_FUSION_GPU_ENABLED explicit vector(U &&... u)
      : base(vector_detail::each_elem(), std::forward<U>(u)...) {}

  template <
      typename Sequence,
      typename = typename boost::enable_if_c<vector_detail::is_longer_sequence<
