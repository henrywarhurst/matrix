...
template <typename T> struct pmd : default_callable_traits<T> {};
...
struct pmd<D T::*> : default_callable_traits<> {
  ...
  using type = D T::*;
  using function_type =
      typename std::add_lvalue_reference<D>::type(invoke_type);
  ...
  template <typename C> using apply_member_pointer = D C::*;

  template <typename R> using apply_return = R T::*;
