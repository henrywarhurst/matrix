...
template <bool B, typename T> struct fail_if : T {
...
using sfinae_try = typename BOOST_CLBL_TRTS_DISJUNCTION(FailIfs...,
                                                        success<T>)::_::type;

template <typename FailMsg, typename ForceTwoPhaseLookup> struct fail {
  using type = typename std::conditional<
      std::is_same<ForceTwoPhaseLookup, std::false_type>::value, FailMsg,
      FailMsg>::type::_::type;
};
...
#define BOOST_CLBL_TRTS_DEFINE_SFINAE_ERROR_ORIGIN(origin)                     \
  ...
  struct origin : ::boost::callable_traits::detail::sfinae_error {             \
    struct _ {};                                                               \
  };                                                                           \
  }                                                                            \
  /**/
...
#define BOOST_CLBL_TRTS_SFINAE_MSG(origin, name)                               \
  ...
  struct name : error::origin<BOOST_CLBL_TRTS_PP_CAT(name, _)> {};             \
  /**/
...
BOOST_CLBL_TRTS_SFINAE_MSG(parameters,
...
BOOST_CLBL_TRTS_SFINAE_MSG(member_qualifiers,
                           member_qualifiers_are_illegal_for_this_type)
BOOST_CLBL_TRTS_SFINAE_MSG(
    member_qualifiers, this_compiler_doesnt_support_abominable_function_types)

BOOST_CLBL_TRTS_DEFINE_SFINAE_ERROR_ORIGIN(transaction_safe_)
BOOST_CLBL_TRTS_SFINAE_MSG(
    transaction_safe_, transaction_safe_is_not_supported_by_this_configuration)
...
BOOST_CLBL_TRTS_SFINAE_MSG(member_pointer_required,
                           type_is_not_a_member_pointer)

BOOST_CLBL_TRTS_DEFINE_SFINAE_ERROR_ORIGIN(reference_error)
BOOST_CLBL_TRTS_SFINAE_MSG(reference_error,
