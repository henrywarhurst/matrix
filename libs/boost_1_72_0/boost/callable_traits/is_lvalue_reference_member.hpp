...
template <typename T> struct is_lvalue_reference_member;
...
template <typename T> struct is_lvalue_reference_member_v {
...
`operator()` has a '&' member qualifier
* On compilers that support variable templates,
`is_lvalue_reference_member_v<T>` is equivalent to
