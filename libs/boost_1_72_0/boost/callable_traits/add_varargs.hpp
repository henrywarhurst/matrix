...
using add_varargs_t = // see below
    //<-
    detail::try_but_fail_if_invalid<typename detail::traits<T>::add_varargs,
                                    varargs_are_illegal_for_this_type>;

namespace detail {

template <typename T, typename = std::false_type> struct add_varargs_impl {};

template <typename T>
struct add_varargs_impl<
    T, typename std::is_same<add_varargs_t<T>, detail::dummy>::type> {
  using type = add_varargs_t<T>;
};
} // namespace detail
//->

template <typename T> struct add_varargs : detail::add_varargs_impl<T> {};
...
* Adds C-style variadics (`...`) to the signature of `T`, if not already
present.
