...
template <typename T> struct add_member_rvalue_reference_t {
  static_assert(
...
template <typename T>
struct add_member_rvalue_reference_impl<
    T, typename std::is_same<add_member_rvalue_reference_t<T>,
                             detail::dummy>::type> {
...
* Adds a member rvalue reference qualifier (`&&`) to `T`, if not already
present.
