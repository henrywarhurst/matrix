...
template <typename T, typename = std::false_type> struct add_member_cv_impl {};

template <typename T>
struct add_member_cv_impl<
    T, typename std::is_same<add_member_cv_t<T>, detail::dummy>::type> {
...
template <typename T> struct add_member_cv : detail::add_member_cv_impl<T> {};
...
[table
    ...
    [[`int(foo::*)() transaction_safe`] [`int(foo::*)() const volatile
transaction_safe`]]
