...
#define BOOST_LOCAL_FUNCTION_AUX_CODE_RESULT_DEDUCE_(id, typename01,           \
                                                     decl_traits)              \
  ...
  struct BOOST_LOCAL_FUNCTION_AUX_CODE_RESULT_PARAMS_(id) {                    \
    typedef BOOST_PP_EXPR_IIF(typename01, typename)::boost::function_traits<   \
        BOOST_PP_EXPR_IIF(typename01, typename)::boost::remove_pointer<        \
            BOOST_SCOPE_EXIT_DETAIL_CAPTURE_T(                                 \
                id, BOOST_LOCAL_FUNCTION_AUX_CODE_RESULT_INDEX_,               \
                BOOST_LOCAL_FUNCTION_AUX_CODE_RESULT_FUNC_(id))>::type>::      \
        result_type BOOST_LOCAL_FUNCTION_AUX_CODE_RESULT_TYPE_(id);            \
  };

// Use result type as explicitly specified by user (no type deduction needed).
// Precondition: RETURNS(decl_traits) != NIL
#define BOOST_LOCAL_FUNCTION_AUX_CODE_RESULT_TYPED_(id, typename01,            \
                                                    decl_traits)               \
  /* user specified result type here */                                        \
  struct BOOST_LOCAL_FUNCTION_AUX_CODE_RESULT_PARAMS_(id) {                    \
    typedef BOOST_PP_LIST_FIRST(                                               \
...
#define BOOST_LOCAL_FUNCTION_AUX_CODE_RESULT_TYPE(id, typename01)              \
  ...
  BOOST_LOCAL_FUNCTION_AUX_CODE_RESULT_PARAMS_(                                \
      id)::BOOST_LOCAL_FUNCTION_AUX_CODE_RESULT_TYPE_(id)
