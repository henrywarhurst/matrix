...
#define BOOST_LOCAL_FUNCTION_AUX_NAME_RECURSIVE_FUNC_(is_recursive,            \
                                                      local_function_name)     \
  BOOST_PP_IIF(                                                                \
      is_recursive, local_function_name,                                       \
      BOOST_LOCAL_FUNCTION_AUX_SYMBOL((nonrecursive_local_function_name)))

#define BOOST_LOCAL_FUNCTION_AUX_NAME_END_LOCAL_FUNCTOR_(                      \
    typename01, local_function_name, is_recursive, local_functor_name,         \
  ...
  BOOST_LOCAL_FUNCTION_AUX_NAME_LOCAL_TYPE_(                                   \
      local_function_name)::BOOST_LOCAL_FUNCTION_AUX_CODE_FUNCTOR_TYPE         \
...
#define BOOST_LOCAL_FUNCTION_AUX_NAME_(typename01, local_function_name)                                                                                                                                                                                         \
  BOOST_LOCAL_FUNCTION_AUX_NAME_END_LOCAL_FUNCTOR_(typename01, local_function_name, /* local function is not recursive (because recursion and its */ /* initialization cannot be inlined even on C++11, */ /* so this allows optimization at least on C++11) */ \
                                                   0 /* not recursive */                                                                                                                                                                                        \
                                                   , /* local functor */                                                                                                                                                                                        \
                                                   BOOST_LOCAL_FUNCTION_AUX_NAME_FUNCTOR_(                                                                                                                                                                      \
                                                       local_function_name), /* local function declared as non-local functor -- but it can */ /* be inlined only by C++11 and it cannot be recursive */                                                         \
                                                   local_function_name)
...
#define BOOST_LOCAL_FUNCTION_AUX_NAME_RECURSIVE_(typename01,                                                                                                                 \
                                                 local_function_name)                                                                                                        \
  BOOST_LOCAL_FUNCTION_AUX_NAME_END_LOCAL_FUNCTOR_(typename01,                                                                                                               \
                                                   local_function_name, /* recursive                                                                                         \
                                                                           local                                                                                             \
                                                                           function                                                                                          \
                                                                           --                                                                                                \
                                                                           but                                                                                               \
                                                   ...
                                                   1 /* recursive */, /* local                                                                                               \
                                                                       ...
                                                                       */                                                                                                    \
                                                   BOOST_LOCAL_FUNCTION_AUX_NAME_FUNCTOR_(                                                                                   \
                                                       local_function_name), /* local function declared as non-local functor -- but it can */ /* be inlined only by C++11 */ \
                                                   local_function_name)
...
#define BOOST_LOCAL_FUNCTION_AUX_NAME_PARSE_INLINE_(typename01,                \
                                                    qualified_name)            \
...
#define BOOST_LOCAL_FUNCTION_AUX_NAME_PARSE_RECURSIVE_(typename01,             \
                                                       qualified_name)         \
  BOOST_PP_IIF(BOOST_LOCAL_FUNCTION_AUX_NAME_IS_RECURSIVE_(                    \
                   qualified_name), /* recursion can never be inlined (not     \
                                       even on C++11) */                       \
