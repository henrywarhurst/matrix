...
#define BOOST_LOCAL_FUNCTION_AUX_typename_seq(z, n, unused) (typename)
...
#define BOOST_LOCAL_FUNCTION_AUX_arg_typedef(z, arg_n, unused)                 \
  typedef BOOST_LOCAL_FUNCTION_AUX_arg_type(                                   \
      z, arg_n,                                                                \
...
#define BOOST_LOCAL_FUNCTION_AUX_arg_param_type(z, arg_n, comma01)             \
  ...
  typename ::boost::call_traits<BOOST_LOCAL_FUNCTION_AUX_arg_type(             \
      z, arg_n, ~)>::param_type
...
#define BOOST_LOCAL_FUNCTION_AUX_bind_member_init(z, bind_n, unused)           \
  BOOST_LOCAL_FUNCTION_AUX_bind_member(z, bind_n, ~) =                         \
      member_addr(BOOST_LOCAL_FUNCTION_AUX_bind_name(z, bind_n, ~));

#define BOOST_LOCAL_FUNCTION_AUX_bind_member_decl(z, bind_n, unused)           \
  /* must be ptr (not ref) so can use default constr */                        \
  typename member_type<BOOST_LOCAL_FUNCTION_AUX_bind_type(                     \
      z, bind_n, ~)>::pointer BOOST_LOCAL_FUNCTION_AUX_bind_member(z, bind_n,  \
                                                                   ~);

#define BOOST_LOCAL_FUNCTION_AUX_call_ptr(z, n, unused)                        \
  BOOST_PP_CAT(call_ptr, n)

#define BOOST_LOCAL_FUNCTION_AUX_call_name(z, n, unused) BOOST_PP_CAT(call, n)
...
#define BOOST_LOCAL_FUNCTION_AUX_call_typedef(z, n, arity)                     \
  typedef R (*BOOST_LOCAL_FUNCTION_AUX_call_ptr(z, n, ~))(                     \
      object_ptr BOOST_PP_IIF(BOOST_LOCAL_FUNCTION_CONFIG_LOCALS_AS_TPARAMS,   \
                              BOOST_PP_TUPLE_EAT(3), BOOST_PP_REPEAT_##z)(     \
          BOOST_LOCAL_FUNCTION_CONFIG_BIND_MAX,                                \
          BOOST_LOCAL_FUNCTION_AUX_comma_bind_ref, ~)                          \
          BOOST_PP_REPEAT_##z(BOOST_PP_SUB(arity, n),                          \
                              BOOST_LOCAL_FUNCTION_AUX_arg_param_type,         \
                              1 /* leading comma */));
...
#define BOOST_LOCAL_FUNCTION_AUX_operator_call(z, defaults_n, arity)           \
  ...
  inline R operator()(BOOST_PP_ENUM_##z(                                       \
      BOOST_PP_SUB(arity, defaults_n),                                         \
    ...
    return BOOST_LOCAL_FUNCTION_AUX_call_member(z, defaults_n, ~)(             \
        object_ BOOST_PP_IIF(BOOST_LOCAL_FUNCTION_CONFIG_LOCALS_AS_TPARAMS,    \
                             BOOST_PP_TUPLE_EAT(3), BOOST_PP_REPEAT_##z)(      \
            BOOST_LOCAL_FUNCTION_CONFIG_BIND_MAX,                              \
            BOOST_LOCAL_FUNCTION_AUX_comma_bind_member_deref, ~)               \
            BOOST_PP_REPEAT_##z(BOOST_PP_SUB(arity, defaults_n),               \
                                BOOST_LOCAL_FUNCTION_AUX_arg_name,             \
                                1 /* leading comma */));                       \
  }
...
template <typename F,
          ...
          >
class function {
}; // Empty template, only use its specializations.
...
template <typename R BOOST_PP_REPEAT(BOOST_LOCAL_FUNCTION_AUX_arity,
...
#endif
          >
class function<R(BOOST_PP_ENUM(BOOST_LOCAL_FUNCTION_AUX_arity,
...
public:
...
#endif
    BOOST_PP_REPEAT(BOOST_PP_INC(BOOST_LOCAL_FUNCTION_AUX_defaults),
                    BOOST_LOCAL_FUNCTION_AUX_call_init,
                    ~) // INC for no defaults.
  ...
  // ...                                      -- etc
  BOOST_PP_REPEAT(
      BOOST_PP_INC(BOOST_LOCAL_FUNCTION_AUX_defaults),
