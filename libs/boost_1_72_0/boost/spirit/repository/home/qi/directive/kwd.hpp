...
struct use_directive<
    qi::domain, terminal_ex<repository::tag::kwd // enables kwd(key,exact)[p]
                            ,
                            fusion::vector2<T1, T2>>> : mpl::true_ {};

template <typename T1, typename T2>
struct use_directive<
    qi::domain, terminal_ex<repository::tag::ikwd // enables ikwd(key,exact)[p]
                            ,
                            fusion::vector2<T1, T2>>> : mpl::true_ {};

template <typename T1, typename T2>
struct use_directive<
    qi::domain, terminal_ex<repository::tag::dkwd // enables dkwd(key,exact)[p]
                            ,
                            fusion::vector2<T1, T2>>> : mpl::true_ {};

template <typename T1, typename T2>
struct use_directive<qi::domain,
                     terminal_ex<repository::tag::idkwd // enables
                                                        // idkwd(key,exact)[p]
                                 ,
                                 fusion::vector2<T1, T2>>> : mpl::true_ {};
...
template <typename T1, typename T2>
struct use_directive<
    qi::domain, terminal_ex<repository::tag::kwd // enables kwd(min, max)[p]
                            ,
                            fusion::vector3<T1, T2, T2>>> : mpl::true_ {};

template <typename T1, typename T2>
struct use_directive<
    qi::domain, terminal_ex<repository::tag::ikwd // enables ikwd(min, max)[p]
                            ,
                            fusion::vector3<T1, T2, T2>>> : mpl::true_ {};

template <typename T1, typename T2>
struct use_directive<
    qi::domain, terminal_ex<repository::tag::dkwd // enables dkwd(min, max)[p]
                            ,
                            fusion::vector3<T1, T2, T2>>> : mpl::true_ {};

template <typename T1, typename T2>
struct use_directive<
    qi::domain, terminal_ex<repository::tag::idkwd // enables idkwd(min, max)[p]
                            ,
                            fusion::vector3<T1, T2, T2>>> : mpl::true_ {};

template <typename T1, typename T2>
struct use_directive<
    qi::domain, terminal_ex<repository::tag::kwd // enables kwd(min, inf)[p]
                            ,
                            fusion::vector3<T1, T2, inf_type>>> : mpl::true_ {};

template <typename T1, typename T2>
struct use_directive<
    qi::domain, terminal_ex<repository::tag::ikwd // enables ikwd(min, inf)[p]
                            ,
                            fusion::vector3<T1, T2, inf_type>>> : mpl::true_ {};

template <typename T1, typename T2>
struct use_directive<
    qi::domain, terminal_ex<repository::tag::dkwd // enables dkwd(min, inf)[p]
                            ,
                            fusion::vector3<T1, T2, inf_type>>> : mpl::true_ {};

template <typename T1, typename T2>
struct use_directive<
    qi::domain, terminal_ex<repository::tag::idkwd // enables idkwd(min, inf)[p]
                            ,
                            fusion::vector3<T1, T2, inf_type>>> : mpl::true_ {};

/*  template <>                                     // enables *lazy*
  kwd(exact)[p] struct use_lazy_directive< qi::domain , tag::kwd , 1 // arity >
  : mpl::true_ {};

  template <>                                     // enables *lazy* kwd(min,
  max)[p] struct use_lazy_directive<                      // and kwd(min,
  inf)[p] qi::domain , tag::kwd , 2 // arity > : mpl::true_ {};
*/
...
{
  kwd_exact_iterator(T const exact) : exact(exact) {}
...
struct kwd_finite_iterator // handles kwd(min, max)[p]
{
  kwd_finite_iterator(T const min, T const max)
      : min BOOST_PREVENT_MACRO_SUBSTITUTION(min), max
        BOOST_PREVENT_MACRO_SUBSTITUTION(max) {}
...
template <typename Skipper, typename NoCasePass> struct skipper_keyword_marker {
  typedef NoCasePass no_case_pass;

  skipper_keyword_marker(Skipper const &skipper, bool &flag, int &counter)
      : skipper(skipper), flag(flag), counter(counter) {}
...
template <typename Subject, typename KeywordType, typename LoopIter,
          typename NoCase, typename Distinct>
struct kwd_parser
    : spirit::qi::unary_parser<
          kwd_parser<Subject, KeywordType, LoopIter, NoCase, Distinct>> {
  ...
  typedef typename remove_const<
      typename traits::char_type_of<KeywordType>::type>::type char_type;
  ...
  template <typename Context, typename Iterator> struct attribute {
    typedef typename traits::build_std_vector<
        typename traits::attribute_of<Subject, Context, Iterator>::type>::type
        type;
  };
  ...
  template <typename Iterator, typename Context, typename Skipper,
            typename Attribute>
  bool parse_impl(Iterator &first, Iterator const &last, Context &context,
                  Skipper const &skipper, Attribute &attr, mpl::false_) const {
  ...
  template <typename Iterator, typename Context, typename Skipper,
            typename Attribute>
  bool parse_impl(Iterator &first, Iterator const &last, Context &context,
                  Skipper const &skipper, Attribute &attr, mpl::true_) const {
  ...
  template <typename Iterator, typename Context, typename Skipper,
            typename Attribute, typename NoCasePass>
  bool parse(Iterator &first, Iterator const &last, Context &context,
    ...
    typedef
        typename mpl::and_<traits::is_container<Attribute>,
                           mpl::not_<traits::is_weak_substitute<
                               subject_attribute, Attribute>>>::type predicate;
  ...
  template <typename Context> info what(Context &context) const {
    if (distinct::value) {
  ...
  typedef typename mpl::if_<
      no_case_keyword, spirit::qi::no_case_literal_string<KeywordType, true>,
...
private:
  ...
  fail_function(Iterator &first, Iterator const &last, Context &context,
...
struct complex_kwd_parser
    : spirit::qi::unary_parser<
          complex_kwd_parser<Subject, KeywordType, LoopIter, Distinct>> {
...
template <typename T1, typename T2, typename Subject, typename Modifiers,
          typename Distinct, typename MakeDirectiveHelper>
struct make_directive_internal_2_args {
  ...
  typedef
      typename mpl::if_<is_string_kwd_type, T1,
                        typename result_of::compile<qi::domain, T1>::type>::type
          keyword_type;
  ...
  typedef typename mpl::if_<
      is_string_kwd_type,
      repository::qi::kwd_parser<Subject, const_keyword, iterator_type, no_case,
                                 Distinct>,
      repository::qi::complex_kwd_parser<Subject, const_keyword, iterator_type,
                                         Distinct>>::type result_type;

  // Return a kwd parser object
  template <typename Terminal>
  result_type create_kwd_string(Terminal const &term, Subject const &subject,
                                boost::mpl::true_) const {
    typename spirit::detail::get_encoding<
        Modifiers, spirit::char_encoding::standard>::type encoding;
    return result_type(subject, MakeDirectiveHelper::make_iterator(term.args),
                       encoding);
  }
  template <typename Terminal>
  result_type create_kwd_string(Terminal const &term, Subject const &subject,
  ...
  template <typename Terminal>
  result_type create_kwd(Terminal const &term, Subject const &subject,
  ...
  template <typename Terminal>
  result_type create_kwd(Terminal const &term, Subject const &subject,
                         Modifiers const &modifiers, boost::mpl::false_) const {
    return result_type(
        subject, compile<qi::domain>(fusion::at_c<0>(term.args), modifiers),
        MakeDirectiveHelper::make_iterator(term.args));
  }
...
struct make_directive_internal {
  ...
  // make the keyword type
  typedef typename mpl::if_<
      is_string_kwd_type, T1,
      typename result_of::compile<qi::domain, T1, Modifiers>::type>::type
      keyword_type;
...
struct make_directive<terminal_ex<repository::tag::kwd, fusion::vector1<T1>>,
                      Subject, Modifiers> {
  typedef make_directive_internal<T1, Subject, Modifiers, mpl::false_>
      make_directive_type;
...
template <typename T1, typename Subject, typename Modifiers>
struct make_directive<terminal_ex<repository::tag::dkwd, fusion::vector1<T1>>,
                      Subject, Modifiers> {
  typedef make_directive_internal<T1, Subject, Modifiers, mpl::true_>
      make_directive_type;
  typedef typename make_directive_type::result_type result_type;
  template <typename Terminal>
  result_type operator()(Terminal const &term, Subject const &subject,
...
template <typename T1, typename Subject, typename Modifiers>
struct make_directive<terminal_ex<repository::tag::ikwd, fusion::vector1<T1>>,
                      Subject, Modifiers> {
  typedef typename add_const<T1>::type const_keyword;
  typedef repository::qi::kwd_pass_iterator<int> iterator_type;

  typedef repository::qi::kwd_parser<Subject, const_keyword, iterator_type,
...
template <typename T1, typename Subject, typename Modifiers>
struct make_directive<terminal_ex<repository::tag::idkwd, fusion::vector1<T1>>,
                      Subject, Modifiers> {
  typedef typename add_const<T1>::type const_keyword;
  typedef repository::qi::kwd_pass_iterator<int> iterator_type;
...
template <typename T> struct make_exact_helper {
...
struct make_directive<
    terminal_ex<repository::tag::kwd, fusion::vector2<T1, T2>>, Subject,
    Modifiers> {
  typedef make_directive_internal_2_args<T1, T2, Subject, Modifiers,
                                         mpl::false_, make_exact_helper<T2>>
      make_directive_type;
  typedef typename make_directive_type::result_type result_type;
  template <typename Terminal>
  result_type operator()(Terminal const &term, Subject const &subject,
...
template <typename T1, typename T2, typename Subject, typename Modifiers>
struct make_directive<
    terminal_ex<repository::tag::dkwd, fusion::vector2<T1, T2>>, Subject,
    Modifiers> {
  typedef make_directive_internal_2_args<T1, T2, Subject, Modifiers, mpl::true_,
...
template <typename T1, typename T2, typename Subject, typename Modifiers>
struct make_directive<
    terminal_ex<repository::tag::ikwd, fusion::vector2<T1, T2>>, Subject,
    Modifiers> {
  typedef typename add_const<T1>::type const_keyword;
  ...
  result_type operator()(Terminal const &term, Subject const &subject,
    ...
    typename spirit::detail::get_encoding<
        Modifiers, spirit::char_encoding::standard>::type encoding;
    return result_type(subject, fusion::at_c<0>(term.args),
                       fusion::at_c<1>(term.args), encoding);
  }
};

template <typename T1, typename T2, typename Subject, typename Modifiers>
struct make_directive<
    terminal_ex<repository::tag::idkwd, fusion::vector2<T1, T2>>, Subject,
    Modifiers> {
  typedef typename add_const<T1>::type const_keyword;
  typedef repository::qi::kwd_exact_iterator<T2> iterator_type;
...
template <typename T> struct make_finite_helper {
...
template <typename T1, typename T2, typename Subject, typename Modifiers>
struct make_directive<
    terminal_ex<repository::tag::kwd, fusion::vector3<T1, T2, T2>>, Subject,
    Modifiers> {
  typedef make_directive_internal_2_args<T1, T2, Subject, Modifiers,
                                         mpl::false_, make_finite_helper<T2>>
      make_directive_type;
...
template <typename T1, typename T2, typename Subject, typename Modifiers>
struct make_directive<
    terminal_ex<repository::tag::dkwd, fusion::vector3<T1, T2, T2>>, Subject,
    Modifiers> {
...
template <typename T1, typename T2, typename Subject, typename Modifiers>
struct make_directive<
    terminal_ex<repository::tag::ikwd, fusion::vector3<T1, T2, T2>>, Subject,
    Modifiers> {
  typedef typename add_const<T1>::type const_keyword;
  ...
  result_type operator()(Terminal const &term, Subject const &subject,
    ...
    typename spirit::detail::get_encoding<
        Modifiers, spirit::char_encoding::standard>::type encoding;
    return result_type(subject, fusion::at_c<0>(term.args),
                       iterator_type(fusion::at_c<1>(term.args),
                                     fusion::at_c<2>(term.args), encoding));
  }
};

template <typename T1, typename T2, typename Subject, typename Modifiers>
struct make_directive<
    terminal_ex<repository::tag::idkwd, fusion::vector3<T1, T2, T2>>, Subject,
    Modifiers> {
  typedef typename add_const<T1>::type const_keyword;
  typedef repository::qi::kwd_finite_iterator<T2> iterator_type;
...
template <typename T> struct make_infinite_helper {
...
struct make_directive<
    ...
    Subject, Modifiers> {
  typedef make_directive_internal_2_args<T1, T2, Subject, Modifiers,
                                         mpl::false_, make_infinite_helper<T2>>
      make_directive_type;
...
template <typename Subject, typename KeywordType, typename LoopIter,
          typename NoCase, typename Distinct>
struct has_semantic_action<repository::qi::kwd_parser<
    Subject, KeywordType, LoopIter, NoCase, Distinct>>
    : unary_has_semantic_action<Subject> {};

template <typename Subject, typename KeywordType, typename LoopIter,
          typename Distinct>
struct has_semantic_action<repository::qi::complex_kwd_parser<
    Subject, KeywordType, LoopIter, Distinct>>
    : unary_has_semantic_action<Subject> {};

template <typename Subject, typename KeywordType, typename LoopIter,
          typename NoCase, typename Attribute, typename Context,
          typename Iterator, typename Distinct>
struct handles_container<repository::qi::kwd_parser<Subject, KeywordType,
                                                    LoopIter, NoCase, Distinct>,
...
template <typename Subject, typename KeywordType, typename LoopIter,
          ...
          typename Distinct>
struct handles_container<repository::qi::complex_kwd_parser<
                             Subject, KeywordType, LoopIter, Distinct>,
                         Attribute, Context, Iterator>
    : unary_handles_container<Subject, Attribute, Context, Iterator> {};
