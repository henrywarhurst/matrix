...
namespace qi {
namespace detail {
// Variant visitor class which handles dispatching the parsing to the selected
// parser This also handles passing the correct attributes and flags/counters to
// the subject parsers
template <typename T> struct is_distinct : T::distinct {};
...
template <typename Elements, typename Iterator, typename Context,
          typename Skipper, typename Flags, typename Counters,
...
public:
  parse_dispatcher(const Elements &elements, Iterator &first,
                   Iterator const &last, Context &context,
                   Skipper const &skipper, Flags &flags, Counters &counters,
                   attr_reference attr)
      : elements(elements), first(first), last(last), context(context),
        skipper(skipper), flags(flags), counters(counters), attr(attr) {}
  ...
  bool call_subject_unused(Subject const &subject, Iterator &first,
                           Iterator const &last, Context &context,
                           Skipper const &skipper, Index & /*idx*/) const {
    Iterator save = first;
    skipper_keyword_marker<Skipper, NoCasePass> marked_skipper(
        skipper, flags[Index::value], counters[Index::value]);
  ...
  template <typename Subject, typename Index>
  bool call_subject(Subject const &subject, Iterator &first,
                    Iterator const &last, Context &context,
                    Skipper const &skipper, Index & /*idx*/) const {
    ...
    skipper_keyword_marker<Skipper, NoCasePass> marked_skipper(
        skipper, flags[Index::value], counters[Index::value]);
    if (subject.parse(first, last, context, marked_skipper,
                      fusion::at_c<Index::value>(attr))) {
      return true;
    }
    first = save;
    return false;
...
#pragma warning(disable : 4127) // conditional expression is constant
  ...
  template <typename T> bool call(T &idx, mpl::false_) const {
    ...
    if ((!is_distinct<ElementType>::value) ||
      ...
      return call_subject_unused(fusion::at_c<T::value>(elements), first, last,
                                 context, skipper, idx);
    }
    return false;
  }
  ...
  template <typename T> bool call(T &idx, mpl::true_) const {
    ...
    if ((!is_distinct<ElementType>::value) ||
        skipper.parse(first, last, unused, unused, unused)) {
      return call_subject(fusion::at_c<T::value>(elements), first, last,
                          context, skipper, idx);
    }
    return false;
  }
...
template <typename Elements, typename StringKeywords, typename IndexList,
          typename FlagsType, typename Modifiers>
struct string_keywords {
  // Create a variant type to be able to store parser indexes in the embedded
  // symbols parser
  typedef
      typename spirit::detail::as_variant<IndexList>::type parser_index_type;
  ...
  ///////////////////////////////////////////////////////////////////////////
  template <typename Sequence> struct build_char_type_sequence {
    ...
    typedef
        typename fusion::result_of::transform<Sequence, element_char_type>::type
            type;
  };
  ...
  ///////////////////////////////////////////////////////////////////////////
  template <typename Sequence> struct get_keyword_char_type {
    // Make sure each of the types occur only once in the type list
    typedef
        typename mpl::fold<Sequence, mpl::vector<>,
  ...
  typedef typename get_keyword_char_type<typename mpl::if_<
      ...
      mpl::vector<boost::spirit::standard::char_type>, char_types>::type>::type
      char_type;
  ...
  template <typename CharEncoding> struct no_case_filter {
  ...
  ///////////////////////////////////////////////////////////////////////////
  template <typename Sequence> struct build_case_type_sequence {
    ...
    // Compute the list of character types of the child kwd directives
    typedef
        typename fusion::result_of::transform<Sequence, element_case_type>::type
            type;
  };
  ...
  ///////////////////////////////////////////////////////////////////////////
  template <typename Sequence, typename CaseType> struct get_nb_case_types {
    // Make sure each of the types occur only once in the type list
    typedef
        typename mpl::count_if<Sequence, mpl::equal_to<mpl::_, CaseType>>::type
            type;
  };
  // Build the case type sequence
  typedef typename build_case_type_sequence<StringKeywords>::type
      case_type_sequence;
  // Count the number of case sensitive entries and case insensitve entries
  typedef typename get_nb_case_types<case_type_sequence, mpl::true_>::type
      ikwd_count;
  typedef typename get_nb_case_types<case_type_sequence, mpl::false_>::type
      kwd_count;
  ...
  typedef
      typename mpl::or_<all_kwd, all_ikwd>::type all_directives_of_same_type;
  ...
  typedef
      typename mpl::or_<no_case_modifier,
                        mpl::and_<all_directives_of_same_type, all_ikwd>>::type
          no_case;

  typedef no_case_filter<typename spirit::detail::get_encoding_with_case<
      Modifiers, char_encoding::standard, no_case::value>::type>
      nc_filter;
  // Determine the standard case filter type
  typedef
      typename mpl::if_<no_case, nc_filter, spirit::qi::tst_pass_through>::type
          first_pass_filter_type;

  typedef typename mpl::or_<all_directives_of_same_type, no_case_modifier>::type
      requires_one_pass;
  ...
  struct keyword_entry_adder {
    ...
    keyword_entry_adder(shared_ptr<keywords_type> lookup, FlagsType &flags,
                        Elements &elements)
        : lookup(lookup), flags(flags), elements(elements) {}

    template <typename T> int operator()(const T &index) const {
      return call(fusion::at_c<T::value>(elements), index);
    }

    template <typename T, typename Position, typename Action>
    int call(const spirit::qi::action<T, Action> &parser,
      ...
      lookup->add(
          ...
          position);
      // Get the initial state of the flags array and store it in the flags
      // initializer
    ...
    int call(const T &parser, const Position position) const {
      // Make the keyword/parse index entry in the tst parser
      lookup->add(traits::get_begin<char_type>(get_string(parser.keyword)),
    ...
    template <typename T, typename Position>
    int call(const spirit::qi::hold_directive<T> &parser,
             const Position position) const {
      // Make the keyword/parse index entry in the tst parser
      lookup->add(
          traits::get_begin<char_type>(get_string(parser.subject.keyword)),
    ...
    const String get_string(
        const boost::spirit::qi::literal_string<String, no_attribute> &parser)
        const {
      return parser.str;
    }

    template <typename String, bool no_attribute>
    const typename boost::spirit::qi::no_case_literal_string<
        String, no_attribute>::string_type &
    get_string(
        const boost::spirit::qi::no_case_literal_string<String, no_attribute>
            &parser) const {
  ...
  string_keywords(Elements &elements, FlagsType &flags_init)
    ...
    // Loop through all the subject parsers to build the keyword parser symbol
    // parser
    IndexList indexes;
  ...
  template <typename Iterator, typename ParseVisitor,
            typename NoCaseParseVisitor, typename Skipper>
  bool parse(Iterator &first, const Iterator &last,
             const ParseVisitor &parse_visitor,
...
struct empty_keywords_list {
  ...
  empty_keywords_list() {}
  template <typename Elements> empty_keywords_list(const Elements &) {}
...
template <typename ComplexKeywords> struct complex_keywords {
  // Functor which performs the flag initialization for the complex keyword
  // parsers
  ...
  template <typename ParseFunction> bool parse(ParseFunction &function) const {
...
struct register_successful_parse {
  ...
  static bool call(spirit::qi::action<Subject, Action> const &subject,
                   bool &flag, int &counter) {
    return subject.subject.iter.register_successful_parse(flag, counter);
  }
  template <typename Subject>
  static bool call(spirit::qi::hold_directive<Subject> const &subject,
                   bool &flag, int &counter) {
    return subject.subject.iter.register_successful_parse(flag, counter);
  }
};
...
struct extract_keyword {
  ...
  template <typename Subject, typename Action>
  static Subject const &
  call(spirit::qi::action<Subject, Action> const &subject) {
    return subject.subject;
  }
  template <typename Subject>
  static Subject const &
  call(spirit::qi::hold_directive<Subject> const &subject) {
    return subject.subject;
  }
};

template <typename ParseDispatcher> struct complex_kwd_function {
  ...
  complex_kwd_function(Iterator &first, Iterator const &last, Context &context,
                       Skipper const &skipper, ParseDispatcher &dispatcher)
      : first(first), last(last), context(context), skipper(skipper),
        dispatcher(dispatcher) {}

  template <typename Component> bool operator()(Component const &component) {
    Iterator save = first;
    if (extract_keyword::call(
            fusion::at_c<Component::value,
