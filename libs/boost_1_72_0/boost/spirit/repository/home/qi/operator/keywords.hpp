...
template <typename T> struct is_kwd_parser : detail::has_kwd_parser_id<T> {};

template <typename Subject, typename Action>
struct is_kwd_parser<spirit::qi::action<Subject, Action>>
    : detail::has_kwd_parser_id<Subject> {};

template <typename Subject>
struct is_kwd_parser<spirit::qi::hold_directive<Subject>>
    : detail::has_kwd_parser_id<Subject> {};
...
struct keywords : spirit::qi::nary_parser<keywords<Elements, Modifiers>> {
  template <typename Context, typename Iterator> struct attribute {
    ...
    typedef typename traits::build_attribute_sequence<
        Elements, Context, traits::sequence_attribute_transform, Iterator,
  ...
  typedef typename mpl::count_if<
      Elements, mpl::not_<mpl::or_<is_kwd_parser<mpl::_1>,
                                   is_complex_kwd_parser<mpl::_1>>>>
  ...
  // dispatcher functor in order to use the parser corresponding to the
  // recognised keyword.
  ///////////////////////////////////////////////////////////////////////////

  template <typename Sequence> struct build_parser_tags {
    ...
    // Transform the range_c to an mpl vector in order to be able to transform
    // it into a variant
    typedef
        typename mpl::copy<int_range, mpl::back_inserter<mpl::vector<>>>::type
            type;
  };
  ...
  struct is_complex_kwd_parser_filter
      : is_complex_kwd_parser<typename mpl::at<Elements, idx>::type> {};

  template <typename idx>
  struct is_kwd_parser_filter
      : is_kwd_parser<typename mpl::at<Elements, idx>::type> {};

  // filter out the string kwd directives
  typedef typename mpl::filter_view<Elements, is_kwd_parser<mpl::_>>::type
      string_keywords;

  typedef typename mpl::filter_view<parser_index_vector,
                                    is_kwd_parser_filter<mpl::_>>::type
      string_keyword_indexes;
  // filter out the complex keywords
  typedef typename mpl::filter_view<parser_index_vector,
                                    is_complex_kwd_parser_filter<mpl::_>>::type
      complex_keywords_indexes;

  // typedef typename fusion::filter_view< Elements, is_complex_kwd_parser<
  // mpl::_ > > complex_keywords_view;

  typedef
      typename mpl::if_<typename mpl::empty<complex_keywords_indexes>::type,
  ...
  typedef boost::array<bool, fusion::result_of::size<Elements>::value>
      flags_type;
  typedef boost::array<int, fusion::result_of::size<Elements>::value>
      counters_type;

  typedef typename mpl::if_<
      typename mpl::empty<string_keyword_indexes>::type,
      detail::empty_keywords_list,
      detail::string_keywords<Elements, string_keywords, string_keyword_indexes,
                              flags_type, Modifiers>>::type
      string_keywords_type;

  keywords(Elements const &elements_)
      : elements(elements_), string_keywords_inst(elements, flags_init),
  ...
  bool parse_impl(Iterator &first, Iterator const &last, Context &context,
    ...
    typedef repository::qi::detail::parse_dispatcher<
        Elements, Iterator, Context, Skipper, flags_type, counters_type,
        typename traits::wrap_if_not_tuple<Attribute>::type, mpl::false_>
        parser_visitor_type;

    parser_visitor_type parse_visitor(elements, first, last, context, skipper,
                                      flags, counters, attr);

    typedef repository::qi::detail::complex_kwd_function<parser_visitor_type>
        complex_kwd_function_type;

    complex_kwd_function_type complex_function(first, last, context, skipper,
                                               parse_visitor);

    // We have a bool array 'flags' with one flag for each parser as well as a
    // 'counter' array. The kwd directive sets and increments the counter when a
    // successeful parse occurred as well as the slot of the corresponding
    // parser to true in the flags array as soon the minimum repetition
    // requirement is met and keeps that value to true as long as the maximum
    // repetition requirement is met. The parsing takes place here in two steps:
    // 1) parse a keyword and fetch the parser index associated with that
    // keyword 2) call the associated parser and store the parsed value in the
    // matching attribute.
    ...
    for (;;) {
      ...
      if (string_keywords_inst.parse(first, last, parse_visitor, skipper)) {
        ...
        if (!complex_keywords_inst.parse(complex_function)) {
          first = save;
          // Check that we are leaving the keywords parser in a successfull
          // state
  ...
  bool parse_impl(Iterator &first, Iterator const &last, Context &context,
    ...
    typedef detail::parse_dispatcher<
        Elements, Iterator, Context, Skipper, flags_type, counters_type,
        typename traits::wrap_if_not_tuple<Attribute>::type, mpl::true_>
        no_case_parser_visitor_type;

    parser_visitor_type parse_visitor(elements, first, last, context, skipper,
                                      flags, counters, attr);
    no_case_parser_visitor_type no_case_parse_visitor(
        elements, first, last, context, skipper, flags, counters, attr);
    ...
    for (;;) {
      ...
      if (string_keywords_inst.parse(first, last, parse_visitor,
                                     no_case_parse_visitor, skipper)) {
        save = first;
      } else {
        first = save;
...
struct handles_container<repository::qi::keywords<Elements, Modifiers>,
