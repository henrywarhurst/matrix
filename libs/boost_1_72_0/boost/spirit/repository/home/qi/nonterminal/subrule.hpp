...
struct subrule_group_parser : spirit::qi::parser<subrule_group_parser<Defs>> {
  ...
  explicit subrule_group_parser(Defs const &defs) : defs(defs) {}
  ...
  bool parse(Iterator &first, Iterator const &last, Context &context,
    ...
    // Forward to first subrule.
    return parse_subrule(fusion::front(defs).second, first, last, context,
                         skipper, attr);
  }
  ...
  template <int ID, typename Iterator, typename Context, typename Skipper,
            typename Attribute>
  bool parse_subrule_id(Iterator &first, Iterator const &last, Context &context,
                        Skipper const &skipper, Attribute &attr) const {
  ...
  template <typename Def, typename Iterator, typename Context, typename Skipper,
            typename Attribute>
  bool parse_subrule(Def const &def, Iterator &first, Iterator const &last,
    ...
    // rhs parser
    typedef traits::transform_attribute<Attribute, subrule_attr_type,
                                        spirit::qi::domain>
        transform;
...
struct subrule_group
    : proto::extends<typename proto::terminal<subrule_group_parser<Defs>>::type,
                     subrule_group<Defs>> {
...
struct subrule : proto::extends<typename proto::terminal<spirit::qi::reference<
  ...
  typedef
      typename spirit::detail::extract_sig<template_params, encoding_type,
                                           spirit::qi::domain>::type sig_type;
  ...
  template <typename Expr, bool Auto> struct def_type_helper {
    ...
    typedef typename result_of::compile<spirit::qi::domain, Expr>::type
        subject_type;
...
#undef SUBRULE_MODULUS_ASSIGN_OPERATOR
  ...
  template <typename Iterator, typename Group, typename Attributes,
            typename Locals, typename Skipper, typename Attribute>
  bool parse(Iterator &first, Iterator const &last,
             subrule_context<Group, Attributes, Locals> &context,
             Skipper const &skipper, Attribute &attr) const {
    return context.group.template parse_subrule_id<ID_>(first, last, context,
                                                        skipper, attr);
  }
  ...
  bool parse(Iterator & /*first*/, Iterator const & /*last*/
    ...
    BOOST_SPIRIT_ASSERT_FAIL(Iterator, subrule_used_outside_subrule_group,
                             (id_type));
