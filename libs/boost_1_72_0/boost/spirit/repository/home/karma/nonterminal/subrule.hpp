...
struct subrule_group_generator
  ...
  explicit subrule_group_generator(Defs const &defs) : defs(defs) {}
  // from a subrule ID, get the type of a reference to its definition
  template <int ID> struct def_type {
    ...
    typedef
        typename fusion::result_of::at_key<defs_type const, id_type>::type type;
  };

  // from a subrule ID, get a reference to its definition
  template <int ID> typename def_type<ID>::type def() const {
  ...
  struct attribute
      ...
      : mpl::identity<
            typename remove_reference<typename fusion::result_of::front<
                Defs>::type>::type::second_type::attr_type> {};
  ...
  bool generate(OutputIterator &sink, Context &context,
    ...
    // Forward to first subrule.
    return generate_subrule(fusion::front(defs).second, sink, context,
                            delimiter, attr);
  }
  ...
  bool generate(OutputIterator &sink, Context &context,
    ...
    // Forward to first subrule.
    return generate_subrule(fusion::front(defs).second, sink, context,
                            delimiter, attr, params);
  }

  template <int ID, typename OutputIterator, typename Context,
            typename Delimiter, typename Attribute>
  bool generate_subrule_id(OutputIterator &sink, Context &context,
  ...
  bool generate_subrule(Def const &def, OutputIterator &sink,
    ...
    typedef traits::transform_attribute<Attribute const, subrule_attr_type,
                                        spirit::karma::domain>
        transform;
...
struct subrule_group
    : proto::extends<
          typename proto::terminal<subrule_group_generator<Defs>>::type,
          subrule_group<Defs>> {
  ...
  struct properties
      // Forward to first subrule.
      : remove_reference<typename fusion::result_of::front<Defs>::type>::type::
            second_type::subject_type::properties {};

  static size_t const params_size =
      // Forward to first subrule.
      remove_reference<typename fusion::result_of::front<Defs>::type>::type::
          second_type::params_size;
  ...
  generator_type const &get_parameterized_subject() const {
    return generator();
  }
...
///////////////////////////////////////////////////////////////////////////
template <int ID_, typename Locals, typename Attr, typename AttrRef,
          typename Parameters, size_t ParamsSize, typename Subject, bool Auto_>
struct subrule_definition {
  ...
  typedef spirit::karma::detail::generator_binder<Subject, auto_type>
      binder_type;
...
struct subrule
    : proto::extends<typename proto::terminal<spirit::karma::reference<
                         subrule<ID_, T1, T2> const>>::type,
                     subrule<ID_, T1, T2>>,
      spirit::karma::generator<subrule<ID_, T1, T2>> {
  // FIXME should go fetch the real properties of this subrule's definition in
  // the current context, but we don't
  // have the context here (properties would need to be 'template<typename
  // Context> struct properties' instead)
  typedef mpl::int_<spirit::karma::generator_properties::all_properties>
      properties;
  ...
  // The subrule's locals_type: a sequence of types to be used as local
  // variables
  ...
  typedef typename spirit::detail::extract_sig<
      template_params, encoding_type, spirit::karma::domain>::type sig_type;

  // This is the subrule's attribute type
  typedef typename spirit::detail::attr_from_sig<sig_type>::type attr_type;
  typedef typename add_reference<typename add_const<attr_type>::type>::type
      attr_reference_type;

  // parameter_types is a sequence of types passed as parameters to the subrule
  typedef
      typename spirit::detail::params_from_sig<sig_type>::type parameter_types;

  static size_t const params_size =
      fusion::result_of::size<parameter_types>::type::value;

  explicit subrule(std::string const &name_ = "unnamed-subrule")
      : base_type(terminal::make(reference_(*this))), name_(name_) {}

  // compute type of this subrule's definition for expr type Expr
  template <typename Expr, bool Auto> struct def_type_helper {
    // Report invalid expression error as early as possible.
    // If you got an error_invalid_expression error message here,
    // then the expression (Expr) is not a valid spirit karma expression.
    BOOST_SPIRIT_ASSERT_MATCH(spirit::karma::domain, Expr);

    typedef typename result_of::compile<spirit::karma::domain, Expr>::type
        subject_type;

    typedef subrule_definition<ID_, locals_type, attr_type, attr_reference_type,
                               parameter_types, params_size, subject_type,
                               Auto> const type;
  };

  // compute type of subrule group containing only this
  // subrule's definition for expr type Expr
  template <typename Expr, bool Auto> struct group_type_helper {
...
#define SUBRULE_MODULUS_ASSIGN_OPERATOR(lhs_ref, rhs_ref)                      \
  template <typename Expr>                                                     \
  friend typename group_type_helper<Expr, true>::type operator%=(              \
      subrule lhs_ref sr, Expr rhs_ref expr) {                                 \
...
#undef SUBRULE_MODULUS_ASSIGN_OPERATOR
  ...
  template <typename OutputIterator, typename Group, typename Attributes,
            typename Locals, typename Delimiter, typename Attribute>
  bool generate(OutputIterator &sink,
                subrule_context<Group, Attributes, Locals> &context,
                Delimiter const &delimiter, Attribute const &attr) const {
    return context.group.template generate_subrule_id<ID_>(sink, context,
                                                           delimiter, attr);
  }
  ...
  bool generate(OutputIterator & /*sink*/
    ...
    BOOST_SPIRIT_ASSERT_FAIL(OutputIterator, subrule_used_outside_subrule_group,
                             (id_type));
