...
struct alternative : binary_parser<Left, Right, alternative<Left, Right>> {
  ...
  bool parse(Iterator &first, Iterator const &last, Context const &context,
             RContext &rcontext, unused_type) const {
  ...
  bool parse(Iterator &first, Iterator const &last, Context const &context,
    ...
    return detail::parse_alternative(this->left, first, last, context, rcontext,
                                     attr) ||
           detail::parse_alternative(this->right, first, last, context,
                                     rcontext, attr);
  }
};

template <typename Left, typename Right>
inline alternative<typename extension::as_parser<Left>::value_type,
