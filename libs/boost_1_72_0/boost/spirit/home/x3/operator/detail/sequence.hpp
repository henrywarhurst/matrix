...
struct pass_sequence_attribute_unused {
  ...
  template <typename T> static unused_type call(T &) { return unused_type(); }
};

template <typename Attribute> struct pass_sequence_attribute_size_one_view {
  typedef typename fusion::result_of::deref<
      typename fusion::result_of::begin<Attribute>::type>::type type;
...
template <typename Attribute> struct pass_through_sequence_attribute {
  typedef Attribute &type;

  template <typename Attribute_> static Attribute_ &call(Attribute_ &attr) {
    return attr;
  }
};
...
struct pass_sequence_attribute
    : mpl::if_<traits::is_size_one_view<Attribute>,
...
struct partition_attribute {
  ...
  static_assert(actual_size >= expected_size,
...
template <typename L, typename R, typename C> struct get_sequence_types {
  typedef mpl::vector<typename traits::attribute_of<L, C>::type,
...
struct get_sequence_types<sequence<LL, LR>, sequence<RL, RR>, C> {
  ...
  typedef typename mpl::insert_range<left, typename mpl::end<left>::type,
                                     right>::type type;
};

template <typename L, typename R, typename C> struct attribute_of_sequence {
  // Get all sequence attribute types
  typedef typename get_sequence_types<L, R, C>::type all_types;

  // Filter all unused_types
  typedef
      typename mpl::copy_if<all_types, mpl::not_<is_same<mpl::_1, unused_type>>,
  ...
  typedef typename mpl::eval_if<
      mpl::empty<filtered_types>, mpl::identity<unused_type>,
...
bool parse_sequence(Parser const &parser, Iterator &first, Iterator const &last,
...
typename enable_if_c<pass_sequence_container_attribute<Parser, Context>,
                     bool>::type
parse_sequence_container(Parser const &parser, Iterator &first,
...
template <typename Parser, typename Iterator, typename Context,
          typename RContext, typename Attribute>
typename disable_if_c<pass_sequence_container_attribute<Parser, Context>,
                      bool>::type
parse_sequence_container(Parser const &parser, Iterator &first,
                         Iterator const &last, Context const &context,
                         RContext &rcontext, Attribute &attr) {
...
bool parse_sequence(Parser const &parser, Iterator &first, Iterator const &last,
  ...
  Iterator save = first;
  if (parse_sequence_container(parser.left, first, last, context, rcontext,
                               attr) &&
      parse_sequence_container(parser.right, first, last, context, rcontext,
                               attr))
    return true;
  first = save;
  return false;
...
template <typename Parser, typename Iterator, typename Context,
          typename RContext, typename Attribute>
bool parse_sequence_assoc(Parser const &parser, Iterator &first,
                          Iterator const &last, Context const &context,
                          RContext &rcontext, Attribute &attr,
...
bool parse_sequence(Parser const &parser, Iterator &first, Iterator const &last,
  ...
  typedef typename traits::attribute_of<decltype(parser.left), Context>::type
      l_attr_type;
  typedef typename traits::attribute_of<decltype(parser.right), Context>::type
      r_attr_type;
...
struct parse_into_container_impl<sequence<Left, Right>, Context, RContext> {
  ...
  static bool call(parser_type const &parser, Iterator &first,
                   Iterator const &last, Context const &context,
    ...
    static_assert(!is_same<typename traits::attribute_category<Attribute>::type,
                  ...
                  "tuple attribute "
                  "where type of first element is existing key in fusion::map "
                  "and second element "
                  "is value to be stored under that key");

    Attribute attr_;
    if (!parse_sequence(parser, first, last, context, rcontext, attr_,
                        traits::container_attribute())) {
      return false;
    }
  ...
  static bool call(parser_type const &parser, Iterator &first,
                   Iterator const &last, Context const &context,
                   RContext &rcontext, Attribute &attr) {
    typedef typename traits::attribute_of<parser_type, Context>::type
        attribute_type;
