...
template <typename Attribute> struct pass_variant_used {
  typedef Attribute &type;

  static Attribute &call(Attribute &v) { return v; }
};

template <> struct pass_variant_used<unused_type> : pass_variant_unused {};
...
struct pass_parser_attribute {
  ...
  typedef
      typename traits::variant_find_substitute<Attribute, attribute_type>::type
          substitute_type;

  typedef typename mpl::if_<is_same<Attribute, substitute_type>, Attribute &,
                            substitute_type>::type type;

  template <typename Attribute_>
  static Attribute_ &call(Attribute_ &attr, mpl::true_) {
    return attr;
  }

  template <typename Attribute_> static type call(Attribute_ &, mpl::false_) {
    return type();
  }

  template <typename Attribute_> static type call(Attribute_ &attr) {
    return call(attr,
                is_same<Attribute_, typename remove_reference<type>::type>());
  }
};
...
struct pass_non_variant_attribute<
  ...
  typedef typename remove_reference<
      typename fusion::result_of::front<Attribute>::type>::type attr_type;
...
struct pass_variant_attribute<alternative<L, R>, Attribute, Context>
    : mpl::if_c<traits::has_attribute<alternative<L, R>, Context>::value,
                pass_variant_used<Attribute>, pass_variant_unused>::type {
  typedef typename mpl::true_ is_alternative;
};

template <typename L, typename R, typename C> struct get_alternative_types {
...
template <typename L, typename R, typename C> struct attribute_of_alternative {
  ...
  typedef typename mpl::eval_if<mpl::empty<filtered_types>,
                                mpl::identity<unused_type>,
                                make_variant_over<filtered_types>>::type type;
};

template <typename IsAlternative> struct move_if_not_alternative {
  template <typename T1, typename T2>
  static void call(T1 & /* attr_ */, T2 & /* attr */) {}
};

template <> struct move_if_not_alternative<mpl::false_ /*is alternative*/> {
  template <typename T1, typename T2> static void call(T1 &attr_, T2 &attr) {
...
bool parse_alternative(Parser const &p, Iterator &first, Iterator const &last,
  ...
  using pseudo =
      traits::pseudo_attribute<Context, typename pass::type, Iterator>;
