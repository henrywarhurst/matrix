...
struct list : binary_parser<Left, Right, list<Left, Right>> {
  ...
  list(Left const &left, Right const &right) : base_type(left, right) {}
  ...
  bool parse(Iterator &first, Iterator const &last, Context const &context,
    ...
    if (!detail::parse_into_container(this->left, first, last, context,
                                      rcontext, attr))
      return false;

    Iterator iter = first;
    while (this->right.parse(iter, last, context, rcontext, unused) &&
           detail::parse_into_container(this->left, iter, last, context,
                                        rcontext, attr)) {
      first = iter;
    }
...
template <typename Left, typename Right>
inline list<typename extension::as_parser<Left>::value_type,
