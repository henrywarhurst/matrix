...
namespace spirit {
namespace x3 {
template <typename Subject> struct plus : unary_parser<Subject, plus<Subject>> {
  typedef unary_parser<Subject, plus<Subject>> base_type;
  static bool const handles_container = true;

  plus(Subject const &subject) : base_type(subject) {}
  ...
  bool parse(Iterator &first, Iterator const &last, Context const &context,
    ...
    while (detail::parse_into_container(this->subject, first, last, context,
                                        rcontext, attr))
      ;
    return true;
  }
