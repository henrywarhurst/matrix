...
namespace x3 {
namespace detail {
template <typename Attribute, typename Value> struct saver_visitor;

// save to associative fusion container where Key is simple type
template <typename Key, typename Enable = void> struct save_to_assoc_attr {
...
template <typename... T> struct save_to_assoc_attr<variant<T...>> {
...
struct saver_visitor : boost::static_visitor<void> {
  saver_visitor(Attribute &attr, Value &value) : attr(attr), value(value){};

  Attribute &attr;
  Value &value;

  template <typename Key> void operator()(Key) const {
...
struct parser_accepts_container
    : traits::is_substitute<
          typename traits::attribute_of<Parser, Context>::type, Container> {};

template <typename Parser> struct parse_into_container_base_impl {
private:
  // Parser has attribute (synthesize; Attribute is a container)
  template <typename Iterator, typename Context, typename RContext,
            typename Attribute>
  static bool call_synthesize_x(Parser const &parser, Iterator &first,
    ...
    typename pseudo::type val =
        pseudo::call(first, last, traits::value_initialize<value_type>::call());
  ...
  template <typename Iterator, typename Context, typename RContext,
            typename Attribute>
  static bool call_synthesize(Parser const &parser, Iterator &first,
                              Iterator const &last, Context const &context,
                              RContext &rcontext, Attribute &attr) {
    typedef parser_accepts_container<Parser, Attribute, Context>
  ...
  // Parser has attribute (synthesize; Attribute is a single element fusion
  // sequence)
  template <typename Iterator, typename Context, typename RContext,
            typename Attribute>
  static bool
  call_synthesize_into_fusion_seq(Parser const &parser, Iterator &first,
                                  Iterator const &last, Context const &context,
                                  RContext &rcontext, Attribute &attr,
  ...
  call_synthesize_into_fusion_seq(Parser const &parser, Iterator &first,
    ...
    save_to_assoc_attr<key>::call(fusion::front(attr_), fusion::back(attr_),
                                  attr);
    return true;
  }
  ...
  template <typename Iterator, typename Context, typename RContext,
            typename Attribute>
  static bool
  call_synthesize_dispatch_by_seq(Parser const &parser, Iterator &first,
                                  Iterator const &last, Context const &context,
                                  RContext &rcontext, Attribute &attr,
  ...
  template <typename Iterator, typename Context, typename RContext,
            typename Attribute>
  static bool call(Parser const &parser, Iterator &first, Iterator const &last,
...
struct parse_into_container_impl<
  ...
  static bool call(Parser const &parser, Iterator &first, Iterator const &last,
                   Context const &context, RContext &rcontext, unused_type attr,
                   mpl::true_) {
    return parser.parse(first, last, context, rcontext, attr);
  }
  ...
  static bool call(Parser const &parser, Iterator &first, Iterator const &last,
    ...
    typedef parser_attr_is_substitute_for_container_value<Parser, Iterator,
                                                          Attribute, Context>
        parser_attr_is_substitute_for_container_value;

    typedef mpl::or_<parser_accepts_container,
...
bool parse_into_container(Parser const &parser, Iterator &first,
                          Iterator const &last, Context const &context,
                          RContext &rcontext, Attribute &attr) {
