...
template <typename Subject, typename Handler> struct guard;

struct parser_base {};
struct parser_id;

template <typename Derived> struct parser : parser_base {
...
struct unary_parser : parser<Derived> {
  ...
  unary_parser(Subject const &subject) : subject(subject) {
    detail::assert_initialized_rule(subject);
  }
...
template <typename T,
          typename R = decltype(as_spirit_parser(boost::declval<T const &>()))>
...
template <typename T> struct deduce_as_parser<T, void> {};
} // namespace as_parser_guard
...
template <> struct as_parser<unused_type> {
  typedef unused_type type;
  typedef unused_type value_type;
  static type call(unused_type) { return unused; }
};
...
struct as_parser<Derived,
  ...
  static type call(Derived const &p) { return p; }
};

template <typename Derived> struct as_parser<parser<Derived>> {
  typedef Derived const &type;
  typedef Derived value_type;
...
template <typename T>
inline typename extension::as_parser<T>::type as_parser(T const &x) {
  return extension::as_parser<T>::call(x);
}

template <typename Derived>
inline Derived const &as_parser(parser<Derived> const &p) {
  return p.derived();
}
...
// "undefined"
///////////////////////////////////////////////////////////////////////////
template <typename Parser, typename Enable = void> struct get_info {
...
template <typename Parser> std::string what(Parser const &p) {
