...
template <typename Context> inline decltype(auto) _val(Context const &context) {
...
template <typename F, typename Iterator, typename Context, typename RuleContext,
          typename Attribute>
auto call(F f, Iterator &first, Iterator const &last, Context const &context,
          RuleContext &rcontext, Attribute &attr) {
  ...
  return detail::call(f, attr_context,
                      is_callable<F(decltype(attr_context) const &)>());
}
} // namespace x3
} // namespace spirit
} // namespace boost
