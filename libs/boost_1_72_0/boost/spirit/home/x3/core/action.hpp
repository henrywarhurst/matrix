...
template <typename Context> inline bool &_pass(Context const &context) {
...
struct action : unary_parser<Subject, action<Subject, Action>> {
  ...
  action(Subject const &subject, Action f) : base_type(subject), f(f) {}

  template <typename Iterator, typename Context, typename RuleContext,
            typename Attribute>
  bool call_action(Iterator &first, Iterator const &last,
                   Context const &context, RuleContext &rcontext,
  ...
  template <typename Iterator, typename Context, typename RuleContext,
            typename Attribute>
  bool parse_main(Iterator &first, Iterator const &last, Context const &context,
                  RuleContext &rcontext, Attribute &attr) const {
    Iterator save = first;
  ...
  template <typename Iterator, typename Context, typename RuleContext>
  bool parse_main(Iterator &first, Iterator const &last, Context const &context,
                  RuleContext &rcontext, raw_attribute_type &) const {
  ...
  bool parse(Iterator &first, Iterator const &last, Context const &context,
             RuleContext &rcontext, unused_type) const {
    typedef
        typename traits::attribute_of<action<Subject, Action>, Context>::type
            attribute_type;
