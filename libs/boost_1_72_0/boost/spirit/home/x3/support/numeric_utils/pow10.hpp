...
template <> struct pow10_helper<unused_type> {
...
template <> struct pow10_helper<double> {
...
template <> struct pow10_helper<float> {
  static float call(unsigned dim) { return pow10_helper<double>::call(dim); }
};
#else
template <typename T> struct pow10_table {
  constexpr static std::size_t size = std::numeric_limits<T>::max_exponent10;

  constexpr pow10_table() : exponents() {
    exponents[0] = T(1);
...
template <typename T> struct native_pow10_helper {
  static T call(unsigned dim) {
...
template <> struct pow10_helper<float> : native_pow10_helper<float> {};

template <> struct pow10_helper<double> : native_pow10_helper<double> {};

template <>
struct pow10_helper<long double> : native_pow10_helper<long double> {};
#endif
} // namespace detail

template <typename T> inline T pow10(unsigned dim) {
