...
public:
  ...
  error_handler(Iterator first, Iterator last, std::ostream &err_out,
      ...
      : err_out(err_out), file(file), tabs(tabs), pos_cache(first, last) {}
  ...
  template <typename AST> void tag(AST &ast, Iterator first, Iterator last) {
...
template <typename Iterator>
void error_handler<Iterator>::print_indicator(Iterator &start, Iterator last,
                                              char ind) const {
...
template <typename Iterator>
void error_handler<Iterator>::skip_whitespace(Iterator &err_pos,
                                              Iterator last) const {
...
template <typename Iterator>
void error_handler<Iterator>::skip_non_whitespace(Iterator &err_pos,
                                                  Iterator last) const {
  // make sure err_pos does not point to white space
  while (err_pos != last) {
    char c = *err_pos;
...
inline Iterator error_handler<Iterator>::get_line_start(Iterator first,
                                                        Iterator pos) const {
