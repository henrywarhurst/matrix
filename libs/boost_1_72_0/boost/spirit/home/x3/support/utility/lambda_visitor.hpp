...
namespace spirit {
namespace x3 {
template <typename RT, typename... Lambdas> struct lambda_visitor;
...
struct lambda_visitor<RT, F, Lambdas...> : F, lambda_visitor<RT, Lambdas...> {
  ...
  lambda_visitor(F f, Lambdas... lambdas) : F(f), base_type(lambdas...) {}
};

template <typename RT, typename F> struct lambda_visitor<RT, F> : F {
  typedef RT result_type;
  using F::operator();
  lambda_visitor(F f) : F(f) {}
};

template <typename RT> struct lambda_visitor<RT> { typedef RT result_type; };
