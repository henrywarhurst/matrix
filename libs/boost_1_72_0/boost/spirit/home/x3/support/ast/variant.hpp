...
namespace spirit {
namespace x3 {
template <typename T> class forward_ast {
public:
  typedef T type;
...
public:
  ...
  forward_ast(forward_ast const &operand) : p_(new T(operand.get())) {}

  forward_ast(forward_ast &&operand) BOOST_NOEXCEPT : p_(operand.p_) {
    operand.p_ = 0;
  }

  forward_ast(T const &operand) : p_(new T(operand)) {}

  forward_ast(T &&operand) : p_(new T(std::move(operand))) {}
...
namespace detail {
template <typename T> struct remove_forward : mpl::identity<T> {};
...
template <typename... Types> struct variant {
  ...
  using non_self_t // used only for SFINAE checks below
      = std::enable_if_t<!(
          std::is_base_of<base_type, std::remove_reference_t<T>>::value)>;

  variant() BOOST_NOEXCEPT_IF(
      std::is_nothrow_default_constructible<variant_type>::value)
      : var() {}
  ...
  variant(variant const &rhs)
      BOOST_NOEXCEPT_IF(std::is_nothrow_copy_constructible<variant_type>::value)
      : var(rhs.var) {}

  variant(variant &rhs) BOOST_NOEXCEPT_IF(
      (std::is_nothrow_constructible<variant_type, variant_type &>::value))
      : var(rhs.var) {}

  variant(variant &&rhs)
      BOOST_NOEXCEPT_IF(std::is_nothrow_move_constructible<variant_type>::value)
  ...
  template <typename T, class = non_self_t<T>>
  variant &operator=(T const &rhs) BOOST_NOEXCEPT_IF(
      (std::is_nothrow_assignable<variant_type, T const &>::value)) {
    var = rhs;
    return *this;
  }

  template <typename T, class = non_self_t<T>>
  variant &operator=(T &&rhs) BOOST_NOEXCEPT_IF(
      (std::is_nothrow_assignable<variant_type, T &&>::value)) {
    var = std::forward<T>(rhs);
    return *this;
  }

  template <typename F> typename F::result_type apply_visitor(F const &v) {
  ...
  template <typename F> typename F::result_type apply_visitor(F &v) {
    return var.apply_visitor(v);
  }

  template <typename F> typename F::result_type apply_visitor(F &v) const {
    return var.apply_visitor(v);
  }
