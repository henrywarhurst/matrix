...
template <typename Encoding> struct case_compare {
  template <typename Char, typename CharSet>
  bool in_set(Char ch, CharSet const &set) {
    return set.test(ch);
  }

  template <typename Char> int32_t operator()(Char lc, Char rc) const {
    return lc - rc;
  }
...
template <typename Encoding> struct no_case_compare {
  template <typename Char, typename CharSet>
  bool in_set(Char ch_, CharSet const &set) {
    using char_type = typename Encoding::classify_type;
    auto ch = char_type(ch_);
    return set.test(ch) ||
           set.test(Encoding::islower(ch) ? Encoding::toupper(ch)
                                          : Encoding::tolower(ch));
  }

  template <typename Char> int32_t operator()(Char lc_, Char const rc_) const {
    using char_type = typename Encoding::classify_type;
    auto lc = char_type(lc_);
    auto rc = char_type(rc_);
    return Encoding::islower(rc) ? Encoding::tolower(lc) - rc
                                 : Encoding::toupper(lc) - rc;
  }
