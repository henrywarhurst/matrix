...
namespace spirit {
namespace x3 {
template <typename Value> struct attr_parser : parser<attr_parser<Value>> {
  ...
  attr_parser(Value const &value) : value_(value) {}
  attr_parser(Value &&value) : value_(std::move(value)) {}

  template <typename Iterator, typename Context, typename RuleContext,
            typename Attribute>
  bool parse(Iterator & /* first */, Iterator const & /* last */
             ,
             Context const & /* context */, RuleContext &,
...
template <typename Value> struct get_info<attr_parser<Value>> {
...
struct attr_gen {
  ...
  attr_parser<typename remove_cv<typename remove_reference<Value>::type>::type>
