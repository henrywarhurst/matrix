...
public:
  any_parser() : _content(nullptr) {}

  template <typename Expr,
            typename Enable = typename enable_if<traits::is_parser<Expr>>::type>
  any_parser(Expr const &expr) : _content(new holder<Expr>(expr)) {}
  ...
  bool parse(Iterator_ &first, Iterator_ const &last, Context_ const &context,
             unused_type, Attribute &attr) const {
    BOOST_STATIC_ASSERT_MSG((is_same<Iterator, Iterator_>::value),
                            "Incompatible iterator used");

    BOOST_ASSERT_MSG((_content != nullptr),
  ...
  bool parse(Iterator_ &first, Iterator_ const &last, Context_ const &context,
             unused_type, Attribute_ &attr_) const {
    Attribute attr;
...
private:
  ...
  template <typename Expr> struct holder : placeholder {
    typedef typename extension::as_parser<Expr>::value_type parser_type;

    explicit holder(Expr const &p) : _parser(as_parser(p)) {}
...
struct get_info<any_parser<Iterator, Attribute, Context>> {
  ...
  std::string
  operator()(any_parser<Iterator, Attribute, Context> const &p) const {
    return p.get_info();
  }
};
} // namespace x3
