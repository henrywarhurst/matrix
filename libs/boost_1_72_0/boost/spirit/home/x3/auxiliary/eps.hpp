...
struct semantic_predicate : parser<semantic_predicate> {
  ...
  semantic_predicate(bool predicate) : predicate(predicate) {}

  template <typename Iterator, typename Context, typename Attribute>
  bool parse(Iterator &first, Iterator const &last, Context const &context,
             unused_type, Attribute &) const {
...
struct lazy_semantic_predicate : parser<lazy_semantic_predicate<F>> {
  ...
  lazy_semantic_predicate(F f) : f(f) {}
...
struct eps_parser : parser<eps_parser> {
  ...
  template <typename F> lazy_semantic_predicate<F> operator()(F f) const {
    return {f};
  }
};
