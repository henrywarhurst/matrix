...
struct with_value_holder : unary_parser<Subject, Derived> {
  ...
  with_value_holder(Subject const &subject, T &&val)
      : base_type(subject), val(std::forward<T>(val)) {}
};
...
struct with_directive
  ...
  typedef with_value_holder<Subject, with_directive<Subject, ID, T>, T>
      base_type;
  ...
  bool parse(Iterator &first, Iterator const &last, Context const &context,
    ...
    return this->subject.parse(
        first, last, make_context<ID>(this->val, context), rcontext, attr);
  }
};

template <typename ID, typename T> struct with_gen {
  T &&val;
...
template <typename ID, typename T> inline with_gen<ID, T> with(T &&val) {
