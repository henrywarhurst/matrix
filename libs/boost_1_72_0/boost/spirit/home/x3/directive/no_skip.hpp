...
struct no_skip_directive : unary_parser<Subject, no_skip_directive<Subject>> {
  ...
  no_skip_directive(Subject const &subject) : base_type(subject) {}
  ...
  parse(Iterator &first, Iterator const &last, Context const &context,
    ...
    typedef unused_skipper<typename remove_reference<decltype(skipper)>::type>
        unused_skipper_type;
    unused_skipper_type unused_skipper(skipper);

    return this->subject.parse(
        first, last, make_context<skipper_tag>(unused_skipper, context),
        rcontext, attr);
  }
  template <typename Iterator, typename Context, typename RContext,
            typename Attribute>
