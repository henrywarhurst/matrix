...
struct expect_directive : unary_parser<Subject, expect_directive<Subject>> {
  ...
  expect_directive(Subject const &subject) : base_type(subject) {}
  ...
  bool parse(Iterator &first, Iterator const &last, Context const &context,
    ...
    if (!r) {
      boost::throw_exception(
          expectation_failure<Iterator>(first, what(this->subject)));
    }
    return r;
  }
};
...
struct parse_into_container_impl<expect_directive<Subject>, Context, RContext> {
  ...
  static bool call(expect_directive<Subject> const &parser, Iterator &first,
                   Iterator const &last, Context const &context,
                   RContext &rcontext, Attribute &attr) {
    bool r = parse_into_container(parser.subject, first, last, context,
                                  rcontext, attr);

    if (!r) {
      boost::throw_exception(
          expectation_failure<Iterator>(first, what(parser.subject)));
    }
    return r;
  }
};
