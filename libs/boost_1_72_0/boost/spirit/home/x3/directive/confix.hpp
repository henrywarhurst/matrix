...
struct confix_directive
  ...
  typedef unary_parser<Subject, confix_directive<Prefix, Subject, Postfix>>
      base_type;
  static bool const is_pass_through_unary = true;
  static bool const handles_container = Subject::handles_container;

  confix_directive(Prefix const &prefix, Subject const &subject,
                   Postfix const &postfix)
      : base_type(subject), prefix(prefix), postfix(postfix) {}
...
template <typename Prefix, typename Postfix> struct confix_gen {
  template <typename Subject>
  confix_directive<Prefix, typename extension::as_parser<Subject>::value_type,
                   Postfix>
