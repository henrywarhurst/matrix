...
#define BOOST_SPIRIT_X3_BASIC_CLASSIFY(name)                                   \
  template <typename Char> static bool is(name##_tag, Char ch) {               \
    return encoding::is##name BOOST_PREVENT_MACRO_SUBSTITUTION(                \
...
#define BOOST_SPIRIT_X3_CLASSIFY(name)                                         \
  template <typename Char> static bool is(name##_tag, Char ch) {               \
    return encoding::is_##name BOOST_PREVENT_MACRO_SUBSTITUTION(               \
        detail::cast_char<char_type>(ch));                                     \
  }                                                                            \
  /***/
