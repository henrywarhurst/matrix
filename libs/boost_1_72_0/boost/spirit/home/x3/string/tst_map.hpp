...
template <typename Char, typename T> struct tst_map {
  ...
  bool add(Iterator first, Iterator last,
    ...
    if (first != last) {
      ...
      if (first != last) {
        return node::add(r.first->second.root, first, last, val, this) ? true
                                                                       : false;
      } else {
  ...
  template <typename Iterator> void remove(Iterator first, Iterator last) {
    if (first != last) {
