...
namespace spirit {
namespace x3 {
template <typename Encoding, typename T = unused_type,
...
struct symbols_parser : parser<symbols_parser<Encoding, T, Lookup>> {
  ...
  symbols_parser(std::string const &name = "symbols")
      : add{*this}, remove{*this}, lookup(std::make_shared<Lookup>()),
        name_(name) {}

  symbols_parser(symbols_parser const &syms)
      : add{*this}, remove{*this}, lookup(syms.lookup), name_(syms.name_) {}
...
struct get_info<symbols_parser<Encoding, T, Lookup>> {
  ...
  result_type
  operator()(symbols_parser<Encoding, T, Lookup> const &symbols) const {
    return symbols.name();
  }
};
