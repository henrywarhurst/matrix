...
namespace spirit {
namespace x3 {
template <typename V, typename T, boost::endian::order endian, std::size_t bits>
struct binary_lit_parser : parser<binary_lit_parser<V, T, endian, bits>> {
  ...
  bool parse(Iterator &first, Iterator const &last, Context const &context,
             unused_type, Attribute &attr_param) const {
...
#define BOOST_SPIRIT_MAKE_BINARY_PRIMITIVE(name, endiantype, attrtype, bits)   \
  typedef any_binary_parser<attrtype, boost::endian::order::endiantype, bits>  \
      name##type;                                                              \
...
// for floating point types is required. Must be removed as soon as the endian
// library properly supports floating point types.
...
struct get_info<any_binary_parser<T, endian::order::little, bits>> {
  ...
  std::string
  operator()(any_binary_parser<T, endian::order::little, bits> const &p) const {
...
struct get_info<any_binary_parser<T, endian::order::big, bits>> {
  typedef std::string result_type;
  std::string
  operator()(any_binary_parser<T, endian::order::big, bits> const &p) const {
...
struct get_info<binary_lit_parser<V, T, endian::order::little, bits>> {
  typedef std::string result_type;
  std::string operator()(
      binary_lit_parser<V, T, endian::order::little, bits> const &p) const {
    return "little-endian binary";
  }
};
...
struct get_info<binary_lit_parser<V, T, endian::order::big, bits>> {
  typedef std::string result_type;
  std::string
  operator()(binary_lit_parser<V, T, endian::order::big, bits> const &p) const {
    return "big-endian binary";
  }
};
