...
namespace x3 {
namespace detail {
template <typename Char> inline void token_printer(std::ostream &o, Char c) {
  // allow customization of the token printer routine
  x3::traits::print_token(o, c);
}
} // namespace detail

template <int IndentSpaces = 2, int CharsToPrint = 20> struct simple_trace {
  simple_trace(std::ostream &out) : out(out), indent(0) {}
  ...
  void operator()(Iterator const &first, Iterator const &last,
                  Attribute const &attr, State state,
...
namespace detail {
typedef simple_trace<BOOST_SPIRIT_X3_DEBUG_INDENT,
                     BOOST_SPIRIT_X3_DEBUG_PRINT_SOME>
    simple_trace_type;
