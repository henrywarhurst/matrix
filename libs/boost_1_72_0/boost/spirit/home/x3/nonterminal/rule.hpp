...
parse_rule(rule<ID, Attribute> /* rule_ */
           ,
...
struct rule_definition
  ...
  rule_definition(RHS const &rhs, char const *name) : rhs(rhs), name(name) {}

  template <typename Iterator, typename Context, typename Attribute_>
  bool parse(Iterator &first, Iterator const &last, Context const &context,
             unused_type, Attribute_ &attr) const {
    return detail::rule_parser<attribute_type, ID>::call_rule_definition(
        rhs, name, first, last, context, attr, mpl::bool_<force_attribute>());
  }
...
struct rule : parser<rule<ID, Attribute>> {
  ...
  rule(char const *name) : name(name) {}
  ...
  bool parse(Iterator &first, Iterator const &last, Context const &context,
    ...
    using transform =
        traits::transform_attribute<Attribute_, attribute_type, parser_id>;
...
namespace traits {
template <typename T, typename Enable = void> struct is_rule : mpl::false_ {};
...
struct get_info<T, typename enable_if<traits::is_rule<T>>::type> {
  ...
  std::string operator()(T const &r) const {
    BOOST_ASSERT_MSG(
        r.name, "uninitialized rule"); // static initialization order fiasco
...
#define BOOST_SPIRIT_DECLARE_(r, data, rule_type)                              \
  template <typename Iterator, typename Context>                               \
  bool parse_rule(rule_type rule_, Iterator &first, Iterator const &last,      \
                  Context const &context, rule_type::attribute_type &attr);    \
  /***/

#define BOOST_SPIRIT_DECLARE(...)                                              \
  BOOST_PP_SEQ_FOR_EACH(BOOST_SPIRIT_DECLARE_, _,                              \
...
#define BOOST_SPIRIT_DEFINE_(r, data, rule_name)                               \
  template <typename Iterator, typename Context>                               \
  inline bool parse_rule(decltype(rule_name) /* rule_ */                       \
                         ,                                                     \
