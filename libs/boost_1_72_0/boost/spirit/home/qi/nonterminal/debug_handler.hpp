...
struct debug_handler {
  ...
  debug_handler(function_type subject_, F f_, std::string const &rule_name_)
      : subject(subject_), f(f_), rule_name(rule_name_) {}
...
void debug(rule<Iterator, T1, T2, T3, T4> &r, F f) {
  ...
  typedef debug_handler<Iterator, typename rule_type::context_type,
                        typename rule_type::skipper_type, F>
      debug_handler;
...
template <typename T> struct get_simple_trace { typedef simple_trace type; };
} // namespace detail
...
void debug(rule<Iterator, T1, T2, T3, T4> &r) {
  ...
  typedef debug_handler<Iterator, typename rule_type::context_type,
                        typename rule_type::skipper_type, simple_trace>
      debug_handler;
