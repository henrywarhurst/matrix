...
template <typename Iterator, bool active> struct reset_on_exit {
...
// side of rule.
template <typename Iterator> struct reset_on_exit<Iterator, true> {
  reset_on_exit(Iterator &it)
      : it_(it), inhibit_clear_queue_(spirit::traits::inhibit_clear_queue(it)) {
...
struct error_handler {
  typedef function<bool(Iterator &first, Iterator const &last, Context &context,
                        Skipper const &skipper)>
      function_type;

  error_handler(function_type subject_, F f_) : subject(subject_), f(f_) {}

  bool operator()(Iterator &first, Iterator const &last, Context &context,
    ...
    for (;;) {
      try {
        ...
        typedef fusion::vector<Iterator &, Iterator const &, Iterator const &,
                               info const &>
            params;
        ...
        switch (r) {
        case fail:
          BOOST_ASSERT(!traits::is_multi_pass<Iterator>::value ||
                       action == retry || action == fail);
          return false;
        case retry:
          BOOST_ASSERT(!traits::is_multi_pass<Iterator>::value ||
...
template <error_handler_result action, typename Iterator, typename T0,
          typename T1, typename T2, typename F>
void on_error(rule<Iterator, T0, T1, T2> &r, F f) {
  typedef rule<Iterator, T0, T1, T2> rule_type;

  typedef error_handler<Iterator, typename rule_type::context_type,
                        typename rule_type::skipper_type, F, action>
      error_handler;
