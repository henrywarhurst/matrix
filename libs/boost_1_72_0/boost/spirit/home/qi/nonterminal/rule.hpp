...
// VC7.1 has problems to resolve 'rule' without explicit template parameters
  ...
  template <typename Context, typename Iterator_> struct attribute {
  ...
  bool parse(Iterator &first, Iterator const &last, Context & /*context*/,
    ...
    BOOST_STATIC_ASSERT_MSG((is_same<skipper_type, unused_type>::value ||
                            ...
                            "The rule was instantiated with a skipper type but "
                            "you have not pass any. "
    ...
    if (f) {
      ...
      // rhs parser
      typedef traits::transform_attribute<Attribute, attr_type, domain>
          transform;
