...
template <typename T> struct real_accumulator : mpl::identity<T> {};
...
template <typename T, typename RealPolicies> struct real_impl {
  ...
  static bool parse(Iterator &first, Iterator const &last, Attribute &attr,
    ...
    if (!got_a_number) {
      ...
      // We got a number and we still see digits. This happens if acc_n (an
      // integer) exceeds the integer's capacity. Collect the excess digits.
