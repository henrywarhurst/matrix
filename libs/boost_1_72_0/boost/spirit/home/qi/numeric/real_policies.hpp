...
template <typename T, typename Enable = void> struct max_digits10 {
...
///////////////////////////////////////////////////////////////////////////
template <typename T> struct ureal_policies {
  // Versioning
  ...
  static bool parse_n(Iterator &first, Iterator const &last, Attribute &attr_) {
    typedef extract_uint<Attribute, 10, 1,
                         traits::max_digits10<T>::value // See notes on
                                                        // max_digits10 above
                         ,
                         false, true>
        extract_uint;
  ...
  template <typename Iterator>
  static std::size_t ignore_excess_digits(Iterator &first,
                                          Iterator const &last) {
    Iterator save = first;
  ...
  static bool parse_frac_n(Iterator &first, Iterator const &last,
    ...
    bool r = extract_uint<Attribute, 10, 1, -1, true, true>::call(first, last,
                                                                  attr_);
    if (r) {
...
///////////////////////////////////////////////////////////////////////////
template <typename T> struct real_policies : ureal_policies<T> {
  template <typename Iterator>
  static bool parse_sign(Iterator &first, Iterator const &last) {
...
template <typename T> struct strict_ureal_policies : ureal_policies<T> {
  static bool const expect_dot = true;
};

template <typename T> struct strict_real_policies : real_policies<T> {
  static bool const expect_dot = true;
};
} // namespace qi
} // namespace spirit
