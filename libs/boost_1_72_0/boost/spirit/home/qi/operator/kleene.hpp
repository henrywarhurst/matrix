...
template <typename Subject> struct kleene : unary_parser<kleene<Subject>> {
  ...
  kleene(Subject const &subject_) : subject(subject_) {}

  template <typename F> bool parse_container(F f) const {
    while (!f(subject))
      ;
    return true;
  }
...
//     template <typename Attribute, typename Subject, typename Context,
//     typename Iterator> struct is_attribute_compatible<Attribute,
