...
#define BOOST_SPIRIT_ENABLE_BINARY(name)                                       \
  template <> struct use_terminal<qi::domain, tag::name> : mpl::true_ {};      \
                                                                               \
...
template <> struct integer<16> {
  enum { size = 2 };
  typedef uint_least16_t type;
};

template <> struct integer<32> {
  enum { size = 4 };
...
template <> struct floating_point<32> {
  enum { size = 4 };
  typedef float type;
};

template <> struct floating_point<64> {
  enum { size = 8 };
  typedef double type;
};
...
template <> struct what<boost::spirit::endian::endianness::native> {
  static std::string is() { return "native-endian binary"; }
};
...
struct any_binary_parser
    : primitive_parser<any_binary_parser<T, endian, bits>> {
  template <typename Context, typename Iterator> struct attribute {
    typedef boost::spirit::endian::endian<endian, typename T::type, bits> type;
  };
