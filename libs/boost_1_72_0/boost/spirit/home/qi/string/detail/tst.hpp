...
template <typename Char, typename T> struct tst_node {
  tst_node(Char id_) : id(id_), data(0), lt(0), eq(0), gt(0) {}
  ...
  static T *find(tst_node *start, Iterator &first, Iterator last,
                 Filter filter) {
    if (first == last)
      return 0;
    ...
    while (p && i != last) {
      typename std::iterator_traits<Iterator>::value_type c =
          filter(*i); // filter only the input
  ...
  static T *add(tst_node *&start, Iterator first, Iterator last,
                typename boost::call_traits<T>::param_type val, Alloc *alloc) {
    if (first == last)
      return 0;
  ...
  template <typename Iterator, typename Alloc>
  static void remove(tst_node *&p, Iterator first, Iterator last,
                     Alloc *alloc) {
    if (p == 0 || first == last)
      return;
