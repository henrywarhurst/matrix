...
struct symbols
  ...
  template <typename Str> adder const &operator=(Str const &str) {
    lookup->clear();
    return add(str);
  }
...
#endif
  template <typename F> void for_each(F f) const { lookup->for_each(f); }

  template <typename Str> value_type &at(Str const &str) {
  ...
  template <typename Str> value_type *find(Str const &str) {
    return find_impl(traits::get_begin<Char>(str), traits::get_end<Char>(str));
  }

  template <typename Str> value_type const *find(Str const &str) const {
    return find_impl(traits::get_begin<Char>(str), traits::get_end<Char>(str));
  }
...
public:
  ...
  struct adder {
    ...
    adder(symbols &sym_) : sym(sym_) {}

    template <typename Iterator>
    adder const &operator()(Iterator const &first, Iterator const &last,
                            T const &val) const {
    ...
    adder const &operator()(Str const &s, T const &val = T()) const {
      sym.lookup->add(traits::get_begin<Char>(s), traits::get_end<Char>(s),
                      val);
      return *this;
    }

    template <typename Str> adder const &operator,(Str const &s) const {
      sym.lookup->add(traits::get_begin<Char>(s), traits::get_end<Char>(s),
                      T());
      return *this;
    }
  ...
  struct remover {
    ...
    remover(symbols &sym_) : sym(sym_) {}

    template <typename Iterator>
    remover const &operator()(Iterator const &first,
                              Iterator const &last) const {
      sym.lookup->remove(first, last);
      return *this;
    }

    template <typename Str> remover const &operator()(Str const &s) const {
...
struct make_primitive<reference<symbols<Char, T, Lookup, Filter>>, Modifiers> {
  ...
  typedef typename mpl::if_<no_case, symbols<Char, T, Lookup, nc_filter>,
...
struct handles_container<qi::symbols<Char, T, Lookup, Filter>, Attr, Context,
