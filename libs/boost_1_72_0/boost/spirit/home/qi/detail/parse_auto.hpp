...
struct parse_impl<
    Expr, typename enable_if<
              mpl::and_<traits::meta_create_exists<qi::domain, Expr>,
...
// parse_impl<wchar_t>
template <> struct parse_impl<char> {
  template <typename Iterator>
...
template <> struct parse_impl<wchar_t> {
  template <typename Iterator>
...
struct phrase_parse_impl<
  ...
  static bool call(Iterator &first, Iterator last, Expr &expr,
    ...
    return qi::phrase_parse(first, last, create_parser<Expr>(), skipper,
                            post_skip, expr);
  }
  ...
  static bool call(Iterator &first, Iterator last, Expr const &expr,
                   ...
                   BOOST_SCOPED_ENUM(skip_flag) post_skip) {
    return qi::phrase_parse(first, last, create_parser<Expr>(), skipper,
                            post_skip, const_cast<Expr &>(expr));
  }
};
...
template <> struct phrase_parse_impl<char> {
  template <typename Iterator, typename Skipper>
  static bool call(Iterator &first, Iterator last, char &expr,
                   Skipper const &skipper,
                   BOOST_SCOPED_ENUM(skip_flag) post_skip) {
    return qi::phrase_parse(first, last, create_parser<char>(), skipper,
                            post_skip, expr);
  }
...
template <> struct phrase_parse_impl<wchar_t> {
  template <typename Iterator, typename Skipper>
  static bool call(Iterator &first, Iterator last, wchar_t &expr,
                   Skipper const &skipper,
                   BOOST_SCOPED_ENUM(skip_flag) post_skip) {
    return qi::phrase_parse(first, last, create_parser<wchar_t>(), skipper,
                            post_skip, expr);
  }
...
inline bool
phrase_parse(Iterator &first, Iterator last, Expr &expr, Skipper const &skipper,
