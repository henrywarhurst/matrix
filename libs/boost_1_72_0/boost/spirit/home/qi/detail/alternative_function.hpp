...
namespace qi {
namespace detail {
template <typename Variant, typename Expected> struct find_substitute {
  ...
  typedef typename mpl::eval_if<is_same<iter, end>, mpl::identity<Expected>,
...
struct alternative_function {
  alternative_function(Iterator &first_, Iterator const &last_,
  ...
  bool call_optional_or_variant(Component const &component, mpl::true_) const {
    ...
    // with the type optional::value_type. If the expected attribute is unused
    // type, use it instead.
    typedef typename traits::attribute_of<Component, Context, Iterator>::type
        expected_type;

    typename mpl::if_<is_same<expected_type, unused_type>, unused_type,
                      typename Attribute::value_type>::type val;
  ...
  bool call_variant(Component const &component, mpl::false_) const {
    ...
    typename find_substitute<
        Attribute,
        typename traits::attribute_of<Component, Context, Iterator>::type>::type
        val;
  ...
  bool call_unused(Component const &component, mpl::true_) const {
    ...
    return call(
        component,
        mpl::and_<spirit::traits::not_is_variant<Attribute, qi::domain>,
...
struct alternative_function<Iterator, Context, Skipper, unused_type const> {
  alternative_function(Iterator &first_, Iterator const &last_,
                       Context &context_, Skipper const &skipper_, unused_type)
      : first(first_), last(last_), context(context_), skipper(skipper_) {}
