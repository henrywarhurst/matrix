...
template <typename Elements> struct sequence_properties {
...
struct base_sequence : nary_generator<Derived> {
  ...
  base_sequence(Elements const &elements) : elements(elements) {}
  ...
  bool generate_impl(OutputIterator &sink, Context &ctx, Delimiter const &d,
                     Attribute &attr_, Pred1, Pred2) const {
    typedef detail::fail_function<OutputIterator, Context, Delimiter>
        fail_function;
    ...
    typename traits::wrap_if_not_tuple<
        Attribute,
        typename mpl::and_<traits::one_element_sequence<attr_type_>,
                           mpl::not_<traits::one_element_sequence<Attribute>>>::
            type>::type attr(attr_);

    // return false if *any* of the generators fail
    bool r = spirit::any_if(elements, attr, fail_function(sink, ctx, d),
                            predicate());
    ...
    return !r && (!Strict::value ||
                  ...
                  // false positives (failing generators, even if they
                  // shouldn't) if the embedded component is restricting the
                  // number of container elements it consumes (i.e. repeat).
                  // This solution is not optimal but much better than letting
                  // _all_ repetitive components fail.
                  Pred1::value ||
                  size_type(traits::sequence_size<attr_type_>::value) ==
                      traits::size(attr_));
  }
  ...
  template <typename OutputIterator, typename Context, typename Delimiter,
            typename Attribute>
  bool generate_impl(OutputIterator &sink, Context &ctx, Delimiter const &d,
                     Attribute const &attr_, mpl::true_, mpl::false_) const {
    // return false if *any* of the generators fail
    typedef detail::fail_function<OutputIterator, Context, Delimiter>
        fail_function;

    typedef typename traits::container_iterator<
        typename add_const<Attribute>::type>::type iterator_type;

    typedef typename traits::make_indirect_iterator<iterator_type>::type
        indirect_iterator_type;
    typedef detail::pass_container<fail_function, Attribute,
                                   indirect_iterator_type, mpl::true_>
        pass_container;
...
struct sequence : base_sequence<Elements, mpl::false_, sequence<Elements>> {
  ...
  sequence(Elements const &subject) : base_sequence_(subject) {}
};
...
struct strict_sequence
  ...
  strict_sequence(Elements const &subject) : base_sequence_(subject) {}
};
...
template <typename Iterator> struct make_sequence_iterator_range {
...
template <typename Elements, typename Attribute, typename Context,
          typename Iterator>
struct handles_container<karma::strict_sequence<Elements>, Attribute, Context,
                         Iterator> : mpl::true_ {};
} // namespace traits
} // namespace spirit
} // namespace boost
