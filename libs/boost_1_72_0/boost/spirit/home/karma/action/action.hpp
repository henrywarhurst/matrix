...
struct action : unary_generator<action<Subject, Action>> {
  ...
  action(Subject const &subject, Action f) : subject(subject), f(f) {}
...
template <> struct make_component<karma::domain, tag::action> {
  template <typename Sig> struct result;

  template <typename This, typename Elements, typename Modifiers>
  struct result<This(Elements, Modifiers)> {
    typedef
        typename remove_const<typename Elements::car_type>::type subject_type;

    typedef typename remove_const<typename Elements::cdr_type::car_type>::type
        action_type;
...
struct handles_container<karma::action<Subject, Action>, Attribute, Context,
                         Iterator>
