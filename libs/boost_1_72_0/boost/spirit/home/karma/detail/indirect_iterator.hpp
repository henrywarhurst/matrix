...
class indirect_iterator
  ...
  typedef
      typename std::iterator_traits<Iterator>::reference base_reference_type;

  typedef boost::iterator_facade<indirect_iterator<Iterator>, base_value_type,
                                 boost::forward_traversal_tag,
                                 base_reference_type>
      base_type;

public:
  indirect_iterator(Iterator &iter) : iter_(&iter) {}
  indirect_iterator(indirect_iterator const &iter) : iter_(iter.iter_) {}
...
namespace spirit {
namespace traits {
template <typename Iterator> struct make_indirect_iterator {
...
template <> struct make_indirect_iterator<unused_type const *> {
