...
namespace detail {
///////////////////////////////////////////////////////////////////////////
template <typename Expr, typename Enable = void> struct generate_impl {
...
struct generate_impl<
    Expr, typename enable_if<traits::matches<karma::domain, Expr>>::type> {
  ...
  static bool call(OutputIterator &target_sink, Expr const &expr) {
    ...
    output_iterator<OutputIterator, mpl::int_<properties::value>> sink(
        target_sink);
    return compile<karma::domain>(expr).generate(sink, unused, unused, unused);
  }

  template <typename OutputIterator, typename Properties>
  static bool call(detail::output_iterator<OutputIterator, Properties> &sink,
                   Expr const &expr) {
    return compile<karma::domain>(expr).generate(sink, unused, unused, unused);
  }
};
...
struct generate_delimited_impl<
  ...
  static bool call(detail::output_iterator<OutputIterator, Properties> &sink,
    ...
    return compile<karma::domain>(expr).generate(sink, unused, delimiter_,
                                                 unused);
  }
};
