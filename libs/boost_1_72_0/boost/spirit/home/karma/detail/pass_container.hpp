...
struct not_compatible_element
    : mpl::and_<negate_weak_substitute_if_not<Sequence, Container, Attribute>,
                negate_weak_substitute_if_not<Sequence, ValueType, Attribute>> {
};
...
struct pass_through_container_container
    : mpl::or_<traits::is_weak_substitute<Container, Attribute>,
...
struct pass_through_container<Container, ValueType, Attribute, Sequence,
                              karma::domain>
    : karma::detail::pass_through_container<Container, ValueType, Attribute,
                                            Sequence> {};
} // namespace traits
} // namespace spirit
} // namespace boost
...
namespace karma {
namespace detail {
template <typename Iterator> struct pass_container_base {
  pass_container_base(Iterator begin, Iterator end) : iter(begin), end(end) {}

  mutable Iterator iter;
  mutable Iterator end;
};

template <typename Iterator> struct pass_container_base<Iterator &> {
  pass_container_base(Iterator &begin, Iterator &end) : iter(begin), end(end) {}
...
struct pass_container : pass_container_base<Iterator> {
  ...
  bool dispatch_attribute(Component const &component, mpl::true_) const {
    ...
    typedef typename traits::attribute_of<Component, context_type>::type
        lhs_attribute;
    ...
    typedef mpl::and_<
        ...
        traits::pass_through_container<Attr, value_type, lhs_attribute,
                                       Sequence, karma::domain>>
        predicate;
