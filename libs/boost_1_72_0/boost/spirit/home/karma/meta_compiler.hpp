...
struct make_primitive // by default, return it as-is
{
  ...
  template <typename T_> T_ &operator()(T_ &val, unused_type) const {
    return val;
  }
...
template <> struct make_component<karma::domain, proto::tag::terminal> {
...
template <typename Tag> struct make_component<karma::domain, Tag> {
  template <typename Sig> struct result;
...
template <> struct make_component<karma::domain, proto::tag::function> {
  template <typename Sig> struct result;
...
template <> struct make_component<karma::domain, tag::directive> {
  template <typename Sig> struct result;
