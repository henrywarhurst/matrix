...
#define BOOST_SPIRIT_ENABLE_BINARY(name)                                       \
  template <> struct use_terminal<karma::domain, tag::name> : mpl::true_ {};   \
                                                                               \
...
#ifdef BOOST_HAS_LONG_LONG
  BOOST_SPIRIT_ASSERT_MSG(bits == 8 || bits == 16 || bits == 32 || bits == 64,
                          not_supported_binary_size, ());
#else
  BOOST_SPIRIT_ASSERT_MSG(bits == 8 || bits == 16 || bits == 32,
                          not_supported_binary_size, ());
#endif
};

template <> struct integer<8> { typedef uint_least8_t type; };

template <> struct integer<16> { typedef uint_least16_t type; };

template <> struct integer<32> { typedef uint_least32_t type; };

#ifdef BOOST_HAS_LONG_LONG
template <> struct integer<64> { typedef uint_least64_t type; };
#endif

template <int bits> struct floating_point {
...
template <> struct what<boost::spirit::endian::endianness::native> {
  static info is() { return info("native-endian binary"); }
};

template <> struct what<boost::spirit::endian::endianness::little> {
  static info is() { return info("little-endian binary"); }
};

template <> struct what<boost::spirit::endian::endianness::big> {
...
// warning C4244: 'argument' : conversion from 'const int' to 'foo', possible
// loss of data
#pragma warning(push)
...
#pragma warning(pop)
  ...
  template <typename Context> static info what(Context const & /*context*/) {
...
struct literal_binary_generator
  ...
  template <typename V> literal_binary_generator(V const &v) {
#if defined(BOOST_MSVC)
// warning C4244: 'argument' : conversion from 'const int' to 'foo', possible
// loss of data
#pragma warning(push)
...
#pragma warning(pop)
  ...
  typedef boost::spirit::endian::endian<endian, typename T::type, bits>
      data_type;
...
#define BOOST_SPIRIT_MAKE_BINARY_PRIMITIVE(name, endiantype, bits)             \
  ...
  struct make_primitive<terminal_ex<tag::name, fusion::vector1<A0>>,           \
                        Modifiers>                                             \
      : detail::basic_binary_literal<                                          \
            Modifiers, detail::integer<bits>,                                  \
...
#define BOOST_SPIRIT_MAKE_BINARY_IEEE754_PRIMITIVE(name, endiantype, bits)     \
  ...
  struct make_primitive<terminal_ex<tag::name, fusion::vector1<A0>>,           \
                        Modifiers>                                             \
      : detail::basic_binary_literal<                                          \
            Modifiers, detail::floating_point<bits>,                           \
            boost::spirit::endian::endianness::endiantype, bits> {};           \
                                                                               \
  /***/
