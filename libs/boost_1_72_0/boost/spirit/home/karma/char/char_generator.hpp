...
struct char_generator : primitive_generator<Derived> {
  ...
  bool generate(OutputIterator &sink, Context &context, Delimiter const &d,
    ...
    Attr ch = Attr();
    if (!this->derived().test(traits::extract_from<Attr>(attr, context), ch,
                              context))
      return false;
...
struct negated_char_generator
  ...
  negated_char_generator(Positive const &positive) : positive(positive) {}
...
///////////////////////////////////////////////////////////////////////////
namespace detail {
template <typename Positive> struct make_negated_char_generator {
...
struct make_composite<proto::tag::complement, Elements, Modifiers> {
  ...
  BOOST_SPIRIT_ASSERT_MSG((traits::is_char_generator<subject>::value),
                          subject_is_not_negatable, (subject));

  typedef typename detail::make_negated_char_generator<subject>::result_type
      result_type;
