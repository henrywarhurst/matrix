...
struct redelimit_generator : unary_generator<redelimit_generator<Subject>> {
  ...
  redelimit_generator(Subject const &subject) : subject(subject) {}
  ...
  bool generate(OutputIterator &sink, Context &ctx, Delimiter const &d,
    ...
    return subject.generate(
        sink, ctx, detail::get_delimiter(d, compile<karma::domain>(' ')), attr);
  }
...
struct make_directive<terminal_ex<tag::delimit, fusion::vector1<Delimiter>>,
                      Subject, Modifiers> {
  typedef typename result_of::compile<karma::domain, Delimiter, Modifiers>::type
      delimiter_type;
