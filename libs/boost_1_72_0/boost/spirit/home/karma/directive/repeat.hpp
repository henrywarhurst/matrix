...
struct use_lazy_directive<karma::domain, tag::repeat, 1 // arity
                          > : mpl::true_ {};

template <>                       // enables *lazy* repeat(min, max)[p]
struct use_lazy_directive<        // and repeat(min, inf)[p]
    karma::domain, tag::repeat, 2 // arity
    > : mpl::true_ {};
} // namespace spirit
} // namespace boost
...
template <typename T> struct exact_iterator {
  exact_iterator(T const exact) : exact(exact) {}
...
template <typename T> struct finite_iterator {
...
template <typename T> struct infinite_iterator {
  infinite_iterator(T const min) : min BOOST_PREVENT_MACRO_SUBSTITUTION(min) {}
...
public:
  ...
  bool generate(OutputIterator &sink, Context &ctx, Delimiter const &d,
    ...
    typedef detail::pass_container<fail_function, Attribute,
                                   indirect_iterator_type, mpl::false_>
        pass_container;

    iterator_type it = traits::begin(attr);
    iterator_type end = traits::end(attr);

    pass_container pass(fail_function(sink, ctx, d), indirect_iterator_type(it),
...
struct repeat_generator
    : base_repeat_generator<Subject, LoopIter, mpl::false_,
                            repeat_generator<Subject, LoopIter>> {
  typedef base_repeat_generator<Subject, LoopIter, mpl::false_,
                                repeat_generator>
...
struct strict_repeat_generator
    : base_repeat_generator<Subject, LoopIter, mpl::true_,
                            strict_repeat_generator<Subject, LoopIter>> {
  typedef base_repeat_generator<Subject, LoopIter, mpl::true_,
...
struct make_directive<tag::repeat, Subject, Modifiers> {
  typedef typename mpl::if_<detail::get_stricttag<Modifiers>,
                            strict_kleene<Subject>, kleene<Subject>>::type
      result_type;
...
struct make_directive<terminal_ex<tag::repeat, fusion::vector1<T>>, Subject,
  ...
  typedef typename mpl::if_<detail::get_stricttag<Modifiers>,
                            ...
                            repeat_generator<Subject, iterator_type>>::type
      result_type;
...
struct handles_container<karma::repeat_generator<Subject, LoopIter>, Attribute,
                         Context, Iterator> : mpl::true_ {};

template <typename Subject, typename LoopIter, typename Attribute,
          typename Context, typename Iterator>
struct handles_container<karma::strict_repeat_generator<Subject, LoopIter>,
