...
struct columns_delimiter {
  columns_delimiter(Delimiter const &delim, ColumnDelimiter const &cdelim,
      ...
      : delimiter(delim), column_delimiter(cdelim), numcolumns(numcols),
        count(0) {}

  template <typename OutputIterator, typename Context, typename Delimiter_,
            typename Attribute>
...
struct columns_generator
    : unary_generator<
          columns_generator<Subject, NumColumns, ColumnsDelimiter>> {
  typedef Subject subject_type;
  typedef ColumnsDelimiter delimiter_type;

  typedef mpl::int_<subject_type::properties::value |
                    delimiter_type::properties::value>
      properties;
  ...
  template <typename OutputIterator, typename Context, typename Delimiter,
            typename Attribute>
  bool generate(OutputIterator &sink, Context &ctx, Delimiter const &delimiter,
                Attribute const &attr) const {
    //  The columns generator dispatches to the embedded generator
    //  while supplying a new delimiter to use, wrapping the outer
    //  delimiter.
    typedef detail::columns_delimiter<Delimiter, ColumnsDelimiter>
...
struct make_directive<tag::columns, Subject, Modifiers> {
  typedef typename result_of::compile<karma::domain, eol_type, Modifiers>::type
      columns_delimiter_type;
  typedef columns_generator<Subject, detail::default_columns,
                            columns_delimiter_type>
      result_type;
...
struct make_directive<
    terminal_ex<tag::columns, fusion::vector1<T>>, Subject, Modifiers,
...
struct make_directive<
  ...
  typedef typename result_of::compile<karma::domain, T, Modifiers>::type
      columns_delimiter_type;
  typedef columns_generator<Subject, detail::default_columns,
                            columns_delimiter_type>
      result_type;
...
struct make_directive<terminal_ex<tag::columns, fusion::vector2<T1, T2>>,
                      Subject, Modifiers> {
  typedef typename result_of::compile<karma::domain, T2, Modifiers>::type
      columns_delimiter_type;
...
struct handles_container<karma::columns_generator<Subject, T1, T2>, Attribute,
                         Context, Iterator>
