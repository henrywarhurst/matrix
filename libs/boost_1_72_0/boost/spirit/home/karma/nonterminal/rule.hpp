...
template <typename OutputIterator, typename T1, typename T2, typename T3,
          typename T4>
struct rule
    : proto::extends<typename proto::terminal<reference<
                         rule<OutputIterator, T1, T2, T3, T4> const>>::type,
  ...
  typedef typename spirit::detail::extract_component<
      karma::domain, template_params>::type delimiter_type;
  ...
  typedef function<bool(output_iterator &, context_type &,
                        delimiter_type const &)>
      function_type;

  typedef typename mpl::if_<is_same<encoding_type, unused_type>, unused_type,
  ...
  rule(rule const &rhs)
      : base_type(terminal::make(reference_(*this))), name_(rhs.name_),
        f(rhs.f) {}
  ...
  template <typename Expr> rule &operator=(Expr const &expr) {
    define<mpl::false_>(*this, expr, traits::matches<karma::domain, Expr>());
    return *this;
  }
...
// VC7.1 has problems to resolve 'rule' without explicit template parameters
  ...
  template <typename Expr> friend rule &operator%=(rule &r, Expr &expr) {
  ...
  bool generate(output_iterator &sink, Context &, Delimiter const &delim,
    ...
    if (f) {
      ...
      typedef traits::transform_attribute<Attribute const, attr_type, domain>
          transform;
...
operator%=(rule<OutputIterator_, T1_, T2_, T3_, T4_> &r, Expr const &expr) {
  ...
  typedef
      typename rule<OutputIterator_, T1_, T2_, T3_, T4_>::encoding_modifier_type
          encoding_modifier_type;
...
struct nonterminal_handles_container
    : mpl::and_<traits::is_container<RuleAttribute>,
...
struct handles_container<karma::rule<IteratorA, T1, T2, T3, T4>, Attribute,
                         Context, IteratorB>
    : detail::nonterminal_handles_container<
          typename attribute_of<karma::rule<IteratorA, T1, T2, T3, T4>, Context,
                                IteratorB>::type,
          Attribute> {};
} // namespace traits
} // namespace spirit
