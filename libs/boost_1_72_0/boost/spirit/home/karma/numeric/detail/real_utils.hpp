...
#pragma warning(disable : 4267) // conversion from 'size_t' to 'unsigned int',
  ...
  static bool call_n(OutputIterator &sink, U n, Policies const &p) {
    ...
    if (std::numeric_limits<U>::digits10) {
      ...
      precision =
          (std::min)(precision, (unsigned)std::numeric_limits<U>::digits10 + 1);
    }
      ...
      if (dim > 0)
        ...
        if (exp > std::numeric_limits<U>::max_exponent10) {
          ...
          n *= spirit::traits::pow10<U>(exp -
                                        std::numeric_limits<U>::max_exponent10);
        } else
          n *= spirit::traits::pow10<U>(exp);
      }
    }
    ...
    if (!p.trailing_zeros(n)) {
      ...
      if (precision != prec) {
        long_frac_part =
            frac_part_floor / spirit::traits::pow10<U>(precision - prec);
      }
    }
    ...
    if (r && 0 == (Policies::fmtflags::fixed & flags)) {
      return p.template exponent<CharEncoding, Tag>(
          sink, traits::truncate_to_long::call(dim));
    }
    return r;
  }
