...
struct symbols_lookup {
  typedef mpl::eval_if<fusion::traits::is_sequence<T>,
                       traits::detail::value_at_c<T, 0>,
                       detail::add_const_ref<T>>
      sequence_type;
  typedef
      typename mpl::eval_if<traits::is_container<T>, traits::container_value<T>,
...
template <typename Attribute> struct symbols_lookup<Attribute, Attribute> {
...
template <typename Attribute> struct symbols_value<Attribute, Attribute> {
...
struct symbols_lookup : mpl::if_<traits::not_is_unused<T>,
                                 std::map<Attribute, T>, std::set<Attribute>> {
};

///////////////////////////////////////////////////////////////////////////
namespace detail {
///////////////////////////////////////////////////////////////////////
template <typename CharEncoding, typename Tag> struct generate_encoded {
  typedef typename proto::terminal<tag::char_code<Tag, CharEncoding>>::type
      encoding_type;
...
template <> struct generate_encoded<unused_type, unused_type> {
  template <typename OutputIterator, typename Expr, typename Attribute>
  static bool call(OutputIterator &sink, Expr const &expr,
                   Attribute const &attr) {
...
template <typename Attribute = char, typename T = unused_type,
          typename Lookup = typename symbols_lookup<T, Attribute>::type,
          typename CharEncoding = unused_type, typename Tag = unused_type>
struct symbols
    : proto::extends<typename proto::terminal<reference<symbols<
                         Attribute, T, Lookup, CharEncoding, Tag>>>::type,
  ...
  template <typename Context, typename Unused> struct attribute {
    typedef Attribute type;
  };

  symbols(std::string const &name = "symbols")
      : base_type(terminal::make(reference_(*this))), add(*this), remove(*this),
        lookup(new Lookup()), name_(name) {}

  symbols(symbols const &syms)
      : base_type(terminal::make(reference_(*this))), add(*this), remove(*this),
        lookup(syms.lookup), name_(syms.name_) {}
  ...
  template <typename Attr> value_type *find(Attr const &attr) {
    typename Lookup::iterator it = lookup->find(attr);
    return (it != lookup->end()) ? &(*it).second : 0;
  }

  template <typename Attr> value_type &at(Attr const &attr) {
    return (*lookup)[attr];
  }
  ...
  bool generate(OutputIterator &sink, Context &, Delimiter const &d,
    ...
    typename Lookup::iterator it =
        lookup->find(traits::symbols_lookup<Attr, Attribute>::call(attr));
    if (it == lookup->end())
      return false;
  ...
  template <typename Context> info what(Context &) const { return info(name_); }
  ...
  ///////////////////////////////////////////////////////////////////////
  struct adder {
    template <typename, typename = unused_type> struct result {
      typedef adder const &type;
    };

    adder(symbols &sym) : sym(sym) {}
    ...
    template <typename Attr> adder const &operator,(Attr const &attr) const {
  ...
  struct remover {
    template <typename> struct result { typedef remover const &type; };

    remover(symbols &sym) : sym(sym) {}

    template <typename Attr> remover const &operator()(Attr const &attr) const {
...
struct symbols<Attribute, unused_type, Lookup, CharEncoding, Tag>
    : proto::extends<
          typename proto::terminal<spirit::karma::reference<symbols<
              Attribute, unused_type, Lookup, CharEncoding, Tag>>>::type,
  ...
  template <typename CharEncoding_, typename Tag_>
  symbols(
      symbols<Attribute, unused_type, Lookup, CharEncoding_, Tag_> const &syms)
      : base_type(terminal::make(reference_(*this))), add(*this), remove(*this),
        lookup(syms.lookup), name_(syms.name_) {}
  ...
  template <typename CharEncoding_, typename Tag_>
  symbols &operator=(
      symbols<Attribute, unused_type, Lookup, CharEncoding_, Tag_> const &rhs) {
    *lookup = *rhs.lookup;
    name_ = rhs.name_;
    return *this;
  }
  ...
  template <typename Attr> adder const &operator=(Attr const &attr) {
    lookup->clear();
    return add(attr);
  }
  ...
  template <typename Attr> value_type const *find(Attr const &attr) {
    typename Lookup::iterator it = lookup->find(attr);
  ...
  bool generate(OutputIterator &sink, Context &, Delimiter const &d,
    ...
    return karma::detail::generate_encoded<CharEncoding, Tag>::call(
               sink, traits::symbols_lookup<Attr, Attribute>::call(attr),
               unused) &&
           karma::delimit_out(sink, d);
  }
...
struct make_primitive<
    reference<symbols<Attribute, T, Lookup, CharEnconding, Tag>>, Modifiers> {
  static bool const lower =
      has_modifier<Modifiers, tag::char_code_base<tag::lower>>::value;
  static bool const upper =
      has_modifier<Modifiers, tag::char_code_base<tag::upper>>::value;

  typedef reference<symbols<Attribute, T, Lookup, CharEnconding, Tag>>
      reference_;
...
///////////////////////////////////////////////////////////////////////////
template <typename Attribute, typename T, typename Lookup,
          typename CharEncoding, typename Tag, typename Attr, typename Context,
          typename Iterator>
struct handles_container<
    karma::symbols<Attribute, T, Lookup, CharEncoding, Tag>, Attr, Context,
    Iterator> : traits::is_container<Attr> {};
} // namespace traits
} // namespace spirit
} // namespace boost
