...
namespace spirit {
namespace lex {
template <typename Derived> struct lexer_type {
  struct lexer_id;
...
template <typename Derived> struct primitive_lexer : lexer_type<Derived> {
  struct primitive_lexer_id;
};

template <typename Derived> struct unary_lexer : lexer_type<Derived> {
...
template <typename Derived> struct nary_lexer : lexer_type<Derived> {
...
template <typename T> struct is_lexer : detail::has_lexer_id<T> {};

template <typename T>
struct is_primitive_lexer : detail::has_primitive_lexer_id<T> {};

template <typename T> struct is_unary_lexer : detail::has_unary_lexer_id<T> {};

template <typename T> struct is_nary_lexer : detail::has_nary_lexer_id<T> {};
