...
struct use_terminal<lex::domain, T,
                    typename enable_if<traits::is_lexer<T>>::type> // enables
                                                                   // lexers
    : mpl::true_ {};
...
template <> struct make_component<lex::domain, proto::tag::terminal> {
...
template <typename Tag> struct make_component<lex::domain, Tag> {
  template <typename Sig> struct result;

  template <typename This, typename Elements, typename Modifiers>
  struct result<This(Elements, Modifiers)> {
    typedef typename lex::make_composite<
