...
struct lexer_def_
    : proto::extends<typename proto::terminal<
                         lex::reference<lexer_def_<LexerDef> const>>::type,
                     lexer_def_<LexerDef>>,
...
private:
  ...
  // syntax
  struct adder {
    adder(lexer_def_ &def_) : def(def_) {}
    ...
    adder const &operator()(token_def<Attribute, char_type, id_type> &tokdef,
                            id_type token_id = id_type()) const {
    ...
    //                 template <typename F>
    //                 adder const& operator()(char_type c, id_type token_id, F
    //                 act) const
    //                 {
    //                     if (id_type() == token_id)
    //                         token_id = def.def.get_next_id();
    //                     std::size_t unique_id =
    //                         def.def.add_token (def.state.c_str(), s,
    //                         token_id);
    //                     def.def.add_action(unique_id, def.state.c_str(),
    //                     act); return *this;
    //                 }
  ...
  struct pattern_adder {
    pattern_adder(lexer_def_ &def_) : def(def_) {}
...
private:
  ...
  template <typename TokenExpr> void compile2pass(TokenExpr const &expr) {
...
public:
  ///////////////////////////////////////////////////////////////////
  template <typename Expr> void define(Expr const &expr) {
  ...
  lexer_def_(LexerDef &def_, string_type const &state_,
      ...
      : proto_base_type(terminal_type::make(alias())), add(this_()),
        add_pattern(this_()), def(def_), state(state_),
  ...
  template <typename Expr> lexer_def_ &operator=(Expr const &xpr) {
...
inline lexer_def_<LexerDef> &operator+=(lexer_def_<LexerDef> &lexdef,
                                        Expr &xpr) {
  // Report invalid expression error as early as possible.
  // If you got an error_invalid_expression error message here,
  // then the expression (expr) is not a valid spirit lex
...
template <typename Lexer> class lexer : public Lexer {
private:
  // avoid warnings about using 'this' in constructor
...
public:
  ...
  lexer(unsigned int flags = match_flags::match_default)
      : lexer_type(flags), next_token_id(min_token_id),
        self(this_(), lexer_type::initial_state()) {}

  lexer(unsigned int flags, id_type first_id)
      : lexer_type(flags), next_token_id(first_id),
        self(this_(), lexer_type::initial_state()) {}
