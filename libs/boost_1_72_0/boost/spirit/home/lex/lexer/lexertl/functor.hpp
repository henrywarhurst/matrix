...
private:
  ...
  template <typename T> struct assign_on_exit {
    assign_on_exit(T &dst, T const &src) : dst_(dst), src_(src) {}
...
public:
  ...
  typedef FunctorData<Iterator, SupportsActors, SupportsState, token_value_type>
      shared;
  ...
  static result_type &get_next(MultiPass &mp, result_type &result) {
    ...
    for (;;) {
      ...
      if (boost::lexer::npos == id) { // no match
        ...
        std::cerr << "Not matched, in state: " << state << ", lookahead: >"
                  << next << "<" << std::endl;
#endif
...
#if defined(BOOST_SPIRIT_LEXERTL_DEBUG)
      {
        ...
        std::cerr << "Matched: " << id << ", in state: " << state
                  << ", string: >"
      ...
      pass = data.invoke_actions(state, id, unique_id, end);
...
#if defined(BOOST_SPIRIT_LEXERTL_DEBUG)
    std::cerr << "Switching state from: " << oldstate << " to: " << state
              << std::endl;
#endif
    return oldstate;
  }

  template <typename MultiPass> static std::size_t get_state(MultiPass &mp) {
    return mp.shared()->ftor.get_state();
  }

  template <typename MultiPass>
  static std::size_t map_state(MultiPass const &mp,
                               char_type const *statename) {
    return mp.shared()->ftor.get_state_id(statename);
  }

  // we don't need this, but it must be there
  template <typename MultiPass> static void destroy(MultiPass const &) {}
};
...
typename functor<Token, FunctorData, Iterator, SupportsActors,
                 SupportsState>::result_type const
