...
///////////////////////////////////////////////////////////////////////////
template <typename CharT> struct string_lit;

template <> struct string_lit<char> {
  static char get(char c) { return c; }
  static std::string get(char const *str = "") { return str; }
};

template <> struct string_lit<wchar_t> {
...
template <typename Char> inline Char L(char c) {
  return string_lit<Char>::get(c);
}

template <typename Char> inline std::basic_string<Char> L(char const *c = "") {
  return string_lit<Char>::get(c);
}
...
generate_cpp_state_info(boost::lexer::basic_rules<Char> const &rules_,
  ...
  typedef typename boost::lexer::basic_rules<
      Char>::string_size_t_map::const_iterator state_iterator;
  ...
  os_ << boost::lexer::detail::strings<Char>::char_name()
      << " const* const lexer_state_names" << (name_suffix[0] ? "_" : "")
      << name_suffix << "[" << rules_.statemap().size() << "] = \n{\n";
  ...
  for (iterator rit = reverse_state_map.begin(); rit != rend; ++last_id) {
    ...
    os_ << "    " << boost::lexer::detail::strings<Char>::char_prefix() << "\""
        << (*rit).second << "\"";
    if (++rit != rend)
      os_ << ",\n";
    else
  ...
  os_ << "std::size_t const lexer_state_count" << (name_suffix[0] ? "_" : "")
      << name_suffix << " = " << rules_.statemap().size() << ";\n\n";
  return os_.good();
}

template <typename Char>
inline bool generate_cpp_state_table(std::basic_ostream<Char> &os_,
                                     Char const *name_suffix, bool bol,
                                     bool eol) {
  ...
  os_ << "    // version number and feature-set of compatible static lexer "
         "engine\n";
  os_ << "    enum\n";
  ...
  os_ << "    {\n        return lexer_state_names" << suffix
      << "[idx]; \n    }\n\n";
  ...
  os_ << "      , Iterator &start_token_, Iterator const& end_, std::size_t& "
         "unique_id_)\n";
...
///////////////////////////////////////////////////////////////////////////
template <typename Char> inline std::basic_string<Char> get_charlit(Char ch) {
...
bool need_label0_0(boost::lexer::basic_state_machine<Char> const &sm_) {
  typedef
      typename boost::lexer::basic_state_machine<Char>::iterator iterator_type;
...
inline bool generate_cpp(boost::lexer::basic_state_machine<Char> const &sm_,
...
inline bool generate_static(Lexer const &lexer,
