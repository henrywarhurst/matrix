...
template <typename FunctorData> struct make_multi_pass {
  ...
  typedef iterator_policies::default_policy<ownership_policy, check_policy,
...
public:
  ...
  iterator(const base_type &base) : base_type(base) {}
  ...
  std::size_t map_state(char_type const *statename) {
    return (0 != statename) ? unique_functor_type::map_state(*this, statename)
                            : 0;
  }
};
} // namespace lexertl
...
template <typename Functor>
void inhibit_clear_queue(spirit::lex::lexertl::iterator<Functor> &mp,
                         bool flag) {
