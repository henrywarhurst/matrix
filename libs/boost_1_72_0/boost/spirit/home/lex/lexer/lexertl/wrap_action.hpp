...
struct wrap_action {
  ...
  template <typename F> static FunctionType call(F const &f) { return f; }
  ...
  static FunctionType call(void (*f)(A0, A1, A2, A3)) {
    void (*pf)(void (*)(A0, A1, A2, A3), Iterator &, Iterator &,
               BOOST_SCOPED_ENUM(pass_flags) &, IdType &, Context const &) =
        &wrap_action::arg4_action;
...
struct wrap_action<unused_type, Iterator, Context, Idtype> {
  ...
  template <typename F> static F const &call(F const &f) { return f; }
};
} // namespace detail
