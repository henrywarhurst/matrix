...
struct token_def
    : proto::extends<
          typename proto::terminal<lex::reference<
              token_def<Attribute, Char, Idtype> const, Idtype>>::type,
...
public:
  ...
  template <typename Context, typename Iterator> struct attribute {
    ...
    typedef typename mpl::if_<traits::not_is_unused<Attribute>,
                              typename mpl::if_<is_same<Attribute, lex::omit>,
                                                unused_type, Attribute>::type,
...
public:
  ...
  bool parse(Iterator &first, Iterator const &last, Context & /*context*/,
             Skipper const &skipper, Attribute_ &attr) const {
  ...
  void collect(LexerDef &lexdef, String const &state,
    ...
    if (0 == def_.which()) {
      unique_id_ = lexdef.add_token(
          state.c_str(), boost::get<string_type>(def_), token_id_, target);
    } else {
      unique_id_ = lexdef.add_token(state.c_str(), boost::get<char_type>(def_),
                                    token_id_, target);
    }
  }

  template <typename LexerDef> void add_actions(LexerDef &) const {}
...
public:
  ...
  token_def()
      : proto_base_type(terminal_type::make(reference_(*this))), def_('\0'),
        token_id_(), unique_id_(std::size_t(~0)),
        token_state_(std::size_t(~0)) {}

  token_def(token_def const &rhs)
      : proto_base_type(terminal_type::make(reference_(*this))), def_(rhs.def_),
        token_id_(rhs.token_id_), unique_id_(rhs.unique_id_),
        token_state_(rhs.token_state_) {}

  explicit token_def(char_type def_, Idtype id_ = Idtype())
      : proto_base_type(terminal_type::make(reference_(*this))), def_(def_),
  ...
  explicit token_def(string_type const &def_, Idtype id_ = Idtype())
      : proto_base_type(terminal_type::make(reference_(*this))), def_(def_),
        token_id_(id_), unique_id_(std::size_t(~0)),
        token_state_(std::size_t(~0)) {}

  template <typename String> token_def &operator=(String const &definition) {
    def_ = definition;
    token_id_ = Idtype();
...
namespace traits {
///////////////////////////////////////////////////////////////////////////
template <typename Attribute, typename Char, typename Idtype, typename Attr,
          typename Context, typename Iterator>
struct handles_container<lex::token_def<Attribute, Char, Idtype>, Attr, Context,
                         Iterator>
    : traits::is_container<typename attribute_of<
          lex::token_def<Attribute, Char, Idtype>, Context, Iterator>::type> {};
} // namespace traits
} // namespace spirit
} // namespace boost
