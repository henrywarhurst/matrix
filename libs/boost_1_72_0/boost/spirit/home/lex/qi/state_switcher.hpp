...
struct state_switcher : primitive_parser<state_switcher<State>> {
  typedef
      typename remove_const<typename traits::char_type_of<State>::type>::type
          char_type;
  ...
  state_switcher(char_type const *state) : state(state) {}
...
///////////////////////////////////////////////////////////////////////////
namespace detail {
template <typename Iterator> struct reset_state_on_exit {
  template <typename State>
  reset_state_on_exit(Iterator &it_, State state_)
      : it(it_), state(set_lexer_state(it_, traits::get_c_string(state_))) {}
...
struct handles_container<qi::state_switcher_context<Subject, State>, Attribute,
                         Context, Iterator>
