...
// enables token
template <> struct use_terminal<qi::domain, tag::token> : mpl::true_ {};
...
struct use_terminal<qi::domain,
    ...
    : mpl::and_<mpl::or_<is_integral<A0>, is_enum<A0>>,
                mpl::or_<is_integral<A1>, is_enum<A1>>> {};

// enables *lazy* token(id)
template <> struct use_lazy_terminal<qi::domain, tag::token, 1> : mpl::true_ {};

// enables *lazy* token(idmin, idmax)
template <> struct use_lazy_terminal<qi::domain, tag::token, 2> : mpl::true_ {};
} // namespace spirit
} // namespace boost
...
struct plain_token : primitive_parser<plain_token<TokenId>> {
  ...
  plain_token(TokenId const &id) : id(id) {}
  ...
  template <typename Context> info what(Context & /*context*/) const {
    return info("token", "token(" + boost::lexical_cast<utf8_string>(id) + ")");
  }
...
struct plain_token_range : primitive_parser<plain_token_range<TokenId>> {
  ...
  template <typename Context> info what(Context & /*context*/) const {
    return info("token_range",
                "token(" + boost::lexical_cast<utf8_string>(idmin) + ", " +
...
///////////////////////////////////////////////////////////////////////////
template <typename Modifiers> struct make_primitive<tag::token, Modifiers> {
...
struct make_primitive<
    terminal_ex<tag::token, fusion::vector2<TokenId, TokenId>>, Modifiers> {
