...
inline bool tokenize_and_parse(Iterator &first, Iterator last, Lexer const &lex,
  ...
  return compile<qi::domain>(xpr).parse(iter, lex.end(), unused, unused,
                                        unused);
}
...
template <typename Iterator, typename Lexer, typename ParserExpr,
          typename Skipper>
inline bool tokenize_and_phrase_parse(Iterator &first, Iterator last,
                                      Lexer const &lex, ParserExpr const &xpr,
  ...
  typedef typename spirit::result_of::compile<qi::domain, Skipper>::type
      skipper_type;
...
inline bool tokenize(Iterator &first, Iterator last, Lexer const &lex, F f,
...
inline bool tokenize(Iterator &first, Iterator last, Lexer const &lex,
                     typename Lexer::char_type const *initial_state = 0) {
  typedef typename Lexer::iterator_type iterator_type;
