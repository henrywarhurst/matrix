...
// a char range
struct ucd_range {
  ucd_range(uint32_t start, uint32_t finish) : start(start), finish(finish) {}
...
public:
  ucd_info(char const *filename) {
    ...
    if (!in) {
      ...
      rule<iterator_type, std::string()> field =
          *(char_ - (';' | endl)) >> (';' | &endl);
      rule<iterator_type, ucd_line()> line = +(field - endl) >> endl;
      rule<iterator_type, std::vector<ucd_line>()> file =
          +(endl | line[push_back(_val, _1)]);
...
private:
  ...
  static int map_property(std::string const &p) {
    std::map<std::string, int> &pm = get_property_map();
    std::map<std::string, int>::iterator i = pm.find(p);
  ...
  static std::map<std::string, int> &get_property_map() {
...
template <typename T, uint32_t block_size_ = 256> class ucd_table_builder {
public:
  ...
  void build(std::vector<uint8_t> &stage1, std::vector<T const *> &stage2) {
    ...
    typedef std::pair<T const *, std::vector<T const *>>
        sorted_blocks_value_type;
...
private:
  struct block_ptr {
    ...
    friend bool operator<(block_ptr a, block_ptr b) {
      return std::lexicographical_compare(a.p, a.p + block_size, b.p,
                                          b.p + block_size);
    }
...
template <typename Out> void print_tab(Out &out, int tab) {
  for (int i = 0; i < tab; ++i)
    out << ' ';
}

template <typename Out, typename C>
void print_table(Out &out, C const &c, bool trailing_comma, int width = 4,
                 int group = 16) {
  int const tab = 4;
...
template <typename Out> void print_head(Out &out) {
  out << "/*==================================================================="
         "==========\n"
      << "    Copyright (c) 2001-2011 Joel de Guzman\n"
      << "\n"
...
template <typename Out> void print_tail(Out &out) {
  out << "\n"
...
void print_file(Out &out, Builder &builder, int field_width, char const *name) {
  ...
  for (int i = 0; i < int(stage2.size()); ++i) {
    ...
    print_table(out,
                boost::iterator_range<value_type const *>(p, p + block_size),
                !last, field_width);
  }
  ...
  out << "\n"
      << "    inline " << int_name << ' ' << name
      << "_lookup(::boost::uint32_t ch)\n"
      << "    {\n"
      << "        ::boost::uint32_t block_offset = " << name << "_stage1[ch / "
      << block_size << "] * " << block_size << ";\n"
      << "        return " << name << "_stage2[block_offset + ch % "
      << block_size << "];\n"
      << "    }\n";
