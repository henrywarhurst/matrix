...
namespace spirit {
namespace detail {
template <int size> struct as_variant_impl;

#if !defined(BOOST_VARIANT_DO_NOT_USE_VARIADIC_TEMPLATES)
#else
template <> struct as_variant_impl<0> {
  template <typename Iterator> struct apply { typedef variant<> type; };
};
#endif
...
template <typename Sequence> struct as_variant {
  // build a variant generator being able to generate a variant holding
  // all of the types as given in the typelist
  typedef
      typename detail::as_variant_impl<fusion::result_of::size<Sequence>::value>
          gen;
...
template <> struct as_variant_impl<N> {
