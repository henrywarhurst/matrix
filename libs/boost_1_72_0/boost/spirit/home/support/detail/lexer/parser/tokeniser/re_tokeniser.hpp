...
namespace lexer {
namespace detail {
template <typename CharT> class basic_re_tokeniser {
public:
    ...
    if (eos_) {
      ...
      if (ch_ == '\\') {
        ...
        case '/':
          ...
          break;
        default:
          if ((state_._flags & icase) && (std::isupper(ch_, state_._locale) ||
...
private:
    ...
    if (!state_.eos() && *state_._curr == '?') {
      ...
      while (!eos_ && ch_ != ':') {
        ...
        case 's':
          if (negate_) {
            state_._flags =
                static_cast<regex_flags>(state_._flags | dot_not_newline);
          } else {
            state_._flags =
                static_cast<regex_flags>(state_._flags & ~dot_not_newline);
          }
  ...
  static void escape(state &state_, token_map &map_, num_token &token_) {
    ...
    const CharT *str_ =
        tokeniser_helper::escape_sequence(state_, ch_, str_len_);
  ...
  static void create_charset_token(const string &charset_, const bool negated_,
                                   token_map &map_, num_token &token_) {
