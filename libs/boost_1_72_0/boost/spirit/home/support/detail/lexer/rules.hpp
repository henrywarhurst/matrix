...
template <typename CharT> struct strings;
...
template <typename CharT> class basic_rules {
public:
  ...
  basic_rules(const regex_flags flags_ = dot_not_newline,
      ...
      : _flags(flags_), _counter(0), _counter_ptr(counter_ptr_) {
    add_state(initial());
  }
  ...
  void add(const CharT *source_, const basic_rules<CharT> &rules_,
    ...
    if (star_) {
      ...
      for (; all_regexes_iter_ != all_regexes_end_;
           ++state_, ++all_regexes_iter_, ++all_ids_iter_, ++all_uids_iter_,
           ++all_states_iter_) {
        ...
        for (; regex_iter_ != regex_end_;
             ++regex_iter_, ++id_iter_, ++uid_iter_, ++state_iter_) {
          // If ..._dot_ then lookup state name from rules_; otherwise
          // pass name through.
          add(dest_dot_ ? rules_.state(state_) : dest_, *regex_iter_, *id_iter_,
              to_dot_ ? rules_.state(*state_iter_) : to_, true, *uid_iter_);
        }
      }
    } else {
      ...
      while (*source_) {
        ...
        for (; regex_iter_ != regex_end_;
          ...
          // If ..._dot_ then lookup state name from rules_; otherwise
          // pass name through.
          add(dest_dot_ ? state_name_.c_str() : dest_, *regex_iter_, *id_iter_,
              to_dot_ ? rules_.state(*state_iter_) : to_, true, *uid_iter_);
        }
      }
    }
...
private:
  ...
  std::size_t add(const CharT *curr_state_, const string &regex_,
                  const std::size_t id_, const CharT *new_state_,
                  const bool check_, const std::size_t uid_ = npos) {
