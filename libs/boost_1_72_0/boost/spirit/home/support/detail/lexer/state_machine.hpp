...
namespace boost {
namespace lexer {
template <typename CharT> class basic_state_machine {
public:
  ...
  public:
    ...
    struct data {
      ...
      data()
          : dfa(npos), states(0), state(npos), transitions(0), transition(npos),
            end_state(false), id(npos), unique_id(npos), goto_dfa(npos),
            bol_index(npos), eol_index(npos), goto_state(npos) {}

      bool operator==(const data &rhs_) const {
        return dfa == rhs_.dfa && states == rhs_.states &&
               state == rhs_.state && transitions == rhs_.transitions &&
               transition == rhs_.transition && end_state == rhs_.end_state &&
               id == rhs_.id && unique_id == rhs_.unique_id &&
               goto_dfa == rhs_.goto_dfa && bol_index == rhs_.bol_index &&
               eol_index == rhs_.eol_index && token == rhs_.token &&
               goto_state == rhs_.goto_state;
      }
    };

    iterator()
        : _sm(0), _dfas(0), _dfa(npos), _states(0), _state(npos),
          _transitions(0), _transition(npos) {}
  ...
  private:
    ...
    typename detail::basic_char_state_machine<
        CharT>::state::size_t_string_token_map::const_iterator _token_iter;
    typename detail::basic_char_state_machine<
        CharT>::state::size_t_string_token_map::const_iterator _token_end;
  ...
  bool empty() const {
    ...
    return _internals._lookup->empty() && _internals._dfa_alphabet.empty() &&
  ...
  iterator begin() const {
    ...
    if (!_csm.empty()) {
      const typename detail::basic_char_state_machine<CharT>::state_vector
          *ptr_ = &_csm._sm_vector.front();
...
private:
  ...
  void human_readable(detail::basic_char_state_machine<CharT> &sm_) const {
    ...
    for (std::size_t start_state_index_ = 0; start_state_index_ < start_states_;
         ++start_state_index_) {
      ...
      const std::size_t states_ = _internals._dfa[start_state_index_]->size() /
                                  (alphabet_ + dfa_offset);
      const std::size_t *read_ptr_ =
          &_internals._dfa[start_state_index_]->front() + alphabet_ +
          dfa_offset;
      ...
      for (std::size_t state_index_ = 1; state_index_ < states_;
        ...
        for (std::size_t col_index_ = 0; col_index_ < alphabet_;
          ...
          if (transition_ != 0) {
            ...
            if (iter_ == state_->_transitions.end()) {
              ...
              typename detail::basic_char_state_machine<
                  CharT>::state::size_t_string_token_pair pair_(i_, token_);
        ...
        for (typename detail::basic_char_state_machine<
                 CharT>::state::size_t_string_token_map::iterator
