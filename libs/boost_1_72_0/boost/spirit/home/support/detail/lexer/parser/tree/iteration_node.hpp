...
public:
  iteration_node(node *next_, const bool greedy_)
      : node(true), _next(next_), _greedy(greedy_) {
    ...
    for (iter_ = _lastpos.begin(), end_ = _lastpos.end(); iter_ != end_;
         ++iter_) {
      (*iter_)->append_followpos(_firstpos);
    }

    for (iter_ = _firstpos.begin(), end_ = _firstpos.end(); iter_ != end_;
         ++iter_) {
...
private:
  ...
  virtual void copy_node(node_ptr_vector &node_ptr_vector_,
                         ...
                         bool_stack &perform_op_stack_, bool &down_) const {
