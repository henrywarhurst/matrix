...
public:
  ...
  static void build(const rules &rules_,
    ...
    detail::internals &internals_ =
        const_cast<detail::internals &>(state_machine_.data());
    ...
    for (index_ = 0, size_ = internals_._lookup->size(); index_ < size_;
         ++index_) {
      internals_._lookup[index_]->resize(
          sizeof(CharT) == 1 ? num_chars : num_wchar_ts, dead_state_index);

      if (!rules_.regexes()[index_].empty()) {
        // vector mapping token indexes to partitioned token index sets
        index_set_vector set_mapping_;
        // syntax tree
        detail::node *root_ = build_tree(rules_, index_, node_ptr_vector_,
                                         internals_, set_mapping_);

        build_dfa(root_, set_mapping_, internals_._dfa_alphabet[index_],
...
protected:
  ...
  static detail::node *build_tree(const rules &rules_, const std::size_t state_,
                                  node_ptr_vector &node_ptr_vector_,
                                  detail::internals &internals_,
                                  index_set_vector &set_mapping_) {
    ...
    build_macros(token_map_, macrodeque_, macromap_, rules_.flags(),
                 rules_.locale(), node_ptr_vector_,
                 internals_._seen_BOL_assertion,
                 internals_._seen_EOL_assertion);

    detail::node *root_ = parser::parse(
        regex_.c_str(), regex_.c_str() + regex_.size(), *ids_iter_,
        *unique_ids_iter_, *states_iter_, rules_.flags(), rules_.locale(),
        node_ptr_vector_, macromap_, token_map_, internals_._seen_BOL_assertion,
        internals_._seen_EOL_assertion);
    ...
    while (regex_iter_ != regex_iter_end_) {
      ...
      root_ = parser::parse(
          regex2_.c_str(), regex2_.c_str() + regex2_.size(), *ids_iter_,
          *unique_ids_iter_, *states_iter_, rules_.flags(), rules_.locale(),
  ...
  static void
  build_macros(token_map &token_map_, const macro_deque &macrodeque_,
    ...
    for (typename macro_deque::const_iterator iter_ = macrodeque_.begin(),
                                              end_ = macrodeque_.end();
         iter_ != end_; ++iter_) {
      const typename rules::string &name_ = iter_->first;
      const typename rules::string &regex_ = iter_->second;
      detail::node *node_ =
          parser::parse(regex_.c_str(), regex_.c_str() + regex_.size(), 0, 0, 0,
                        flags_, locale_, node_ptr_vector_, macromap_,
                        token_map_, seen_BOL_assertion_, seen_EOL_assertion_);
      macro_iter_pair map_iter_ = macromap_.insert(
          macro_pair(name_, static_cast<const detail::node *>(0)));
  ...
  static void build_dfa(detail::node *root_,
                        ...
                        const std::size_t dfa_alphabet_, size_t_vector &dfa_) {
    ...
    closure(followpos_, seen_sets_, seen_vectors_, hash_vector_, dfa_alphabet_,
            dfa_);
    ...
    for (std::size_t index_ = 0; index_ < seen_vectors_->size(); ++index_) {
      ...
      for (typename equivset_list::list::const_iterator
               iter_ = equiv_list_->begin(),
               end_ = equiv_list_->end();
           iter_ != end_; ++iter_) {
        equivset *equivset_ = *iter_;
        const std::size_t transition_ =
            closure(&equivset_->_followpos, seen_sets_, seen_vectors_,
  ...
  static std::size_t closure(typename detail::node::node_vector *followpos_,
                             ...
                             node_vector_vector &seen_vectors_,
                             size_t_vector &hash_vector_,
                             const std::size_t size_, size_t_vector &dfa_) {
    ...
    for (typename detail::node::node_vector::const_iterator
             iter_ = followpos_->begin(),
             end_ = followpos_->end();
         iter_ != end_; ++iter_) {
      closure_ex(*iter_, end_state_, id_, unique_id_, state_, set_ptr_.get(),
  ...
  static void closure_ex(detail::node *node_, bool &end_state_,
                         std::size_t &id_, std::size_t &unique_id_,
                         std::size_t &state_, node_set *set_ptr_,
  ...
  static void fill_lookup(const string_token &token_, size_t_vector *lookup_,
      ...
      if (!node_->end_state()) {
        ...
        if (token_ != null_token) {
          ...
          if (token_ == bol_token || token_ == eol_token) {
            ...
            list_->back() = new equivset(index_set_, node_->greedy(), token_,
                                         node_->followpos());
          } else {
            list_->back() = new equivset(set_mapping_[token_], node_->greedy(),
  ...
  static void minimise_dfa(const std::size_t dfa_alphabet_, size_t_vector &dfa_,
                           std::size_t size_) {
