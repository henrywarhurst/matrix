...
namespace lexer {
namespace detail {
template <typename CharT> struct basic_num_token {
  enum type {
    BEGIN,
    REGEX,
    OREXP,
  ...
  basic_num_token(const type type_ = BEGIN, const std::size_t id_ = null_token)
      : _type(type_), _id(id_), _min(0), _comma(false), _max(0) {
    *_macro = 0;
  }
...
const char *basic_num_token<CharT>::_precedence_strings[END + 1] =
    ...
    {{"BEGIN"},      {"REGEX"},  {"OREXP"},     {"SEQUENCE"}, {"SUB"},
     {"EXPRESSION"}, {"REPEAT"}, {"DUPLICATE"}, {"|"},        {"CHARSET"},
     {"MACRO"},      {"("},      {")"},         {"?"},        {"??"},
     {"*"},          {"*?"},     {"+"},         {"+?"},       {"{n[,[m]]}"},
     {"{n[,[m]]}?"}, {"END"}};
#else
