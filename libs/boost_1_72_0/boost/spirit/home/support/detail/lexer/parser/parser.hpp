...
namespace lexer {
namespace detail {
template <typename CharT> class basic_parser {
public:
  ...
  /*
      ...
      - When a complete sequence of regex tokens is on the stack it is
  processed.
  ...
  static node *parse(const CharT *start_, const CharT *const end_,
    ...
    do {
      ...
      case '=':
        ...
        break;
      case '>':
        reduce(token_stack_, macromap_, node_ptr_vector_, tree_node_stack_);
        break;
      default:
        std::ostringstream ss_;

        ss_ << "A syntax error occurred: '" << lhs_token_.precedence_string()
            << "' against '" << rhs_token_.precedence_string() << "' at index "
            << state_.index() << ".";
...
private:
  ...
  static void reduce(token_stack &token_stack_, const macro_map &macromap_,
    ...
    case token::REPEATN:
    case token::AREPEATN:
      repeatn(rhs_._type == token::REPEATN, handle_.top(), node_vector_ptr_,
              tree_node_stack_);
      token_stack_.push(token::DUP);
      break;
    default:
      throw runtime_error("Internal error regex_parser::reduce");
      break;
    }
  }
  ...
  static void repeat(token_stack &handle_, token_stack &token_stack_) {
    BOOST_ASSERT(handle_.top()._type == token::REPEAT && handle_.size() >= 1 &&
                 handle_.size() <= 3);
  ...
  static void optional(const bool greedy_, node_ptr_vector &node_ptr_vector_,
                       tree_node_stack &tree_node_stack_) {
    // perform ?
    node *lhs_ = tree_node_stack_.top();
  ...
  static void one_or_more(const bool greedy_, node_ptr_vector &node_ptr_vector_,
                          tree_node_stack &tree_node_stack_) {
    // perform +
    node *lhs_ = tree_node_stack_.top();
