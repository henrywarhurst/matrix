...
namespace boost {
namespace lexer {
template <typename CharT> struct basic_string_token {
  typedef std::basic_string<CharT> string;

  bool _negated;
  string _charset;

  basic_string_token() : _negated(false) {}

  basic_string_token(const bool negated_, const string &charset_)
      : _negated(negated_), _charset(charset_) {}
  ...
  void normalise() {
    const std::size_t max_chars_ =
        sizeof(CharT) == 1 ? num_chars : num_wchar_ts;
  ...
  void intersect(basic_string_token &rhs_, basic_string_token &overlap_) {
    if ((any() && rhs_.any()) ||
        (_negated == rhs_._negated && !any() && !rhs_.any())) {
