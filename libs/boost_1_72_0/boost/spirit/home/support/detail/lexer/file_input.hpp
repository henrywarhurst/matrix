...
public:
  ...
  public:
    ...
    struct data {
      ...
      data() : id(0), unique_id(npos), state(npos) {}

      bool operator==(const data &rhs_) const {
        return id == rhs_.id && unique_id == rhs_.unique_id &&
               start == rhs_.start && end == rhs_.end && state == rhs_.state;
      }
    };
    ...
    void next_token() {
      ...
      if (internals_._dfa->size() == 1) {
        _data.id = _input->next(&internals_._lookup->front()->front(),
                                internals_._dfa_alphabet.front(),
                                &internals_._dfa->front()->front(), _data.start,
                                _data.end, _data.unique_id);
      } else {
        _data.id = _input->next(internals_, _data.state, _data.start, _data.end,
                                _data.unique_id);
      }
  ...
  basic_file_input(const basic_state_machine<CharT> *state_machine_,
      ...
      : _state_machine(state_machine_), _stream(is_),
        _buffer_size(buffer_size_), _buffer_increment(buffer_increment_),
...
private:
  ...
  std::size_t next(const detail::internals &internals_,
                   std::size_t &start_state_, const CharT *&start_,
                   const CharT *&end_, std::size_t &unique_id_) {
  ...
  again:
    ...
    for (;;) {
      ...
      if (BOL_state_ &&
          (_start_token == _start_buffer || *(_start_token - 1) == '\n')) {
  ...
  std::size_t next(const std::size_t *const lookup_,
                   ...
                   const std::size_t *const dfa_, const CharT *&start_,
                   const CharT *&end_, std::size_t &unique_id_) {
    _start_token = _end_token;
