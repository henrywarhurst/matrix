...
namespace lexer {
namespace detail {
template <typename CharT> struct basic_re_tokeniser_state {
  ...
  basic_re_tokeniser_state(const CharT *start_, const CharT *const end_,
      ...
      : _start(start_), _end(end_), _curr(start_), _flags(flags_),
        _locale(locale_), _paren_count(0), _in_string(false),
        _seen_BOL_assertion(false), _seen_EOL_assertion(false) {}

  // prevent VC++ 7.1 warning:
  const basic_re_tokeniser_state &
  operator=(const basic_re_tokeniser_state &rhs_) {
    _start = rhs_._start;
    _end = rhs_._end;
    _curr = rhs_._curr;
    _flags = rhs_._flags;
