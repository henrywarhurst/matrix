...
///////////////////////////////////////////////////////////////////////////////
namespace boost {
template <typename T> inline T get(boost::spirit::utree const &x) {
  return x.get<T>();
}
} // namespace boost
...
template <> struct variant_which<utree> {
  static int call(utree const &u) { return u.which(); }
};

template <> struct variant_which<utree::list_type> {
...
template <typename String> struct extract_c_string;
...
template <> struct is_substitute<utree, utree> : mpl::true_ {};

template <> struct is_weak_substitute<utree, utree> : mpl::true_ {};
...
struct assign_to_utree_visitor : static_visitor<> {
  ...
  template <typename T> void operator()(T &val) const { ut_ = val; }
...
// related to utree
template <> struct assign_to_container_from_value<utree, utree> {
...
template <> struct assign_to_container_from_value<utree::list_type, utree> {
...
template <> struct assign_to_container_from_value<utree, utf8_string_type> {
...
template <> struct assign_to_container_from_value<utree, utf8_symbol_type> {
...
template <> struct assign_to_container_from_value<utree, binary_string_type> {
...
template <> struct assign_to_container_from_value<utree, binary_range_type> {
...
template <> struct assign_to_container_from_value<utree, std::string> {
...
template <typename T> struct push_back_container<utree, T> {
...
template <> struct make_container_attribute<utree> {
...
template <> struct build_std_vector<utree> { typedef utree type; };

template <> struct build_std_vector<utree::list_type> {
  typedef utree::list_type type;
};

///////////////////////////////////////////////////////////////////////////
// debug support for utree
template <typename Out> struct print_attribute_debug<Out, utree> {
...
struct handles_utree_list_container
    : mpl::and_<mpl::not_<is_same<utree::list_type, Attribute>>,
...
template <typename Attribute>
struct handles_utree_list_container<
    Attribute, typename enable_if<fusion::traits::is_sequence<Attribute>>::type>
    : mpl::true_ {};
...
template <BOOST_VARIANT_ENUM_PARAMS(typename T)>
struct handles_utree_list_container<
    boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>> : mpl::true_ {};
} // namespace detail

template <typename IteratorA, typename IteratorB, typename Context, typename T1,
          typename T2, typename T3, typename T4>
struct handles_container<qi::rule<IteratorA, T1, T2, T3, T4>, utree, Context,
                         IteratorB>
...
template <typename IteratorA, typename IteratorB, typename Context, typename T1,
          typename T2, typename T3, typename T4>
struct handles_container<qi::grammar<IteratorA, T1, T2, T3, T4>, utree, Context,
                         IteratorB>
    : detail::handles_utree_list_container<typename attribute_of<
          qi::grammar<IteratorA, T1, T2, T3, T4>, Context, IteratorB>::type> {};

template <typename IteratorA, typename IteratorB, typename Context, typename T1,
          typename T2, typename T3, typename T4>
struct handles_container<qi::rule<IteratorA, T1, T2, T3, T4>, utree::list_type,
                         Context, IteratorB>
    : detail::handles_utree_list_container<typename attribute_of<
          qi::rule<IteratorA, T1, T2, T3, T4>, Context, IteratorB>::type> {};
...
template <typename Attribute, typename Sequence>
struct pass_through_container<utree::list_type, utree, Attribute, Sequence,
                              qi::domain>
    : detail::handles_utree_list_container<Attribute> {};
...
// new sub-node in the result.
template <typename Attribute, typename Enable = void>
struct handles_utree_container : mpl::and_<mpl::not_<is_same<utree, Attribute>>,
                                           traits::is_container<Attribute>> {};
...
template <typename IteratorA, typename IteratorB, typename Context, typename T1,
          typename T2, typename T3, typename T4>
struct handles_container<karma::rule<IteratorA, T1, T2, T3, T4>, utree, Context,
                         IteratorB>
    : detail::handles_utree_container<typename attribute_of<
          karma::rule<IteratorA, T1, T2, T3, T4>, Context, IteratorB>::type> {};

template <typename IteratorA, typename IteratorB, typename Context, typename T1,
          typename T2, typename T3, typename T4>
struct handles_container<karma::grammar<IteratorA, T1, T2, T3, T4>, utree,
                         Context, IteratorB>
    : detail::handles_utree_container<
          typename attribute_of<karma::grammar<IteratorA, T1, T2, T3, T4>,
                                Context, IteratorB>::type> {};
...
template <> struct optional_attribute<utree> {
...
template <> struct build_optional<utree> { typedef utree type; };

template <> struct build_optional<utree::list_type> {
  typedef utree::list_type type;
};

// an utree is an optional (in any domain)
template <> struct not_is_optional<utree, qi::domain> : mpl::false_ {};

template <>
struct not_is_optional<utree::list_type, qi::domain> : mpl::false_ {};

template <> struct not_is_optional<utree, karma::domain> : mpl::false_ {};
...
template <> struct not_is_variant<utree, karma::domain> : mpl::false_ {};
...
template <>
struct compute_compatible_component_variant<utree,
                                            iterator_range<utree::iterator>>
    : mpl::true_ {
...
struct compute_compatible_component_variant<
    utree, iterator_range<utree::const_iterator>> : mpl::true_ {
...
///////////////////////////////////////////////////////////////////////////
template <> struct symbols_lookup<utree, utf8_symbol_type> {
...
template <> struct symbols_lookup<utf8_symbol_type, utf8_symbol_type> {
  typedef std::string type;
...
///////////////////////////////////////////////////////////////////////////
namespace detail {
template <typename T> inline T get_or_deref(utree const &t) {
  if (detail::is_list(t))
...
template <> struct extract_from_container<utree, utree::nil_type> {
  typedef utree::nil_type type;

  template <typename Context> static type call(utree const &, Context &) {
    return nil;
  }
};

template <> struct extract_from_container<utree, char> {
  typedef char type;

  template <typename Context> static type call(utree const &t, Context &) {
    utf8_symbol_range_type r = detail::get_or_deref<utf8_symbol_range_type>(t);
    return r.front();
  }
};

template <> struct extract_from_container<utree, bool> {
  typedef bool type;
...
template <> struct extract_from_container<utree, int> {
  typedef int type;
...
template <> struct extract_from_container<utree, double> {
  typedef double type;
...
template <> struct extract_from_container<utree, utf8_symbol_type> {
...
template <> struct extract_from_container<utree, utf8_string_type> {
...
template <> struct transform_attribute<utree const, char, karma::domain> {
  typedef char type;
...
template <> struct transform_attribute<utree const, bool, karma::domain> {
  typedef bool type;

  static type pre(utree const &t) { return detail::get_or_deref<bool>(t); }
};

template <> struct transform_attribute<utree const, int, karma::domain> {
  typedef int type;

  static type pre(utree const &t) { return detail::get_or_deref<int>(t); }
};

template <> struct transform_attribute<utree const, double, karma::domain> {
  typedef double type;
...
template <typename Traits, typename Alloc>
struct transform_attribute<utree const, std::basic_string<char, Traits, Alloc>,
                           karma::domain> {
  typedef std::basic_string<char, Traits, Alloc> type;
...
// of a sequence
template <typename Iterator>
struct transform_attribute<iterator_range<Iterator> const, utree,
                           karma::domain> {
  typedef utree type;
