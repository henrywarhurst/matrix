...
struct bad_type_exception : utree_exception {
  ...
  bad_type_exception(char const *error, utree_type::info got) : msg() {
  ...
  bad_type_exception(char const *error, utree_type::info got1,
      ...
      : msg() {
    std::ostringstream oss;
    oss << "utree: " << error << " (got utree types '" << got1 << "' and '"
        << got2 << "')";
    msg = oss.str();
  }
...
///////////////////////////////////////////////////////////////////////////
template <typename Base, utree_type::info type_> struct basic_string : Base {
...
*/
typedef basic_string<boost::iterator_range<char const *>,
...
template <typename F> struct stored_function : function_base {
  F f;
...
template <typename F> struct referenced_function : function_base {
  F &f;
...
public:
  ...
  typename boost::disable_if<
      boost::is_polymorphic<typename boost::remove_pointer<Ptr>::type>,
      Ptr>::type
  get() const {
    if (*i == typeid(Ptr)) {
  ...
  template <typename T> any_ptr(T *p) : p(p), i(&typeid(T *)) {}
...
private:
  ...
  template <typename UTreeX, typename UTreeY> friend struct detail::visit_impl;
...
public:
  ...
  */
  ...
  template <class Iterator> utree(boost::iterator_range<Iterator>);
  ...
  // single dispatch
  template <class F>
  typename boost::result_of<F(utree const &)>::type static visit(utree const &,
                                                                 F);
  ...
  // double dispatch
  template <class F>
  typename boost::result_of<F(utree const &, utree const &)>::type static visit(
      utree const &, utree const &, F);

  template <class F>
  typename boost::result_of<F(utree &, utree const &)>::type static visit(
      utree &, utree const &, F);

  template <class F>
  typename boost::result_of<F(utree const &, utree &)>::type static visit(
      utree const &, utree &, F);

  template <class F>
  typename boost::result_of<F(utree &, utree &)>::type static visit(utree &,
                                                                    utree &, F);
  ...
  // insertion
  template <class T> void push_back(T const &);
  template <class T> void push_front(T const &);
  template <class T> iterator insert(iterator, T const &);
  template <class T> void insert(iterator, std::size_t, T const &);
  template <class Iterator> void insert(iterator, Iterator, Iterator);
...
private:
  ...
  template <class UTreeX, class UTreeY> friend struct detail::visit_impl;
...
struct utree::list_type : utree {
  ...
  template <typename T0> list_type(T0 t0) : utree(t0) {}

  template <typename T0, typename T1> list_type(T0 t0, T1 t1) : utree(t0, t1) {}
};
