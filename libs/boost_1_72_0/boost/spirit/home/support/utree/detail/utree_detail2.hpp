...
namespace spirit {
namespace detail {
inline char &fast_string::info() { return buff[small_string_size]; }

inline char fast_string::info() const { return buff[small_string_size]; }

inline int fast_string::get_type() const { return info() >> 1; }

inline void fast_string::set_type(int t) { info() = (t << 1) | (info() & 1); }
...
inline bool fast_string::is_heap_allocated() const { return info() & 1; }
...
inline void fast_string::swap(fast_string &other) { std::swap(*this, other); }
...
class list::node_iterator
    : public boost::iterator_facade<node_iterator<Value>, Value,
                                    boost::bidirectional_traversal_tag> {
public:
  node_iterator() : node(0), prev(0) {}

  node_iterator(list::node *node, list::node *prev) : node(node), prev(prev) {}
...
class list::node_iterator<boost::reference_wrapper<Value>>
    : public boost::iterator_facade<
          ...
          boost::reference_wrapper<Value>, boost::bidirectional_traversal_tag> {
public:
  node_iterator() : node(0), prev(0), curr(nil_node) {}
...
template <typename T> inline void list::push_front(T const &val) {
  detail::list::node *new_node;
  if (first == 0) {
...
template <typename T> inline void list::push_back(T const &val) {
  if (last == 0)
    push_front(val);
  else {
    detail::list::node *new_node =
...
template <typename F, typename X> struct bind_impl {
  typedef typename F::result_type result_type;
  X &x; // always by reference
  F f;
  bind_impl(F f, X &x) : x(x), f(f) {}

  template <typename Y> typename F::result_type operator()(Y &y) const {
    return f(x, y);
  }

  template <typename Y> typename F::result_type operator()(Y const &y) const {
    return f(x, y);
  }
};

template <typename F, typename X> bind_impl<F, X const> bind(F f, X const &x) {
  return bind_impl<F, X const>(f, x);
}

template <typename F, typename X> bind_impl<F, X> bind(F f, X &x) {
  return bind_impl<F, X>(f, x);
}

template <typename UTreeX, typename UTreeY = UTreeX> struct visit_impl {
  template <typename F>
  typename F::result_type static apply(UTreeX &x, F f) // single dispatch
  {
    typedef typename boost::mpl::if_<boost::is_const<UTreeX>,
  ...
  template <typename F>
  typename F::result_type static apply(UTreeX &x, UTreeY &y,
                                       F f) // double dispatch
  {
    typedef typename boost::mpl::if_<boost::is_const<UTreeX>,
                                     typename UTreeX::const_iterator,
    ...
    case type::list_type:
      return visit_impl::apply(
          y, detail::bind<F, list_range>(
                 f, list_range(iterator(x.l.first, 0), iterator(0, x.l.last))));

    case type::range_type:
      return visit_impl::apply(
          y, detail::bind<F, list_range>(
                 f, list_range(iterator(x.r.first, 0), iterator(0, x.r.last))));

    case type::string_type:
      return visit_impl::apply(
          y, detail::bind(f, utf8_string_range_type(x.s.str(), x.s.size())));

    case type::string_range_type:
      return visit_impl::apply(
          y, detail::bind(f, utf8_string_range_type(x.sr.first, x.sr.last)));

    case type::symbol_type:
      return visit_impl::apply(
          y, detail::bind(f, utf8_symbol_range_type(x.s.str(), x.s.size())));

    case type::binary_type:
      return visit_impl::apply(
          y, detail::bind(f, binary_range_type(x.s.str(), x.s.size())));

    case type::reference_type:
      return apply(*x.p, y, f);

    case type::any_type:
      return visit_impl::apply(y, detail::bind(f, any_ptr(x.v.p, x.v.i)));
  ...
  static utree &apply(utree &ut, std::size_t i) {
    ...
    default:
      BOOST_THROW_EXCEPTION(bad_type_exception(
          "index operation performed on non-list utree type", ut.get_type()));
    }
  }
...
namespace boost {
namespace spirit {
template <typename F> stored_function<F>::stored_function(F f) : f(f) {}

template <typename F> stored_function<F>::~stored_function() {}
...
template <typename F> utree stored_function<F>::operator()(utree &env) const {
  return f(env);
}

template <typename F> function_base *stored_function<F>::clone() const {
  return new stored_function<F>(f);
}

template <typename F>
referenced_function<F>::referenced_function(F &f) : f(f) {}

template <typename F> referenced_function<F>::~referenced_function() {}
...
template <typename F> function_base *referenced_function<F>::clone() const {
...
template <typename Iter> inline utree::utree(boost::iterator_range<Iter> r) {
  s.initialize();
...
template <typename F>
typename boost::result_of<F(utree const &)>::type inline utree::visit(
    utree const &x, F f) {
...
typename boost::result_of<
    F(utree const &, utree const &)>::type inline utree::visit(utree const &x,
                                                               utree const &y,
                                                               F f) {
...
template <typename F>
typename boost::result_of<F(utree const &, utree &)>::type inline utree::visit(
    utree const &x, utree &y, F f) {
  return detail::visit_impl<utree const, utree>::apply(x, y, f);
}

template <typename F>
typename boost::result_of<F(utree &, utree const &)>::type inline utree::visit(
    utree &x, utree const &y, F f) {
  return detail::visit_impl<utree, utree const>::apply(x, y, f);
}

template <typename F>
typename boost::result_of<F(utree &, utree &)>::type inline utree::visit(
    utree &x, utree &y, F f) {
...
inline utree::const_reference get(utree::const_reference ut,
                                  utree::size_type i) {
  return detail::index_impl::apply(ut, i);
}

template <typename T> inline void utree::push_front(T const &val) {
...
template <typename T> inline void utree::push_back(T const &val) {
  if (get_type() == type::reference_type)
...
inline void utree::pop_front() {
  ...
  if (get_type() != type::list_type)
    BOOST_THROW_EXCEPTION(bad_type_exception(
        "pop_front() called on non-list utree type", get_type()));
...
inline void utree::pop_back() {
  ...
  if (get_type() != type::list_type)
    BOOST_THROW_EXCEPTION(bad_type_exception(
        "pop_back() called on non-list utree type", get_type()));
...
inline utree::iterator utree::erase(iterator pos) {
  ...
  if (get_type() != type::list_type)
    BOOST_THROW_EXCEPTION(bad_type_exception(
        "erase() called on non-list utree type", get_type()));
...
inline utree::const_iterator utree::begin() const {
  ...
  if (get_type() != type::list_type)
    BOOST_THROW_EXCEPTION(bad_type_exception(
        "begin() called on non-list utree type", get_type()));
...
inline utree::const_iterator utree::end() const {
  ...
  if (get_type() != type::list_type)
    BOOST_THROW_EXCEPTION(
        bad_type_exception("end() called on non-list utree type", get_type()));
...
inline std::size_t utree::size() const {
  ...
  if (t != type::nil_type)
    BOOST_THROW_EXCEPTION(bad_type_exception(
        "size() called on non-list and non-string utree type", get_type()));
...
inline utree &utree::back() {
  ...
  if (get_type() != type::list_type)
    BOOST_THROW_EXCEPTION(
        bad_type_exception("back() called on non-list utree type", get_type()));
  else if (!l.last)
...
inline void utree::swap(utree &other) { s.swap(other.s); }
...
template <typename T> struct is_iterator_range : boost::mpl::false_ {};

template <typename Iterator>
struct is_iterator_range<boost::iterator_range<Iterator>> : boost::mpl::true_ {
};

template <typename To> struct utree_cast {
  ...
  template <typename From> To dispatch(From const &, boost::mpl::false_) const {
    // From is NOT convertible to To !!!
    throw std::bad_cast();
    return To();
  }

  template <typename From> To operator()(From const &val) const {
    // boost::iterator_range has a templated constructor, accepting
    // any argument and hence any type is 'convertible' to it.
    typedef typename boost::mpl::eval_if<
        is_iterator_range<To>, boost::is_same<From, To>,
        boost::is_convertible<From, To>>::type is_convertible;
    return dispatch(val, is_convertible());
  }
};

template <typename T> struct utree_cast<T *> {
  typedef T *result_type;

  template <typename From> T *operator()(From const &) const {
...
template <typename T> inline T utree::get() const {
...
inline short utree::tag() const { return s.tag(); }

inline void utree::tag(short tag) { s.tag(tag); }
...
inline utree utree::operator()(utree const &env) const { return eval(env); }

inline utree utree::operator()(utree &env) const { return eval(env); }
} // namespace spirit
} // namespace boost
