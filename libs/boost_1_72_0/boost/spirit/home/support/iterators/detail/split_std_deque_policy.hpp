...
struct split_std_deque {
  ...
  protected:
    ...
    void swap(unique &x) { boost::swap(queued_position, x.queued_position); }
    ...
    template <typename MultiPass> static void increment(MultiPass &mp) {
      queue_type &queue = mp.shared()->queued_elements;
      typename queue_type::size_type size = queue.size();

      BOOST_ASSERT(mp.queued_position <= size);

      //                 // do not increment iterator as long as the current
      //                 token is
      //                 // invalid
    ...
    template <typename MultiPass> static void clear_queue(MultiPass &mp) {
  ...
  ///////////////////////////////////////////////////////////////////////
  template <typename Value> struct shared {
