...
namespace boost {
namespace spirit {
template <typename Terminal, typename Args> struct terminal_ex {
...
template <typename Terminal, typename Actor, int Arity> struct lazy_terminal {
  typedef Terminal terminal_type;
...
template <typename Terminal> struct terminal;
...
template <typename F, typename A0> struct make_lazy<F, A0> {
...
template <typename F, typename A0, typename A1> struct make_lazy<F, A0, A1> {
  typedef typename proto::terminal<lazy_terminal<
      typename F::terminal_type,
  ...
  result_type operator()(F f, A0 const &_0_, A1 const &_1_) const {
    ...
    return result_type::make(
        child_type(phoenix::detail::expression::function_eval<F, A0, A1>::make(
                       f, _0_, _1_),
                   f.proto_base().child0));
  }
};
...
template <> struct bool_<true> : mpl::bool_<true> {
  typedef bool_<true> *is_true;
};

template <> struct bool_<false> : mpl::bool_<false> {
...
template <typename A> struct to_lazy_arg<const A> : to_lazy_arg<A> {};

template <typename A> struct to_lazy_arg<A &> : to_lazy_arg<A> {};

template <> struct to_lazy_arg<unused_type> {
  // unused arg: make_lazy wants unused_type
  typedef unused_type type;
};

// to_nonlazy_arg: convert a terminal arg type to the type make_vector needs
template <typename A> struct to_nonlazy_arg {
  // identity
  typedef A type;
};

template <typename A> struct to_nonlazy_arg<const A> : to_nonlazy_arg<A> {};

template <typename A> struct to_nonlazy_arg<A &> : to_nonlazy_arg<A> {};

// incomplete type: should not be appeared unused_type in nonlazy arg.
template <> struct to_nonlazy_arg<unused_type>;
} // namespace detail
...
#pragma warning(pop)
  ...
  template <typename A0> struct result_helper<false, A0> {
    typedef typename proto::terminal<terminal_ex<
        Terminal, typename detail::result_of::make_vector<
                      typename detail::to_nonlazy_arg<A0>::type>::type>>::type
        type;
  };

  template <typename A0, typename A1> struct result_helper<false, A0, A1> {
    typedef typename proto::terminal<terminal_ex<
        Terminal, typename detail::result_of::make_vector<
                      typename detail::to_nonlazy_arg<A0>::type,
                      typename detail::to_nonlazy_arg<A1>::type>::type>>::type
        type;
  };
  ...
  struct result_helper<false, A0, A1, A2> {
    typedef typename proto::terminal<terminal_ex<
        Terminal, typename detail::result_of::make_vector<
                      ...
                      typename detail::to_nonlazy_arg<A2>::type>::type>>::type
        type;
  };
  ...
  struct result_helper<true, A0, A1, A2> {
    typedef
        typename make_lazy<this_type, typename detail::to_lazy_arg<A0>::type,
  ...
  template <typename A0, typename A1 = unused_type,
            ...
            >
  struct result {
    typedef typename result_helper<detail::contains_actor<A0, A1, A2>::value,
                                   A0, A1, A2>::type type;
  };

  template <typename This, typename A0> struct result<This(A0)> {
  ...
  operator()(A0 const &_0_, A1 const &_1_,
    ...
    typedef typename result_type::proto_child0 child_type;
    return result_type::make(
        child_type(detail::make_vector(_0_, _1_), this->proto_base().child0));
  }
...
template <typename Tag> struct terminal { typedef spirit::terminal<Tag> type; };

template <typename Tag, typename A0> struct terminal<Tag(A0)> {
  typedef typename spirit::terminal<Tag>::template result<A0>::type type;
};

template <typename Tag, typename A0, typename A1> struct terminal<Tag(A0, A1)> {
...
template <typename StatefulTag> struct get_stateful_data {
  typedef typename StatefulTag::data_type data_type;

  // is invoked if given tag is != Tag
  template <typename Tag_> static data_type call(Tag_) { return data_type(); }
