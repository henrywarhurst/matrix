...
struct make_unary : proto::transform<make_unary<Domain, Tag, Grammar>> {
  ...
  struct impl : proto::transform_impl<Expr, State, Data> {
    ...
    typedef typename Grammar::template result<Grammar(child, State, Data)>::type
        child_component;
    ...
    result_type operator()(typename impl::expr_param expr,
      ...
      return typename impl::make_component_()(
          detail::make_cons(Grammar()(proto::child(expr), state, data)), data);
    }
  };
};
...
struct make_binary {
  ...
  struct impl : proto::transform_impl<Expr, State, Data> {
    typedef typename Grammar::template result<Grammar(
        typename proto::result_of::child_c<Expr, 0>::type, State, Data)>::type
        lhs_component;

    typedef typename Grammar::template result<Grammar(
        typename proto::result_of::child_c<Expr, 1>::type, State, Data)>::type
        rhs_component;

    typedef typename result_of::make_cons<
        lhs_component, typename result_of::make_cons<rhs_component>::type>::type
        elements_type;
...
struct make_binary_helper : proto::transform<make_binary_helper<Grammar>> {
  ...
  struct impl : proto::transform_impl<Expr, State, Data> {
    typedef
        typename Grammar::template result<Grammar(Expr, State, Data)>::type lhs;
...
struct make_action : proto::transform<make_action<Domain, Grammar>> {
  ...
  struct impl : proto::transform_impl<Expr, State, Data> {
    ...
    result_type operator()(typename impl::expr_param expr,
                           typename impl::state_param state,
                           typename impl::data_param data, mpl::false_) const {
      elements_type elements = detail::make_cons(
          Grammar()(proto::child_c<0>(expr), state, data) // LHS
          ,
