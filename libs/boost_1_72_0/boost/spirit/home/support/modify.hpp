...
struct compound_modifier<Current, New,
                         typename enable_if<has_modifier<Current, New>>::type>
    : Current {
  compound_modifier() : Current() {}

  compound_modifier(Current const &current, New const &) : Current(current) {}
};
...
struct compound_modifier<unused_type, New, Enable> : New {
  compound_modifier() : New() {}

  compound_modifier(unused_type, New const &new_) : New(new_) {}
};
...
template <typename Domain, typename Enable = void> struct modify {
  template <typename Sig> struct result;

  template <typename This, typename Tag, typename Modifiers>
  struct result<This(Tag, Modifiers)> {
    typedef typename remove_const<typename remove_reference<Tag>::type>::type
        tag_type;
    typedef
        typename remove_const<typename remove_reference<Modifiers>::type>::type
            modifiers_type;

    typedef typename mpl::if_<is_modifier_directive<Domain, tag_type>,
  ...
  template <typename Tag, typename Modifiers>
  compound_modifier<Modifiers, Tag> op(Tag tag, Modifiers modifiers,
                                       mpl::true_) const {
...
struct is_callable<spirit::modify<Domain, Enable>> : mpl::true_ {};
} // namespace proto
} // namespace boost
