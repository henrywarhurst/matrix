...
template <typename T> struct is_char<T const> : is_char<T> {};
...
template <typename T> struct is_string<T const> : is_string<T> {};

template <> struct is_string<char const *> : mpl::true_ {};

template <> struct is_string<wchar_t const *> : mpl::true_ {};

template <> struct is_string<char *> : mpl::true_ {};

template <> struct is_string<wchar_t *> : mpl::true_ {};

template <std::size_t N> struct is_string<char[N]> : mpl::true_ {};

template <std::size_t N> struct is_string<wchar_t[N]> : mpl::true_ {};

template <std::size_t N> struct is_string<char const[N]> : mpl::true_ {};

template <std::size_t N> struct is_string<wchar_t const[N]> : mpl::true_ {};
...
template <typename T> struct char_type_of<T const> : char_type_of<T> {};

template <> struct char_type_of<char> : mpl::identity<char> {};

template <> struct char_type_of<wchar_t> : mpl::identity<wchar_t> {};

template <> struct char_type_of<char const *> : mpl::identity<char const> {};
...
template <std::size_t N> struct char_type_of<char[N]> : mpl::identity<char> {};
...
template <typename String> struct extract_c_string {
  ...
  template <typename T> static T const *call(T *str) { return (T const *)str; }

  template <typename T> static T const *call(T const *str) { return str; }
};

// Forwarder that strips const
template <typename T> struct extract_c_string<T const> {
...
template <typename T> struct extract_c_string<T &> {
  typedef typename extract_c_string<T>::char_type char_type;
...
#if defined(__GNUC__) && (__GNUC__ < 4)
template <typename T> inline typename add_const<T>::type *get_begin(T *str) {
  return str;
}

template <typename T> inline typename add_const<T>::type *get_end(T *str) {
  T *last = str;
  while (*last)
    last++;
  return last;
...
#else
template <typename T> inline T const *get_begin(T const *str) { return str; }

template <typename T> inline T *get_begin(T *str) { return str; }

template <typename T> inline T const *get_end(T const *str) {
  T const *last = str;
  while (*last)
    last++;
  return last;
...
template <typename T> inline T *get_end(T *str) {
  T *last = str;
  while (*last)
    last++;
  return last;
...
template <typename T, typename Str>
inline typename disable_if<is_container<Str>, T const *>::type
...
namespace result_of {
template <typename Char, typename T, typename Enable = void> struct get_begin {
  typedef typename traits::char_type_of<T>::type char_type;

  typedef
      typename mpl::if_<is_const<char_type>, char_type const, char_type>::type
          *type;
};
...
struct get_begin<Char, Str,
  ...
  typedef typename mpl::if_<is_const<Str>, typename Str::const_iterator,
                            typename Str::iterator>::type type;
};

template <typename Char, typename T> struct get_end : get_begin<Char, T> {};
} // namespace result_of
