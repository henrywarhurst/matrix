...
namespace spirit {
namespace traits {
template <typename Component> struct action_dispatch {
#if !defined(BOOST_NO_CXX11_RVALUE_REFERENCES) &&                              \
    !defined(BOOST_NO_CXX11_LAMBDAS) &&                                        \
    !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES) &&                             \
    !defined(BOOST_NO_CXX11_DECLTYPE)
...
private:
  ...
  template <typename> struct fwd_tag {};
...
#define BOOST_SPIRIT_FAKE_CALL(T) (*(T *)0)
  ...
  template <typename F, typename... A> static void caller(F &&f, A &&... a) {
...
public:
  ...
  bool operator()(phoenix::actor<Eval> const &f, Attribute &attr,
                  Context &context) {
    bool pass = true;
  ...
  template <typename RT, typename A0, typename A1, typename Attribute,
            typename Context>
