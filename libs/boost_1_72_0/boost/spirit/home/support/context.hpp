...
namespace boost {
namespace spirit {
template <int> struct attribute;

template <int> struct local_variable;
} // namespace spirit
} // namespace boost
...
namespace boost {
namespace spirit {
template <typename Attributes, typename Locals> struct context {
...
template <typename Context> struct attributes_of {
  typedef typename Context::attributes_type type;
};

template <typename Context> struct attributes_of<Context const> {
...
template <typename Context> struct locals_of {
  typedef typename Context::locals_type type;
};

template <typename Context> struct locals_of<Context const> {
  typedef typename Context::locals_type const type;
};

template <typename Context> struct locals_of<Context &> {
  typedef typename Context::locals_type type;
};
...
template <int N> struct attribute {
  ...
  template <typename Env> struct result {
    ...
    typedef
        typename fusion::result_of::size<attributes_type>::type attributes_size;
...
template <int N> struct local_variable {
  typedef mpl::true_ no_nullary;

  template <typename Env> struct result {
    typedef typename locals_of<
        typename mpl::at_c<typename Env::args_type, 1>::type>::type locals_type;
