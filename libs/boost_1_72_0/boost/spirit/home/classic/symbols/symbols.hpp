...
public:
  ...
  parse_main(ScannerT const &scan) const {
    ...
    if (result.data)
      return scan.create_match(result.length, symbol_ref_t(*result.data), first,
                               scan.first);
    else
  ...
  template <typename ScannerT> T *find(ScannerT const &scan) const {
...
public:
  ...
  symbol_inserter const &operator()(IteratorT first, IteratorT const &last,
  ...
  symbol_inserter const &operator()(CharT const *str,
                                    T const &data = T()) const {
