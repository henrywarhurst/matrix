...
#error                                                                         \
    "You must include boost/spirit/debug.hpp, not boost/spirit/debug/debug_node.hpp"
#endif
...
struct token_printer_aux_for_chars {
  template <typename CharT> static void print(std::ostream &o, CharT c) {
...
template <typename CharT> inline void token_printer(std::ostream &o, CharT c) {
...
inline void print_node_info(bool hit, int level, bool close,
                            std::string const &name, IteratorT first,
                            IteratorT last) {
  if (!name.empty()) {
...
inline ResultT &print_closure_info(ResultT &hit, int level,
...
template <typename ContextT> struct parser_context_linker : public ContextT {
  ...
  void pre_parse(ParserT const &p, ScannerT &scan) {
    ...
    if (trace_parser(p.derived())) {
      impl::print_node_info(false, scan.get_level(), false,
                            parser_name(p.derived()), scan.first, scan.last);
    }
    scan.get_level()++;
#endif // BOOST_SPIRIT_DEBUG_FLAGS & BOOST_SPIRIT_DEBUG_FLAGS_NODES
  }
  ...
  ResultT &post_parse(ResultT &hit, ParserT const &p, ScannerT &scan) {
    ...
    --scan.get_level();
    if (trace_parser(p.derived())) {
      impl::print_node_info(hit, scan.get_level(), true,
                            parser_name(p.derived()), scan.first, scan.last);
    }
#endif // BOOST_SPIRIT_DEBUG_FLAGS & BOOST_SPIRIT_DEBUG_FLAGS_NODES
...
struct closure_context_linker : public parser_context_linker<ContextT> {
  ...
  ResultT &post_parse(ResultT &hit, ParserT const &p, ScannerT &scan) {
    ...
    if (hit && trace_parser(p.derived())) {
      // for now, print out the return value only
      return impl::print_closure_info(this->base_t::post_parse(hit, p, scan),
                                      scan.get_level(),
