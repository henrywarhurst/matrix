...
struct f_chlit : public char_parser<f_chlit<ChGenT>> {
  f_chlit(ChGenT chgen_) : chgen(chgen_) {}

  template <typename T> bool test(T ch) const { return ch == chgen(); }

  ChGenT chgen;
};

template <typename ChGenT> inline f_chlit<ChGenT> f_ch_p(ChGenT chgen) {
...
struct f_range : public char_parser<f_range<ChGenAT, ChGenBT>> {
  f_range(ChGenAT first_, ChGenBT last_) : first(first_), last(last_) {}
...
public:
  ...
  f_chseq(IterGenAT first_, IterGenBT last_) : first(first_), last(last_) {}
...
template <typename IterGenAT, typename IterGenBT>
inline f_chseq<IterGenAT, IterGenBT> f_chseq_p(IterGenAT first,
                                               IterGenBT last) {
...
public:
  ...
  f_strlit(IterGenAT first, IterGenBT last) : seq(first, last) {}
