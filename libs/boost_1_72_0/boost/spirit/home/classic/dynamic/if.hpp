...
struct if_else_parser
  ...
  parse(ScannerT const &scan) const {
    ...
    typedef
        typename parser_result<parser_false_t, ScannerT>::type else_result_t;
...
template <typename ParsableTrueT, typename CondT> struct if_else_parser_gen {
  ...
  operator[](ParsableFalseT const &p_false) const {
    return if_else_parser<ParsableTrueT, ParsableFalseT, CondT>(p_true, p_false,
                                                                cond);
  }
...
// if-parser.
template <typename CondT> struct if_parser_gen {
...
template <typename CondT> impl::if_parser_gen<CondT> if_p(CondT const &cond) {
