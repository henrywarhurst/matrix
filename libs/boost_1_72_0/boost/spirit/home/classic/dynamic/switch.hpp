...
struct switch_parser
  ...
  switch_parser(CaseT const &case_) : base_t(case_), cond(CondT()) {}
...
///////////////////////////////////////////////////////////////////////////////
template <typename CondT> struct switch_cond_parser {
  switch_cond_parser(CondT const &cond_) : cond(cond_) {}
...
struct case_parser
  ...
  BOOST_STATIC_CONSTANT(
      bool,
      is_epsilon = (is_default &&
                    boost::is_same<self_subject_t, epsilon_parser>::value));

  case_parser(parser<ParserT> const &p) : base_t(p.derived()) {}
  ...
  parse(ScannerT const &scan, CondT const &cond) const {
    ...
    if (!scan.at_end()) {
      typedef impl::default_delegate_parse<value, is_default, default_t::value>
          default_parse_t;
  ...
  impl::compound_case_parser<self_t, case_parser<N1, ParserT1, IsDefault1>,
                             IsDefault1>
...
struct switch_parser_gen {
  ...
  //  This generates a switch parser, which is driven by the next
  //  character/token found in the input stream.
