...
struct select_parser : public parser<select_parser<TupleT, BehaviourT, T>> {
  ...
  select_parser(TupleT const &t_) : t(t_) {}
  ...
  parse(ScannerT const &scan) const {
    ...
    if (!scan.at_end()) {
      return impl::parse_tuple_element<TupleT::length, result_t, TupleT,
                                       BehaviourT>::do_(t, scan);
    }
...
///////////////////////////////////////////////////////////////////////////////
template <typename BehaviourT, typename T = int> struct select_parser_gen {
...
#define BOOST_SPIRIT_SELECT_EMBEDDED_TYPEDEF(z, N, _)                          \
  typedef impl::as_embedded_parser<BOOST_PP_CAT(ParserT, N)> BOOST_PP_CAT(     \
      parser_t, N);                                                            \
  /**/
...
#define BOOST_SPIRIT_SELECT_PARSER(z, N, _)                                    \
  ...
  select_parser<::phoenix::tuple<BOOST_PP_ENUM_##z(                            \
                    BOOST_PP_INC(N), BOOST_SPIRIT_SELECT_EMBEDDED, _)>,        \
                BehaviourT, T>                                                 \
  operator()(BOOST_PP_ENUM_BINARY_PARAMS_Z(z, BOOST_PP_INC(N), ParserT,        \
                                           const &p)) const {                  \
    BOOST_PP_REPEAT_##z(BOOST_PP_INC(N), BOOST_SPIRIT_SELECT_EMBEDDED_TYPEDEF, \
                        _)                                                     \
                                                                               \
        typedef ::phoenix::tuple<BOOST_PP_ENUM_BINARY_PARAMS_Z(                \
            z, BOOST_PP_INC(N), typename parser_t, ::type BOOST_PP_INTERCEPT)> \
            tuple_t;                                                           \
    typedef select_parser<tuple_t, BehaviourT, T> result_t;                    \
                                                                               \
    return result_t(tuple_t(                                                   \
        BOOST_PP_ENUM_##z(BOOST_PP_INC(N), BOOST_SPIRIT_SELECT_CONVERT, _)));  \
  }                                                                            \
  /**/
