...
struct iteration_policy {
  template <typename ScannerT> void advance(ScannerT const &scan) const {
    ++scan.first;
  }

  template <typename ScannerT> bool at_end(ScannerT const &scan) const {
    return scan.first == scan.last;
  }

  template <typename T> T filter(T ch) const { return ch; }
...
///////////////////////////////////////////////////////////////////////////
struct match_policy {
  template <typename T> struct result { typedef match<T> type; };
...
//
///////////////////////////////////////////////////////////////////////////
template <typename AttrT> struct attributed_action_policy {
...
//////////////////////////////////
template <> struct attributed_action_policy<nil_t> {
  template <typename ActorT, typename IteratorT>
...
struct scanner_policies : public IterationPolicyT,
  ...
  scanner_policies(IterationPolicyT const &i_policy = IterationPolicyT(),
      ...
      : IterationPolicyT(i_policy), MatchPolicyT(m_policy),
...
public:
  ...
  template <typename PoliciesT2> struct rebind_policies {
  ...
  template <typename IteratorT2> struct rebind_iterator {
