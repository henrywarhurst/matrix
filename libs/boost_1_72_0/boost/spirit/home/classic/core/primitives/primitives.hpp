...
template <typename DerivedT> struct char_parser : public parser<DerivedT> {
  ...
  template <typename ScannerT> struct result {
    typedef
        typename match_result<ScannerT, typename ScannerT::value_t>::type type;
  };
...
struct negated_char_parser
  ...
  template <typename T> bool test(T ch) const { return !positive.test(ch); }
...
struct chlit : public char_parser<chlit<CharT>> {
  ...
  template <typename T> bool test(T ch_) const { return ch_ == ch; }

  CharT ch;
};

template <typename CharT> inline chlit<CharT> ch_p(CharT ch) {
...
struct range : public char_parser<range<CharT>> {
  ...
  template <typename T> bool test(T ch) const {
...
template <typename CharT> inline range<CharT> range_p(CharT first, CharT last) {
...
template <typename CharT> inline strlit<CharT const *> str_p(CharT const *str) {
  return strlit<CharT const *>(str);
}

template <typename CharT> inline strlit<CharT *> str_p(CharT *str) {
...
template <typename CharT> inline chlit<CharT> str_p(CharT ch) {
  return chlit<CharT>(ch);
}
...
struct anychar_parser : public char_parser<anychar_parser> {
  ...
  template <typename CharT> bool test(CharT) const { return true; }
};

anychar_parser const anychar_p = anychar_parser();

inline nothing_parser operator~(anychar_parser) { return nothing_p; }
...
struct alnum_parser : public char_parser<alnum_parser> {
  ...
  template <typename CharT> bool test(CharT ch) const {
