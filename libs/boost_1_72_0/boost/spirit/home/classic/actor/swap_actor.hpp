...
public:
  swap_actor(T &ref_, T &swap_ref_) : ref(ref_), swap_ref(swap_ref_){};
  ...
  void operator()(IteratorT const & /*first*/, IteratorT const & /*last*/
  ) const {
    ref.swap(swap_ref);
  }
};

template <typename T> inline swap_actor<T> swap_a(T &ref_, T &swap_ref_) {
