...
template <typename ValueT = nil_t> class node_iter_data_factory;

template <typename ValueT = nil_t> class node_val_data_factory;

template <typename ValueT = nil_t> class node_all_val_data_factory;
...
template <typename T> struct no_tree_gen_node_parser;

template <typename T> struct leaf_node_parser;

template <typename T, typename NodeParserT> struct node_parser;
...
template <typename T, typename ActionParserT> struct action_directive_parser;

struct access_match_action {
  template <typename ParserT, typename ActionT> struct action;
};
