...
template <typename T> void swap(tree_node<T> &a, tree_node<T> &b);

template <typename T, typename V>
void swap(node_iter_data<T, V> &a, node_iter_data<T, V> &b);

namespace impl {
template <typename T> inline void cp_swap(T &t1, T &t2);
}
...
//////////////////////////////////
template <typename IteratorT, typename ValueT> struct node_iter_data {
...
inline std::ostream &operator<<(std::ostream &o, nil_t const &) { return o; }
...
template <typename T> inline void swap(tree_node<T> &a, tree_node<T> &b) {
  a.swap(b);
}
...
//////////////////////////////////
template <typename ValueT> class node_iter_data_factory {
public:
  // This inner class is so that node_iter_data_factory can simulate
  // a template template parameter
  template <typename IteratorT> class factory {
  public:
    typedef IteratorT iterator_t;
...
public:
  ...
  public:
    ...
    static node_t group_nodes(ContainerT const &nodes) {
      ...
      for (typename ContainerT::const_iterator i = nodes.begin(); i != i_end;
           ++i) {
...
//////////////////////////////////
template <typename ValueT> class node_all_val_data_factory {
public:
...
public:
  ...
  template <typename T2> tree_match &operator=(match<T2> const &other) {
...
operator<<(std::ostream &o, tree_match<IteratorT, NodeFactoryT, T> const &m) {
  typedef typename tree_match<IteratorT, NodeFactoryT, T>::container_t::iterator
      iterator;
...
struct common_tree_match_policy : public match_policy {
  ...
  create_match(std::size_t length, AttrT const &val, Iterator1T const &first,
               Iterator2T const &last) {
#if defined(BOOST_SPIRIT_DEBUG) &&                                             \
    (BOOST_SPIRIT_DEBUG_FLAGS & BOOST_SPIRIT_DEBUG_FLAGS_NODES)
...
struct leaf_node_parser : public unary<T, parser<leaf_node_parser<T>>> {
  ...
  parse(ScannerT const &scanner) const {
    typedef scanner_policies<typename ScannerT::iteration_policy_t,
                             match_policy, typename ScannerT::action_policy_t>
        policies_t;
...
struct leaf_node_parser_gen {
  template <typename T> struct result { typedef leaf_node_parser<T> type; };
...
template <typename MatchPolicyT> struct tree_policy_selector {
...
//////////////////////////////////
template <typename NodeParserT> struct node_parser_gen;
...
struct node_parser : public unary<T, parser<node_parser<T, NodeParserT>>> {
  ...
  parse(ScannerT const &scanner) const {
    ...
    if (hit) {
      impl::tree_policy_selector<typename ScannerT::match_policy_t>::type::
          apply_op_to_match(NodeParserT(), hit);
    }
    return hit;
  }
};
...
struct reduced_node_op {
  template <typename MatchT> void operator()(MatchT &m) const {
    if (m.trees.size() == 1) {
...
template <typename ActionParserT> struct action_directive_parser_gen;
...
template <typename ParserT, typename ActionT>
ActionT const &
access_match_action::action<ParserT, ActionT>::predicate() const {
  return actor;
}
