...
template <typename CharT = char, typename BaseIterator =
...
#error                                                                         \
    "Please use at least Boost V1.31.0 while compiling the file_iterator class!"
#else // BOOST_ITERATOR_ADAPTORS_VERSION < 0x0200

template <typename CharT, typename BaseIteratorT> struct file_iter_generator {
public:
  typedef BaseIteratorT adapted_t;
  typedef typename adapted_t::value_type value_type;

  typedef boost::iterator_adaptor<
      file_iterator<CharT, BaseIteratorT>, adapted_t, value_type const,
      std::random_access_iterator_tag, boost::use_default, std::ptrdiff_t>
      type;
};
...
private:
  typedef
      typename fileiter_impl::file_iter_generator<CharT, BaseIteratorT>::type
          base_t;
  typedef typename fileiter_impl::file_iter_generator<
      CharT, BaseIteratorT>::adapted_t adapted_t;

public:
  file_iterator() {}

  file_iterator(std::string const &fileName) : base_t(adapted_t(fileName)) {}

  file_iterator(const base_t &iter) : base_t(iter) {}
