...
//
///////////////////////////////////////////////////////////////////////////
template <typename T0, typename T = T0> struct make_const_pointer {

private:
  // T0 shouldn't be of type 'same'
  BOOST_STATIC_ASSERT((!boost::is_same<T0, same>::value));

  typedef typename boost::mpl::if_c<boost::is_same<T, same>::value, T0 const *,
                                    T const *>::type ptr_type;
...
public:
  ...
  typedef typename boost::mpl::if_c<boost::is_same<T, ::phoenix::nil_t>::value,
                                    ::phoenix::nil_t, ptr_type>::type type;
};

///////////////////////////////////////////////////////////////////////////
template <int N, typename ElementT> struct assign_zero_to_tuple_member {

  template <typename TupleT> static void do_(TupleT &t) {
    t[::phoenix::tuple_index<N>()] = 0;
  }
};

template <int N> struct assign_zero_to_tuple_member<N, ::phoenix::nil_t> {
...
template <int N> struct init_tuple_member {

  template <typename TupleT> static void do_(TupleT &t) {
    typedef typename boost::mpl::eval_if_c<(N < TupleT::length),
                                           ::phoenix::tuple_element<N, TupleT>,
                                           phoenix_nil_type>::type element_type;
...
#define BOOST_SPIRIT_GRAMMARDEF_TUPLE_PARAM(z, N, _)                           \
  ...
  typedef ::phoenix::tuple<
      typename impl::make_const_pointer<T>::type,
      BOOST_PP_ENUM(BOOST_PP_DEC(BOOST_SPIRIT_GRAMMAR_STARTRULE_TYPE_LIMIT_A),
                    BOOST_SPIRIT_GRAMMARDEF_TUPLE_PARAM, _)>
      tuple_t;
...
#define BOOST_SPIRIT_GRAMMARDEF_ENUM_START(z, N, _)                            \
  ...
  void start_parsers(BOOST_PP_ENUM_##z(                                        \
      BOOST_PP_INC(N), BOOST_SPIRIT_GRAMMARDEF_ENUM_PARAMS, _)) {              \
    BOOST_PP_REPEAT_##z(BOOST_PP_INC(N), BOOST_SPIRIT_GRAMMARDEF_ENUM_ASSIGN,  \
                        _)                                                     \
  }                                                                            \
  /**/

  BOOST_PP_REPEAT(BOOST_SPIRIT_GRAMMAR_STARTRULE_TYPE_LIMIT_A,
...
#define BOOST_SPIRIT_GRAMMARDEF_ENUM_INIT(z, N, _)                             \
  ...
  grammar_def() {
    using ::phoenix::tuple_index_names::_1;
    t[_1] = 0;
    BOOST_PP_REPEAT_FROM_TO(1, BOOST_SPIRIT_GRAMMAR_STARTRULE_TYPE_LIMIT_A,
