...
#define BOOST_SPIRIT_RP_IMPL_I(name, pars, acts, mbrs, expr)                   \
  BOOST_SPIRIT_RP_IMPL_II(name, name##_t, pars,                                \
                          BOOST_SPIRIT_RP_ARRAY_SIZE(pars), acts,              \
                          BOOST_SPIRIT_RP_ARRAY_SIZE(acts), mbrs,              \
...
#define BOOST_SPIRIT_RP_IMPL_III(name, name_t, pars, np, acts, na, mbrs, nm,   \
                                 x)                                            \
                                                                               \
  template <BOOST_SPIRIT_RP_TPL_PARAMS(pars, acts, typename __, 1)>            \
  class name_t : public ::BOOST_SPIRIT_CLASSIC_NS::parser<                     \
                     name_t<BOOST_SPIRIT_RP_TPL_PARAMS(pars, acts, __, 0)>> {  \
    class __rule {                                                             \
  ...
  public:                                                                      \
    ...
    typedef                                                                    \
        typename __rule::__expr::type::parser_category_t parser_category_t;    \
                                                                               \
  ...
  protected:                                                                   \
    ...
    BOOST_SPIRIT_RP_IF(na, SPIRIT_RP_AP_EXTRA_MBRS, 2)                         \
    (np, na)                                                                   \
                                                                               \
  ...
  public:                                                                      \
    explicit name_t(BOOST_SPIRIT_RP_CTOR(PARAMS, pars, np, acts))              \
        : BOOST_SPIRIT_RP_EMIT(MV_CTOR_INIT_LIST, mbrs, -)                     \
              BOOST_PP_COMMA_IF(nm) BOOST_SPIRIT_RP_IF(                        \
                  na, SPIRIT_RP_CTOR_COMMA, 4)(INIT_LIST, pars, np, acts)      \
                  __parser(x) {}                                               \
                                                                               \
    name_t(name_t const &that)                                                 \
        : BOOST_SPIRIT_RP_EMIT(MV_CTOR_COPY_INIT_LIST, mbrs, that)             \
              BOOST_PP_COMMA_IF(nm) BOOST_SPIRIT_RP_IF(                        \
                  na, SPIRIT_RP_CTOR_COMMA, 4)(COPY_INIT_LIST, pars, np, acts) \
  ...
  (name, name_t, np, na) BOOST_SPIRIT_RP_REGISTER_TEMPLATE(                    \
      name_t, BOOST_PP_INC(BOOST_PP_ADD(np, na)))
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// RP_OPAQUE_IMPL_I
//
#define BOOST_SPIRIT_RP_OPAQUE_IMPL_I(name, pars, mbrs, expr)                  \
  BOOST_SPIRIT_RP_OPAQUE_IMPL_II(name, name##_t, pars,                         \
                                 BOOST_SPIRIT_RP_ARRAY_SIZE(pars), -, -, mbrs, \
                                 BOOST_SPIRIT_RP_ARRAY_SIZE(mbrs), expr)
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// RP_OPAQUE_IMPL_II
//
#define BOOST_SPIRIT_RP_OPAQUE_IMPL_II(name, name_t, pars, np, _1, _2, mbrs,   \
                                       nm, x)                                  \
  class name_t;                                                                \
                                                                               \
...
private:                                                                       \
  template <typename A> struct __rebound_1st {                                 \
    typedef name_t<                                                            \
        ...
        typename ns ::action_concatenator<__A0, A>::type BOOST_PP_COMMA_IF(    \
            BOOST_PP_DEC(na)) BOOST_PP_ENUM_SHIFTED_PARAMS(na, __A)>           \
        type;                                                                  \
  };                                                                           \
                                                                               \
...
public:                                                                        \
  ...
  typename __rebound<ns ::action_chain<PH, M, A>>::type const operator[](      \
      ns ::action_chain<PH, M, A> const &x) const {                            \
...
#define BOOST_SPIRIT_RP_GEN_FUNC(name, name_t, np, na)                         \
  ...
  inline name_t<void BOOST_PP_ENUM_TRAILING_PARAMS(np, T)> name(               \
      BOOST_PP_ENUM_BINARY_PARAMS(np, T, const &p)) {                          \
    return name_t<void BOOST_PP_ENUM_TRAILING_PARAMS(np, T)>(                  \
        BOOST_PP_ENUM_PARAMS(np, p) BOOST_PP_ENUM_TRAILING_PARAMS(             \
            na, ::BOOST_SPIRIT_CLASSIC_NS::type_of::nop_functor()              \
                    BOOST_PP_INTERCEPT));                                      \
  }
// RP_GEN_OPAQUE
//
...
#define BOOST_SPIRIT_RP_GLOB_VAR(name, name_t, np, na)                         \
  static name_t<void> const name = name_t<void>(BOOST_PP_ENUM_PARAMS(          \
      na,                                                                      \
...
#define BOOST_SPIRIT_RP__PM_OPAQUE_STATIC(r, data, i, elem)                    \
  static ::boost::call_traits<BOOST_SPIRIT_RP_TYPE(BOOST_PP_TUPLE_ELEM(        \
      2, 0, elem))>::reference BOOST_PP_TUPLE_ELEM(2, 1, elem);

// PM_OPAQUE_CTOR_PARAMS
#define BOOST_SPIRIT_RP__PM_OPAQUE_CTOR_PARAMS(r, data, i, elem)               \
  BOOST_PP_COMMA_IF(i)::boost::call_traits<BOOST_SPIRIT_RP_TYPE(               \
      BOOST_PP_TUPLE_ELEM(2, 0, elem))>::param_type BOOST_PP_TUPLE_ELEM(2, 1,  \
                                                                        elem)

// PM_OPAQUE_GEN_PARAMS
#define BOOST_SPIRIT_RP__PM_OPAQUE_GEN_PARAMS(r, data, i, elem)                \
  BOOST_PP_COMMA_IF(i)::boost::call_traits<BOOST_SPIRIT_RP_TYPE(               \
      BOOST_PP_TUPLE_ELEM(2, 0, elem))>::param_type data##i
...
#define BOOST_SPIRIT_RP__MV_STATIC(r, data, i, elem)                           \
  static data()::boost::call_traits<data() BOOST_SPIRIT_RP_TYPE(               \
      BOOST_PP_TUPLE_ELEM(3, 0, elem))>::reference BOOST_PP_TUPLE_ELEM(3, 1,   \
                                                                       elem);
...
#define BOOST_SPIRIT_RP__AP_REBOUND_ARGS(r, data, i, elem)                     \
  BOOST_PP_COMMA_IF(i)                                                         \
  ::BOOST_SPIRIT_CLASSIC_NS::type_of::get_placeholdee<                         \
      __action_placeholder::elem>(__a##i, data)
...
#define BOOST_SPIRIT_RP_ARRAY_FOR_EACH_I(macro, data, optional_array)          \
  BOOST_PP_IIF(BOOST_PP_IS_BINARY(optional_array),                             \
               BOOST_SPIRIT_RP_ARRAY_FOR_EACH_I_IMPL, BOOST_PP_TUPLE_EAT(3))   \
  (macro, data, optional_array)
...
#define BOOST_SPIRIT_RP_ARRAY_FOR_EACH_I_IMPL(macro, data, array)              \
  ...
  (macro, data,                                                                \
   BOOST_SPIRIT_RP_IF(BOOST_PP_ARRAY_SIZE(array), PP_TUPLE_TO_SEQ, 2) array)
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// RP_ARRAY_SIZE
//
...
#define BOOST_SPIRIT_RP_ARRAY_SIZE(optional_array)                             \
  BOOST_PP_IIF(BOOST_PP_IS_BINARY(optional_array), BOOST_PP_ARRAY_SIZE,        \
...
template <class P> class parser_reference : public parser<parser_reference<P>> {
  P const &ref_that;
...
template <typename Action1, typename Action2> class composite_action {
  Action1 fnc_a1;
  Action2 fnc_a2;
...
public:
  ...
  template <typename T> void operator()(T const &inp) const {
    fnc_a1(inp);
    fnc_a2(inp);
  }
...
template <typename Action1, typename Action2> struct action_concatenator {
...
public:
  action_chain(Action const &a) : fnc_action(a) {}
...
template <class ChainOrChains, class LastChain> class action_chains {
...
template <class PH1, action_chain_mode M1, typename A1, class PH2,
          action_chain_mode M2, typename A2>
...
struct placeholdee<Placeholder, StartAction, action_chains<Head, Tail>> {
  typedef typename placeholdee<
      Placeholder, typename placeholdee<Placeholder, StartAction, Head>::type,
      Tail>::type type;
...
struct placeholdee<Placeholder, StartAction,
                   action_chain<Placeholder, replace, A>> {
  typedef A type;
...
template <class Placeholder, typename StartAction, typename A>
struct placeholdee<Placeholder, StartAction,
                   action_chain<Placeholder, append, A>> {
...
BOOST_TYPEOF_REGISTER_TEMPLATE(
    BOOST_SPIRIT_CLASSIC_NS::type_of::composite_action, 2)
...
template <typename T, typename X> X const &depend_on_type(X const &x) {
  return x;
}
