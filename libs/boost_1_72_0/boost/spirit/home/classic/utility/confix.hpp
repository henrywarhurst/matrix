...
template <typename NestedT, typename LexemeT> struct confix_parser_gen {
  ...
  struct paren_op_result_type {
    typedef confix_parser<
        ...
        typename as_parser<ExprT>::type::parser_category_t, NestedT, LexemeT>
        type;
  };
...
template <typename NestedT> struct comment_parser_gen {
  ...
  template <typename StartT> struct paren_op1_result_type {
    typedef confix_parser<
  ...
  template <typename StartT, typename EndT> struct paren_op2_result_type {
    typedef confix_parser<
        typename as_parser<StartT>::type, kleene_star<anychar_parser>,
        typename as_parser<EndT>::type,
        unary_parser_category, // there is no action to re-attach
...
template <typename OpenT, typename CloseT> struct comment_nest_p_result {
