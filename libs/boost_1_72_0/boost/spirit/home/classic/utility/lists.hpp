...
struct list_parser
  ...
  parse(ScannerT const &scan) const {
    return impl::list_parser_type<CategoryT>::parse(scan, *this, item, delim,
                                                    end);
  }
...
struct list_parser_gen
  ...
  list_parser<typename as_parser<ItemT>::type, typename as_parser<DelimT>::type,
              no_list_endtoken,
  ...
  // attached semantic action or is a unary_parser_category type parser and the
  // structure of the resulting list parser should _not_ be refactored during
  // parser construction (see comment above).
