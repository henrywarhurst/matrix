...
#pragma warning(push)
#pragma warning(                                                               \
    disable : 488) // template parameter unused in declaring parameter types,
 ...
 * and not returned to the OS before the queue is destroyed.
 *
 *  \b Policies:
 *  - \ref boost::lockfree::fixed_sized, defaults to \c
 * boost::lockfree::fixed_sized<false> \n Can be used to completely disable
 * dynamic memory allocations during push in order to ensure lockfree behavior.
 * \n If the data structure is configured as fixed-sized, the internal nodes are
 * stored inside an array and they are addressed by array indexing. This limits
 * the possible size of the queue to the number of elements that can be
 * addressed by the index type (usually 2**16-2), but on platforms that lack
 * double-width compare-and-exchange instructions, this is the best way to
 * achieve lock-freedom.
 *
 *  - \ref boost::lockfree::capacity, optional \n
 *    If this template argument is passed to the options, the size of the queue
 * is set at compile-time.\n This option implies \c fixed_sized<true>
 *
 *  - \ref boost::lockfree::allocator, defaults to \c
...
private:
  ...
  static const size_t capacity =
      ...
      1; // the queue uses one dummy node
  ...
  struct BOOST_ALIGNMENT(BOOST_LOCKFREE_CACHELINE_BYTES) node {
    typedef typename detail::select_tagged_handle<
        node, node_based>::tagged_handle_type tagged_node_handle;
...
public:
   ...
   *
   * \warning It only checks, if the queue head and tail nodes and the freelist
   * can be modified in a lock-free manner. On most platforms, the whole
   * implementation is lock-free, if this is true. Using c++0x-style atomics,
  ...
  // @{
  queue(void)
      : head_(tagged_node_handle(0, 0)), tail_(tagged_node_handle(0, 0)),
  ...
  template <typename U>
  explicit queue(typename detail::allocator_rebind_helper<node_allocator,
                                                          U>::type const &alloc)
      : head_(tagged_node_handle(0, 0)), tail_(tagged_node_handle(0, 0)),
        pool(alloc, capacity) {
   ...
   * */
  void reserve_unsafe(size_type n) { pool.template reserve<false>(n); }
   ...
   * \note The result is only accurate, if no other thread modifies the queue.
   ...
   * be lock-free.
   * */
  bool push(T const &t) { return do_push<false>(t); }
   ...
   * operation will fail \throws if memory allocator throws
   * */
  bool bounded_push(T const &t) { return do_push<true>(t); }
    ...
    for (;;) {
      ...
      if (BOOST_LIKELY(tail == tail2)) {
        if (next_ptr == 0) {
          ...
          tagged_node_handle new_tail(pool.get_handle(next_ptr),
                                      tail.get_next_tag());
          tail_.compare_exchange_strong(tail, new_tail);
        }
      }
...
public:
  ...
  bool unsynchronized_push(T const &t) {
    ...
    for (;;) {
      ...
      if (next_ptr == 0) {
        ...
        tail_.store(tagged_node_handle(next_ptr, tail.get_next_tag()),
                    memory_order_relaxed);
    }
  }
   ...
   * convertible to U \post if pop operation is successful, object will be
   * copied to ret. \returns true, if the pop operation is successful, false if
   * queue was empty.
   *
   * \note Thread-safe and non-blocking
   * */
    ...
    for (;;) {
      ...
      if (BOOST_LIKELY(head == head2)) {
        if (pool.get_handle(head) == pool.get_handle(tail)) {
          ...
          tagged_node_handle new_tail(pool.get_handle(next),
                                      tail.get_next_tag());
          tail_.compare_exchange_strong(tail, new_tail);
          ...
          tagged_node_handle new_head(pool.get_handle(next),
                                      head.get_next_tag());
    ...
    for (;;) {
      ...
      if (pool.get_handle(head) == pool.get_handle(tail)) {
        ...
        if (next_ptr == 0)
           ...
           * michael and scott
           *
           * however we reuse the tagged_ptr part for the freelist and clear the
           * next part during node allocation. we can observe a null-pointer
           * here.
           * */
          continue;
        detail::copy_payload(next_ptr->data, ret);
