 ...
 * popping is lock-free, construction/destruction has to be synchronized. It
 * uses a freelist for memory management, freed nodes are pushed to the freelist
 * and not returned to the OS before the stack is destroyed.
 *
 *  \b Policies:
 *
 *  - \c boost::lockfree::fixed_sized<>, defaults to \c
 * boost::lockfree::fixed_sized<false> <br> Can be used to completely disable
 * dynamic memory allocations during push in order to ensure lockfree
 * behavior.<br> If the data structure is configured as fixed-sized, the
 * internal nodes are stored inside an array and they are addressed by array
 * indexing. This limits the possible size of the stack to the number of
 * elements that can be addressed by the index type (usually 2**16-2), but on
 * platforms that lack double-width compare-and-exchange instructions, this is
 * the best way to achieve lock-freedom.
 *
 *  - \c boost::lockfree::capacity<>, optional <br>
 *    If this template argument is passed to the options, the size of the stack
 * is set at compile-time. <br> It this option implies \c fixed_sized<true>
 *
 *  - \c boost::lockfree::allocator<>, defaults to \c
 * boost::lockfree::allocator<std::allocator<void>> <br> Specifies the allocator
 * that is used for the internal freelist
 *
 *  \b Requirements:
...
private:
  ...
  struct node {
    ...
    typedef typename detail::select_tagged_handle<node, node_based>::handle_type
        handle_t;
    handle_t next;
    const T v;
  };
  ...
  BOOST_STATIC_ASSERT(
      (mpl::if_c<has_capacity,
                 mpl::bool_<capacity - 1 <
                            boost::integer_traits<boost::uint16_t>::const_max>,
...
public:
   ...
   *
   * \warning It only checks, if the top stack node and the freelist can be
   * modified in a lock-free manner. On most platforms, the whole implementation
   * is lock-free, if this is true. Using c++0x-style atomics, there is no
   * possibility to provide a completely accurate implementation, because one
   * would need to test every internal node, which is impossible if further
  ...
  template <typename U>
  explicit stack(typename detail::allocator_rebind_helper<node_allocator,
                                                          U>::type const &alloc)
      : pool(alloc, capacity) {
  ...
  template <typename U>
  stack(size_type n,
        typename detail::allocator_rebind_helper<node_allocator, U>::type const
            &alloc)
      : pool(alloc, n) {
    BOOST_STATIC_ASSERT(!has_capacity);
    initialize();
...
private:
  ...
  tuple<node *, node *> prepare_node_list(ConstIterator begin,
                                          ConstIterator end,
                                          ConstIterator &ret) {
...
public:
   ...
   *
   * \note Thread-safe. If internal memory pool is exhausted and the memory pool
   * is not fixed-sized, a new node will be allocated from the OS. This may not
   * be lock-free. \throws if memory allocator throws
   * */
  bool push(T const &v) { return do_push<false>(v); }
   ...
   * \returns true, if the push operation is successful.
   *
   * \note Thread-safe and non-blocking. If internal memory pool is exhausted,
   * the push operation will fail
   * */
  bool bounded_push(T const &v) { return do_push<true>(v); }
...
public:
  ...
  /** Pushes as many objects from the range [begin, end) as freelist node can be
   * allocated.
   *
   ...
   * \note Operation is applied atomically
   * \note Thread-safe and non-blocking. If internal memory pool is exhausted,
   * the push operation will fail \throws if memory allocator throws
   */
  template <typename ConstIterator>
   ...
   * \returns true, if the push operation is successful.
   *
   * \note Not thread-safe. If internal memory pool is exhausted and the memory
   * pool is not fixed-sized, a new node will be allocated from the OS. This may
   * not be lock-free. \throws if memory allocator throws
   * */
   ...
   *
   * */
  bool pop(T &ret) { return pop<T>(ret); }
   ...
   *
   * */
  bool unsynchronized_pop(T &ret) { return unsynchronized_pop<T>(ret); }
  ...
  template <typename U> bool unsynchronized_pop(U &ret) {
    ...
    tagged_node_handle new_tos(pool.get_handle(new_tos_ptr),
   ...
   *
   * sequentially pops all elements from the stack and applies the functor on
   * each object
   *
  ...
  /** consumes all elements via a functor
   *
   * atomically pops all elements from the stack and applies the functor on each
   * object
   *
   * \returns number of elements that are consumed
   *
  ...
  /// \copydoc boost::lockfree::stack::consume_all_atomic_reversed(Functor &
  /// rhs)
   ...
   *
   * \note It only guarantees that at some point during the execution of the
   * function the stack has been empty. It is rarely practical to use this value
   * in program logic, because the stack can be modified by other threads.
   * */
  bool empty(void) const { return pool.get_pointer(tos.load()) == NULL; }
...
private:
  ...
  static const int padding_size =
      BOOST_LOCKFREE_CACHELINE_BYTES - sizeof(tagged_node_handle);
