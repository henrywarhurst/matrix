...
public:
  ...
  freelist_stack(Allocator const &alloc, std::size_t n = 0)
      : Alloc(alloc), pool_(tagged_node_ptr(NULL)) {
  ...
  template <bool ThreadSafe> void reserve(std::size_t count) {
  ...
  template <bool ThreadSafe, bool Bounded> T *construct(void) {
  ...
  template <bool ThreadSafe, bool Bounded, typename ArgumentType1,
  ...
  template <bool ThreadSafe> void destruct(T *n) {
    n->~T();
    deallocate<ThreadSafe>(n);
  }
...
protected: // allow use from subclasses
  template <bool ThreadSafe, bool Bounded> T *allocate(void) {
    if (ThreadSafe)
...
private:
  template <bool Bounded> T *allocate_impl(void) {
  ...
  template <bool Bounded> T *allocate_impl_unsafe(void) {
...
protected:
  template <bool ThreadSafe> void deallocate(T *n) {
    if (ThreadSafe)
      deallocate_impl(n);
    else
...
public:
  ...
  void set_index(index_t i) { index = i; }
  /* @} */
  ...
  bool operator!=(tagged_index const &rhs) const { return !operator==(rhs); }
...
struct compiletime_sized_freelist_storage {
  ...
  template <typename Allocator>
  compiletime_sized_freelist_storage(Allocator const & /* alloc */,
                                     std::size_t /* count */) {}

  T *nodes(void) const {
    char *data_pointer = const_cast<char *>(data.data());
    return reinterpret_cast<T *>(boost::alignment::align_up(
        data_pointer, BOOST_LOCKFREE_CACHELINE_BYTES));
  }

  std::size_t node_count(void) const { return size; }
};

template <typename T, typename Alloc = std::allocator<T>>
struct runtime_sized_freelist_storage
    : boost::alignment::aligned_allocator_adaptor<
          Alloc, BOOST_LOCKFREE_CACHELINE_BYTES> {
  typedef boost::alignment::aligned_allocator_adaptor<
      Alloc, BOOST_LOCKFREE_CACHELINE_BYTES>
      allocator_type;
  T *nodes_;
  ...
  runtime_sized_freelist_storage(Allocator const &alloc, std::size_t count)
    ...
    if (count > 65535)
      boost::throw_exception(
          std::runtime_error("boost.lockfree: freelist size is limited to a "
                             "maximum of 65535 objects"));
  ...
  T *nodes(void) const { return nodes_; }

  std::size_t node_count(void) const { return node_count_; }
};
...
public:
  ...
  template <bool ThreadSafe> void destruct(tagged_node_handle tagged_index) {
  ...
  T *get_pointer(T *ptr) const { return ptr; }

protected: // allow use from subclasses
  template <bool ThreadSafe> index_t allocate(void) {
    if (ThreadSafe)
...
private:
  ...
  template <bool ThreadSafe> void deallocate(index_t index) {
    if (ThreadSafe)
...
struct select_freelist {
  typedef typename mpl::if_c<
      IsCompileTimeSized, compiletime_sized_freelist_storage<T, Capacity>,
...
template <typename T, bool IsNodeBased> struct select_tagged_handle {
  typedef typename mpl::if_c<IsNodeBased, tagged_ptr<T>, tagged_index>::type
      tagged_handle_type;

  typedef
      typename mpl::if_c<IsNodeBased, T *, typename tagged_index::index_t>::type
          handle_type;
};
