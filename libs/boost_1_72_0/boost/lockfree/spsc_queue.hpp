...
template <typename T> class ringbuffer_base {
...
protected:
  ...
  char padding1[padding_size]; /* force read_index and write_index to different
                                  cache lines */
...
protected:
  ...
  size_t push(const T *input_buffer, size_t input_count, T *internal_buffer,
              size_t max_size) {
    return push(input_buffer, input_buffer + input_count, internal_buffer,
                max_size) -
           input_buffer;
  }

  template <typename ConstIterator>
  ConstIterator push(ConstIterator begin, ConstIterator end, T *internal_buffer,
                     size_t max_size) {
  ...
  bool consume_one(Functor &functor, T *buffer, size_t max_size) {
    ...
    const size_t read_index =
        read_index_.load(memory_order_relaxed); // only written from pop thread
  ...
  template <typename Functor>
  size_t consume_all(Functor const &functor, T *internal_buffer,
                     size_t max_size) {
    const size_t write_index = write_index_.load(memory_order_acquire);
    const size_t read_index =
        read_index_.load(memory_order_relaxed); // only written from pop thread
    ...
    if (read_index + output_count > max_size) {
      ...
      run_functor_and_delete(internal_buffer + read_index,
                             internal_buffer + max_size, functor);
      run_functor_and_delete(internal_buffer, internal_buffer + count1,
                             functor);

      new_read_index -= max_size;
    } else {
      run_functor_and_delete(internal_buffer + read_index,
                             internal_buffer + read_index + output_count,
                             functor);
  ...
  size_t pop(T *output_buffer, size_t output_count, T *internal_buffer,
             size_t max_size) {
    const size_t write_index = write_index_.load(memory_order_acquire);
    const size_t read_index =
        read_index_.load(memory_order_relaxed); // only written from pop thread
    ...
    if (read_index + output_count > max_size) {
      ...
      copy_and_delete(internal_buffer + read_index, internal_buffer + max_size,
                      output_buffer);
      copy_and_delete(internal_buffer, internal_buffer + count1,
                      output_buffer + count0);

      new_read_index -= max_size;
    } else {
      copy_and_delete(internal_buffer + read_index,
                      internal_buffer + read_index + output_count,
                      output_buffer);
      if (new_read_index == max_size)
        new_read_index = 0;
  ...
  size_t pop_to_output_iterator(OutputIterator it, T *internal_buffer,
                                size_t max_size) {
    const size_t write_index = write_index_.load(memory_order_acquire);
    const size_t read_index =
        read_index_.load(memory_order_relaxed); // only written from pop thread
    ...
    if (read_index + avail > max_size) {
      ...
      it = copy_and_delete(internal_buffer + read_index,
                           internal_buffer + max_size, it);
...
class compile_time_sized_ringbuffer : public ringbuffer_base<T> {
  ...
  typedef typename boost::aligned_storage<
      max_size * sizeof(T), boost::alignment_of<T>::value>::type storage_type;

  storage_type storage_;

  T *data() { return static_cast<T *>(storage_.address()); }

  const T *data() const { return static_cast<const T *>(storage_.address()); }

protected:
  size_type max_number_of_elements() const { return max_size; }
...
public:
  ...
  template <size_type size> size_type push(T const (&t)[size]) {
    return push(t, size);
  }
  ...
  const T &front(void) const { return ringbuffer_base<T>::front(data()); }

  T &front(void) { return ringbuffer_base<T>::front(data()); }
};
...
protected:
  size_type max_number_of_elements() const { return max_elements_; }
...
public:
  ...
  runtime_sized_ringbuffer(
      typename detail::allocator_rebind_helper<Alloc, U>::type const &alloc,
  ...
  template <typename OutputIterator>
  size_type pop_to_output_iterator(OutputIterator it) {
    return ringbuffer_base<T>::pop_to_output_iterator(it, &*array_,
                                                      max_elements_);
  }

  const T &front(void) const { return ringbuffer_base<T>::front(&*array_); }

  T &front(void) { return ringbuffer_base<T>::front(&*array_); }
};
...
struct make_ringbuffer {
  ...
  typedef
      typename mpl::if_c<runtime_sized, runtime_sized_ringbuffer<T, allocator>,
                         compile_time_sized_ringbuffer<T, capacity>>::type
          ringbuffer_type;
};
 ...
 *    If this template argument is passed to the options, the size of the
 * ringbuffer is set at compile-time.
 *
 *  - \c boost::lockfree::allocator<>, defaults to \c
 * boost::lockfree::allocator<std::allocator<T>> <br> Specifies the allocator
 * that is used to allocate the ringbuffer. This option is only valid, if the
 * ringbuffer is configured to be sized at run-time
 *
 *  \b Requirements:
...
#ifdef BOOST_NO_CXX11_VARIADIC_TEMPLATES
  typedef
      typename detail::make_ringbuffer<T, A0, A1>::ringbuffer_type base_type;
  static const bool runtime_sized =
      detail::make_ringbuffer<T, A0, A1>::runtime_sized;
  typedef typename detail::make_ringbuffer<T, A0, A1>::allocator allocator_arg;
#else
  typedef typename detail::make_ringbuffer<T, Options...>::ringbuffer_type
      base_type;
  static const bool runtime_sized =
      detail::make_ringbuffer<T, Options...>::runtime_sized;
  typedef
      typename detail::make_ringbuffer<T, Options...>::allocator allocator_arg;
#endif
...
public:
   ...
   */
  // @{
  spsc_queue(void) { BOOST_ASSERT(!runtime_sized); }

  template <typename U>
  explicit spsc_queue(
      typename detail::allocator_rebind_helper<allocator, U>::type const &) {
  ...
  spsc_queue(
      ...
      typename detail::allocator_rebind_helper<allocator, U>::type const &alloc)
   ...
   * */
  bool push(T const &t) { return base_type::push(t); }
   ...
   * \return true, if the pop operation is successful, false if ringbuffer was
   * empty.
   *
   * \note Thread-safe and wait-free
   */
   ...
   * \note Thread-safe and wait-free
   * */
  size_type pop(T *ret, size_type size) { return base_type::pop(ret, size); }
   ...
   * \note Thread-safe and wait-free
   * */
  template <size_type size> size_type pop(T (&ret)[size]) {
    return pop(ret, size);
  }
   ...
   * */
  template <typename OutputIterator>
  typename boost::disable_if<typename is_convertible<T, OutputIterator>::type,
                             size_type>::type
   ...
   *
   * sequentially pops all elements from the queue and applies the functor on
   * each object
   *
   ...
   *
   * \note Thread-safe and wait-free, should only be called from the consumer
   * thread
   * */
   ...
   *
   * \note Thread-safe and wait-free, should only be called from the producer
   * thread
   * */
   ...
   * \pre read_available() > 0. If ringbuffer is empty, it's undefined behaviour
   * to invoke this method. \return reference to the first element in the queue
   *
   * \note Thread-safe and wait-free
   */
  const T &front() const {
