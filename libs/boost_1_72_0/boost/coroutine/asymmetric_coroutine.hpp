...
template <typename R> class pull_coroutine;

template <typename Arg> class push_coroutine {
private:
...
public:
  push_coroutine() BOOST_NOEXCEPT : impl_(0) {}

#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
#ifdef BOOST_MSVC
  typedef void (*coroutine_fn)(pull_coroutine<Arg> &);

  explicit push_coroutine(coroutine_fn, attributes const & = attributes());

  template <typename StackAllocator>
  explicit push_coroutine(coroutine_fn, attributes const &, StackAllocator);
#endif
  template <typename Fn>
  explicit push_coroutine(BOOST_RV_REF(Fn), attributes const & = attributes());

  template <typename Fn, typename StackAllocator>
  explicit push_coroutine(BOOST_RV_REF(Fn), attributes const &, StackAllocator);
#else
  template <typename Fn>
  explicit push_coroutine(Fn fn, attributes const & = attributes());

  template <typename Fn, typename StackAllocator>
  explicit push_coroutine(Fn fn, attributes const &, StackAllocator);
  ...
  push_coroutine(BOOST_RV_REF(push_coroutine) other) BOOST_NOEXCEPT : impl_(0) {
    swap(other);
  }
  ...
  public:
    ...
    explicit iterator(push_coroutine<Arg> *c) : c_(c) {}
...
template <typename Arg> class push_coroutine<Arg &> {
private:
  template <typename V, typename X, typename Y, typename Z>
  friend class detail::pull_coroutine_object;
...
#else
  template <typename Fn>
  explicit push_coroutine(Fn, attributes const & = attributes());

  template <typename Fn, typename StackAllocator>
  explicit push_coroutine(Fn, attributes const &, StackAllocator);
...
public:
  ...
  inline push_coroutine &operator=(BOOST_RV_REF(push_coroutine)
...
public:
  pull_coroutine() BOOST_NOEXCEPT : impl_(0) {}
  ...
  explicit pull_coroutine(coroutine_fn fn,
    ...
    typedef detail::pull_coroutine_object<push_coroutine<R>, R, coroutine_fn,
                                          stack_allocator>
        object_t;
    ...
    impl_ = new (sp)
        object_t(boost::forward<coroutine_fn>(fn), attrs,
  ...
  explicit pull_coroutine(BOOST_RV_REF(Fn) fn,
    ...
    BOOST_ASSERT(0 != stack_ctx.sp);
    // typedef of internal coroutine-type
    typedef detail::pull_coroutine_object<push_coroutine<R>, R, Fn,
                                          stack_allocator>
        object_t;
    // reserve space on top of coroutine-stack for internal coroutine-type
    std::size_t size = stack_ctx.size - sizeof(object_t);
    ...
    // placement new for internal coroutine
    impl_ = new (sp)
        object_t(boost::forward<Fn>(fn), attrs,
                 detail::preallocated(sp, size, stack_ctx), stack_alloc);
    BOOST_ASSERT(impl_);
    impl_->pull();
  }
  ...
  template <typename Fn, typename StackAllocator>
  explicit pull_coroutine(BOOST_RV_REF(Fn) fn, attributes const &attrs,
                          StackAllocator stack_alloc)
      : impl_(0) {
    // create a stack-context
    stack_context stack_ctx;
...
#else
  template <typename Fn>
  explicit pull_coroutine(Fn fn, attributes const &attrs = attributes())
      : impl_(0) {
    // create a stack-context
    stack_context stack_ctx;
    stack_allocator stack_alloc;
  ...
  template <typename Fn, typename StackAllocator>
  explicit pull_coroutine(Fn fn, attributes const &attrs,
                          StackAllocator stack_alloc)
      : impl_(0) {
    // create a stack-context
    stack_context stack_ctx;
  ...
  pull_coroutine(BOOST_RV_REF(pull_coroutine) other) BOOST_NOEXCEPT : impl_(0) {
    swap(other);
  }
  ...
  public:
    ...
    iterator() : c_(0), val_(0) {}

    explicit iterator(pull_coroutine<R> *c) : c_(c), val_(0) { fetch_(); }

    iterator(iterator const &other) : c_(other.c_), val_(other.val_) {}
    ...
    reference_t operator*() const {
      if (!val_)
        boost::throw_exception(invalid_result());
      return *val_;
    }
  ...
  public:
    ...
    const_iterator() : c_(0), val_(0) {}

    explicit const_iterator(pull_coroutine<R> const *c)
        : c_(const_cast<pull_coroutine<R> *>(c)), val_(0) {
      fetch_();
    }
...
template <typename R> class pull_coroutine<R &> {
private:
  template <typename V, typename X, typename Y, typename Z>
  friend class detail::push_coroutine_object;
...
public:
  ...
  explicit pull_coroutine(coroutine_fn fn,
    ...
    BOOST_ASSERT(0 != stack_ctx.sp);
    // typedef of internal coroutine-type
    typedef detail::pull_coroutine_object<push_coroutine<R &>, R &,
                                          coroutine_fn, stack_allocator>
        object_t;
    // reserve space on top of coroutine-stack for internal coroutine-type
    std::size_t size = stack_ctx.size - sizeof(object_t);
    BOOST_ASSERT(0 != size);
...
public:
  ...
  explicit pull_coroutine(coroutine_fn fn,
    ...
    BOOST_ASSERT(0 != stack_ctx.sp);
    // typedef of internal coroutine-type
    typedef detail::pull_coroutine_object<push_coroutine<void>, void,
                                          coroutine_fn, stack_allocator>
        object_t;
    // reserve space on top of coroutine-stack for internal coroutine-type
    std::size_t size = stack_ctx.size - sizeof(object_t);
  ...
  explicit pull_coroutine(BOOST_RV_REF(Fn) fn,
    ...
    BOOST_ASSERT(0 != stack_ctx.sp);
    // typedef of internal coroutine-type
    typedef detail::pull_coroutine_object<push_coroutine<void>, void, Fn,
                                          stack_allocator>
        object_t;
    // reserve space on top of coroutine-stack for internal coroutine-type
    std::size_t size = stack_ctx.size - sizeof(object_t);
  ...
  inline pull_coroutine &operator=(BOOST_RV_REF(pull_coroutine)
                                       other) BOOST_NOEXCEPT {
    pull_coroutine tmp(boost::move(other));
    swap(tmp);
    return *this;
...
push_coroutine<Arg>::push_coroutine(coroutine_fn fn, attributes const &attrs)
    : impl_(0) {
  // create a stack-context
  stack_context stack_ctx;
  stack_allocator stack_alloc;
  ...
  BOOST_ASSERT(0 != stack_ctx.sp);
  // typedef of internal coroutine-type
  typedef detail::push_coroutine_object<pull_coroutine<Arg>, Arg, coroutine_fn,
                                        stack_allocator>
      object_t;
  // reserve space on top of coroutine-stack for internal coroutine-type
  std::size_t size = stack_ctx.size - sizeof(object_t);
  ...
  // placement new for internal coroutine
  impl_ =
      new (sp) object_t(boost::forward<coroutine_fn>(fn), attrs,
                        detail::preallocated(sp, size, stack_ctx), stack_alloc);
  BOOST_ASSERT(impl_);
}
...
push_coroutine<Arg &>::push_coroutine(coroutine_fn fn, attributes const &attrs)
  ...
  BOOST_ASSERT(0 != stack_ctx.sp);
  // typedef of internal coroutine-type
  typedef detail::push_coroutine_object<pull_coroutine<Arg &>, Arg &,
                                        coroutine_fn, stack_allocator>
      object_t;
  // reserve space on top of coroutine-stack for internal coroutine-type
  std::size_t size = stack_ctx.size - sizeof(object_t);
...
inline push_coroutine<void>::push_coroutine(coroutine_fn fn,
  ...
  BOOST_ASSERT(0 != stack_ctx.sp);
  // typedef of internal coroutine-type
  typedef detail::push_coroutine_object<pull_coroutine<void>, void,
                                        coroutine_fn, stack_allocator>
      object_t;
  // reserve space on top of coroutine-stack for internal coroutine-type
  std::size_t size = stack_ctx.size - sizeof(object_t);
...
template <typename StackAllocator>
push_coroutine<void>::push_coroutine(coroutine_fn fn, attributes const &attrs,
                                     StackAllocator stack_alloc)
    : impl_(0) {
  // create a stack-context
  stack_context stack_ctx;
...
push_coroutine<Arg>::push_coroutine(BOOST_RV_REF(Fn) fn,
  ...
  BOOST_ASSERT(0 != stack_ctx.sp);
  // typedef of internal coroutine-type
  typedef detail::push_coroutine_object<pull_coroutine<Arg>, Arg, Fn,
                                        stack_allocator>
      object_t;
  // reserve space on top of coroutine-stack for internal coroutine-type
  std::size_t size = stack_ctx.size - sizeof(object_t);
  ...
  // placement new for internal coroutine
  impl_ =
      new (sp) object_t(boost::forward<Fn>(fn), attrs,
                        detail::preallocated(sp, size, stack_ctx), stack_alloc);
  BOOST_ASSERT(impl_);
}
...
push_coroutine<void>::push_coroutine(BOOST_RV_REF(Fn) fn,
  ...
  BOOST_ASSERT(0 != stack_ctx.sp);
  // typedef of internal coroutine-type
  typedef detail::push_coroutine_object<pull_coroutine<void>, void, Fn,
                                        stack_allocator>
      object_t;
  // reserve space on top of coroutine-stack for internal coroutine-type
  std::size_t size = stack_ctx.size - sizeof(object_t);
...
template <typename Arg>
template <typename Fn>
push_coroutine<Arg>::push_coroutine(Fn fn, attributes const &attrs) : impl_(0) {
  // create a stack-context
  stack_context stack_ctx;
  stack_allocator stack_alloc;
  // allocate the coroutine-stack
...
template <typename Arg>
template <typename Fn, typename StackAllocator>
push_coroutine<Arg>::push_coroutine(Fn fn, attributes const &attrs,
                                    StackAllocator stack_alloc)
    : impl_(0) {
  // create a stack-context
  stack_context stack_ctx;
...
template <typename Fn>
push_coroutine<void>::push_coroutine(Fn fn, attributes const &attrs)
    : impl_(0) {
  // create a stack-context
  stack_context stack_ctx;
  stack_allocator stack_alloc;
...
template <typename R>
typename pull_coroutine<R>::iterator range_begin(pull_coroutine<R> &c) {
...
template <typename R>
typename pull_coroutine<R>::iterator range_end(pull_coroutine<R> &) {
...
template <typename Arg>
typename push_coroutine<Arg>::iterator range_begin(push_coroutine<Arg> &c) {
  return typename push_coroutine<Arg>::iterator(&c);
}

template <typename Arg>
typename push_coroutine<Arg>::iterator range_end(push_coroutine<Arg> &) {
  return typename push_coroutine<Arg>::iterator();
}

template <typename T> struct asymmetric_coroutine {
...
template <typename R>
typename pull_coroutine<R>::iterator begin(pull_coroutine<R> &c) {
  return boost::begin(c);
}

template <typename R>
typename pull_coroutine<R>::const_iterator begin(pull_coroutine<R> const &c) {
  return boost::begin(c);
}

template <typename R>
typename pull_coroutine<R>::iterator end(pull_coroutine<R> &c) {
  return boost::end(c);
}

template <typename R>
typename pull_coroutine<R>::const_iterator end(pull_coroutine<R> const &c) {
  return boost::end(c);
}

template <typename R>
typename push_coroutine<R>::iterator begin(push_coroutine<R> &c) {
  return boost::begin(c);
}

template <typename R>
typename push_coroutine<R>::iterator end(push_coroutine<R> &c) {
  return boost::end(c);
}
