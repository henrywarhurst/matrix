...
public:
  ...
  symmetric_coroutine_object(Fn fn, attributes const &attrs,
      ...
      : impl_t(palloc, stack_unwind == attrs.do_unwind),
        fn_(fn),
  ...
  void run(R *r) BOOST_NOEXCEPT {
    ...
    impl_t::callee_.jump(impl_t::caller_, &to);
...
template <typename R, typename Fn, typename StackAllocator>
class symmetric_coroutine_object<R &, Fn, StackAllocator>
    : public symmetric_coroutine_impl<R &> {
private:
  typedef symmetric_coroutine_impl<R &> impl_t;
  typedef symmetric_coroutine_object<R &, Fn, StackAllocator> obj_t;
...
class symmetric_coroutine_object<void, Fn, StackAllocator>
    : public symmetric_coroutine_impl<void> {
private:
