...
template <typename R> class symmetric_coroutine_yield {
private:
...
public:
  symmetric_coroutine_yield() BOOST_NOEXCEPT : impl_(0), result_(0) {}
  ...
  symmetric_coroutine_yield &operator=(BOOST_RV_REF(symmetric_coroutine_yield)
  ...
  bool operator!() const BOOST_NOEXCEPT { return 0 == impl_; }
  ...
  operator()(Coro &other, typename Coro::value_type x,
             typename disable_if<is_same<typename Coro::value_type, void>,
                                 dummy *>::type = 0) {
    BOOST_ASSERT(other);
  ...
  symmetric_coroutine_yield &
  operator()(Coro &other,
             typename enable_if<is_same<typename Coro::value_type, void>,
                                dummy *>::type = 0) {
    BOOST_ASSERT(other);
...
template <typename R> class symmetric_coroutine_yield<R &> {
private:
  template <typename X, typename Y, typename Z>
  friend class symmetric_coroutine_object;
...
template <> class symmetric_coroutine_yield<void> {
private:
  template <typename X, typename Y, typename Z>
  friend class symmetric_coroutine_object;
  ...
  symmetric_coroutine_yield(impl_type *impl) BOOST_NOEXCEPT : impl_(impl) {
    BOOST_ASSERT(0 != impl_);
  }

public:
  symmetric_coroutine_yield() BOOST_NOEXCEPT : impl_(0) {}
  ...
  inline bool operator!() const BOOST_NOEXCEPT { return 0 == impl_; }
