...
template <typename R> class symmetric_coroutine_impl : private noncopyable {
public:
  ...
  R *yield() {
    ...
    flags_ &= ~flag_running;
    param_type to;
    param_type *from(static_cast<param_type *>(callee_.jump(caller_, &to)));
    flags_ |= flag_running;
    if (from->do_unwind)
      throw forced_unwind();
  ...
  template <typename X> R *yield_to(symmetric_coroutine_impl<X> *other, X x) {
  ...
  template <typename X> R *yield_to(symmetric_coroutine_impl<X> *other) {
...
protected:
  template <typename X> friend class symmetric_coroutine_impl;
  ...
  R *yield_to_(Other *other, typename Other::param_type *to) {
    ...
    flags_ &= ~flag_running;
    param_type *from(
        static_cast<param_type *>(callee_.jump(other->callee_, to)));
    flags_ |= flag_running;
    if (from->do_unwind)
      throw forced_unwind();
    BOOST_ASSERT(from->data);
...
template <> class symmetric_coroutine_impl<void> : private noncopyable {
public:
  ...
  template <typename X> void yield_to(symmetric_coroutine_impl<X> *other, X x) {
    typename symmetric_coroutine_impl<X>::param_type to(&x, other);
    yield_to_(other, &to);
  }
  ...
  template <typename X> void yield_to(symmetric_coroutine_impl<X> *other) {
    typename symmetric_coroutine_impl<X>::param_type to(other);
    yield_to_(other, &to);
  }
