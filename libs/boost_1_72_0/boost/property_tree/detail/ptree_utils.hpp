...
template <class T> struct less_nocase {
...
template <typename Ch> struct is_character : public boost::false_type {};
template <> struct is_character<char> : public boost::true_type {};
template <> struct is_character<wchar_t> : public boost::true_type {};
...
template <typename Str> Str widen(const char *text) {
  Str result;
  while (*text) {
...
template <typename Str, typename char_type> Str narrow(const char_type *text) {
  Str result;
  while (*text) {
