...
void write_xml_indent(std::basic_ostream<typename Str::value_type> &stream,
                      int indent, const xml_writer_settings<Str> &settings) {
  stream << std::basic_string<typename Str::value_type>(
      indent * settings.indent_count, settings.indent_char);
}

template <class Str>
void write_xml_comment(std::basic_ostream<typename Str::value_type> &stream,
                       const Str &s, int indent, bool separate_line,
...
void write_xml_element(
    ...
    const typename Ptree::key_type &key, const Ptree &pt, int indent,
  ...
  } else // Nonempty key
  {
    ...
    if (indent >= 0) {
      ...
      // Write attributes
      if (optional<const Ptree &> attribs =
              pt.get_child_optional(xmlattr<Str>()))
        for (It it = attribs.get().begin(); it != attribs.get().end(); ++it)
          stream << Ch(' ') << it->first << Ch('=') << Ch('"')
                 << encode_char_entities(it->second.template get_value<Str>())
                 << Ch('"');
    ...
    if (!pt.data().empty())
      write_xml_text(stream, pt.template get_value<Str>(), indent + 1,
                     has_elements && want_pretty, settings);
    ...
    for (It it = pt.begin(); it != pt.end(); ++it) {
      if (it->first == xmlattr<Str>())
        ...
        write_xml_comment(stream, it->second.template get_value<Str>(),
                          indent + 1, want_pretty, settings);
      else if (it->first == xmltext<Str>())
        write_xml_text(stream, it->second.template get_value<Str>(), indent + 1,
                       has_elements && want_pretty, settings);
      else
...
void write_xml_internal(
  ...
  stream << detail::widen<Str>("<?xml version=\"1.0\" encoding=\"")
         << settings.encoding << detail::widen<Str>("\"?>\n");
