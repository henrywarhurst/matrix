...
template <class K, class D, class C> struct basic_ptree<K, D, C>::subs {
  struct by_name {};
  ...
  typedef multi_index_container<
      value_type,
      multi_index::indexed_by<multi_index::sequenced<>,
...
template <class K, class D, class C> basic_ptree<K, D, C>::~basic_ptree() {
  delete &subs::ch(this);
}
...
template <class K, class D, class C>
inline typename basic_ptree<K, D, C>::value_type &
...
namespace impl {
struct by_first {
  template <typename P> bool operator()(const P &lhs, const P &rhs) const {
    return lhs.first < rhs.first;
  }
};

template <typename C> struct equal_pred {
  template <typename P> bool operator()(const P &lhs, const P &rhs) const {
    C c;
...
template <class K, class D, class C> inline void basic_ptree<K, D, C>::sort() {
  sort(impl::by_first());
}
...
template <class K, class D, class C>
inline bool
basic_ptree<K, D, C>::operator==(const basic_ptree<K, D, C> &rhs) const {
...
template <class K, class D, class C> inline void basic_ptree<K, D, C>::clear() {
  m_data = data_type();
  subs::ch(this).clear();
}
...
basic_ptree<K, D, C> &basic_ptree<K, D, C>::force_path(path_type &p) {
  ...
  self_type &child = el == not_found()
                         ? push_back(value_type(fragment, self_type()))->second
                         : el->second;
