...
inline std::string dump_sequence(const std::string &s) { return s; }
...
public:
  ...
  string_path &operator/=(const string_path &o) {
    ...
    BOOST_ASSERT((m_separator == o.m_separator || o.empty() || o.single()) &&
                 "Incompatible paths.");
    if (!o.empty()) {
      String sub;
      if (!this->empty()) {
...
inline string_path<String, Translator>::string_path(const String &value,
                                                    char_type separator,
                                                    Translator tr)
    : m_value(value), m_separator(separator), m_tr(tr),
