...
void merge_blocks<Block_size, Group_size, Iter_t, Compare>::tail_process(
  ...
  if (vblkpos1.size() == 0 or vblkpos2.size() == 0)
    return;
...
//-------------------------------------------------------------------------
template <uint32_t Block_size, uint32_t Group_size, class Iter_t, class Compare>
void merge_blocks<Block_size, Group_size, Iter_t, Compare>::cut_range(
    range_pos rng_input) {
...
//-------------------------------------------------------------------------
template <uint32_t Block_size, uint32_t Group_size, class Iter_t, class Compare>
void merge_blocks<Block_size, Group_size, Iter_t, Compare>::merge_range_pos(
    range_pos rng_input) {
  if (rng_input.size() < 2)
    return;
...
void merge_blocks<Block_size, Group_size, Iter_t, Compare>::extract_ranges(
  ...
  for (size_t posx = posx_ini + 1; posx <= range_input.last; ++posx) {
    ...
    if (not final) {
      ...
      mergeable =
          (side_max != side_posx and is_mergeable(rng_max, rng_posx, bk.cmp));
    };
    if (bk.error)
      return;
