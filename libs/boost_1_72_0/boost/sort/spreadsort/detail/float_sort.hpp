...
inline bool is_sorted_or_find_extremes(RandomAccessIter current,
                                       RandomAccessIter last, Div_type &max,
                                       Div_type &min, Right_shift rshift) {
...
inline void inner_float_swap_loop(RandomAccessIter *bins,
                                  ...
                                  unsigned ii, const unsigned log_divisor,
  ...
  for (RandomAccessIter current = *local_bin; current < nextbinstart;
    ...
    for (RandomAccessIter *target_bin =
      ...
      RandomAccessIter *b_bin =
          bins +
          ((cast_float_iter<Div_type, RandomAccessIter>(b) >> log_divisor) -
           div_min);
...
inline void float_swap_loop(RandomAccessIter *bins,
                            ...
                            const size_t *bin_sizes, const unsigned log_divisor,
                            const Div_type div_min) {
  nextbinstart += bin_sizes[ii];
  inner_float_swap_loop<RandomAccessIter, Div_type>(bins, nextbinstart, ii,
                                                    log_divisor, div_min);
}
...
inline bool is_sorted_or_find_extremes(RandomAccessIter current,
                                       RandomAccessIter last, Cast_type &max,
                                       Cast_type &min) {
...
inline void positive_float_sort_rec(RandomAccessIter first,
                                    RandomAccessIter last,
                                    std::vector<RandomAccessIter> &bin_cache,
                                    unsigned cache_offset, size_t *bin_sizes) {
  Div_type max, min;
  if (is_sorted_or_find_extremes<RandomAccessIter, Div_type>(first, last, max,
                                                             min))
    return;
  ...
  for (RandomAccessIter current = first; current != last;)
    bin_sizes[unsigned(
        (cast_float_iter<Div_type, RandomAccessIter>(current++) >>
         log_divisor) -
        div_min)]++;
  bins[0] = first;
  ...
  // Recursing
  size_t max_count =
      get_min_count<float_log_mean_bin_size, float_log_min_split_count,
...
template <class RandomAccessIter, class Div_type, class Size_type>
inline void negative_float_sort_rec(RandomAccessIter first,
                                    RandomAccessIter last,
                                    std::vector<RandomAccessIter> &bin_cache,
                                    unsigned cache_offset, size_t *bin_sizes) {
  Div_type max, min;
  ...
  for (int ii = cache_end - 1; ii >= static_cast<int>(cache_offset);
    ...
    if (count < max_count)
      ...
      negative_float_sort_rec<RandomAccessIter, Div_type, Right_shift,
                              Size_type>(lastPos, bin_cache[ii], bin_cache,
...
inline void negative_float_sort_rec(RandomAccessIter first,
  ...
  for (int ii = cache_end - 1; ii >= static_cast<int>(cache_offset);
    ...
    if (count < max_count)
      ...
      negative_float_sort_rec<RandomAccessIter, Div_type, Right_shift, Compare,
                              Size_type>(lastPos, bin_cache[ii], bin_cache,
...
template <class RandomAccessIter, class Div_type, class Size_type>
inline void float_sort_rec(RandomAccessIter first, RandomAccessIter last,
                           std::vector<RandomAccessIter> &bin_cache,
                           unsigned cache_offset, size_t *bin_sizes) {
  Div_type max, min;
  if (is_sorted_or_find_extremes<RandomAccessIter, Div_type>(first, last, max,
  ...
  for (unsigned u = 0; u < bin_count; ++u) {
    ...
    inner_float_swap_loop<RandomAccessIter, Div_type>(bins, nextbinstart, u,
                                                      log_divisor, div_min);
  }
  ...
  for (int ii = cache_offset + first_positive - 1;
       ii >= static_cast<int>(cache_offset); lastPos = bin_cache[ii--]) {
    size_t count = bin_cache[ii] - lastPos;
    if (count < 2)
      continue;
    if (count < max_count)
...
template <class RandomAccessIter, class Div_type, class Right_shift>
inline
    typename boost::enable_if_c<sizeof(size_t) >= sizeof(Div_type), void>::type
...
template <class RandomAccessIter, class Div_type, class Right_shift>
inline
    typename boost::disable_if_c<sizeof(boost::uintmax_t) >= sizeof(Div_type),
                                 void>::type
    float_sort(RandomAccessIter first, RandomAccessIter last, Div_type,
               Right_shift rshift) {
...
float_sort(RandomAccessIter first, RandomAccessIter last, Div_type,
  ...
  float_sort_rec<RandomAccessIter, Div_type, Right_shift, Compare,
                 boost::uintmax_t>(first, last, bin_cache, 0, bin_sizes, rshift,
                                   comp);
}
