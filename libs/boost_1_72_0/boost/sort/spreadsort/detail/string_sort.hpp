...
inline void update_offset(RandomAccessIter first, RandomAccessIter finish,
                          size_t &char_offset) {
...
inline void update_offset(RandomAccessIter first, RandomAccessIter finish,
                          size_t &char_offset, Get_char get_character,
                          Get_length length) {
  size_t nextOffset = char_offset;
  while (true) {
    RandomAccessIter curr = first;
    do {
      ...
      if (length(*curr) > char_offset &&
          (length(*curr) <= (nextOffset + 1) ||
           get_character((*curr), nextOffset) !=
               get_character((*first), nextOffset))) {
        char_offset = nextOffset;
        return;
      }
    } while (++curr != finish);
...
template <class Data_type, class Unsigned_char_type> struct offset_less_than {
...
template <class RandomAccessIter, class Unsigned_char_type>
inline void string_sort_rec(RandomAccessIter first, RandomAccessIter last,
                            size_t char_offset,
  ...
  RandomAccessIter *bins =
      size_bins(bin_sizes, bin_cache, cache_offset, cache_end, membin_count) +
      1;
...
template <class RandomAccessIter, class Unsigned_char_type>
inline void reverse_string_sort_rec(RandomAccessIter first,
  ...
  for (RandomAccessIter current = first; current != last; ++current) {
    if ((*current).size() <= char_offset) {
      ...
      bin_sizes[max_bin -
                static_cast<Unsigned_char_type>((*current)[char_offset])]++;
  }
  // Assign the bin positions
  bin_cache[cache_offset] = first;
  for (unsigned u = 0; u < membin_count - 1; u++)
  ...
  *local_bin = next_bin_start;
  ...
  for (unsigned u = 0; u < last_bin; ++u) {
    ...
    for (RandomAccessIter current = *local_bin; current < next_bin_start;
         ++current) {
      // Swapping into place until the correct element has been swapped in
      for (target_bin = end_bin - static_cast<Unsigned_char_type>(
                                      (*current)[char_offset]);
           target_bin != local_bin;
           target_bin = end_bin - static_cast<Unsigned_char_type>(
                                      (*current)[char_offset]))
        iter_swap(current, (*target_bin)++);
    }
    *local_bin = next_bin_start;
  ...
  *local_bin = next_bin_start;
  ...
  for (unsigned u = cache_offset + 1; u < cache_offset + last_bin + 2;
    ...
    if (count < max_size)
      boost::sort::pdqsort(
          lastPos, bin_cache[u],
          offset_char_less_than<Data_type, Get_char, Get_length>(char_offset +
                                                                 1));
    else
      string_sort_rec<RandomAccessIter, Unsigned_char_type, Get_char,
                      Get_length>(lastPos, bin_cache[u], char_offset + 1,
                                  bin_cache, cache_end, bin_sizes,
                                  get_character, length);
  }
}
...
string_sort(RandomAccessIter first, RandomAccessIter last, Unsigned_char_type) {
  // Warning that we're using boost::sort::pdqsort, even though string_sort was
  // called
...
string_sort(RandomAccessIter first, RandomAccessIter last,
  ...
  string_sort_rec<RandomAccessIter, Unsigned_char_type, Get_char, Get_length>(
      first, last, 0, bin_cache, 0, bin_sizes, get_character, length);
}
...
string_sort(RandomAccessIter first, RandomAccessIter last,
  ...
  std::vector<RandomAccessIter> bin_cache;
  string_sort_rec<RandomAccessIter, Unsigned_char_type, Get_char, Get_length,
                  Compare>(first, last, 0, bin_cache, 0, bin_sizes,
                           get_character, length, comp);
}
