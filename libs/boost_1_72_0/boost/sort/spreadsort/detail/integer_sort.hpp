...
inline bool is_sorted_or_find_extremes(RandomAccessIter current,
                                       ...
                                       RandomAccessIter &min, Compare comp) {
  min = max = current;
...
inline void spreadsort_rec(RandomAccessIter first, RandomAccessIter last,
...
inline void inner_swap_loop(RandomAccessIter *bins,
                            ...
                            Right_shift &rshift, const unsigned log_divisor,
...
template <class RandomAccessIter, class Div_type, class Right_shift>
inline void swap_loop(RandomAccessIter *bins, RandomAccessIter &next_bin_start,
                      unsigned ii, Right_shift &rshift, const size_t *bin_sizes,
                      const unsigned log_divisor, const Div_type div_min) {
  next_bin_start += bin_sizes[ii];
  inner_swap_loop<RandomAccessIter, Div_type, Right_shift>(
      bins, next_bin_start, ii, rshift, log_divisor, div_min);
}
...
inline void spreadsort_rec(RandomAccessIter first, RandomAccessIter last,
  ...
  for (unsigned u = 0; u < bin_count - 1; ++u)
    swap_loop<RandomAccessIter, Div_type, Right_shift>(
        bins, next_bin_start, u, rshift, bin_sizes, log_divisor, div_min);
  bins[bin_count - 1] = last;
  ...
  for (unsigned u = cache_offset; u < cache_end; lastPos = bin_cache[u], ++u) {
    ...
    if (count < max_count)
      ...
      spreadsort_rec<RandomAccessIter, Div_type, Right_shift, Compare,
                     Size_type, log_mean_bin_size, log_min_split_count,
                     log_finishing_count>(lastPos, bin_cache[u], bin_cache,
...
inline
    typename boost::enable_if_c<sizeof(Div_type) <= sizeof(size_t), void>::type
    integer_sort(RandomAccessIter first, RandomAccessIter last, Div_type) {
  size_t bin_sizes[1 << max_finishing_splits];
  std::vector<RandomAccessIter> bin_cache;
  spreadsort_rec<RandomAccessIter, Div_type, size_t>(first, last, bin_cache, 0,
                                                     bin_sizes);
}
...
integer_sort(RandomAccessIter first, RandomAccessIter last, Div_type) {
  size_t bin_sizes[1 << max_finishing_splits];
  std::vector<RandomAccessIter> bin_cache;
  spreadsort_rec<RandomAccessIter, Div_type, boost::uintmax_t>(
...
integer_sort(RandomAccessIter first, RandomAccessIter last, Div_type) {
  // Warning that we're using boost::sort::pdqsort, even though integer_sort was
  // called
...
inline
  ...
  size_t bin_sizes[1 << max_finishing_splits];
  std::vector<RandomAccessIter> bin_cache;
  spreadsort_rec<RandomAccessIter, Div_type, Right_shift, Compare, size_t,
                 int_log_mean_bin_size, int_log_min_split_count,
                 int_log_finishing_count>(first, last, bin_cache, 0, bin_sizes,
                                          shift, comp);
}
...
integer_sort(RandomAccessIter first, RandomAccessIter last, Div_type,
  ...
  size_t bin_sizes[1 << max_finishing_splits];
  std::vector<RandomAccessIter> bin_cache;
  spreadsort_rec<RandomAccessIter, Div_type, Right_shift, boost::uintmax_t,
                 int_log_mean_bin_size, int_log_min_split_count,
                 int_log_finishing_count>(first, last, bin_cache, 0, bin_sizes,
                                          shift);
}
