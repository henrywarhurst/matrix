...
template <typename T> inline unsigned rough_log_2_size(const T &input) {
  unsigned result = 0;
...
size_bins(size_t *bin_sizes, std::vector<RandomAccessIter> &bin_cache,
          unsigned cache_offset, unsigned &cache_end, unsigned bin_count) {
  // Clear the bin sizes
