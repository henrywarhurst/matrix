...
*/
template <class Range> inline void float_sort(Range &range) {
  float_sort(boost::begin(range), boost::end(range));
}

/*!
  \brief Floating-point sort algorithm using random access iterators with just
  right-shift functor.

  \param[in] first Iterator pointer to first element.
  \param[in] last Iterator pointing to one beyond the end of data.
  \param[in] rshift Functor that returns the result of shifting the value_type
...
/*!
 \brief Float sort algorithm using random access iterators with both right-shift
 and user-defined comparison operator.
...
/*!
\brief Float sort algorithm using range with both right-shift and user-defined
comparison operator.
