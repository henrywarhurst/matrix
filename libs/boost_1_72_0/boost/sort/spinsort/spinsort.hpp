...
static bool check_stable_sort(const range<Iter1_t> &rng_data,
  ...
  Iter1_t alpha(rng_data.first), beta(it2 - 1),
      mid(rng_data.first + (nreverse >> 1));
  while (alpha != mid)
    std::swap(*(alpha++), *(beta--));
...
static void range_sort(const range<Iter1_t> &range1,
  ...
  if (level < 2) {
    ...
    range_sort(range_it2(range2.first, range2.first + nelem1), range_input1,
               comp, level - 1);

    range_sort(range_it2(range2.first + nelem1, range2.last), range_input2,
               comp, level - 1);
  };
...
spinsort<Iter_t, Compare>::spinsort(Iter_t first, Iter_t last, Compare comp,
