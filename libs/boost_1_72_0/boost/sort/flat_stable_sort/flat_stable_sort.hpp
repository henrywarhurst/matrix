...
void flat_stable_sort<Iter_t, Compare, Power2>::divide(it_index itx_first,
                                                       it_index itx_last) {
...
//------------------------------------------------------------------------
template <class Iter_t, typename Compare, uint32_t Power2>
void flat_stable_sort<Iter_t, Compare, Power2>::sort_small(it_index itx_first,
                                                           it_index itx_last) {
  size_t nblock = size_t(itx_last - itx_first);
...
//------------------------------------------------------------------------
template <class Iter_t, typename Compare, uint32_t Power2>
bool flat_stable_sort<Iter_t, Compare, Power2>::is_sorted_forward(
    it_index itx_first, it_index itx_last) {
  size_t nblock = size_t(itx_last - itx_first);
  ...
  size_t nsorted1 =
      bsc::number_stable_sorted_forward(rng.first, rng.last, min_process, cmp);
  if (nsorted1 == nelem)
    return true;
  if (nsorted1 == 0)
    return false;
...
//------------------------------------------------------------------------
template <class Iter_t, typename Compare, uint32_t Power2>
bool flat_stable_sort<Iter_t, Compare, Power2>::is_sorted_backward(
    it_index itx_first, it_index itx_last) {
  size_t nblock = size_t(itx_last - itx_first);
  range_it rng = get_group_range(*itx_first, nblock);
  ...
  size_t nsorted2 =
      bsc::number_stable_sorted_backward(rng.first, rng.last, min_process, cmp);
  if (nsorted2 == nelem)
    return true;
  if (nsorted2 == 0)
    return false;
...
template <size_t Size> struct block_size_fss {
...
inline void indirect_flat_stable_sort(Iter_t first, Iter_t last,
  ...
  common::indirect_sort(flat_stable_sort<itx_iter, itx_comp>, first, last,
                        comp);
};
