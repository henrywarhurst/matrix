...
/// @remarks
//----------------------------------------------------------------------------
template <class Iter_t> struct range {
  Iter_t first, last;
  //
  //------------------------------------------------------------------------
  //  function : range
...
inline range<Iter_t> concat(const range<Iter_t> &it1,
                            const range<Iter_t> &it2) {
...
inline range<Iter2_t> move_backward(const range<Iter2_t> &dest,
  ...
  assert(dest.size() >= src.size());
  Iter2_t it_aux =
      util::move_backward(dest.first + src.size(), src.first, src.last);
...
//-----------------------------------------------------------------------------
template <class Iter_t> inline void destroy(range<Iter_t> rng) {
...
inline range<Value_t *>
merge_construct(const range<Value_t *> &dest, const range<Iter1_t> &src1,
                const range<Iter2_t> &src2, Compare comp) {
...
inline bool merge_uncontiguous(const range<Iter1_t> &src1,
  ...
  return util::merge_uncontiguous(src1.first, src1.last, src2.first, src2.last,
                                  aux.first, comp);
};
//
//-----------------------------------------------------------------------------
...
merge_contiguous(const range<Iter1_t> &src1, const range<Iter1_t> &src2,
...
static void merge_flow(range<Iter1_t> rng1, range<Iter2_t> rbuf,
  ...
  while (rx1.first != rx1.last) {
    *(rx1.first++) = (cmp(*rbx.first, *rx2.first)) ? std::move(*(rbx.first++))
                                                   : std::move(*(rx2.first++));
  };
