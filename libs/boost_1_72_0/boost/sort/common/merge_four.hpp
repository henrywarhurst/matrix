...
range<Iter1_t> full_merge4(const range<Iter1_t> &rdest,
  ...
  // sorting networks
  //-----------------------------------------------------------------------
  if (less_range(vrange_input[pos[1]].first, pos[1], vrange_input[pos[0]].first,
                 pos[0], comp)) {
  ...
  if (less_range(vrange_input[pos[2]].first, pos[2], vrange_input[pos[0]].first,
                 pos[0], comp)) {
  ...
  if (less_range(vrange_input[pos[2]].first, pos[2], vrange_input[pos[1]].first,
                 pos[1], comp)) {
  ...
  if (pos[0] < pos[1]) {
    return concat(
        raux1, merge(raux2, vrange_input[pos[0]], vrange_input[pos[1]], comp));
  } else {
    return concat(
        raux1, merge(raux2, vrange_input[pos[1]], vrange_input[pos[0]], comp));
  };
};
...
range<Value_t *> uninit_full_merge4(const range<Value_t *> &dest,
  ...
  if (pos[0] < pos[1]) {
    return concat(raux1, merge_construct(raux2, vrange_input[pos[0]],
                                         vrange_input[pos[1]], comp));
  } else {
    return concat(raux1, merge_construct(raux2, vrange_input[pos[1]],
