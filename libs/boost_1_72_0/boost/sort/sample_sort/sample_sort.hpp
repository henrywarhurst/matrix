...
/// @remarks
//----------------------------------------------------------------------------
template <class Iter_t, class Compare> struct sample_sort {
  //------------------------------------------------------------------------
  //                     DEFINITIONS
  //------------------------------------------------------------------------
  ...
  sample_sort(Iter_t first, Iter_t last, Compare cmp, uint32_t num_thread,
      ...
      : sample_sort(first, last, cmp, num_thread, range_buf_initial.first,
                    range_buf_initial.size()){};
  ...
  void execute(void) {
    uint32_t job = 0;
    while ((job = atomic_add(njob, 1)) < ninterval) {
      merge_vector4(vrange_buf_ini[job], vrange_it_ini[job], vv_range_buf[job],
                    vv_range_it[job], comp);
    };
  };
  //
  //-----------------------------------------------------------------------
...
sample_sort<Iter_t, Compare>::sample_sort(Iter_t first, Iter_t last,
                                          Compare cmp, uint32_t num_thread,
                                          value_t *paux, size_t naux)
...
void sample_sort<Iter_t, Compare>::initial_configuration(void) {
  ...
  for (uint32_t i = 0; i < nthread; ++i) {
    auto func = [=]() {
      bss::spinsort<Iter_t, Compare>(vmem_thread[i].first, vmem_thread[i].last,
                                     comp, vbuf_thread[i]);
    };
  ...
  for (uint32_t i = 0; i < nthread; ++i) {
    ...
    for (uint32_t k = 0; k < (ninterval - 1); ++k) {
      Iter_t it2 =
          std::upper_bound(itaux, vmem_thread[i].last, *vmilestone[k], comp);
...
/// @param last : iterator after the last element to the range to sort
//-----------------------------------------------------------------------------
template <class Iter_t> void sample_sort(Iter_t first, Iter_t last) {
