...
template <class Pointee> struct opaque {
  opaque() {
...
template <class Pointee> opaque<Pointee> opaque<Pointee>::instance;
...
#define BOOST_PYTHON_OPAQUE_SPECIALIZED_TYPE_ID(Pointee)                       \
  namespace boost {                                                            \
  namespace python {                                                           \
  template <> inline type_info type_id<Pointee>() {                            \
    return type_info(typeid(Pointee *));                                       \
  }                                                                            \
  template <> inline type_info type_id<const volatile Pointee &>() {           \
    return type_info(typeid(Pointee *));                                       \
  }                                                                            \
  }                                                                            \
  }
