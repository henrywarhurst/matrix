...
template <class T> PyObject *convert_result(T const &x) {
...
template <operator_id> struct operator_1 { template <class T> struct apply; };
...
// self op self
template <operator_id id> struct binary_op : operator_l<id> {
  template <class T> struct apply : operator_l_inner<id, T, T> {};
};

// self op R
template <operator_id id, class R> struct binary_op_l : operator_l<id> {
  template <class T> struct apply : operator_l_inner<id, T, R> {};
};

// L op self
template <operator_id id, class L> struct binary_op_r : operator_r<id> {
  template <class T> struct apply : operator_r_inner<id, L, T> {};
};

template <operator_id id> struct unary_op : operator_1<id> {
  template <class T> struct apply : operator_1_inner<id, T> {};
};
...
private:
  template <class ClassT> void visit(ClassT &cl) const {
    typedef typename mpl::eval_if<
        is_same<L, self_t>,
        mpl::if_<is_same<R, self_t>, binary_op<id>,
                 binary_op_l<id, BOOST_DEDUCED_TYPENAME unwrap_other<R>::type>>,
        mpl::if_<is_same<L, not_specified>, unary_op<id>,
                 binary_op_r<id, BOOST_DEDUCED_TYPENAME
                                     unwrap_other<L>::type>>>::type generator;

    cl.def(generator::name(),
           &generator::template apply<
               BOOST_DEDUCED_TYPENAME ClassT::wrapped_type>::execute);
  }
...
#define BOOST_PYTHON_BINARY_OPERATION(id, rid, expr)                           \
  namespace detail {                                                           \
  template <> struct operator_l<op_##id> {                                     \
    template <class L, class R> struct apply {                                 \
    ...
    static char const *name() { return "__" #id "__"; }                        \
                                                                               ...
                                                                               \
  template <> struct operator_r<op_##id> {                                     \
    template <class L, class R> struct apply {                                 \
      typedef typename unwrap_wrapper_<L>::type lhs;                           \
      typedef typename unwrap_wrapper_<R>::type rhs;                           \
...
#define BOOST_PYTHON_BINARY_OPERATOR(id, rid, op)                              \
  ...
  inline detail::operator_<detail::op_##id, L, R> operator op(L const &,       \
                                                              R const &) {     \
...
template <class R>
inline detail::operator_<detail::op_pow, self_t, R> pow(self_t const &,
                                                        R const &) {
  return detail::operator_<detail::op_pow, self_t, R>();
}
template <class L>
inline detail::operator_<detail::op_pow, L, self_t> pow(L const &,
                                                        self_t const &) {
...
#define BOOST_PYTHON_UNARY_OPERATOR(id, op, func_name)                         \
  namespace detail {                                                           \
  template <> struct operator_1<op_##id> {                                     \
