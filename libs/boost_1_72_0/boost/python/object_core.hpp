...
#include BOOST_PP_ITERATE()
  ...
  template <class T> object contains(T const &key) const;
  ...
  template <class T> const_object_item operator[](T const &key) const;

  template <class T> object_item operator[](T const &key);
  ...
  template <class T, class V> object_slice slice(T const &start, V const &end);
...
private: // def visitation for adding callable objects as class methods
  ...
  void visit(ClassT &cl, char const *name,
    ...
    BOOST_STATIC_ASSERT((detail::is_same<char const *, DocStringT>::value ||
...
template <class T> PyObject *object_base_initializer(T const &x) {
...
struct object_initializer_impl {
  ...
  template <class T> static PyObject *get(T const &x, detail::false_) {
    return python::incref(converter::arg_to_python<T>(x).get());
  }
};

template <> struct object_initializer_impl<true, false> {
...
template <> struct object_initializer_impl<false, true> {
  template <class T, class U> static PyObject *get(T const &x, U) {
...
template <typename U>
object
api::object_operators<U>::operator()(detail::args_proxy const &args) const {
...
template <typename U>
object
api::object_operators<U>::operator()(detail::args_proxy const &args,
                                     detail::kwds_proxy const &kwds) const {
  U const &self = *static_cast<U const *>(this);
  PyObject *result =
...
inline api::object_base &
api::object_base::operator=(api::object_base const &rhs) {
  Py_INCREF(rhs.m_ptr);
...
inline PyObject *api::object_base::ptr() const { return m_ptr; }

inline bool api::object_base::is_none() const { return (m_ptr == Py_None); }
...
template <> struct object_manager_traits<object> {
...
inline PyObject *get_managed_object(object const &x, tag_t) { return x.ptr(); }
