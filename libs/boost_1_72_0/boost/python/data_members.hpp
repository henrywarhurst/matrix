...
struct default_getter_by_ref
    : mpl::and_<
          mpl::bool_<
              to_python_value<typename value_arg<T>::type>::uses_registry>,
...
struct default_member_getter_policy
    : mpl::if_<default_getter_by_ref<T>, return_internal_reference<>,
...
struct default_datum_getter_policy
    : mpl::if_<default_getter_by_ref<T>,
...
inline object make_getter(D *d, Policies const &policies, detail::false_, int) {
  return python::make_function(detail::datum<D>(d), policies,
                               mpl::vector1<D &>());
}
...
inline object make_getter(D C::*pm, Policies const &policies, detail::true_,
                          int) {
  ...
  typedef C Class;
#endif
  return python::make_function(detail::member<D, Class>(pm), policies,
                               mpl::vector2<D &, Class &>());
}
...
inline object make_setter(D *p, Policies const &policies, detail::false_, int) {
  return python::make_function(detail::datum<D>(p), policies,
                               mpl::vector2<void, D const &>());
}

// Handle pointers-to-members
template <class C, class D, class Policies>
inline object make_setter(D C::*pm, Policies const &policies, detail::true_,
                          int) {
  return python::make_function(detail::member<D, C>(pm), policies,
...
template <class D> inline object make_getter(D &x) {
...
#if !BOOST_WORKAROUND(__EDG_VERSION__, <= 238)
template <class D> inline object make_getter(D const &d) {
  detail::not_specified policy =
      detail::not_specified(); // Suppress a SunPro warning
...
template <class D> inline object make_setter(D &x) {
  return detail::make_setter(x, default_call_policies(),
                             detail::is_member_pointer<D>(), 0);
}

#if !BOOST_WORKAROUND(__EDG_VERSION__, <= 238)
template <class D> inline object make_setter(D const &x) {
  return detail::make_setter(x, default_call_policies(),
                             detail::is_member_pointer<D>(), 0);
}
#endif
