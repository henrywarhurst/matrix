...
#define BOOST_PYTHON_OVERLOAD_TYPES_WITH_DEFAULT                               \
  BOOST_PP_ENUM_PARAMS_WITH_A_DEFAULT(BOOST_PYTHON_MAX_ARITY, class T,         \
                                      mpl::void_)

#define BOOST_PYTHON_OVERLOAD_TYPES                                            \
  BOOST_PP_ENUM_PARAMS_Z(1, BOOST_PYTHON_MAX_ARITY, class T)

#define BOOST_PYTHON_OVERLOAD_ARGS                                             \
  BOOST_PP_ENUM_PARAMS_Z(1, BOOST_PYTHON_MAX_ARITY, T)
...
struct more_keywords_than_init_arguments {
  typedef char too_many_keywords[init_args - keywords >= 0
                                     ? 1
                                     : -1] BOOST_ATTRIBUTE_UNUSED;
};
} // namespace error
...
template <class T> struct is_optional : mpl::false_ {};

template <BOOST_PYTHON_OVERLOAD_TYPES>
struct is_optional<optional<BOOST_PYTHON_OVERLOAD_ARGS>> : mpl::true_ {};

template <int NDefaults> struct define_class_init_helper;

} // namespace detail

template <class DerivedT> struct init_base : def_visitor<DerivedT> {
  init_base(char const *doc_, detail::keyword_range const &keywords_)
      : m_doc(doc_), m_keywords(keywords_) {}

  init_base(char const *doc_) : m_doc(doc_) {}
...
private:
  ...
  //          __init__(int, char)
  //          __init__(int)
  template <class classT> void visit(classT &cl) const {
    ...
    detail::define_class_init_helper<n_defaults::value>::apply(
        cl, derived().call_policies(), signature(), n_arguments(),
        derived().doc_string(), derived().keywords());
  }
...
public:
  ...
  init_with_call_policies(CallPoliciesT const &policies_, char const *doc_,
      ...
      : base(doc_, keywords), m_policies(policies_) {}
...
namespace detail {
template <class S>
struct drop1 : mpl::iterator_range<
                   typename mpl::begin<S>::type,
                   typename mpl::prior<typename mpl::end<S>::type>::type> {};
} // namespace detail
...
public:
  ...
  init(char const *doc_ = 0) : base(doc_) {}

  template <std::size_t N>
  init(char const *doc_, detail::keywords<N> const &kw)
      : base(doc_, kw.range()) {
    typedef typename detail::error::more_keywords_than_init_arguments<
        N, n_arguments::value + 1>::too_many_keywords assertion
  ...
  typedef detail::is_optional<typename mpl::eval_if<
      mpl::empty<signature_>, mpl::false_, mpl::back<signature_>>::type>
      back_is_optional;

  typedef typename mpl::eval_if<back_is_optional, mpl::back<signature_>,
                                mpl::vector0<>>::type optional_args;

  typedef typename mpl::eval_if<
      back_is_optional,
      mpl::if_<mpl::empty<optional_args>, detail::drop1<signature_>,
...
inline void def_init_aux(ClassT &cl, Signature const &, NArgs,
                         CallPoliciesT const &policies, char const *doc,
                         detail::keyword_range const &keywords_) {
  cl.def("__init__",
         detail::make_keyword_range_constructor<Signature, NArgs>(
             policies, keywords_, (typename ClassT::metadata::holder *)0),
         doc);
}
...
template <int NDefaults> struct define_class_init_helper {
  ...
  template <class ClassT, class CallPoliciesT, class Signature, class NArgs>
  static void apply(ClassT &cl, CallPoliciesT const &policies,
                    Signature const &args, NArgs, char const *doc,
    ...
    define_class_init_helper<NDefaults - 1>::apply(cl, policies, Signature(),
                                                   next_nargs(), doc, keywords);
  }
};
...
//
///////////////////////////////////////////////////////////////////////////////
template <> struct define_class_init_helper<0> {
