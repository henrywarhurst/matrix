...
struct overloads_base {
  overloads_base(char const *doc_) : m_doc(doc_) {}
...
struct overloads_proxy : public overloads_base {
  ...
  overloads_proxy(CallPoliciesT const &policies_, char const *doc,
      ...
      : overloads_base(doc, kw), policies(policies_) {}
...
// its operator[]
template <class DerivedT> struct overloads_common : public overloads_base {
  overloads_common(char const *doc) : overloads_base(doc) {}

  overloads_common(char const *doc, keyword_range const &kw)
      : overloads_base(doc, kw) {}

  default_call_policies call_policies() const {
...
#define BOOST_PYTHON_FUNC_WRAPPER_GEN(z, index, data)                          \
  static RT BOOST_PP_CAT(                                                      \
      func_, BOOST_PP_SUB_D(1, index, BOOST_PP_TUPLE_ELEM(3, 1, data)))(       \
  ...
  struct fstubs_name {                                                         \
    ...
    template <typename SigT> struct gen {                                      \
      ...
      BOOST_PP_REPEAT_2ND(n_args, BOOST_PYTHON_TYPEDEF_GEN, 0)                 \
                                                                               \
      BOOST_PP_REPEAT_FROM_TO_2(BOOST_PP_SUB_D(1, n_args, n_dflts),            \
                                BOOST_PP_INC(n_args),                          \
                                BOOST_PYTHON_FUNC_WRAPPER_GEN,                 \
                                (fname, BOOST_PP_SUB_D(1, n_args, n_dflts),    \
                                 ret))                                         \
    };                                                                         \
  };
...
#define BOOST_PYTHON_MEM_FUNC_WRAPPER_GEN(z, index, data)                      \
  static RT BOOST_PP_CAT(                                                      \
      ...
      ClassT obj BOOST_PP_COMMA_IF(index)                                      \
          BOOST_PP_ENUM_BINARY_PARAMS_Z(1, index, T, arg)) {                   \
    BOOST_PP_TUPLE_ELEM(3, 2, data)                                            \
    obj.BOOST_PP_TUPLE_ELEM(3, 0, data)(BOOST_PP_ENUM_PARAMS(index, arg));     \
  }

#define BOOST_PYTHON_GEN_MEM_FUNCTION(fname, fstubs_name, n_args, n_dflts,     \
                                      ret)                                     \
  struct fstubs_name {                                                         \
    BOOST_STATIC_CONSTANT(int, n_funcs = BOOST_PP_INC(n_dflts));               \
...
#define BOOST_PYTHON_OVERLOAD_CONSTRUCTORS(fstubs_name, n_args, n_dflts)       \
  ...
  fstubs_name(char const *doc,                                                 \
              ::boost::python::detail::keywords<N> const &keywords)            \
      : ::boost::python::detail::overloads_common<fstubs_name>(                \
            doc, keywords.range()) {                                           \
    typedef typename ::boost::python::detail::error::                          \
        more_keywords_than_function_arguments<N, n_args>::too_many_keywords    \
            assertion BOOST_ATTRIBUTE_UNUSED;                                  \
  }                                                                            \
  template <std::size_t N>                                                     \
  fstubs_name(::boost::python::detail::keywords<N> const &keywords,            \
              char const *doc = 0)                                             \
      : ::boost::python::detail::overloads_common<fstubs_name>(                \
            doc, keywords.range()) {                                           \
    typedef typename ::boost::python::detail::error::                          \
...
#define BOOST_PYTHON_GEN_FUNCTION_STUB(fname, fstubs_name, n_args, n_dflts)    \
  struct fstubs_name                                                           \
    ...
    BOOST_PYTHON_GEN_FUNCTION(fname, non_void_return_type, n_args, n_dflts,    \
                              return )                                         \
...
#define BOOST_PYTHON_GEN_MEM_FUNCTION_STUB(fname, fstubs_name, n_args,         \
                                           n_dflts)                            \
  struct fstubs_name                                                           \
      : public ::boost::python::detail::overloads_common<fstubs_name> {        \
    BOOST_PYTHON_GEN_MEM_FUNCTION(fname, non_void_return_type, n_args,         \
                                  n_dflts, return )                            \
...
#define BOOST_PYTHON_FUNCTION_OVERLOADS(generator_name, fname, min_args,       \
                                        max_args)                              \
  BOOST_PYTHON_GEN_FUNCTION_STUB(fname, generator_name, max_args,              \
                                 BOOST_PP_SUB_D(1, max_args, min_args))

#define BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(generator_name, fname,          \
                                               min_args, max_args)             \
  BOOST_PYTHON_GEN_MEM_FUNCTION_STUB(fname, generator_name, max_args,          \
                                     BOOST_PP_SUB_D(1, max_args, min_args))
