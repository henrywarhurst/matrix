...
#define BOOST_PYTHON_CV_QUALIFIER(i)                                           \
  BOOST_PYTHON_APPLY(BOOST_PP_TUPLE_ELEM(4, i, BOOST_PYTHON_CV_QUALIFIER_I))

#define BOOST_PYTHON_CV_QUALIFIER_I                                            \
  (BOOST_PYTHON_NIL, BOOST_PYTHON_ITEM(const), BOOST_PYTHON_ITEM(volatile),    \
   BOOST_PYTHON_ITEM(const volatile))

// enumerators
#define BOOST_PYTHON_UNARY_ENUM(c, text)                                       \
  BOOST_PP_REPEAT(c, BOOST_PYTHON_UNARY_ENUM_I, text)
#define BOOST_PYTHON_UNARY_ENUM_I(z, n, text) BOOST_PP_COMMA_IF(n) text##n

#define BOOST_PYTHON_BINARY_ENUM(c, a, b)                                      \
  BOOST_PP_REPEAT(c, BOOST_PYTHON_BINARY_ENUM_I, (a, b))
#define BOOST_PYTHON_BINARY_ENUM_I(z, n, _)                                    \
  BOOST_PP_COMMA_IF(n)                                                         \
  BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2, 0, _), n)                                \
      BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2, 1, _), n)

#define BOOST_PYTHON_ENUM_WITH_DEFAULT(c, text, def)                           \
  BOOST_PP_REPEAT(c, BOOST_PYTHON_ENUM_WITH_DEFAULT_I, (text, def))
#define BOOST_PYTHON_ENUM_WITH_DEFAULT_I(z, n, _)                              \
  BOOST_PP_COMMA_IF(n)                                                         \
  BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2, 0, _), n) = BOOST_PP_TUPLE_ELEM(2, 1, _)
