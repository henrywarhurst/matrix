...
// metafunction class.
template <class Tuple, class Predicate> struct tuple_extract;

// Implementation class for when the tuple's head type does not
// satisfy the Predicate
template <bool matched> struct tuple_extract_impl {
  template <class Tuple, class Predicate> struct apply {
...
template <> struct tuple_extract_impl<false> {
  template <class Tuple, class Predicate> struct apply {
...
template <class Tuple, class Predicate> struct tuple_extract_base_select {
  typedef typename Tuple::head_type head_type;
  typedef typename mpl::apply1<
      Predicate, typename add_lvalue_reference<head_type>::type>::type match_t;
  BOOST_STATIC_CONSTANT(bool, match = match_t::value);
  typedef typename tuple_extract_impl<match>::template apply<Tuple, Predicate>
      type;
};

template <class Tuple, class Predicate>
struct tuple_extract
    : tuple_extract_base_select<Tuple,
                                typename mpl::lambda<Predicate>::type>::type {};
...
struct policy_extract
    : tuple_extract<
          Tuple, mpl::and_<mpl::not_<is_same<not_specified const &, mpl::_1>>,
                           indirect_traits::is_reference_to_class<mpl::_1>,
...
// passed to def().
//
template <class T1, class T2, class T3, class T4> struct def_helper {
  ...
  typedef boost::tuples::tuple<
      T1 const &, T2 const &, T3 const &, T4 const &, default_call_policies,
      detail::keywords<0>, char const *,
...
private: // types
  typedef typename default_implementation_extract<all_t>::result_type
...
public: // Constants which can be used for static assertions.
  ...
  // methods.
  BOOST_STATIC_CONSTANT(bool, has_default_implementation =
                                  (!is_same<default_implementation_t,
                                            void (not_specified::*)()>::value));
