...
object
make_function_aux(F f // An object that can be invoked by detail::invoke()
                  ,
...
object
make_function_aux(F f, CallPolicies const &p, Sig const &,
...
//
//   @group {
template <class F> object make_function(F f) {
  return detail::make_function_aux(f, default_call_policies(),
...
object make_function(F f, CallPolicies const &policies,
  ...
  return detail::make_function_dispatch(f, policies, keywords_or_signature,
                                        is_kw());
}

template <class F, class CallPolicies, class Keywords, class Signature>
object make_function(F f, CallPolicies const &policies, Keywords const &kw,
                     Signature const &sig) {
  return detail::make_function_aux(f, policies, sig, kw.range(),
