 ...
 * object_manager_traits<matrix>::get_pytype() is implemented by importing numpy
 * and getting the "matrix" attribute of the module. We then just hope that
 * doesn't get destroyed while we need it, because if we put a dynamic python
 * object in a static-allocated boost::python::object or handle<>, bad things
 * happen when Python shuts down.  I think this solution is safe, but I'd love
 ...
 */
template <typename Base = default_call_policies> struct as_matrix : Base {
