...
class BOOST_NUMPY_DECL dtype : public object {
  static python::detail::new_reference convert(object::object_cref arg,
                                               bool align);
...
public:
  ...
  /**
   *  @brief Get the built-in numpy dtype associated with the given scalar
   * template type.
   *
   ...
   * any integer or floating point type supported by numpy, and will also work
   * for std::complex if sizeof(std::complex<T>) == 2*sizeof(T).
   *
   *  It can also be useful for users to add explicit specializations for POD
   * structs that return field-based dtypes.
   */
   ...
   * int are the same size, the dtypes corresponding to each will be equivalent,
   * but not equal.
   */
...
template <typename T> inline dtype dtype::get_builtin() {
