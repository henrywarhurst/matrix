 ...
 * numpy.ndarray.
 *
 *  @todo This could have a lot more functionality (like
 * boost::python::numeric::array). Right now all that exists is what was needed
 * to move raw data between C++ and Python.
 */
...
class BOOST_NUMPY_DECL ndarray : public object {
   ...
   *
   *  This is just a hack to allow inline access to this stuff while hiding
   * numpy/arrayobject.h from the user.
   */
  struct array_struct {
  ...
  array_struct *get_struct() const {
    return reinterpret_cast<array_struct *>(this->ptr());
  }
...
public:
   ...
   *
   *  These don't match the actual Numpy flag values; we can't get those without
   * including numpy/arrayobject.h or copying them directly.  That's very
   * unfortunate.
   *
   *  @todo I'm torn about whether this should be an enum.  It's very convenient
   * to not make these simple integer values for overloading purposes, but the
   * need to define every possible combination and custom bitwise operators is
   * ugly.
   */
  enum bitflag {
    NONE = 0x0,
  ...
  /// @brief Return the size of the nth dimension. raises IndexError if k not in
  /// [-get_nd() : get_nd()-1 ]
  Py_intptr_t shape(int n) const;

  /// @brief Return the stride of the nth dimension. raises IndexError if k not
  /// in [-get_nd() : get_nd()-1]
  ...
  /// @brief Return the object that owns the array's data, or None if the array
  /// owns its own data.
  ...
  /// @brief Return the shape of the array as an array of integers (length ==
  /// get_nd()).
  Py_intptr_t const *get_shape() const { return get_struct()->shape; }

  /// @brief Return the stride of the array as an array of integers (length ==
  /// get_nd()).
  ...
  /**
   *  @brief If the array contains only a single element, return it as an array
   * scalar; otherwise return the array.
   *
...
BOOST_NUMPY_DECL ndarray zeros(int nd, Py_intptr_t const *shape,
                               dtype const &dt);
...
BOOST_NUMPY_DECL ndarray empty(int nd, Py_intptr_t const *shape,
                               dtype const &dt);

/**
 *  @brief Construct a new array from an arbitrary Python sequence.
 *
 *  @todo This does't seem to handle ndarray subtypes the same way that
...
BOOST_NUMPY_DECL ndarray from_data_impl(void *data, dtype const &dt,
                                        std::vector<Py_intptr_t> const &shape,
                                        std::vector<Py_intptr_t> const &strides,
                                        object const &owner, bool writeable);

template <typename Container>
ndarray
from_data_impl(void *data, dtype const &dt, Container shape, Container strides,
               object const &owner, bool writeable,
               typename boost::enable_if<boost::python::detail::is_integral<
                   typename Container::value_type>>::type *enabled = NULL) {
 ...
 * have begin() and end().
 *  @param[in] strides Shape of the array as STL container of integers; must
 * have begin() and end().
 *  @param[in] owner   An arbitray Python object that owns that data pointer.
 * The array object will keep a reference to the object, and decrement it's
 * reference count when the array goes out of scope.  Pass None at your own
 * peril.
 *
 *  @todo Should probably take ranges of iterators rather than actual container
 * objects.
 */
template <typename Container>
inline ndarray from_data(void *data, dtype const &dt, Container shape,
                         Container strides, python::object const &owner) {
 ...
 * peril.
 *
 *  This overload takes a const void pointer and sets the "writeable" flag of
 * the array to false.
 *
 *  @todo Should probably take ranges of iterators rather than actual container
 * objects.
 */
template <typename Container>
inline ndarray from_data(void const *data, dtype const &dt, Container shape,
                         Container strides, python::object const &owner) {
  return numpy::detail::from_data_impl(const_cast<void *>(data), dt, shape,
                                       strides, owner, false);
}

/**
 *  @brief Transform an arbitrary object into a numpy array with the given
 * requirements.
 *
 *  @param[in] obj     An arbitrary python object to convert.  Arrays that meet
 * the requirements will be passed through directly.
 *  @param[in] dt      Data type descriptor.  Often retrieved with
 * dtype::get_builtin().
...
BOOST_NUMPY_DECL ndarray from_object(object const &obj, dtype const &dt,
                                     int nd_min, int nd_max,
                                     ndarray::bitflag flags = ndarray::NONE);

BOOST_NUMPY_DECL inline ndarray
from_object(object const &obj, dtype const &dt, int nd,
            ndarray::bitflag flags = ndarray::NONE) {
  return from_object(obj, dt, nd, nd, flags);
}

BOOST_NUMPY_DECL inline ndarray
from_object(object const &obj, dtype const &dt,
            ndarray::bitflag flags = ndarray::NONE) {
...
BOOST_NUMPY_DECL inline ndarray
from_object(object const &obj, ndarray::bitflag flags = ndarray::NONE) {
