...
/**
 *  @brief Utilities to create ufunc-like broadcasting functions out of C++
 * functors.
 */
 ...
 * PyArray_MultiIter.
 *
 *  multi_iter is a Python object, but a very low-level one.  It should
 * generally only be used in loops of the form:
 *  @code
 *  while (iter.not_done()) {
 *      ...
 *      iter.next();
 ...
 *  @endcode
 *
 *  @todo I can't tell if this type is exposed in Python anywhere; if it is, we
 * should use that name. It's more dangerous than most object managers, however
 * - maybe it actually belongs in a detail namespace?
 */
...
public:
  ...
  /// @brief Return the number of dimensions of the broadcasted array
  /// expression.
  int get_nd() const;

  /// @brief Return the shape of the broadcasted array expression as an array of
  /// integers.
  Py_intptr_t const *get_shape() const;

  /// @brief Return the shape of the broadcasted array expression in the nth
  /// dimension.
...
/**
 *  @brief Helps wrap a C++ functor taking a single scalar argument as a
 * broadcasting ufunc-like Python object.
 *
...
struct unary_ufunc {
  ...
  /**
   *  @brief A C++ function with object arguments that broadcasts its arguments
   * before passing them to the underlying C++ functor.
   */
  static object call(TUnaryFunctor &self, object const &input,
                     object const &output) {
    ...
    ndarray out_array =
        !output.is_none()
            ? from_object(output, out_dtype,
                          ndarray::ALIGNED | ndarray::WRITEABLE)
  ...
  /**
   *  @brief Construct a boost.python function object from call() with
   * reasonable keyword names.
   *
   *  Users will often want to specify their own keyword names with the same
   * signature, but this is a convenient shortcut.
   */
  static object make() {
    return make_function(call, default_call_policies(),
                         (arg("input"), arg("output") = object()));
  }
};

/**
 *  @brief Helps wrap a C++ functor taking a pair of scalar arguments as a
 * broadcasting ufunc-like Python object.
 *
 *  Typical usage looks like this:
 *  @code
 ...
 *    typedef double result_type;
 *    double operator()(double input1, double input2) const { return
 * std::cos(input1 + input2); }
 *  };
 *
 *  BOOST_PYTHON_MODULE(example)
 *  {
...
struct binary_ufunc {
  ...
  static object call(TBinaryFunctor &self, object const &input1,
                     object const &input2, object const &output) {
