...
void invoke_matching_nd(int nd, Function f) {
  ...
  PyErr_SetString(PyExc_TypeError,
                  "number of dimensions not found in template list.");
...
struct array_template_invoker_wrapper_2 {
  template <int N> void apply() const {
...
struct array_template_invoker_wrapper_1 {
  template <typename T> void apply() const {
    invoke_matching_nd<DimSequence>(
        m_nd, array_template_invoker_wrapper_2<T, Function>(m_func));
  }
...
template <typename DimSequence, typename Function>
struct array_template_invoker_wrapper_1<DimSequence,
                                        boost::reference_wrapper<Function>>
...
void invoke_matching_array(ndarray const &array_, Function f) {
  detail::array_template_invoker_wrapper_1<DimSequence, Function> wrapper(
      array_.get_nd(), f);
