...
template <class T> inline type_info type_id() {
  return type_info(
...
#define BOOST_PYTHON_SIGNED_INTEGRAL_TYPE_ID(T)                                \
  template <> inline type_info type_id<T>() { return type_info(typeid(T)); }
...
template <> inline type_info type_id<void>() {
  return type_info(typeid(void *));
}
#ifndef BOOST_NO_CV_VOID_SPECIALIZATIONS
template <> inline type_info type_id<const volatile void>() {
  return type_info(typeid(void *));
}
#endif
