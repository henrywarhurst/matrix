...
struct BOOST_PYTHON_DECL list_base : object {
  ...
  void
  extend(object_cref sequence); // extend list by appending sequence elements

  long
  index(object_cref value) const; // return index of first occurrence of value
...
public:
  ...
  template <class T> void append(T const &x) { base::append(object(x)); }

  template <class T> ssize_t count(T const &value) const {
    return base::count(object(value));
  }

  template <class T> void extend(T const &x) { base::extend(object(x)); }

  template <class T> long index(T const &x) const {
  ...
  template <class T> object pop(T const &index) {
    return base::pop(object(index));
  }

  template <class T> void remove(T const &value) {
  ...
  template <class T> void sort(T const &value) { base::sort(object(value)); }
#endif
