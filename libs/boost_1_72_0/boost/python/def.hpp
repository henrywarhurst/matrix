...
template <> struct multiple_functions_passed_to_def<false> {
  typedef char type;
};
} // namespace error
...
void def_from_helper(char const *name, F const &fn, Helper const &helper) {
  ...
  typedef typename error::multiple_functions_passed_to_def<
      Helper::has_default_implementation>::type assertion
...
void def_maybe_overloads(char const *name, SigT sig, StubsT const &stubs,
                         detail::overloads_base const *) {
  scope current;

  detail::define_with_defaults(name, stubs, current,
                               detail::get_signature(sig));
}

template <class T> object make_function1(T fn, ...) {
  return make_function(fn);
}

inline object make_function1(object const &x, object const *) { return x; }
} // namespace detail

template <class Fn> void def(char const *name, Fn fn) {
