...
//
template <class Proxy> class proxy_group {
public:
  ...
  iterator first_proxy(index_type i) {
    ...
    return boost::detail::lower_bound(proxies.begin(), proxies.end(), i,
                                      compare_proxy_index<Proxy>());
  }

  void remove(Proxy &proxy) {
    // Remove a proxy
    for (iterator iter = first_proxy(proxy.get_index()); iter != proxies.end();
         ++iter) {
  ...
  void add(PyObject *prox) {
    BOOST_PYTHON_INDEXING_CHECK_INVARIANT;
    // Add a proxy
    proxies.insert(first_proxy(extract<Proxy &>(prox)().get_index()), prox);
    BOOST_PYTHON_INDEXING_CHECK_INVARIANT;
  }
  ...
  void replace(index_type from, index_type to,
    ...
    typename std::vector<PyObject *>::size_type offset = left - proxies.begin();
    ...
    while (right != proxies.end()) {
      ...
      p().set_index(extract<Proxy &>(*right)().get_index() -
  ...
  PyObject *find(index_type i) {
    ...
    iterator iter = first_proxy(i);
    if (iter != proxies.end() && extract<Proxy &>(*iter)().get_index() == i) {
      BOOST_PYTHON_INDEXING_CHECK_INVARIANT;
      return *iter;
    }
    BOOST_PYTHON_INDEXING_CHECK_INVARIANT;
  ...
  typename std::vector<PyObject *>::size_type size() const {
    BOOST_PYTHON_INDEXING_CHECK_INVARIANT;
...
// the map.
//
template <class Proxy, class Container> class proxy_links {
public:
  ...
  void replace(Container &container, index_type from, index_type to,
               index_type len) {
    // Erase all proxies with indexes from..to.
    // Adjust the displaced indexes such that the
    // final effect is that we have inserted *len*
...
public:
  ...
  void detach() {
    if (!is_detached()) {
      ptr.reset(new element_type(Policies::get_item(get_container(), index)));
      container = object(); // free container. reset it to None
    }
  }

  bool is_detached() const { return get_pointer(ptr) != 0; }

  Container &get_container() const { return extract<Container &>(container)(); }

  Index get_index() const { return index; }

  void set_index(Index i) { index = i; }
...
template <class Container, class DerivedPolicies, class ContainerElement,
          class Index>
struct no_proxy_helper {
  static void register_container_element() {}

  template <class DataType>
  static object base_get_item_helper(DataType const &p, detail::true_) {
    return object(ptr(p));
  }

  template <class DataType>
  static object base_get_item_helper(DataType const &x, detail::false_) {
    return object(x);
  }

  static object base_get_item_(back_reference<Container &> const &container,
                               PyObject *i) {
    return base_get_item_helper(
        DerivedPolicies::get_item(
            container.get(),
            DerivedPolicies::convert_index(container.get(), i)),
        is_pointer<BOOST_DEDUCED_TYPENAME Container::value_type>());
  }

  static void base_replace_indexes(Container & /*container*/, Index /*from*/,
                                   Index /*to*/, Index /*n*/) {}

  template <class NoSlice>
  static void base_erase_index(Container & /*container*/, Index /*i*/,
                               NoSlice /*no_slice*/) {}

  static void base_erase_indexes(Container & /*container*/, Index /*from*/,
                                 Index /*to*/) {}
};
...
struct proxy_helper {
  ...
  static object base_get_item_(back_reference<Container &> const &container,
    ...
    if (PyObject *shared =
      ...
      ContainerElement::get_links().add(prox.ptr(), container.get());
      return prox;
    }
  }

  static void base_replace_indexes(Container &container, Index from, Index to,
                                   Index n) {
...
template <class Container, class DerivedPolicies, class ProxyHandler,
...
struct slice_helper {
  ...
  static void base_get_slice_data(Container &container, PySliceObject *slice,
  ...
  static void base_set_slice(Container &container, PySliceObject *slice,
                             PyObject *v) {
    Index from, to;
    base_get_slice_data(container, slice, from, to);
    ...
    if (elem.check()) {
      ...
      if (elem.check()) {
        ...
        for (int i = 0; i < l.attr("__len__")(); i++) {
          ...
          if (x.check()) {
            ...
            if (x.check()) {
              ...
              PyErr_SetString(PyExc_TypeError, "Invalid sequence element");
              throw_error_already_set();
            }
          }
        }
        ...
        ProxyHandler::base_replace_indexes(container, from, to,
                                           temp.end() - temp.begin());
        DerivedPolicies::set_slice(container, from, to, temp.begin(),
                                   temp.end());
      }
    }
  }
...
struct no_slice_helper {
  ...
  static object base_get_slice(Container & /*container*/,
                               PySliceObject * /*slice*/) {
    slicing_not_suported();
    return object();
  }

  static void base_set_slice(Container & /*container*/,
                             PySliceObject * /*slice*/, PyObject * /*v*/) {
    slicing_not_suported();
  }

  static void base_delete_slice(Container & /*container*/,
                                PySliceObject * /*slice*/) {
    slicing_not_suported();
  }
};
...
template <class Container, class Index, class Policies>
inline typename Policies::data_type *get_pointer(
