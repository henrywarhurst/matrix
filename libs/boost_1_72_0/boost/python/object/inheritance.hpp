...
register_dynamic_id_aux(class_id static_id, dynamic_id_function get_dynamic_id);
...
template <class T> struct polymorphic_id_generator {
...
template <class T> struct non_polymorphic_id_generator {
  static dynamic_id_t execute(void *p_) {
...
// system.
template <class T> void register_dynamic_id(T * = 0) {
...
template <class Source, class Target> struct dynamic_cast_generator {
  static void *execute(void *source) {
    return dynamic_cast<Target *>(static_cast<Source *>(source));
  }
};

template <class Source, class Target> struct implicit_cast_generator {
  static void *execute(void *source) {
