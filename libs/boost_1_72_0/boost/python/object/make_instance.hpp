...
template <class T, class Holder, class Derived> struct make_instance_impl {
  ...
  template <class Arg> static inline PyObject *execute(Arg &x) {
    ...
    PyObject *raw_result =
        type->tp_alloc(type, objects::additional_instance_size<Holder>::value);
...
struct make_instance : make_instance_impl<T, Holder, make_instance<T, Holder>> {
  template <class U> static inline PyTypeObject *get_class_object(U &) {
    return converter::registered<T>::converters.get_class_object();
  }

  static inline Holder *construct(void *storage, PyObject *instance,
