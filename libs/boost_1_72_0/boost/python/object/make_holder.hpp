...
template <> struct make_holder<N> {
  template <class Holder, class ArgList> struct apply {
#if N
...
#define BOOST_PP_LOCAL_MACRO(n)                                                \
  ...
  typedef typename mpl::next<iter##n>::type BOOST_PP_CAT(                      \
      iter, BOOST_PP_INC(n)); // Next iterator type
...
#if !defined(BOOST_PYTHON_NO_PY_SIGNATURES) &&                                 \
    defined(BOOST_PYTHON_PY_SIGNATURES_PROPER_INIT_SELF_TYPE)
        boost::python::detail::python_class<
            BOOST_DEDUCED_TYPENAME Holder::value_type> *p
#else
        PyObject *p
#endif
      ...
      try {
        (new (memory)
             Holder(p BOOST_PP_REPEAT_1ST(N, BOOST_PYTHON_DO_FORWARD_ARG, nil)))
            ->install(p);
      } catch (...) {
