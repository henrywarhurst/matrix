...
#define BOOST_PYTHON_UNFORWARD_LOCAL(z, n, _)                                  \
  BOOST_PP_COMMA_IF(n) objects::do_unforward(a##n, 0)

template <class Pointer, class Value> struct pointer_holder : instance_holder {
...
private: // required holder implementation
  ...
  inline void *holds_wrapped(type_info, ...) { return 0; }
...
template <class Pointer, class Value>
inline pointer_holder_back_reference<
    Pointer, Value>::pointer_holder_back_reference(Pointer p)
#if defined(BOOST_NO_CXX11_SMART_PTR)
    : m_p(p)
#else
    : m_p(std::move(p))
...
void *pointer_holder<Pointer, Value>::holds(type_info dst_t,
                                            bool null_ptr_only) {
  typedef
      typename boost::python::detail::remove_const<Value>::type non_const_value;
...
#endif
pointer_holder(PyObject *self BOOST_PP_COMMA_IF(N)
                   BOOST_PP_ENUM_BINARY_PARAMS_Z(1, N, A, a))
    : m_p(new Value(
...
template <BOOST_PP_ENUM_PARAMS_Z(1, N, class A)>
#endif
pointer_holder_back_reference(PyObject *p BOOST_PP_COMMA_IF(N)
                                  BOOST_PP_ENUM_BINARY_PARAMS_Z(1, N, A, a))
    : m_p(new held_type(p BOOST_PP_COMMA_IF(N) BOOST_PP_REPEAT_1ST(
          N, BOOST_PYTHON_UNFORWARD_LOCAL, nil))) {
}
