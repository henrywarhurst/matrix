...
template <class Value> struct value_holder : instance_holder {
...
#define BOOST_PP_ITERATION_PARAMS_1                                            \
  (4, (0, BOOST_PYTHON_MAX_ARITY,                                              \
       <boost / python / object / value_holder.hpp>, 1))
...
#define BOOST_PP_ITERATION_PARAMS_1                                            \
  (4, (0, BOOST_PYTHON_MAX_ARITY,                                              \
       <boost / python / object / value_holder.hpp>, 2))
#include BOOST_PP_ITERATE()
...
void *value_holder<Value>::holds(type_info dst_t, bool /*null_ptr_only*/) {
  if (void *wrapped = holds_wrapped(dst_t, boost::addressof(m_held),
                                    boost::addressof(m_held)))
    return wrapped;
...
template <class Value, class Held>
void *value_holder_back_reference<Value, Held>::holds(type_info dst_t,
                                                      bool /*null_ptr_only*/) {
  type_info src_t = python::type_id<Value>();
  Value *x = &m_held;
...
#endif
value_holder(PyObject *self BOOST_PP_COMMA_IF(N)
...
template <BOOST_PP_ENUM_PARAMS_Z(1, N, class A)>
#endif
value_holder_back_reference(PyObject *p BOOST_PP_COMMA_IF(N)
                                BOOST_PP_ENUM_BINARY_PARAMS_Z(1, N, A, a))
    : m_held(p BOOST_PP_COMMA_IF(N)
