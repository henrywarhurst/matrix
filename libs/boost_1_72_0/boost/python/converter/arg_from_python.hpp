...
// Used when T == U*const&
template <class T> struct pointer_cref_arg_from_python {
...
template <class T> struct arg_rvalue_from_python {
...
template <class C, class T, class F> struct if_2 {
...
template <class T> struct select_arg_from_python {
  typedef typename if_2<
      is_object_manager<T>, object_manager_value_arg_from_python<T>,
      if_2<
          is_reference_to_object_manager<T>,
          object_manager_ref_arg_from_python<T>,
          if_2<is_pointer<T>, pointer_arg_from_python<T>,
               if_2<mpl::and_<indirect_traits::is_reference_to_pointer<T>,
                              indirect_traits::is_reference_to_const<T>,
                              mpl::not_<indirect_traits::
                                            is_reference_to_volatile<T>>>,
                    pointer_cref_arg_from_python<T>,
                    if_2<mpl::or_<indirect_traits::is_reference_to_non_const<T>,
                                  indirect_traits::is_reference_to_volatile<T>>,
                         reference_arg_from_python<T>,
                         mpl::if_<boost::python::is_back_reference<T>,
...
template <class T> struct null_ptr_owner { static T value; };
template <class T> T null_ptr_owner<T>::value = 0;

template <class U> inline U &null_ptr_reference(U &(*)()) {
  return null_ptr_owner<U>::value;
}
} // namespace detail

template <class T>
inline pointer_cref_arg_from_python<T>::pointer_cref_arg_from_python(
    PyObject *p) {
  ...
  python::detail::write_void_ptr_reference(
      m_result.bytes,
      p == Py_None ? p
                   : converter::get_lvalue_from_python(
                         p, registered_pointee<T>::converters),
      (T(*)())0);
}
...
template <class T> inline T pointer_arg_from_python<T>::operator()() const {
...
template <class T> inline T reference_arg_from_python<T>::operator()() const {
...
inline arg_rvalue_from_python<T>::arg_rvalue_from_python(PyObject *obj)
    : m_data(
          converter::rvalue_from_python_stage1(obj, registered<T>::converters)),
      m_source(obj) {}

template <class T> inline bool arg_rvalue_from_python<T>::convertible() const {
...
template <class T> inline T back_reference_arg_from_python<T>::operator()() {
