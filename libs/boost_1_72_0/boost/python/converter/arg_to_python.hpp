...
namespace detail {
template <class T> struct function_arg_to_python : handle<> {
  function_arg_to_python(T const &x);
};

template <class T> struct reference_arg_to_python : handle<> {
...
template <class T> struct shared_ptr_arg_to_python : handle<> {
...
template <class T> struct value_arg_to_python : arg_to_python_base {
  // Throw an exception if the conversion can't succeed
  value_arg_to_python(T const &);
};

template <class Ptr> struct pointer_deep_arg_to_python : arg_to_python_base {
  // Throw an exception if the conversion can't succeed
  pointer_deep_arg_to_python(Ptr);
};

template <class Ptr> struct pointer_shallow_arg_to_python : handle<> {
  // Throw an exception if the conversion can't succeed
...
template <class T> struct object_manager_arg_to_python {
...
template <class T> struct select_arg_to_python {
...
template <class T, class Convertibility> struct reject_raw_object_helper {
...
template <class T> inline void reject_raw_object_ptr(T *) {
