...
namespace detail {
template <class T> struct install_holder : converter::context_result_converter {
  install_holder(PyObject *args_) : m_self(PyTuple_GetItem(args_, 0)) {}
...
private:
  template <class U> void dispatch(U *x, detail::true_) const {
  ...
  template <class Ptr> void dispatch(Ptr x, detail::false_) const {
...
struct constructor_result_converter {
  template <class T> struct apply { typedef install_holder<T> type; };
};

template <class BaseArgs, class Offset> struct offset_args {
...
// ETI workaround
template <> struct outer_constructor_signature<int> { typedef int type; };
...
object make_constructor_aux(
  ...
  return objects::function_object(objects::py_function(
...
object make_constructor_aux(
  ...
  typedef typename detail::error::more_keywords_than_function_arguments<
      NumKeywords::value, arity>::too_many_keywords assertion
...
//
template <class F> object make_constructor(F f) {
  return detail::make_constructor_aux(f, default_call_policies(),
...
object make_constructor(F f, CallPolicies const &policies,
  ...
  typedef typename detail::is_reference_to_keywords<KeywordsOrSignature &>::type
      is_kw;

  return detail::make_constructor_dispatch(f, policies, keywords_or_signature,
                                           is_kw());
}

template <class F, class CallPolicies, class Keywords, class Signature>
object make_constructor(F f, CallPolicies const &policies, Keywords const &kw,
                        Signature const &sig) {
  return detail::make_constructor_aux(f, policies, sig, kw.range(),
