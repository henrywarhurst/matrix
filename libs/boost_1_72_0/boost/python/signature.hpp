...
// otherwise
template <class C1, class C2> struct most_derived {
  typedef
...
inline BOOST_PYTHON_LIST_INC(N)<RT BOOST_PP_ENUM_TRAILING_PARAMS_Z(
    1, N, T)> get_signature(RT(BOOST_PYTHON_FN_CC *)(BOOST_PP_ENUM_PARAMS_Z(1,
                                                                            N,
                                                                            T)),
                            void * = 0) {
...
inline BOOST_PYTHON_LIST_INC(BOOST_PP_INC(N))<
  ...
  return BOOST_PYTHON_LIST_INC(
      BOOST_PP_INC(N))<RT, ClassT & BOOST_PP_ENUM_TRAILING_PARAMS_Z(1, N, T)>();
}
