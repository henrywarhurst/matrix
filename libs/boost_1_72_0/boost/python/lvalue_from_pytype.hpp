...
template <class T, class U> inline type_info extractor_type_id(T &(*)(U)) {
  return type_id<T>();
}
...
// is being extracted.
template <class Extractor, class U> struct normalized_extractor {
  static inline void *execute(PyObject *op) {
    typedef typename add_lvalue_reference<U>::type param;
    return &Extractor::execute(
        boost::python::detail::void_ptr_to_reference(op, (param(*)())0));
  }
};
...
template <class InstanceType, class MemberType,
          MemberType(InstanceType::*member)>
...
template <class InstanceType> struct extract_identity {
...
struct lvalue_from_pytype {
  lvalue_from_pytype() {
    converter::registry::insert(&extract,
