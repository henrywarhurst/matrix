...
#define BOOST_PYTHON_SERIALIZATION_ARCHIVE(IArchiver, OArchiver)               \
  ...
  namespace python {                                                           \
  namespace api {                                                              \
  template <typename R, typename T> struct enable_binary<IArchiver, R, T> {};  \
                                                                               \
  template <typename R, typename T> struct enable_binary<OArchiver, R, T> {};  \
  }                                                                            \
  }                                                                            \
  }
#else
...
#define BOOST_PYTHON_DIRECT_SERIALIZATION_ARCHIVE(IArchiver, OArchiver)        \
  ...
  struct has_direct_serialization<IArchiver, OArchiver> : mpl::true_ {};       \
                                                                               \
  template <> struct output_archiver<IArchiver> { typedef OArchiver type; };   \
                                                                               \
  template <> struct input_archiver<OArchiver> { typedef IArchiver type; };    \
  }                                                                            \
  }
...
#define BOOST_PYTHON_DIRECT_SERIALIZATION_ARCHIVE_IMPL(IArchiver, OArchiver)   \
  ...
  namespace python {                                                           \
  namespace detail {                                                           \
  template class BOOST_MPI_PYTHON_DECL                                         \
...
public:
  typedef boost::function3<void, OArchiver &, const object &,
                           const unsigned int>
      saver_t;
...
protected:
  template <typename T> struct default_saver {
  ...
  template <typename T> struct default_loader {
...
void save_impl(Archiver &ar, const boost::python::object &obj,
...
template <typename Archiver>
void load_impl(Archiver &ar, boost::python::object &obj,
               const unsigned int /*version*/,
               mpl::false_ /*has_direct_serialization*/) {
  int len;
  ar >> len;
...
template <typename Archiver>
void save(Archiver &ar, const boost::python::object &obj,
          const unsigned int version) {
  typedef Archiver OArchiver;
  typedef typename input_archiver<OArchiver>::type IArchiver;
...
template <typename Archiver>
void load(Archiver &ar, boost::python::object &obj,
          const unsigned int version) {
  typedef Archiver IArchiver;
  typedef typename output_archiver<IArchiver>::type OArchiver;
...
inline void serialize(Archive &ar, boost::python::object &obj,
                      const unsigned int version) {
...
BOOST_PYTHON_DIRECT_SERIALIZATION_ARCHIVE(::boost::mpi::packed_iarchive,
...
template <typename T>
void register_serialized(const T &value, PyTypeObject *type) {
