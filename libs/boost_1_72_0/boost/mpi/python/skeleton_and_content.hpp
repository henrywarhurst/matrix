 ...
 */
template <typename T> class skeleton_proxy : public skeleton_proxy_base {
public:
...
template <typename T> struct skeleton_saver {
  void operator()(packed_oarchive &ar, const object &obj, const unsigned int) {
    packed_skeleton_oarchive pso(ar);
    pso << extract<T &>(obj.attr("object"))();
  }
};

template <typename T> struct skeleton_loader {
  void operator()(packed_iarchive &ar, object &obj, const unsigned int) {
 ...
 */
template <typename T> struct do_get_skeleton_proxy {
  object operator()(object value) { return object(skeleton_proxy<T>(value)); }
};
 ...
 */
template <typename T> struct do_get_content {
...
void register_skeleton_and_content(const T &value, PyTypeObject *type) {
  ...
  table.register_type(detail::skeleton_saver<T>(), detail::skeleton_loader<T>(),
