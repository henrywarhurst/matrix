...
std::pair<status, ForwardIterator> wait_any(ForwardIterator first,
 ...
 */
template <typename ForwardIterator>
optional<std::pair<status, ForwardIterator>> test_any(ForwardIterator first,
                                                      ForwardIterator last) {
  while (first != last) {
    // Check if we have found a completed request. If so, return it.
...
OutputIterator wait_all(ForwardIterator first, ForwardIterator last,
                        OutputIterator out) {
  typedef typename std::iterator_traits<ForwardIterator>::difference_type
      difference_type;
  ...
  while (num_outstanding_requests > 0) {
    ...
    if (all_trivial_requests &&
      ...
      BOOST_MPI_CHECK_RESULT(
          MPI_Waitall, (num_outstanding_requests, &requests[0], &stats[0]));
...
void wait_all(ForwardIterator first, ForwardIterator last) {
  ...
  while (num_outstanding_requests > 0) {
    ...
    if (all_trivial_requests &&
      ...
      // Let MPI wait until all of these operations completes.
      BOOST_MPI_CHECK_RESULT(MPI_Waitall, (num_outstanding_requests,
 ...
 */
template <typename ForwardIterator, typename OutputIterator>
optional<OutputIterator> test_all(ForwardIterator first, ForwardIterator last,
                                  OutputIterator out) {
  std::vector<MPI_Request> requests;
  ...
  while (true) {
    ...
    if (++current == start_of_completed) {
      ...
      if (all_trivial_requests) {
        ...
        int num_completed;
        BOOST_MPI_CHECK_RESULT(MPI_Waitsome, (n, &requests[0], &num_completed,
                                              &indices[0], &stats[0]));
...
BidirectionalIterator wait_some(BidirectionalIterator first,
