...
template <typename T>
void scatter_impl(const communicator &comm, const T *in_values, T *out_values,
                  int n, int root, mpl::true_) {
  MPI_Datatype type = get_mpi_datatype<T>(*in_values);
  BOOST_MPI_CHECK_RESULT(MPI_Scatter, (const_cast<T *>(in_values), n, type,
...
// datatype, so we'll use MPI_Scatter to do all of the work.
template <typename T>
void scatter_impl(const communicator &comm, T *out_values, int n, int root,
                  mpl::true_) {
  MPI_Datatype type = get_mpi_datatype<T>(*out_values);
  BOOST_MPI_CHECK_RESULT(MPI_Scatter,
                         (0, n, type, out_values, n, type, root, comm));
}
...
template <typename T>
void fill_scatter_sendbuf(const communicator &comm, T const *values,
  ...
  for (int dest = 0; dest < nproc; ++dest) {
    if (skipped_slots) { // wee need to keep this for backward compatibility
      for (int k = 0; k < skipped_slots[dest]; ++k)
        ++values;
    }
...
template <typename T, class A> T *non_const_data(std::vector<T, A> const &v) {
  using detail::c_data;
...
template <typename T>
void dispatch_scatter_sendbuf(const communicator &comm,
                              packed_oarchive::buffer_type const &sendbuf,
                              std::vector<int> const &archsizes,
                              T const *in_values, T *out_values, int n,
                              int root) {
  // Distribute the sizes
  int myarchsize;
  BOOST_MPI_CHECK_RESULT(MPI_Scatter, (non_const_data(archsizes), 1, MPI_INT,
  ...
  BOOST_MPI_CHECK_RESULT(MPI_Scatterv,
                         (non_const_data(sendbuf), non_const_data(archsizes),
                          c_data(offsets), MPI_BYTE, c_data(recvbuf),
                          recvbuf.size(), MPI_BYTE, root, MPI_Comm(comm)));
  // Unserialize
...
void scatter_impl(const communicator &comm, const T *in_values, T *out_values,
  ...
  if (root == comm.rank()) {
    ...
    fill_scatter_sendbuf(comm, in_values, c_data(nslots), (int const *)0,
                         sendbuf, archsizes);
  }
  dispatch_scatter_sendbuf(comm, sendbuf, archsizes, in_values, out_values, n,
                           root);
}
