...
void reduce_impl(const communicator &comm, const T *in_values, int n,
...
// datatype and operation, so we'll use MPI_Reduce directly.
template <typename T, typename Op>
void reduce_impl(const communicator &comm, const T *in_values, int n, Op /*op*/,
...
template <typename T, typename Op>
void tree_reduce_impl(const communicator &comm, const T *in_values, int n,
...
void reduce(const communicator &comm, const T *in_values, int n, T *out_values,
  ...
  if (comm.rank() == root)
    ...
    detail::reduce_impl(comm, in_values, n, op, root, is_mpi_op<Op, T>(),
                        is_mpi_datatype<T>());
}
...
template <typename T, typename Op>
void reduce(const communicator &comm, std::vector<T> const &in_values, Op op,
            int root) {
  reduce(comm, &in_values.front(), in_values.size(), op, root);
}

template <typename T, typename Op>
void reduce(const communicator &comm, std::vector<T> const &in_values,
...
void reduce(const communicator &comm, const T &in_value, T &out_value, Op op,
  ...
  if (comm.rank() == root)
    ...
    detail::reduce_impl(comm, &in_value, 1, op, root, is_mpi_op<Op, T>(),
                        is_mpi_datatype<T>());
}
