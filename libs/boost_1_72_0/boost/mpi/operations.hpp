 ...
 */
template <typename Op, typename T> struct is_mpi_op : public mpl::false_ {};
...
template <typename Op, typename T> class user_op {
public:
  user_op() {
    BOOST_MPI_CHECK_RESULT(
        MPI_Op_create,
        (&user_op<Op, T>::perform, is_commutative<Op, T>::value, &mpi_op));
  }
...
private:
  ...
  static void BOOST_MPI_CALLING_CONVENTION perform(void *vinvec, void *voutvec,
                                                   int *plen, MPI_Datatype *) {
