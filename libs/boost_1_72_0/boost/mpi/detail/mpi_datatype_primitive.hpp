...
public:
  ...
  mpi_datatype_primitive() : is_committed(false), origin(0) {}

  mpi_datatype_primitive(void const *orig) : is_committed(false), origin() {
#if BOOST_MPI_VERSION >= 2
    BOOST_MPI_CHECK_RESULT(MPI_Get_address,
                           (const_cast<void *>(orig), &origin));
#else
  ...
  void save_array(serialization::array_wrapper<T> const &x,
    ...
    if (x.count())
      save_impl(x.address(), boost::mpi::get_mpi_datatype(*x.address()),
                x.count());
  }
  ...
  MPI_Datatype get_mpi_datatype() {
    if (!is_committed) {
      ...
      BOOST_MPI_CHECK_RESULT(MPI_Type_create_struct,
                             (addresses.size(), c_data(lengths),
                              c_data(addresses), c_data(types), &datatype_));
#else
...
private:
  ...
  template <class T> static T *get_data(std::vector<T> &v) {
