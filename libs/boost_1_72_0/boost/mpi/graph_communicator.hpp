...
protected:
   ...
   */
  template <typename Graph, typename RankMap>
  void setup_graph(const communicator &comm, const Graph &graph, RankMap rank,
                   bool reorder);
};
...
graph_communicator::graph_communicator(const communicator &comm,
                                       const Graph &graph, bool reorder) {
...
template <typename Graph, typename RankMap>
void graph_communicator::setup_graph(const communicator &comm,
                                     const Graph &graph, RankMap rank,
                                     bool reorder) {
  ...
  BOOST_MPI_CHECK_RESULT(MPI_Graph_create,
                         ((MPI_Comm)comm, nvertices, &indices[0],
                          edges.empty() ? (int *)0 : &edges[0], reorder,
                          &newcomm));
...
class comm_out_edge_iterator
    : public iterator_facade<comm_out_edge_iterator, std::pair<int, int>,
...
protected:
  ...
  bool equal(const comm_out_edge_iterator &other) const {
    return (edge.first == other.edge.first && index == other.index);
  }
...
class comm_adj_iterator
    : public iterator_facade<comm_adj_iterator, int,
                             random_access_traversal_tag, int, int> {
public:
  comm_adj_iterator() {}
...
protected:
  ...
  bool equal(const comm_adj_iterator &other) const {
    return (neighbors == other.neighbors && index == other.index);
  }
...
class comm_edge_iterator
    : public iterator_facade<comm_edge_iterator, std::pair<int, int>,
                             forward_traversal_tag, const std::pair<int, int> &,
                             int> {
public:
  comm_edge_iterator() {}
...
template <> struct graph_traits<mpi::graph_communicator> {
   ...
   * INTERNAL ONLY
   */
  struct traversal_category : incidence_graph_tag,
                              adjacency_graph_tag,
 ...
 * INTERNAL ONLY
 */
template <> struct property_map<mpi::graph_communicator, vertex_index_t> {
