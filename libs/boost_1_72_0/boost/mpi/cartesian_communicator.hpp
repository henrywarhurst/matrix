...
template <> struct is_mpi_datatype<cartesian_dimension> : mpl::true_ {};

/**
 * @brief Test if the dimensions values are identical.
 */
inline bool operator==(cartesian_dimension const &d1,
 ...
 * This is a lightweight object, so that any constructor that could be
 * considered missing could be replaced with a function (move constructor
 * provided when supported).
 */
...
public:
   ...
   */
  cartesian_topology(int ndim) : super(ndim) {}
  ...
  /**
   * @brief Use dimensions specification provided in the sequence container as
   * initial values. #param dims must be a sequence container.
   */
  template <class InitArr>
  explicit cartesian_topology(InitArr dims) : super(0) {
    BOOST_FOREACH (cartesian_dimension const &d, dims) { push_back(d); }
  }
#if !defined(BOOST_NO_CXX11_HDR_INITIALIZER_LIST)
  /**
   * @brief Use dimensions specification provided in the initialization list as
   * initial values. #param dims can be of the form { dim_1, false}, ....
   * {dim_n, true}
   */
   ...
   * the missing values will be complete with zeros of the dim and assumed non
   * periodic.
  ...
  cartesian_topology(DimRg const &dim_rg, PerRg const &period_rg) : super(0) {
    BOOST_FOREACH (int d, dim_rg) { super::push_back(cartesian_dimension(d)); }
  ...
  cartesian_topology(DimIter dit, PerIter pit, int n) : super(n) {
    for (int i = 0; i < n; ++i) {
...
inline bool operator==(cartesian_topology const &t1,
...
public:
   ...
   *  will be based on.
   *
   *  @param dims the cartesian dimension of the new communicator. The size
   * indicate the number of dimension. Some dimensions be set to zero, in which
   * case the corresponding dimension value is left to the system.
   *
   ...
   * @param coords the coordinates. the size must match the communicator's
   * topology.
   */
  int rank(const std::vector<int> &coords) const;
  /**
   * Return the rank of the source and target destination process through a
   * shift.
   * @param dim the dimension in which the shift takes place. 0 <= dim <=
   * ndim().
   * @param disp the shift displacement, can be positive (upward) or negative
   * (downward).
   */
