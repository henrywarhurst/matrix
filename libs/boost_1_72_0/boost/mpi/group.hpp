 ...
 ************************************************************************/
template <typename InputIterator, typename OutputIterator>
OutputIterator group::translate_ranks(InputIterator first, InputIterator last,
  ...
  BOOST_MPI_CHECK_RESULT(MPI_Group_translate_ranks,
                         ((MPI_Group) * this, in_array.size(), &in_array[0],
                          (MPI_Group)to_group, &out_array[0]));
 ...
 */
template <>
BOOST_MPI_DECL int *group::translate_ranks(int *first, int *last,
                                           const group &to_group, int *out);
...
group group::include(InputIterator first, InputIterator last) {
  ...
  BOOST_MPI_CHECK_RESULT(
      MPI_Group_incl, ((MPI_Group) * this, ranks.size(), &ranks[0], &result));
...
group group::exclude(InputIterator first, InputIterator last) {
  ...
  MPI_Group result;
  BOOST_MPI_CHECK_RESULT(
      MPI_Group_excl, ((MPI_Group) * this, ranks.size(), &ranks[0], &result));
  return group(result, /*adopt=*/true);
}
