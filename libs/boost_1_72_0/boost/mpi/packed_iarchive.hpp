...
public:
  ...
  template <class T> void load_override(T &x, mpl::false_) {
    archive::detail::common_iarchive<packed_iarchive>::load_override(x);
  }

  // Load it directly using the primnivites
  template <class T> void load_override(T &x, mpl::true_) {
    iprimitive::load(x);
  }

  // Load all supported datatypes directly
  template <class T> void load_override(T &x) {
    typedef
        typename mpl::apply1<use_array_optimization,
                             BOOST_DEDUCED_TYPENAME remove_const<T>::type>::type
            use_optimized;
