...
public:
  ...
  packed_oarchive(MPI_Comm const &comm, buffer_type &b,
      ...
      : oprimitive(b, comm), archive::detail::common_oarchive<packed_oarchive>(
                                 flags) {}
  ...
  template <class T> void save_override(T const &x, mpl::false_) {
    archive::detail::common_oarchive<packed_oarchive>::save_override(x);
  }

  // Save it directly using the primitives
  template <class T> void save_override(T const &x, mpl::true_) {
    oprimitive::save(x);
  }

  // Save all supported datatypes directly
  template <class T> void save_override(T const &x) {
