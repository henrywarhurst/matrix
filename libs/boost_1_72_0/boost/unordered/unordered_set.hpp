...
public:
  ...
  template <class InputIt>
  unordered_set(InputIt, InputIt, size_type, const hasher &,
                const allocator_type &);

#if !defined(BOOST_NO_CXX11_HDR_INITIALIZER_LIST)
  unordered_set(std::initializer_list<value_type>, size_type,
                const allocator_type &);
  ...
  std::pair<iterator, bool> emplace(BOOST_FWD_REF(A0) a0, BOOST_FWD_REF(A1) a1,
                                    BOOST_FWD_REF(A2) a2) {
    return table_.emplace_unique(
        table::extractor::extract(boost::forward<A0>(a0),
                                  boost::forward<A1>(a1)),
...
public:
  ...
  size_type bucket_count() const BOOST_NOEXCEPT { return table_.bucket_count_; }
...
#endif
  template <typename, typename, typename, typename> friend class unordered_set;
...
public:
  ...
  explicit unordered_multiset(size_type, const hasher &,
                              const allocator_type &);
...
template <class InputIterator, class Hash, class Allocator>
unordered_multiset(InputIterator, InputIterator, std::size_t, Hash, Allocator)
    ->unordered_multiset<
        typename std::iterator_traits<InputIterator>::value_type, Hash,
        std::equal_to<typename std::iterator_traits<InputIterator>::value_type>,
        Allocator>;
...
unordered_set<T, H, P, A>::unordered_set(unordered_set const &other)
  ...
  if (other.table_.size_) {
    table_.copy_buckets(other.table_, boost::unordered::detail::true_type());
  }
}
...
template <class T, class H, class P, class A>
unordered_set<T, H, P, A>::unordered_set(BOOST_RV_REF(unordered_set) other,
                                         allocator_type const &a)
...
unordered_set<T, H, P, A>::unordered_set(InputIt f, InputIt l, size_type n,
                                         const hasher &hf,
                                         const allocator_type &a)
    : table_(boost::unordered::detail::initial_size(f, l, n), hf, key_equal(),
             a) {
  this->insert(f, l);
}
...
unordered_set<T, H, P, A>::operator=(std::initializer_list<value_type> list) {
  this->clear();
  this->insert(list.begin(), list.end());
  return *this;
}
...
unordered_multiset<T, H, P, A>::unordered_multiset(
    ...
    : table_(other.table_,
             unordered_multiset::value_allocator_traits::
                 select_on_container_copy_construction(other.get_allocator())) {
  if (other.table_.size_) {
    table_.copy_buckets(other.table_, boost::unordered::detail::false_type());
  }
}
...
template <class T, class H, class P, class A>
typename unordered_multiset<T, H, P, A>::iterator
unordered_multiset<T, H, P, A>::erase(const_iterator first,
                                      const_iterator last) {
  node_pointer last_node = table::get_node(last);
  if (first == last)
    return iterator(last_node);
...
template <typename N, typename T, typename A> class node_handle_set {
  ...
  template <class T2, class H2, class P2, class A2> friend class unordered_set;
...
template <typename N, typename T, typename A>
void swap(insert_return_type_set<N, T, A> &x,
          insert_return_type_set<N, T, A> &y) {
