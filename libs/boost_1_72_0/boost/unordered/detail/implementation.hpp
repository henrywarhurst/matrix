  ...
  struct BOOST_PP_CAT(has_, name) {                                            \
    struct impl {                                                              \
      ...
      BOOST_UNORDERED_DEFAULT_MEMBER(2, 2);                                    \
                                                                               \
      enum { value = sizeof(choice2::type) == sizeof(test<base>(choose())) };  \
    };                                                                         \
                                                                               \
...
template <typename Alloc, typename T> struct rebind_wrap {
  ...
  typedef typename boost::detail::if_true<value>::BOOST_NESTED_TEMPLATE
      then<Alloc, fallback>::type::BOOST_NESTED_TEMPLATE rebind<T>::other type;
};
} // namespace detail
} // namespace unordered
} // namespace boost
...
template <typename Alloc> struct allocator_traits {
  ...
  typedef BOOST_UNORDERED_DEFAULT_TYPE(Alloc, size_type, std::size_t) size_type;
...
template <typename Alloc, typename T> struct rebind_wrap {
  typedef typename std::allocator_traits<Alloc>::template rebind_alloc<T> type;
};
} // namespace detail
} // namespace unordered
} // namespace boost
...
template <typename A0> struct detect_boost_tuple {
  ...
  static choice1::type
  test(choice1, boost::tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9> const &);
...
construct_from_args(Alloc &alloc, std::pair<A, B> *address, BOOST_FWD_REF(A0),
  ...
  BOOST_CATCH(...) {
    boost::unordered::detail::func::destroy(boost::addressof(address->first));
    BOOST_RETHROW
  }
  BOOST_CATCH_END
}
...
#define BOOST_UNORDERED_CONSTRUCT_IMPL(z, num_params, _)                       \
  ...
  inline void construct_from_args(                                             \
    ...
    new ((void *)address) T(                                                   \
        BOOST_PP_ENUM_##z(num_params, BOOST_UNORDERED_CALL_FORWARD, args.a));  \
  }
...
public:
  ...
  bool operator==(l_iterator x) const BOOST_NOEXCEPT { return ptr_ == x.ptr_; }
...
public:
  ...
  template <typename T> inline node_pointer copy_of(T const &v) {
    ...
    BOOST_UNORDERED_CALL_CONSTRUCT1(node_allocator_traits, constructor_.alloc_,
...
public:
  ...
  static const bool nothrow_swappable = boost::is_nothrow_swappable<H>::value &&
...
template <typename T> struct rv_ref_impl { typedef BOOST_RV_REF(T) type; };
...
public:
  ...
  typedef
      typename boost::unordered::detail::rebind_wrap<value_allocator,
  ...
  std::size_t min_buckets_for_size(std::size_t size) const {
    ...
    return policy::new_bucket_count(boost::unordered::detail::double_to_size(
        floor(static_cast<double>(size) / static_cast<double>(mlf_)) + 1));
  }
  ...
  table(table &x, node_allocator const &a, boost::unordered::detail::move_tag m)
  ...
  void swap(table &x) {
    BOOST_ASSERT(
        allocator_traits<node_allocator>::propagate_on_container_swap::value ||
  ...
  iterator emplace_hint_unique(c_iterator hint, no_key,
    ...
    if (pos) {
      ...
      return iterator(this->resize_and_add_node_unique(b.release(), key_hash));
    }
  }
  ...
  void move_insert_node_type_unique(NodeType &np, InsertReturnType &result) {
    if (np) {
      ...
      if (pos) {
        ...
        this->reserve_for_insert(this->size_ + 1);
        result.position = iterator(this->add_node_unique(np.ptr_, key_hash));
  ...
  void copy_buckets(table const &src, true_type) {
    ...
    for (node_pointer n = src.begin(); n; n = next_node(n)) {
      ...
      this->add_node_unique(boost::unordered::detail::func::construct_node(
  ...
  iterator emplace_hint_equiv(c_iterator hint, node_pointer n) {
    ...
    if (hint.node_ && this->key_eq()(k, this->get_key(hint.node_))) {
      this->reserve_for_insert(this->size_ + 1);
      return iterator(this->add_using_hint_equiv(a.release(), hint.node_));
    } else {
      std::size_t key_hash = this->hash(k);
      node_pointer position = this->find_node(key_hash, k);
      this->reserve_for_insert(this->size_ + 1);
  ...
  iterator move_insert_node_type_with_hint_equiv(c_iterator hint,
    ...
    if (np) {
      ...
      if (hint.node_ && this->key_eq()(k, this->get_key(hint.node_))) {
        this->reserve_for_insert(this->size_ + 1);
        result = iterator(this->add_using_hint_equiv(np.ptr_, hint.node_));
      } else {
        std::size_t key_hash = this->hash(k);
        node_pointer pos = this->find_node(key_hash, k);
        this->reserve_for_insert(this->size_ + 1);
  ...
  void insert_range_equiv(
    ...
    if (distance == 1) {
      ...
      for (; i != j; ++i) {
        emplace_no_rehash_equiv(boost::unordered::detail::func::construct_node(
            this->node_alloc(), *i));
      }
    }
  }
  ...
  void copy_buckets(table const &src, false_type) {
    ...
    for (node_pointer n = src.begin(); n;) {
      ...
      node_pointer pos =
          this->add_node_equiv(boost::unordered::detail::func::construct_node(
                                   this->node_alloc(), n->value()),
  ...
  void move_assign_buckets(table &src, false_type) {
    ...
    for (node_pointer n = src.begin(); n;) {
      std::size_t key_hash = this->hash(this->get_key(n));
      node_pointer group_end(next_group(n));
      node_pointer pos = this->add_node_equiv(holder.move_copy_of(n->value()),
                                              key_hash, node_pointer());
      for (n = next_node(n); n != group_end; n = next_node(n)) {
        this->add_node_equiv(holder.move_copy_of(n->value()), key_hash, pos);
      }
    }
  }
};
...
struct node : boost::unordered::detail::value_base<T> {
  typedef typename ::boost::unordered::detail::rebind_wrap<A, node<A, T>>::type
      allocator;
  typedef
      typename ::boost::unordered::detail::allocator_traits<allocator>::pointer
          node_pointer;
