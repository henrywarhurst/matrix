...
void move_data_backward(
    RandIt cur_pos, typename iterator_traits<RandIt>::size_type const l_data,
    RandIt new_pos, bool const xbuf_used) {
  // Move buffer to the total combination right
  if (xbuf_used) {
    boost::move_backward(cur_pos, cur_pos + l_data, new_pos + l_data);
  } else {
    boost::adl_move_swap_ranges_backward(cur_pos, cur_pos + l_data,
                                         new_pos + l_data);
...
// output: [0, l_build_buf) elements are buffer, blocks 2*l_build_buf and last
// subblock sorted
//
...
//
// Once elements are merged left to right in blocks of l_build_buf, then a
// single left to right merge step is performed to achieve merged blocks of size
// 2K. If external memory is available, usual merge is used, swap merging
// otherwise.
//
...
typename iterator_traits<RandIt>::size_type adaptive_sort_build_blocks(
    RandIt const first, typename iterator_traits<RandIt>::size_type const len,
    typename iterator_traits<RandIt>::size_type const l_base,
    typename iterator_traits<RandIt>::size_type const l_build_buf, XBuf &xbuf,
    Compare comp) {
  ...
  // If there is no enough buffer for the insertion sort step, just avoid the
  // external buffer
  size_type kbuf =
      min_value<size_type>(l_build_buf, size_type(xbuf.capacity()));
  kbuf = kbuf < l_base ? 0 : kbuf;

  if (kbuf) {
    // Backup internal buffer values in external buffer so they can be
    // overwritten
    xbuf.move_assign(first + l_build_buf - kbuf, kbuf);
    l_merged = op_insertion_sort_step_left(first_block, elements_in_blocks,
                                           l_base, comp, move_op());
    ...
    l_merged = op_merge_left_step_multiple(
        first_block - l_merged, elements_in_blocks, l_merged, l_build_buf,
        kbuf - l_merged, comp, move_op());

    // Restore internal buffer from external buffer unless kbuf was l_build_buf,
    // in that case restoration will happen later
    if (kbuf != l_build_buf) {
      boost::move(xbuf.data() + kbuf - l_merged, xbuf.data() + kbuf,
                  first_block - l_merged + elements_in_blocks);
    }
  } else {
    l_merged =
        insertion_sort_step(first_block, elements_in_blocks, l_base, comp);
    rotate_gcd(first_block - l_merged, first_block,
               first_block + elements_in_blocks);
  }
  ...
  l_merged = op_merge_left_step_multiple(
      first_block - l_merged, elements_in_blocks, l_merged, l_build_buf,
      l_build_buf - l_merged, comp, swap_op());
  ...
  if (kbuf && kbuf == l_build_buf) {
    op_merge_right_step_once(first, elements_in_blocks, l_build_buf, comp,
                             move_op());
...
void adaptive_sort_combine_blocks(
    RandItKeys const keys, KeyCompare key_comp, RandIt const first,
    ...
    bool const use_buf, bool const xbuf_used, XBuf &xbuf, Compare comp,
    bool merge_left) {
  (void)xbuf;
  typedef typename iterator_traits<RandIt>::size_type size_type;
  ...
  size_type const l_total_combined =
      calculate_total_combined(len, l_prev_merged, &l_irreg_combined);
  ...
  if (merge_left || !use_buf) {
    for (size_type combined_i = 0; combined_i != max_i;) {
      ...
      size_type const l_cur_combined =
          is_last ? l_irreg_combined : l_reg_combined;

      range_xbuf<RandIt, size_type, move_op> rbuf(
          (use_buf && xbuf_used) ? (combined_first - l_block) : combined_first,
          combined_first);
      size_type n_block_a, n_block_b, l_irreg1, l_irreg2;
      combine_params(keys, key_comp, l_cur_combined, l_prev_merged, l_block,
                     rbuf, n_block_a, n_block_b, l_irreg1, l_irreg2); // Outputs
      BOOST_MOVE_ADAPTIVE_SORT_PRINT_L2("   A combpar:            ",
                                        len + l_block);
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(boost::movelib::is_sorted(
          combined_first, combined_first + n_block_a * l_block + l_irreg1,
          comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(boost::movelib::is_sorted(
          combined_first + n_block_a * l_block + l_irreg1,
          combined_first + n_block_a * l_block + l_irreg1 +
              n_block_b * l_block + l_irreg2,
          comp));
      if (!use_buf) {
        merge_blocks_bufferless(keys, key_comp, combined_first, l_block, 0u,
                                n_block_a, n_block_b, l_irreg2, comp);
      } else {
        merge_blocks_left(keys, key_comp, combined_first, l_block, 0u,
                          n_block_a, n_block_b, l_irreg2, comp, xbuf_used);
      }
      BOOST_MOVE_ADAPTIVE_SORT_PRINT_L2("   After merge_blocks_L: ",
                                        len + l_block);
      ++combined_i;
      if (combined_i != max_i)
    ...
    for (size_type combined_i = max_i; combined_i;) {
      ...
      range_xbuf<RandIt, size_type, move_op> rbuf(
          combined_last, xbuf_used ? (combined_last + l_block) : combined_last);
      size_type n_block_a, n_block_b, l_irreg1, l_irreg2;
      combine_params(keys, key_comp, l_cur_combined, l_prev_merged, l_block,
                     rbuf, n_block_a, n_block_b, l_irreg1, l_irreg2); // Outputs
      BOOST_MOVE_ADAPTIVE_SORT_PRINT_L2("   A combpar:            ",
      ...
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(boost::movelib::is_sorted(
          ...
          combined_first + n_block_a * l_block + l_irreg1 +
              n_block_b * l_block + l_irreg2,
          comp));
      merge_blocks_right(keys, key_comp, combined_first, l_block, n_block_a,
                         n_block_b, l_irreg2, comp, xbuf_used);
      BOOST_MOVE_ADAPTIVE_SORT_PRINT_L2("   After merge_blocks_R: ",
                                        len + l_block);
      if (combined_i)
        combined_first -= l_reg_combined;
    }
...
bool adaptive_sort_combine_all_blocks(
    RandIt keys, typename iterator_traits<RandIt>::size_type &n_keys,
    RandIt const buffer,
    typename iterator_traits<RandIt>::size_type const l_buf_plus_data,
    typename iterator_traits<RandIt>::size_type l_merged,
    typename iterator_traits<RandIt>::size_type &l_intbuf, XBuf &xbuf,
    Compare comp) {
  typedef typename iterator_traits<RandIt>::size_type size_type;
  ...
  bool const common_xbuf =
      l_data > l_merged && l_intbuf && l_intbuf <= xbuf.capacity();
  if (common_xbuf) {
  ...
  for (size_type n = 0; l_data > l_merged; l_merged *= 2, ++n) {
    // If l_intbuf is non-zero, use that internal buffer.
    //    Implies l_block == l_intbuf && use_internal_buf == true
    // If l_intbuf is zero, see if half keys can be reused as a reduced
    // emergency buffer,
    //    Implies l_block == n_keys/2 && use_internal_buf == true
    // Otherwise, just give up and and use all keys to merge using rotations
    // (use_internal_buf = false)
    bool use_internal_buf = false;
    size_type const l_block = lblock_for_combine(
        l_intbuf, n_keys, size_type(2 * l_merged), use_internal_buf);
    ...
    size_type const l_total_combined =
        calculate_total_combined(l_data, l_merged);
    if (n && prev_use_internal_buf && prev_merge_left) {
      if (is_merge_left || !use_internal_buf) {
        move_data_backward(first - l_prev_block, l_prev_total_combined, first,
                           common_xbuf);
      } else {
        ...
        if (l_prev_total_combined > l_total_combined) {
          ...
          move_data_backward(buf_beg - l_diff, l_diff, buf_end - l_diff,
                             common_xbuf);
      ...
      BOOST_MOVE_ADAPTIVE_SORT_PRINT_L2("   After move_data     : ",
                                        l_data + l_intbuf);
    }
    ...
    if (n_keys) {
      ...
      if (upper_n_keys_this_iter > 256) {
        adaptive_sort_combine_blocks(
            keys, comp,
            !use_internal_buf || is_merge_left ? first : first - l_block,
            l_data, l_merged, l_block, use_internal_buf, common_xbuf, xbuf,
            comp, is_merge_left);
      } else {
      ...
      adaptive_sort_combine_blocks(
          uint_keys, less(),
          !use_internal_buf || is_merge_left ? first : first - l_block, l_data,
          l_merged, l_block, use_internal_buf, common_xbuf, xbuf, comp,
          is_merge_left);
    }

    BOOST_MOVE_ADAPTIVE_SORT_PRINT_L1(is_merge_left
                                          ? "   After comb blocks L:  "
                                          : "   After comb blocks R:  ",
                                      l_data + l_intbuf);
...
template <class RandIt, class Compare, class XBuf>
void adaptive_sort_final_merge(
    bool buffer_right, RandIt const first,
    typename iterator_traits<RandIt>::size_type const l_intbuf,
    typename iterator_traits<RandIt>::size_type const n_keys,
    typename iterator_traits<RandIt>::size_type const len, XBuf &xbuf,
    Compare comp) {
  ...
  if (buffer_right) {
    // Use stable sort as some buffer elements might not be unique (see
    // non_unique_buf)
    stable_sort(first + len - l_intbuf, first + len, comp, xbuf);
    stable_merge(first + n_keys, first + len - l_intbuf, first + len,
                 antistable<Compare>(comp), xbuf);
    ...
    if (xbuf.capacity() >= n_key_plus_buf) {
      ...
      stable_merge(first + n_keys, first + n_key_plus_buf, first + len, comp,
                   xbuf);
      stable_merge(first, first + n_keys, first + len, comp, xbuf);
    } else {
...
bool adaptive_sort_build_params(RandIt first, Unsigned const len, Compare comp,
                                Unsigned &n_keys, Unsigned &l_intbuf,
                                Unsigned &l_base, Unsigned &l_build_buf,
                                XBuf &xbuf) {
  ...
  if (xbuf.template supports_aligned_trailing<size_type>(
          l_intbuf, (len - l_intbuf - 1) / l_intbuf + 1)) {
    n_keys = 0u;
    l_build_buf = l_intbuf;
  } else {
    // Try to achieve a l_build_buf of length l_intbuf*2, so that we can merge
    // with that l_intbuf*2 buffer in "build_blocks" and use half of them as
    // buffer and the other half as keys in combine_all_blocks. In that case
    // n_keys >= n_min_ideal_keys but by a small margin.
    //
    // If available memory is 2*sqrt(l), then only sqrt(l) unique keys are
    // needed, (to be used for keys in combine_all_blocks) as the whole
    //l_build_buf will be backuped in the buffer during build_blocks.
    bool const non_unique_buf = xbuf.capacity() >= l_intbuf;
    size_type const to_collect =
        non_unique_buf ? n_min_ideal_keys : l_intbuf * 2;
    size_type collected =
        collect_unique(first, first + len, to_collect, comp, xbuf);
    ...
    // If collected keys are not enough, try to fix n_keys and l_intbuf. If no
    // fix is possible (due to very low unique keys), then go to a slow sort
    // based on rotations.
    else {
      ...
      // AdaptiveSortInsertionSortThreshold is always power of two so the
      // minimum is power of two
...
//
// * First, 2*csqrtlen unique elements elements are extracted from elements to
// be
//   sorted and placed in the beginning of the range.
//
// * Step "build_blocks": In this nearly-classic merge step, 2*csqrtlen unique
// elements
...
//   2*csqrtlen unique elements are again the leading elements of the whole
//   range.
//
// * Step "combine_blocks": pairs of previously formed blocks are merged with a
// different
//   ("smart") algorithm to form blocks of 8*csqrtlen elements. This step is
//   slower than the "build_blocks" step and repeated iteratively (forming
//   blocks of 16*csqrtlen, 32*csqrtlen elements, etc) of until all trailing
//   (len-2*csqrtlen) elements are merged.
//
//   In "combine_blocks" len/csqrtlen elements used are as "keys" (markers) to
//   know if elements belong to the first or second block to be merged and
//   another leading csqrtlen elements are used as buffer. Explanation of the
//   "combine_blocks" step:
//
...
//         * Groups are selection-sorted by first or last element (depending
//         whether they are going
//           to be merged to left or right) and keys are reordered accordingly
//           as an imitation-buffer.
//         * Elements of each block pair are merged using the csqrtlen buffer
//         taking into account
//           if they belong to the first half or second half (marked by the
//           key).
//
// * In the final merge step leading elements (2*csqrtlen) are sorted and merged
// with
...
//
//    * If csqrtlen+len/csqrtlen are extracted, then only csqrtlen elements are
//    used
//      as buffer in the "build_blocks" step forming blocks of 2*csqrtlen
//      elements. This means that an additional "combine_blocks" step will be
//      needed to merge all elements.
//
//    * If no csqrtlen+len/csqrtlen elements can be extracted, but still more
//    than a minimum,
//      then reduces the number of elements used as buffer and keys in the
//      "build_blocks" and "combine_blocks" steps. If "combine_blocks" has no
//      enough keys due to this reduction then uses a rotation based smart
//      merge.
//
//    * If the minimum number of keys can't be extracted, a rotation-based
//    sorting is performed.
//
// * If auxiliary memory is more or equal than ceil(len/2), half-copying
// mergesort is used.
//
// * If auxiliary memory is more than csqrtlen+n_keys*sizeof(std::size_t),
//   then only csqrtlen elements need to be extracted and "combine_blocks" will
//   use integral keys to combine blocks.
//
// * If auxiliary memory is available, the "build_blocks" will be extended to
// build bigger blocks
//   using classic merge and "combine_blocks" will use bigger blocks when
//   merging.
template <class RandIt, class Compare, class XBuf>
void adaptive_sort_impl(RandIt first,
                        typename iterator_traits<RandIt>::size_type const len,
  ...
  if (len <= size_type(AdaptiveSortInsertionSortThreshold)) {
    ...
    // Calculate and extract needed unique elements. If a minimum is not
    // achieved fallback to a slow stable sort
    if (!adaptive_sort_build_params(first, len, comp, n_keys, l_intbuf, l_base,
                                    l_build_buf, xbuf)) {
      ...
      size_type const l_merged = adaptive_sort_build_blocks(
          ...
          len - n_key_plus_buf + l_build_buf, l_base, l_build_buf, xbuf, comp);
      BOOST_MOVE_ADAPTIVE_SORT_PRINT_L1("   After build_blocks:   ", len);

      // Non-trivial merge
      bool const buffer_right = adaptive_sort_combine_all_blocks(
          first, n_keys, first + n_keys, len - n_keys, l_merged, l_intbuf, xbuf,
          comp);

      // Sort keys and buffer and merge the whole sequence
      adaptive_sort_final_merge(buffer_right, first, l_intbuf, n_keys, len,
                                xbuf, comp);
    }
  }
}
...
//! <b>Effects</b>: Sorts the elements in the range [first, last) in ascending
//! order according
//!   to comparison functor "comp". The sort is stable (order of equal elements
//!   is guaranteed to be preserved). Performance is improved if additional raw
//!   storage is provided.
//!
//! <b>Requires</b>:
//!   - RandIt must meet the requirements of ValueSwappable and
//!   RandomAccessIterator.
//!   - The type of dereferenced RandIt must meet the requirements of
//!   MoveAssignable and MoveConstructible.
//!
//! <b>Parameters</b>:
//!   - first, last: the range of elements to sort
//!   - comp: comparison function object which returns true if the first
//!   argument is is ordered before the second.
//!   - uninitialized, uninitialized_len: raw storage starting on
//!   "uninitialized", able to hold "uninitialized_len"
//!      elements of type iterator_traits<RandIt>::value_type. Maximum
//!      performance is achieved when uninitialized_len is
//!      ceil(std::distance(first, last)/2).
//!
//! <b>Throws</b>: If comp throws or the move constructor, move assignment or
//! swap of the type
//!   of dereferenced RandIt throws.
//!
//! <b>Complexity</b>: Always K x O(Nxlog(N)) comparisons and move
//! assignments/constructors/swaps.
//!   Comparisons are close to minimum even with no additional memory. Constant
//!   factor for data movement is minimized when uninitialized_len is
//!   ceil(std::distance(first, last)/2). Pretty good enough performance is
//!   achieved when ceil(sqrt(std::distance(first, last)))*2.
//!
//! <b>Caution</b>: Experimental implementation, not production-ready.
template <class RandIt, class RandRawIt, class Compare>
void adaptive_sort(
    RandIt first, RandIt last, Compare comp, RandRawIt uninitialized,
    typename iterator_traits<RandIt>::size_type uninitialized_len) {
  typedef typename iterator_traits<RandIt>::size_type size_type;
  typedef typename iterator_traits<RandIt>::value_type value_type;

  ::boost::movelib::adaptive_xbuf<value_type, RandRawIt, size_type> xbuf(
      uninitialized, uninitialized_len);
  ::boost::movelib::detail_adaptive::adaptive_sort_impl(
      first, size_type(last - first), comp, xbuf);
}
