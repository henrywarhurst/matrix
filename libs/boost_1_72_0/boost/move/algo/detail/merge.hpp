...
template <class T, class RandRawIt = T *,
          class SizeType = typename iterator_traits<RandRawIt>::size_type>
class adaptive_xbuf {
...
public:
  ...
  adaptive_xbuf() : m_ptr(), m_size(0), m_capacity(0) {}

  adaptive_xbuf(RandRawIt raw_memory, size_type capacity)
      : m_ptr(raw_memory), m_size(0), m_capacity(capacity) {}

  template <class RandIt> void move_assign(RandIt first, size_type n) {
    if (n <= m_size) {
  ...
  template <class RandIt> void push_back(RandIt first, size_type n) {
    BOOST_ASSERT(m_capacity - m_size >= n);
    boost::uninitialized_move(first, first + n, m_ptr + m_size);
    m_size += n;
  }

  template <class RandIt> iterator add(RandIt it) {
  ...
  template <class RandIt> void insert(iterator pos, RandIt it) {
    if (pos == (m_ptr + m_size)) {
      this->add(it);
    } else {
...
private:
  template <class RIt> static bool is_raw_ptr(RIt) { return false; }

  static bool is_raw_ptr(T *) { return true; }
...
public:
  ...
  bool supports_aligned_trailing(size_type size, size_type trail_count) const {
    if (this->is_raw_ptr(this->data()) && m_capacity) {
      ...
      return (u_addr_cp >= u_addr_sz) &&
             ((u_addr_cp - u_addr_sz) / sizeof(U) >= trail_count);
    }
    return false;
  }

  template <class U> U *aligned_trailing() const {
    return this->aligned_trailing<U>(this->size());
  }

  template <class U> U *aligned_trailing(size_type pos) const {
    uintptr_t u_addr = uintptr_t(&*(this->data() + pos));
    u_addr = ((u_addr + sizeof(U) - 1) / sizeof(U)) * sizeof(U);
    return (U *)u_addr;
  }

  ~adaptive_xbuf() { this->clear(); }

  size_type capacity() const { return m_capacity; }

  iterator data() const { return m_ptr; }
  ...
  void clear() { this->shrink_to_fit(0u); }
...
template <class Iterator, class SizeType, class Op> class range_xbuf {
...
public:
  ...
  size_type capacity() const { return m_cap - m_first; }

  Iterator data() const { return m_first; }
...
// Modified version from "An Optimal In-Place Array Rotation Algorithm",
// Ching-Kuang Shene
template <typename Unsigned> Unsigned gcd(Unsigned x, Unsigned y) {
  if (0 == ((x & (x - 1)) | (y & (y - 1)))) {
    return x < y ? x : y;
  } else {
    Unsigned z = 1;
    ...
    for (RandIt it_i(first), it_gcd(it_i + gcd(length, middle_pos));
      ...
      do {
        *it_j = boost::move(*it_k);
        ...
        it_k =
            left > middle_pos ? it_j + middle_pos : first + (middle_pos - left);
      } while (it_k != it_i);
...
RandIt lower_bound(RandIt first, const RandIt last, const T &key,
                   Compare comp) {
  typedef typename iterator_traits<RandIt>::size_type size_type;
...
template <class RandIt, class T, class Compare>
RandIt upper_bound(RandIt first, const RandIt last, const T &key,
                   Compare comp) {
  typedef typename iterator_traits<RandIt>::size_type size_type;
  size_type len = size_type(last - first);
  RandIt middle;
...
void op_merge_left(RandIt buf_first, RandIt first1, RandIt const last1,
                   RandIt const last2, Compare comp, Op op) {
  ...
  if (buf_first !=
      first1) { // In case all remaining elements are in the same place
...
// Note: distance(buf_first, first1) >= distance(last1, last2), so no
// overlapping occurs
template <class RandIt, class Compare>
void merge_left(RandIt buf_first, RandIt first1, RandIt const last1,
                RandIt const last2, Compare comp) {
...
// overlapping occurs
template <class RandIt, class Compare>
void swap_merge_left(RandIt buf_first, RandIt first1, RandIt const last1,
                     RandIt const last2, Compare comp) {
  op_merge_left(buf_first, first1, last1, last2, comp, swap_op());
}

template <class RandIt, class Compare, class Op>
void op_merge_right(RandIt const first1, RandIt last1, RandIt last2,
                    RandIt buf_last, Compare comp, Op op) {
  ...
  if (last2 !=
      buf_last) { // In case all remaining elements are in the same place
...
// Note: distance[last2, buf_last) >= distance[first1, last1), so no overlapping
// occurs
template <class RandIt, class Compare>
void merge_right(RandIt first1, RandIt last1, RandIt last2, RandIt buf_last,
                 Compare comp) {
...
// occurs
template <class RandIt, class Compare>
void swap_merge_right(RandIt first1, RandIt last1, RandIt last2,
                      RandIt buf_last, Compare comp) {
...
void op_buffered_merge(RandIt first, RandIt const middle, RandIt last,
                       Compare comp, Op op, Buf &xbuf) {
  if (first != middle && middle != last && comp(*middle, middle[-1])) {
    ...
    if (len1 <= len2) {
      ...
      op_merge_with_right_placed(xbuf.data(), xbuf.end(), first, middle, last,
                                 comp, op);
    } else {
      last = boost::movelib::lower_bound(middle, last, middle[-1], comp);
      xbuf.move_assign(middle, size_type(last - middle));
      op_merge_with_left_placed(first, middle, last, xbuf.data(), xbuf.end(),
                                comp, op);
    }
  }
}
...
void buffered_merge(RandIt first, RandIt const middle, RandIt last,
...
template <class RandIt, class Compare>
void merge_bufferless_ON2(RandIt first, RandIt middle, RandIt last,
                          Compare comp) {
...
void merge_bufferless_ONlogN_recursive(
    ...
    typename iterator_traits<RandIt>::size_type len2, Compare comp) {
  typedef typename iterator_traits<RandIt>::size_type size_type;
  ...
  while (1) {
    ...
    if (!len2) {
      ...
      return;
    } else if (size_type(len1 + len2) <
               MergeBufferlessONLogNRotationThreshold) {
    ...
    // Avoid one recursive call doing a manual tail call elimination on the
    // biggest range
    const size_type len_internal = len11 + len22;
    if (len_internal < (len1 + len2 - len_internal)) {
      merge_bufferless_ONlogN_recursive(first, first_cut, new_middle, len11,
                                        len22, comp);
      first = new_middle;
      middle = second_cut;
      len1 -= len11;
      ...
      merge_bufferless_ONlogN_recursive(new_middle, second_cut, last,
...
// Complexity: NlogN
template <class RandIt, class Compare>
void merge_bufferless_ONlogN(RandIt first, RandIt middle, RandIt last,
                             Compare comp) {
  typedef typename iterator_traits<RandIt>::size_type size_type;
  merge_bufferless_ONlogN_recursive(first, middle, last,
                                    size_type(middle - first),
                                    size_type(last - middle), comp);
}
...
void op_merge_with_right_placed(InputIterator first, InputIterator last,
                                InputOutIterator dest_first,
                                InputOutIterator r_first,
                                InputOutIterator r_last, Compare comp, Op op) {
...
void swap_merge_with_right_placed(InputIterator first, InputIterator last,
                                  InputOutIterator dest_first,
                                  InputOutIterator r_first,
                                  InputOutIterator r_last, Compare comp) {
  op_merge_with_right_placed(first, last, dest_first, r_first, r_last, comp,
                             swap_op());
}
...
void op_merge_with_left_placed(BidirOutIterator const first,
                               BidirOutIterator last,
                               BidirOutIterator dest_last,
                               BidirIterator const r_first,
                               BidirIterator r_last, Compare comp, Op op) {
...
void merge_with_left_placed(BidirOutIterator const first, BidirOutIterator last,
                            BidirOutIterator dest_last,
                            BidirIterator const r_first, BidirIterator r_last,
                            Compare comp) {
  op_merge_with_left_placed(first, last, dest_last, r_first, r_last, comp,
                            move_op());
}

// [r_first, r_last) are already in the right part of the destination range.
template <class Compare, class InputIterator, class InputOutIterator>
void merge_with_right_placed(InputIterator first, InputIterator last,
                             InputOutIterator dest_first,
                             InputOutIterator r_first, InputOutIterator r_last,
                             Compare comp) {
  op_merge_with_right_placed(first, last, dest_first, r_first, r_last, comp,
                             move_op());
}
...
void uninitialized_merge_with_right_placed(
    InputIterator first, InputIterator last, InputOutIterator dest_first,
    InputOutIterator r_first, InputOutIterator r_last, Compare comp) {
  BOOST_ASSERT((last - first) == (r_first - dest_first));
  ...
  while (first != last && dest_first != original_r_first) {
    if (r_first == r_last) {
      for (; dest_first != original_r_first; ++dest_first, ++first) {
        ::new ((iterator_to_raw_pointer)(dest_first))
            value_type(::boost::move(*first));
        d.incr();
      }
      d.release();
...
void uninitialized_merge_with_left_placed
   ( BidirOutIterator dest_first, BidirOutIterator r_first, BidirOutIterator
r_last , BidirIterator first, BidirIterator last , Compare comp)
{
   ...
   d.release();
   merge_with_right_placed(first, last, original_r_first, r_first, r_last,
comp);
}
*/
...
BidirectionalIterator1 rotate_adaptive(
    BidirectionalIterator1 first, BidirectionalIterator1 middle,
...
template <typename BidirectionalIterator, typename Pointer, typename Compare>
void merge_adaptive_ONlogN_recursive(
  ...
  BidirectionalIterator new_middle =
      rotate_adaptive(first_cut, middle, second_cut, size_type(len1 - len11),
                      len22, buffer, buffer_size);
  merge_adaptive_ONlogN_recursive(first, first_cut, new_middle, len11, len22,
                                  buffer, buffer_size, comp);
  merge_adaptive_ONlogN_recursive(new_middle, second_cut, last, len1 - len11,
                                  len2 - len22, buffer, buffer_size, comp);
}

template <typename BidirectionalIterator, typename Compare, typename RandRawIt>
void merge_adaptive_ONlogN(
    BidirectionalIterator first, BidirectionalIterator middle,
    BidirectionalIterator last, Compare comp, RandRawIt uninitialized,
    typename iterator_traits<BidirectionalIterator>::size_type
        uninitialized_len) {
  typedef
      typename iterator_traits<BidirectionalIterator>::value_type value_type;
  typedef typename iterator_traits<BidirectionalIterator>::size_type size_type;
  ...
  if (uninitialized_len) {
    ...
    ::boost::movelib::adaptive_xbuf<value_type, RandRawIt> xbuf(
        uninitialized, uninitialized_len);
    xbuf.initialize_until(uninitialized_len, *first);
    merge_adaptive_ONlogN_recursive(first, middle, last, len1, len2,
                                    xbuf.begin(), uninitialized_len, comp);
  } else {
