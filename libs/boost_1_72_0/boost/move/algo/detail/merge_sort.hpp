...
void merge_sort_copy(RandIt first, RandIt last, RandIt2 dest, Compare comp) {
  typedef typename iterator_traits<RandIt>::size_type size_type;
  ...
  if (count <= MergeSortInsertionSortThreshold) {
    ...
    merge_with_right_placed(first + half, first + half + half, dest,
                            dest + half, dest + count, comp);
  }
}

template <class RandIt, class RandItRaw, class Compare>
void merge_sort_uninitialized_copy(RandIt first, RandIt last,
                                   RandItRaw uninitialized, Compare comp) {
  typedef typename iterator_traits<RandIt>::size_type size_type;
  ...
  if (count <= MergeSortInsertionSortThreshold) {
    ...
    size_type const half = count / 2;
    merge_sort_uninitialized_copy(first + half, last, uninitialized + half,
                                  comp);
    destruct_n<value_type, RandItRaw> d(uninitialized + half);
    d.incr(count - half);
    merge_sort_copy(first, first + half, first + half, comp);
    uninitialized_merge_with_right_placed(first + half, first + half + half,
                                          uninitialized, uninitialized + half,
                                          uninitialized + count, comp);
    d.release();
  }
}
...
void merge_sort(RandIt first, RandIt last, Compare comp,
  ...
  if (count <= MergeSortInsertionSortThreshold) {
    ...
    merge_with_right_placed(uninitialized, uninitialized + rest, first, rest_it,
                            last, antistable<Compare>(comp));
  }
}
...
template <class RandIt, class RandItRaw, class Compare>
void merge_sort_with_constructed_buffer(RandIt first, RandIt last, Compare comp,
                                        RandItRaw buffer) {
  typedef typename iterator_traits<RandIt>::size_type size_type;
  ...
  if (count <= MergeSortInsertionSortThreshold) {
    ...
    merge_sort_copy(first, half_it, rest_it, comp);
    merge_with_right_placed(buffer, buffer + rest, first, rest_it, last,
                            antistable<Compare>(comp));
  }
}

template <typename RandIt, typename Pointer, typename Distance,
          typename Compare>
void stable_sort_ONlogN_recursive(RandIt first, RandIt last, Pointer buffer,
  ...
  if (size_type(last - first) <= size_type(MergeSortInsertionSortThreshold)) {
    ...
    merge_adaptive_ONlogN_recursive(
        first, middle, last, size_type(middle - first),
        size_type(last - middle), buffer, buffer_size, comp);
  }
}
...
void stable_sort_adaptive_ONlogN2(BidirectionalIterator first,
  ...
  stable_sort_ONlogN_recursive(first, last, uninitialized, uninitialized_len,
                               comp);
}
