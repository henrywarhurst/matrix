...
//
// This implementation is the adaptation by Ion Gaztanaga of code originally in
// GitHub with permission from the author to relicense it under the Boost
// Software License (see the Boost mailing list for details).
//
...
template <class Unsigned> Unsigned log2(Unsigned n) {
  Unsigned log = 0;
  while (n >>= 1)
    ++log;
  return log;
...
template <class Iter, class Compare>
pdqsort_detail::pair<Iter, bool> partition_right(Iter begin, Iter end,
                                                 Compare comp) {
  typedef typename boost::movelib::iterator_traits<Iter>::value_type T;
...
void pdqsort_loop(
  ...
  while (true) {
    ...
    pdqsort_detail::pair<Iter, bool> part_result =
    ...
    if (highly_unbalanced) {
      ...
      if (l_size >= insertion_sort_threshold) {
        ...
        if (l_size > ninther_threshold) {
          ...
          boost::adl_move_iter_swap(pivot_pos - 2,
                                    pivot_pos - (l_size / 4 + 1));
          boost::adl_move_iter_swap(pivot_pos - 3,
...
void pdqsort(Iter begin, Iter end, Compare comp) {
  ...
  pdqsort_detail::pdqsort_loop<Iter, Compare>(
      begin, end, comp, pdqsort_detail::log2(size_type(end - begin)));
}
