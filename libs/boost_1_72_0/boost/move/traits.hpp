...
//! <pre>boost::is_nothrow_move_constructible<T>::value &&
//! boost::is_nothrow_move_assignable<T>::value </pre>. Classes with
//! non-throwing move constructor and assignment can specialize this trait to
//! obtain some performance improvements.
...
template <class T> struct is_nothrow_move_constructible_or_uncopyable {
  // The standard requires is_nothrow_move_constructible for move_if_noexcept
  // but a user (usually in C++03) might specialize has_nothrow_move which
  // includes it
