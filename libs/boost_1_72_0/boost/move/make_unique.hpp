...
//!
//! This header can be a bit heavyweight in C++03 compilers due to the use of
//! the preprocessor library, that's why it's a a separate header from
//! <tt>unique_ptr.hpp</tt>
...
template <class T> struct unique_ptr_if {
  typedef ::boost::movelib::unique_ptr<T> t_is_not_array;
};

template <class T> struct unique_ptr_if<T[]> {
  typedef ::boost::movelib::unique_ptr<T[]> t_is_array_of_unknown_bound;
};

template <class T, std::size_t N> struct unique_ptr_if<T[N]> {
  typedef void t_is_array_of_known_bound;
};

template <int Dummy = 0> struct nothrow_holder {
  static std::nothrow_t *pnothrow;
};

template <int Dummy>
std::nothrow_t *
    nothrow_holder<Dummy>::pnothrow = reinterpret_cast<std::nothrow_t *>(
        0x1234); // Avoid reference to null errors in sanitizers
...
//! <b>Remarks</b>: This function shall not participate in overload resolution
//! unless T is not an array.
//!
//! <b>Returns</b>: <tt>unique_ptr<T>(new T(std::forward<Args>(args)...))</tt>.
template <class T, class... Args>
inline BOOST_MOVE_DOC1ST(
    unique_ptr<T>,
...
//! unless T is not an array.
//!
//! <b>Returns</b>: <tt>unique_ptr<T>(new
//! T(std::nothrow)(std::forward<Args>(args)...))</tt>.
template <class T, class... Args>
inline BOOST_MOVE_DOC1ST(
    unique_ptr<T>,
    typename ::boost::move_upmu::unique_ptr_if<T>::t_is_not_array)
    make_unique_nothrow(BOOST_FWD_REF(Args)... args) {
  return unique_ptr<T>(new (*boost::move_upmu::nothrow_holder<>::pnothrow)
...
#define BOOST_MOVE_MAKE_UNIQUE_CODE(N)                                         \
  ...
  typename ::boost::move_upmu::unique_ptr_if<T>::t_is_not_array make_unique(   \
      BOOST_MOVE_UREF##N) {                                                    \
...
//! unless T is not an array.
//!
//! <b>Returns</b>: <tt>unique_ptr<T>(new T(std::nothrow)</tt> (default
//! initialization)
template <class T>
inline BOOST_MOVE_DOC1ST(
    unique_ptr<T>,
...
//! <b>Remarks</b>: This function shall not participate in overload resolution
//! unless T is an array of
//!   unknown bound.
//!
//! <b>Returns</b>: <tt>unique_ptr<T>(new remove_extent_t<T>[n]())</tt> (value
//! initialization)
template <class T>
inline BOOST_MOVE_DOC1ST(
    unique_ptr<T>,
...
//!
//! <b>Returns</b>: <tt>unique_ptr<T>(new
//! (std::nothrow)remove_extent_t<T>[n]())</tt> (value initialization)
template <class T>
inline BOOST_MOVE_DOC1ST(
    unique_ptr<T>,
    typename ::boost::move_upmu::unique_ptr_if<T>::t_is_array_of_unknown_bound)
...
//!   unknown bound.
//!
//! <b>Returns</b>: <tt>unique_ptr<T>(new remove_extent_t<T>[n])</tt> (default
//! initialization)
template <class T>
inline BOOST_MOVE_DOC1ST(
    unique_ptr<T>,
...
//!
//! <b>Returns</b>: <tt>unique_ptr<T>(new
//! (std::nothrow)remove_extent_t<T>[n])</tt> (default initialization)
template <class T>
inline BOOST_MOVE_DOC1ST(
    unique_ptr<T>,
    typename ::boost::move_upmu::unique_ptr_if<T>::t_is_array_of_unknown_bound)
