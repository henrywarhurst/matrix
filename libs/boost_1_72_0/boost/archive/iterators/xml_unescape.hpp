...
template <class Base>
class xml_unescape : public unescape<xml_unescape<Base>, Base> {
...
// msvc versions prior to 14.0 crash with and ICE
  ...
  xml_unescape(const xml_unescape &rhs) : super_t(rhs.base_reference()) {}
};
  ...
  do {
    if (*literal != *++(this->base_reference()))
      boost::serialization::throw_exception(
          dataflow_exception(dataflow_exception::invalid_xml_escape_sequence));
  } while ('\0' != *++literal);
}
