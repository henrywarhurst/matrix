...
class wchar_from_mb
  ...
  template <typename T> struct sliding_buffer {
    ...
    sliding_buffer(const sliding_buffer &rhs)
        : m_next_available(std::copy(rhs.m_buffer.begin(), rhs.m_next_available,
                                     m_buffer.begin())),
...
template <class Base> void wchar_from_mb<Base>::drain() {
  ...
  BOOST_ATTRIBUTE_UNUSED // redundant with ignore_unused below but clarifies
                         // intention
                             std::codecvt_base::result r = m_codecvt_facet.in(
                                 m_mbs, m_input.m_buffer.begin(),
                                 m_input.m_next_available, input_new_start,
                                 m_output.m_buffer.begin(),
                                 m_output.m_buffer.end(), next_available);
