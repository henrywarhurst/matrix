...
protected:
  ...
  void load_override(class_id_type &t) {
     ...
     * 16bit class_id_type 2) in boost 1.44 this load_override has disappeared
     * (and thus boost 1.44 is not backward compatible at all !!) 3) recent
     * boosts reintroduced load_override with a test on the version :
     *     - v > 7 : this->detail_common_iarchive::load_override(t, version)
     *     - v > 6 : 16bit
     *     - other : 32bit
     *   --> which is obviously incorrect, see point 1
     *
     * the fix here decodes class_id_type on 16bit for all v <= 7, which seems
