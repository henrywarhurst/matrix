...
template <typename T> struct value : expression::terminal<T> {
...
template <typename Expr> struct is_custom_terminal<actor<Expr>> : mpl::true_ {};

// Special handling for actor
template <typename Expr> struct custom_terminal<actor<Expr>> {
...
template <typename T, int N> struct decay<T[N]> : decay<T const *> {};
} // namespace meta

template <typename T> struct as_actor<T, mpl::false_> {
