...
template <typename T, typename Dummy> struct custom_terminal;
...
struct terminal : proto::terminal<T> {
  typedef proto::basic_expr<proto::tag::terminal
                            // tag::terminal //cannot change to use phoenix tag
                            // - breaks code.
                            ,
                            proto::term<T>, 0>
      base_type;
...
namespace detail {
template <typename N> struct placeholder_idx : mpl::int_<N::value> {};
} // namespace detail
