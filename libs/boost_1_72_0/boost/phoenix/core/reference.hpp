...
template <typename T>
struct reference : expression::terminal<reference_wrapper<T>> {
...
template <typename T>
struct reference<T const> : expression::terminal<reference_wrapper<T const>> {
...
namespace rule {
struct reference : expression::reference<proto::_> {};
} // namespace rule

template <typename T>
inline typename expression::reference<T>::type const ref(T &t) {
  return expression::reference<T>::make(t);
}

template <typename T>
inline typename expression::reference<T const>::type const cref(T const &t) {
  return expression::reference<T const>::make(t);
}

// Call out boost::reference_wrapper for special handling
template <typename T>
struct is_custom_terminal<boost::reference_wrapper<T>> : mpl::true_ {};

// Special handling for boost::reference_wrapper
template <typename T> struct custom_terminal<boost::reference_wrapper<T>> {
...
struct custom_terminal<boost::reference_wrapper<actor<Expr>>> {
  ...
  operator()(boost::reference_wrapper<actor<Expr>> &r,
...
struct custom_terminal<boost::reference_wrapper<actor<Expr> const>> {
  ...
  template <typename This, typename Context>
  struct result<This(boost::reference_wrapper<actor<Expr> const> const &,
                     Context)>
  ...
  template <typename Context>
  typename boost::result_of<evaluator(actor<Expr> const &,
                                      Context const &)>::type
  operator()(boost::reference_wrapper<actor<Expr> const> const &r,
             Context &ctx) const {
