...
#pragma wave option(preserve : 2, line : 0, output                             \
                    : "preprocessed/expression_" BOOST_PHOENIX_LIMIT_STR       \
                      ".hpp")
#endif
...
template <template <typename> class Actor, typename Tag,
...
template <typename Tag,
          ...
          typename Dummy = void>
struct expr
    : expr_ext<actor, Tag, BOOST_PHOENIX_A(BOOST_PHOENIX_COMPOSITE_LIMIT)> {};
...
#define M1(Z, N, D)                                                            \
  BOOST_PP_COMMA_IF(N)                                                         \
  typename boost::add_reference<typename boost::add_const<BOOST_PP_CAT(        \
      A, N)>::type>::type BOOST_PP_CAT(a, N)
...
struct expr_ext<Actor, Tag, BOOST_PHOENIX_A>
  ...
  typedef typename proto::result_of::make_expr<
      ...
      ,
      BOOST_PP_REPEAT(BOOST_PHOENIX_ITERATION, M0, _)>::type base_type;

  typedef Actor<base_type> type;

  typedef typename proto::nary_expr<Tag, BOOST_PHOENIX_A>::proto_grammar
      proto_grammar;

  static type make(BOOST_PP_REPEAT(BOOST_PHOENIX_ITERATION, M1,
                                   _)) { //?? actor or Actor??
