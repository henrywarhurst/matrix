...
#pragma wave option(preserve : 2, line : 0, output                             \
                    : "preprocessed/function_eval_" BOOST_PHOENIX_LIMIT_STR    \
                      ".hpp")
#endif
/*=============================================================================
...
#define PHOENIX_GET_ARG(z, n, data)                                            \
  typedef typename boost::add_reference<typename boost::add_const<             \
      typename boost::result_of<boost::phoenix::evaluator(                     \
          BOOST_PP_CAT(A, n), Context)>::type>::type>::type BOOST_PP_CAT(a,    \
                                                                         n);

#define PHOENIX_EVAL_ARG(z, n, data)                                           \
  help_rvalue_deduction(boost::phoenix::eval(BOOST_PP_CAT(a, n), ctx))

#define M0(z, n, data) typename proto::detail::uncvref<BOOST_PP_CAT(a, n)>::type
...
#else
template <typename This, typename F, BOOST_PHOENIX_typename_A, typename Context>
struct result<This(F, BOOST_PHOENIX_A, Context)> {
  ...
  typedef typename boost::result_of<fn(BOOST_PHOENIX_a)>::type type;
  /*
  typedef typename
      mpl::eval_if_c<
          has_phx2_result<
...
typename result<function_eval(F const &, BOOST_PHOENIX_A_ref,
                              Context const &)>::type
operator()(F const &f, BOOST_PHOENIX_A_ref_a, Context const &ctx) const {
  return boost::phoenix::eval(f, ctx)(
      BOOST_PP_ENUM(BOOST_PHOENIX_ITERATION, PHOENIX_EVAL_ARG, _));
}

template <typename F, BOOST_PHOENIX_typename_A, typename Context>
typename result<function_eval(F &, BOOST_PHOENIX_A_ref, Context const &)>::type
