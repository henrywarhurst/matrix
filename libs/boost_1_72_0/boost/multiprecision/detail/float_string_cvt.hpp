...
//
// Generic routines for converting floating point values to and from decimal
// strings. Note that these use "naive" algorithms which result in rounding
// error - so they do not round trip to and from the string representation (but
// should only be out in the last bit).
//
...
std::string convert_to_string(Backend b, std::streamsize digits,
  ...
  if (fpt == (int)FP_ZERO) {
    ...
    if (b.compare(ui_type(0)) < 0)
      ...
      return ((f & std::ios_base::showpos) == std::ios_base::showpos) ? "+inf"
                                                                      : "inf";
...
template <class Backend> void convert_from_string(Backend &b, const char *p) {
  ...
  static const int max_digits =
      limits::is_specialized ? limits::max_digits10 + 1 : INT_MAX;
