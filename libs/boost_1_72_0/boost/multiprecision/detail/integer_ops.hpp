...
inline BOOST_MP_CXX14_CONSTEXPR void eval_qr(const Backend &x, const Backend &y,
                                             Backend &q, Backend &r) {
  eval_divide(q, x, y);
  eval_modulus(r, x, y);
}
...
inline BOOST_MP_CXX14_CONSTEXPR Integer eval_integer_modulus(const Backend &x,
                                                             Integer val) {
  BOOST_MP_USING_ABS
...
template <class B>
inline BOOST_MP_CXX14_CONSTEXPR void eval_gcd(B &result, const B &a,
                                              const B &b) {
...
template <class B>
inline BOOST_MP_CXX14_CONSTEXPR void eval_lcm(B &result, const B &a,
                                              const B &b) {
...
inline BOOST_MP_CXX14_CONSTEXPR typename enable_if_c<
    number_category<Backend>::value == number_kind_integer>::type
divide_qr(const multiprecision::detail::expression<tag, A1, A2, A3, A4> &x,
...
//
template <class Backend>
BOOST_MP_CXX14_CONSTEXPR void eval_powm(Backend &result, const Backend &a,
                                        const Backend &p, const Backend &c) {
  using default_ops::eval_bit_test;
  using default_ops::eval_get_sign;
  using default_ops::eval_modulus;
...
template <class Backend, class Integer>
BOOST_MP_CXX14_CONSTEXPR typename enable_if<is_unsigned<Integer>>::type
...
template <class Backend, class Integer>
BOOST_MP_CXX14_CONSTEXPR typename enable_if<is_signed<Integer>>::type
eval_powm(Backend &result, const Backend &a, Integer b, const Backend &c) {
  if (b < 0) {
    BOOST_THROW_EXCEPTION(
        std::runtime_error("powm requires a positive exponent."));
...
BOOST_MP_CXX14_CONSTEXPR typename enable_if<is_unsigned<Integer1>>::type
...
template <class Backend, class Integer1, class Integer2>
BOOST_MP_CXX14_CONSTEXPR typename enable_if<is_signed<Integer1>>::type
eval_powm(Backend &result, const Backend &a, Integer1 b, Integer2 c) {
  if (b < 0) {
    BOOST_THROW_EXCEPTION(
        std::runtime_error("powm requires a positive exponent."));
...
struct powm_func {
  ...
  BOOST_MP_CXX14_CONSTEXPR void operator()(T &result, const T &b, const U &p,
                                           const V &m) const {
