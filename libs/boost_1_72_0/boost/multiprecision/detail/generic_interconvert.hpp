...
template <class To, class From> inline To do_cast(const From &from) {
  return static_cast<To>(from);
}
template <class To, class B,
...
void generic_interconvert(
  ...
  // smallest unsigned type handled natively by "From" is likely to be it's
  // limb_type:
...
#pragma warning(disable : 4127)
#endif
  //
  // The code here only works when the radix of "From" is 2, we could try
  // shifting by other radixes but it would complicate things.... use a string
  // conversion when the radix is other than 2:
  //
  ...
  while (!eval_is_zero(f)) {
    ...
    typename boost::multiprecision::detail::canonical<boost::intmax_t, To>::type
        ll;
...
R safe_convert_to_float(const LargeInteger &i) {
  ...
  if (std::numeric_limits<R>::is_specialized &&
      std::numeric_limits<R>::max_exponent) {
    LargeInteger val(i);
    if (val.sign() < 0)
      val = -val;
    unsigned mb = msb(val);
...
inline typename disable_if_c<is_number<To>::value ||
                             is_floating_point<To>::value>::type
generic_convert_rational_to_float_imp(To &result, const Integer &n,
                                      const Integer &d, const mpl::true_ &) {
  //
  ...
  number<To> fn(safe_convert_to_float<number<To>>(n)),
      fd(safe_convert_to_float<number<To>>(d));
  eval_divide(result, fn.backend(), fd.backend());
}
template <class To, class Integer>
inline typename enable_if_c<is_number<To>::value ||
                            is_floating_point<To>::value>::type
generic_convert_rational_to_float_imp(To &result, const Integer &n,
                                      const Integer &d, const mpl::true_ &) {
  //
...
generic_convert_rational_to_float_imp(To &result, Integer &num, Integer &denom,
                                      const mpl::false_ &) {
  //
  // If we get here, then the precision of type To is known, and the integer
  // type is unbounded so we can use integer division plus manipulation of the
  // remainder to get an exactly rounded result.
  //
  if (num == 0) {
    result = 0;
    return;
...
inline void generic_convert_rational_to_float(To &result, const From &f) {
  ...
  //
  typedef typename mpl::if_c<is_number<From>::value, From, number<From>>::type
      actual_from_type;
  typedef
      typename mpl::if_c<is_number<To>::value || is_floating_point<To>::value,
                         To, number<To>>::type actual_to_type;
...
void generic_interconvert(
  ...
  generic_interconvert_float2rational(
      to, from, mpl::int_<std::numeric_limits<number<From>>::radix>());
}
...
void generic_interconvert_float2int(To &to, const From &from,
  ...
  static const exponent_type shift =
...
void generic_interconvert_complex_to_scalar(To &to, const From &from,
  ...
  component_backend r;
  generic_interconvert_complex_to_scalar(r, from, mpl::true_(), mpl::true_());
  generic_interconvert(to, r, mpl::int_<number_category<To>::value>(),
                       mpl::int_<number_category<To>::value>());
}
...
void generic_interconvert(
    To &to, const From &from,
    const mpl::int_<number_kind_floating_point> & /*to_type*/,
    const mpl::int_<number_kind_complex> & /*from_type*/) {
  typedef typename component_type<number<From>>::type component_number;
  typedef typename component_number::backend_type component_backend;

  generic_interconvert_complex_to_scalar(
      to, from, mpl::bool_<boost::is_same<component_backend, To>::value>(),
      mpl::bool_<
          boost::is_constructible<To, const component_backend &>::value>());
}
template <class To, class From>
void generic_interconvert(
    To &to, const From &from,
