...
template <class T> void hyp0F1(T &result, const T &b, const T &x) {
  typedef
      typename boost::multiprecision::detail::canonical<boost::int32_t, T>::type
          si_type;
...
template <class T> void eval_sin(T &result, const T &x) {
  ...
  case FP_NAN:
    if (std::numeric_limits<number<T, et_on>>::has_quiet_NaN) {
      ...
      BOOST_THROW_EXCEPTION(
          std::domain_error("Result is undefined or complex and there is no "
                            "NaN for this number type."));
    return;
  case FP_ZERO:
    result = x;
    return;
...
template <class T> void eval_cos(T &result, const T &x) {
  BOOST_STATIC_ASSERT_MSG(
      number_category<T>::value == number_kind_floating_point,
...
template <class T> void eval_tan(T &result, const T &x) {
  BOOST_STATIC_ASSERT_MSG(
      number_category<T>::value == number_kind_floating_point,
...
template <class T> void eval_asin(T &result, const T &x) {
  BOOST_STATIC_ASSERT_MSG(
      number_category<T>::value == number_kind_floating_point,
  ...
  typedef
      typename boost::multiprecision::detail::canonical<long double, T>::type
          guess_type;
#else
  ...
  // Newton-Raphson iteration, we should double our precision with each
  // iteration, in practice this seems to not quite work in all cases... so
  // terminate when we have at least 2/3 of the digits correct on the assumption
  // that the correction we've just added will finish the job...

  boost::intmax_t current_precision = eval_ilogb(result);
  boost::intmax_t target_precision =
  ...
  while (current_precision > target_precision) {
    ...
    if (current_precision <=
        (std::numeric_limits<typename T::exponent_type>::min)() + 1)
      break;
  }
  if (b_neg)
    result.negate();
...
template <class T> inline void eval_acos(T &result, const T &x) {
  BOOST_STATIC_ASSERT_MSG(
      number_category<T>::value == number_kind_floating_point,
...
template <class T> void eval_atan(T &result, const T &x) {
  BOOST_STATIC_ASSERT_MSG(
      number_category<T>::value == number_kind_floating_point,
...
template <class T> void eval_atan2(T &result, const T &y, const T &x) {
  BOOST_STATIC_ASSERT_MSG(
      number_category<T>::value == number_kind_floating_point,
  ...
  case FP_ZERO: {
    if (eval_signbit(x)) {
      ...
      result = y; // Note we allow atan2(0,0) to be +-zero, even though it's
...
inline typename enable_if<is_arithmetic<A>, void>::type
