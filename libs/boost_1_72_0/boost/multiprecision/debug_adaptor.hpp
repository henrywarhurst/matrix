...
template <class Backend> struct debug_adaptor {
...
public:
  ...
  debug_adaptor() { update_view(); }
  debug_adaptor(const debug_adaptor &o)
      : debug_value(o.debug_value), m_value(o.m_value) {}
  ...
  template <class T> debug_adaptor(const T &i, const T &j) : m_value(i, j) {
    update_view();
  }
  template <class T>
...
template <class T> inline const T &unwrap_debug_type(const T &val) {
  return val;
}
...
#define NON_MEMBER_OP4(name, str)                                              \
  ...
  inline void BOOST_JOIN(eval_, name)(                                         \
      debug_adaptor<Backend> & result, const debug_adaptor<Backend> &a,        \
  ...
  inline void BOOST_JOIN(eval_, name)(                                         \
      debug_adaptor<Backend> & result, const debug_adaptor<Backend> &a,        \
      const T &b, const debug_adaptor<Backend> &c) {                           \
    using default_ops::BOOST_JOIN(eval_, name);                                \
    BOOST_JOIN(eval_, name)                                                    \
    (result.value(), unwrap_debug_type(a), unwrap_debug_type(b),               \
     unwrap_debug_type(c));                                                    \
  ...
  template <class Backend, class T>                                            \
  inline void BOOST_JOIN(eval_, name)(                                         \
      debug_adaptor<Backend> & result, const T &a,                             \
...
inline void eval_frexp(debug_adaptor<Backend> &result,
                       const debug_adaptor<Backend> &arg, Exp *exp) {
  eval_frexp(result.value(), arg.value(), exp);
  result.update_view();
}

template <class Backend, class Exp>
inline void eval_ldexp(debug_adaptor<Backend> &result,
                       const debug_adaptor<Backend> &arg, Exp exp) {
...
template <class Backend, class Exp>
inline void eval_scalbn(debug_adaptor<Backend> &result,
                        const debug_adaptor<Backend> &arg, Exp exp) {
...
eval_ilogb(const debug_adaptor<Backend> &arg) {
...
template <class Backend>
inline void eval_left_shift(debug_adaptor<Backend> &arg,
                            const debug_adaptor<Backend> &a, std::size_t b) {
  using default_ops::eval_left_shift;
...
template <class Backend>
inline void eval_right_shift(debug_adaptor<Backend> &arg,
                             const debug_adaptor<Backend> &a, std::size_t b) {
  using default_ops::eval_right_shift;
...
template <class Backend, class T>
inline unsigned eval_integer_modulus(const debug_adaptor<Backend> &arg,
                                     const T &a) {
...
template <class Backend> int eval_signbit(const debug_adaptor<Backend> &val) {
...
template <class Backend,
          ...
          class Policy>
struct precision<
    boost::multiprecision::number<boost::multiprecision::debug_adaptor<Backend>,
                                  ExpressionTemplates>,
    Policy>
    : public precision<
