...
divide_unsigned_helper(CppInt1 *result, const CppInt2 &x, const CppInt3 &y,
                       CppInt1 &r) {
  ...
  //
  // See if we can short-circuit long division, and use basic arithmetic
  // instead:
  //
  if (r_order == 0) {
    if (result) {
    ...
    double_limb_type a =
        (static_cast<double_limb_type>(px[1]) << CppInt1::limb_bits) | px[0];
    double_limb_type b =
        y_order ? (static_cast<double_limb_type>(py[1]) << CppInt1::limb_bits) |
                      py[0]
                : py[0];
    if (result) {
      *result = a / b;
  ...
  // This is initialised just to keep the compiler from emitting useless
  // warnings later on:
  ...
  do {
    ...
    if ((prem[r_order] <= py[y_order]) && (r_order > 0)) {
      ...
      double_limb_type a =
          (static_cast<double_limb_type>(prem[r_order]) << CppInt1::limb_bits) |
          prem[r_order - 1];
      double_limb_type b = (y_order > 0)
                               ? (static_cast<double_limb_type>(py[y_order])
                                  << CppInt1::limb_bits) |
                                     py[y_order - 1]
                               : (static_cast<double_limb_type>(py[y_order])
                                  << CppInt1::limb_bits);
    ...
    for (unsigned i = 0; i < y.size(); ++i) {
      ...
      pt[i + shift] =
          static_cast<limb_type>(carry & ~static_cast<limb_type>(0));
#else
  ...
  BOOST_ASSERT(
      r.compare_unsigned(y) <
      0); // remainder must be less than the divisor or our code has failed
}

template <class CppInt1, class CppInt2>
BOOST_MP_CXX14_CONSTEXPR void divide_unsigned_helper(CppInt1 *result,
                                                     const CppInt2 &x,
                                                     limb_type y, CppInt1 &r) {
  if (((void *)result == (void *)&x) || ((void *)&r == (void *)&x)) {
    CppInt2 t(x);
    divide_unsigned_helper(result, t, y, r);
    return;
  ...
  if (r_order == 0) {
    ...
    *pr %= y;
    ...
    double_limb_type a =
        (static_cast<double_limb_type>(pr[r_order]) << CppInt1::limb_bits) |
        pr[0];
    if (result) {
      *result = a / y;
      result->sign(x.sign());
  ...
  if (result) {
    ...
    if (result->size() > r_order)
      pres[r_order] =
          0; // just in case we don't set the most significant limb below.
  }
  ...
  // Termination condition is really just a check that r >= y, but with two
  // common short-circuit cases handled first:
...
eval_divide(cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>
                &result,
...
eval_divide(cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>
  ...
  cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> a(
      result);
...
BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR
    ...
    eval_divide(cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                                Allocator1> &result,
                limb_type b) {
  // There is no in place divide:
  cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> a(
...
BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR typename enable_if_c<
    ...
    !is_trivial_cpp_int<cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2,
                                        Allocator2>>::value>::type
eval_modulus(cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                             Allocator1> &result,
             const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2,
                                   Allocator2> &a,
             limb_type b) {
