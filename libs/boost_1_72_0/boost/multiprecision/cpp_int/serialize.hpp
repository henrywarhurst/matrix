...
template <class T> struct is_binary_archive : public mpl::false_ {};
template <>
...
//
// We have 8 serialization methods to fill out (and test), they are all
// permutations of: Load vs Store. Trivial or non-trivial cpp_int type. Binary
// or not archive.
//
...
template <class Archive, unsigned MinBits, unsigned MaxBits,
...
void serialize(
  ...
  typedef mpl::bool_<mp::backends::is_trivial_cpp_int<mp::cpp_int_backend<
  ...
  cpp_int_detail::do_serialize(ar, val, save_tag(), trivial_tag(),
                               binary_tag());
}
