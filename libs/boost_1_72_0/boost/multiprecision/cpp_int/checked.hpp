...
//
// Simple routines for performing checked arithmetic with a builtin arithmetic
// type. Note that this is not a complete header, it must be included as part of
...
inline void raise_add_overflow() { raise_overflow("addition"); }
...
inline void raise_mul_overflow() { raise_overflow("multiplication"); }
...
template <class A>
inline BOOST_MP_CXX14_CONSTEXPR A
...
inline BOOST_MP_CXX14_CONSTEXPR A checked_left_shift(
    A a, boost::ulong_long_type shift, const mpl::int_<checked> &) {
  if (a && shift) {
...
template <class A>
inline BOOST_MP_CXX14_CONSTEXPR A checked_left_shift(
    A a, boost::ulong_long_type shift, const mpl::int_<unchecked> &) {
