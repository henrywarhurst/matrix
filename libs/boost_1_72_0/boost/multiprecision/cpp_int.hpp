...
#ifdef BOOST_MSVC
#pragma warning(push)
#pragma warning(disable : 4307) // integral constant overflow (oveflow is in a
                                // branch not taken when it would overflow)
#pragma warning(disable : 4127) // conditional expression is constant
#pragma warning(disable : 4702) // Unreachable code (reachability depends on
                                // template params)
#endif

template <unsigned MinBits = 0, unsigned MaxBits = 0,
          boost::multiprecision::cpp_integer_type SignType = signed_magnitude,
          cpp_int_check_type Checked = unchecked,
          class Allocator =
              typename mpl::if_c<MinBits && (MinBits == MaxBits), void,
...
struct min_precision<
    cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator>> {
  static const unsigned value =
      (is_void<Allocator>::value ? static_unsigned_max<MinBits, MaxBits>::value
                                 : MinBits);
};
//
// Traits class determines whether the number of bits precision requested could
// fit in a native type, we call this a "trivial" cpp_int:
//
template <class T> struct is_trivial_cpp_int {
...
template <unsigned MinBits, unsigned MaxBits, cpp_integer_type SignType,
          cpp_int_check_type Checked, class Allocator>
struct is_trivial_cpp_int<
    cpp_int_base<MinBits, MaxBits, SignType, Checked, Allocator, true>> {
...
struct is_unsigned_number<
    backends::cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator>>
    : public mpl::bool_<(SignType == unsigned_magnitude) ||
                        (SignType == unsigned_packed)> {};
...
//
template <class T, class U> struct is_implicit_cpp_int_conversion;

template <unsigned MinBits, unsigned MaxBits, cpp_integer_type SignType,
          cpp_int_check_type Checked, class Allocator, unsigned MinBits2,
...
struct is_implicit_cpp_int_conversion<
  ...
  typedef cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>
      t2;
  static const bool value =
...
//
template <class T> struct is_non_throwing_cpp_int : public mpl::false_ {};
template <unsigned MinBits, unsigned MaxBits, cpp_integer_type SignType>
struct is_non_throwing_cpp_int<
    cpp_int_backend<MinBits, MaxBits, SignType, unchecked, void>>
    : public mpl::true_ {};

//
// Traits class, determines whether the cpp_int is fixed precision or not:
//
template <class T> struct is_fixed_precision;
template <unsigned MinBits, unsigned MaxBits, cpp_integer_type SignType,
          cpp_int_check_type Checked, class Allocator>
struct is_fixed_precision<
    cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator>>
    : public mpl::bool_<
          max_precision<cpp_int_backend<MinBits, MaxBits, SignType, Checked,
                                        Allocator>>::value != UINT_MAX> {};

namespace detail {

inline BOOST_MP_CXX14_CONSTEXPR void
verify_new_size(unsigned new_size, unsigned min_size,
                const mpl::int_<checked> &) {
  if (new_size < min_size)
    BOOST_THROW_EXCEPTION(std::overflow_error(
        "Unable to allocate sufficient storage for the value of the result: "
        "value overflows the maximum allowable magnitude."));
}
inline BOOST_MP_CXX14_CONSTEXPR void
verify_new_size(unsigned /*new_size*/, unsigned /*min_size*/,
...
template <class U>
inline BOOST_MP_CXX14_CONSTEXPR void
verify_limb_mask(bool b, U limb, U mask, const mpl::int_<checked> &) {
  // When we mask out "limb" with "mask", do we loose bits?  If so it's an
  // overflow error:
  if (b && (limb & ~mask))
    BOOST_THROW_EXCEPTION(std::overflow_error(
        "Overflow in cpp_int arithmetic: there is insufficient precision in "
        "the target type to hold all of the bits of the result."));
}
template <class U>
inline BOOST_MP_CXX14_CONSTEXPR void
verify_limb_mask(bool /*b*/, U /*limb*/, U /*mask*/,
                 const mpl::int_<unchecked> &) {}

} // namespace detail

//
// Now define the various data layouts that are possible as partial
// specializations of the base class, starting with the default arbitrary
// precision signed integer type:
//
template <unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked,
          class Allocator>
struct cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked, Allocator,
                    false>
    : private boost::empty_value<
          typename detail::rebind<limb_type, Allocator>::type> {
...
public:
  ...
  BOOST_STATIC_CONSTANT(limb_type,
                        max_limb_value = ~static_cast<limb_type>(0u));
  BOOST_STATIC_CONSTANT(limb_type, sign_bit_mask = static_cast<limb_type>(1u)
                                                   << (limb_bits - 1));
  BOOST_STATIC_CONSTANT(
      unsigned,
      internal_limb_count = MinBits
                                ? (MinBits / limb_bits +
...
private:
  union data_type {
    ...
    BOOST_CONSTEXPR data_type(signed_limb_type i)
        : first(i < 0 ? static_cast<limb_type>(
                            boost::multiprecision::detail::unsigned_abs(i))
                      : i) {}
#if BOOST_ENDIAN_LITTLE_BYTE
    BOOST_CONSTEXPR data_type(double_limb_type i) : double_first(i) {}
    BOOST_CONSTEXPR data_type(signed_double_limb_type i)
        : double_first(i < 0
...
public:
  ...
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(limb_type i) BOOST_NOEXCEPT
      : m_data(i),
        ...
        m_sign(false),
        m_internal(true) {}
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(signed_limb_type i)
      BOOST_NOEXCEPT : m_data(i),
                       m_limbs(1),
                       m_sign(i < 0),
                       m_internal(true) {}
#if BOOST_ENDIAN_LITTLE_BYTE && !defined(BOOST_MP_TEST_NO_LE)
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(double_limb_type i)
      BOOST_NOEXCEPT : m_data(i),
                       m_limbs(i > max_limb_value ? 2 : 1),
                       m_sign(false),
                       m_internal(true) {}
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(signed_double_limb_type i)
      BOOST_NOEXCEPT
      : m_data(i),
        m_limbs(i < 0 ? (static_cast<double_limb_type>(
                             boost::multiprecision::detail::unsigned_abs(i)) >
                                 static_cast<double_limb_type>(max_limb_value)
                             ? 2
                             : 1)
  ...
  void resize(unsigned new_size, unsigned min_size) {
    static const unsigned max_limbs =
  ...
  BOOST_MP_FORCEINLINE cpp_int_base(const cpp_int_base &o)
      : base_type(o), m_limbs(0), m_internal(true) {
  ...
  cpp_int_base(cpp_int_base &&o)
      : base_type(static_cast<base_type &&>(o)), m_limbs(o.m_limbs),
...
protected:
  template <class A> void check_in_range(const A &) BOOST_NOEXCEPT {}
};
...
template <unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked,
          class Allocator>
const unsigned cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked,
                            Allocator, false>::limb_bits;
template <unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked,
          class Allocator>
const limb_type cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked,
                             Allocator, false>::max_limb_value;
template <unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked,
          class Allocator>
const limb_type cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked,
                             Allocator, false>::sign_bit_mask;
template <unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked,
          class Allocator>
const unsigned cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked,
                            Allocator, false>::internal_limb_count;
template <unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked,
          class Allocator>
const bool cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked, Allocator,
                        false>::variable;
...
template <unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked,
          class Allocator>
struct cpp_int_base<MinBits, MaxBits, unsigned_magnitude, Checked, Allocator,
                    false>
    : private boost::empty_value<
          typename detail::rebind<limb_type, Allocator>::type> {
  //
  // There is currently no support for unsigned arbitrary precision arithmetic,
  // largely because it's not clear what subtraction should do:
  //
  BOOST_STATIC_ASSERT_MSG(((sizeof(Allocator) == 0) &&
                           !is_void<Allocator>::value),
                          "There is curently no support for unsigned arbitrary "
                          "precision integers.");
};
//
// Fixed precision (i.e. no allocator), signed-magnitude type with limb-usage
// count:
//
...
struct cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, false> {
  ...
  BOOST_STATIC_ASSERT_MSG(
      MinBits > sizeof(double_limb_type) * CHAR_BIT,
...
public:
  ...
  BOOST_STATIC_CONSTANT(unsigned,
                        internal_limb_count = MinBits / limb_bits +
                                              ((MinBits % limb_bits) ? 1 : 0));
  BOOST_STATIC_CONSTANT(bool, variable = false);
  BOOST_STATIC_CONSTANT(limb_type,
                        upper_limb_mask = (MinBits % limb_bits)
                                              ? (limb_type(1)
                                                 << (MinBits % limb_bits)) -
                                                    1
                                              : (~limb_type(0)));
  BOOST_STATIC_ASSERT_MSG(
      internal_limb_count >= 2,
...
private:
  union data_type {
    ...
    BOOST_CONSTEXPR data_type(literals::detail::value_pack<VALUES...>)
        : m_data{VALUES...} {}
#endif
  } m_wrapper;
...
public:
  ...
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(signed_limb_type i)
      BOOST_NOEXCEPT
      : m_wrapper(limb_type(i < 0
                                ? static_cast<limb_type>(
                                      -static_cast<signed_double_limb_type>(i))
                                : i)),
        m_limbs(1),
        m_sign(i < 0) {}
#if BOOST_ENDIAN_LITTLE_BYTE && !defined(BOOST_MP_TEST_NO_LE)
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(double_limb_type i)
      BOOST_NOEXCEPT : m_wrapper(i),
                       m_limbs(i > max_limb_value ? 2 : 1),
                       m_sign(false) {}
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(signed_double_limb_type i)
      BOOST_NOEXCEPT
      : m_wrapper(double_limb_type(
            i < 0 ? static_cast<double_limb_type>(
                        boost::multiprecision::detail::unsigned_abs(i))
                  : i)),
        m_limbs(i < 0 ? (static_cast<double_limb_type>(
                             boost::multiprecision::detail::unsigned_abs(i)) >
  ...
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR limb_pointer limbs()
      BOOST_NOEXCEPT {
    return m_wrapper.m_data;
  }
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR const_limb_pointer
  ...
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void resize(unsigned new_size,
    ...
    m_limbs =
        static_cast<boost::uint16_t>((std::min)(new_size, internal_limb_count));
  ...
  // reason:
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(const cpp_int_base &o)
      BOOST_NOEXCEPT : m_wrapper(o.m_wrapper),
                       m_limbs(o.m_limbs),
                       m_sign(o.m_sign) {}
  // Defaulted functions:
  ...
  isneg() const BOOST_NOEXCEPT {
    return m_sign;
  }
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void
  do_swap(cpp_int_base &o) BOOST_NOEXCEPT {
...
template <unsigned MinBits, cpp_int_check_type Checked>
const unsigned cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void,
                            false>::limb_bits;
template <unsigned MinBits, cpp_int_check_type Checked>
const limb_type cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void,
                             false>::max_limb_value;
template <unsigned MinBits, cpp_int_check_type Checked>
const limb_type cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void,
                             false>::sign_bit_mask;
template <unsigned MinBits, cpp_int_check_type Checked>
const unsigned cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void,
                            false>::internal_limb_count;
template <unsigned MinBits, cpp_int_check_type Checked>
const bool cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void,
                        false>::variable;
...
//
template <unsigned MinBits, cpp_int_check_type Checked>
struct cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked, void,
                    false> {
  typedef limb_type *limb_pointer;
  typedef const limb_type *const_limb_pointer;
  typedef mpl::int_<Checked> checked_type;
...
public:
  ...
  cpp_int_base(signed_double_limb_type i)
      BOOST_MP_NOEXCEPT_IF((Checked == unchecked))
      : m_wrapper(double_limb_type(
            i < 0 ? static_cast<double_limb_type>(
                        boost::multiprecision::detail::unsigned_abs(i))
  ...
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void
  assign(const cpp_int_base &o) BOOST_NOEXCEPT {
    if (this != &o) {
      m_limbs = o.m_limbs;
#ifndef BOOST_MP_NO_CONSTEXPR_DETECTION
      if (BOOST_MP_IS_CONST_EVALUATED(m_limbs)) {
...
public:
  BOOST_MP_CXX14_CONSTEXPR void negate()
    ...
    normalize();
    eval_increment(
        static_cast<cpp_int_backend<MinBits, MinBits, unsigned_magnitude,
                                    Checked, void> &>(*this));
  }
...
template <unsigned MinBits, cpp_int_check_type Checked>
const unsigned cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked, void,
                            false>::limb_bits;
template <unsigned MinBits, cpp_int_check_type Checked>
const limb_type cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked,
                             void, false>::max_limb_value;
template <unsigned MinBits, cpp_int_check_type Checked>
const limb_type cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked,
                             void, false>::sign_bit_mask;
template <unsigned MinBits, cpp_int_check_type Checked>
const unsigned cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked, void,
                            false>::internal_limb_count;
template <unsigned MinBits, cpp_int_check_type Checked>
const bool cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked, void,
                        false>::variable;

#endif
//
// Traits classes to figure out a native type with N bits, these vary from
// boost::uint_t<N> only because some platforms have native integer types longer
// than boost::long_long_type, "really boost::long_long_type" anyone??
//
template <unsigned N, bool s> struct trivial_limb_type_imp {
  typedef double_limb_type type;
};

template <unsigned N> struct trivial_limb_type_imp<N, true> {
  typedef typename boost::uint_t<N>::least type;
};

template <unsigned N>
struct trivial_limb_type
    : public trivial_limb_type_imp<N, N <= sizeof(boost::long_long_type) *
                                               CHAR_BIT> {};
//
// Backend for fixed precision signed-magnitude type which will fit entirely
// inside a "double_limb_type":
//
template <unsigned MinBits, cpp_int_check_type Checked>
...
protected:
  BOOST_STATIC_CONSTANT(unsigned,
                        limb_bits = sizeof(local_limb_type) * CHAR_BIT);
  BOOST_STATIC_CONSTANT(
      local_limb_type,
      limb_mask =
          (MinBits < limb_bits)
...
protected:
  ...
  BOOST_MP_CXX14_CONSTEXPR
    ...
    if (static_cast<common_type>(boost::multiprecision::detail::unsigned_abs(
            val)) > static_cast<common_type>(limb_mask))
      BOOST_THROW_EXCEPTION(
          std::range_error("The argument to a cpp_int constructor exceeded the "
  ...
  BOOST_MP_CXX14_CONSTEXPR
    ...
    if (static_cast<common_type>(abs(val)) >
  ...
  check_in_range(T, const mpl::int_<C> &) BOOST_NOEXCEPT {}

  template <class T>
  BOOST_MP_CXX14_CONSTEXPR void check_in_range(T val)
      BOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(
          std::declval<T>(), checked_type()))) {
...
public:
  ...
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(
      ...
      BOOST_MP_NOEXCEPT_IF(noexcept(
          std::declval<cpp_int_base>().check_in_range(std::declval<SI>())))
      : m_data(i < 0 ? static_cast<local_limb_type>(
                           static_cast<typename make_unsigned<SI>::type>(
                               boost::multiprecision::detail::unsigned_abs(i)) &
                           limb_mask)
  ...
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR cpp_int_base(
      UI i, typename boost::enable_if_c<is_unsigned<UI>::value &&
                                        (Checked == checked)>::type const * = 0)
      BOOST_MP_NOEXCEPT_IF(noexcept(
          std::declval<cpp_int_base>().check_in_range(std::declval<UI>())))
  ...
  BOOST_CONSTEXPR
  cpp_int_base(literals::detail::value_pack<a, b>) BOOST_NOEXCEPT
      : m_data(static_cast<local_limb_type>(a) |
               (static_cast<local_limb_type>(b) << bits_per_limb)),
        m_sign(false) {}
  BOOST_CONSTEXPR
  cpp_int_base(const cpp_int_base &a,
  ...
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void
  resize(unsigned /* new_size */, unsigned min_size) {
  ...
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base() BOOST_NOEXCEPT
      : m_data(0),
        m_sign(false) {}
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(const cpp_int_base &o)
      BOOST_NOEXCEPT : m_data(o.m_data),
                       m_sign(o.m_sign) {}
  //~cpp_int_base() BOOST_NOEXCEPT {}
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void
  assign(const cpp_int_base &o) BOOST_NOEXCEPT {
...
//
// Backend for unsigned fixed precision (i.e. no allocator) type which will fit
// entirely inside a "double_limb_type":
//
template <unsigned MinBits, cpp_int_check_type Checked>
...
protected:
  template <class T>
  BOOST_MP_CXX14_CONSTEXPR typename boost::disable_if_c<
      std::numeric_limits<T>::is_specialized &&
  ...
  template <class T>
  BOOST_MP_CXX14_CONSTEXPR void
  check_in_range(T val, const mpl::int_<checked> &, const boost::true_type &) {
    typedef typename common_type<T, local_limb_type>::type common_type;
    ...
    if (val < 0)
      BOOST_THROW_EXCEPTION(std::range_error(
          "The argument to an unsigned cpp_int constructor was negative."));
  }
  ...
  template <class T>
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void check_in_range(T val)
      BOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(
          std::declval<T>(), checked_type(), is_signed<T>()))) {
...
public:
  ...
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void resize(unsigned,
                                                            unsigned min_size) {
    detail::verify_new_size(2, min_size, checked_type());
  }
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void normalize()
  ...
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void negate()
    ...
    if (Checked == checked) {
      BOOST_THROW_EXCEPTION(
          std::range_error("Attempt to negate an unsigned type."));
    }
    m_data = ~m_data;
    ++m_data;
  }
...
//
// Traits class, lets us know whether type T can be directly converted to the
// base type, used to enable/disable constructors etc:
//
template <class Arg, class Base>
struct is_allowed_cpp_int_base_conversion
    : public mpl::if_c<is_same<Arg, limb_type>::value ||
                           is_same<Arg, signed_limb_type>::value
#if BOOST_ENDIAN_LITTLE_BYTE && !defined(BOOST_MP_TEST_NO_LE)
                           || is_same<Arg, double_limb_type>::value ||
                           is_same<Arg, signed_double_limb_type>::value
#endif
#if defined(BOOST_MP_USER_DEFINED_LITERALS)
                           || literals::detail::is_value_pack<Arg>::value
#endif
                           || (is_trivial_cpp_int<Base>::value &&
                               is_arithmetic<Arg>::value),
...
struct cpp_int_backend
    : public cpp_int_base<
          min_precision<cpp_int_backend<MinBits, MaxBits, SignType, Checked,
                                        Allocator>>::value,
          max_precision<cpp_int_backend<MinBits, MaxBits, SignType, Checked,
                                        Allocator>>::value,
          SignType, Checked, Allocator,
          is_trivial_cpp_int<cpp_int_backend<MinBits, MaxBits, SignType,
                                             Checked, Allocator>>::value> {
  typedef cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator>
      self_type;
  typedef cpp_int_base<min_precision<self_type>::value,
                       max_precision<self_type>::value, SignType, Checked,
                       Allocator, is_trivial_cpp_int<self_type>::value>
      base_type;
...
public:
  typedef typename mpl::if_<
      trivial_tag,
      mpl::list<signed char, short, int, long, boost::long_long_type,
                signed_double_limb_type>,
      mpl::list<signed_limb_type, signed_double_limb_type>>::type signed_types;
  typedef typename mpl::if_<
      trivial_tag,
      mpl::list<unsigned char, unsigned short, unsigned, unsigned long,
                boost::ulong_long_type, double_limb_type>,
      mpl::list<limb_type, double_limb_type>>::type unsigned_types;
  typedef typename mpl::if_<trivial_tag, mpl::list<float, double, long double>,
  ...
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_backend(const cpp_int_backend &o)
      ...
      : base_type(o) {}
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_backend(cpp_int_backend &&o)
      BOOST_NOEXCEPT : base_type(static_cast<base_type &&>(o)) {}
#endif
  //
  ...
  BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_backend(
      Arg i, typename boost::enable_if_c<is_allowed_cpp_int_base_conversion<
...
private:
  template <unsigned MinBits2, unsigned MaxBits2, cpp_integer_type SignType2,
            cpp_int_check_type Checked2, class Allocator2>
  BOOST_MP_CXX14_CONSTEXPR void
  do_assign(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2,
                                  Allocator2> &other,
            mpl::true_ const &, mpl::true_ const &) {
    // Assigning trivial type to trivial type:
    this->check_in_range(*other.limbs());
    *this->limbs() =
        static_cast<typename self_type::local_limb_type>(*other.limbs());
  ...
  do_assign(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2,
    ...
    if (other.size() > 1) {
      ...
      if ((Checked == checked) && (other.size() > 2)) {
        BOOST_THROW_EXCEPTION(
            std::range_error("Assignment of a cpp_int that is out of range for "
                             "the target type."));
      }
    }
    *this = v;
  ...
  do_assign(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2,
    ...
    // std::memcpy(this->limbs(), other.limbs(), (std::min)(other.size(),
    // this->size()) * sizeof(this->limbs()[0]));
...
public:
  ...
  BOOST_MP_CXX14_CONSTEXPR cpp_int_backend(
      ...
      : base_type() {
    do_assign(
        other, mpl::bool_<is_trivial_cpp_int<self_type>::value>(),
        mpl::bool_<is_trivial_cpp_int<cpp_int_backend<
            MinBits2, MaxBits2, SignType2, Checked2, Allocator2>>::value>());
  }
  template <unsigned MinBits2, unsigned MaxBits2, cpp_integer_type SignType2,
            cpp_int_check_type Checked2, class Allocator2>
  ...
  template <unsigned MinBits2, unsigned MaxBits2, cpp_integer_type SignType2,
            cpp_int_check_type Checked2, class Allocator2>
  BOOST_MP_CXX14_CONSTEXPR cpp_int_backend &
  operator=(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2,
                                  Allocator2> &other) {
    do_assign(
        other, mpl::bool_<is_trivial_cpp_int<self_type>::value>(),
        mpl::bool_<is_trivial_cpp_int<cpp_int_backend<
...
#ifdef BOOST_MP_USER_DEFINED_LITERALS
  BOOST_CONSTEXPR cpp_int_backend(const cpp_int_backend &a,
  ...
  operator=(const cpp_int_backend &o)
      BOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_backend>().assign(
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR cpp_int_backend &
  operator=(cpp_int_backend &&o) BOOST_MP_NOEXCEPT_IF(
...
private:
  template <class A>
  BOOST_MP_CXX14_CONSTEXPR typename boost::enable_if<is_unsigned<A>>::type
  do_assign_arithmetic(A val, const mpl::true_ &) BOOST_MP_NOEXCEPT_IF(noexcept(
      std::declval<cpp_int_backend>().check_in_range(std::declval<A>()))) {
  ...
  BOOST_MP_CXX14_CONSTEXPR
      ...
      do_assign_arithmetic(A val, const mpl::true_ &) BOOST_MP_NOEXCEPT_IF(
          noexcept(std::declval<cpp_int_backend>().check_in_range(
              std::declval<A>())) &&
          noexcept(std::declval<cpp_int_backend>().sign(true))) {
    this->check_in_range(val);
    *this->limbs() =
        (val < 0) ? static_cast<typename self_type::local_limb_type>(
  ...
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void
  do_assign_arithmetic(signed_limb_type i, const mpl::false_ &)
      BOOST_MP_NOEXCEPT_IF(
          noexcept(std::declval<cpp_int_backend>().sign(true))) {
    this->resize(1, 1);
    *this->limbs() =
        static_cast<limb_type>(boost::multiprecision::detail::unsigned_abs(i));
    this->sign(i < 0);
  }
  BOOST_MP_CXX14_CONSTEXPR void
  do_assign_arithmetic(double_limb_type i, const mpl::false_ &) BOOST_NOEXCEPT {
  ...
  BOOST_MP_CXX14_CONSTEXPR void do_assign_arithmetic(signed_double_limb_type i,
    ...
    if (i < 0)
      s = true;
    double_limb_type ui = static_cast<double_limb_type>(
        boost::multiprecision::detail::unsigned_abs(i));
    typename base_type::limb_pointer p = this->limbs();
#ifdef __MSVC_RUNTIME_CHECKS
  ...
  BOOST_MP_CXX14_CONSTEXPR void do_assign_arithmetic(long double a,
    ...
    if ((boost::math::isinf)(a) || (boost::math::isnan)(a)) {
      BOOST_THROW_EXCEPTION(std::runtime_error(
          "Cannot convert a non-finite number to an integer."));
    }
...
public:
  ...
  operator=(Arithmetic val) BOOST_MP_NOEXCEPT_IF(
      noexcept(std::declval<cpp_int_backend>().do_assign_arithmetic(
    ...
    if (n) {
      if (radix == 16) {
        ...
        result.resize(
            ...
            static_cast<unsigned>(newsize)); // will throw if this is a checked
                                             // integer that cannot be resized
        ...
        while (*s) {
          ...
          if (result.size() > limb) {
            ...
            if (shift > sizeof(limb_type) * CHAR_BIT - 3) {
              ...
              if (val) {
                // If this is the most-significant-limb, we may need to allocate
                // an extra one for the overflow:
                if (limb + 1 == newsize)
...
public:
  ...
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void
  swap(cpp_int_backend &o) BOOST_NOEXCEPT {
    this->do_swap(o);
  }
...
private:
  std::string do_get_trivial_string(std::ios_base::fmtflags f,
    ...
    typedef
        typename mpl::if_c<sizeof(typename base_type::local_limb_type) == 1,
                           unsigned, typename base_type::local_limb_type>::type
            io_type;
    if (this->sign() && (((f & std::ios_base::hex) == std::ios_base::hex) ||
                         ((f & std::ios_base::oct) == std::ios_base::oct)))
      BOOST_THROW_EXCEPTION(std::runtime_error(
          "Base 8 or 16 printing of negative numbers is not supported."));
    std::stringstream ss;
  ...
  std::string do_get_string(std::ios_base::fmtflags f,
    ...
    return do_get_trivial_string(
        f, mpl::bool_<is_same<typename base_type::local_limb_type,
                              double_limb_type>::value>());
#else
  ...
  void construct_from_container(const Container &c, const mpl::false_ &) {
    ...
    if (newsize) {
      ...
      while (i != j) {
        ...
        if (this->size() > limb)
          this->limbs()[limb] |=
              static_cast<limb_type>(static_cast<unsigned char>(*i)) << shift;
        ++i;
        --byte_location;
      }
    }
...
public:
  ...
  BOOST_MP_CXX14_CONSTEXPR cpp_int_backend(
      ...
      typename boost::enable_if_c<
          boost::multiprecision::detail::is_byte_container<Container>::value>::
          type const * = 0) {
    //
    // We assume that c is a sequence of (unsigned) bytes with the most
    // significant byte first:
  ...
  compare_imp(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2,
                                    Allocator2> &o,
              const mpl::false_ &, const mpl::false_ &) const BOOST_NOEXCEPT {
    if (this->sign() != o.sign())
  ...
  compare_imp(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2,
    ...
    cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2> t(
        *this);
    return t.compare(o);
  }
  template <unsigned MinBits2, unsigned MaxBits2, cpp_integer_type SignType2,
  ...
  compare_imp(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2,
    ...
    if (this->sign()) {
      if (o.sign()) {
        return *this->limbs() < *o.limbs()
                   ? 1
                   : (*this->limbs() > *o.limbs() ? -1 : 0);
      } else
        return -1;
    } else {
      if (o.sign())
  ...
  template <unsigned MinBits2, unsigned MaxBits2, cpp_integer_type SignType2,
            cpp_int_check_type Checked2, class Allocator2>
  BOOST_MP_CXX14_CONSTEXPR int
  compare(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2,
                                Allocator2> &o) const BOOST_NOEXCEPT {
    typedef mpl::bool_<is_trivial_cpp_int<
        cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator>>::value>
        t1;
    typedef mpl::bool_<is_trivial_cpp_int<cpp_int_backend<
        MinBits2, MaxBits2, SignType2, Checked2, Allocator2>>::value>
        t2;
  ...
  template <class Arithmetic>
  BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR
      typename boost::enable_if<is_arithmetic<Arithmetic>, int>::type
...
template <class Backend> struct double_precision_type;
...
struct double_precision_type<
  ...
  typedef typename mpl::if_c<
      backends::is_fixed_precision<backends::cpp_int_backend<
      ...
      backends::cpp_int_backend<
          ...
          2 * backends::max_precision<backends::cpp_int_backend<
                  MinBits, MaxBits, SignType, Checked, Allocator>>::value,
          SignType, Checked, Allocator>,
      backends::cpp_int_backend<MinBits, MaxBits, SignType, Checked,
                                Allocator>>::type type;
};
...
struct expression_template_default<
    backends::cpp_int_backend<MinBits, MaxBits, SignType, Checked, void>> {
...
typedef number<cpp_int_backend<0, 0, signed_magnitude, checked>>
    checked_cpp_int;
typedef rational_adaptor<cpp_int_backend<0, 0, signed_magnitude, checked>>
...
// Fixed precision signed types:
typedef number<cpp_int_backend<128, 128, signed_magnitude, checked, void>>
    checked_int128_t;
typedef number<cpp_int_backend<256, 256, signed_magnitude, checked, void>>
    checked_int256_t;
typedef number<cpp_int_backend<512, 512, signed_magnitude, checked, void>>
    checked_int512_t;
typedef number<cpp_int_backend<1024, 1024, signed_magnitude, checked, void>>
    checked_int1024_t;
...
//
// Last of all we include the implementations of all the eval_* non member
// functions:
//
