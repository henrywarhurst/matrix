...
#ifdef BOOST_MSVC
#pragma warning(push)
#pragma warning(disable : 4522 6326) // multiple assignment operators specified,
...
template <class Float, int,
          bool = number_category<Float>::value == number_kind_floating_point>
struct is_cpp_bin_float_implicitly_constructible_from_type {
  static const bool value = false;
};

template <class Float, int bit_count>
struct is_cpp_bin_float_implicitly_constructible_from_type<Float, bit_count,
                                                           true> {
...
template <class Float, int bit_count>
struct is_cpp_bin_float_explicitly_constructible_from_type<Float, bit_count,
                                                           true> {
  static const bool value =
      (std::numeric_limits<Float>::digits > (int)bit_count) &&
      (std::numeric_limits<Float>::radix == 2) &&
...
template <unsigned Digits, digit_base_type DigitBase = digit_base_10,
          class Allocator = void, class Exponent = int,
          Exponent MinExponent = 0, Exponent MaxExponent = 0>
class cpp_bin_float {
public:
  static const unsigned bit_count =
      DigitBase == digit_base_2
          ? Digits
          : (Digits * 1000uL) / 301uL + (((Digits * 1000uL) % 301) ? 2u : 1u);
  typedef cpp_int_backend<is_void<Allocator>::value ? bit_count : 0, bit_count,
                          is_void<Allocator>::value ? unsigned_magnitude
                                                    : signed_magnitude,
                          unchecked, Allocator>
      rep_type;
  typedef cpp_int_backend<
      is_void<Allocator>::value ? 2 * bit_count : 0, 2 * bit_count,
  ...
  static const exponent_type max_exponent_limit =
      ...
      2 * static_cast<exponent_type>(bit_count);
  static const exponent_type min_exponent_limit =
      boost::integer_traits<exponent_type>::const_min +
      2 * static_cast<exponent_type>(bit_count);

  BOOST_STATIC_ASSERT_MSG(MinExponent >= min_exponent_limit,
                          "Template parameter MinExponent is too negative for "
                          "our internal logic to function correctly, sorry!");
  BOOST_STATIC_ASSERT_MSG(MaxExponent <= max_exponent_limit,
                          "Template parameter MaxExponent is too large for our "
                          "internal logic to function correctly, sorry!");
  BOOST_STATIC_ASSERT_MSG(
      MinExponent <= 0, "Template parameter MinExponent can not be positive!");
  BOOST_STATIC_ASSERT_MSG(
      MaxExponent >= 0, "Template parameter MaxExponent can not be negative!");
...
public:
  ...
  cpp_bin_float(const Float &f,
                typename boost::enable_if_c<
                    detail::is_cpp_bin_float_implicitly_constructible_from_type<
  ...
  explicit cpp_bin_float(
      const Float &f,
      typename boost::enable_if_c<
          detail::is_cpp_bin_float_explicitly_constructible_from_type<
              Float, bit_count>::value>::type const * = 0)
      : m_data(), m_exponent(0), m_sign(false) {
    this->assign_float(f);
  }
...
#endif
  cpp_bin_float &operator=(const cpp_bin_float &o) BOOST_MP_NOEXCEPT_IF(
  ...
  cpp_bin_float &operator=(const cpp_bin_float<D, B, A, E, MinE, MaxE> &f) {
    ...
    default:
      ...
      this->exponent() = f.exponent() + (E)bit_count -
                         (E)cpp_bin_float<D, B, A, E, MinE, MaxE>::bit_count;
      this->sign() = f.sign();
...
#ifdef BOOST_HAS_FLOAT128
  template <class Float>
  typename boost::enable_if_c<is_floating_point<Float>::value &&
                                  !is_same<Float, __float128>::value,
                              cpp_bin_float &>::type
  assign_float(Float f)
#else
  template <class Float>
  ...
  template <class I>
  typename boost::enable_if<is_integral<I>, cpp_bin_float &>::type
  operator=(const I &i) {
    ...
    if (!i) {
      ...
      ui_type fi =
          static_cast<ui_type>(boost::multiprecision::detail::unsigned_abs(i));
      typedef typename boost::multiprecision::detail::canonical<
          ui_type, rep_type>::type ar_type;
  ...
  template <class A> int compare(const A &o) const BOOST_NOEXCEPT {
    cpp_bin_float b;
    b = o;
    return compare(b);
  }
...
template <unsigned Digits, digit_base_type DigitBase, class Allocator,
          class Exponent, Exponent MinE, Exponent MaxE, class Int>
inline void copy_and_round(
    cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res,
    Int &arg,
    int bits_to_keep = cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                     MinE, MaxE>::bit_count) {
  // Precondition: exponent of res must have been set before this function is
  // called as we may need to adjust it based on how many bits_to_keep in arg
  // are set.
  using default_ops::eval_bit_test;
  using default_ops::eval_get_sign;
  ...
  if (eval_get_sign(arg) == 0) {
    res.exponent() = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                                   MaxE>::exponent_zero;
    res.sign() = false;
  ...
  if (static_cast<int>(bits_to_keep) > msb + 1) {
    ...
    if (bits_to_keep != cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                      MinE, MaxE>::bit_count) {
      // Normalize result when we're rounding to fewer bits than we can hold,
      // only happens in conversions to narrower types:
      eval_left_shift(arg, cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                         MinE, MaxE>::bit_count -
                               bits_to_keep);
      res.exponent() -=
          static_cast<Exponent>(cpp_bin_float<Digits, DigitBase, Allocator,
  ...
  if (res.exponent() > cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
...
inline void do_eval_add(
  ...
  typedef typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                                 MaxE>::exponent_type exponent_type;

  typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                         MaxE>::double_rep_type dt;
  ...
  case cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                     MaxE>::exponent_infinity:
    if (b.exponent() == cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                      MinE, MaxE>::exponent_nan)
      res = b;
    else
      res = a;
...
inline void do_eval_subtract(
  ...
  case cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
    ...
    if (b.exponent() == cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                      MinE, MaxE>::exponent_nan)
      res = std::numeric_limits<number<cpp_bin_float<
          Digits, DigitBase, Allocator, Exponent, MinE, MaxE>>>::quiet_NaN()
                .backend();
    else {
      bool s = a.sign();
      res = b;
      if (res.exponent() == cpp_bin_float<Digits, DigitBase, Allocator,
                                          Exponent, MinE, MaxE>::exponent_zero)
        res.sign() = false;
      else if (res.sign() == s)
        res.negate();
    }
  ...
  case cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
    ...
    if ((b.exponent() == cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                       MinE, MaxE>::exponent_nan) ||
        (b.exponent() == cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
  ...
  if ((a.exponent() > b.exponent()) ||
    ...
    dt = a.bits();
    if (a.exponent() <= (int)cpp_bin_float<Digits, DigitBase, Allocator,
                                           Exponent, MinE, MaxE>::bit_count +
                            b.exponent()) {
      typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                             MaxE>::exponent_type e_diff =
          a.exponent() - b.exponent();
      eval_left_shift(dt, e_diff);
      res.exponent() = a.exponent() - e_diff;
      eval_subtract(dt, b.bits());
    } else if (a.exponent() ==
               (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                                  MaxE>::bit_count +
                   b.exponent() + 1) {
      if (eval_lsb(b.bits()) != cpp_bin_float<Digits, DigitBase, Allocator,
                                              Exponent, MinE, MaxE>::bit_count -
                                    1) {
    ...
    dt = b.bits();
    if (b.exponent() <= (int)cpp_bin_float<Digits, DigitBase, Allocator,
                                           Exponent, MinE, MaxE>::bit_count +
                            a.exponent()) {
      typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                             MaxE>::exponent_type e_diff =
      ...
      eval_subtract(dt, a.bits());
               ...
               (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                                  MaxE>::bit_count +
                   a.exponent() + 1) {
      if (eval_lsb(a.bits()) != cpp_bin_float<Digits, DigitBase, Allocator,
                                              Exponent, MinE, MaxE>::bit_count -
                                    1) {
        eval_left_shift(dt, 1);
        eval_decrement(dt);
...
inline void eval_multiply(
  ...
  res.exponent() = a.exponent() + b.exponent() -
                   (Exponent)cpp_bin_float<Digits, DigitBase, Allocator,
                                           Exponent, MinE, MaxE>::bit_count +
                   1;
  copy_and_round(res, dt);
  res.check_invariants();
...
inline typename enable_if_c<is_unsigned<U>::value>::type eval_multiply(
  ...
  typedef typename boost::multiprecision::detail::canonical<
      U, typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                                MaxE>::double_rep_type>::type canon_ui_type;
...
inline typename enable_if_c<is_signed<S>::value>::type eval_multiply(
  ...
  eval_multiply(
      res, a,
      static_cast<ui_type>(boost::multiprecision::detail::unsigned_abs(b)));
  if (b < 0)
    res.negate();
}
...
#pragma warning(disable : 6326) // comparison of two constants
  ...
  case cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
    ...
    bool s = u.sign() != v.sign();
    res = std::numeric_limits<number<cpp_bin_float<
        Digits, DigitBase, Allocator, Exponent, MinE, MaxE>>>::infinity()
              .backend();
    res.sign() = s;
    return;
  }
  ...
  // q + r/v = u/v
  //
  // From this, assuming q has cpp_bin_float<Digits, DigitBase, Allocator,
  // Exponent, MinE, MaxE>::bit_count bits we only need to determine whether r/v
  // is less than, equal to, or greater than 0.5 to determine rounding - this we
  // can do with a shift and comparison.
  //
  ...
  //
  typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                         MaxE>::double_rep_type t(u.bits()),
      t2(v.bits()), q, r;
  eval_left_shift(t, cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                                   MaxE>::bit_count);
  eval_qr(t, t2, q, r);
  //
  // We now have either "cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
  // MinE, MaxE>::bit_count" or "cpp_bin_float<Digits, DigitBase, Allocator,
  // Exponent, MinE, MaxE>::bit_count+1" significant bits in q.
  //
  static const unsigned limb_bits = sizeof(limb_type) * CHAR_BIT;
  if (eval_bit_test(q, cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                     MinE, MaxE>::bit_count)) {
    //
    // OK we have cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
    // MaxE>::bit_count+1 bits, so we already have rounding info, we just need
    // to changes things if the last bit is 1 and either the remainder is
    // non-zero (ie we do not have a tie) or the quotient would be odd if it
    // were shifted to the correct number of bits (ie a tiebreak).
    //
    BOOST_ASSERT(
        (eval_msb(q) == cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                      MinE, MaxE>::bit_count));
    ...
    //
    // We have exactly "cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
    // MinE, MaxE>::bit_count" bits in q. Get rounding info, which we can get by
    // comparing 2r with v. We want to call copy_and_round to handle rounding
    // and general cleanup, so we'll left shift q and add some fake digits on
    // the end to represent how we'll be rounding.
    //
    BOOST_ASSERT(
        (eval_msb(q) == cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                      MinE, MaxE>::bit_count -
...
#pragma warning(disable : 6326) // comparison of two constants
  ...
  //
  // From this, assuming q has "cpp_bin_float<Digits, DigitBase, Allocator,
  // Exponent, MinE, MaxE>::bit_count" cpp_bin_float<Digits, DigitBase,
  // Allocator, Exponent, MinE, MaxE>::bit_count, we only need to determine
  // whether r/v is less than, equal to, or greater than 0.5 to determine
  ...
  // We now have either "cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
  // MinE, MaxE>::bit_count" or "cpp_bin_float<Digits, DigitBase, Allocator,
  // Exponent, MinE, MaxE>::bit_count+1" significant cpp_bin_float<Digits,
  // DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count in q.
  //
  static const unsigned limb_bits = sizeof(limb_type) * CHAR_BIT;
  if (eval_bit_test(q, cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                     MinE, MaxE>::bit_count)) {
    //
    // OK we have cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
    // MaxE>::bit_count+1 cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
    // MinE, MaxE>::bit_count, so we already have rounding info, we just need to
    // changes things if the last bit is 1 and the remainder is non-zero (ie we
    // do not have a tie).
    //
    BOOST_ASSERT(
        (eval_msb(q) == cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
    ...
    //
    // We have exactly "cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
    // MinE, MaxE>::bit_count" cpp_bin_float<Digits, DigitBase, Allocator,
    // Exponent, MinE, MaxE>::bit_count in q. Get rounding info, which we can
    // get by comparing 2r with v. We want to call copy_and_round to handle
    // rounding and general cleanup, so we'll left shift q and add some fake
    // cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
    // MaxE>::bit_count on the end to represent how we'll be rounding.
    //
    BOOST_ASSERT(
        (eval_msb(q) == cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                      MinE, MaxE>::bit_count -
...
inline typename enable_if_c<is_signed<S>::value>::type eval_divide(
  ...
  eval_divide(
      res, u,
      static_cast<ui_type>(boost::multiprecision::detail::unsigned_abs(v)));
  if (v < 0)
    res.negate();
}
...
inline bool eval_eq(
  ...
  if (a.exponent() == b.exponent()) {
    if (a.exponent() == cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                      MinE, MaxE>::exponent_zero)
      return true;
    return (a.sign() == b.sign()) && (a.bits().compare(b.bits()) == 0) &&
           (a.exponent() != cpp_bin_float<Digits, DigitBase, Allocator,
                                          Exponent, MinE, MaxE>::exponent_nan);
  }
  return false;
}
...
template <unsigned Digits, digit_base_type DigitBase, class Allocator,
          class Exponent, Exponent MinE, Exponent MaxE>
inline void eval_convert_to(boost::long_long_type *res,
                            const cpp_bin_float<Digits, DigitBase, Allocator,
                                                Exponent, MinE, MaxE> &arg) {
  switch (arg.exponent()) {
  case cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                     MaxE>::exponent_zero:
  ...
  typedef typename mpl::if_c<
      sizeof(typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                    MinE, MaxE>::exponent_type) < sizeof(int),
      int,
      typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                             MaxE>::exponent_type>::type shift_type;
  typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                         MaxE>::rep_type man(arg.bits());
  shift_type shift =
      (shift_type)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                                MaxE>::bit_count -
      1 - arg.exponent();
  if (shift > (shift_type)cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                        MinE, MaxE>::bit_count -
                  1) {
    *res = 0;
    return;
  ...
  if (arg.sign() &&
      (arg.compare((std::numeric_limits<boost::long_long_type>::min)()) <= 0)) {
    *res = (std::numeric_limits<boost::long_long_type>::min)();
    return;
  } else if (!arg.sign() &&
             (arg.compare(
                  (std::numeric_limits<boost::long_long_type>::max)()) >= 0)) {
    *res = (std::numeric_limits<boost::long_long_type>::max)();
    return;
  }
  ...
  if (shift < 0) {
    if (cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
      ...
      // We have more bits in long_long_type than the float, so it's OK to left
      // shift:
...
template <unsigned Digits, digit_base_type DigitBase, class Allocator,
          class Exponent, Exponent MinE, Exponent MaxE>
inline void eval_convert_to(boost::ulong_long_type *res,
                            const cpp_bin_float<Digits, DigitBase, Allocator,
                                                Exponent, MinE, MaxE> &arg) {
  switch (arg.exponent()) {
  case cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
  ...
  if (shift > (shift_type)cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
    ...
    *res = 0;
    ...
    if (cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
      ...
      // We have more bits in ulong_long_type than the float, so it's OK to left
      // shift:
      eval_convert_to(res, man);
      *res <<= -shift;
      return;
...
template <class Float, unsigned Digits, digit_base_type DigitBase,
...
eval_convert_to(Float *res,
                const cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                    MinE, MaxE> &original_arg) {
  typedef cpp_bin_float<std::numeric_limits<Float>::digits, digit_base_2, void,
                        Exponent, MinE, MaxE>
      conv_type;
  typedef typename common_type<typename conv_type::exponent_type, int>::type
      common_exp_type;
  //
  // Special cases first:
  //
...
template <unsigned Digits, digit_base_type DigitBase, class Allocator,
          class Exponent, Exponent MinE, Exponent MaxE, class I>
inline void eval_frexp(
    cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res,
    const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>
        &arg,
  ...
  if ((e > (std::numeric_limits<I>::max)()) ||
      (e < (std::numeric_limits<I>::min)())) {
    BOOST_THROW_EXCEPTION(std::runtime_error(
        "Exponent was outside of the range of the argument type to frexp."));
  }
  *pe = static_cast<I>(e);
}
...
inline void eval_ldexp(
  ...
  if ((e > 0) && (cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                                MaxE>::max_exponent -
                      e <
                  arg.exponent())) {
    // Overflow:
    res = std::numeric_limits<number<cpp_bin_float<
        Digits, DigitBase, Allocator, Exponent, MinE, MaxE>>>::infinity()
              .backend();
    res.sign() = arg.sign();
  } else if ((e < 0) && (cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                       MinE, MaxE>::min_exponent -
                             e >
                         arg.exponent())) {
    // Underflow:
    res = limb_type(0);
...
template <unsigned Digits, digit_base_type DigitBase, class Allocator,
          class Exponent, Exponent MinE, Exponent MaxE, class I>
inline typename enable_if_c<is_unsigned<I>::value>::type eval_ldexp(
    cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res,
    const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>
        &arg,
    I e) {
...
template <unsigned Digits, digit_base_type DigitBase, class Allocator,
          class Exponent, Exponent MinE, Exponent MaxE, class I>
inline typename enable_if_c<is_signed<I>::value>::type eval_ldexp(
    cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res,
    const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>
        &arg,
    I e) {
  if ((e > (std::numeric_limits<Exponent>::max)()) ||
      (e < (std::numeric_limits<Exponent>::min)())) {
    res = std::numeric_limits<number<cpp_bin_float<
        Digits, DigitBase, Allocator, Exponent, MinE, MaxE>>>::infinity()
              .backend();
    if (e < 0)
...
template <unsigned Digits, digit_base_type DigitBase, class Allocator,
          class Exponent, Exponent MinE, Exponent MaxE>
inline int eval_fpclassify(const cpp_bin_float<Digits, DigitBase, Allocator,
                                               Exponent, MinE, MaxE> &arg) {
  switch (arg.exponent()) {
  case cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                     MaxE>::exponent_zero:
...
inline void eval_sqrt(
  ...
  typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                         MaxE>::double_rep_type t(arg.bits()),
      r, s;
  eval_left_shift(t, arg.exponent() & 1
                         ? cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                         MinE, MaxE>::bit_count
                         : cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                         MinE, MaxE>::bit_count -
  ...
  if (!eval_bit_test(s, cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                      MinE, MaxE>::bit_count)) {
    // We have exactly the right number of cpp_bin_float<Digits, DigitBase,
    // Allocator, Exponent, MinE, MaxE>::bit_count in the result, round as
    // required:
    if (s.compare(r) < 0) {
      eval_increment(s);
    }
  ...
  typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                         MaxE>::exponent_type ae = arg.exponent();
  res.exponent() = ae / 2;
  if ((ae & 1) && (ae < 0))
    --res.exponent();
...
inline void eval_floor(
  ...
  if (fractional && res.sign()) {
    ...
    if (eval_msb(res.bits()) != cpp_bin_float<Digits, DigitBase, Allocator,
                                              Exponent, MinE, MaxE>::bit_count -
                                    1 - shift) {
...
template <unsigned D1, backends::digit_base_type B1, class A1, class E1, E1 M1,
          E1 M2>
...
template <unsigned D1, backends::digit_base_type B1, class A1, class E1, E1 M1,
          E1 M2>
inline std::size_t
hash_value(const cpp_bin_float<D1, B1, A1, E1, M1, M2> &val) {
...
template <unsigned D1, backends::digit_base_type B1, class A1, class E1, E1 M1,
          E1 M2, unsigned D2, backends::digit_base_type B2, class A2, class E2,
          E2 M3, E2 M4>
struct is_explicitly_convertible<
    backends::cpp_bin_float<D1, B1, A1, E1, M1, M2>,
    backends::cpp_bin_float<D2, B2, A2, E2, M3, M4>> : public mpl::true_ {};
template <class FloatT, unsigned D2, backends::digit_base_type B2, class A2,
          class E2, E2 M3, E2 M4>
struct is_explicitly_convertible<
    FloatT, backends::cpp_bin_float<D2, B2, A2, E2, M3, M4>>
...
template <unsigned Digits,
          ...
          class Exponent, Exponent MinE, Exponent MaxE, class Allocator,
          boost::multiprecision::expression_template_option ExpressionTemplates>
inline boost::multiprecision::number<
    boost::multiprecision::backends::cpp_bin_float<Digits, DigitBase, Allocator,
                                                   Exponent, MinE, MaxE>,
    ExpressionTemplates>
...
template <unsigned Digits, backends::digit_base_type DigitBase, class Exponent,
...
struct expression_template_default<
    cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>> {
  static const expression_template_option value =
      is_void<Allocator>::value ? et_off : et_on;
};

typedef number<backends::cpp_bin_float<50>> cpp_bin_float_50;
typedef number<backends::cpp_bin_float<100>> cpp_bin_float_100;

typedef number<backends::cpp_bin_float<24, backends::digit_base_2, void,
                                       boost::int16_t, -126, 127>,
               et_off>
    cpp_bin_float_single;
typedef number<backends::cpp_bin_float<53, backends::digit_base_2, void,
                                       boost::int16_t, -1022, 1023>,
               et_off>
    cpp_bin_float_double;
typedef number<backends::cpp_bin_float<64, backends::digit_base_2, void,
                                       boost::int16_t, -16382, 16383>,
               et_off>
    cpp_bin_float_double_extended;
typedef number<backends::cpp_bin_float<113, backends::digit_base_2, void,
                                       boost::int16_t, -16382, 16383>,
               et_off>
    cpp_bin_float_quad;
typedef number<backends::cpp_bin_float<237, backends::digit_base_2, void,
                                       boost::int32_t, -262142, 262143>,
               et_off>
    cpp_bin_float_oct;
...
public:
  ...
  static number_type(min)() {
    ...
    if (!value.first) {
      ...
      typedef typename boost::mpl::front<
          typename number_type::backend_type::unsigned_types>::type ui_type;
      value.second.backend() = ui_type(1u);
      value.second.backend().exponent() = boost::multiprecision::cpp_bin_float<
          Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::min_exponent;
    }
    return value.second;
  }
  static number_type(max)() {
    ...
    if (!value.first) {
      ...
      if (boost::is_void<Allocator>::value)
        eval_complement(value.second.backend().bits(),
                        value.second.backend().bits());
      else {
        // We jump through hoops here using the backend type directly just to
        // keep VC12 happy (ie compiler workaround, for very strange compiler
        // bug):
        ...
        typedef typename boost::mpl::front<
            typename int_backend_type::unsigned_types>::type ui_type;
        int_backend_type i;
        i = ui_type(1u);
        eval_left_shift(
            i,
            boost::multiprecision::cpp_bin_float<
                Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count -
                1);
      ...
      value.second.backend().exponent() = boost::multiprecision::cpp_bin_float<
          Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::max_exponent;
    }
    return value.second;
  }
  ...
  static number_type epsilon() {
    ...
    if (!value.first) {
      ...
      // workaround, for very strange compiler bug):
      typedef typename boost::mpl::front<
          typename number_type::backend_type::unsigned_types>::type ui_type;
      value.first = true;
      value.second.backend() = ui_type(1u);
  ...
  BOOST_STATIC_CONSTEXPR typename boost::multiprecision::cpp_bin_float<
      Digits, DigitBase, Allocator, Exponent, MinE,
      MaxE>::exponent_type min_exponent =
      boost::multiprecision::cpp_bin_float<Digits, DigitBase, Allocator,
                                           Exponent, MinE, MaxE>::min_exponent;
  BOOST_STATIC_CONSTEXPR typename boost::multiprecision::cpp_bin_float<
      Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::exponent_type
      min_exponent10 = (min_exponent / 1000) * 301L;
  BOOST_STATIC_CONSTEXPR typename boost::multiprecision::cpp_bin_float<
      Digits, DigitBase, Allocator, Exponent, MinE,
      MaxE>::exponent_type max_exponent =
      boost::multiprecision::cpp_bin_float<Digits, DigitBase, Allocator,
                                           Exponent, MinE, MaxE>::max_exponent;
  BOOST_STATIC_CONSTEXPR typename boost::multiprecision::cpp_bin_float<
      Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::exponent_type
  ...
  static number_type quiet_NaN() {
    ...
    if (!value.first) {
      value.first = true;
      value.second.backend().exponent() = boost::multiprecision::cpp_bin_float<
          Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::exponent_nan;
    }
    return value.second;
  }
    ...
    data_initializer() {
      std::numeric_limits<
          boost::multiprecision::number<boost::multiprecision::cpp_bin_float<
              Digits, DigitBase, Allocator, Exponent, MinE, MaxE>>>::epsilon();
      std::numeric_limits<
          boost::multiprecision::number<boost::multiprecision::cpp_bin_float<
              Digits, DigitBase, Allocator, Exponent, MinE, MaxE>>>::
          round_error();
      (std::numeric_limits<
          boost::multiprecision::number<boost::multiprecision::cpp_bin_float<
              Digits, DigitBase, Allocator, Exponent, MinE, MaxE>>>::min)();
      (std::numeric_limits<
          boost::multiprecision::number<boost::multiprecision::cpp_bin_float<
              Digits, DigitBase, Allocator, Exponent, MinE, MaxE>>>::max)();
      std::numeric_limits<
          boost::multiprecision::number<boost::multiprecision::cpp_bin_float<
              Digits, DigitBase, Allocator, Exponent, MinE, MaxE>>>::infinity();
      std::numeric_limits<
