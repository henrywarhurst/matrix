...
BOOST_HEADER_DEPRECATED("Boost.Test minimal is deprecated. Please convert to "
                        "the header only variant of Boost.Test.")

#define BOOST_CHECK(exp)                                                       \
  ((exp) ? static_cast<void>(0)                                                \
         : boost::minimal_test::report_error(#exp, __FILE__, __LINE__,         \
                                             BOOST_CURRENT_FUNCTION))

#define BOOST_REQUIRE(exp)                                                     \
  ((exp) ? static_cast<void>(0)                                                \
         : boost::minimal_test::report_critical_error(                         \
               #exp, __FILE__, __LINE__, BOOST_CURRENT_FUNCTION))
...
#define BOOST_FAIL(msg_)                                                       \
  boost::minimal_test::report_critical_error((msg_), __FILE__, __LINE__,       \
                                             BOOST_CURRENT_FUNCTION, true)
...
inline unit_test::counter_t &errors_counter() {
  static unit_test::counter_t ec = 0;
  return ec;
}

inline void report_error(const char *msg, const char *file, int line,
                         const_string func_name, bool is_msg = false) {
  ++errors_counter();
  std::cerr << file << "(" << line << "): ";
...
inline void report_critical_error(const char *msg, const char *file, int line,
                                  const_string func_name, bool is_msg = false) {
...
public:
  // constructor
  caller(int argc, char **argv) : m_argc(argc), m_argv(argv) {}
...
int BOOST_TEST_CALL_DECL main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (exex.code() != boost::execution_exception::no_error)
      BOOST_ERROR(
          (std::string("exception \"") + exex.what() + "\" caught").c_str());
    std::cerr << "\n**** Testing aborted.";
  }

  if (boost::minimal_test::errors_counter() != 0) {
    std::cerr << "\n**** " << errors_counter() << " error"
              << (errors_counter() > 1 ? "s" : "") << " detected\n";
