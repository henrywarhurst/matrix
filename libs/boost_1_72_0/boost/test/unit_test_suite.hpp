...
#define BOOST_AUTO_TEST_SUITE_WITH_DECOR(suite_name, decorators)               \
  ...
  (BOOST_STRINGIZE(suite_name), __FILE__, __LINE__, decorators);               \
  /**/

#define BOOST_AUTO_TEST_SUITE_NO_DECOR(suite_name)                             \
  BOOST_AUTO_TEST_SUITE_WITH_DECOR(                                            \
      suite_name, boost::unit_test::decorator::collector_t::instance())        \
  /**/

#if BOOST_PP_VARIADICS
#define BOOST_AUTO_TEST_SUITE(...)                                             \
  BOOST_TEST_INVOKE_IF_N_ARGS(1, BOOST_AUTO_TEST_SUITE_NO_DECOR,               \
                              BOOST_AUTO_TEST_SUITE_WITH_DECOR, __VA_ARGS__)   \
  /**/
...
#define BOOST_FIXTURE_TEST_SUITE(...)                                          \
  BOOST_TEST_INVOKE_IF_N_ARGS(2, BOOST_FIXTURE_TEST_SUITE_NO_DECOR,            \
...
#define BOOST_FIXTURE_TEST_CASE_NO_DECOR(test_name, F)                         \
  BOOST_FIXTURE_TEST_CASE_WITH_DECOR(                                          \
      test_name, F, boost::unit_test::decorator::collector_t::instance())      \
  /**/
...
#define BOOST_FIXTURE_TEST_CASE(...)                                           \
  BOOST_TEST_INVOKE_IF_N_ARGS(2, BOOST_FIXTURE_TEST_CASE_NO_DECOR,             \
                              BOOST_FIXTURE_TEST_CASE_WITH_DECOR, __VA_ARGS__) \
  /**/
...
#define BOOST_AUTO_TEST_CASE_NO_DECOR(test_name)                               \
  BOOST_FIXTURE_TEST_CASE_NO_DECOR(test_name, BOOST_AUTO_TEST_CASE_FIXTURE)    \
  /**/

#define BOOST_AUTO_TEST_CASE_WITH_DECOR(test_name, decorators)                 \
  BOOST_FIXTURE_TEST_CASE_WITH_DECOR(test_name, BOOST_AUTO_TEST_CASE_FIXTURE,  \
                                     decorators)                               \
  /**/
...
#define BOOST_AUTO_TEST_CASE(...)                                              \
  BOOST_TEST_INVOKE_IF_N_ARGS(1, BOOST_AUTO_TEST_CASE_NO_DECOR,                \
                              BOOST_AUTO_TEST_CASE_WITH_DECOR, __VA_ARGS__)    \
  /**/
...
#define BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_name, type_name, TL, F)          \
  template <typename type_name> struct test_name : public F {                  \
    void test_method();                                                        \
  };                                                                           \
                                                                               \
  ...
  (boost::unit_test::ut_detail::template_test_case_gen<                        \
       BOOST_AUTO_TC_INVOKER(test_name), TL>(BOOST_STRINGIZE(test_name),       \
                                             __FILE__, __LINE__),              \
   boost::unit_test::decorator::collector_t::instance());                      \
                                                                               \
  template <typename type_name> void test_name<type_name>::test_method() /**/
...
#define BOOST_GLOBAL_FIXTURE(F)                                                \
  static boost::unit_test::ut_detail::global_configuration_impl<F> BOOST_JOIN( \
      gf_, F) /**/
...
#define BOOST_TEST_GLOBAL_FIXTURE(F)                                           \
  static boost::unit_test::ut_detail::global_fixture_impl<F> BOOST_JOIN(       \
      gf_, F) /**/
...
#define BOOST_TEST_DECORATOR(D)                                                \
  static boost::unit_test::decorator::collector_t const                        \
      &BOOST_TEST_APPEND_UNIQUE_ID(decorator_collector)                        \
          BOOST_ATTRIBUTE_UNUSED = D;                                          \
  /**/
...
// Intentionally is in global namespace, so that FIXTURE_TEST_SUITE can reset it
// in user code.
...
// Facility for having a unique name based on __LINE__ and __COUNTER__ (later if
// available)
...
/**/
#else
#define BOOST_TEST_APPEND_UNIQUE_ID(name) BOOST_JOIN(name, __LINE__)
/**/
#endif
/**/
...
static const boost::unit_test::framework::impl::master_test_suite_name_setter
    mtsetter(BOOST_TEST_STRINGIZE(BOOST_TEST_MODULE).trim("\""));
#endif
