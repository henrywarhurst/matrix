...
template <class T> struct parameter_holder {
...
boost_bind_rvalue_holder_helper_impl(
    T &&value, boost::false_type /* is copy constructible */) {
  return parameter_holder<typename std::remove_reference<T>::type>(
      std::forward<T>(value));
}

template <class T>
T &&boost_bind_rvalue_holder_helper_impl(
    T &&value, boost::true_type /* is copy constructible */) {
...
auto boost_bind_rvalue_holder_helper(T &&value)
    -> decltype(boost_bind_rvalue_holder_helper_impl(
        std::forward<T>(value),
        typename boost::is_copy_constructible<
            typename std::remove_reference<T>::type>::type())) {
  // need to use boost::is_copy_constructible because std::is_copy_constructible
  // is broken on MSVC12
...
public:
  ...
  test_case_gen(const_string tc_name, const_string tc_file, std::size_t tc_line,
                DataSet &&ds)
      : m_dataset(std::forward<DataSet>(ds)), m_generated(false),
        m_tc_name(ut_detail::normalize_test_case_name(tc_name)),
        m_tc_file(tc_file), m_tc_line(tc_line), m_tc_index(0) {}
  test_case_gen(test_case_gen &&gen)
      : m_dataset(std::move(gen.m_dataset)), m_generated(gen.m_generated),
        m_tc_name(gen.m_tc_name), m_tc_file(gen.m_tc_file),
        m_tc_line(gen.m_tc_line), m_tc_index(gen.m_tc_index),
        m_test_cases(std::move(gen.m_test_cases)) {}
#else
  test_case_gen(const_string tc_name, const_string tc_file, std::size_t tc_line,
                DataSet const &ds)
      : m_dataset(ds), m_generated(false),
        m_tc_name(ut_detail::normalize_test_case_name(tc_name)),
        m_tc_file(tc_file), m_tc_line(tc_line), m_tc_index(0) {}
#endif
...
#define TC_MAKE(z, arity, _)                                                   \
  ...
  void operator()(BOOST_PP_ENUM_BINARY_PARAMS(arity, Arg, const &arg)) const { \
    m_test_cases.push_back(new test_case(                                      \
        genTestCaseName(), m_tc_file, m_tc_line,                               \
        boost::bind(                                                           \
...
#else
  template <typename... Arg> void operator()(Arg &&... arg) const {
    m_test_cases.push_back(new test_case(
        genTestCaseName(), m_tc_file, m_tc_line,
...
make_test_case_gen(const_string tc_name, const_string tc_file,
                   std::size_t tc_line, DataSet &&ds) {
  return boost::shared_ptr<test_unit_generator>(
      new test_case_gen<TestCase, DataSet>(tc_name, tc_file, tc_line,
                                           std::forward<DataSet>(ds)));
}
#else
template <typename TestCase, typename DataSet>
test_case_gen<TestCase, DataSet>
make_test_case_gen(const_string tc_name, const_string tc_file,
                   std::size_t tc_line, DataSet const &ds) {
...
#define BOOST_DATA_TEST_CASE_PARAM(r, _, i, param)                             \
  (BOOST_PP_CAT(Arg, i) const &param)
#define BOOST_DATA_TEST_CONTEXT(r, _, param)                                   \
  << BOOST_STRINGIZE(param) << " = "                                           \
  << boost::test_tools::tt_detail::print_helper(param) << "; "
...
#define BOOST_DATA_TEST_CASE_IMPL(arity, F, test_name, dataset, params)        \
  ...
  (boost::unit_test::data::ds_detail::make_test_case_gen<BOOST_PP_CAT(         \
       test_name, case)>(BOOST_STRINGIZE(test_name), __FILE__, __LINE__,       \
  ...
  BOOST_AUTO_TEST_SUITE_END()                                                  \
                                                                               \
  template <BOOST_PP_ENUM_PARAMS(arity, typename Arg)>                         \
  void BOOST_PP_CAT(test_name,                                                 \
                    case)::_impl(BOOST_DATA_TEST_CASE_PARAMS(params)) /**/

#define BOOST_DATA_TEST_CASE_WITH_PARAMS(F, test_name, dataset, ...)           \
  BOOST_DATA_TEST_CASE_IMPL(BOOST_PP_VARIADIC_SIZE(__VA_ARGS__), F, test_name, \
                            dataset, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))    \
/**/
...
#define BOOST_DATA_TEST_CASE_F(F, ...)                                         \
  BOOST_PP_CAT(                                                                \
      BOOST_PP_IIF(BOOST_PP_EQUAL(BOOST_PP_VARIADIC_SIZE(__VA_ARGS__), 2),     \
                   BOOST_DATA_TEST_CASE_NO_PARAMS,                             \
                   BOOST_DATA_TEST_CASE_WITH_PARAMS)(F, __VA_ARGS__), )        \
  /**/
