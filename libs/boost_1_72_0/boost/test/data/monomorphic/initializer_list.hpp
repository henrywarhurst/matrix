...
public:
  ...
  init_list(std::initializer_list<T> il) : m_data(il) {}

#if !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES) &&                             \
    !defined(BOOST_TEST_ERRONEOUS_INIT_LIST)
  //! Variadic template initialization
  template <class... Args> init_list(Args &&... args) {
...
public:
  ...
  init_list(std::initializer_list<bool> &&il)
    ...
    !defined(BOOST_TEST_ERRONEOUS_INIT_LIST)
  //! Variadic template initialization
  template <class... Args> init_list(Args &&... args) : m_data{args...} {}
#endif
  ...
  struct non_proxy_iterator {
    ...
    bool operator*() const { return *iterator; }
...
template <typename T> struct is_dataset<init_list<T>> : mpl::true_ {};
...
template <typename T>
inline monomorphic::init_list<T> make(std::initializer_list<T> &&il) {
...
make(T &&arg0, Args &&... args) {
  return monomorphic::init_list<T>(std::forward<T>(arg0),
