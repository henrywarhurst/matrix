 ...
 * boost::unit_test::data::monomorphic::generated_by) for implementing a range
 * like sequence of numbers.
 */
template <typename SampleType, typename StepType = SampleType> class xrange_t {
public:
  typedef SampleType sample;

  xrange_t(SampleType const &begin_, StepType const &step_, data::size_t size_)
      : m_begin(begin_), m_curr(begin_), m_step(step_), m_index(0),
        m_size(size_) {}
...
struct make_xrange {
  ...
  static generated_by<range_gen> _(Params const &params) {
    SampleType begin_val =
        params.has(data::begin) ? params[data::begin] : SampleType();
    optional<SampleType> end_val =
        params.has(data::end) ? params[data::end] : optional<SampleType>();
    ...
    if (!end_val.is_initialized())
      ...
      BOOST_TEST_DS_ASSERT((step_val < 0) ^ (begin_val < *end_val),
                           "Invalid step direction");

      SampleType abs_distance =
          step_val < 0 ? begin_val - *end_val : *end_val - begin_val;
      StepType abs_step = make_xrange::abs(
          step_val, (typename boost::is_unsigned<StepType>::type *)0);
...
//! - @c end_val is the end of the sequence. If ommited, the dataset has
//! infinite size.\n
//! - @c step_val is the step between two consecutive elements of the sequence,
//! and defaults to 1.\n
//! - @c param is the named parameters that describe the sequence. The following
...
//!
//! @note the step size cannot be null, and it should be positive if @c
//! begin_val < @c end_val, negative otherwise.
...
xrange(SampleType const &end_val) {
  return monomorphic::ds_detail::make_xrange<SampleType>::_(data::end =
                                                                end_val);
}
...
inline typename enable_if_c<
    nfp::is_named_param_pack<Params>::value,
...
xrange(SampleType const &begin_val, SampleType const &end_val) {
  return monomorphic::ds_detail::make_xrange<SampleType>::_(
      (data::begin = begin_val, data::end = end_val));
}
...
xrange(SampleType const &begin_val, SampleType const &end_val,
  ...
  return monomorphic::ds_detail::make_xrange<SampleType, StepType>::_(
      (data::begin = begin_val, data::end = end_val, data::step = step_val));
}
