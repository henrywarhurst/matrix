...
#if !defined(BOOST_TEST_NO_RANDOM_DATASET_AVAILABLE) ||                        \
    defined(BOOST_TEST_DOXYGEN_DOC__)
...
namespace ds_detail {
template <typename SampleType> struct default_distribution {
  typedef
      typename mpl::if_<std::is_integral<SampleType>,
 ...
 *
 * This class implements the generator concept (see @ref
 * boost::unit_test::data::monomorphic::generated_by) for implementing a random
 * number generator.
 */
...
public:
  ...
  random_t() : m_distribution(), m_engine(std::random_device()()) {}
  ...
  SampleType next() { return m_distribution(m_engine); }
  void reset() {}

  //! Sets the seed of the pseudo-random number engine.
  template <typename SeedType> void seed(SeedType &&seed) {
    m_engine.seed(std::forward<SeedType>(seed));
  }
...
//!
//!
//! - The first overload uses the default distribution, which is uniform and
//! which elements
//!   are @c double type (the values are in [0, 1) ).
//! - The second overload generates numbers in the given interval. The
//! distribution is uniform (in [begin, end)
//!   for real numbers, and in [begin, end] for integers). The type of the
//!   distribution is deduced from the type of the @c begin and @c end
//!   parameters.
//! - The third overload generates numbers using the named parameter inside @c
//! params , which are:
//!   - @c distribution: the distribution used. In this overload, since the type
//!   of the samples cannot be deduced,
//!     the samples are of type @c double and the distribution is uniform real
//!     in [0, 1).
...
namespace ds_detail {
template <typename Params> struct random_gen_type {
  typedef typename nfp::param_type<Params, decltype(distribution),
                                   std::uniform_real_distribution<>>::type
      distr_type;
  typedef
      typename nfp::param_type<Params, decltype(engine),
                               std::default_random_engine>::type engine_type;
...
inline monomorphic::generated_by<
    typename ds_detail::random_gen_type<Params>::type>
