...
/// constructed with the @c data::make_delayed<dataset_t>(arg1,....) instead of
/// the
/// @c data::make.
...
public:
  ...
  delayed_dataset(delayed_dataset &&b)
      : m_args(std::move(b.m_args)), m_dataset(std::move(b.m_dataset)) {}

  boost::unit_test::data::size_t size() const { return this->get().size(); }

  // iterator
  iterator begin() const { return this->get().begin(); }
...
make_delayed(Args... args) {
  return monomorphic::delayed_dataset<dataset_t, Args...>(
