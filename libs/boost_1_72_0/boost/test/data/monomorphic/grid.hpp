...
#if !defined(BOOST_TEST_NO_GRID_COMPOSITION_AVAILABLE) ||                      \
...
//! Implements the dataset resulting from a cartesian product/grid operation on
//! datasets.
//!
...
public:
  struct iterator {
    ...
    explicit iterator(dataset1_iter iter1, DataSet2 const &ds2)
        : m_iter1(std::move(iter1)), m_iter2(std::move(ds2.begin())),
          m_ds2(&ds2), m_ds2_pos(0) {}

    using iterator_sample = decltype(sample_merge(
        *std::declval<dataset1_iter>(), *std::declval<dataset2_iter>()));
...
public:
  enum {
    arity = boost::decay<DataSet1>::type::arity +
            boost::decay<DataSet2>::type::arity
  };
  ...
  data::size_t size() const {
    BOOST_TEST_DS_ASSERT(!m_ds1.size().is_inf() && !m_ds2.size().is_inf(),
                         "Grid axes can't have infinite size");
...
inline typename boost::lazy_enable_if_c<
    ...
    result_of::grid<mpl::identity<DataSet1>, mpl::identity<DataSet2>>>::type
operator*(DataSet1 &&ds1, DataSet2 &&ds2) {
  return grid<DataSet1, DataSet2>(std::forward<DataSet1>(ds1),
...
inline typename boost::lazy_enable_if_c<
    is_dataset<DataSet1>::value && !is_dataset<DataSet2>::value,
    result_of::grid<mpl::identity<DataSet1>,
...
inline typename boost::lazy_enable_if_c<
    !is_dataset<DataSet1>::value && is_dataset<DataSet2>::value,
    result_of::grid<data::result_of::make<DataSet1>,
