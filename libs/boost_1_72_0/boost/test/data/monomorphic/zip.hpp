...
#if !defined(BOOST_TEST_NO_ZIP_COMPOSITION_AVAILABLE) ||                       \
...
//! Zip datasets
//!
//! A zip of two datasets is a dataset whose arity is the sum of the operand
//! datasets arity. The size is given by the function creating the instance (see
//! @c operator^ on datasets).
...
public:
  ...
  struct iterator {
    ...
    explicit iterator(dataset1_iter iter1, dataset2_iter iter2)
        : m_iter1(std::move(iter1)), m_iter2(std::move(iter2)) {}
...
inline typename boost::lazy_enable_if_c<
    ...
    result_of::zip<mpl::identity<DataSet1>, mpl::identity<DataSet2>>>::type
...
inline typename boost::lazy_enable_if_c<
    is_dataset<DataSet1>::value && !is_dataset<DataSet2>::value,
    result_of::zip<mpl::identity<DataSet1>,
...
inline typename boost::lazy_enable_if_c<
    !is_dataset<DataSet1>::value && is_dataset<DataSet2>::value,
    result_of::zip<data::result_of::make<DataSet1>,
