...
template <class dataset_t, class... Args> class delayed_dataset;
#endif
...
template <typename DataSet> struct is_dataset : mpl::false_ {};
...
template <class DataSet, class...> struct has_dataset : is_dataset<DataSet> {};

template <class DataSet0, class DataSet1, class... DataSetTT>
struct has_dataset<DataSet0, DataSet1, DataSetTT...>
    : std::integral_constant<bool,
                             is_dataset<DataSet0>::value ||
                                 has_dataset<DataSet1, DataSetTT...>::value> {};
#endif
...
//!
//! // creates a singleton dataset, for non forward iterable and non dataset
//! type T
...
//! template<typename T, std::size_t size> monomorphic::array<T> make( T
//! (&a)[size] );
//! @endcode
template <typename DataSet>
inline typename std::enable_if<monomorphic::is_dataset<DataSet>::value,
                               DataSet>::type
make(DataSet &&ds) {
...
// warning: doxygen is apparently unable to handle @overload from different
// files, so if the overloads below are not declared with @overload in THIS
// file, they do not appear in the documentation.

//! @overload boost::unit_test::data::make()
template <typename T>
inline typename std::enable_if<
    !is_container_forward_iterable<T>::value &&
...
//! @overload boost::unit_test::data::make()
template <typename C>
inline typename std::enable_if<is_container_forward_iterable<C>::value,
...
//! @overload boost::unit_test::data::make()
template <typename T>
inline monomorphic::init_list<T> make(std::initializer_list<T> &&);
...
inline typename std::enable_if<!monomorphic::has_dataset<T, Args...>::value,
                               monomorphic::init_list<T>>::type
...
inline typename std::enable_if<
    ...
    monomorphic::delayed_dataset<dataset_t, Args...>>::type
