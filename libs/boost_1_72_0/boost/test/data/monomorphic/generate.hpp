 ...
 *
 * This class implements the dataset concept over a generator. Examples of
 * generators are:
 * - xrange_t
 * - random_t
 *
 ...
 * - the member function @c capacity should return the size of the collection
 * being generated (potentially infinite)
 * - the member function @c next should change the state of the generator to the
 * next generated value
 * - the member function @c reset should put the state of the object in the same
 * state as right after its instanciation
 */
template <typename Generator> class generated_by {
public:
  ...
  struct iterator {
    // Constructor
    explicit iterator(Generator &gen) : m_gen(&gen) {
      if (m_gen->capacity() > 0) {
        m_gen->reset();
        ++*this;
      }
