...
struct test_unit_id_restore {
  ...
  ~test_unit_id_restore() { to_restore = bkup; }
...
template <class T> struct static_constant { static T value; };

template <class T> T static_constant<T>::value;
...
#define BOOST_TEST_SINGLETON_INST(inst)                                        \
  static BOOST_JOIN(inst, _t) &inst BOOST_ATTRIBUTE_UNUSED =                   \
      BOOST_JOIN(inst, _t)::instance();
...
#define BOOST_TEST_SINGLETON_INST(inst)                                        \
  namespace {                                                                  \
  BOOST_JOIN(inst,                                                             \
             _t) &inst BOOST_ATTRIBUTE_UNUSED = BOOST_JOIN(inst,               \
                                                           _t)::instance();    \
  }
