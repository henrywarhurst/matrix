...
template <typename CharT> class default_char_compare {
public:
...
template <typename CharT, typename CharCompare> class delim_policy {
...
template <typename TraversalTag> struct token_assigner {
...
template <> struct token_assigner<single_pass_traversal_tag> {
  template <typename Iterator, typename Token>
  static void assign(Iterator /*b*/, Iterator /*e*/, Token & /*t*/) {}
...
protected:
  ...
  template <typename Modifier> void apply_modifier(Modifier const &m) {
  ...
  template <typename Iter> bool get(Iter &begin, Iter end) {
...
public:
  ...
  // definition was "template<typename Src, typename Modifier>
...
private:
  ...
  // input iterator implementation
  bool get() { return base::get(m_begin, m_end); }
