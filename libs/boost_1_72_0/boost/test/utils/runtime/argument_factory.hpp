...
template <typename ValueType, bool is_enum> struct value_interpreter;

//____________________________________________________________________________//

template <typename ValueType> struct value_interpreter<ValueType, false> {
  template <typename Modifiers> explicit value_interpreter(Modifiers const &) {}
...
template <> struct value_interpreter<std::string, false> {
  template <typename Modifiers> explicit value_interpreter(Modifiers const &) {}
...
template <> struct value_interpreter<cstring, false> {
  template <typename Modifiers> explicit value_interpreter(Modifiers const &) {}

  cstring interpret(cstring, cstring source) const { return source; }
};

//____________________________________________________________________________//

template <> struct value_interpreter<bool, false> {
  template <typename Modifiers> explicit value_interpreter(Modifiers const &) {}
...
template <typename EnumType> struct value_interpreter<EnumType, true> {
  ...
  EnumType interpret(cstring param_name, cstring source) const {
    ...
    BOOST_TEST_I_ASSRT(
        ...
        format_error(param_name)
            << source << " is not a valid enumeration value name for parameter "
            << param_name << ".");
...
public:
  ...
  void produce_argument(cstring source, cstring param_name,
    ...
    store.set(param_name, source.empty()
                              ? m_optional_value
