...
template <typename T> class typed_argument : public argument {
public:
  // Constructor
...
public:
  ...
  template <typename T> T const &get(cstring parameter_name) const {
    return const_cast<arguments_store *>(this)->get<T>(parameter_name);
  }

  template <typename T> T &get(cstring parameter_name) {
  ...
  template <typename T> void set(cstring parameter_name, T const &value) {
