...
struct parameter_trie {
  ...
  /// If subtrie corresponding to the char c exists returns it otherwise creates
  /// new
  parameter_trie_ptr make_subtrie(char c) {
    ...
    if (it == m_subtrie.end())
      it =
          m_subtrie
              .insert(std::make_pair(c, parameter_trie_ptr(new parameter_trie)))
              .first;
  ...
  void add_candidate_id(parameter_cla_id const &param_id,
    ...
    BOOST_TEST_I_ASSRT(
        ...
        conflicting_param()
            << "Parameter cla id " << param_id.m_tag << " conflicts with the "
...
static void report_foreing_token(cstring program_name, cstring token) {
  std::cerr << "Boost.Test WARNING: token \"" << token
            ...
            << "                    and should be placed after all Boost.Test "
               "arguments and the -- separator.\n"
...
public:
...
#endif
  {
    ...
    BOOST_TEST_I_ASSRT(algorithm::all_of(m_end_of_param_indicator.begin(),
                       ...
                       invalid_cla_id()
                           << "End of parameters indicator can only consist of "
                              "prefix characters.");

    BOOST_TEST_I_ASSRT(
        algorithm::all_of(m_negation_prefix.begin(), m_negation_prefix.end(),
  ...
  int parse(int argc, char **argv, runtime::arguments_store &res) {
    ...
    while (!tr.eoi()) {
      ...
      // Perform format validations and split the argument into prefix, name and
      // separator False return value indicates end of params indicator is met
      if (!validate_token_format(curr_token, prefix, name, value_separator,
                                 negative_form)) {
      ...
      // Skip validations if parameter has optional value and we are at the end
      // of token
      if (!value_separator.is_empty() || !found_param->p_has_optional_value) {
        ...
        value = curr_token;
        if (value.is_empty()) {
          tr.next_token(); // tokenization broke the value, we check the next
                           // one
          ...
          if (!found_param->p_has_optional_value) {
            ...
            if (validate_token_format(value_check, prefix_test, name_test,
                                      value_separator_test,
                                      negative_form_test) &&
      ...
      BOOST_TEST_I_ASSRT(!res.has(found_param->p_name) ||
                             found_param->p_repeatable,
  ...
  void usage(std::ostream &ostr, cstring param_name = cstring(),
    ...
    if (!param_name.is_empty()) {
      basic_param_ptr param =
          locate_parameter(m_param_trie[help_prefix], param_name, "").second;
  ...
  void help(std::ostream &ostr, parameters_store const &parameters,
    ...
    runtime::commandline_pretty_print(
        ...
        "environment "
        "variable. If a flag is specified as a command line argument and an "
        "environment variable "
        "at the same time, the command line takes precedence. "
        "The command line argument "
        "support name guessing, and works with shorter names as long as those "
        "are not ambiguous.");
  ...
  bool validate_token_format(cstring token, cstring &prefix, cstring &name,
    ...
    if (name.empty()) {
      ...
      BOOST_TEST_I_THROW(format_error()
                         << "Invalid format for an actual argument " << token);
    }
    ...
    BOOST_TEST_FOREACH(char, c, name) {
      if (curr_trie) {
        ...
        if (next_trie)
          ...
          // Initiate search for typo candicates. We will account for 'wrong
          // char' typo 'missing char' typo and 'extra char' typo
          BOOST_TEST_FOREACH(trie_per_char::value_type const &, typo_cand,
                             curr_trie->m_subtrie) {
            // 'wrong char' typo
