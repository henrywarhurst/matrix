...
public:
  ...
  template <typename T> Derived operator<<(T const &val) && {
  ...
  template <typename T> Derived const &operator<<(T const &val) const {
...
class BOOST_SYMBOL_VISIBLE unrecognized_param
