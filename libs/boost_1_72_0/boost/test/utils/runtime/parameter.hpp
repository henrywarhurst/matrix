...
struct parameter_cla_id {
  parameter_cla_id(cstring prefix, cstring tag, cstring value_separator,
                   bool negatable)
      : m_prefix(prefix.begin(), prefix.size()), m_tag(tag.begin(), tag.size()),
        m_value_separator(value_separator.begin(), value_separator.size()),
        m_negatable(negatable) {

    BOOST_TEST_I_ASSRT(
        algorithm::all_of(m_prefix.begin(), m_prefix.end(), valid_prefix_char),
    ...
    BOOST_TEST_I_ASSRT(
        algorithm::all_of(m_value_separator.begin(), m_value_separator.end(),
                          valid_separator_char),
        invalid_cla_id() << "Parameter " << m_tag
                         << " has invalid characters in value separator.");
  }

  static bool valid_prefix_char(char c) { return c == '-' || c == '/'; }
...
protected:
  void add_cla_id_impl(cstring prefix, cstring tag, cstring value_separator,
    ...
    // We trim value separator from all the spaces, so token end will indicate
    // separator
    value_separator.trim();
    BOOST_TEST_I_ASSRT(!validate_value_separator ||
                           !value_separator.is_empty() || !p_has_optional_value,
                       invalid_cla_id()
                           << "Parameter " << tag
                           << " with optional value attribute can't use space "
                              "as value separator.");
...
class parameters_store {
  struct lg_compare {
    bool operator()(cstring lh, cstring rh) const {
      return std::lexicographical_compare(lh.begin(), lh.end(), rh.begin(),
                                          rh.end());
    }
  };
...
public:
  ...
  basic_param_ptr get(cstring name) const {
    ...
    BOOST_TEST_I_ASSRT(found != m_parameters.end(), unknown_param()
                                                        << "Parameter " << name
                                                        << " is unknown.");
