...
#if defined(BOOST_NO_CXX11_DECLTYPE) || defined(BOOST_NO_CXX11_NULLPTR) ||     \
    defined(BOOST_NO_CXX11_TRAILING_RESULT_TYPES)

// this feature works with VC2012 upd 5 while
// BOOST_NO_CXX11_TRAILING_RESULT_TYPES is defined
...
template <typename T> struct is_forward_iterable : public mpl::false_ {};
...
template <> struct is_forward_iterable<std::string> : public mpl::true_ {};
...
// SFINAE helper
template <typename T> struct is_present : public mpl::true_ {};

//____________________________________________________________________________//

// some compiler do not implement properly decltype non expression involving
// members (eg. VS2013) a workaround is to use -> decltype syntax.
template <class T> struct has_member_size {
private:
  struct nil_t {};
  template <typename U>
  static auto test(U *) -> decltype(boost::declval<U>().size());
  template <typename> static nil_t test(...);

public:
  static bool const value =
      !std::is_same<decltype(test<T>(nullptr)), nil_t>::value;
};
...
private:
  ...
  static auto test(U *) -> decltype(
      std::begin(boost::declval<U &>())); // does not work with boost::begin
  template <typename> static nil_t test(...);
...
template <class T> struct has_member_end {
private:
  struct nil_t {};
  template <typename U>
  static auto test(U *) -> decltype(
      std::end(boost::declval<U &>())); // does not work with boost::end
  template <typename> static nil_t test(...);
...
struct is_forward_iterable_impl<
    T, typename std::enable_if<has_member_begin<T>::value &&
                               has_member_end<T>::value>::type>
    : std::true_type {};
...
struct is_container_forward_iterable_impl<
    T, typename std::enable_if<is_present<typename T::const_iterator>::value &&
                               is_present<typename T::value_type>::value &&
                               has_member_size<T>::value &&
                               is_forward_iterable_impl<T>::value>::type>
...
template <typename T> struct bt_iterator_traits<T, true> {
...
#else
  typedef decltype(boost::declval<typename boost::add_const<
  ...
  static const_iterator end(T const &container) { return container.end(); }

#if defined(BOOST_TEST_FWD_ITERABLE_CXX03) ||                                  \
    (defined(BOOST_MSVC) && (BOOST_MSVC_FULL_VER <= 170061232))
  static std::size_t size(T const &container) { return container.size(); }
#else
...
private:
  static std::size_t size(T const &container, std::true_type) {
    return container.size();
  }

  static std::size_t size(T const &container, std::false_type) {
...
template <typename T, std::size_t N> struct bt_iterator_traits<T[N], true> {
  ...
  static const_iterator begin(T_const (&array)[N]) { return &array[0]; }
  static const_iterator end(T_const (&array)[N]) { return &array[N]; }
  static std::size_t size(T_const (&)[N]) { return N; }
};
