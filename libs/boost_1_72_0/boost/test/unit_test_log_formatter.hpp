...
struct BOOST_TEST_DECL log_entry_data {
  log_entry_data() { m_file_name.reserve(200); }
...
public:
  ...
  ///
  /// @param[in] os               output stream to write a messages into
  /// @param[in] log_build_info   indicates if build info should be logged or
  /// not
  ...
  virtual void test_unit_timed_out(std::ostream & /* os */,
                                   test_unit const & /* tu */) {}
  ...
  ///
  /// @param[in] os   output stream to write a messages into
  /// @param[in] lcd  information about the last checkpoint before the exception
  /// was triggered
  ...
  ///
  /// @note call to this function may happen before any call to test_unit_start
  /// or all calls to test_unit_finish as the framework might log errors raised
  /// during global initialization/shutdown.
  ...
  /// This is one of two overloaded methods to report log entry content. This
  /// one is used to report some complicated expression passed as an expression
  /// template lazy_ostream. In most cases default implementation provided by
  /// the framework should work as is (it just converts the lazy expression into
  /// a string.
  /// @param[in] os   output stream to write a messages into
  ...
  /// context if one was defined by a test module. Context consists of multiple
  /// "scopes" identified by description messages assigned by the test module
  /// using BOOST_TEST_INFO/BOOST_TEST_CONTEXT statements.
  /// @param[in] os   output stream to write a messages into
  ...
  /// and @b stderr have special meaning indicating the standard output or error
  /// stream respectively.
  ///
  /// @par Since Boost 1.62
