...
#define DEFINE_CSTRING_COMPARISON(oper, name, rev)                             \
  ...
  struct name<Lhs, Rhs,                                                        \
    ...
    typedef                                                                    \
        typename unit_test::deduce_cstring_transform<Lhs>::type lhs_char_type; \
    typedef                                                                    \
        typename unit_test::deduce_cstring_transform<Rhs>::type rhs_char_type; \
                                                                               \
  public:                                                                      \
    typedef assertion_result result_type;                                      \
                                                                               \
    typedef name<typename lhs_char_type::value_type,                           \
                 typename rhs_char_type::value_type>                           \
        elem_op;                                                               \
                                                                               \
    static bool eval(Lhs const &lhs, Rhs const &rhs) {                         \
