...
#define BOOST_TEST_TOOL_DIRECT_IMPL(P, level, M)                               \
  do {                                                                         \
    ...
    report_assertion(res, BOOST_TEST_LAZY_MSG(M), BOOST_TEST_L(__FILE__),      \
...
#define BOOST_TEST_TOOL_ET_IMPL(P, level)                                      \
  do {                                                                         \
    ...
    ::boost::test_tools::tt_detail::report_assertion(                          \
        ...
        BOOST_TEST_LAZY_MSG(BOOST_TEST_STRINGIZE(P)), BOOST_TEST_L(__FILE__),  \
        static_cast<std::size_t>(__LINE__),                                    \
        ::boost::test_tools::tt_detail::level,                                 \
...
// Implementation based on expression template construction with extra tool
// arguments
#define BOOST_TEST_TOOL_ET_IMPL_EX(P, level, arg)                              \
  do {                                                                         \
    ...
    ::boost::test_tools::tt_detail::report_assertion(                          \
        ...
        ::boost::test_tools::tt_detail::level,                                 \
        ::boost::test_tools::tt_detail::assertion_type() << arg, 0);           \
  } while (::boost::test_tools::tt_detail::dummy_cond()) /**/
...
#define BOOST_TEST_WARN(...)                                                   \
  BOOST_TEST_INVOKE_IF_N_ARGS(2, BOOST_TEST_TOOL_UNIV,                         \
                              BOOST_TEST_TOOL_UNIV_EX, WARN, __VA_ARGS__)      \
/**/
#define BOOST_TEST_CHECK(...)                                                  \
  BOOST_TEST_INVOKE_IF_N_ARGS(2, BOOST_TEST_TOOL_UNIV,                         \
                              BOOST_TEST_TOOL_UNIV_EX, CHECK, __VA_ARGS__)     \
/**/
#define BOOST_TEST_REQUIRE(...)                                                \
  BOOST_TEST_INVOKE_IF_N_ARGS(2, BOOST_TEST_TOOL_UNIV,                         \
...
#define BOOST_TEST_IS_DEFINED(symb)                                            \
  ::boost::test_tools::tt_detail::is_defined_impl(symb, BOOST_STRINGIZE(= symb))
...
#define BOOST_WARN_THROW(S, E)                                                 \
  BOOST_CHECK_THROW_IMPL(S, E const &, WARN, false,                            \
                         "exception " BOOST_STRINGIZE(E) " is expected", true, \
                         "exception " BOOST_STRINGIZE(E) " is caught")         \
/**/
#define BOOST_CHECK_THROW(S, E)                                                \
  BOOST_CHECK_THROW_IMPL(S, E const &, CHECK, false,                           \
                         "exception " BOOST_STRINGIZE(E) " is expected", true, \
                         "exception " BOOST_STRINGIZE(E) " is caught")         \
/**/
#define BOOST_REQUIRE_THROW(S, E)                                              \
  BOOST_CHECK_THROW_IMPL(S, E const &, REQUIRE, false,                         \
                         "exception " BOOST_STRINGIZE(E) " is expected", true, \
                         "exception " BOOST_STRINGIZE(E) " is caught")         \
  /**/
...
#define BOOST_WARN_EXCEPTION(S, E, P)                                          \
  BOOST_CHECK_THROW_IMPL(                                                      \
      S, E const &ex, WARN, false,                                             \
      "exception " BOOST_STRINGIZE(E) " is expected", P(ex),                   \
      "incorrect exception " BOOST_STRINGIZE(E) " is caught")                  \
/**/
#define BOOST_CHECK_EXCEPTION(S, E, P)                                         \
  BOOST_CHECK_THROW_IMPL(                                                      \
      S, E const &ex, CHECK, false,                                            \
      "exception " BOOST_STRINGIZE(E) " is expected", P(ex),                   \
      "incorrect exception " BOOST_STRINGIZE(E) " is caught")                  \
/**/
#define BOOST_REQUIRE_EXCEPTION(S, E, P)                                       \
  BOOST_CHECK_THROW_IMPL(                                                      \
      S, E const &ex, REQUIRE, false,                                          \
      "exception " BOOST_STRINGIZE(E) " is expected", P(ex),                   \
      "incorrect exception " BOOST_STRINGIZE(E) " is caught")                  \
  /**/
...
#define BOOST_WARN_NO_THROW(S)                                                 \
  BOOST_CHECK_THROW_IMPL(S, ..., WARN, true,                                   \
                         "no exceptions thrown by " BOOST_STRINGIZE(S), false, \
                         "exception thrown by " BOOST_STRINGIZE(S))            \
/**/
#define BOOST_CHECK_NO_THROW(S)                                                \
  BOOST_CHECK_THROW_IMPL(S, ..., CHECK, true,                                  \
                         "no exceptions thrown by " BOOST_STRINGIZE(S), false, \
                         "exception thrown by " BOOST_STRINGIZE(S))            \
/**/
#define BOOST_REQUIRE_NO_THROW(S)                                              \
  BOOST_CHECK_THROW_IMPL(S, ..., REQUIRE, true,                                \
                         "no exceptions thrown by " BOOST_STRINGIZE(S), false, \
                         "exception thrown by " BOOST_STRINGIZE(S))            \
  /**/
...
#define BOOST_WARN_CLOSE(L, R, T)                                              \
  BOOST_TEST_WARN(L == R, T % ::boost::test_tools::tolerance())
#define BOOST_CHECK_CLOSE(L, R, T)                                             \
  BOOST_TEST_CHECK(L == R, T % ::boost::test_tools::tolerance())
#define BOOST_REQUIRE_CLOSE(L, R, T)                                           \
  BOOST_TEST_REQUIRE(L == R, T % ::boost::test_tools::tolerance())

#define BOOST_WARN_CLOSE_FRACTION(L, R, T)                                     \
  BOOST_TEST_WARN(L == R, ::boost::test_tools::tolerance(T))
#define BOOST_CHECK_CLOSE_FRACTION(L, R, T)                                    \
  BOOST_TEST_CHECK(L == R, ::boost::test_tools::tolerance(T))
#define BOOST_REQUIRE_CLOSE_FRACTION(L, R, T)                                  \
  BOOST_TEST_REQUIRE(L == R, ::boost::test_tools::tolerance(T))

#define BOOST_WARN_SMALL(FPV, T)                                               \
  BOOST_TEST_WARN(FPV == 0., ::boost::test_tools::tolerance(T))
#define BOOST_CHECK_SMALL(FPV, T)                                              \
  BOOST_TEST_CHECK(FPV == 0., ::boost::test_tools::tolerance(T))
#define BOOST_REQUIRE_SMALL(FPV, T)                                            \
  BOOST_TEST_REQUIRE(FPV == 0., ::boost::test_tools::tolerance(T))

//____________________________________________________________________________//

#define BOOST_WARN_EQUAL_COLLECTIONS(L_begin, L_end, R_begin, R_end)           \
  BOOST_TEST_WARN(                                                             \
      ::boost::test_tools::tt_detail::make_it_pair(L_begin, L_end) ==          \
...
#define BOOST_WARN_BITWISE_EQUAL(L, R)                                         \
  BOOST_TEST_WARN(L == R, ::boost::test_tools::bitwise())
#define BOOST_CHECK_BITWISE_EQUAL(L, R)                                        \
  BOOST_TEST_CHECK(L == R, ::boost::test_tools::bitwise())
#define BOOST_REQUIRE_BITWISE_EQUAL(L, R)                                      \
  BOOST_TEST_REQUIRE(L == R, ::boost::test_tools::bitwise())

//____________________________________________________________________________//

#define BOOST_WARN_PREDICATE(P, ARGS)                                          \
  BOOST_TEST_WARN(P BOOST_PP_SEQ_TO_TUPLE(ARGS))
#define BOOST_CHECK_PREDICATE(P, ARGS)                                         \
  BOOST_TEST_CHECK(P BOOST_PP_SEQ_TO_TUPLE(ARGS))
#define BOOST_REQUIRE_PREDICATE(P, ARGS)                                       \
  BOOST_TEST_REQUIRE(P BOOST_PP_SEQ_TO_TUPLE(ARGS))
