...
#define BOOST_TEST_FOR_EACH_CONST_OP(action)                                   \
  action(->*, MEMP, ->*)                                                       \
                                                                               \
      action(*, MUL, *) action(/, DIV, /) action(%, MOD, %)                    \
                                                                               \
          action(+, ADD, +) action(-, SUB, -)                                  \
                                                                               \
              action(<<, LSH, <<) action(>>, RSH, >>)                          \
                                                                               \
                  BOOST_TEST_FOR_EACH_COMP_OP(action)                          \
                                                                               \
                      action(&, BAND, &) action(^, XOR, ^)                     \
                          action(|, BOR, |) /**/
...
#define BOOST_TEST_FOR_EACH_MUT_OP(action)                                     \
  action(=, SET, =) action(+=, IADD, +=) action(-=, ISUB, -=)                  \
      action(*=, IMUL, *=) action(/=, IDIV, /=) action(%=, IMOD, %=)           \
          action(<<=, ILSH, <<=) action(>>=, IRSH, >>=) action(&=, IAND, &=)   \
              action(^=, IXOR, ^=) action(|=, IOR, |=) /**/
...
#define DEFINE_CONST_OPER(oper, name, rev)                                     \
  template <typename Lhs, typename Rhs, typename Enabler = void> struct name { \
    typedef DEDUCE_RESULT_TYPE(oper) result_type;                              \
                                                                               \
    static result_type eval(Lhs const &lhs, Rhs const &rhs) {                  \
      return lhs oper rhs;                                                     \
    }                                                                          \
                                                                               \
    template <typename PrevExprType>                                           \
    static void report(std::ostream &ostr, PrevExprType const &lhs,            \
                       Rhs const &rhs) {                                       \
      lhs.report(ostr);                                                        \
...
template <typename ExprType, typename ValType> class expression_base {
public:
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
  template <typename T>
...
#define ADD_OP_SUPPORT(oper, name, _)                                          \
  template <typename T>                                                        \
  binary_expr<ExprType, T, op::name<ValType, typename RhsT<T>::type>>          \
  operator oper(T &&rhs) {                                                     \
    return binary_expr<ExprType, T,                                            \
                       op::name<ValType, typename RhsT<T>::type>>(             \
        std::forward<ExprType>(*static_cast<ExprType *>(this)),                \
        std::forward<T>(rhs));                                                 \
  }                                                                            \
/**/
#else
...
#define ADD_OP_SUPPORT(oper, name, _)                                          \
  ...
  operator oper(T const &rhs) const {                                          \
    ...
    return binary_expr<ExprType, Rhs, op::name<ValType, Rhs>>(                 \
        *static_cast<ExprType const *>(this), rhs);                            \
  }                                                                            \
/**/
#endif
...
#undef ADD_OP_SUPPORT
  ...
  // Disabled operators
  template <typename T> ExprType &operator||(T const & /*rhs*/) {
    BOOST_MPL_ASSERT_MSG(
        false, CANT_USE_LOGICAL_OPERATOR_OR_WITHIN_THIS_TESTING_TOOL, ());
  ...
  template <typename T> ExprType &operator&&(T const & /*rhs*/) {
    BOOST_MPL_ASSERT_MSG(
        false, CANT_USE_LOGICAL_OPERATOR_AND_WITHIN_THIS_TESTING_TOOL, ());
  ...
  operator bool() {
    BOOST_MPL_ASSERT_MSG(
        false, CANT_USE_TERNARY_OPERATOR_WITHIN_THIS_TESTING_TOOL, ());
...
public:
  ...
  // Constructor
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
  value_expr(value_expr &&ve) : m_value(std::forward<T>(ve.m_value)) {}
  explicit value_expr(T &&val) : m_value(std::forward<T>(val)) {}
#else
  explicit value_expr(T const &val) : m_value(val) {}
#endif
...
#define ADD_OP_SUPPORT(OPER, ID, _)                                            \
  template <typename U> value_expr<T> &operator OPER(U const &rhs) {           \
    m_value OPER rhs;                                                          \
                                                                               \
    return *this;                                                              \
  }                                                                            \
...
private:
  ...
  static void format_message(wrap_stringstream & /*ostr*/,
                             assertion_result const & /*v*/) {}
...
class binary_expr : public expression_base<binary_expr<LExpr, Rhs, OP>,
...
public:
  ...
  binary_expr(LExpr &&lhs, Rhs &&rhs)
      : m_lhs(std::forward<LExpr>(lhs)), m_rhs(std::forward<Rhs>(rhs)) {}
#else
  binary_expr(LExpr const &lhs, Rhs const &rhs) : m_lhs(lhs), m_rhs(rhs) {}
#endif

  // Specific expression interface
  result_type value() const { return OP::eval(m_lhs.value(), m_rhs); }
  void report(std::ostream &ostr) const {
  ...
  assertion_result evaluate(bool no_message = false) const {
    ...
    return tt_detail::format_assertion_result(buff.stream().str(),
                                              expr_res.message());
  }
