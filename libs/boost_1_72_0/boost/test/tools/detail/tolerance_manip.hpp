...
template <typename FPT> struct tolerance_manip {
...
inline tolerance_manip<FPT> operator%(FPT v, tolerance_manip_delay const &) {
  BOOST_STATIC_ASSERT_MSG(
      (fpc::tolerance_based<FPT>::value),
...
inline assertion_result operator<<(assertion_evaluate_t<E> const &ae,
                                   tolerance_manip<FPT> const &tol) {
...
template <typename FPT>
inline int operator<<(unit_test::lazy_ostream const &,
                      tolerance_manip<FPT> const &) {
  return 0;
}
...
template <typename FPT>
inline check_type operator<<(assertion_type const & /*at*/,
                             tolerance_manip<FPT> const &) {
 ...
 *
 * These functions return a manipulator that can be used in conjunction with
 * BOOST_TEST in order to specify the tolerance with which floating point
 * comparisons are made.
 */
template <typename FPT>
inline tt_detail::tolerance_manip<FPT> tolerance(FPT v) {
...
//! @overload tolerance( FPT v )
inline tt_detail::tolerance_manip_delay tolerance() {
