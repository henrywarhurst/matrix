...
inline int operator<<(unit_test::lazy_ostream const &, per_element) {
  return 0;
}
...
operator<<(assertion_evaluate_t<assertion::binary_expr<T1, T2, OP>> const &ae,
  ...
  return assertion::op::element_compare<elem_op>(ae.m_e.lhs().value(),
                                                 ae.m_e.rhs());
}

//____________________________________________________________________________//

inline check_type operator<<(assertion_type const &, per_element) {
