...
inline int operator<<(unit_test::lazy_ostream const &, bitwise) { return 0; }
 ...
 *
 * This class constructs an @ref assertion_result that contains precise bit
 * comparison information. In particular the location of the mismatches (if any)
 * are printed in the assertion result.
 */
template <typename Lhs, typename Rhs, typename E>
inline assertion_result bitwise_compare(Lhs const &lhs, Rhs const &rhs,
                                        E const &expr) {
...
//! Returns an assertion_result using the bitwise comparison out of an
//! expression
//!
...
inline assertion_result operator<<(
    assertion_evaluate_t<
        assertion::binary_expr<T1, T2, assertion::op::EQ<T3, T4>>> const &ae,
    bitwise) {
...
inline check_type operator<<(assertion_type const &, bitwise) {
