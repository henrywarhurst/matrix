...
public:
  static const bool value = !is_reference<T>::value && !is_void<T>::value &&
                            (sizeof(test<T>(0)) == sizeof(Two));
};

// warning: we cannot instanciate std::numeric_limits for incomplete types, we
// use is_abstract_class_or_function prior to the specialization below
template <typename T>
 ...
 * @c T can be compared using a tolerance based method, typically for floating
 * point types.
 *
...
struct tolerance_based
    : tolerance_based_delegate<
          T, !is_array<T>::value &&
                 !is_abstract_class_or_function<T>::value>::type {};
...
inline FPT fraction_tolerance(ToleranceType tolerance) {
...
inline FPT2 fraction_tolerance(percent_tolerance_t<FPT> tolerance) {
 ...
 *
 * This predicate is used to compare floating point numbers. In addition the
 * comparison produces maximum related difference, which can be used to generate
 * detailed error message The methods for comparing floating points are detailed
 * in the documentation. The method is chosen by the @ref
 * boost::math::fpc::strength given at construction.
 *
...
public:
  ...
  explicit close_at_tolerance(ToleranceType tolerance,
      ...
      : m_fraction_tolerance(fpc_detail::fraction_tolerance<FPT>(tolerance)),
        m_strength(fpc_strength), m_tested_rel_diff(0) {
    BOOST_ASSERT_MSG(
        m_fraction_tolerance >= FPT(0),
        "tolerance must not be negative!"); // no reason for tolerance to be
                                            // negative
  }
  ...
  /*! Compares two floating point numbers a and b such that their "left"
   * relative difference |a-b|/a and/or "right" relative difference |a-b|/b does
   * not exceed specified relative (fraction) tolerance.
   *
   *  @param[in] left first floating point number to be compared
   *  @param[in] right second floating point number to be compared
   *
   * What is reported by @c tested_rel_diff in case of failure depends on the
   * comparison method:
   * - for @c FPC_STRONG: the max of the two fractions
   * - for @c FPC_WEAK: the min of the two fractions
   * The rationale behind is to report the tolerance to set in order to make a
   * test pass.
   */
  bool operator()(FPT left, FPT right) const {
    FPT diff = fpc_detail::fpt_abs<FPT>(left - right);
    FPT fraction_of_right =
        fpc_detail::safe_fpt_division(diff, fpc_detail::fpt_abs(right));
    FPT fraction_of_left =
        fpc_detail::safe_fpt_division(diff, fpc_detail::fpt_abs(left));
 ...
 *
 * Serves the same purpose as boost::math::fpc::close_at_tolerance, but used
...
public:
  ...
  explicit small_with_tolerance(FPT tolerance) // <= absolute tolerance
    ...
    BOOST_ASSERT(m_tolerance >=
                 FPT(0)); // no reason for the tolerance to be negative
  }
...
template <typename FPT> inline bool is_small(FPT fpv, FPT tolerance) {
