...
template <typename T> struct specialized_compare : public mpl::false_ {};

template <typename T> struct is_c_array : public mpl::false_ {};
...
#define BOOST_TEST_SPECIALIZED_COLLECTION_COMPARE(Col)                         \
  ...
  namespace test_tools {                                                       \
  namespace assertion {                                                        \
  template <> struct specialized_compare<Col> : public mpl::true_ {};          \
  }                                                                            \
  }                                                                            \
  }                                                                            \
  /**/
...
template <typename OP, bool can_be_equal, bool prefer_shorter, typename Lhs,
          typename Rhs>
...
lexicographic_compare(Lhs const &lhs, Rhs const &rhs) {
  ...
  for (; (first1 != last1) && (first2 != last2); ++first1, ++first2, ++pos) {
    ...
    if (element_ar || !reverse_ar)
      continue; // (a<=b and b<=a) or (!(a<b) and !(b<a)) => a == b => keep
                // looking

    // !(a<=b) and b<=a => b < a => return false
    ar = false;
    ar.message() << "\nFailure at position " << pos << ": "
                 << tt_detail::print_helper(*first1) << OP::revert()
...
template <typename OP, bool can_be_equal, bool prefer_shorter, typename Lhs,
          typename Rhs>
inline typename boost::enable_if_c<(unit_test::is_cstring<Lhs>::value ||
                                    unit_test::is_cstring<Rhs>::value),
                                   assertion_result>::type
lexicographic_compare(Lhs const &lhs, Rhs const &rhs) {
  ...
  return lexicographic_compare<OP, can_be_equal, prefer_shorter>(
      lhs_char_type(lhs), rhs_char_type(rhs));
}
...
element_compare(Lhs const &lhs, Rhs const &rhs) {
  ...
  if (t_Lhs_iterator::size(lhs) != t_Rhs_iterator::size(rhs)) {
    ar = false;
    ar.message() << "\nCollections size mismatch: " << t_Lhs_iterator::size(lhs)
                 << " != " << t_Rhs_iterator::size(rhs);
    return ar;
  }
  ...
  for (; pos < t_Lhs_iterator::size(lhs); ++left, ++right, ++pos) {
    ...
    ar.message() << "\nMismatch at position " << pos << ": "
                 << tt_detail::print_helper(*left) << OP::revert()
...
element_compare(Lhs const &lhs, Rhs const &rhs) {
  ...
  return element_compare<OP>(lhs_char_type(lhs), rhs_char_type(rhs));
}
...
template <typename OP, typename Lhs, typename Rhs>
inline assertion_result non_equality_compare(Lhs const &lhs, Rhs const &rhs) {
  typedef unit_test::bt_iterator_traits<Lhs> t_Lhs_iterator;
  typedef unit_test::bt_iterator_traits<Rhs> t_Rhs_iterator;
...
template <typename OP> struct cctraits;

template <typename Lhs, typename Rhs> struct cctraits<op::EQ<Lhs, Rhs>> {
  typedef specialized_compare<Lhs> is_specialized;
};

template <typename Lhs, typename Rhs> struct cctraits<op::NE<Lhs, Rhs>> {
  typedef specialized_compare<Lhs> is_specialized;
};

template <typename Lhs, typename Rhs> struct cctraits<op::LT<Lhs, Rhs>> {
...
template <typename Lhs, typename Rhs> struct cctraits<op::LE<Lhs, Rhs>> {
...
template <typename Lhs, typename Rhs> struct cctraits<op::GT<Lhs, Rhs>> {
  static const bool can_be_equal = false;
...
template <typename Lhs, typename Rhs> struct cctraits<op::GE<Lhs, Rhs>> {
  static const bool can_be_equal = true;
  static const bool prefer_short = false;
...
// **************              compare_collections             ************** //
// ************************************************************************** //
// Overloaded set of functions dispatching to specific implementation of
// comparison

template <typename Lhs, typename Rhs, typename L, typename R>
inline assertion_result compare_collections(Lhs const &lhs, Rhs const &rhs,
                                            boost::type<op::EQ<L, R>> *,
                                            mpl::true_) {
...
template <typename OP, typename Lhs, typename Rhs>
inline assertion_result lexicographic_compare(Lhs const &lhs, Rhs const &rhs) {
  return assertion::op::lexicographic_compare<OP, cctraits<OP>::can_be_equal,
                                              cctraits<OP>::prefer_short>(lhs,
                                                                          rhs);
}
...
#define DEFINE_COLLECTION_COMPARISON(oper, name, rev)                          \
  ...
  public:                                                                      \
    ...
    typedef typename mpl::if_c<                                                \
        mpl::or_<typename is_c_array<Lhs>::type,                               \
                 typename is_c_array<Rhs>::type>::value,                       \
        mpl::true_,                                                            \
        typename mpl::if_c<is_same<typename decay<Lhs>::type,                  \
                                   typename decay<Rhs>::type>::value,          \
                           typename cctraits<OP>::is_specialized,              \
                           mpl::false_>::type>::type is_specialized;           \
                                                                               \
    typedef name<typename t_Lhs_iterator_helper::value_type,                   \
                 typename t_Rhs_iterator_helper::value_type>                   \
        elem_op;                                                               \
                                                                               \
    static assertion_result eval(Lhs const &lhs, Rhs const &rhs) {             \
      return assertion::op::compare_collections(                               \
          lhs, rhs, (boost::type<elem_op> *)0, is_specialized());              \
    }                                                                          \
                                                                               \
    template <typename PrevExprType>                                           \
    static void report(std::ostream &, PrevExprType const &, Rhs const &) {}   \
                                                                               \
    static char const *revert() { return " " #rev " "; }                       \
  };                                                                           \
  /**/
