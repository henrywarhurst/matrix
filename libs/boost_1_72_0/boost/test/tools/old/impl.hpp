...
// This function adds level of indirection, but it makes sure we evaluate
// predicate arguments only once
...
#define ARG_INFO(z, m, dummy)                                                  \
  , BOOST_JOIN(BOOST_JOIN(arg, m), _descr),                                    \
      &static_cast<const unit_test::lazy_ostream &>(                           \
          unit_test::lazy_ostream::instance()                                  \
          << ::boost::test_tools::tt_detail::print_helper(                     \
                 BOOST_JOIN(arg, m))) /**/

#define IMPL_FRWD(z, n, dummy)                                                 \
  template <typename Pred BOOST_PP_REPEAT_##z(BOOST_PP_ADD(n, 1),              \
                                              TEMPL_PARAMS, _)>                \
  inline bool check_frwd(                                                      \
      Pred P, unit_test::lazy_ostream const &assertion_descr,                  \
      const_string file_name, std::size_t line_num, tool_level tl,             \
      check_type ct BOOST_PP_REPEAT_##z(BOOST_PP_ADD(n, 1), FUNC_PARAMS, _)) { \
...
inline assertion_result equal_impl(char *left, char const *right) {
  return equal_impl(static_cast<char const *>(left),
                    static_cast<char const *>(right));
}
...
BOOST_TEST_DECL assertion_result equal_impl(wchar_t const *left,
                                            wchar_t const *right);
inline assertion_result equal_impl(wchar_t *left, wchar_t const *right) {
  return equal_impl(static_cast<wchar_t const *>(left),
                    static_cast<wchar_t const *>(right));
}
...
struct equal_impl_frwd {
  ...
  inline assertion_result call_impl(Left const &left, Right const &right,
                                    mpl::false_) const {
  ...
  template <typename Left, typename Right>
  inline assertion_result operator()(Left const &left,
...
struct bitwise_equal_impl {
  ...
  assertion_result operator()(Left const &left, Right const &right) {
    ...
    std::size_t total_bits =
        left_bit_size < right_bit_size ? left_bit_size : right_bit_size;
...
template <typename FPT1, typename FPT2> struct comp_supertype {
  ...
  // value of integral type is promoted to the floating. The same for float and
  // double But we don't want to compare two values of integral types using this
  // tool.
  typedef typename numeric::conversion_traits<FPT1, FPT2>::supertype type;
  BOOST_STATIC_ASSERT_MSG(!is_integral<type>::value,
                          "Only floating-point types can be compared!");
};
...
struct BOOST_TEST_DECL check_is_close_t {
  ...
  assertion_result operator()(FPT1 left, FPT2 right,
                              ToleranceType tolerance) const {
    fpc::close_at_tolerance<
        typename tt_detail::comp_supertype<FPT1, FPT2>::type>
        pred(tolerance, fpc::FPC_STRONG);
...
template <typename FPT1, typename FPT2, typename ToleranceType>
inline assertion_result check_is_close(FPT1 left, FPT2 right,
...
struct BOOST_TEST_DECL check_is_small_t {
  ...
  template <typename FPT> bool operator()(FPT fpv, FPT tolerance) const {
...
template <typename FPT> inline bool check_is_small(FPT fpv, FPT tolerance) {
  return fpc::is_small(fpv, tolerance);
}
