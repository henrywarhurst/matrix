...
template <typename OP> struct fpctraits {
...
template <typename Lhs, typename Rhs> struct fpctraits<op::LT<Lhs, Rhs>> {
  static const bool equality_logical_disjunction = false;
};

template <typename Lhs, typename Rhs> struct fpctraits<op::GT<Lhs, Rhs>> {
  static const bool equality_logical_disjunction = false;
};
...
// and then need EQ. For example a <= b (tolerance t) IFF a <= b OR a == b
// (tolerance t)

template <typename FPT, typename Lhs, typename Rhs, typename OP>
inline assertion_result compare_fpv(Lhs const &lhs, Rhs const &rhs,
                                    OP *cmp_operator) {
...
inline assertion_result compare_fpv_near_zero(FPT const &fpv,
                                              op::EQ<Lhs, Rhs> *) {
  fpc::small_with_tolerance<FPT> P(fpc_tolerance<FPT>());

  assertion_result ar(P(fpv));
  if (!ar)
    ar.message() << "Absolute value exceeds tolerance [|" << fpv << "| > "
                 << fpc_tolerance<FPT>() << ']';
...
inline assertion_result compare_fpv_near_zero(FPT const &fpv,
  ...
  assertion_result ar(!P(fpv));
  if (!ar)
    ar.message() << "Absolute value is within tolerance [|" << fpv << "| < "
                 << fpc_tolerance<FPT>() << ']';
  return ar;
}
...
#define DEFINE_FPV_COMPARISON(oper, name, rev)                                 \
  ...
  struct name<                                                                 \
      Lhs, Rhs,                                                                \
      typename boost::enable_if_c<(fpc::tolerance_based<Lhs>::value &&         \
  ...
  public:                                                                      \
    ...
    typedef assertion_result result_type;                                      \
                                                                               \
    static bool eval_direct(Lhs const &lhs, Rhs const &rhs) {                  \
      return lhs oper rhs;                                                     \
    }                                                                          \
                                                                               \
