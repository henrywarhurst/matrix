...
template <typename T> struct has_setup {
private:
...
template <typename T> struct has_teardown {
private:
...
template <bool has_setup = false> struct call_setup {
  template <class U> void operator()(U &) {}
};

template <> struct call_setup<true> {
  template <class U> void operator()(U &u) { u.setup(); }
};

template <bool has_teardown = false> struct call_teardown {
  template <class U> void operator()(U &) {}
};

template <> struct call_teardown<true> {
  template <class U> void operator()(U &u) { u.teardown(); }
};
} // namespace impl_fixture

//! Calls the fixture "setup" if detected by the compiler, otherwise does
//! nothing.
template <class U> void setup_conditional(U &u) {
...
//! nothing.
template <class U> void teardown_conditional(U &u) {
...
public:
  ...
  function_based_fixture(boost::function<void()> const &setup_,
      ...
      : m_setup(setup_), m_teardown(teardown_) {}
