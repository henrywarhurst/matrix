...
//!
//! This decorator may be used in places where the currently accumulated
//! decorators in the collector should be applied to lower levels of the
//! hierarchy rather than the current one. This is for instance for dataset test
//! cases, where the macro does not let the user specify decorators for the
//! underlying generated tests (but rather on the main generator function),
//! applying the stack_decorator at the parent level lets us consume the
//! decorator at the underlying test cases level.
...
template <bool condition> class enable_if : public enable_if_impl {
private:
  // decorator::base interface
...
template <typename F> inline fixture_t fixture() {
  return fixture_t(
...
template <typename F, typename Arg> inline fixture_t fixture(Arg const &arg) {
  return fixture_t(
