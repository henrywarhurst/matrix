...
public:
  ...
  //!
  //! This will override the log sink/stream of all loggers, whether enabled or
  //! not.
  ...
  //!
  //! Adding a logger means that the specified logger is enabled. The log level
  //! is managed by the formatter itself and specifies what events are forwarded
  //! to the underlying formatter.
  //! @par Since Boost 1.62
  ...
  //!
  //! @returns the logger/formatter instance, or @c (unit_test_log_formatter*)0
  ...
  //! currently active logger. In case several loggers are active, the order of
  //! priority is CUSTOM, HRF, XML, and JUNIT. If (unit_test_log_formatter*)0 is
  //! given as argument, the custom logger (if any) is removed.
  //!
  ...
  //! If (unit_test_log_formatter*)0 is given as argument, the custom logger (if
  //! any) is removed and no other action is performed.
  //!
  //! @note The ownership of the pointer is transfered to the Boost.Test
  //! framework.
  //! @par Since Boost 1.62
...
#define BOOST_TEST_FRAMEWORK_MESSAGE(M)                                        \
  (::boost::unit_test::unit_test_log << ::boost::unit_test::log::begin(        \
       "boost.test framework", 0))(::boost::unit_test::log_messages)           \
