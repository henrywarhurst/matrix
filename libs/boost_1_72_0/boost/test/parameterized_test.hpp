...
#define BOOST_PARAM_TEST_CASE(function, begin, end)                            \
  boost::unit_test::make_test_case(function, BOOST_TEST_STRINGIZE(function),   \
                                   __FILE__, __LINE__, (begin), (end)) /**/

#define BOOST_PARAM_CLASS_TEST_CASE(function, tc_instance, begin, end)         \
  boost::unit_test::make_test_case(function, BOOST_TEST_STRINGIZE(function),   \
                                   __FILE__, __LINE__, (tc_instance), (begin), \
                                   (end)) /**/
...
public:
  param_test_case_generator(boost::function<void(ParamType)> const &test_func,
                            ...
                            std::size_t tc_line, ParamIter par_begin,
                            ParamIter par_end)
      : m_test_func(test_func),
        m_tc_name(ut_detail::normalize_test_case_name(tc_name)),
        m_tc_file(tc_file), m_tc_line(tc_line), m_par_begin(par_begin),
        m_par_end(par_end), m_index(0) {}
  ...
  virtual test_unit *next() const {
    ...
    test_unit *res =
        new test_case(m_tc_name + "_" + utils::string_cast(m_index), m_tc_file,
                      m_tc_line, boost::bind(m_test_func, *m_par_begin));
...
struct user_param_tc_method_invoker {
  ...
  // Constructor
  user_param_tc_method_invoker(shared_ptr<UserTestCase> inst,
...
make_test_case(boost::function<void(ParamType)> const &test_func,
               const_string tc_name, const_string tc_file, std::size_t tc_line,
               ParamIter par_begin, ParamIter par_end) {
  return ut_detail::param_test_case_generator<ParamType, ParamIter>(
      test_func, tc_name, tc_file, tc_line, par_begin, par_end);
}
...
inline ut_detail::param_test_case_generator<
    BOOST_DEDUCED_TYPENAME remove_const<
        BOOST_DEDUCED_TYPENAME remove_reference<ParamType>::type>::type,
    ParamIter>
make_test_case(void (*test_func)(ParamType), const_string tc_name,
               const_string tc_file, std::size_t tc_line, ParamIter par_begin,
               ParamIter par_end) {
...
make_test_case(void (UserTestCase::*test_method)(ParamType),
  ...
  typedef BOOST_DEDUCED_TYPENAME remove_const<
      ...
      param_value_type;
  return ut_detail::param_test_case_generator<param_value_type, ParamIter>(
      ut_detail::user_param_tc_method_invoker<UserTestCase, ParamType>(
          user_test_case, test_method),
      tc_name, tc_file, tc_line, par_begin, par_end);
}
