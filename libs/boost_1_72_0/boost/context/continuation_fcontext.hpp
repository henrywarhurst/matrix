...
template <typename Rec> transfer_t context_exit(transfer_t t) noexcept {
...
template <typename Ctx, typename Fn> transfer_t context_ontop(transfer_t t) {
...
template <typename Ctx, typename StackAlloc, typename Fn> class record {
private:
  stack_context sctx_;
...
public:
  record(stack_context sctx, StackAlloc &&salloc, Fn &&fn) noexcept
      : sctx_(sctx), salloc_(std::forward<StackAlloc>(salloc)),
...
fcontext_t create_context1(StackAlloc &&salloc, Fn &&fn) {
  ...
  void *storage =
      reinterpret_cast<void *>((reinterpret_cast<uintptr_t>(sctx.sp) -
                                static_cast<uintptr_t>(sizeof(Record))) &
                               ~static_cast<uintptr_t>(0xff));
  // placment new for control structure on context stack
  Record *record = new (storage)
      Record{sctx, std::forward<StackAlloc>(salloc), std::forward<Fn>(fn)};
  ...
  const fcontext_t fctx =
      make_fcontext(stack_top, size, &context_entry<Record>);
  ...
  template <typename Fn> continuation resume_with(Fn &&fn) && {
    ...
    return {
      detail::ontop_fcontext(
          ...
          std::exchange(fctx_, nullptr),
#endif
          &p, detail::context_ontop<continuation, Fn>)
          .fctx
    };
  }
...
continuation callcc(std::allocator_arg_t, StackAlloc &&salloc, Fn &&fn) {
  ...
  return continuation{
      detail::create_context1<Record>(std::forward<StackAlloc>(salloc),
                                      std::forward<Fn>(fn))}
      .resume();
}
...
continuation callcc(std::allocator_arg_t, preallocated palloc,
  ...
  using Record = detail::record<continuation, StackAlloc, Fn>;
  return continuation{
      detail::create_context2<Record>(palloc, std::forward<StackAlloc>(salloc),
                                      std::forward<Fn>(fn))}
      .resume();
}
