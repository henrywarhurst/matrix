...
inline transfer_t fiber_unwind(transfer_t t) {
  throw forced_unwind(t.fctx);
  return {nullptr, nullptr};
}

template <typename Rec> transfer_t fiber_exit(transfer_t t) noexcept {
...
template <typename Rec> void fiber_entry(transfer_t t) noexcept {
...
template <typename Ctx, typename Fn> transfer_t fiber_ontop(transfer_t t) {
...
template <typename Ctx, typename StackAlloc, typename Fn> class fiber_record {
private:
  stack_context sctx_;
...
public:
  fiber_record(stack_context sctx, StackAlloc &&salloc, Fn &&fn) noexcept
...
public:
  ...
  template <typename Fn> fiber(std::allocator_arg_t, segmented_stack, Fn &&);
  ...
  template <typename Fn> fiber resume_with(Fn &&fn) && {
    ...
    return {
      detail::ontop_fcontext(
          ...
          std::exchange(fctx_, nullptr),
#endif
          &p, detail::fiber_ontop<fiber, decltype(p)>)
          .fctx
    };
  }
  ...
  void swap(fiber &other) noexcept { std::swap(fctx_, other.fctx_); }
};
