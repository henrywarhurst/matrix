...
#if defined(BOOST_CONTEXT_USE_MAP_STACK)
namespace detail {
template <typename traitsT> struct map_stack_allocator {
  ...
  static char *malloc(const size_type bytes) {
    ...
    if (mmap(block, bytes, PROT_READ | PROT_WRITE,
             MAP_PRIVATE | MAP_ANON | MAP_FIXED | MAP_STACK, -1,
             0) == MAP_FAILED) {
      std::free(block);
      return 0;
    }
...
template <typename traitsT> class basic_pooled_fixedsize_stack {
private:
  class storage {
  private:
  ...
  public:
    ...
    friend void intrusive_ptr_add_ref(storage *s) noexcept { ++s->use_count_; }
...
public:
  ...
  stack_context allocate() { return storage_->allocate(); }
