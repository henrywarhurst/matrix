...
template <class T>
inline void
do_fill_uninitialized_memory(T & /*data*/,
                             std::size_t /*size_in_bytes*/) BOOST_NOEXCEPT {
  // Do nothing
}
...
public:
  ...
  */
  void unregister_iterator(const debug_iterator_base *it) const {
    ...
    for (const debug_iterator_base *p = m_iterators; p != it;
         previous = p, p = p->next()) {
    }
    remove(it, previous);
  }
  ...
  template <class Iterator> void invalidate_iterators(const Iterator &it) {
    const debug_iterator_base *previous = 0;
  ...
  //! Invalidate all iterators except an iterator poining to the same element as
  //! the iterator passed as a parameter.
...
inline debug_iterator_base::debug_iterator_base(
    const debug_iterator_registry *registry)
...
inline bool
debug_iterator_base::is_valid(const debug_iterator_registry *registry) const {
  return m_registry == registry;
}

inline void debug_iterator_base::invalidate() const { m_registry = 0; }

inline const debug_iterator_base *debug_iterator_base::next() const {
  return m_next;
}
