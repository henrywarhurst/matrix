...
#define BOOST_TYPE_ERASURE_MEMBER_ARG(z, n, data)                              \
  typename ::boost::type_erasure::as_param<Base, BOOST_PP_CAT(A, n)>::type     \
      BOOST_PP_CAT(a, n)
...
/** INTERNAL ONLY */
#define BOOST_TYPE_ERASURE_MEMBER_TPL_ARG_LIST(N, X)                           \
...
#define BOOST_TYPE_ERASURE_FORWARD_REBIND1(z, n, data)                         \
  ::std::forward<typename ::boost::type_erasure::as_param<                     \
      Base, BOOST_PP_CAT(A, n)>::type>(BOOST_PP_CAT(a, n))
/** INTERNAL ONLY*/
...
#define BOOST_TYPE_ERASURE_MEMBER_FORWARD_PARAMS(N, X, x)                      \
  ::std::forward<X>(x)...
#define BOOST_TYPE_ERASURE_MEMBER_FORWARD_REBIND(N)                            \
  , ::std::forward<typename ::boost::type_erasure::as_param<Base, A>::type>(   \
        a)...
#else
...
#define BOOST_TYPE_ERASURE_MEMBER_ENUM_ARGS(N)                                 \
  typename ::boost::type_erasure::as_param<Base, A>::type... a
...
#define BOOST_TYPE_ERASURE_MEMBER_II(qual_name, concept_name, member, N)       \
  ...
  struct concept_name;                                                         \
  template <class R BOOST_TYPE_ERASURE_MEMBER_TPL_ARG_LIST(N, class A),        \
            class T>                                                           \
  struct concept_name<R(BOOST_TYPE_ERASURE_MEMBER_ENUM_PARAMS(N, A)), T> {     \
    static R apply(                                                            \
        T &t BOOST_TYPE_ERASURE_MEMBER_ENUM_TRAILING_BINARY_PARAMS(N, A, a)) { \
  ...
  struct concept_interface<                                                    \
      BOOST_TYPE_ERASURE_MEMBER_QUALIFIED_ID(qual_name, N), Base,              \
    ...
    member(BOOST_TYPE_ERASURE_MEMBER_ENUM_ARGS(N)) {                           \
  ...
  struct concept_interface<                                                    \
    ...
    typedef void BOOST_PP_CAT(_boost_type_erasure_has_member, member);         \
    typename rebind_any<Base, R>::type                                         \
    member(BOOST_TYPE_ERASURE_MEMBER_ENUM_ARGS(N)) const {                     \
      return ::boost::type_erasure::call(                                      \
          BOOST_TYPE_ERASURE_MEMBER_QUALIFIED_ID(qual_name, N)(),              \
          *this BOOST_TYPE_ERASURE_MEMBER_FORWARD_REBIND(N));                  \
    }                                                                          \
  ...
  struct concept_interface<                                                    \
      ...
      typename ::boost::enable_if<                                             \
          ::boost::type_erasure::detail::should_be_non_const<T, Base>,         \
          typename ::boost::remove_const<T>::type>::type,                      \
      typename Base::BOOST_PP_CAT(_boost_type_erasure_has_member, member)>     \
      : Base {                                                                 \
    using Base::member;                                                        \
    typename rebind_any<Base, R>::type                                         \
    member(BOOST_TYPE_ERASURE_MEMBER_ENUM_ARGS(N)) {                           \
