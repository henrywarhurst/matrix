...
#define BOOST_TYPE_ERASURE_UNARY_INPLACE_OPERATOR(name, op)                    \
  template <class T = _self> struct name {                                     \
  ...
  struct concept_interface<                                                    \
      name<T>, Base, T,                                                        \
      typename ::boost::enable_if<detail::should_be_non_const<T, Base>>::type> \
      : Base {                                                                 \
  ...
  struct concept_interface<                                                    \
      name<T>, Base, T,                                                        \
      typename ::boost::enable_if<detail::should_be_const<T, Base>>::type>     \
      : Base {                                                                 \
    typedef typename ::boost::type_erasure::derived<Base>::type _derived;      \
...
#define BOOST_TYPE_ERASURE_UNARY_OPERATOR(name, op)                            \
  template <class T = _self, class R = T> struct name {                        \
...
#define BOOST_TYPE_ERASURE_BINARY_OPERATOR(name, op)                           \
  template <class T = _self, class U = T, class R = T> struct name {           \
  ...
  struct concept_interface<                                                    \
      ...
      typename ::boost::disable_if<                                            \
          ::boost::type_erasure::is_placeholder<T>>::type> : Base {            \
    friend typename rebind_any<Base, R>::type                                  \
    operator op(const T &lhs, const typename derived<Base>::type &rhs) {       \
      return ::boost::type_erasure::call(name<T, U, R>(), lhs, rhs);           \
    }                                                                          \
  };
...
#define BOOST_TYPE_ERASURE_ASSIGNMENT_OPERATOR(name, op)                       \
  template <class T = _self, class U = T> struct name {                        \
...
template <class T = _self, class U = T> struct equality_comparable {
...
struct concept_interface<equality_comparable<T, U>, Base, T> : Base {
  friend bool operator==(const typename derived<Base>::type &lhs,
                         typename as_param<Base, const U &>::type rhs) {
    if (::boost::type_erasure::check_match(equality_comparable<T, U>(), lhs,
                                           rhs)) {
      return ::boost::type_erasure::unchecked_call(equality_comparable<T, U>(),
                                                   lhs, rhs);
    } else {
      return false;
    }
...
template <class T, class U, class Base>
struct concept_interface<equality_comparable<T, U>, Base, U,
                         typename ::boost::disable_if<
...
template <class T = _self, class U = T> struct less_than_comparable {
  static bool apply(const T &lhs, const U &rhs) { return lhs < rhs; }
};
...
bool less_impl(const F &f, const T &lhs, const U &rhs, ::boost::mpl::true_) {
  if (::boost::type_erasure::check_match(f, lhs, rhs)) {
    ...
    return ::boost::type_erasure::typeid_of(
               ...
               .before(::boost::type_erasure::typeid_of(
...
struct concept_interface<less_than_comparable<T, T>, Base, T> : Base {
  friend bool operator<(const typename derived<Base>::type &lhs,
    ...
    return ::boost::type_erasure::detail::less_impl(
        less_than_comparable<T, T>(), lhs, rhs,
...
template <class T, class U, class Base>
struct concept_interface<less_than_comparable<T, U>, Base, U,
                         typename ::boost::disable_if<
                             ::boost::type_erasure::is_placeholder<T>>::type>
    : Base {
...
struct concept_interface<
    subscriptable<R, T, N>, Base, typename ::boost::remove_const<T>::type,
    typename ::boost::enable_if<
        ::boost::type_erasure::detail::should_be_non_const<T, Base>>::type>
    : Base {
...
struct concept_interface<
    subscriptable<R, T, N>, Base, typename ::boost::remove_const<T>::type,
    typename ::boost::enable_if<
        ::boost::type_erasure::detail::should_be_const<T, Base>>::type> : Base {
  typename ::boost::type_erasure::rebind_any<Base, R>::type operator[](
      typename ::boost::type_erasure::as_param<Base, const N &>::type index)
      const {
    return ::boost::type_erasure::call(subscriptable<R, const T, N>(), *this,
                                       index);
  }
};
 ...
 */
template <class Os = std::ostream, class T = _self> struct ostreamable {
...
template <class Base, class Os, class T>
struct concept_interface<ostreamable<Os, T>, Base, T,
                         typename ::boost::disable_if<
                             ::boost::type_erasure::is_placeholder<Os>>::type>
    : Base {
  friend Os &
  operator<<(Os &lhs,
 ...
 */
template <class Is = std::istream, class T = _self> struct istreamable {
...
template <class Base, class Is, class T>
struct concept_interface<istreamable<Is, T>, Base, T,
                         typename ::boost::disable_if<
                             ::boost::type_erasure::is_placeholder<Is>>::type>
    : Base {
  friend Is &
  operator>>(Is &lhs,
