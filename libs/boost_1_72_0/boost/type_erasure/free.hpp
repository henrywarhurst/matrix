...
template <BOOST_PP_ENUM_PARAMS_WITH_A_DEFAULT(BOOST_TYPE_ERASURE_MAX_ARITY,
                                              class T, void)>
struct first_placeholder {
  typedef typename ::boost::mpl::eval_if<
      is_placeholder<T0>, ::boost::mpl::identity<T0>,
      first_placeholder<BOOST_PP_ENUM_SHIFTED_PARAMS(
          BOOST_TYPE_ERASURE_MAX_ARITY, T)>>::type type;
};
...
struct first_placeholder_index
    : ::boost::mpl::eval_if<is_placeholder<T0>, ::boost::mpl::int_<0>,
                            ::boost::mpl::next<first_placeholder_index<
                                BOOST_PP_ENUM_SHIFTED_PARAMS(
                                    BOOST_TYPE_ERASURE_MAX_ARITY, T)>>>::type {
};
...
#define BOOST_TYPE_ERASURE_FREE_UNQUALIFIED_PARAM_TYPE(z, n, data)             \
  typename ::boost::remove_cv<                                                 \
      typename ::boost::remove_reference<BOOST_PP_CAT(T, n)>::type>::type

/** INTERNAL ONLY */
#define BOOST_TYPE_ERASURE_FREE_PARAM_TYPE(z, n, data)                         \
  typename ::boost::mpl::eval_if_c<                                            \
      (_boost_type_erasure_free_p_idx::value == n),                            \
      ::boost::type_erasure::detail::maybe_const_this_param<                   \
          BOOST_PP_CAT(T, n), Base>,                                           \
...
/** INTERNAL ONLY */
#define BOOST_TYPE_ERASURE_FREE_FORWARD_I(z, n, data)                          \
  ::std::forward<BOOST_PP_CAT(T, n)>(BOOST_PP_CAT(t, n))
/** INTERNAL ONLY */
#define BOOST_TYPE_ERASURE_FREE_FORWARD(n)                                     \
  BOOST_PP_ENUM(n, BOOST_TYPE_ERASURE_FREE_FORWARD_I, ~)
/** INTERNAL ONLY */
...
#define BOOST_TYPE_ERASURE_FREE_II(qual_name, concept_name, function_name, N)  \
  BOOST_TYPE_ERASURE_OPEN_NAMESPACE(qual_name)                                 \
                                                                               \
  template <class Sig> struct concept_name;                                    \
                                                                               \
  ...
  struct concept_interface<                                                    \
      BOOST_TYPE_ERASURE_FREE_QUALIFIED_ID(qual_name, N), Base,                \
      typename ::boost::type_erasure::detail::first_placeholder<BOOST_PP_ENUM( \
          N, BOOST_TYPE_ERASURE_FREE_UNQUALIFIED_PARAM_TYPE, ~)>::type>        \
      : Base {                                                                 \
    typedef typename ::boost::type_erasure::detail::first_placeholder_index<   \
        BOOST_PP_ENUM(N, BOOST_TYPE_ERASURE_FREE_UNQUALIFIED_PARAM_TYPE,       \
...
template <int... N> struct index_list {};

namespace detail {

template <class... T> struct first_placeholder;

template <class T0, class... T> struct first_placeholder<T0, T...> {
  typedef typename ::boost::mpl::eval_if<is_placeholder<T0>,
                                         ::boost::mpl::identity<T0>,
                                         first_placeholder<T...>>::type type;
};
...
struct first_placeholder_index<T0, T...>
    : ::boost::mpl::eval_if<
          is_placeholder<T0>, ::boost::mpl::int_<0>,
          ::boost::mpl::next<first_placeholder_index<T...>>>::type {};

template <class Sig> struct transform_free_signature;

template <class T, int N> struct push_back_index;

template <int... N, int X> struct push_back_index<index_list<N...>, X> {
  typedef index_list<N..., X> type;
};

template <int N> struct make_index_list {
  typedef typename push_back_index<typename make_index_list<N - 1>::type,
                                   N - 1>::type type;
};

template <> struct make_index_list<0> { typedef index_list<> type; };

#if !defined(BOOST_NO_CXX11_TEMPLATE_ALIASES) &&                               \
    !defined(BOOST_NO_CXX11_DECLTYPE)

template <int N>
using make_index_list_t =
    typename ::boost::type_erasure::detail::make_index_list<N>::type;
...
using first_placeholder_index_t =
    typename ::boost::type_erasure::detail::first_placeholder_index_<
        T...>::type;
...
using free_param_t = typename ::boost::mpl::eval_if_c<
    (::boost::type_erasure::detail::first_placeholder_index_t<T...>::value ==
     I),
    ::boost::type_erasure::detail::maybe_const_this_param<Tn, Base>,
    ::boost::type_erasure::as_param<Base, Tn>>::type;

template <class Sig, class ID> struct free_interface_chooser {
...
struct free_choose_interface {
  ...
  using apply =
      typename free_interface_chooser<Sig, ID>::template apply<Base, C, F>;
};
...
#define BOOST_TYPE_ERASURE_FREE_I(concept_name, function_name)                 \
  ...
  struct concept_name##_free_interface<                                        \
      R(T...), Base, ::boost::type_erasure::index_list<I...>> : Base {         \
    friend ::boost::type_erasure::rebind_any_t<Base, R> function_name(         \
        ::boost::type_erasure::detail::free_param_t<Base, T, I, T...>... t) {  \
      return ::boost::type_erasure::call(                                      \
          concept_name<R(T...)>(),                                             \
          std::forward<                                                        \
              ::boost::type_erasure::detail::free_param_t<Base, T, I, T...>>(  \
              t)...);                                                          \
    }                                                                          \
  };                                                                           \
                                                                               \
  template <class Sig> struct concept_name##free;                              \
                                                                               \
  template <class R, class... T> struct concept_name##free<R(T...)> {          \
    static R apply(T... t) { return function_name(std::forward<T>(t)...); }    \
  };                                                                           \
                                                                               \
  template <class... T> struct concept_name##free<void(T...)> {                \
  ...
  struct concept_name : boost_type_erasure_impl::concept_name##free<Sig> {};   \
                                                                               \
  template <class Sig>                                                         \
  ::boost::type_erasure::detail::free_choose_interface<                        \
      Sig, concept_name,                                                       \
...
#define BOOST_TYPE_ERASURE_FREE_NS(concept_name, name)                         \
  ...
  BOOST_TYPE_ERASURE_FREE_NS_I(                                                \
      BOOST_PP_SEQ_ELEM(BOOST_PP_DEC(BOOST_PP_SEQ_SIZE(concept_name)),         \
                        concept_name),                                         \
      name)                                                                    \
...
#define BOOST_TYPE_ERASURE_FREE_NAMED(concept_name, name, ...)                 \
  BOOST_PP_IF(BOOST_PP_IS_BEGIN_PARENS(concept_name),                          \
              BOOST_TYPE_ERASURE_FREE_NS, BOOST_TYPE_ERASURE_FREE_I)           \
  (concept_name, name)

#define BOOST_TYPE_ERASURE_FREE_CAT(x, y) x y

#define BOOST_TYPE_ERASURE_FREE(name, ...)                                     \
  BOOST_TYPE_ERASURE_FREE_CAT(BOOST_PP_IF(BOOST_VMD_IS_EMPTY(__VA_ARGS__),     \
...
#define BOOST_TYPE_ERASURE_FREE_II(qual_name, concept_name, function_name)     \
  ...
  template <class Sig> struct concept_name;                                    \
                                                                               \
  template <class R, class... T> struct concept_name<R(T...)> {                \
    static R apply(T... t) { return function_name(std::forward<T>(t)...); }    \
  };                                                                           \
                                                                               \
  template <class... T> struct concept_name<void(T...)> {                      \
    static void apply(T... t) { function_name(std::forward<T>(t)...); }        \
  };                                                                           \
                                                                               \
  BOOST_TYPE_ERASURE_CLOSE_NAMESPACE(qual_name)                                \
  ...
  namespace type_erasure {                                                     \
                                                                               \
  template <class Sig, class Base, class Idx> struct inject##concept_name;     \
  template <class R, class... T, class Base, int... I>                         \
  ...
  struct concept_interface<BOOST_TYPE_ERASURE_QUALIFIED_NAME(qual_name) <      \
                           R(T...)>,                                           \
      Base,                                                                    \
      typename ::boost::type_erasure::detail::first_placeholder<               \
          typename ::boost::remove_cv<                                         \
              typename ::boost::remove_reference<T>::type>::type...>::type >   \
      : inject##concept_name<R(T...), Base,                                    \
                             typename ::boost::type_erasure::detail::          \
                                 make_index_list<sizeof...(T)>::type> {};      \
  }                                                                            \
  }
