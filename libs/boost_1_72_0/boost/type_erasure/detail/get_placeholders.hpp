...
template <class T> struct get_placeholders_in_argument_impl {
  template <class Out>
  using apply = ::boost::type_erasure::detail::eval_if<
      ::boost::type_erasure::is_placeholder<T>::value,
      ::boost::mp11::mp_set_push_back, ::boost::type_erasure::detail::first,
      Out, T>;
};

template <class T> struct get_placeholders_in_argument_impl<T &> {
  template <class Out>
  using apply =
      typename ::boost::type_erasure::detail::get_placeholders_in_argument_impl<
          T>::template apply<Out>;
};
...
template <class T> struct get_placeholders_in_argument_impl<const T> {
  template <class Out>
  using apply =
      typename ::boost::type_erasure::detail::get_placeholders_in_argument_impl<
          T>::template apply<Out>;
...
using get_placeholders_f =
    typename get_placeholders_in_argument_impl<T>::template apply<Out>;

template <class Out, class... T>
using get_placeholders_impl =
    ::boost::mp11::mp_fold<::boost::mp11::mp_list<T...>, Out,
...
struct get_placeholders_in_argument_impl<R(T...)> {
  template <class Out>
  using apply =
      ::boost::type_erasure::detail::get_placeholders_impl<Out, R, T...>;
};
...
struct get_placeholders_in_argument_impl<R (C::*)(T...)> {
  template <class Out>
  using apply =
      ::boost::type_erasure::detail::get_placeholders_impl<Out, R, C, T...>;
};
...
using get_placeholders_t =
    typename ::boost::type_erasure::detail::get_placeholders<T, Out>::type;

#else

template <class T, class Out> struct get_placeholders_in_argument {
  typedef
      typename ::boost::mpl::eval_if<::boost::type_erasure::is_placeholder<T>,
...
template <class T, class Out> struct get_placeholders_in_argument<T &, Out> {
...
template <class Out, class... T> struct get_placeholders_impl;

template <class Out, class T0, class... T>
struct get_placeholders_impl<Out, T0, T...> {
  typedef typename ::boost::type_erasure::detail::get_placeholders_in_argument<
      T0, typename get_placeholders_impl<Out, T...>::type>::type type;
};

template <class Out> struct get_placeholders_impl<Out> { typedef Out type; };
...
#define BOOST_TYPE_ERASURE_GET_PLACEHOLDER(z, n, data)                         \
  typedef                                                                      \
      typename ::boost::type_erasure::detail::get_placeholders_in_argument<    \
