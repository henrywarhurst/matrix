...
using choose_member_interface = typename ::boost::mpl::if_c<
    ...
    typename ::boost::mpl::if_c<
        ...
        interface<Sig, Concept, Base, const ID>, Base>::type,
    interface<Sig, Concept, Base, ID>>::type;

struct dummy {};

template <class Sig> struct choose_member_impl;
...
template <class Sig, class P, template <class> class M,
          template <class, class> class Self>
using choose_member_impl_t =
    typename ::boost::type_erasure::detail::choose_member_impl<Sig(
        ::boost::type_erasure::detail::dummy::*)>::template apply<P, M, Self>;

template <class Sig, class T, class ID> struct member_interface_chooser {
  template <class Base, template <class, class> class C,
...
struct member_interface_chooser<R(A...), T, T> {
  ...
  using apply = ::boost::type_erasure::detail::choose_member_interface<
      ::boost::type_erasure::placeholder_of_t<Base>, M, R(A...), C<R(A...), T>,
      Base, T>;
};
...
template <class Sig, class T, template <class, class> class C,
          template <class...> class M>
struct member_choose_interface {
  template <class Concept, class Base, class ID>
  using apply =
      typename ::boost::type_erasure::detail::member_interface_chooser<
          Sig, T, ID>::template apply<Base, C, M>;
};
...
#define BOOST_TYPE_ERASURE_MEMBER_I(concept_name, function_name)               \
  ...
  using concept_name##self = concept_name<Sig, T>;                             \
                                                                               \
  template <class Sig, class Concept, class Base, class ID,                    \
            class Enable = void>                                               \
  struct concept_name##_member_interface;                                      \
                                                                               \
  template <class R, class... A, class Concept, class Base, class ID, class V> \
  struct concept_name##_member_interface<R(A...), Concept, Base, ID, V>        \
      : Base {                                                                 \
    ...
    function_name(::boost::type_erasure::as_param_t<Base, A>... a) {           \
      return ::boost::type_erasure::call(Concept(), *this,                     \
                                         std::forward<A>(a)...);               \
    }                                                                          \
  };                                                                           \
                                                                               \
    ...
    function_name(::boost::type_erasure::as_param_t<Base, A>... a) {           \
      return ::boost::type_erasure::call(Concept(), *this,                     \
                                         std::forward<A>(a)...);               \
                                                                               ...
                                                                               \
  template <class R, class... A, class Concept, class Base, class ID, class V> \
  struct concept_name##_member_interface<R(A...), Concept, Base, const ID, V>  \
      : Base {                                                                 \
    typedef void _boost_type_erasure_has_member##function_name;                \
    ::boost::type_erasure::rebind_any_t<Base, R>                               \
    ...
    function_name(::boost::type_erasure::as_param_t<Base, A>... a) const {     \
      return ::boost::type_erasure::call(Concept(), *this,                     \
                                         std::forward<A>(a)...);               \
                                                                               ...
                                                                               \
  template <class Sig> struct concept_name##member;                            \
                                                                               \
  ...
  struct concept_name                                                          \
      : ::boost::type_erasure::detail::choose_member_impl_t<                   \
            Sig, T, boost_type_erasure_impl::concept_name##member,             \
...
#define BOOST_TYPE_ERASURE_MEMBER_NAMED(concept_name, name, ...)               \
  BOOST_PP_IF(BOOST_PP_IS_BEGIN_PARENS(concept_name),                          \
              BOOST_TYPE_ERASURE_MEMBER_NS, BOOST_TYPE_ERASURE_MEMBER_I)       \
  (concept_name, name)

#define BOOST_TYPE_ERASURE_MEMBER_CAT(x, y) x y

#define BOOST_TYPE_ERASURE_MEMBER(name, ...)                                   \
  BOOST_TYPE_ERASURE_MEMBER_CAT(BOOST_PP_IF(BOOST_VMD_IS_EMPTY(__VA_ARGS__),   \
