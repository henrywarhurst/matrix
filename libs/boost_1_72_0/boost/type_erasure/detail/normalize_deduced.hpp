...
template <class M, class T> struct normalize_deduced;
...
#define BOOST_TYPE_ERASURE_NORMALIZE_PLACEHOLDER(z, n, data)                   \
  typename ::boost::type_erasure::detail::normalize_placeholder<               \
      M, BOOST_PP_CAT(U, n)>::type

template <class M, template <BOOST_PP_ENUM_PARAMS(N, class T)> class T,
          BOOST_PP_ENUM_PARAMS(N, class U)>
struct normalize_deduced<M, T<BOOST_PP_ENUM_PARAMS(N, U)>> {
  typedef typename ::boost::type_erasure::deduced<T<BOOST_PP_ENUM(
      N, BOOST_TYPE_ERASURE_NORMALIZE_PLACEHOLDER, ~)>>::type type;
};
