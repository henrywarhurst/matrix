...
template <class Sig> struct null_construct;

template <class C> struct get_null_vtable_entry;

template <class C, class Sig> struct vtable_adapter;
...
template <class R, class... T> struct constructible<R(T...)> {
...
struct concept_interface<::boost::type_erasure::constructible<Tag(T...)>, Base,
                         Tag> : Base {
...
template <class... T> struct null_construct<void(T...)> {
...
struct get_null_vtable_entry<
    vtable_adapter<constructible<T(const T &)>, R(U...)>> {
...
#define BOOST_TYPE_ERASURE_FORWARD_I(z, n, data)                               \
  ::std::forward<BOOST_PP_CAT(T, n)>(BOOST_PP_CAT(arg, n))
#define BOOST_TYPE_ERASURE_FORWARD(n)                                          \
  BOOST_PP_ENUM(n, BOOST_TYPE_ERASURE_FORWARD_I, ~)
#else
...
struct concept_interface<
    ::boost::type_erasure::constructible<Tag(BOOST_PP_ENUM_PARAMS(N, T))>, Base,
    Tag> : Base {
  using Base::_boost_type_erasure_deduce_constructor;
...
struct null_construct<void(BOOST_PP_ENUM_PARAMS(N, T))> {
  static ::boost::type_erasure::detail::storage value(BOOST_PP_ENUM_PARAMS(N,
                                                                           T)) {
    ::boost::type_erasure::detail::storage result;
    result.data = 0;
    return result;
...
struct get_null_vtable_entry<vtable_adapter<constructible<T(const T &)>,
