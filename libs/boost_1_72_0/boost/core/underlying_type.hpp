...
template <typename EnumType, typename Void = void> struct underlying_type_impl;

#if defined(BOOST_NO_CXX11_SCOPED_ENUMS)

// Support for boost/core/scoped_enum.hpp
template <typename EnumType>
struct underlying_type_impl<EnumType,
                            typename EnumType::is_boost_scoped_enum_tag> {
  /**
   * The member typedef type names the underlying type of EnumType. It is
   * EnumType::underlying_type when the EnumType is an emulated scoped enum,
   */
...
template <typename EnumType, typename Void> struct underlying_type_impl {
...
#if !defined(BOOST_NO_CXX11_SCOPED_ENUMS) &&                                   \
    !defined(BOOST_DETAIL_HAS_STD_UNDERLYING_TYPE)
