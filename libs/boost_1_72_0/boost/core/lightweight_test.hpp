...
public:
  ...
  ~test_result() {
    if (!report_) {
      BOOST_LIGHTWEIGHT_TEST_OSTREAM << "main() should return report_errors()"
                                     << std::endl;
      std::abort();
    }
  }
...
inline void test_failed_impl(char const *expr, char const *file, int line,
                             char const *function) {
  BOOST_LIGHTWEIGHT_TEST_OSTREAM << file << "(" << line << "): test '" << expr
...
inline void error_impl(char const *msg, char const *file, int line,
                       char const *function) {
  BOOST_LIGHTWEIGHT_TEST_OSTREAM << file << "(" << line << "): " << msg
                                 << " in function '" << function << "'"
                                 << std::endl;
  ++test_results().errors();
}

inline void throw_failed_impl(char const *excep, char const *file, int line,
                              char const *function) {
  BOOST_LIGHTWEIGHT_TEST_OSTREAM << file << "(" << line << "): Exception '"
                                 << excep << "' not thrown in function '"
                                 << function << "'" << std::endl;
  ++test_results().errors();
}

// In the comparisons below, it is possible that T and U are signed and unsigned
// integer types, which generates warnings in some compilers. A cleaner fix
// would require common_type trait or some meta-programming, which would
// introduce a dependency on Boost.TypeTraits. To avoid the dependency we just
...
inline void test_with_impl(BinaryPredicate pred, char const *expr1,
  ...
  if (pred(t, u)) {
    ...
    BOOST_LIGHTWEIGHT_TEST_OSTREAM
        << file << "(" << line << "): test '" << expr1 << " " << pred.op()
        << " " << expr2 << "' ('" << test_output_impl(t) << "' " << pred.op()
...
inline void test_cstr_eq_impl(char const *expr1, char const *expr2,
  ...
  if (std::strcmp(t, u) == 0) {
    ...
    BOOST_LIGHTWEIGHT_TEST_OSTREAM
        ...
        << "' ('" << t << "' == '" << u << "') failed in function '" << function
        << "'" << std::endl;
    ++test_results().errors();
  }
}
...
void test_all_eq_impl(FormattedOutputFunction &output, char const *file,
...
void test_all_with_impl(FormattedOutputFunction &output, char const *file,
                        int line, char const *function,
                        InputIterator1 first_begin, InputIterator1 first_end,
                        InputIterator2 second_begin, InputIterator2 second_end,
...
inline int report_errors() {
  ...
  if (errors == 0) {
    BOOST_LIGHTWEIGHT_TEST_OSTREAM << "No errors detected." << std::endl;
  } else {
    BOOST_LIGHTWEIGHT_TEST_OSTREAM << errors << " error"
                                   << (errors == 1 ? "" : "s") << " detected."
                                   << std::endl;
  }
...
#define BOOST_TEST(expr)                                                       \
  ((expr) ? (void)::boost::detail::test_results()                              \
...
#define BOOST_TEST_EQ(expr1, expr2)                                            \
  (::boost::detail::test_with_impl(::boost::detail::lw_test_eq(), #expr1,      \
                                   #expr2, __FILE__, __LINE__,                 \
                                   BOOST_CURRENT_FUNCTION, expr1, expr2))
#define BOOST_TEST_NE(expr1, expr2)                                            \
  (::boost::detail::test_with_impl(::boost::detail::lw_test_ne(), #expr1,      \
                                   #expr2, __FILE__, __LINE__,                 \
                                   BOOST_CURRENT_FUNCTION, expr1, expr2))

#define BOOST_TEST_LT(expr1, expr2)                                            \
  (::boost::detail::test_with_impl(::boost::detail::lw_test_lt(), #expr1,      \
                                   #expr2, __FILE__, __LINE__,                 \
                                   BOOST_CURRENT_FUNCTION, expr1, expr2))
#define BOOST_TEST_LE(expr1, expr2)                                            \
  (::boost::detail::test_with_impl(::boost::detail::lw_test_le(), #expr1,      \
                                   #expr2, __FILE__, __LINE__,                 \
                                   BOOST_CURRENT_FUNCTION, expr1, expr2))
#define BOOST_TEST_GT(expr1, expr2)                                            \
  (::boost::detail::test_with_impl(::boost::detail::lw_test_gt(), #expr1,      \
                                   #expr2, __FILE__, __LINE__,                 \
                                   BOOST_CURRENT_FUNCTION, expr1, expr2))
#define BOOST_TEST_GE(expr1, expr2)                                            \
  (::boost::detail::test_with_impl(::boost::detail::lw_test_ge(), #expr1,      \
                                   #expr2, __FILE__, __LINE__,                 \
                                   BOOST_CURRENT_FUNCTION, expr1, expr2))

#define BOOST_TEST_CSTR_EQ(expr1, expr2)                                       \
  (::boost::detail::test_cstr_eq_impl(#expr1, #expr2, __FILE__, __LINE__,      \
                                      BOOST_CURRENT_FUNCTION, expr1, expr2))
#define BOOST_TEST_CSTR_NE(expr1, expr2)                                       \
  (::boost::detail::test_cstr_ne_impl(#expr1, #expr2, __FILE__, __LINE__,      \
                                      BOOST_CURRENT_FUNCTION, expr1, expr2))

#define BOOST_TEST_ALL_EQ(begin1, end1, begin2, end2)                          \
  (::boost::detail::test_all_eq_impl(BOOST_LIGHTWEIGHT_TEST_OSTREAM, __FILE__, \
                                     __LINE__, BOOST_CURRENT_FUNCTION, begin1, \
                                     end1, begin2, end2))
#define BOOST_TEST_ALL_WITH(begin1, end1, begin2, end2, predicate)             \
  (::boost::detail::test_all_with_impl(                                        \
      BOOST_LIGHTWEIGHT_TEST_OSTREAM, __FILE__, __LINE__,                      \
      BOOST_CURRENT_FUNCTION, begin1, end1, begin2, end2, predicate))

#ifndef BOOST_NO_EXCEPTIONS
#define BOOST_TEST_THROWS(EXPR, EXCEP)                                         \
  try {                                                                        \
    EXPR;                                                                      \
    ::boost::detail::throw_failed_impl(#EXCEP, __FILE__, __LINE__,             \
                                       BOOST_CURRENT_FUNCTION);                \
  } catch (EXCEP const &) {                                                    \
