...
#if !defined(BOOST_NO_CXX11_POINTER_TRAITS)
template <class T> struct pointer_traits : std::pointer_traits<T> {
...
template <class> struct ptr_void { typedef void type; };
...
#else
template <template <class> class T, class U> struct ptr_first<T<U>> {
  typedef U type;
};
...
template <class T, class U, class = void> struct ptr_rebind {
...
template <class T> struct ptr_value { typedef T type; };

template <> struct ptr_value<void> {
  typedef struct {
  } type;
};
...
#if !defined(BOOST_NO_CXX11_TEMPLATE_ALIASES)
  template <class U> using rebind = typename detail::ptr_rebind<T, U>::type;
#endif
...
template <class T> BOOST_CONSTEXPR inline T *to_address(T *v) BOOST_NOEXCEPT {
  return v;
}
...
template <class T> inline T *ptr_address(T *v, int) BOOST_NOEXCEPT { return v; }
...
template <class T> inline auto ptr_address(const T &v, long) BOOST_NOEXCEPT {
  return boost::detail::ptr_address(v.operator->(), 0);
}

} // namespace detail

template <class T> inline auto to_address(const T &v) BOOST_NOEXCEPT {
