...
protected:
  ...
  BOOST_SERIALIZATION_DECL extended_type_info(const unsigned int type_info_key,
                                              const char *key);
...
template <class T> struct guid_defined : boost::mpl::false_ {};
...
template <class T> inline const char *guid() {
