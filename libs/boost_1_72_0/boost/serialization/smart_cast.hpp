...
template <class T> struct reference {
  ...
  struct polymorphic {
    ...
    struct linear {
      template <class U> static T cast(U &u) { return static_cast<T>(u); }
    };

    struct cross {
      template <class U> static T cast(U &u) { return dynamic_cast<T>(u); }
    };

    template <class U> static T cast(U &u) {
// if we're in debug mode
      ...
      typedef typename mpl::eval_if<
          typename mpl::and_<mpl::not_<is_base_and_derived<
                                 typename remove_reference<T>::type, U>>,
                             mpl::not_<is_base_and_derived<
                                 U, typename remove_reference<T>::type>>>,
          // borland chokes w/o full qualification here
          mpl::identity<cross>, mpl::identity<linear>>::type typex;
  ...
  template <class U> static T cast(U &u) {
    typedef typename mpl::eval_if<boost::is_polymorphic<U>,
...
template <class TPtr> struct void_pointer {
  template <class UPtr> static TPtr cast(UPtr uptr) {
...
template <class T> struct error {
  ...
  // specialization
  template <class U> static T cast(U) {
...
// smart_cast<Target &>(s)
template <class T, class U> T smart_cast(U u) {
  typedef typename mpl::eval_if<
      typename mpl::or_<boost::is_same<void *, U>, boost::is_same<void *, T>,
                        boost::is_same<const void *, U>,
                        boost::is_same<const void *, T>>,
      mpl::identity<smart_cast_impl::void_pointer<T>>,
      // else
      typename mpl::eval_if<
          boost::is_pointer<U>, mpl::identity<smart_cast_impl::pointer<T>>,
          // else
...
template <class T, class U> T smart_cast_reference(U &u) {
