...
template <class T> struct tracking_level_impl {
  template <class U> struct traits_class_tracking {
  ...
  typedef typename mpl::eval_if<
      ...
      typename mpl::eval_if<
          ...
          typename mpl::eval_if<
              // for primitives
              typename mpl::equal_to<implementation_level<T>,
...
// wrap it in your own type so its not a primitive anymore.  Then it will
// compile without problem.
#define BOOST_CLASS_TRACKING(T, E)                                             \
  namespace boost {                                                            \
  namespace serialization {                                                    \
  template <> struct tracking_level<T> {                                       \
    typedef mpl::integral_c_tag tag;                                           \
    typedef mpl::int_<E> type;                                                 \
    BOOST_STATIC_CONSTANT(int, value = tracking_level::type::value);           \
    /* tracking for a class  */                                                \
    BOOST_STATIC_ASSERT((mpl::greater</* that is a prmitive */                 \
