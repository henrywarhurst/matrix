...
template <template <class T> class SPT> class shared_ptr_helper {
  typedef std::map<const void *,   // address of object
...
public:
  ...
  friend void boost::serialization::load(Archive &ar, SPT<U> &t,
    ...
    get_object_type(U &u) {
      return boost::serialization::singleton<
                 typename boost::serialization::type_info_implementation<
                     U>::type>::get_const_instance()
          .get_derived_extended_type_info(u);
    }
  };

public:
  template <class T> void reset(SPT<T> &s, T *t) {
    if (NULL == t) {
      s.reset();
      return;
    }
    ...
    typedef
        typename mpl::if_<is_polymorphic<T>, polymorphic, non_polymorphic>::type
            type;
