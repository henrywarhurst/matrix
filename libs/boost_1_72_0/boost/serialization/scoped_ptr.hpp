...
void save(Archive &ar, const boost::scoped_ptr<T> &t,
...
template <class Archive, class T>
void load(Archive &ar, boost::scoped_ptr<T> &t, const unsigned int /* version */
) {
  T *r;
  ar >> boost::serialization::make_nvp("scoped_ptr", r);
  t.reset(r);
...
template <class Archive, class T>
void serialize(Archive &ar, boost::scoped_ptr<T> &t,
               const unsigned int version) {
