...
template <class B, class D> struct base_cast {
...
template <class Base, class Derived> struct base_register {
  struct polymorphic {
  ...
  static void const *invoke() {
    typedef
        typename mpl::eval_if<is_polymorphic<Base>, mpl::identity<polymorphic>,
