...
void save(Archive &ar, const STD::valarray<U> &t,
  ...
  if (t.size()) {
    ...
    ar << serialization::make_array<const U, collection_size_type>(
        static_cast<const U *>(boost::addressof(t[0])), count);
  }
}

template <class Archive, class U>
void load(Archive &ar, STD::valarray<U> &t,
          const unsigned int /*file_version*/) {
  ...
  if (t.size()) {
    ...
    ar >> serialization::make_array<U, collection_size_type>(
              static_cast<U *>(boost::addressof(t[0])), count);
  }
}
...
template <class Archive, class U>
inline void serialize(Archive &ar, STD::valarray<U> &t,
                      const unsigned int file_version) {
