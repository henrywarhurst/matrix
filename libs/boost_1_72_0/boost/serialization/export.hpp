...
template <class Archive, class T> class pointer_iserializer;
template <class Archive, class T> class pointer_oserializer;

template <class Archive, class Serializable> struct export_impl {
...
template <class Archive, class Serializable> struct ptr_serialization_support {
...
#define BOOST_CLASS_EXPORT_IMPLEMENT(T)                                        \
  ...
  namespace detail {                                                           \
  namespace extra_detail {                                                     \
  template <> struct init_guid<T> { static guid_initializer<T> const &g; };    \
...
#define BOOST_CLASS_EXPORT_KEY2(T, K)                                          \
  ...
  template <> inline const char *guid<T>() { return K; }                       \
  } /* serialization */                                                        \
  } /* boost */                                                                \
  /**/
...
#define BOOST_SERIALIZATION_MWERKS_BASE_AND_DERIVED(Base, Derived)             \
  ...
  static int BOOST_PP_CAT(boost_serialization_mwerks_init_, __LINE__) =        \
      (::boost::archive::detail::instantiate_ptr_serialization((Derived *)0,   \
                                                               0),             \
       3);                                                                     \
