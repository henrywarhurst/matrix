...
template <class T> struct type_info_implementation {
  template <class U> struct traits_class_typeinfo_implementation {
...
#define BOOST_CLASS_TYPE_INFO(T, ETI)                                          \
  namespace boost {                                                            \
  namespace serialization {                                                    \
  template <> struct type_info_implementation<T> { typedef ETI type; };        \
  template <> struct type_info_implementation<const T> { typedef ETI type; };  \
  }                                                                            \
  }                                                                            \
  /**/
