...
template <class T> struct implementation_level_impl {
  template <class U> struct traits_class_level {
  ...
  typedef typename mpl::eval_if<
      ...
      typename mpl::eval_if<
          is_fundamental<T>, mpl::int_<primitive_type>,
          // else
          typename mpl::eval_if<
              is_class<T>, mpl::int_<object_class_info>,
              // else
              typename mpl::eval_if<
                  is_array<T>, mpl::int_<object_serializable>,
                  // else
                  typename mpl::eval_if<is_enum<T>, mpl::int_<primitive_type>,
                                        // else
                                        mpl::int_<not_serializable>>>>>>::type
      type;
...
#define BOOST_CLASS_IMPLEMENTATION(T, E)                                       \
  namespace boost {                                                            \
  namespace serialization {                                                    \
  template <> struct implementation_level_impl<const T> {                      \
    typedef mpl::integral_c_tag tag;                                           \
