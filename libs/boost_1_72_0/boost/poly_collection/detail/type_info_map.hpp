...
template <typename T, typename Allocator> class type_info_map {
  using map_type = std::unordered_map<
      const std::type_info *, T, type_info_ptr_hash, type_info_ptr_equal_to,
      typename std::allocator_traits<Allocator>::template rebind_alloc<
          std::pair<const std::type_info *const, T>>>;
...
public:
  ...
  type_info_map(const type_info_map &x)
      : map{x.map}, cache{make<cache_type>(
                        std::allocator_traits<cache_allocator_type>::
                            select_on_container_copy_construction(
                                x.cache.get_allocator()))} {
    build_cache(x.cache);
  }
  ...
  type_info_map(type_info_map &&x, const allocator_type &al)
      : map{make(std::move(x.map), al)},
        cache{al == allocator_type{x.map.get_allocator()} && x.map.empty()
  ...
  std::pair<iterator, bool> insert(const key_type &key, P &&x) {
    ...
    if (map.bucket_count() != c)
      rebuild_cache();
    cache.insert({&key, p.first});
    return p;
  }

  void swap(type_info_map &x) {
    map.swap(x.map);
    cache.swap(x.cache);
  }
...
private:
  using cache_type = std::unordered_map<
      const std::type_info *, iterator, std::hash<const std::type_info *>,
      std::equal_to<const std::type_info *>,
      typename std::allocator_traits<Allocator>::template rebind_alloc<
          std::pair<const std::type_info *const, iterator>>>;
  ...
  static UnorderedMap make(const typename UnorderedMap::allocator_type &al) {
    return UnorderedMap{10, typename UnorderedMap::hasher{},
                        typename UnorderedMap::key_equal{}, al};
  }

  template <typename UnorderedMap>
  static typename std::decay<UnorderedMap>::type
  make(UnorderedMap &&x,
       const typename std::decay<UnorderedMap>::type::allocator_type &al) {
    using RawUnorderedMap = typename std::decay<UnorderedMap>::type;

    return RawUnorderedMap{x.begin(),
                           x.end(),
                           0,
                           typename RawUnorderedMap::hasher{},
                           typename RawUnorderedMap::key_equal{},
                           al};
  }
#else
  template <typename UnorderedMap>
  ...
  void build_cache(const cache_type &x) {
    for (const auto &p : x)
      cache.insert({p.first, map.find(p.first)});
  }

  void rebuild_cache() {
    for (auto &p : cache)
      p.second = map.find(p.first);
  }
