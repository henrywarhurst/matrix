...
template <typename F, typename... TailArgs> struct tail_closure_class {
  ...
  return_type<Args &&...> operator()(Args &&... args) {
    return call(mp11::make_index_sequence<sizeof...(TailArgs)>{},
...
template <typename F, typename... HeadArgs> struct head_closure_class {
  ...
  template <typename... Args>
  return_type<Args &&...> operator()(Args &&... args) {
    return call(mp11::make_index_sequence<sizeof...(HeadArgs)>{},
                std::forward<Args>(args)...);
  }
...
template <typename ReturnType, typename F> struct cast_return_class {
...
template <typename F> struct deref_to_class {
...
template <typename F> deref_to_class<F> deref_to(const F &f) { return {f}; }

template <typename F> struct deref_1st_to_class {
...
template <typename F> deref_1st_to_class<F> deref_1st_to(const F &f) {
  return {f};
}
