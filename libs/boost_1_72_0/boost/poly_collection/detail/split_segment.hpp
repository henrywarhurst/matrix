...
class split_segment : public segment_backend<Model, Allocator> {
  ...
  using store =
      std::vector<store_value_type,
                  typename std::allocator_traits<
                      Allocator>::template rebind_alloc<store_value_type>>;
  using store_iterator = typename store::iterator;
  using const_store_iterator = typename store::const_iterator;
  using index =
      std::vector<value_type, typename std::allocator_traits<Allocator>::
  ...
  using segment_allocator_type = typename std::allocator_traits<
      Allocator>::template rebind_alloc<split_segment>;
...
public:
  ...
  range nv_emplace(Iterator p, Args &&... args) {
    auto q = prereserve(p);
    auto it = s.emplace(iterator_from(q), value_holder_emplacing_ctor,
                        std::forward<Args>(args)...);
    push_index_entry();
    return range_from(it);
  }

  template <typename... Args> range nv_emplace_back(Args &&... args) {
    prereserve();
  ...
  range nv_insert(InputIterator first, InputIterator last) {
    return nv_insert(const_iterator(concrete_ptr(s.data() + s.size())), first,
                     last);
  }
...
private:
  ...
  static segment_backend_unique_ptr new_(segment_allocator_type al,
                                         Args &&... args) {
  ...
  split_segment(const Allocator &al)
      : s{typename store::allocator_type{al}}, i{{},
                                                 typename index::allocator_type{
                                                     al}} {
  ...
  split_segment(store &&s_)
      : s{std::move(s_)}, i{{},
                            typename index::allocator_type{s.get_allocator()}} {
    s.reserve(1); /* --> s.data()!=nullptr */
    build_index();
  }
  ...
  void push_index_entry() { build_index(s.size()); }
  ...
  static const Concrete *
  const_concrete_ptr(const store_value_type *p) noexcept {
  ...
  template <typename InputIterator>
  range insert(const_iterator p, InputIterator first, InputIterator last,
