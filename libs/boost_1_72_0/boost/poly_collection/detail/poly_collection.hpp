...
template <typename Model, typename Allocator> class poly_collection {
  template <typename T> static const std::type_info &subtypeid(const T &x) {
    return Model::subtypeid(x);
  }
  template <typename...> struct for_all_types { using type = void *; };
  template <typename... T> using for_all = typename for_all_types<T...>::type;
  template <typename T>
  struct is_implementation
      : /* using makes VS2015 choke, hence we derive */
  ...
  using enable_if_not_terminal =
      typename std::enable_if<!is_terminal<T>::value>::type *;
  template <typename InputIterator>
  using derefs_to_terminal =
      is_terminal<typename std::iterator_traits<InputIterator>::value_type>;
  template <typename InputIterator>
  ...
  using enable_if_not_same = typename std::enable_if<!std::is_same<
  ...
  using segment_map =
      type_info_map<segment_type,
                    typename std::allocator_traits<segment_allocator_type>::
                        template rebind_alloc<segment_type>>;
...
private:
  template <typename, bool> friend class detail::iterator_impl;
  template <typename, typename> friend class detail::local_iterator_impl;
  template <bool Const>
...
public:
  ...
  public:
    ...
    operator=(const const_base_segment_info &) = default;
    ...
    template <typename T> const_local_iterator<T> begin() const noexcept {
      return const_local_iterator<T>{begin()};
    }
    template <typename T> const_local_iterator<T> end() const noexcept {
      return const_local_iterator<T>{end()};
    }
    template <typename T> const_local_iterator<T> cbegin() const noexcept {
      return begin<T>();
    }
    template <typename T> const_local_iterator<T> cend() const noexcept {
      return end<T>();
    }
  ...
  public:
    ...
    template <typename T> local_iterator<T> begin() noexcept {
      return local_iterator<T>{begin()};
    }
    template <typename T> local_iterator<T> end() noexcept {
      return local_iterator<T>{end()};
    }
  ...
  template <typename T> class segment_info : public const_segment_info<T> {
  public:
...
private:
  ...
  class segment_info_iterator_impl
      : public boost::iterator_adaptor<segment_info_iterator_impl<SegmentInfo>,
                                       const_segment_map_iterator, SegmentInfo,
                                       std::input_iterator_tag, SegmentInfo> {
  ...
  public:
    ...
    operator=(const segment_info_iterator_impl &) = default;

    template <typename SegmentInfo2,
              typename std::enable_if<std::is_base_of<
                  SegmentInfo, SegmentInfo2>::value>::type * = nullptr>
    segment_info_iterator_impl(
    ...
    template <typename SegmentInfo2,
              typename std::enable_if<std::is_base_of<
                  SegmentInfo, SegmentInfo2>::value>::type * = nullptr>
    segment_info_iterator_impl &
    operator=(const segment_info_iterator_impl<SegmentInfo2> &x) {
      this->base_reference() = x.base();
      return *this;
    }

  private:
    template <typename> friend class segment_info_iterator_impl;
    friend class poly_collection;
    friend class boost::iterator_core_access;
    template <typename> friend struct detail::iterator_traits;
...
private:
  template <typename Iterator> static Iterator nonconst_hlp(Iterator);
  ...
  static base_segment_info_iterator
      nonconst_hlp(const_base_segment_info_iterator);
...
public:
  ...
  public:
    const_segment_traversal_info(const const_segment_traversal_info &) =
        default;
    const_segment_traversal_info &
    operator=(const const_segment_traversal_info &) = default;

    const_base_segment_info_iterator begin() const noexcept {
      return pmap->cbegin();
    }
    const_base_segment_info_iterator end() const noexcept {
      return pmap->cend();
    }
  ...
  template <typename InputIterator>
  poly_collection(InputIterator first, InputIterator last,
  ...
  template <typename... T, for_all<enable_if_acceptable<T>...> = nullptr>
  void register_types() {
    /* http://twitter.com/SeanParent/status/558765089294020609 */

    using seq = int[1 + sizeof...(T)];
    (void)seq{0, (map.insert(typeid(T),
                             segment_type::template make<T>(get_allocator())),
                  0)...};
  }
  ...
  template <typename T, enable_if_acceptable<T> = nullptr>
  const_local_iterator<T> cbegin() const {
    return begin<T>();
  }

  template <typename T, enable_if_acceptable<T> = nullptr>
  const_local_iterator<T> cend() const {
    return end<T>();
  }
  ...
  template <typename T, enable_if_acceptable<T> = nullptr>
  segment_info<T> segment() {
    return get_map_iterator_for(typeid(T));
  }
  ...
  template <typename T, enable_if_acceptable<T> = nullptr> bool empty() const {
  ...
  iterator emplace_hint(const_iterator hint, Args &&... args) {
    ...
    return {it, map.end(),
            hint.mapit == it ? /* hint in segment */
                segment(it).template emplace<T>(hint.segpos,
                                                std::forward<Args>(args)...)
                             : segment(it).template emplace_back<T>(
                                   std::forward<Args>(args)...)};
  }

  template <typename T, typename... Args, enable_if_acceptable<T> = nullptr>
  local_base_iterator emplace_pos(local_base_iterator pos, Args &&... args) {
    return emplace_pos<T>(const_local_base_iterator{pos},
                          std::forward<Args>(args)...);
  }

  template <typename T, typename... Args, enable_if_acceptable<T> = nullptr>
  local_base_iterator emplace_pos(const_local_base_iterator pos,
                                  Args &&... args) {
    BOOST_ASSERT(pos.type_info() == typeid(T));
    return {pos.mapit, pos.segment().template emplace<T>(
                           pos.base(), std::forward<Args>(args)...)};
  }

  template <typename T, typename... Args>
  local_iterator<T> emplace_pos(local_iterator<T> pos, Args &&... args) {
    return emplace_pos(const_local_iterator<T>{pos},
                       std::forward<Args>(args)...);
  }

  template <typename T, typename... Args>
  local_iterator<T> emplace_pos(const_local_iterator<T> pos, Args &&... args) {
    return {pos.mapit, pos.segment().template emplace<T>(
                           pos.base(), std::forward<Args>(args)...)};
  }
  ...
  template <typename T, enable_if_not_same<const_iterator, T> = nullptr,
  ...
  insert(local_iterator_impl<BaseIterator> pos, T &&x) {
    ...
    return {pos.mapit, pos.segment().insert(pos.base(), std::forward<T>(x))};
  }
  ...
  template <typename BaseIterator>
  void insert(local_iterator_impl<BaseIterator> first,
  ...
  void insert(const_iterator hint, InputIterator first, InputIterator last) {
    ...
    auto &seg = segment(it);
    if (hint.mapit == it)
      seg.insert(hint.segpos, first, last); /* hint in segment */
    else
      seg.insert(first, last);
  }

  template <bool Const>
  void insert(const_iterator hint, iterator_impl<Const> first,
              iterator_impl<Const> last) {
    for (; first != last; ++first) {
  ...
  template <typename BaseIterator>
  void insert(const_iterator hint, local_iterator_impl<BaseIterator> first,
              local_iterator_impl<BaseIterator> last) {
    if (first == last)
      return;
  ...
  template <typename InputIterator,
            enable_if_derefs_to_implementation<InputIterator> = nullptr>
  local_base_iterator insert(const_local_base_iterator pos, InputIterator first,
                             InputIterator last) {
  ...
  local_iterator<T> insert(const_local_iterator<T> pos, InputIterator first,
                           InputIterator last) {
    auto &seg = pos.segment();
  ...
  template <typename T, typename InputIterator>
  local_iterator<T> insert(local_iterator<T> pos, InputIterator first,
                           InputIterator last) {
  ...
  iterator erase(const_iterator first, const_iterator last) {
    const_segment_map_iterator fseg = first.mapit, lseg = last.mapit,
                               end = first.mapend;
  ...
  template <typename BaseIterator>
  nonconst_version<local_iterator_impl<BaseIterator>>
  erase(local_iterator_impl<BaseIterator> first,
        local_iterator_impl<BaseIterator> last) {
    BOOST_ASSERT(first.mapit == last.mapit);
    return {first.mapit, first.segment().erase(first.base(), last.base())};
  }
  ...
  template <typename T, enable_if_acceptable<T> = nullptr> void clear() {
...
private:
  ...
  friend bool operator==(const poly_collection<M, A> &,
                         const poly_collection<M, A> &);
  ...
  const_segment_map_iterator get_map_iterator_for(const T &x) {
    ...
    if (it != map.end())
      ...
      return map
          .insert(typeid(T), segment_type::template make<T>(get_allocator()))
          .first;
  }
  ...
  template <typename T, enable_if_not_acceptable<T> = nullptr,
            enable_if_not_terminal<T> = nullptr>
  ...
  const_segment_map_iterator get_map_iterator_for(const T &) const {
    static_assert(is_acceptable<T>::value,
  ...
  const_segment_map_iterator get_map_iterator_for(const T &x,
    ...
    if (it != map.end())
      ...
      return map
          .insert(id, segment_type::make_from_prototype(seg, get_allocator()))
          .first;
  }

  template <typename T> const_segment_map_iterator get_map_iterator_for() {
    auto it = map.find(typeid(T));
    if (it != map.end())
      return it;
    else
  ...
  const_segment_map_iterator get_map_iterator_for(const std::type_info &info) {
    return const_cast<const poly_collection *>(this)->get_map_iterator_for(
        info);
  }

  const_segment_map_iterator
  get_map_iterator_for(const std::type_info &info) const {
  ...
  template <typename T, typename BaseIterator, typename U,
            enable_if_implementation<U> = nullptr,
            enable_if_not_constructible<T, U &&> = nullptr>
  static segment_base_iterator local_insert(segment_type &seg, BaseIterator pos,
                                            U &&x) {
  ...
  template <typename T, typename BaseIterator, typename U,
            enable_if_not_implementation<U> = nullptr,
            enable_if_not_constructible<T, U &&> = nullptr>
  static segment_base_iterator local_insert(segment_type &, BaseIterator,
                                            U &&) {
...
bool operator==(const poly_collection<Model, Allocator> &x,
  ...
  for (const auto &p : mapx) {
    ...
    if (it == mapy.end() ? ss != 0 : p.second != it->second)
      return false;
    s += ss;
  }
  return s == y.size();
...
template <typename Model, typename Allocator>
void swap(poly_collection<Model, Allocator> &x,
          poly_collection<Model, Allocator> &y) {
  x.swap(y);
}
