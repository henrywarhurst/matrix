...
class base_collection
  ...
  using base_type =
      common_impl::poly_collection<detail::base_model<Base>, Allocator>;
...
public:
  ...
  friend bool operator==(const base_collection<B, A> &,
                         const base_collection<B, A> &);
};

template <typename Base, typename Allocator>
bool operator==(const base_collection<Base, Allocator> &x,
...
template <typename Base, typename Allocator>
void swap(base_collection<Base, Allocator> &x,
          base_collection<Base, Allocator> &y) {
  x.swap(y);
}
