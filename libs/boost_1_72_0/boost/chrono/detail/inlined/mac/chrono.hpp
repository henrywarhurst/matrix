...
// It returns the number of microseconds since New Years 1970 in a struct called
// timeval which has a field for seconds and a field for microseconds.
...
time_t system_clock::to_time_t(const time_point &t) BOOST_NOEXCEPT {
  return time_t(duration_cast<seconds>(t.time_since_epoch()).count());
}

// Just turn the time_t into a count of seconds and construct a time_point with
// it.
system_clock::time_point system_clock::from_time_t(time_t t) BOOST_NOEXCEPT {
...
BOOST_CHRONO_STATIC
steady_clock::rep steady_simplified() { return mach_absolute_time(); }

#if !defined BOOST_CHRONO_DONT_PROVIDE_HYBRID_ERROR_HANDLING
BOOST_CHRONO_STATIC
steady_clock::rep steady_simplified_ec(system::error_code &ec) {
  if (!::boost::chrono::is_throws(ec)) {
    ec.clear();
  }
...
BOOST_CHRONO_STATIC
double compute_steady_factor(kern_return_t &err) {
...
#if !defined BOOST_CHRONO_DONT_PROVIDE_HYBRID_ERROR_HANDLING
BOOST_CHRONO_STATIC
steady_clock::rep steady_full_ec(system::error_code &ec) {
  kern_return_t err;
  const double factor = chrono_detail::compute_steady_factor(err);
  if (err != 0) {
    if (::boost::chrono::is_throws(ec)) {
      boost::throw_exception(system::system_error(
          err, ::boost::system::system_category(), "chrono::steady_clock"));
    } else {
...
BOOST_CHRONO_STATIC
FP init_steady_clock(kern_return_t &err) {
  mach_timebase_info_data_t MachInfo;
  err = mach_timebase_info(&MachInfo);
  if (err != 0) {
    return 0;
...
#if !defined BOOST_CHRONO_DONT_PROVIDE_HYBRID_ERROR_HANDLING
BOOST_CHRONO_STATIC
FP_ec init_steady_clock_ec(kern_return_t &err) {
  mach_timebase_info_data_t MachInfo;
  err = mach_timebase_info(&MachInfo);
  if (err != 0) {
    return 0;
