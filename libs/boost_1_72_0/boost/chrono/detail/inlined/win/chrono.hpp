...
steady_clock::time_point steady_clock::now() BOOST_NOEXCEPT {
  ...
  while (!boost::winapi::QueryPerformanceCounter(&pcount)) {
    if (++times > 3) {
      BOOST_ASSERT(0 &&
                   "Boost::Chrono - QueryPerformanceCounter Internal Error");
      return steady_clock::time_point();
    }
  }
...
steady_clock::time_point steady_clock::now(system::error_code &ec) {
  ...
  if ((nanosecs_per_tic <= 0.0L) ||
    ...
    boost::winapi::DWORD_ cause =
        ((nanosecs_per_tic <= 0.0L) ? ERROR_NOT_SUPPORTED
                                    : boost::winapi::GetLastError());
    if (::boost::chrono::is_throws(ec)) {
      boost::throw_exception(system::system_error(
          cause, ::boost::system::system_category(), "chrono::steady_clock"));
    } else {
...
system_clock::from_time_t(std::time_t t) BOOST_NOEXCEPT {
  __int64 temp = t;
  temp *= 10000000;
