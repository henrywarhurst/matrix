...
template <class CharT> class duration_punct : public std::locale::facet {
public:
...
private:
  ...
  template <class Period> string_type short_name(Period) const {
  ...
  template <class Period> string_type long_name(Period) const {
...
public:
  ...
  explicit duration_punct(int use = use_long) : use_short_(use == use_short) {
    init_C();
  }

  duration_punct(int use, const string_type &long_seconds,
                 const string_type &long_minutes, const string_type &long_hours,
  ...
  template <class Period> string_type plural() const {
    return long_name(typename Period::type());
  }

  template <class Period> string_type singular() const {
    return string_type(long_name(typename Period::type()), 0,
                       long_name(typename Period::type()).size() - 1);
  }
  ...
  template <class Period, class D> string_type name(D v) const {
    if (use_short_)
      return short_name<Period>();
    else {
      if (v == -1 || v == 1)
...
template <class CharT> std::locale::id duration_punct<CharT>::id;

template <class CharT> void duration_punct<CharT>::init_C() {
...
duration_punct<CharT>::duration_punct(int use, const string_type &long_seconds,
                                      const string_type &long_minutes,
                                      const string_type &long_hours,
                                      const string_type &short_seconds,
                                      const string_type &short_minutes,
                                      const string_type &short_hours)
    : use_short_(use == use_short), long_seconds_(long_seconds),
      long_minutes_(long_minutes), long_hours_(long_hours),
      short_seconds_(short_seconds), short_minutes_(short_minutes),
      short_hours_(short_hours) {}

template <class CharT>
duration_punct<CharT>::duration_punct(int use, const duration_punct &d)
    : use_short_(use == use_short), long_seconds_(d.long_seconds_),
      long_minutes_(d.long_minutes_), long_hours_(d.long_hours_),
      short_seconds_(d.short_seconds_), short_minutes_(d.short_minutes_),
...
operator>>(std::basic_istream<CharT, Traits> &is, duration<Rep, Period> &d) {
  ...
  loc = is.getloc();
  const Facet &f = std::use_facet<Facet>(loc);
  typedef typename chrono_detail::duration_io_intermediate<Rep>::type
      intermediate_type;
  intermediate_type r;
      ...
      if (i != e) {
        ...
        if (*i == '[') {
          ...
          const std::basic_string<CharT> units[] = {
              f.template singular<ratio<1>>(), f.template plural<ratio<1>>(),
              f.template short_name<ratio<1>>()};
          // std::cerr << __FILE__ << "[" << __LINE__ << "]"<< std::endl;
          const std::basic_string<CharT> *k = chrono_detail::scan_keyword(
              i, e, units, units + sizeof(units) / sizeof(units[0]),
        ...
        typedef
            typename common_type<intermediate_type, unsigned long long>::type
                common_type_t;
        ...
        // std::cerr << __FILE__ << "[" << __LINE__ << "]"<< std::endl;
        if (chrono::detail::gt(
                r, ((duration_values<common_type_t>::max)() / num))) {
