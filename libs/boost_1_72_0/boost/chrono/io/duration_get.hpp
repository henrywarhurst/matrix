...
template <class Rep> struct duration_io_intermediate<Rep, true> {
  typedef
      typename mpl::if_c<is_floating_point<Rep>::value, long double,
                         typename mpl::if_c<is_signed<Rep>::value, long long,
...
reduce(intermediate_type &r, unsigned long long &den,
  ...
  // Reduce r * num / den
  common_type_t t =
      integer::gcd<common_type_t>(common_type_t(r), common_type_t(den));
  r /= t;
  den /= t;
  if (den != 1) {
 ...
 * specifier to time_put<>::put. If the sequence being parsed matches the
 * correct format, the corresponding member of the class duration argument are
 * set to the value used to produce the sequence; otherwise either an error is
 * reported or unspecified values are assigned. In other words, user
 * confirmation is required for reliable parsing of user-entered durations, but
 * machine-generated formats can be parsed reliably. This allows parsers to be
...
public:
  ...
  explicit duration_get(size_t refs = 0) : std::locale::facet(refs) {}
   ...
   * s = get_value(s, end, ios, err, r) when the conversion specification is 'v'
   * and s = get_value(s, end, ios, err, rt) when the conversion specification
   * is 'u'. If err == std::ios_base::goodbit holds after the evaluation of the
   * expression, the function increments pattern to point just past the end of
  ...
  iter_type get(iter_type s, iter_type end, std::ios_base &ios,
                std::ios_base::iostate &err, duration<Rep, Period> &d,
    ...
    if (std::has_facet<duration_units<CharT>>(ios.getloc())) {
      duration_units<CharT> const &facet =
          std::use_facet<duration_units<CharT>>(ios.getloc());
  ...
  template <typename Rep, typename Period>
  iter_type get(duration_units<CharT> const &facet, iter_type s, iter_type end,
                std::ios_base &ios, std::ios_base::iostate &err,
                duration<Rep, Period> &d, const char_type *pattern,
                const char_type *pat_end) const {

    typedef
        typename detail::duration_io_intermediate<Rep>::type intermediate_type;
    intermediate_type r;
    rt_ratio rt;
    bool value_found = false, unit_found = false;

    const std::ctype<char_type> &ct =
        std::use_facet<std::ctype<char_type>>(ios.getloc());
    while (pattern != pat_end && err == std::ios_base::goodbit) {
      ...
      if (ct.narrow(*pattern, 0) == '%') {
        ...
        for (++pattern;
             pattern != pat_end && ct.is(std::ctype_base::space, *pattern);
             ++pattern)
          ;
    ...
    unsigned long long gcd_n1_n2 =
        integer::gcd<unsigned long long>(num, Period::num);
    unsigned long long gcd_d1_d2 =
        integer::gcd<unsigned long long>(den, Period::den);
    num /= gcd_n1_n2;
    den /= gcd_d1_d2;
    ...
    if (chrono::detail::gt(r,
                           ((duration_values<common_type_t>::max)() / num))) {
   ...
   * @param err the ios_base state
   * @param d the duration
   * Stores the duration pattern from the @c duration_unit facet in let say @c
   * str. Last as if
   * @code
   *   return get(s, end, ios, err, ios, d, str.data(), str.data() +
   * str.size());
   * @codeend
  ...
  iter_type get(iter_type s, iter_type end, std::ios_base &ios,
    ...
    if (std::has_facet<duration_units<CharT>>(ios.getloc())) {
      ...
      return get(facet, s, end, ios, err, d, str.data(),
                 str.data() + str.size());
    } else {
      duration_units_default<CharT> facet;
      std::basic_string<CharT> str = facet.get_pattern();
   ...
   * @Effects As if
   * @code
   * return std::use_facet<std::num_get<cahr_type, iter_type>
   * >(ios.getloc()).get(s, end, ios, err, r);
   * @endcode
   *
   * @Returns An iterator pointing just beyond the last character that can be
   * determined to be part of a valid name
  ...
  iter_type get_value(iter_type s, iter_type end, std::ios_base &ios,
                      std::ios_base::iostate &err, Rep &r) const {
    return std::use_facet<std::num_get<CharT, iter_type>>(ios.getloc())
        .get(s, end, ios, err, r);
  }
  template <typename Rep>
  iter_type get_value(iter_type s, iter_type end, std::ios_base &ios,
                      std::ios_base::iostate &err,
                      process_times<Rep> &r) const {
    if (s == end) {
      err |= std::ios_base::eofbit;
      return s;
    } else if (*s != '{') { // mandatory '{'
    ...
    ++s;
    s = std::use_facet<std::num_get<CharT, iter_type>>(ios.getloc())
            .get(s, end, ios, err, r.real);
    if (s == end) {
      err |= std::ios_base::eofbit;
      return s;
    } else if (*s != ';') { // mandatory ';'
   ...
   * determined to be part of a valid name
   */
  iter_type get_unit(iter_type i, iter_type e, std::ios_base &is,
                     std::ios_base::iostate &err, rt_ratio &rt) const {
    if (std::has_facet<duration_units<CharT>>(is.getloc())) {
      return get_unit(std::use_facet<duration_units<CharT>>(is.getloc()), i, e,
                      is, err, rt);
    } else {
      duration_units_default<CharT> facet;
  ...
  iter_type get_unit(duration_units<CharT> const &facet, iter_type i,
                     iter_type e, std::ios_base &is,
                     std::ios_base::iostate &err, rt_ratio &rt) const {

    if (*i == '[') {
      // parse [N/D]s or [N/D]second or [N/D]seconds format
      ++i;
      i = std::use_facet<std::num_get<CharT, iter_type>>(is.getloc())
              .get(i, e, is, err, rt.num);
...
protected:
  /**
   * Extracts the run-time ratio associated to the duration when it is given in
   * prefix form.
   *
   * This is an extension point of this facet so that we can take in account
   * other periods that can have a useful translation in other contexts, as e.g.
   * days and weeks.
   *
   ...
   * @return @c s
   */
  iter_type do_get_n_d_valid_unit(duration_units<CharT> const &facet,
                                  iter_type i, iter_type e, std::ios_base &,
    ...
    const string_type *k =
        chrono_detail::scan_keyword(i, e, units, units_end,
   ...
   * determined to be part of a valid name.
   */
  iter_type do_get_valid_unit(duration_units<CharT> const &facet, iter_type i,
                              iter_type e, std::ios_base &,
                              std::ios_base::iostate &err, rt_ratio &rt) const {
    // parse SI name, short or long
