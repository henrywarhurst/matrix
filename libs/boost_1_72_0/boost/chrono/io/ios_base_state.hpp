...
template <typename CharT> struct ios_base_data_aux {
...
template <typename CharT> struct ios_base_data {};
namespace /**/ {
...
inline std::basic_string<CharT> get_time_fmt(std::ios_base &ios) {
  ios_state_not_null_ptr<detail::ios_base_data<CharT>,
                         detail::ios_base_data_aux<CharT>>
      ptr(ios);
  return ptr->time_fmt;
}
template <typename CharT>
inline void set_time_fmt(std::ios_base &ios,
