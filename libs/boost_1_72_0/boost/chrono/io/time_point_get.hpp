...
public:
  ...
  explicit time_point_get(size_t refs = 0) : std::locale::facet(refs) {}
   ...
   * specifier character, the functions @c get_duration or @c get_epoch are
   * called depending on whether the format is @c 'd' or @c 'e'. If the number
   * of elements in the range [pattern,pat_end) is not sufficient to
   * unambiguously determine whether the conversion specification is complete
   * and valid, the function evaluates err = std::ios_base::failbit. Otherwise,
   * the function evaluates s = do_get(s, end, ios, err, d). If err ==
   * std::ios_base::goodbit holds after the evaluation of the expression, the
   * function increments pattern to point just past the end of the conversion
   * specification and continues looping.
  ...
  iter_type get(iter_type i, iter_type e, std::ios_base &is,
                std::ios_base::iostate &err, time_point<Clock, Duration> &tp,
    ...
    if (std::has_facet<time_point_units<CharT>>(is.getloc())) {
      time_point_units<CharT> const &facet =
          std::use_facet<time_point_units<CharT>>(is.getloc());
  ...
  template <class Clock, class Duration>
  iter_type get(time_point_units<CharT> const &facet, iter_type s,
                iter_type end, std::ios_base &ios, std::ios_base::iostate &err,
                time_point<Clock, Duration> &tp, const char_type *pattern,
   ...
   * @endcode
   * where @c facet is either the @c duration_get facet associated to the @c ios
   * or an instance of the default @c duration_get facet.
   *
   * @Returns An iterator pointing just beyond the last character that can be
   * determined to be part of a valid duration.
   */
  template <typename Rep, typename Period>
  iter_type get_duration(iter_type i, iter_type e, std::ios_base &is,
                         std::ios_base::iostate &err,
                         duration<Rep, Period> &d) const {
    if (std::has_facet<duration_get<CharT>>(is.getloc())) {
      duration_get<CharT> const &facet =
  ...
  template <typename Rep, typename Period>
  iter_type get_duration(duration_get<CharT> const &facet, iter_type s,
                         iter_type end, std::ios_base &ios,
                         std::ios_base::iostate &err,
                         duration<Rep, Period> &d) const {
    return facet.get(s, end, ios, err, d);
  }
  ...
  /**
   *
   * @Effects Let @c facet be the @c time_point_units facet associated to @c is
   * or a new instance of the default @c time_point_units_default facet. Let @c
   * epoch be the epoch string associated to the Clock using this facet. Scans
   * @c i to match @c epoch or @c e is reached.
   *
   * If not match before the @c e is reached @c std::ios_base::failbit is set in
   * @c err. If @c e is reached @c std::ios_base::failbit is set in @c err.
   *
   * @Returns An iterator pointing just beyond the last character that can be
   * determined to be part of a valid epoch.
   */
  template <class Clock>
  iter_type get_epoch(iter_type i, iter_type e, std::ios_base &is,
  ...
  template <class Clock>
  iter_type get_epoch(time_point_units<CharT> const &facet, iter_type i,
                      iter_type e, std::ios_base &,
                      std::ios_base::iostate &err) const {
    const std::basic_string<CharT> epoch = facet.template get_epoch<Clock>();
    std::ptrdiff_t k = chrono_detail::scan_keyword(
                           i, e, &epoch, &epoch + 1,
