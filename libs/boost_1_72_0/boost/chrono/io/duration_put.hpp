...
namespace detail {
template <class T> struct propagate { typedef T type; };
template <> struct propagate<boost::int_least32_t> {
 ...
 *
 * The @c duration_put facet provides facilities for formatted output of
 * duration values. The member function of @c duration_put take a duration and
 * format it into character string representation.
 *
 */
...
public:
   ...
   */
  explicit duration_put(size_t refs = 0) : std::locale::facet(refs) {}
   ...
   * @c ct.narrow(c,0), where @c ct is a reference to @c ctype<charT> obtained
   * from
  ...
  iter_type put(iter_type s, std::ios_base &ios, char_type fill,
                duration<Rep, Period> const &d, const CharT *pattern,
  ...
  template <typename Rep, typename Period>
  iter_type put(duration_units<CharT> const &units_facet, iter_type s,
   ...
   * @param fill the character used as filler
   * @param d the duration
   * @Effects imbue in @c ios the @c duration_units_default facet if not already
   * present. Retrieves Stores the duration pattern from the @c duration_unit
   * facet in let say @c str. Last as if
   * @code
   ...
   * @param fill the character used as filler
   * @param d the duration
   * @Effects As if s=std::use_facet<std::num_put<CharT, iter_type>
   * >(ios.getloc()).put(s, ios, fill, static_cast<long int> (d.count())).
   * @Returns s, iterator pointing immediately after the last character
   * produced.
   */
  template <typename Rep, typename Period>
  iter_type put_value(iter_type s, std::ios_base &ios, char_type fill,
                      duration<Rep, Period> const &d,
                      const char_type *val = 0) const {
    if (val) {
      while (*val) {
    ...
    return std::use_facet<std::num_put<CharT, iter_type>>(ios.getloc())
        .put(s, ios, fill,
  ...
  iter_type put_value(iter_type s, std::ios_base &ios, char_type fill,
                      ...
                      const char_type * = 0) const {
    *s++ = CharT('{');
    s = put_value(s, ios, fill,
                  process_real_cpu_clock::duration(d.count().real));
    *s++ = CharT(';');
    s = put_value(s, ios, fill,
                  process_user_cpu_clock::duration(d.count().user));
    *s++ = CharT(';');
    s = put_value(s, ios, fill,
                  process_system_cpu_clock::duration(d.count().system));
    *s++ = CharT('}');
    return s;
  }
   ...
   * @param fill the character used as filler
   * @param d the duration
   * @Effects Let facet be the duration_units<CharT> facet associated to ios. If
   the associated unit is named,
   * as if
   * @code
      string_type str = facet.get_unit(get_duration_style(ios), d);
      s=std::copy(str.begin(), str.end(), s);
   * @endcode
   * Otherwise, format the unit as "[Period::num/Period::den]" followed by the
   unit associated to [N/D] obtained using
   facet.get_n_d_unit(get_duration_style(ios), d)
   * @Returns s, iterator pointing immediately after the last character
   produced.
   */
  template <typename Rep, typename Period>
  iter_type put_unit(iter_type s, std::ios_base &ios, char_type fill,
  ...
  template <typename Rep, typename Period>
  iter_type put_unit(duration_units<CharT> const &facet, iter_type s,
                     std::ios_base &ios, char_type fill,
                     duration<Rep, Period> const &d) const {
    if (facet.template is_named_unit<Period>()) {
      ...
      *s++ = CharT('[');
      std::use_facet<std::num_put<CharT, iter_type>>(ios.getloc())
          .put(s, ios, fill, Period::num);
      *s++ = CharT('/');
      std::use_facet<std::num_put<CharT, iter_type>>(ios.getloc())
          .put(s, ios, fill, Period::den);
      *s++ = CharT(']');
