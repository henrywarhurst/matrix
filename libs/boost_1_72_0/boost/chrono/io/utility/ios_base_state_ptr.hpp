 ...
 * xalloc key holder.
 */
template <typename T> struct xalloc_key_holder {
  static int value;        //< the xalloc value associated to T.
  static bool initialized; //< whether the value has been initialized or not.
};

template <typename T> int xalloc_key_holder<T>::value = 0;

template <typename T> bool xalloc_key_holder<T>::initialized = false;
 ...
 *
 * Declare a static variable of this type to ensure that the
 * xalloc_key_holder<T> is initialized correctly.
 */
template <typename T> struct xalloc_key_initializer {
 ...
 */
template <typename Final, typename T> class ios_state_ptr {
...
public:
   ...
   */
  explicit ios_state_ptr(std::ios_base &ios) : ios_(ios) {}
  /**
   ...
   * Registers the callback responsible of maintaining the state pointer
   * coherency, if not already done. Retrieves the associated ios pointer
   ...
   * @return the retrieved pointer.
   */
  T *operator->() BOOST_NOEXCEPT { return get(); }
  /**
   * @Effects as if @c return get();
   * @return the retrieved pointer.
   */
  T const *operator->() const BOOST_NOEXCEPT { return get(); }
  ...
  /**
   * @Effects reset the current pointer after storing in a temporary variable
   ...
   */
  explicit operator bool() const BOOST_NOEXCEPT { return get() != 0; }
#endif

  std::ios_base &getios() BOOST_NOEXCEPT { return ios_; }
  std::ios_base &getios() const BOOST_NOEXCEPT { return ios_; }
  /**
   * Implicit conversion to the ios_base
   */
  operator std::ios_base &() BOOST_NOEXCEPT { return ios_; }
  /**
   * Implicit conversion to the ios_base const
   */
  operator std::ios_base &() const BOOST_NOEXCEPT { return ios_; }
...
private:
  ...
  static inline void callback(std::ios_base::event evt, std::ios_base &ios,
                              int index) {
    switch (evt) {
  ...
  static inline int index() { return detail::xalloc_key_holder<Final>::value; }
...
// detail::xalloc_key_initializer<Final>
// ios_state_ptr<Final,T>::xalloc_key_initializer_;
...
public:
  explicit ios_state_not_null_ptr(std::ios_base &ios) : base_type(ios) {
    if (this->get() == 0) {
 ...
 */
template <typename Final> class ios_flags {
public:
  /**
   *
   ...
   */
  explicit ios_flags(std::ios_base &ios) : ios_(ios) {}
  ~ios_flags() {}
  /**
   * @Returns The format control information.
   */
  long flags() const BOOST_NOEXCEPT { return value(); }
   ...
   */
  operator std::ios_base const &() const BOOST_NOEXCEPT { return ios_; }

private:
  long value() const BOOST_NOEXCEPT { return ios_.iword(index()); }
  long &ref() BOOST_NOEXCEPT { return ios_.iword(index()); }
  static inline int index() { return detail::xalloc_key_holder<Final>::value; }
...
// detail::xalloc_key_initializer<Final>
// ios_flags<Final>::xalloc_key_initializer_;
