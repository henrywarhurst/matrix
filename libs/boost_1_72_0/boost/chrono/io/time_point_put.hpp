 ...
 *
 * The @c time_point_put facet provides facilities for formatted output of @c
 * time_point values. The member function of @c time_point_put take a @c
 * time_point and format it into character string representation.
 *
 */
...
public:
   ...
   */
  explicit time_point_put(size_t refs = 0) : std::locale::facet(refs) {}
   ...
   * <c>put_duration(s, ios, fill, tp.time_since_epoch())</c> or <c>put_epoch(s,
   * ios)</c>.
   *
  ...
  iter_type put(iter_type i, std::ios_base &ios, char_type fill,
                time_point<Clock, Duration> const &tp, const CharT *pattern,
  ...
  template <class Clock, class Duration>
  iter_type put(time_point_units<CharT> const &units_facet, iter_type s,
   ...
   * @param pat_end end of the formatting pattern
   *
   * @Effects Stores the time_point pattern from the @c time_point_unit facet in
   * let say @c str. Last as if
   * @code
  ...
  template <class Clock, class Duration>
  iter_type put(iter_type i, std::ios_base &ios, char_type fill,
                time_point<Clock, Duration> const &tp) const {
    if (std::has_facet<time_point_units<CharT>>(ios.getloc())) {
      time_point_units<CharT> const &facet =
          std::use_facet<time_point_units<CharT>>(ios.getloc());
   ...
   * @Effects As if <c>facet.put(s, ios, fill, d)</c> where facet is the @c
   * duration_put<CharT> facet associated to the @c ios or a new instance of @c
   * duration_put<CharT>.
   * @Returns An iterator pointing immediately after the last character
   * produced.
   */
  template <typename Rep, typename Period>
  iter_type put_duration(iter_type i, std::ios_base &ios, char_type fill,
                         duration<Rep, Period> const &d) const {
    if (std::has_facet<duration_put<CharT>>(ios.getloc())) {
      duration_put<CharT> const &facet =
          std::use_facet<duration_put<CharT>>(ios.getloc());
  ...
  iter_type put_epoch(iter_type i, std::ios_base &os) const {
    if (std::has_facet<time_point_units<CharT>>(os.getloc())) {
      time_point_units<CharT> const &facet =
          std::use_facet<time_point_units<CharT>>(os.getloc());
  ...
  template <typename Clock>
  iter_type put_epoch(time_point_units<CharT> const &facet, iter_type s,
                      std::ios_base &) const {
    string_type str = facet.template get_epoch<Clock>();
    s = std::copy(str.begin(), str.end(), s);
    return s;
