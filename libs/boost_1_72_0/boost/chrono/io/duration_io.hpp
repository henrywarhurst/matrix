...
public:
   ...
   */
  explicit duration_fmt(duration_style style) BOOST_NOEXCEPT : style_(style) {}
...
struct duration_style_io_saver {
   ...
   *
   * Stores a reference to the i/o stream and the value @c new_value @c
   * duration_style to set.
   */
   ...
   */
  ~duration_style_io_saver() { this->restore(); }
...
struct duration_put_enabled
    : integral_constant<bool, is_integral<Rep>::value ||
...
typename boost::enable_if_c<!duration_put_enabled<Rep>::value,
    ...
    BOOST_TRY {
      ...
      if (bool(opfx)) {
        if (!std::has_facet<duration_put<CharT>>(os.getloc())) {
          if (duration_put<CharT>()
                  .put(os, os, os.fill(), dd, ostr.str().c_str())
                  .failed()) {
            err = std::ios_base::badbit;
          }
        } else if (std::use_facet<duration_put<CharT>>(os.getloc())
                       .put(os, os, os.fill(), dd, ostr.str().c_str())
                       .failed()) {
          err = std::ios_base::badbit;
        }
  ...
  BOOST_TRY {
    ...
    if (bool(ipfx)) {
      if (!std::has_facet<duration_get<CharT>>(is.getloc())) {
        duration_get<CharT>().get(is, std::istreambuf_iterator<CharT, Traits>(),
                                  is, err, d);
      } else {
