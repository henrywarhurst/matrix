...
#define _REENTRANT
    ...
    defined(_GLIBCXX_HAS_GTHREADS) || defined(_WIN32) || defined(_AIX) ||      \
    defined(__HAIKU__)
//
...
// Apple doesn't seem to reliably defined a *unix* macro
    ...
    (defined(__unix__) || defined(__unix) || defined(unix) ||                  \
     defined(__APPLE__) || defined(__APPLE) || defined(APPLE))
#include <unistd.h>
#endif
...
//  libstdc++ C++0x support is detected via __GNUC__, __GNUC_MINOR__, and
//  possibly
//  __GNUC_PATCHLEVEL__ at the suggestion of Jonathan Wakely, one of the
//  libstdc++ developers. He also commented:
//
...
//
//  However, using the GCC version number fails when the compiler is clang since
//  this only ever claims to emulate GCC-4.2, see
//  https://svn.boost.org/trac/boost/ticket/7473 for a long discussion on this
//  issue.  What we can do though is use clang's __has_include to detect the
//  presence of a C++11 header that was introduced with a specific GCC release.
//  We still have to be careful though as many such headers were buggy and/or
//  incomplete when first introduced, so we only check for headers that were
//  fully featured from day 1, and then use that to infer the underlying GCC
//  version:
//
#ifdef __clang__
...
// libstdc++ does not define this function as it's deprecated in C++11, but
// clang still looks for it, defining it here is a terrible cludge, but should
// get things working:
...
#define BOOST_LIBSTDCXX_VERSION                                                \
...
// Note that although <chrono> existed prior to 4.7, "steady_clock" is spelled
// "monotonic_clock" so 4.7.0 is the first truly conforming one.
#define BOOST_NO_CXX11_HDR_CHRONO
...
// Note that although <atomic> existed prior to gcc 4.8 it was largely
// unimplemented for many types:
...
// Although <regex> is present and compilable against, the actual implementation
// is not functional even for the simplest patterns such as "\d" or "[0-9]".
// This is the case at least in gcc up to 4.8, inclusively.
#define BOOST_NO_CXX11_HDR_REGEX
#endif
...
#define BOOST_NO_CXX14_HDR_SHARED_MUTEX
#endif
#elif __cplusplus < 201402 || (BOOST_LIBSTDCXX_VERSION < 40900) ||             \
...
#if (!defined(_GTHREAD_USE_MUTEX_TIMEDLOCK) ||                                 \
     (_GTHREAD_USE_MUTEX_TIMEDLOCK == 0)) &&                                   \
