...
//                 type. Eventually this leads to compile errors in
//                 ratio_fwd.hpp, when trying to define several constants which
//                 do not fit into a long type! We correct them here by
//                 redefining.
...
// vxWorks-around: The required functions getrlimit() and getrlimit() are
// missing.
//                 But we have the similar functions getprlimit() and
//                 setprlimit(), which may serve the purpose. Problem: The
//                 vxWorks-documentation regarding these functions doesn't
//                 deserve its name! It isn't documented what the first two
//                 parameters idtype and id mean, so we must fall back to an
//                 educated guess - null, argh... :-/

// TODO: getprlimit() and setprlimit() do exist for RTPs only, for whatever
// reason.
...
inline ssize_t readlink(const char *path1 ___unused, char *path2 ___unused,
                        size_t size ___unused) {
  // vxWorks has no symlinks -> always return an error!
  errno = EACCES;
  return -1;
}
...
inline clock_t times(struct tms *t) {
  ...
  clock_t ticks(static_cast<clock_t>(
...
#ifdef __cplusplus
extern "C" void
bzero(void *, size_t); // FD_ZERO uses bzero() but doesn't include strings.h
...
inline int lstat(p, b) {
  return stat(p, b);
} // lstat() == stat(), as vxWorks has no symlinks!
#endif
...
#ifndef FPE_FLTINV
#define FPE_FLTINV                                                             \
  (FPE_FLTSUB + 1) // vxWorks has no FPE_FLTINV, so define one as a dummy
#endif
#if !defined(BUS_ADRALN) && defined(BUS_ADRALNR)
#define BUS_ADRALN BUS_ADRALNR // Correct a supposed typo in vxWorks' <signal.h>
#endif
typedef int locale_t; // locale_t is a POSIX-extension, currently not present in
                      // vxWorks!
...
#define BOOST_NO_CXX11_NUMERIC_LIMITS // max_digits10 in
                                      // test/../print_helper.hpp
...
// most libraries will use select() but this define allows 'iostream'
// functionality which is based on poll() only
