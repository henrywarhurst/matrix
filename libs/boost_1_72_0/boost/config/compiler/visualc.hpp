...
// C++ 11:
//
// This is supported with /permissive- for 15.5 onwards, unfortunately we appear
// to have no way to tell if this is in effect or not, in any case nothing in
// Boost is currently using this, so we'll just go on defining it for now:
//
...
#error                                                                         \
    "boost: Unknown EVC++ compiler version - please run the configure tests and report the results"
#else
#pragma message(                                                               \
    "boost: Unknown EVC++ compiler version - please run the configure tests and report the results")
#endif
...
// BOOST_PRAGMA_MESSAGE("Info: Boost.Config is older than your compiler version
// - probably nothing bad will happen - but you may wish to look for an updated
// Boost version. Define BOOST_CONFIG_SUPPRESS_OUTDATED_MESSAGE to suppress this
// message.")
#endif
#endif
