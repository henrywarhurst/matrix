...
/** Write part of a message to a stream asynchronously using a serializer.
    ...
    void handler(
        ...
        std::size_t bytes_transferred   // the number of bytes written to the
   stream
    );
    @endcode
...
async_write_some(
    AsyncWriteStream &stream, serializer<isRequest, Body, Fields> &sr,
...
async_write(
    ...
    ,
    typename std::enable_if<is_mutable_body_writer<Body>::value>::type * = 0
#endif
);
