...
struct chunk_extensions_impl : chunk_extensions {
  ...
  chunk_extensions_impl(ChunkExtensions const &ext) : ext_(ext) {}

  net::const_buffer str() override {
...
struct is_chunk_extensions<
    T, beast::detail::void_t<decltype(std::declval<string_view &>() =
                                          std::declval<T &>().str())>>
    : std::true_type {};
 ...
 */
class chunk_size {
  template <class OutIter> static OutIter to_hex(OutIter last, std::size_t n) {
    if (n == 0) {
      *--last = '0';
      return last;
    }
  ...
  struct sequence {
    ...
    explicit sequence(std::size_t n) {
      ...
      b = {it, static_cast<std::size_t>(it0 - it)};
    }
  };
...
public:
  ...
  */
  chunk_size(std::size_t n) : sp_(std::make_shared<sequence>(n)) {}

  const_iterator begin() const { return &sp_->b; }
...
struct chunk_size0 {
  ...
  const_iterator begin() const { return &chunk_last(); }
...
template <class T, bool = is_fields<T>::value> struct buffers_or_fields {
  using type = typename T::writer::const_buffers_type;
};

template <class T> struct buffers_or_fields<T, false> { using type = T; };
