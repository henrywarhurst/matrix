...
{
public:
  static_assert(is_fields<Fields>::value, "Fields type requirements not met");
  ...
  */
#if BOOST_BEAST_DOXYGEN
  template <class... Args> explicit header(Args &&... args);
...
private:
  ...
  header(verb method, string_view target_, unsigned version_value,
...
public:
  ...
  */
  ...
  explicit message(header_type &&h, BodyArgs &&... body_args);
  ...
  */
  template <class... BodyArgs>
  explicit message(header_type const &h, BodyArgs &&... body_args);
  ...
  */
  ...
  template <
      class Version,
      class = typename std::enable_if<
          isRequest && std::is_convertible<Version, unsigned>::value>::type>
  ...
  */
  ...
  message(verb method, string_view target, unsigned version, BodyArg &&body_arg,
          FieldsArg &&fields_arg);
#else
  template <
      class Version, class BodyArg, class FieldsArg,
      class = typename std::enable_if<
          isRequest && std::is_convertible<Version, unsigned>::value>::type>
  message(verb method, string_view target, Version version, BodyArg &&body_arg,
          FieldsArg &&fields_arg);
  ...
  */
  ...
  /** Constructor
      ...
      @param fields_arg An argument forwarded to the `Fields` base class
     constructor.
  ...
  */
  explicit message(std::piecewise_construct_t);
  ...
  */
  ...
  header_type const &base() const { return *this; }

  /// Returns the header portion of the message
  header_type &base() { return *this; }

  /// Returns `true` if the chunked Transfer-Encoding is specified
  bool chunked() const { return this->get_chunked_impl(); }
  ...
  */
  bool has_content_length() const { return this->has_content_length_impl(); }
  ...
  */
  bool keep_alive() const { return this->get_keep_alive_impl(this->version()); }
  ...
  */
  bool need_eof() const { return need_eof(typename header_type::is_request{}); }
  ...
  */
  ...
  detail::value_type_t<Body> &
      ...
      body() &
      noexcept {
    return this->boost::empty_value<typename Body::value_type>::get();
  }
  ...
  detail::value_type_t<Body> &&
      ...
      noexcept {
    return std::move(
        this->boost::empty_value<typename Body::value_type>::get());
  }
...
private:
  ...
  template <class... BodyArgs, std::size_t... IBodyArgs>
  message(std::piecewise_construct_t, std::tuple<BodyArgs...> &body_args,
          mp11::index_sequence<IBodyArgs...>)
      : boost::empty_value<typename Body::value_type>(
            boost::empty_init_t(),
            std::forward<BodyArgs>(std::get<IBodyArgs>(body_args))...) {
    boost::ignore_unused(body_args);
  }

  template <class... BodyArgs, class... FieldsArgs, std::size_t... IBodyArgs,
  ...
  message(std::piecewise_construct_t, std::tuple<BodyArgs...> &body_args,
      ...
      : header_type(
            std::forward<FieldsArgs>(std::get<IFieldsArgs>(fields_args))...),
        boost::empty_value<typename Body::value_type>(
            boost::empty_init_t(),
            std::forward<BodyArgs>(std::get<IBodyArgs>(body_args))...) {
    boost::ignore_unused(body_args);
  ...
  bool need_eof(std::true_type) const { return !keep_alive(); }

  bool need_eof(std::false_type) const;

  boost::optional<std::uint64_t> payload_size(std::true_type) const {
    return Body::size(this->body());
  }

  boost::optional<std::uint64_t> payload_size(std::false_type) const {
    return boost::none;
  }

  void prepare_payload(std::true_type);

  void prepare_payload(std::false_type);
};
