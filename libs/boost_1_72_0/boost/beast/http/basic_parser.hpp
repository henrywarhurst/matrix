...
/** A parser for decoding HTTP/1 wire format messages.
    ...
    such as @ref read, @ref read_some, @ref async_read, and @ref
   async_read_some. Alternatively, the caller may use custom techniques to
   ensure that the structured portion of the HTTP message (header or chunk
   header) is contained in a linear buffer.
...
class basic_parser : private detail::basic_parser_base {
  ...
  static constexpr std::uint64_t default_body_limit(std::true_type) {
    // limit for requests
    return 1 * 1024 * 1024; // 1MB
  }

  static constexpr std::uint64_t default_body_limit(std::false_type) {
    // limit for responses
    return 8 * 1024 * 1024; // 8MB
  }

  template <bool OtherIsRequest> friend class basic_parser;
...
public:
  ...
  bool got_some() const { return state_ != state::nothing_yet; }
  ...
  */
   ...
   */
  bool is_header_done() const { return state_ > state::fields; }
  ...
  */
  bool upgrade() const { return (f_ & flagConnectionUpgrade) != 0; }
  ...
  /** Returns `true` if the last value for Transfer-Encoding is "chunked".
      ...
      @ref is_header_done would return `true`.
  */
  bool chunked() const { return (f_ & flagChunked) != 0; }
  ...
  */
  ...
  /** Returns the remaining content length if known
      ...
      @note The return value is undefined unless
            @ref is_header_done would return `true`.
  */
  boost::optional<std::uint64_t> content_length_remaining() const;
  ...
  /** Returns `true` if the message semantics require an end of file.
      ...
      data from the input.
  */
  bool need_eof() const { return (f_ & flagNeedEOF) != 0; }
  ...
  */
  void body_limit(std::uint64_t v) { body_limit_ = v; }
  ...
  /** Set a limit on the total size of the header.
      ...
      results in undefined behavior.
  */
  void header_limit(std::uint32_t v) { header_limit_ = v; }

  /// Returns `true` if the eager parse option is set.
  bool eager() const { return (f_ & flagEager) != 0; }
  ...
  */
  ...
  bool skip() const { return (f_ & flagSkipBody) != 0; }
  ...
  */
  ...
  std::size_t put(net::const_buffer buffer, error_code &ec);
#endif
...
protected:
  ...
  */
  virtual void on_request_impl(verb method, string_view method_str,
                               string_view target, int version,
                               error_code &ec) = 0;
  ...
  /** Called after receiving the status-line.
      ...
      an error. The error will be clear before this function is invoked.
  */
  virtual void on_response_impl(int code, string_view reason, int version,
                                error_code &ec) = 0;
  ...
  /** Called once for each complete field in the HTTP header.
      ...
      an error. The error will be clear before this function is invoked.
  */
  virtual void on_field_impl(field name, string_view name_string,
                             string_view value, error_code &ec) = 0;
  ...
  /** Called once after the complete HTTP header is received.
      ...
      an error. The error will be clear before this function is invoked.
  */
  virtual void on_header_impl(error_code &ec) = 0;
  ...
  */
  virtual void
  on_body_init_impl(boost::optional<std::uint64_t> const &content_length,
                    error_code &ec) = 0;

  /** Called each time additional data is received representing the content
     body.
  ...
  */
  virtual std::size_t on_body_impl(string_view body, error_code &ec) = 0;
  ...
  /** Called each time a new chunk header of a chunk encoded body is received.
      ...
      an error. The error will be clear before this function is invoked.
  */
  virtual void on_chunk_header_impl(std::uint64_t size, string_view extensions,
                                    error_code &ec) = 0;
  ...
  /** Called each time additional data is received representing part of a body
     chunk.
      ...
      @see on_body_impl
  */
  virtual std::size_t on_chunk_body_impl(std::uint64_t remain, string_view body,
                                         error_code &ec) = 0;
  ...
  /** Called once when the complete message is received.
      ...
      an error. The error will be clear before this function is invoked.
  */
  virtual void on_finish_impl(error_code &ec) = 0;

private:
  template <class ConstBufferSequence>
  std::size_t put_from_stack(std::size_t size,
                             ConstBufferSequence const &buffers,
                             error_code &ec);

  void maybe_need_more(char const *p, std::size_t n, error_code &ec);

  void parse_start_line(char const *&p, char const *last, error_code &ec,
                        std::true_type);

  void parse_start_line(char const *&p, char const *last, error_code &ec,
                        std::false_type);

  void parse_fields(char const *&p, char const *last, error_code &ec);

  void finish_header(error_code &ec, std::true_type);

  void finish_header(error_code &ec, std::false_type);

  void parse_body(char const *&p, std::size_t n, error_code &ec);

  void parse_body_to_eof(char const *&p, std::size_t n, error_code &ec);

  void parse_chunk_header(char const *&p, std::size_t n, error_code &ec);

  void parse_chunk_body(char const *&p, std::size_t n, error_code &ec);

  void do_field(field f, string_view value, error_code &ec);
};
