...
template <class DynamicBuffer> struct basic_dynamic_body {
  ...
  */
  static std::uint64_t size(value_type const &v) { return v.size(); }
  ...
  public:
    ...
    std::size_t put(ConstBufferSequence const &buffers, error_code &ec) {
      ...
      auto const mb = beast::detail::dynamic_buffer_prepare(
          body_, (std::min)(n, body_.max_size() - body_.size()), ec,
          error::buffer_overflow);
      if (ec)
        return 0;
      auto const bytes_transferred = net::buffer_copy(*mb, buffers);
