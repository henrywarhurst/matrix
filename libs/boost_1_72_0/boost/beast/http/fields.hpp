...
public:
  ...
  public:
    ...
    string_view const name_string() const;
  ...
  */
...
#endif
  {
    ...
    bool operator()(string_view lhs, value_type const &rhs) const noexcept {
      if (lhs.size() < rhs.name_string().size()) return true;
...
struct element
    : public boost::intrusive::list_base_hook<
      ...
      public boost::intrusive::set_base_hook<
          boost::intrusive::link_mode<boost::intrusive::normal_link>>,
      public value_type {
  element(field name, string_view sname, string_view value);
};
...
using align_type = typename boost::type_with_alignment<alignof(element)>::type;

using rebind_type = typename beast::detail::allocator_traits<
    Allocator>::template rebind_alloc<align_type>;

using alloc_traits = beast::detail::allocator_traits<rebind_type>;

using size_type =
    typename beast::detail::allocator_traits<Allocator>::size_type;
...
*/
explicit basic_fields(Allocator const &alloc) noexcept;
...
/// Copy constructor.
template <class OtherAlloc> basic_fields(basic_fields<OtherAlloc> const &);
...
allocator_type get_allocator() const { return this->get(); }
...
*/
string_view const at(field name) const;
...
/** Returns the value for a field, or throws an exception.
    ...
    @throws std::out_of_range if the field is not found.
*/
string_view const at(string_view name) const;
...
/** Returns the value for a field, or `""` if it does not exist.
    ...
    @param name The name of the field.
*/
string_view const operator[](field name) const;

/** Returns the value for a case-insensitive matching header, or `""` if it does
   not exist.
    ...
    @param name The name of the field.
*/
string_view const operator[](string_view name) const;
...
private:
template <class OtherAlloc> friend class basic_fields;

element &new_element(field name, string_view sname, string_view value);

void delete_element(element &e);

void set_element(element &e);

void realloc_string(string_view &dest, string_view s);

void realloc_target(string_view &dest, string_view s);

template <class OtherAlloc> void copy_all(basic_fields<OtherAlloc> const &);

void clear_all();

void delete_list();

void move_assign(basic_fields &, std::true_type);

void move_assign(basic_fields &, std::false_type);

void copy_assign(basic_fields const &, std::true_type);

void copy_assign(basic_fields const &, std::false_type);

void swap(basic_fields &other, std::true_type);

void swap(basic_fields &other, std::false_type);
