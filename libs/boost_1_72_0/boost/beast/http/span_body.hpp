...
private:
  static_assert(std::is_pod<T>::value, "POD requirements not met");
  ...
  */
  ...
  public:
    ...
    std::size_t put(ConstBufferSequence const &buffers, error_code &ec) {
      ...
      ec = {};
      net::buffer_copy(net::buffer(body_.data(), n), buffers);
      body_ = value_type{body_.data() + n, body_.size() - n};
      return n;
    }
  ...
  */
  ...
  public:
    ...
    boost::optional<std::pair<const_buffers_type, bool>> get(error_code &ec) {
      ec = {};
      return {{{body_.data(),
                body_.size() * sizeof(typename value_type::value_type)},
               false}};
    }
  };
#endif
