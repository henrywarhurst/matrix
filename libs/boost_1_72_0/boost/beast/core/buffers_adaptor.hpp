...
template <class MutableBufferSequence> class buffers_adaptor {
  ...
  using iter_type = buffers_iterator_type<MutableBufferSequence>;
  ...
  buffers_adaptor(buffers_adaptor const &other, std::size_t nbegin,
                  std::size_t nout, std::size_t nend);
...
public:
  ...
  /** Construct a buffers adaptor.
      ...
      transferred.
  */
  explicit buffers_adaptor(MutableBufferSequence const &buffers);
  ...
  */
  template <class... Args>
  explicit buffers_adaptor(boost::in_place_init_t, Args &&... args);
  ...
  value_type const &value() const { return bs_; }
