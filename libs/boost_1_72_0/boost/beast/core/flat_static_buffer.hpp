...
public:
  ...
  */
  flat_static_buffer_base(void *p, std::size_t n) noexcept { reset(p, n); }
  ...
  */
  ...
  template <std::size_t I = 0> void reset() noexcept {
    static_assert(I != 0, BOOST_BEAST_DEPRECATION_STRING);
  }
#endif
  ...
  std::size_t size() const noexcept { return out_ - in_; }

  /// Return the maximum number of bytes, both readable and writable, that can
  /// ever be held.
  std::size_t max_size() const noexcept { return dist(begin_, end_); }
...
protected:
  ...
  */
  BOOST_BEAST_DECL
  void reset(void *p, std::size_t n) noexcept;
...
public:
  ...
  /// Constructor
  flat_static_buffer() : flat_static_buffer_base(buf_, N) {}

  /// Assignment
  flat_static_buffer &operator=(flat_static_buffer const &);

  /// Returns the @ref flat_static_buffer_base portion of this object
  flat_static_buffer_base &base() { return *this; }

  /// Returns the @ref flat_static_buffer_base portion of this object
  flat_static_buffer_base const &base() const { return *this; }

  /// Return the maximum sum of the input and output sequence sizes.
  std::size_t constexpr max_size() const { return N; }

  /// Return the maximum sum of input and output sizes that can be held without
  /// an allocation.
  std::size_t constexpr capacity() const { return N; }
};
