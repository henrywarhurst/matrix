...
#else
detail::ostream_helper<DynamicBuffer, char, std::char_traits<char>,
...
ostream(DynamicBuffer &buffer) {
  ...
  return detail::ostream_helper<DynamicBuffer, char, std::char_traits<char>,
...
#ifdef BOOST_BEAST_ALLOW_DEPRECATED
template <class T> detail::make_printable_adaptor<T> buffers(T const &t) {
  return make_printable(t);
}
#else
template <class T> void buffers(T const &) {
  static_assert(
      sizeof(T) == 0,
