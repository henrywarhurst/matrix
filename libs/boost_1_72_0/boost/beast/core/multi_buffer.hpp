...
#endif
{
  ...
  class element
    ...
    using size_type = typename detail::allocator_traits<Allocator>::size_type;
  ...
  public:
    ...
    explicit element(size_type n) noexcept : size_(n) {}

    size_type size() const noexcept { return size_; }

    char *data() const noexcept {
      return const_cast<char *>(reinterpret_cast<char const *>(this + 1));
    }
  };

  template <bool> class readable_bytes;

  using size_type = typename detail::allocator_traits<Allocator>::size_type;

  using align_type =
      typename boost::type_with_alignment<alignof(element)>::type;
...
public:
  ...
  */
  explicit basic_multi_buffer(std::size_t limit) noexcept(default_nothrow);
  ...
  /** Constructor
      ...
      No-throw guarantee.
  */
  explicit basic_multi_buffer(Allocator const &alloc) noexcept;
  ...
  /** Constructor
      ...
      No-throw guarantee.
  */
  basic_multi_buffer(std::size_t limit, Allocator const &alloc) noexcept;
  ...
  */
  ...
  friend void swap(basic_multi_buffer<Alloc> &lhs,
  ...
  size_type size() const noexcept { return in_size_; }

  /// Return the maximum number of bytes, both readable and writable, that can
  /// ever be held.
  size_type max_size() const noexcept { return max_; }
  ...
  */
  ...
  /** Returns a mutable buffer sequence representing the readable bytes.
      ...
      @note The sequence may contain multiple contiguous memory regions.
  */
  mutable_data_type data() noexcept;
...
private:
  template <class OtherAlloc> friend class basic_multi_buffer;
