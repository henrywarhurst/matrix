...
template <class BufferSequence> class buffers_prefix_view {
  using iter_type = buffers_iterator_type<BufferSequence>;
  ...
  buffers_prefix_view(buffers_prefix_view const &other, std::size_t dist);
...
public:
  ...
  */
  template <class... Args>
  buffers_prefix_view(std::size_t size, boost::in_place_init_t,
                      Args &&... args);
...
#if !BOOST_BEAST_DOXYGEN
  std::size_t buffer_bytes_impl() const noexcept { return size_; }
#endif
};
...
buffers_prefix(std::size_t size, BufferSequence const &buffers) {
  static_assert(net::is_const_buffer_sequence<BufferSequence>::value,
...
*/
template <class BufferSequence>
buffers_type<BufferSequence> buffers_front(BufferSequence const &buffers) {
