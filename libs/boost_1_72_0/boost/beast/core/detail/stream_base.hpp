...
struct any_endpoint {
  ...
  bool operator()(Error const &, Endpoint const &) const noexcept {
    return true;
  }
};
...
struct stream_base {
  ...
  struct op_state {
    ...
    template <class... Args>
    explicit op_state(Args &&... args) : timer(std::forward<Args>(args)...) {}
  };
  ...
  public:
    ...
    explicit pending_guard(bool &b) : b_(b) {
    ...
    pending_guard(pending_guard &&other) noexcept
        : b_(other.b_), clear_(boost::exchange(other.clear_, false)) {}
