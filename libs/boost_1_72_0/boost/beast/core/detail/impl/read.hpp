...
struct dynamic_read_ops {
  ...
  class read_op : public asio::coroutine,
                  public async_base<Handler, beast::executor_type<Stream>> {
    Stream &s_;
    DynamicBuffer &b_;
    Condition cond_;
    error_code ec_;
  ...
  public:
    ...
    read_op(Handler_ &&h, Stream &s, DynamicBuffer &b, Condition_ &&cond)
        : async_base<Handler, beast::executor_type<Stream>>(
              std::forward<Handler_>(h), s.get_executor()),
          s_(s), b_(b), cond_(std::forward<Condition_>(cond)) {
      (*this)({}, 0, false);
    }
    ...
    void operator()(error_code ec, std::size_t bytes_transferred,
      ...
      BOOST_ASIO_CORO_REENTER(*this) {
        for (;;) {
          ...
          s_.async_read_some(b_.prepare(max_prepare), std::move(*this));
        ...
        if (!cont) {
          ...
          ec_ = ec;
          BOOST_ASIO_CORO_YIELD
          s_.async_read_some(b_.prepare(0), std::move(*this));
          ec = ec_;
        }
  ...
  struct run_read_op {
    ...
    void operator()(ReadHandler &&h, AsyncReadStream *s, DynamicBuffer *b,
      ...
      read_op<AsyncReadStream, DynamicBuffer,
          ...
          std::forward<ReadHandler>(h), *s, *b, std::forward<Condition>(c));
    }
  };
};
...
std::size_t read(SyncReadStream &stream, DynamicBuffer &buffer,
  ...
  static_assert(
      detail::is_invocable<CompletionCondition, void(error_code &, std::size_t,
                                                     DynamicBuffer &)>::value,
      "CompletionCondition type requirements not met");
  error_code ec;
  auto const bytes_transferred =
      detail::read(stream, buffer, std::move(cond), ec);
  if (ec)
...
template <class SyncReadStream, class DynamicBuffer, class CompletionCondition,
          class>
std::size_t read(SyncReadStream &stream, DynamicBuffer &buffer,
                 CompletionCondition cond, error_code &ec) {
  static_assert(is_sync_read_stream<SyncReadStream>::value,
                "SyncReadStream type requirements not met");
  static_assert(net::is_dynamic_buffer<DynamicBuffer>::value,
                "DynamicBuffer type requirements not met");
...
template <class AsyncReadStream, class DynamicBuffer, class CompletionCondition,
          class ReadHandler, class>
BOOST_BEAST_ASYNC_RESULT2(ReadHandler)
async_read(AsyncReadStream &stream, DynamicBuffer &buffer,
           CompletionCondition &&cond, ReadHandler &&handler) {
  ...
  return net::async_initiate<ReadHandler, void(error_code, std::size_t)>(
      typename dynamic_read_ops::run_read_op{}, handler, &stream, &buffer,
