...
template <std::size_t R> class chacha {
  ...
  void generate_block() {
    std::uint32_t constexpr constants[4] = {0x61707865, 0x3320646e, 0x79622d32,
                                            0x6b206574};
...
#define BOOST_BEAST_CHACHA_QUARTERROUND(x, a, b, c, d)                         \
  x[a] = x[a] + x[b];                                                          \
  x[d] ^= x[a];                                                                \
  x[d] = BOOST_BEAST_CHACHA_ROTL32(x[d], 16);                                  \
  x[c] = x[c] + x[d];                                                          \
  x[b] ^= x[c];                                                                \
  x[b] = BOOST_BEAST_CHACHA_ROTL32(x[b], 12);                                  \
  x[a] = x[a] + x[b];                                                          \
  x[d] ^= x[a];                                                                \
  x[d] = BOOST_BEAST_CHACHA_ROTL32(x[d], 8);                                   \
  x[c] = x[c] + x[d];                                                          \
  x[b] ^= x[c];                                                                \
  x[b] = BOOST_BEAST_CHACHA_ROTL32(x[b], 7)
