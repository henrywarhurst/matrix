...
/** A stream socket wrapper with timeouts, an executor, and a rate limit policy.
    ...
    <a
   href="http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2018/p1322r0.html">[P1322R0]
   Networking TS enhancement to enable custom I/O executors</a>.
    ...
    void process_http_2 (tcp_stream& stream, net::yield_context yield)
    {
        ...
        // Require that the read and write combined take no longer than 30
   seconds stream.expires_after(std::chrono::seconds(30));
    ...
    @code
    void do_ssl_handshake (net::ssl::stream<tcp_stream>& stream,
   ...
   Networking TS enhancement to enable custom I/O executors</a>.
*/
template <class Protocol, class Executor = net::executor,
...
public:
  ...
  */
  ...
  template <class Executor1> struct rebind_executor {
...
private:
  ...
  struct impl_type : boost::enable_shared_from_this<impl_type>,
    ...
    template <class... Args> explicit impl_type(std::false_type, Args &&...);

    template <class RatePolicy_, class... Args>
    explicit impl_type(std::true_type, RatePolicy_ &&policy, Args &&...);

    impl_type &operator=(impl_type &&) = delete;

    beast::executor_type<socket_type> ex() noexcept {
    ...
    RatePolicy const &policy() const noexcept {
      return this->boost::empty_value<RatePolicy>::get();
    }

    template <class Executor2> void on_timer(Executor2 const &ex2);
  ...
  // DEPRECATED
  template <class> friend class boost::asio::ssl::stream;
  // DEPRECATED
  using lowest_layer_type = socket_type;
  // DEPRECATED
  lowest_layer_type &lowest_layer() noexcept { return impl_->socket; }
  // DEPRECATED
...
public:
  ...
  */
#if BOOST_BEAST_DOXYGEN
  template <class... Args> explicit basic_stream(Args &&... args);
#else
  template <class Arg0, class... Args,
            class = typename std::enable_if<
                !std::is_constructible<RatePolicy, Arg0>::value>::type>
  explicit basic_stream(Arg0 &&argo, Args &&... args);
#endif
...
#if BOOST_BEAST_DOXYGEN
  template <class RatePolicy_, class... Args>
  explicit basic_stream(RatePolicy_ &&policy, Args &&... args);
#else
  template <class RatePolicy_, class Arg0, class... Args,
            class = typename std::enable_if<
                std::is_constructible<RatePolicy, RatePolicy_>::value>::type>
  basic_stream(RatePolicy_ &&policy, Arg0 &&arg, Args &&... args);
#endif
  ...
  */
  ...
  socket_type &socket() noexcept { return impl_->socket; }

  /// Return a reference to the underlying socket
  socket_type const &socket() const noexcept { return impl_->socket; }
  ...
  */
  socket_type release_socket();

  //--------------------------------------------------------------------------

  /// Returns the rate policy associated with the object
  RatePolicy &rate_policy() noexcept { return impl_->policy(); }

  /// Returns the rate policy associated with the object
  RatePolicy const &rate_policy() const noexcept { return impl_->policy(); }
  ...
  */
  executor_type get_executor() noexcept { return impl_->ex(); }
  ...
  /** Connect the stream to the specified endpoint.
      ...
      @see connect
  */
  void connect(endpoint_type const &ep) { socket().connect(ep); }
  ...
  /** Connect the stream to the specified endpoint.
      ...
      @see connect
  */
  void connect(endpoint_type const &ep, error_code &ec) {
  ...
  */
  template <class EndpointSequence
            ...
            ,
            class = typename std::enable_if<
                net::is_endpoint_sequence<EndpointSequence>::value>::type
#endif
            >
  typename Protocol::endpoint connect(EndpointSequence const &endpoints) {
  ...
  */
  ...
  /** Establishes a connection by trying each endpoint in a sequence.
      ...
      Otherwise, contains the error from the last connection attempt.
  */
  template <class Iterator> Iterator connect(Iterator begin, Iterator end) {
  ...
  */
  ...
  Iterator connect(Iterator begin, Iterator end, error_code &ec) {
  ...
  */
  ...
  /** Establishes a connection by trying each endpoint in a sequence.
     ...
     tried, and false if it should be skipped.
      ...
      Otherwise, contains the error from the last connection attempt.
  */
  template <class Iterator, class ConnectCondition>
  Iterator connect(Iterator begin, Iterator end,
                   ConnectCondition connect_condition) {
  ...
  */
  template <class Iterator, class ConnectCondition>
  Iterator connect(Iterator begin, Iterator end,
                   ConnectCondition connect_condition, error_code &ec) {
  ...
  */
  ...
  async_connect(endpoint_type const &ep,
  ...
  */
  template <class EndpointSequence,
            BOOST_ASIO_COMPLETION_TOKEN_FOR(void(
                error_code, typename Protocol::endpoint)) RangeConnectHandler =
                net::default_completion_token_t<executor_type>
#if !BOOST_BEAST_DOXYGEN
            ,
            class = typename std::enable_if<
  ...
  async_connect(EndpointSequence const &endpoints,
  ...
  */
  ...
  /** Establishes a connection by trying each endpoint in a sequence
     asynchronously.
      ...
      manner equivalent to using `net::post`.
  */
  template <class Iterator, BOOST_ASIO_COMPLETION_TOKEN_FOR(void(
                                error_code, Iterator)) IteratorConnectHandler =
                                net::default_completion_token_t<executor_type>>
  BOOST_ASIO_INITFN_RESULT_TYPE(IteratorConnectHandler,
                                void(error_code, Iterator))
  async_connect(Iterator begin, Iterator end,
  ...
  */
  template <class Iterator, class ConnectCondition,
            BOOST_ASIO_COMPLETION_TOKEN_FOR(void(error_code, Iterator))
  ...
  */
  ...
  /** Write some data asynchronously.
      ...
      This function is used to asynchronously write data to the underlying
     socket.
     ...
     requested number of bytes. Consider using the function `net::async_write`
     if you need to ensure that the requested amount of data is sent before the
     asynchronous operation completes.
  */
  template <class ConstBufferSequence,
