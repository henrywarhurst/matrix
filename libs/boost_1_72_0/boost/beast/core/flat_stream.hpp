...
/** Stream wrapper to improve write performance.
    ...
    @code
        websocket::stream<flat_stream<net::ssl::stream<ip::tcp::socket>>>
   ws{ioc, ctx};
    @endcode
...
#endif
{
  ...
  std::size_t stack_write_some(std::size_t size,
...
public:
  ...
  */
  template <class... Args> explicit flat_stream(Args &&... args);
  ...
  */
  ...
  /** Get a reference to the next layer
      ...
      stream layers.
  */
  next_layer_type const &next_layer() const noexcept { return stream_; }
  ...
  */
  ...
  /** Start an asynchronous read.
     ...
     from the stream. The function call always returns immediately.
      ...
      @note The `read_some` operation may not read all of the requested number
     of bytes. Consider using the function `net::async_read` if you need to
