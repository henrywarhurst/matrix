...
template <class Buffers> class make_printable_adaptor {
  Buffers b_;

public:
  explicit make_printable_adaptor(Buffers const &b) : b_(b) {}
...
std::ostream &operator<<(std::ostream &os,
  ...
  for (auto it = net::buffer_sequence_begin(v.b_),
            end = net::buffer_sequence_end(v.b_);
       it != end; ++it) {
    net::const_buffer cb = *it;
    os.write(static_cast<char const *>(cb.data()), cb.size());
  }
  return os;
}
...
make_printable(ConstBufferSequence const &buffers) {
  ...
  return detail::make_printable_adaptor<ConstBufferSequence>{buffers};
}
