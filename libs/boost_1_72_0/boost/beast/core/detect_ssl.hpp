...
/** Return `true` if the buffer contains a TLS Protocol client_hello message.
    ...
    <a href="https://tools.ietf.org/html/rfc2246#section-7.4">7.4. Handshake
   protocol</a> (RFC2246: The TLS Protocol)
*/
template <class ConstBufferSequence>
boost::tribool is_tls_client_hello(ConstBufferSequence const &buffers);
...
boost::tribool is_tls_client_hello(ConstBufferSequence const &buffers) {
  ...
  */
  ...
  unsigned char buf[9];
  auto const n =
      net::buffer_copy(net::mutable_buffer(buf, sizeof(buf)), buffers);
  ...
  std::uint32_t const size = (buf[6] << 16) + (buf[7] << 8) + buf[8];
...
bool detect_ssl(SyncReadStream &stream, DynamicBuffer &buffer, error_code &ec) {
  ...
  for (;;) {
    ...
    std::size_t const bytes_transferred =
        stream.read_some(buffer.prepare(beast::read_size(buffer, 1536)), ec);
...
async_detect_ssl(AsyncReadStream &stream, DynamicBuffer &buffer,
    ...
    ->typename net::async_result<
        typename std::decay<CompletionToken>::type, /*< `async_result`
                                                       customizes the return
                                                       value based on the
                                                       completion token >*/
        void(error_code, bool)>::return_type; /*< This is the signature for the
                                                 completion handler >*/
//]
...
struct run_detect_ssl_op {
  ...
  template <class DetectHandler, class AsyncReadStream, class DynamicBuffer>
  void operator()(DetectHandler &&h,
                  AsyncReadStream *s, // references are passed as pointers
                  DynamicBuffer &b) {
    detect_ssl_op<typename std::decay<DetectHandler>::type, AsyncReadStream,
                  DynamicBuffer>(std::forward<DetectHandler>(h), *s, b);
  }
};
...
async_detect_ssl(AsyncReadStream &stream, DynamicBuffer &buffer,
  ...
  return net::async_initiate<CompletionToken, void(error_code, bool)>(
      detail::run_detect_ssl_op{}, token,
...
class detect_ssl_op
    : public boost::asio::coroutine,
      public async_base<DetectHandler, executor_type<AsyncReadStream>> {
...
public:
  ...
  detect_ssl_op(DetectHandler_ &&handler, AsyncReadStream &stream,
                DynamicBuffer &buffer)
      : beast::async_base<DetectHandler, beast::executor_type<AsyncReadStream>>(
            std::forward<DetectHandler_>(handler), stream.get_executor()),
...
// detect_ssl_op is callable with the signature void(error_code,
// bytes_transferred), allowing `*this` to be used as a ReadHandler
//
...
void detect_ssl_op<AsyncStream, DynamicBuffer, Handler>::operator()(
    error_code ec, std::size_t bytes_transferred, bool cont) {
  namespace beast = boost::beast;
  ...
  reenter(*this) {
    ...
    for (;;) {
      ...
      yield stream_.async_read_some(buffer_.prepare(read_size(buffer_, 1536)),
                                    std::move(*this));
