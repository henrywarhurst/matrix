...
/** Return the lowest layer in a stack of stream layers.
    ...
    calling `next_layer()` and accessing its members. This is necessary when it
   is desired to access functionality in the next layer which is not available
    ...
    @param t The layer in a stack of layered objects for which the lowest layer
   is returned.

    @see close_socket, lowest_layer_type
*/
template <class T> lowest_layer_type<T> &get_lowest_layer(T &t) noexcept {
  return detail::get_lowest_layer_impl(t, detail::has_next_layer<T>{});
}
...
#if BOOST_BEAST_DOXYGEN
template <class T> using executor_type = __see_below__;
#elif BOOST_WORKAROUND(BOOST_GCC, < 40900)
template <class T>
using executor_type = typename std::decay<T>::type::executor_type;
#else
template <class T>
...
/** Determine if `T` has the `get_executor` member function.
    ...
    static_assert(has_get_executor<stream>::value, "Missing get_executor
   member");
    @endcode
*/
#if BOOST_BEAST_DOXYGEN
template <class T> using has_get_executor = __see_below__;
#else
...
*/
#if BOOST_BEAST_DOXYGEN
template <class T> using is_sync_read_stream = __see_below__;
#else
...
*/
#if BOOST_BEAST_DOXYGEN
template <class T> using is_sync_write_stream = __see_below__;
#else
template <class T, class = void>
...
*/
#if BOOST_BEAST_DOXYGEN
template <class T> using is_sync_stream = __see_below__;
#else
template <class T>
using is_sync_stream =
    std::integral_constant<bool, is_sync_read_stream<T>::value &&
                                     is_sync_write_stream<T>::value>;
#endif
...
/** Determine if `T` meets the requirements of <em>AsyncReadStream</em>.
    ...
    @code
        template<class AsyncReadStream>
        typename
   std::enable_if<is_async_read_stream<AsyncReadStream>::value>::type
        f(AsyncReadStream& stream);
    @endcode
*/
#if BOOST_BEAST_DOXYGEN
template <class T> using is_async_read_stream = __see_below__;
#else
template <class T, class = void>
...
struct is_async_read_stream<
    T, boost::void_t<decltype(std::declval<T &>().async_read_some(
           std::declval<detail::MutableBufferSequence>(),
           std::declval<detail::ReadHandler>()))>>
    : std::integral_constant<bool, has_get_executor<T>::value> {};
#endif
...
/** Determine if `T` meets the requirements of <em>AsyncWriteStream</em>.
    ...
    template<class AsyncWriteStream>
    typename
   std::enable_if<is_async_write_stream<AsyncWriteStream>::value>::type
    f(AsyncWriteStream& stream);
    @endcode
*/
#if BOOST_BEAST_DOXYGEN
template <class T> using is_async_write_stream = __see_below__;
#else
template <class T, class = void>
...
struct is_async_write_stream<
    T, boost::void_t<decltype(std::declval<T &>().async_write_some(
           std::declval<detail::ConstBufferSequence>(),
...
*/
#if BOOST_BEAST_DOXYGEN
template <class T> using is_async_stream = __see_below__;
#else
template <class T>
using is_async_stream =
    std::integral_constant<bool, is_async_read_stream<T>::value &&
                                     is_async_write_stream<T>::value>;
#endif
...
void beast_close_socket(net::basic_socket<Protocol, Executor> &sock) {
