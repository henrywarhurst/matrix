...
/** Close status codes.
    ...
    @see <a href="https://tools.ietf.org/html/rfc6455#section-7.4.1">RFC
   6455 7.4.1 Defined Status Codes</a>
*/
...
struct close_reason {
  ...
  */
  ...
  close_reason(std::uint16_t code_) : code(code_) {}

  /// Construct from a reason string. code is @ref close_code::normal.
  close_reason(string_view s) : code(close_code::normal), reason(s) {}

  /// Construct from a reason string literal. code is @ref close_code::normal.
  close_reason(char const *s) : code(close_code::normal), reason(s) {}

  /// Construct from a close code and reason string.
  close_reason(close_code code_, string_view s) : code(code_), reason(s) {}

  /// Returns `true` if a code was specified
  operator bool() const { return code != close_code::none; }
};
