...
void impl_base<true>::build_response_pmd(
...
void impl_base<false>::build_response_pmd(
    ...
    http::request<Body, http::basic_fields<Allocator>> const &) {}

} // namespace detail

template <class NextLayer, bool deflateSupported>
template <class Body, class Allocator, class Decorator>
response_type stream<NextLayer, deflateSupported>::impl_type::build_response(
    http::request<Body, http::basic_fields<Allocator>> const &req,
    Decorator const &decorator, error_code &result) {
  auto const decorate = [this, &decorator](response_type &res) {
    decorator_opt(res);
    decorator(res);
    if (!res.count(http::field::server))
      res.set(http::field::server, string_view(BOOST_BEAST_VERSION_STRING));
  };
  auto err = [&](error e) {
    result = e;
    response_type res;
...
public:
  template <class Handler_, class Body, class Allocator, class Decorator>
  response_op(Handler_ &&h, boost::shared_ptr<impl_type> const &sp,
              http::request<Body, http::basic_fields<Allocator>> const &req,
              Decorator const &decorator, bool cont = false)
      : stable_async_base<Handler, beast::executor_type<stream>>(
            std::forward<Handler_>(h), sp->stream().get_executor()),
        wp_(sp), res_(beast::allocate_stable<response_type>(
                     *this, sp->build_response(req, decorator, result_))) {
    (*this)({}, 0, cont);
  }
  ...
  void operator()(error_code ec = {}, std::size_t bytes_transferred = 0,
    ...
    BOOST_ASIO_CORO_REENTER(*this) {
      ...
      http::async_write(impl.stream(), res_, std::move(*this));
...
public:
  ...
  accept_op(Handler_ &&h, boost::shared_ptr<impl_type> const &sp,
            Decorator const &decorator, Buffers const &buffers)
      : stable_async_base<Handler, beast::executor_type<stream>>(
            std::forward<Handler_>(h), sp->stream().get_executor()),
        wp_(sp),
        p_(beast::allocate_stable<http::request_parser<http::empty_body>>(
            *this)),
        d_(decorator) {
    auto &impl = *sp;
    error_code ec;
    auto const mb = beast::detail::dynamic_buffer_prepare(
        impl.rd_buf, buffer_bytes(buffers), ec, error::buffer_overflow);
    if (!ec)
      impl.rd_buf.commit(net::buffer_copy(*mb, buffers));
    (*this)(ec);
  }
  ...
  void operator()(error_code ec = {}, std::size_t bytes_transferred = 0,
    ...
    BOOST_ASIO_CORO_REENTER(*this) {
      ...
      BOOST_ASIO_CORO_YIELD
      http::async_read(impl.stream(), impl.rd_buf, p_, std::move(*this));
...

      {
        ...
        response_op<Handler>(this->release_handler(), sp, req, decorator, true);
        return;
      }
...
template <class NextLayer, bool deflateSupported>
struct stream<NextLayer, deflateSupported>::run_response_op {
  template <class AcceptHandler, class Body, class Allocator, class Decorator>
  void operator()(AcceptHandler &&h, boost::shared_ptr<impl_type> const &sp,
                  http::request<Body, http::basic_fields<Allocator>> const *m,
                  Decorator const &d) {
    ...
    static_assert(
        beast::detail::is_invocable<AcceptHandler, void(error_code)>::value,
        "AcceptHandler type requirements not met");

    response_op<typename std::decay<AcceptHandler>::type>(
        std::forward<AcceptHandler>(h), sp, *m, d);
  }
};

template <class NextLayer, bool deflateSupported>
struct stream<NextLayer, deflateSupported>::run_accept_op {
  template <class AcceptHandler, class Decorator, class Buffers>
  void operator()(AcceptHandler &&h, boost::shared_ptr<impl_type> const &sp,
                  Decorator const &d, Buffers const &b) {
    // If you get an error on the following line it means
    // that your handler does not meet the documented type
    // requirements for the handler.
    ...
    accept_op<typename std::decay<AcceptHandler>::type, Decorator>(
        std::forward<AcceptHandler>(h), sp, d, b);
  }
};
...
template <class NextLayer, bool deflateSupported>
template <class Body, class Allocator, class Decorator>
void stream<NextLayer, deflateSupported>::do_accept(
    http::request<Body, http::basic_fields<Allocator>> const &req,
...
void stream<NextLayer, deflateSupported>::do_accept(Buffers const &buffers,
                                                    Decorator const &decorator,
                                                    error_code &ec) {
  impl_->reset();
  auto const mb = beast::detail::dynamic_buffer_prepare(
      impl_->rd_buf, buffer_bytes(buffers), ec, error::buffer_overflow);
  if (ec)
    return;
...
template <class NextLayer, bool deflateSupported>
void stream<NextLayer, deflateSupported>::accept() {
...
template <class NextLayer, bool deflateSupported>
void stream<NextLayer, deflateSupported>::accept(error_code &ec) {
  static_assert(is_sync_stream<next_layer_type>::value,
                "SyncStream type requirements not met");
  do_accept(net::const_buffer{}, &default_decorate_res, ec);
}
...
stream<NextLayer, deflateSupported>::accept(
...
typename std::enable_if<
    !http::detail::is_header<ConstBufferSequence>::value>::type
stream<NextLayer, deflateSupported>::accept(ConstBufferSequence const &buffers,
                                            error_code &ec) {
  static_assert(is_sync_stream<next_layer_type>::value,
                "SyncStream type requirements not met");
  static_assert(net::is_const_buffer_sequence<ConstBufferSequence>::value,
...
stream<NextLayer, deflateSupported>::async_accept(AcceptHandler &&handler) {
  static_assert(is_async_stream<next_layer_type>::value,
                "AsyncStream type requirements not met");
  impl_->reset();
  return net::async_initiate<AcceptHandler, void(error_code)>(
      run_accept_op{}, handler, impl_, &default_decorate_res,
      net::const_buffer{});
}
...
BOOST_BEAST_ASYNC_RESULT1(AcceptHandler)
stream<NextLayer, deflateSupported>::async_accept_ex(
    ResponseDecorator const &decorator, AcceptHandler &&handler) {
  static_assert(is_async_stream<next_layer_type>::value,
                "AsyncStream type requirements not met");
  static_assert(detail::is_response_decorator<ResponseDecorator>::value,
                "ResponseDecorator requirements not met");
  impl_->reset();
  return net::async_initiate<AcceptHandler, void(error_code)>(
      run_accept_op{}, handler, impl_, decorator, net::const_buffer{});
}

template <class NextLayer, bool deflateSupported>
template <class ConstBufferSequence, class AcceptHandler>
BOOST_BEAST_ASYNC_RESULT1(AcceptHandler)
stream<NextLayer, deflateSupported>::async_accept(
  ...
  impl_->reset();
  return net::async_initiate<AcceptHandler, void(error_code)>(
      run_accept_op{}, handler, impl_, &default_decorate_res, buffers);
}
...
stream<NextLayer, deflateSupported>::async_accept_ex(
  ...
  impl_->reset();
  return net::async_initiate<AcceptHandler, void(error_code)>(
      run_accept_op{}, handler, impl_, decorator, buffers);
}
...
stream<NextLayer, deflateSupported>::async_accept(
  ...
  impl_->reset();
  return net::async_initiate<AcceptHandler, void(error_code)>(
      run_response_op{}, handler, impl_, &req, &default_decorate_res);
}
...
stream<NextLayer, deflateSupported>::async_accept_ex(
  ...
  impl_->reset();
  return net::async_initiate<AcceptHandler, void(error_code)>(
      run_response_op{}, handler, impl_, &req, decorator);
}
...
void stream<NextLayer, deflateSupported>::accept_ex(
    ResponseDecorator const &decorator) {
#ifndef BOOST_BEAST_ALLOW_DEPRECATED
  static_assert(sizeof(ResponseDecorator) == 0, BOOST_BEAST_DEPRECATION_STRING);
#endif
  static_assert(is_sync_stream<next_layer_type>::value,
                "SyncStream type requirements not met");
  static_assert(detail::is_response_decorator<ResponseDecorator>::value,
...
void stream<NextLayer, deflateSupported>::accept_ex(
  ...
  static_assert(detail::is_response_decorator<ResponseDecorator>::value,
                "ResponseDecorator requirements not met");
  do_accept(net::const_buffer{}, decorator, ec);
}
...
typename std::enable_if<
    !http::detail::is_header<ConstBufferSequence>::value>::type
stream<NextLayer, deflateSupported>::accept_ex(
    ConstBufferSequence const &buffers, ResponseDecorator const &decorator) {
#ifndef BOOST_BEAST_ALLOW_DEPRECATED
  static_assert(sizeof(ResponseDecorator) == 0, BOOST_BEAST_DEPRECATION_STRING);
#endif
