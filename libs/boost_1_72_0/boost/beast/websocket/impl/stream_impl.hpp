...
struct stream<NextLayer, deflateSupported>::impl_type
  ...
  boost::weak_ptr<impl_type> weak_from_this() {
    return boost::static_pointer_cast<impl_type>(
        this->detail::service::impl_type::shared_from_this());
  }

  boost::shared_ptr<impl_type> shared_this() {
    return boost::static_pointer_cast<impl_type>(
        this->detail::service::impl_type::shared_from_this());
  }
  ...
  static_buffer<+tcp_frame_size> rd_buf; // buffer for reads
  detail::opcode rd_op /* current message binary or text */ =
      detail::opcode::text;
  ...
  std::unique_ptr<std::uint8_t[]> wr_buf; // write buffer
  ...
  template <class... Args>
  impl_type(Args &&... args)
      : boost::empty_value<NextLayer>(boost::empty_init_t{},
                                      std::forward<Args>(args)...),
        detail::service::impl_type(this->boost::empty_value<NextLayer>::get()
                                       .get_executor()
                                       .context()),
  ...
  void begin_msg() {
    wr_frag = wr_frag_opt;
    wr_compress = this->pmd_enabled() && wr_compress_opt;

    // Maintain the write buffer
    if (this->pmd_enabled() || role == role_type::client) {
      if (!wr_buf || wr_buf_size != wr_buf_opt) {
        wr_buf_size = wr_buf_opt;
        wr_buf = boost::make_unique_noinit<std::uint8_t[]>(wr_buf_size);
      }
    } else {
      wr_buf_size = wr_buf_opt;
      wr_buf.reset();
  ...
  request_type build_request(detail::sec_ws_key_type &key, string_view host,
                             string_view target, Decorator const &decorator);

  void on_response(response_type const &res, detail::sec_ws_key_type const &key,
                   error_code &ec);

  template <class Body, class Allocator, class Decorator>
  response_type
  build_response(http::request<Body, http::basic_fields<Allocator>> const &req,
                 Decorator const &decorator, error_code &result);

  // Attempt to read a complete frame header.
  // Returns `false` if more bytes are needed
  template <class DynamicBuffer>
  bool parse_fh(detail::frame_header &fh, DynamicBuffer &b, error_code &ec);
  ...
  template <class DynamicBuffer>
  std::size_t read_size_hint_db(DynamicBuffer &buffer) const {
    auto const initial_size =
        (std::min)(+tcp_frame_size, buffer.max_size() - buffer.size());
    if (initial_size == 0)
      return 1; // buffer is full
    return this->read_size_hint_pmd(initial_size, rd_done, rd_remain, rd_fh);
  }

  template <class DynamicBuffer>
  void write_ping(DynamicBuffer &db, detail::opcode code,
                  ping_data const &data);

  template <class DynamicBuffer>
  void write_close(DynamicBuffer &db, close_reason const &cr);

  //--------------------------------------------------------------------------

  void set_option(timeout const &opt) {
    if (opt.handshake_timeout == none() && opt.idle_timeout == none()) {
      // turn timer off
      timer.cancel();
      timer.expires_at(never());
    }
  ...
  // completion.
  //
  bool check_stop_now(error_code &ec) {
    ...
    if (status_ == status::closed || status_ == status::failed) {
  ...
  void reset_idle() { idle_counter = 0; }

  // Maintain the expiration timer
  template <class Executor> void update_timer(Executor const &ex) {
    switch (status_) {
    case status::handshake:
      BOOST_ASSERT(idle_counter == 0);
      if (!is_timer_set() && timeout_opt.handshake_timeout != none()) {
        timer.expires_after(timeout_opt.handshake_timeout);
        timer.async_wait(timeout_handler<Executor>(ex, this->weak_from_this()));
      }
      break;
    ...
    case status::open:
      if (timeout_opt.idle_timeout != none()) {
        ...
        if (timeout_opt.keep_alive_pings)
          timer.expires_after(timeout_opt.idle_timeout / 2);
        else
          timer.expires_after(timeout_opt.idle_timeout);
        timer.async_wait(timeout_handler<Executor>(ex, this->weak_from_this()));
      } else {
        timer.cancel();
        timer.expires_at(never());
...
private:
  bool is_timer_set() const { return timer.expiry() != never(); }
  ...
  public:
    timeout_handler(Executor const &ex, boost::weak_ptr<impl_type> &&wp)
        : boost::empty_value<Executor>(boost::empty_init_t{}, ex),
          wp_(std::move(wp)) {}
    ...
    void operator()(error_code ec) {
      ...
      case status::open:
        ...
        if (impl.timeout_opt.keep_alive_pings && impl.idle_counter < 1) {
          idle_ping_op<Executor>(sp, get_executor());

          ++impl.idle_counter;
          impl.timer.expires_after(impl.timeout_opt.idle_timeout / 2);
...
template <class NextLayer, bool deflateSupported>
template <class Decorator>
request_type stream<NextLayer, deflateSupported>::impl_type::build_request(
    detail::sec_ws_key_type &key, string_view host, string_view target,
...
template <class NextLayer, bool deflateSupported>
void stream<NextLayer, deflateSupported>::impl_type::on_response(
    response_type const &res, detail::sec_ws_key_type const &key,
    error_code &ec) {
  auto const err = [&](error e) { ec = e; };
...
template <class NextLayer, bool deflateSupported>
template <class DynamicBuffer>
bool stream<NextLayer, deflateSupported>::impl_type::parse_fh(
    detail::frame_header &fh, DynamicBuffer &b, error_code &ec) {
  if (buffer_bytes(b.data()) < 2) {
    // need more bytes
    ec = {};
    return false;
  }
  buffers_suffix<typename DynamicBuffer::const_buffers_type> cb{b.data()};
  std::size_t need;
  {
    std::uint8_t tmp[2];
    ...
    fh.op = static_cast<detail::opcode>(tmp[0] & 0x0f);
    fh.fin = (tmp[0] & 0x80) != 0;
  ...
  case detail::opcode::text:
    ...
    if (fh.rsv2 || fh.rsv3 || !this->rd_deflated(fh.rsv1)) {
  ...
  if (detail::is_control(fh.op) && buffer_bytes(cb) < need + fh.len) {
  ...
  case 126: {
    ...
    cb.consume(
        net::buffer_copy(net::mutable_buffer(&len_be, sizeof(len_be)), cb));
  ...
  if (fh.mask) {
    ...
    cb.consume(
        net::buffer_copy(net::mutable_buffer(&key_le, sizeof(key_le)), cb));
  ...
  if (!detail::is_control(fh.op)) {
    if (fh.op != detail::opcode::cont) {
      ...
      if (rd_size > (std::numeric_limits<std::uint64_t>::max)() - fh.len) {
    ...
    if (!this->rd_deflated()) {
      if (rd_msg_max &&
          beast::detail::sum_exceeds(rd_size, fh.len, rd_msg_max)) {
        // message size exceeds configured limit
        ec = error::message_too_big;
        return false;
      }
...
template <class NextLayer, bool deflateSupported>
template <class DynamicBuffer>
void stream<NextLayer, deflateSupported>::impl_type::write_ping(
    DynamicBuffer &db, detail::opcode code, ping_data const &data) {
  ...
  net::buffer_copy(mb, net::const_buffer(data.data(), data.size()));
  if (fh.mask)
...
template <class NextLayer, bool deflateSupported>
template <class DynamicBuffer>
void stream<NextLayer, deflateSupported>::impl_type::write_close(
  ...
  fh.rsv2 = false;
  fh.rsv3 = false;
  fh.len = cr.code == close_code::none ? 0 : 2 + cr.reason.size();
  ...
  if (cr.code != close_code::none) {
      ...
      detail::prepare_key(key, fh.key);
    {
      ...
      net::buffer_copy(mb, net::const_buffer(&code_be, sizeof(code_be)));
      if (fh.mask)
        detail::mask_inplace(mb, key);
      db.commit(2);
    }
    if (!cr.reason.empty()) {
      ...
      net::buffer_copy(mb,
                       net::const_buffer(cr.reason.data(), cr.reason.size()));
      if (fh.mask)
        detail::mask_inplace(mb, key);
