...
class stream<NextLayer, deflateSupported>::read_some_op
    : public beast::async_base<Handler, beast::executor_type<stream>>,
...
public:
  ...
  read_some_op(Handler_ &&h, boost::shared_ptr<impl_type> const &sp,
               MutableBufferSequence const &bs)
      : async_base<Handler, beast::executor_type<stream>>(
            std::forward<Handler_>(h), sp->stream().get_executor()),
        wp_(sp), bs_(bs), cb_(bs), code_(close_code::none) {
    (*this)({}, 0, false);
  }
  ...
  void operator()(error_code ec = {}, std::size_t bytes_transferred = 0,
    ...
    acquire_read_lock:
      ...
      do_suspend:
        ...
        if (impl.status_ == status::closed || impl.status_ == status::failed) {
          ec = net::error::operation_aborted;
          goto upcall;
        }
      }
    ...
    loop:
      ...
      //
      if (impl.rd_remain == 0 && (!impl.rd_fh.fin || impl.rd_done)) {
        // Read frame header
        while (!impl.parse_fh(impl.rd_fh, impl.rd_buf, result_)) {
          if (result_) {
          ...
          impl.stream().async_read_some(
              impl.rd_buf.prepare(
                  read_size(impl.rd_buf, impl.rd_buf.max_size())),
              std::move(*this));
          BOOST_ASSERT(impl.rd_block.is_locked(this));
        ...
        if (impl.rd_fh.len > 0 && impl.rd_fh.mask)
          detail::mask_inplace(
              buffers_prefix(clamp(impl.rd_fh.len), impl.rd_buf.data()),
              impl.rd_key);
        if (detail::is_control(impl.rd_fh.op)) {
          ...
          if (impl.rd_fh.op == detail::opcode::ping) {
            ...
            }
            {
              ...
              impl.rd_fb.clear();
              impl.template write_ping<flat_static_buffer_base>(
                  impl.rd_fb, detail::opcode::pong, payload);
            }
            ...
            impl.op_close.maybe_invoke() || impl.op_idle_ping.maybe_invoke() ||
                impl.op_ping.maybe_invoke() || impl.op_wr.maybe_invoke();
      ...
      if (!impl.rd_deflated()) {
        if (impl.rd_remain > 0) {
          if (impl.rd_buf.size() == 0 &&
              impl.rd_buf.max_size() >
                  (std::min)(clamp(impl.rd_remain), buffer_bytes(cb_))) {
            ...
            if (impl.rd_fh.mask)
              detail::mask_inplace(
                  buffers_prefix(clamp(impl.rd_remain), impl.rd_buf.data()),
                  impl.rd_key);
          }
          if (impl.rd_buf.size() > 0) {
            // Copy from the read buffer.
            // The mask was already applied.
            bytes_transferred = net::buffer_copy(cb_, impl.rd_buf.data(),
                                                 clamp(impl.rd_remain));
            auto const mb = buffers_prefix(bytes_transferred, cb_);
            ...
            BOOST_ASSERT(
                buffer_bytes(buffers_prefix(clamp(impl.rd_remain), cb_)) > 0);
            BOOST_ASIO_CORO_YIELD
            impl.stream().async_read_some(
                buffers_prefix(clamp(impl.rd_remain), cb_), std::move(*this));
            if (impl.check_stop_now(ec))
              goto upcall;
            impl.reset_idle();
        ...
        while (buffer_bytes(cb_) > 0) {
          if (impl.rd_remain > 0 && impl.rd_buf.size() == 0 && !did_read_) {
            // read new
            BOOST_ASIO_CORO_YIELD
            impl.stream().async_read_some(
                impl.rd_buf.prepare(
          ...
          if (impl.rd_remain > 0) {
            if (impl.rd_buf.size() > 0) {
              // use what's there
              auto const in = buffers_prefix(clamp(impl.rd_remain),
                                             buffers_front(impl.rd_buf.data()));
              zs.avail_in = in.size();
              zs.next_in = in.data();
            } else {
              break;
            ...
            std::uint8_t constexpr empty_block[4] = {0x00, 0x00, 0xff, 0xff};
          ...
          if (impl.check_stop_now(ec))
            goto upcall;
          if (impl.rd_msg_max &&
              beast::detail::sum_exceeds(impl.rd_size, zs.total_out,
                                         impl.rd_msg_max)) {
            // _Fail the WebSocket Connection_
            code_ = close_code::too_big;
        ...
        if (impl.rd_op == detail::opcode::text) {
          // check utf8
          if (!impl.rd_utf8.write(buffers_prefix(bytes_written_, bs_)) ||
    ...
    close:
      ...
      if (!impl.wr_close) {
        ...
        impl.rd_fb.clear();
        impl.template write_close<flat_static_buffer_base>(impl.rd_fb, code_);
...
public:
  template <class Handler_>
  read_op(Handler_ &&h, boost::shared_ptr<impl_type> const &sp,
          DynamicBuffer &b, std::size_t limit, bool some)
      : async_base<Handler, beast::executor_type<stream>>(
            std::forward<Handler_>(h), sp->stream().get_executor()),
        wp_(sp), b_(b),
        limit_(limit ? limit : (std::numeric_limits<std::size_t>::max)()),
        some_(some) {
    (*this)({}, 0, false);
  }
      ...
      do {
        ...
        BOOST_ASIO_CORO_YIELD {
          auto mb = beast::detail::dynamic_buffer_prepare(
              b_, clamp(impl.read_size_hint_db(b_), limit_), ec,
              error::buffer_overflow);
          if (impl.check_stop_now(ec))
            goto upcall;
          read_some_op<read_op, mutable_buffers_type>(std::move(*this), sp,
                                                      *mb);
        }
...
template <class NextLayer, bool deflateSupported>
struct stream<NextLayer, deflateSupported>::run_read_some_op {
  template <class ReadHandler, class MutableBufferSequence>
  void operator()(ReadHandler &&h, boost::shared_ptr<impl_type> const &sp,
    ...
    read_some_op<typename std::decay<ReadHandler>::type, MutableBufferSequence>(
        std::forward<ReadHandler>(h), sp, b);
  }
};

template <class NextLayer, bool deflateSupported>
struct stream<NextLayer, deflateSupported>::run_read_op {
  template <class ReadHandler, class DynamicBuffer>
  void operator()(ReadHandler &&h, boost::shared_ptr<impl_type> const &sp,
                  DynamicBuffer *b, std::size_t limit, bool some) {
    // If you get an error on the following line it means
    // that your handler does not meet the documented type
    ...
    read_op<typename std::decay<ReadHandler>::type, DynamicBuffer>(
        std::forward<ReadHandler>(h), sp, *b, limit, some);
  }
};
...
std::size_t stream<NextLayer, deflateSupported>::read(DynamicBuffer &buffer) {
...
stream<NextLayer, deflateSupported>::async_read(DynamicBuffer &buffer,
                                                ReadHandler &&handler) {
  ...
  return net::async_initiate<ReadHandler, void(error_code, std::size_t)>(
      run_read_op{}, handler, impl_, &buffer, 0, false);
}
...
template <class DynamicBuffer>
std::size_t
stream<NextLayer, deflateSupported>::read_some(DynamicBuffer &buffer,
                                               std::size_t limit) {
  static_assert(is_sync_stream<next_layer_type>::value,
                "SyncStream type requirements not met");
  static_assert(net::is_dynamic_buffer<DynamicBuffer>::value,
                "DynamicBuffer type requirements not met");
  error_code ec;
  auto const bytes_written = read_some(buffer, limit, ec);
  if (ec)
    BOOST_THROW_EXCEPTION(system_error{ec});
  return bytes_written;
}
...
template <class NextLayer, bool deflateSupported>
template <class DynamicBuffer>
std::size_t stream<NextLayer, deflateSupported>::read_some(
    DynamicBuffer &buffer, std::size_t limit, error_code &ec) {
  static_assert(is_sync_stream<next_layer_type>::value,
                "SyncStream type requirements not met");
  static_assert(net::is_dynamic_buffer<DynamicBuffer>::value,
                "DynamicBuffer type requirements not met");
  ...
  auto const size = clamp(read_size_hint(buffer), limit);
  BOOST_ASSERT(size > 0);
  auto mb = beast::detail::dynamic_buffer_prepare(buffer, size, ec,
                                                  error::buffer_overflow);
...
template <class DynamicBuffer, class ReadHandler>
BOOST_BEAST_ASYNC_RESULT2(ReadHandler)
stream<NextLayer, deflateSupported>::async_read_some(DynamicBuffer &buffer,
                                                     std::size_t limit,
                                                     ReadHandler &&handler) {
  static_assert(is_async_stream<next_layer_type>::value,
                "AsyncStream type requirements not met");
  static_assert(net::is_dynamic_buffer<DynamicBuffer>::value,
                "DynamicBuffer type requirements not met");
  return net::async_initiate<ReadHandler, void(error_code, std::size_t)>(
      run_read_op{}, handler, impl_, &buffer, limit, true);
}
...
template <class MutableBufferSequence>
std::size_t stream<NextLayer, deflateSupported>::read_some(
    MutableBufferSequence const &buffers, error_code &ec) {
  static_assert(is_sync_stream<next_layer_type>::value,
                "SyncStream type requirements not met");
  static_assert(net::is_mutable_buffer_sequence<MutableBufferSequence>::value,
                "MutableBufferSequence type requirements not met");
...
loop:
  ...
  if (impl.rd_remain == 0 && (!impl.rd_fh.fin || impl.rd_done)) {
    ...
    while (!impl.parse_fh(impl.rd_fh, impl.rd_buf, result)) {
      ...
      auto const bytes_transferred = impl.stream().read_some(
          impl.rd_buf.prepare(read_size(impl.rd_buf, impl.rd_buf.max_size())),
          ec);
      impl.rd_buf.commit(bytes_transferred);
      if (impl.check_stop_now(ec))
        return bytes_written;
    ...
    if (detail::is_control(impl.rd_fh.op)) {
      ...
      auto const b = buffers_prefix(clamp(impl.rd_fh.len), impl.rd_buf.data());
      auto const len = buffer_bytes(b);
      BOOST_ASSERT(len == impl.rd_fh.len);
      ...
      BOOST_ASSERT(impl.rd_fh.op == detail::opcode::close);
      {
        ...
        if (result) {
          // _Fail the WebSocket Connection_
          do_fail(close_code::protocol_error, result, ec);
          return bytes_written;
        }
        impl.cr = cr;
        impl.rd_buf.consume(len);
        ...
        BOOST_ASSERT(!impl.wr_close);
        // _Start the WebSocket Closing Handshake_
        do_fail(cr.code == close_code::none ? close_code::normal
                                            : static_cast<close_code>(cr.code),
                error::closed, ec);
        return bytes_written;
      }
  ...
  if (!impl.rd_deflated()) {
    if (impl.rd_remain > 0) {
      if (impl.rd_buf.size() == 0 &&
          impl.rd_buf.max_size() >
              (std::min)(clamp(impl.rd_remain), buffer_bytes(buffers))) {
        // Fill the read buffer first, otherwise we
        // get fewer bytes at the cost of one I/O.
      ...
      if (impl.rd_buf.size() > 0) {
        ...
        auto const bytes_transferred = net::buffer_copy(
            buffers, impl.rd_buf.data(), clamp(impl.rd_remain));
        auto const mb = buffers_prefix(bytes_transferred, buffers);
        impl.rd_remain -= bytes_transferred;
        if (impl.rd_op == detail::opcode::text) {
          if (!impl.rd_utf8.write(mb) ||
              (impl.rd_remain == 0 && impl.rd_fh.fin &&
               !impl.rd_utf8.finish())) {
            // _Fail the WebSocket Connection_
            do_fail(close_code::bad_payload, error::bad_frame_payload, ec);
            return bytes_written;
          }
        }
        ...
        BOOST_ASSERT(
            buffer_bytes(buffers_prefix(clamp(impl.rd_remain), buffers)) > 0);
        auto const bytes_transferred = impl.stream().read_some(
            buffers_prefix(clamp(impl.rd_remain), buffers), ec);
    ...
    while (buffer_bytes(cb) > 0) {
      ...
      if (impl.rd_remain > 0) {
        if (impl.rd_buf.size() > 0) {
          ...
          auto const in = buffers_prefix(
              clamp(impl.rd_remain), beast::buffers_front(impl.rd_buf.data()));
          zs.avail_in = in.size();
          zs.next_in = in.data();
        } else if (!did_read) {
          // read new
          auto const bytes_transferred =
              impl.stream().read_some(impl.rd_buf.prepare(read_size(
        ...
        // append the empty block codes
        static std::uint8_t constexpr empty_block[4] = {0x00, 0x00, 0xff, 0xff};
        zs.next_in = empty_block;
        zs.avail_in = sizeof(empty_block);
        impl.inflate(zs, zlib::Flush::sync, ec);
        if (!ec) {
      ...
      if (impl.rd_msg_max && beast::detail::sum_exceeds(
        ...
        do_fail(close_code::too_big, error::message_too_big, ec);
        return bytes_written;
      }
    ...
    if (impl.rd_op == detail::opcode::text) {
      // check utf8
      if (!impl.rd_utf8.write(beast::buffers_prefix(bytes_written, buffers)) ||
          (impl.rd_done && !impl.rd_utf8.finish())) {
        // _Fail the WebSocket Connection_
        do_fail(close_code::bad_payload, error::bad_frame_payload, ec);
        return bytes_written;
...
BOOST_BEAST_ASYNC_RESULT2(ReadHandler)
stream<NextLayer, deflateSupported>::async_read_some(
  ...
  static_assert(net::is_mutable_buffer_sequence<MutableBufferSequence>::value,
                "MutableBufferSequence type requirements not met");
  return net::async_initiate<ReadHandler, void(error_code, std::size_t)>(
      run_read_some_op{}, handler, impl_, buffers);
}
