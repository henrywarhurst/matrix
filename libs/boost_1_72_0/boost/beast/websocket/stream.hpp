...
/** Provides message-oriented functionality using WebSocket.
   ...
   Opening Handshake Server Requirements (RFC6455)</a>
        @li <a
   href="https://tools.ietf.org/html/rfc6455#section-7.1.2">Websocket Closing
   Handshake (RFC6455)</a>
        @li <a
   href="https://tools.ietf.org/html/rfc6455#section-5.5.1">Websocket Close
   (RFC6455)</a>
        @li <a
   href="https://tools.ietf.org/html/rfc6455#section-5.5.2">WebSocket Ping
   (RFC6455)</a>
        @li <a
   href="https://tools.ietf.org/html/rfc6455#section-5.5.3">WebSocket Pong
   (RFC6455)</a>
...
#endif
{
  ...
  using control_cb_type = std::function<void(frame_type, string_view)>;
  ...
  */
  ...
  static time_point never() noexcept { return (time_point::max)(); }
...
public:
  ...
  */
  ...
  /** Constructor
      ...
      layer's constructor.
  */
  template <class... Args> explicit stream(Args &&... args);
...
#if BOOST_BEAST_DOXYGEN
  template <class Option> void get_option(Option &opt);

  template <class Option> void set_option(Option opt);
#else
  ...
  */
  ...
  /** Perform the WebSocket handshake in the client role.
     ...
     the handshake, the client sends the Websocket Upgrade HTTP request, and the
     server replies with an HTTP response indicating the result of the
     handshake.
      ...
      <a
     href="https://tools.ietf.org/html/rfc7230#section-3.1.2">status-code</a> of
  ...
  */
  ...
  /** Send a websocket close control frame.
     ...
     when both ends of the connection have sent and received a close frame.
      ...
      @see
      @li <a href="https://tools.ietf.org/html/rfc6455#section-7.1.2">Websocket
  ...
  */
  ...
  /** Send a websocket pong control frame.
     ...
     frame</a>, which is usually sent automatically in response to a ping frame
  ...
  */
  ...
  /** Read a complete message.
      ...
      @throws system_error Thrown on failure.
  */
  template <class DynamicBuffer> std::size_t read(DynamicBuffer &buffer);
  ...
  */
  ...
  BOOST_BEAST_ASYNC_RESULT2(ReadHandler)
  async_read_some(
      DynamicBuffer &buffer, std::size_t limit,
      ReadHandler &&handler = net::default_completion_token_t<executor_type>{});
  ...
  */
  ...
  void handshake_ex(string_view host, string_view target,
                    RequestDecorator const &decorator);

  template <class RequestDecorator>
  void handshake_ex(response_type &res, string_view host, string_view target,
                    RequestDecorator const &decorator);
  ...
  BOOST_BEAST_ASYNC_RESULT1(HandshakeHandler)
  async_handshake_ex(string_view host, string_view target,
                     RequestDecorator const &decorator,
                     HandshakeHandler &&handler);

  template <class RequestDecorator, class HandshakeHandler>
  BOOST_BEAST_ASYNC_RESULT1(HandshakeHandler)
  async_handshake_ex(response_type &res, string_view host, string_view target,
                     RequestDecorator const &decorator,
                     HandshakeHandler &&handler);
  ...
  void accept_ex(http::request<Body, http::basic_fields<Allocator>> const &req,
                 ResponseDecorator const &decorator);
  ...
  BOOST_BEAST_ASYNC_RESULT1(AcceptHandler)
  async_accept_ex(ResponseDecorator const &decorator, AcceptHandler &&handler);
  ...
  template <class Body, class Allocator, class ResponseDecorator,
            class AcceptHandler>
  BOOST_BEAST_ASYNC_RESULT1(AcceptHandler)
  async_accept_ex(http::request<Body, http::basic_fields<Allocator>> const &req,
...
private:
  ...
  void do_accept(http::request<Body, http::basic_fields<Allocator>> const &req,
                 Decorator const &decorator, error_code &ec);

  template <class RequestDecorator>
  void do_handshake(response_type *res_p, string_view host, string_view target,
                    RequestDecorator const &decorator, error_code &ec);
  ...
  void do_fail(std::uint16_t code, error_code ev, error_code &ec);
};
...
*/
inline void seed_prng(std::seed_seq &ss) { detail::prng_seed(&ss); }
