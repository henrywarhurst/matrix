...
/** This class is used as a base for the @ref websocket::stream class template
 * to group common types and constants.
 */
struct stream_base {
  ...
  static duration none() noexcept { return (duration::max)(); }

  /** Stream option used to adjust HTTP fields of WebSocket upgrade request and
   * responses.
   */
  class decorator {
    detail::decorator d_;

    template <class, bool> friend class stream;
    ...
    */
    template <class Decorator
              ...
              class = typename std::enable_if<
                  detail::is_decorator<Decorator>::value>::type
#endif
              >
    explicit decorator(Decorator &&f) : d_(std::forward<Decorator>(f)) {
    }
  };
  ...
  */
  struct timeout {
    ...
    */
    static timeout suggested(role_type role) noexcept {
      timeout opt{};
      switch (role) {
