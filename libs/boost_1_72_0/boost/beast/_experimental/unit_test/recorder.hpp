...
private:
  ...
  virtual void on_suite_end() override { m_results.insert(std::move(m_suite)); }
  ...
  virtual void on_pass() override { m_case.tests.pass(); }

  virtual void on_fail(std::string const &reason) override {
    m_case.tests.fail(reason);
  }

  virtual void on_log(std::string const &s) override { m_case.log.insert(s); }
};
