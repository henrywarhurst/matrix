...
std::string make_reason(String const &reason, char const *file, int line) {
  std::string s(reason);
  if (!s.empty())
    s.append(": ");
...
private:
  ...
  struct abort_exception : public std::exception {
    char const *what() const noexcept override { return "test suite aborted"; }
  };
  ...
  public:
    ...
    */
    ...
    template <class T> scoped_testcase operator<<(T const &t);
  };
...
public:
  ...
  */
  static suite *this_suite() { return *p_this_suite(); }
  ...
  */
  ...
  template <class F, class String> bool except(F &&f, String const &reason);
  template <class F> bool except(F &&f) { return except(f, ""); }
  template <class E, class F, class String>
  bool except(F &&f, String const &reason);
  template <class E, class F> bool except(F &&f) { return except<E>(f, ""); }
  template <class F, class String> bool unexcept(F &&f, String const &reason);
  template <class F> bool unexcept(F &&f) { return unexcept(f, ""); }

  /** Return the argument associated with the runner. */
  std::string const &arg() const { return runner_->arg(); }
  ...
  template <class Condition> bool unexpected(Condition shouldBeFalse) {
...
private:
  ...
  template <class = void> void run(runner &r);
};
...
inline void suite::testcase_t::operator()(std::string const &name,
                                          abort_t abort) {
  suite_.abort_ = abort == abort_on_fail;
  suite_.runner_->testcase(name);
}

inline suite::scoped_testcase suite::testcase_t::operator()(abort_t abort) {
  suite_.abort_ = abort == abort_on_fail;
  return {suite_, ss_};
}

template <class T>
inline suite::scoped_testcase suite::testcase_t::operator<<(T const &t) {
  return {suite_, ss_, t};
}

//------------------------------------------------------------------------------

template <class> void suite::operator()(runner &r) {
  *p_this_suite() = this;
  try {
    run(r);
...
template <class Condition, class String>
bool suite::expect(Condition const &shouldBeTrue, String const &reason) {
  if (shouldBeTrue) {
    pass();
    return true;
  }
...
bool suite::except(F &&f, String const &reason) {
  try {
    f();
    fail(reason);
    return false;
...
template <class F, class String>
bool suite::unexcept(F &&f, String const &reason) {
  try {
    f();
    pass();
    return true;
...
template <class Condition, class String>
bool suite::unexpected(Condition shouldBeFalse, String const &reason) {
...
template <class> void suite::pass() {
  propagate_abort();
  runner_->pass();
}

// ::fail
template <class> void suite::fail(std::string const &reason) {
  propagate_abort();
  runner_->fail(reason);
  if (abort_) {
    aborted_ = true;
...
template <class String>
void suite::fail(String const &reason, char const *file, int line) {
  fail(detail::make_reason(reason, file, line));
}

inline void suite::propagate_abort() {
  if (abort_ && aborted_)
    BOOST_THROW_EXCEPTION(abort_exception());
}

template <class> void suite::run(runner &r) {
  runner_ = &r;
...
#ifndef BEAST_FAIL
#define BEAST_FAIL()                                                           \
  ::boost::beast::unit_test::suite::this_suite()->fail("", __FILE__, __LINE__)
#endif
...
*/
#define BEAST_EXPECT(cond)                                                     \
  ::boost::beast::unit_test::suite::this_suite()->expect(cond, __FILE__,       \
                                                         __LINE__)
#endif
...
#define BEAST_EXPECTS(cond, reason)                                            \
  ((cond) ? (::boost::beast::unit_test::suite::this_suite()->pass(), true)     \
          : (::boost::beast::unit_test::suite::this_suite()->fail(             \
...
#define BEAST_DEFINE_TESTSUITE_INSERT(Library, Module, Class, manual)          \
  static ::boost::beast::unit_test::detail::insert_suite<Class##_test>         \
      Library##Module##Class##_test_instance(#Class, #Module, #Library,        \
                                             manual)
