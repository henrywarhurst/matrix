...
class stream {
  ...
  struct state {
    ...
    BOOST_BEAST_DECL
    state(net::io_context &ioc_, boost::weak_ptr<service_impl> wp_,
          fail_count *fc_);
  ...
  template <class Handler, class Buffers> class read_op;

  struct run_read_op;
  struct run_write_op;

  BOOST_BEAST_DECL
  static void initiate_read(boost::shared_ptr<state> const &in,
                            std::unique_ptr<read_op_base> &&op,
                            std::size_t buf_size);
  ...
  // DEPRECATED
  lowest_layer_type const &lowest_layer() const noexcept { return *this; }
#endif
...
public:
  ...
  */
  BOOST_BEAST_DECL
  explicit stream(net::io_context &ioc);
  ...
  */
  BOOST_BEAST_DECL
  stream(net::io_context &ioc, fail_count &fc);
  ...
  */
  BOOST_BEAST_DECL
  stream(net::io_context &ioc, string_view s);
  ...
  */
  BOOST_BEAST_DECL
  stream(net::io_context &ioc, fail_count &fc, string_view s);

  /// Establish a connection
  BOOST_BEAST_DECL
  void connect(stream &remote);

  /// The type of the executor associated with the object.
  using executor_type = net::io_context::executor_type;

  /// Return the executor associated with the object.
  executor_type get_executor() noexcept { return in_->ioc.get_executor(); };

  /// Set the maximum number of bytes returned by read_some
  void read_size(std::size_t n) noexcept { in_->read_max = n; }

  /// Set the maximum number of bytes returned by write_some
  void write_size(std::size_t n) noexcept { in_->write_max = n; }

  /// Direct input buffer access
  buffer_type &buffer() noexcept { return in_->b; }
  ...
  std::size_t nread() const noexcept { return in_->nread; }

  /// Return the number of bytes read
  std::size_t nread_bytes() const noexcept { return in_->nread_bytes; }

  /// Return the number of writes
  std::size_t nwrite() const noexcept { return in_->nwrite; }

  /// Return the number of bytes written
  std::size_t nwrite_bytes() const noexcept { return in_->nwrite_bytes; }
  ...
  */
  ...
  /** Read some data from the stream.
     ...
     of bytes. Consider using the function `net::read` if you need to ensure
  ...
  */
  ...
  /** Start an asynchronous write.
      ...
      to ensure that all data is written before the asynchronous operation
     completes.
  */
...
#if !BOOST_BEAST_DOXYGEN
  friend BOOST_BEAST_DECL void teardown(role_type, stream &s,
                                        boost::system::error_code &ec);

  template <class TeardownHandler>
  friend BOOST_BEAST_DECL void async_teardown(role_type role, stream &s,
                                              TeardownHandler &&handler);
#endif
};

#if !BOOST_BEAST_DOXYGEN
inline void beast_close_socket(stream &s) { s.close(); }
#endif
...
#else
BOOST_BEAST_DECL
stream connect(stream &to);

BOOST_BEAST_DECL
void connect(stream &s1, stream &s2);
