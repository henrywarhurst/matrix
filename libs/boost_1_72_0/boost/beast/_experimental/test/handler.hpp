...
public:
  ...
  explicit handler(error_code ec) : ec_(ec) {}

  explicit handler(boost::none_t) {}

  handler(handler &&other)
      : ec_(other.ec_), pass_(boost::exchange(other.pass_, true)) {}

  ~handler() { BEAST_EXPECT(pass_); }

  template <class... Args> void operator()(error_code ec, Args &&...) {
    BEAST_EXPECT(!pass_); // can't call twice
    BEAST_EXPECTS(!ec_ || ec == *ec_, ec.message());
    pass_ = true;
  }
...
*/
inline handler success_handler() noexcept { return handler(error_code{}); }
...
*/
inline handler any_handler() noexcept { return handler(boost::none); }
...
*/
inline handler fail_handler(error_code ec) noexcept { return handler(ec); }
...
*/
inline void run(net::io_context &ioc) {
  ioc.run();
  ioc.restart();
}
...
void run_for(net::io_context &ioc, std::chrono::duration<Rep, Period> elapsed) {
  ioc.run_for(elapsed);
  ioc.restart();
}
