...
template <typename LeastInt> struct int_fast_t {
...
template <int Bits> struct exact_signed_base_helper {};
template <int Bits> struct exact_unsigned_base_helper {};
...
struct int_t : public boost::detail::exact_signed_base_helper<Bits> {
  BOOST_STATIC_ASSERT_MSG(Bits <= (int)(sizeof(boost::intmax_t) * CHAR_BIT),
                          "No suitable signed integer type with the requested "
                          "number of bits is available.");
...
struct uint_t : public boost::detail::exact_unsigned_base_helper<Bits> {
  ...
  typedef
      typename detail::int_least_helper<::boost::uint_t<Bits>::s>::least least;
#else
