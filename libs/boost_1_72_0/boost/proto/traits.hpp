...
namespace proto {
namespace detail {
template <typename T, typename Void = void> struct if_vararg {};

template <typename T> struct if_vararg<T, typename T::proto_is_vararg_> : T {};
...
template <typename T> struct is_callable : proto::detail::is_callable_<T> {};

/// INTERNAL ONLY
///
template <> struct is_callable<proto::_> : mpl::true_ {};

/// INTERNAL ONLY
///
template <> struct is_callable<proto::callable> : mpl::false_ {};
...
/// false.
template <typename T> struct is_transform : proto::detail::is_transform_<T> {};
...
template <typename T> struct is_aggregate : proto::detail::is_aggregate_<T> {};
...
template <typename T> struct is_expr<T &, void> : is_expr<T> {};
...
template <typename Expr> struct tag_of<Expr &> {
  typedef typename Expr::proto_tag type;
};
...
template <typename Expr> struct arity_of<Expr &> : Expr::proto_arity {};
...
template <typename Expr> struct value {
  ...
  /// \li <tt>T</tt> becomes <tt>T</tt>
  typedef typename detail::term_traits<typename Expr::proto_child0>::value_type
      type;
};

template <typename Expr> struct value<Expr &> {
  /// Verify that we are actually operating on a terminal
  BOOST_STATIC_ASSERT(0 == Expr::proto_arity_c);
  ...
  /// \li <tt>T</tt> becomes <tt>T &</tt>
  typedef
      typename detail::term_traits<typename Expr::proto_child0>::reference type;
};

template <typename Expr> struct value<Expr const &> {
  /// Verify that we are actually operating on a terminal
  BOOST_STATIC_ASSERT(0 == Expr::proto_arity_c);
  ...
  typedef
      typename detail::term_traits<typename Expr::proto_child0>::const_reference
          type;
};
...
template <typename Expr> struct left : child_c<Expr, 0> {};
...
template <typename Expr> struct right : child_c<Expr, 1> {};
...
template <typename T> struct terminal : proto::transform<terminal<T>, int> {
...
#define BOOST_PROTO_DEFINE_UNARY_METAFUNCTION(Op)                              \
  template <typename T> struct Op : proto::transform<Op<T>, int> {             \
...
/// INTERNAL ONLY
///
template <long N> struct is_callable<functional::child_c<N>> : mpl::true_ {};

/// INTERNAL ONLY
///
template <typename N> struct is_callable<functional::child<N>> : mpl::true_ {};
