...
struct domain : Generator {
  ...
  /// expressions, if they are not already, by making them Proto terminals held
  /// by value if possible. Objects that are already Proto expressions are left
  /// alone.
  ///
  /// If <tt>wants_basic_expr\<Generator\>::value</tt> is true, then let \c E be
  /// \c basic_expr; otherwise, let \t E be \c expr. Given an lvalue \c t of
  /// type \c T:
  ///
  ...
  /// expressions, if they are not already, by making them Proto terminals held
  /// by reference. Objects that are already Proto expressions are simply
...
/// INTERNAL ONLY
///
template <typename T> struct domain_of<T, typename T::proto_is_expr_> {
  typedef typename T::proto_domain type;
};

/// INTERNAL ONLY
///
template <typename T> struct domain_of<T &, void> {
  typedef typename domain_of<T>::type type;
};

/// INTERNAL ONLY
///
template <typename T> struct domain_of<boost::reference_wrapper<T>, void> {
  typedef typename domain_of<T>::type type;
};
