...
namespace detail {
template <typename Expr> struct by_value_generator_;

template <typename Tag, typename Arg>
struct by_value_generator_<proto::expr<Tag, term<Arg>, 0>> {
  typedef proto::expr<Tag, term<typename detail::term_traits<Arg>::value_type>,
                      0>
      type;
...
template <template <typename> class Extends> struct generator {
  ...
  template <typename This, typename Expr> struct result<This(Expr const &)> {
    typedef Extends<Expr> type;
  };
...
template <template <typename> class Extends> struct pod_generator {
  ...
  BOOST_FORCEINLINE
      ...
      operator()(basic_expr<tag::terminal, proto::term<Member Class::*>> const
                     &e) const {
...
/// INTERNAL ONLY
template <> struct is_callable<default_generator> : mpl::true_ {};
...
/// INTERNAL ONLY
template <> struct is_callable<by_value_generator> : mpl::true_ {};
...
namespace boost {
template <typename Expr> struct result_of<proto::default_domain(Expr)> {
  typedef Expr type;
};

template <typename Expr> struct result_of<proto::basic_default_domain(Expr)> {
  typedef Expr type;
};

template <typename Expr> struct result_of<proto::default_generator(Expr)> {
  typedef Expr type;
};
...
template <typename Expr> struct tr1_result_of<proto::default_domain(Expr)> {
  typedef Expr type;
};
...
template <typename Expr> struct tr1_result_of<proto::default_generator(Expr)> {
  typedef Expr type;
};
