...
namespace detail {
template <typename Expr, typename BasicExpr, typename Grammar> struct matches_;

template <bool B, typename Pred> struct and_2;

template <typename And, typename Expr, typename State, typename Data>
struct _and_impl;

template <typename T, typename U> struct array_matches : mpl::false_ {};
...
template <typename T, typename U> struct is_cv_ref_compatible : mpl::true_ {};
...
template <typename T> struct terminal_matches<T, T> : mpl::true_ {};

template <typename T> struct terminal_matches<T &, T> : mpl::true_ {};

template <typename T> struct terminal_matches<T const &, T> : mpl::true_ {};

template <typename T> struct terminal_matches<T, proto::_> : mpl::true_ {};

template <typename T> struct terminal_matches<T, exact<T>> : mpl::true_ {};
...
template <typename Grammar> struct not_ : transform<not_<Grammar>> {
...
struct if_ : transform<if_<If, Then, Else>> {
  ...
  template <typename Expr, typename State, typename Data>
  struct impl : transform_impl<Expr, State, Data> {
    typedef typename when<_, If>::template impl<Expr, State, Data>::result_type
        condition;

    typedef typename mpl::if_c<static_cast<bool>(
...
/// case)
///
template <typename Cases> struct switch_<Cases> : transform<switch_<Cases>> {
  typedef switch_ proto_grammar;

  template <typename Expr, typename State, typename Data>
  struct impl : Cases::template case_<typename Expr::proto_tag>::template impl<
                    Expr, State, Data> {};
...
/// <tt>G</tt>'s transform.
template <typename Grammar> struct vararg : Grammar {
  /// INTERNAL ONLY
...
/// INTERNAL ONLY
///
template <typename Grammar> struct is_callable<not_<Grammar>> : mpl::true_ {};
...
/// INTERNAL ONLY
///
template <typename Grammar> struct is_callable<vararg<Grammar>> : mpl::true_ {};
