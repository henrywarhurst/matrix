...
template <typename Tag, typename Arg> struct address_of_hack {
...
BOOST_FORCEINLINE Expr make_terminal(T (&t)[N], Expr *,
                                     proto::term<Arg0[N]> *) {
  Expr that;
...
template <typename T, typename U> struct same_cv { typedef U type; };

template <typename T, typename U> struct same_cv<T const, U> {
...
template <typename Sig, typename This, typename Domain> struct funop;
...
/// expression.
template <typename Expr> struct unexpr : Expr {
  BOOST_PROTO_UNEXPR()
