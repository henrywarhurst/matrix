...
#define BOOST_PROTO_COMMON_DOMAIN2(Z, N, DATA)                                 \
  typedef typename common_domain2<common##N, A##N>::type BOOST_PP_CAT(         \
      common, BOOST_PP_INC(N));                                                \
  /**/

#if defined(__WAVE__) && defined(BOOST_PROTO_CREATE_PREPROCESSED_FILES)
#pragma wave option(preserve : 2, line : 0, output                             \
                    : "preprocessed/deduce_domain_n.hpp")
#endif
...
#define BOOST_PP_ITERATION_PARAMS_1                                            \
  (3,                                                                          \
   (3, BOOST_PROTO_MAX_ARITY, <boost / proto / detail / deduce_domain_n.hpp>))
...
struct BOOST_PP_CAT(deduce_domain, N)
    : BOOST_PP_CAT(common_domain, N)<BOOST_PP_ENUM_BINARY_PARAMS(
          N, typename domain_of<E, >::type BOOST_PP_INTERCEPT)> {};
