...
//==============================================================================
#define BOOST_PROTO_DETAILS_KEYWORD_FACILITY_REMOVE_FRONT(                     \
    TOKENS, IS_FRONT_MACRO, REMOVING_PREFIX)                                   \
  BOOST_PP_EXPAND(/* without EXPAND doesn't expand on MSVC */                  \
                  BOOST_PP_IIF(IS_FRONT_MACRO(TOKENS), BOOST_PP_CAT,           \
                               TOKENS BOOST_PP_TUPLE_EAT(2))(REMOVING_PREFIX,  \
                                                             TOKENS))          \
  /**/
...
#define BOOST_PROTO_DETAILS_KEYWORD_IS_TYPENAME_FRONT(TOKENS)                  \
  BOOST_PROTO_DETAILS_KEYWORD_FACILITY_IS_FRONT(                               \
      TOKENS, BOOST_PROTO_DETAILS_KEYWORD_TYPENAME_IS_)                        \
  /**/

//==============================================================================
/*!
 * \ingroup preprocessor
 * For any symbol \c X, this macro returns the same symbol from which a
 * potential leading \c typename keyword has been removed. If no typename
 * keyword is present, this macros evaluates to \c X itself without error.
 *
...
#define BOOST_PROTO_REMOVE_TYPENAME(X)                                         \
  BOOST_PROTO_DETAILS_KEYWORD_FACILITY_REMOVE_FRONT(                           \
      X, BOOST_PROTO_DETAILS_KEYWORD_IS_TYPENAME_FRONT,                        \
