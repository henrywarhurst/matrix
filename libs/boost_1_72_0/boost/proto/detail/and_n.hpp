...
#else
    : BOOST_PP_CAT(and_,
                   BOOST_PP_DEC(N))<P0::value BOOST_PP_COMMA_IF(BOOST_PP_SUB(
          N, 2)) BOOST_PP_ENUM_SHIFTED_PARAMS(BOOST_PP_DEC(N), P)> {
};
#endif
...
template <BOOST_PP_ENUM_PARAMS(N, typename G), typename Expr, typename State,
          typename Data>
struct _and_impl<proto::and_<BOOST_PP_ENUM_PARAMS(N, G)>, Expr, State, Data>
    : proto::transform_impl<Expr, State, Data> {
#define M0(Z, N, DATA)                                                         \
  typedef typename proto::when<proto::_, BOOST_PP_CAT(G, N)>::template impl<   \
      Expr, State, Data>                                                       \
...
#undef M0
  ...
  typedef
      typename BOOST_PP_CAT(Gimpl, BOOST_PP_DEC(N))::result_type result_type;
