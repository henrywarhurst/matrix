...
#define BOOST_PROTO_FUSION_NEXT_ITERATOR_TYPE(Z, N, DATA)                      \
  typedef                                                                      \
      typename fusion::result_of::next<BOOST_PP_CAT(fusion_iterator, N)>::type \
...
#define BOOST_PROTO_FUSION_ITERATORS_TYPE(N)                                   \
  typedef typename fusion::result_of::begin<Sequence const>::type              \
      fusion_iterator0;                                                        \
  BOOST_PP_REPEAT(BOOST_PP_DEC(N), BOOST_PROTO_FUSION_NEXT_ITERATOR_TYPE,      \
                  fusion_iterator)                                             \
  /**/
...
#define BOOST_PROTO_FUSION_AT_TYPE(Z, N, DATA)                                 \
  typename add_const<typename fusion::result_of::value_of<BOOST_PP_CAT(        \
      fusion_iterator, N)>::type>::type /**/
...
#define BOOST_PROTO_FUSION_ITERATORS(N)                                        \
  ...
  BOOST_PP_REPEAT(BOOST_PP_DEC(N), BOOST_PROTO_FUSION_NEXT_ITERATOR,           \
                  fusion_iterator)                                             \
  /**/
...
#define BOOST_PROTO_FUSION_AS_CHILD_AT_TYPE(Z, N, DATA)                        \
  typename detail::protoify<BOOST_PROTO_FUSION_AT_TYPE(Z, N, DATA),            \
                            Domain>::result_type /**/

/// INTERNAL ONLY
///
#define BOOST_PROTO_FUSION_AS_CHILD_AT(Z, N, DATA)                             \
  detail::protoify<BOOST_PROTO_FUSION_AT_TYPE(Z, N, DATA), Domain>()(          \
      BOOST_PROTO_FUSION_AT(Z, N, DATA)) /**/

#if defined(__WAVE__) && defined(BOOST_PROTO_CREATE_PREPROCESSED_FILES)
#pragma wave option(preserve : 2, line : 0, output                             \
                    : "preprocessed/unpack_expr_.hpp")
#endif
...
struct unpack_expr_<Tag, Domain, Sequence, N> {
  ...
  typedef BOOST_PP_CAT(
      list,
      N)<BOOST_PP_ENUM(N, BOOST_PROTO_FUSION_AS_CHILD_AT_TYPE, ~)> proto_args;
...
struct unpack_expr_<Tag, deduce_domain, Sequence, N> {
  ...
  typedef unpack_expr_<Tag, typename BOOST_PP_CAT(deduce_domain, N) <
