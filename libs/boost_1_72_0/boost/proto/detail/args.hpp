...
/// \brief A type sequence, for use as the 2nd parameter to the \c expr\<\>
/// class template.
///
/// A type sequence, for use as the 2nd parameter to the \c expr\<\> class
/// template. The types in the sequence correspond to the children of a node in
/// an expression tree.
...
/// template. The types in the sequence correspond to the children of a node in
/// an expression tree.
template <BOOST_PP_ENUM_PARAMS(N, typename Arg)> struct BOOST_PP_CAT(list, N) {
