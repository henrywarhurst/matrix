...
#define BOOST_PROTO_MATCHES_N_FUN(Z, N, DATA)                                  \
  matches_<typename detail::expr_traits<typename Args1::BOOST_PP_CAT(          \
               child, N)>::value_type::proto_derived_expr,                     \
           typename detail::expr_traits<typename Args1::BOOST_PP_CAT(          \
               child, N)>::value_type::proto_grammar,                          \
           typename Args2::BOOST_PP_CAT(child, N)::proto_grammar>

#define BOOST_PROTO_DEFINE_MATCHES(Z, N, DATA)                                 \
  matches_<Expr, BasicExpr, typename BOOST_PP_CAT(G, N)::proto_grammar>

#if defined(__WAVE__) && defined(BOOST_PROTO_CREATE_PREPROCESSED_FILES)
#pragma wave option(preserve : 2, line : 0, output                             \
                    : "preprocessed/matches_.hpp")
#endif
...
#define BOOST_PP_ITERATION_PARAMS_1                                            \
  (3, (2, BOOST_PP_MAX(BOOST_PROTO_MAX_ARITY, BOOST_PROTO_MAX_LOGICAL_ARITY),  \
...
struct matches_<Expr, proto::basic_expr<Tag, Args1, N>,
                proto::basic_expr<Tag, Args2, N>>
    : BOOST_PP_CAT(and_,
                   N)<BOOST_PROTO_MATCHES_N_FUN(~, 0, ~)::value,
                      BOOST_PP_ENUM_SHIFTED(N, BOOST_PROTO_MATCHES_N_FUN, ~)> {
};

template <typename Expr, typename Tag, typename Args1, typename Args2>
struct matches_<Expr, proto::basic_expr<Tag, Args1, N>,
                proto::basic_expr<proto::_, Args2, N>>
    : BOOST_PP_CAT(and_,
                   N)<BOOST_PROTO_MATCHES_N_FUN(~, 0, ~)::value,
                      BOOST_PP_ENUM_SHIFTED(N, BOOST_PROTO_MATCHES_N_FUN, ~)> {
};
