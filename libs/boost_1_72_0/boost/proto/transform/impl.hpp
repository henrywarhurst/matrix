...
struct empty_env {
  ...
  struct lookup {
    ...
    typedef typename add_reference<typename add_const<OtherValue>::type>::type
        const_reference;
  };

  key_not_found operator[](detail::any) const { return key_not_found(); }

  template <typename T> T const &at(detail::any, T const &t) const { return t; }
};
} // namespace envns_

////////////////////////////////////////////////////////////////////////////////////////////
// is_env
template <typename T, typename Void> struct is_env : mpl::false_ {};

template <typename T>
struct is_env<T, typename T::proto_environment_> : mpl::true_ {};

template <typename T> struct is_env<T &, void> : is_env<T> {};
...
#define BOOST_PROTO_TRANSFORM_(PrimitiveTransform, X)                          \
  ...
  template <typename Sig> struct result {                                      \
    typedef                                                                    \
        typename boost::proto::detail::apply_transform<Sig>::result_type type; \
  };                                                                           \
                                                                               \
...
namespace detail {
template <typename Sig> struct apply_transform;
...
template <typename PrimitiveTransform, typename X> struct transform {
