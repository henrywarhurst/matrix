...
template <typename T> struct value_type {
  ...
  typedef
      typename mpl::if_c<is_noncopyable<T>::value, reference, value>::type type;
};
  ...
  struct TAG {                                                                 \
    ...
    operator=(Value &value) const {                                            \
      return boost::proto::env<                                                \
          TAG, typename boost::proto::detail::value_type<Value>::type>(value); \
    }                                                                          \
    template <typename Value>                                                  \
    boost::proto::env<TAG, typename boost::proto::detail::value_type<          \
    ...
    operator=(Value const &value) const {                                      \
      return boost::proto::env<                                                \
          TAG, typename boost::proto::detail::value_type<Value const>::type>(  \
          value);                                                              \
    }                                                                          \
  };                                                                           \
                                                                               \
...
public:
  ...
  /// INTERNAL ONLY
  template <typename OtherValue> struct lookup<Key, OtherValue> {
    typedef Value type;
    typedef typename add_reference<typename add_const<Value>::type>::type
        const_reference;
  };
  ...
  using Base::at;
  template <typename T> const_reference at(Key, T const &) const {
    return this->value_;
  }
};
...
struct as_env {
  ...
  /// INTERNAL ONLY
  template <typename T, bool B = is_env<T>::value> struct impl {
  ...
  template <typename This, typename T> struct result<This(T)> {
    typedef typename impl<typename detail::normalize_arg<T>::type>::result_type
        type;
  };
...
struct has_env_var : detail::poly_function<has_env_var<Key>> {
  ...
  template <typename Env, bool IsEnv = is_env<Env>::value> struct impl {
  ...
  template <typename Env> struct impl<Env, false> {
...
////////////////////////////////////////////////////////////////////////////////////////
// env_var
template <typename Key> struct env_var : detail::poly_function<env_var<Key>> {
  BOOST_PROTO_CALLABLE()

  template <typename Env> struct impl {
...
struct env_var<data_type> : detail::poly_function<env_var<data_type>> {
  ...
  template <typename Env, bool B = is_env<Env>::value> struct impl {
    typedef Env result_type;

    result_type operator()(detail::arg<Env> e) const { return e(); }
  };

  template <typename Env> struct impl<Env, true> {
    typedef
        typename remove_reference<Env>::type::template lookup<data_type>::type
            result_type;
...
////////////////////////////////////////////////////////////////////////////////////////////
// _env_var
template <typename Key> struct _env_var : proto::transform<_env_var<Key>> {
  ...
  struct impl : transform_impl<Expr, State, Data> {
    ...
    BOOST_PROTO_RETURN_TYPE_STRICT_LOOSE(
        result_type, typename impl::data::template lookup<Key>::const_reference)
...
/// INTERNAL ONLY
template <typename Key> struct is_callable<_env_var<Key>> : mpl::true_ {};
