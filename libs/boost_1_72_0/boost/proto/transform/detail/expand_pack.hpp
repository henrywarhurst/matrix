...
#define M0(Z, X, DATA)                                                         \
  typename expand_pattern_helper<Tfx, BOOST_PP_CAT(A, X)>::type
#define M1(Z, X, DATA)                                                         \
  expand_pattern_helper<Tfx, BOOST_PP_CAT(A, X)>::applied::value ||

template <typename Tfx,
          typename Ret BOOST_PP_ENUM_TRAILING_PARAMS(N, typename A)>
