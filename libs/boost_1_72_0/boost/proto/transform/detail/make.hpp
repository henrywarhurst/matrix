...
#define BOOST_PROTO_CONSTRUCT_ARG(Z, M, DATA)                                  \
  detail::as_lvalue(typename when<_, BOOST_PP_CAT(A, M)>::template impl<       \
...
template <template <BOOST_PP_ENUM_PARAMS(N, typename BOOST_PP_INTERCEPT)>
          class R BOOST_PP_ENUM_TRAILING_PARAMS(N, typename A),
          typename Expr, typename State, typename Data>
struct make_<R<BOOST_PP_ENUM_PARAMS(N, A)>, Expr, State,
             Data BOOST_PROTO_TEMPLATE_ARITY_PARAM(N)>
    : nested_type_if<R<BOOST_PP_ENUM(N, BOOST_PROTO_MAKE_IF_TYPE, ~)>,
                     (BOOST_PP_REPEAT(N, BOOST_PROTO_MAKE_IF_APPLIED,
                                      ~) false)> {};

template <template <BOOST_PP_ENUM_PARAMS(N, typename BOOST_PP_INTERCEPT)>
          class R BOOST_PP_ENUM_TRAILING_PARAMS(N, typename A),
          typename Expr, typename State, typename Data>
struct make_<noinvoke<R<BOOST_PP_ENUM_PARAMS(N, A)>>, Expr, State,
...
struct construct_<proto::expr<T, A, N>, true> {
  ...
  operator()(BOOST_PP_ENUM_BINARY_PARAMS(BOOST_PP_MAX(N, 1), A, &a)) const {
...
struct make<Object(BOOST_PP_ENUM_PARAMS(N, A))>
  ...
  struct impl : transform_impl<Expr, State, Data> {
    /// \brief <tt>boost::result_of\<make\<Object\>(Expr, State,
    /// Data)\>::type</tt>
    typedef
    ...
    result_type operator()(typename impl::expr_param e,
      ...
      return detail::construct<result_type>(
          BOOST_PP_ENUM(N, BOOST_PROTO_CONSTRUCT_ARG, DATA));
    }
  };
};
...
struct make<Object(BOOST_PP_ENUM_PARAMS(N, A)...)>
  ...
  template <typename Expr, typename State, typename Data>
      ...
      : make<typename detail::expand_pattern<
            proto::arity_of<Expr>::value, BOOST_PP_CAT(A, BOOST_PP_DEC(N)),
            detail::BOOST_PP_CAT(expand_pattern_rest_, BOOST_PP_DEC(N)) <
                Object BOOST_PP_ENUM_TRAILING_PARAMS(BOOST_PP_DEC(N), A)>>::
