...
#define BOOST_PP_ITERATION_PARAMS_1                                            \
  (3, (0, BOOST_PP_DEC(BOOST_PROTO_MAX_ARITY),                                 \
...
#define M0(Z, X, D)                                                            \
  typename expand_pattern_helper<proto::_child_c<X>, Fun>::type
...
struct BOOST_PP_CAT(expand_pattern_rest_, N) {
  template <BOOST_PP_ENUM_PARAMS_WITH_A_DEFAULT(BOOST_PP_INC(M), typename C,
                                                void)>
  struct cat;
...
struct cat<BOOST_PP_ENUM_PARAMS(I, C)> {
  typedef msvc_fun_workaround<Ret(BOOST_PP_ENUM_PARAMS(J, A) BOOST_PP_COMMA_IF(
      J) BOOST_PP_ENUM_PARAMS(I, C))>
      type;
};
#undef J
#undef I
