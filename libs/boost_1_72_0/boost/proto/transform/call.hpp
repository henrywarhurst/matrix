...
/// {};
/// \endcode
template <typename PrimitiveTransform> struct call : PrimitiveTransform {};
...
/// properly.
template <typename Fun> struct call<Fun *> : call<Fun> {};
...
/// arguments.
template <typename Fun> struct call<Fun()> : transform<call<Fun()>> {
  /// INTERNAL ONLY
...
struct call<Fun(A0)> : transform<call<Fun(A0)>> {
  ...
  struct impl2 : transform_impl<Expr, State, Data> {
    typedef
        typename when<_, A0>::template impl<Expr, State, Data>::result_type a0;
    ...
    result_type operator()(typename impl2::expr_param e,
      ...
      return
          typename detail::poly_function_traits<Fun, Fun(a0)>::function_type()(
              detail::as_lvalue(
  ...
  struct impl2<Expr, State, Data, true> : transform_impl<Expr, State, Data> {
    typedef
        typename when<_, A0>::template impl<Expr, State, Data>::result_type a0;
    typedef
        typename Fun::template impl<a0, State, Data>::result_type result_type;
    ...
    result_type operator()(typename impl2::expr_param e,
      ...
      return typename Fun::template impl<a0, State, Data>()(
          typename when<_, A0>::template impl<Expr, State, Data>()(e, s, d), s,
          d);
    }
  };
...
struct call<Fun(A0, A1)> : transform<call<Fun(A0, A1)>> {
  ...
  struct impl2 : transform_impl<Expr, State, Data> {
    typedef
        typename when<_, A0>::template impl<Expr, State, Data>::result_type a0;
    typedef
        typename when<_, A1>::template impl<Expr, State, Data>::result_type a1;
    typedef typename detail::poly_function_traits<Fun, Fun(a0, a1)>::result_type
        result_type;
  ...
  struct impl2<Expr, State, Data, true> : transform_impl<Expr, State, Data> {
    ...
    result_type operator()(typename impl2::expr_param e,
      ...
      return typename Fun::template impl<a0, a1, Data>()(
          ...
          typename when<_, A1>::template impl<Expr, State, Data>()(e, s, d), d);
    }
  };
...
struct call<Fun(A0, A1, A2)> : transform<call<Fun(A0, A1, A2)>> {
  ...
  struct impl2 : transform_impl<Expr, State, Data> {
    ...
    typedef
        typename when<_, A1>::template impl<Expr, State, Data>::result_type a1;
    typedef
        typename when<_, A2>::template impl<Expr, State, Data>::result_type a2;
    typedef
        typename detail::poly_function_traits<Fun, Fun(a0, a1, a2)>::result_type
            result_type;
...
/// INTERNAL ONLY
///
template <typename Fun> struct is_callable<call<Fun>> : mpl::true_ {};
