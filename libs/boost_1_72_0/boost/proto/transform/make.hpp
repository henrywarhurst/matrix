...
template <typename T> struct nested_type<T, false> { typedef T type; };

template <typename T, bool Applied> struct nested_type_if {
  typedef T type;
  static bool const applied = false;
};

template <typename T> struct nested_type_if<T, true> : nested_type<T> {
...
template <typename Type> struct construct_<Type, true> {
  typedef Type result_type;
...
template <typename Object> struct make : transform<make<Object>> {
...
/// INTERNAL ONLY
///
template <typename Object> struct is_callable<make<Object>> : mpl::true_ {};
