...
template <typename T> yes_type check_is_expr_handled(T const &);
...
struct callable_context {
  ...
  /// as <tt>ctx(Expr::proto_tag(), child_c\<0\>(expr),
  /// child_c\<1\>(expr)...)</tt>, then <tt>eval\<Expr, ThisContext\></tt>
  /// inherits from <tt>callable_eval\<Expr, ThisContext\></tt>. Otherwise,
  /// <tt>eval\<Expr, ThisContext\></tt> inherits from
  /// <tt>DefaultCtx::eval\<Expr, Context\></tt>.
  template <typename Expr, typename ThisContext = Context>
  struct eval
      : mpl::if_c<detail::is_expr_handled<Expr, Context>::value,
                  callable_eval<Expr, ThisContext>,
