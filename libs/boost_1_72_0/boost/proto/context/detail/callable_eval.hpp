...
#define BOOST_PROTO_CHILD_N_TYPE(Z, N, Expr)                                   \
  typedef                                                                      \
      typename proto::result_of::child_c<Expr const &, N>::type BOOST_PP_CAT(  \
          child, N);                                                           \
  /**/
...
#pragma wave option(preserve : 2, line : 0, output                             \
                    : "preprocessed/callable_eval.hpp")
#endif
...
struct is_expr_handled<Expr, Context, N> {
  ...
  static const bool value =
      ...
      sizeof(detail::check_is_expr_handled(
          (sctx_(stag_ BOOST_PP_ENUM_TRAILING(N, BOOST_PROTO_CHILD_N, sexpr_)),
           0)));
...
struct callable_eval<Expr, Context, N> {
  ...
  typedef typename BOOST_PROTO_RESULT_OF<Context(
      typename Expr::proto_tag BOOST_PP_ENUM_TRAILING_PARAMS(N, child))>::type
      result_type;

  /// \param expr The current expression
  /// \param context The callable evaluation context
  /// \return <tt>context(Expr::proto_tag(), child_c\<0\>(expr),
  /// child_c\<1\>(expr), ...)</tt>
  result_type operator()(Expr &expr, Context &context) const {
    return context(typename Expr::proto_tag()
