...
public:
  ...
  result_type const operator()(Expr &expr, Context &ctx) const {
    return detail::memfun<r0, r1>(proto::eval(proto::child_c<0>(expr), ctx),
                                  proto::eval(proto::child_c<1>(expr), ctx));
  }
};
...
struct default_eval<Expr, Context, tag::mem_ptr, 2>
    : memfun_eval<Expr, Context,
                  is_member_function_eval<Expr, Context>::value> {};
...
private:
  result_type invoke(Expr &expr, Context &context, mpl::false_,
    ...
    return BOOST_PROTO_DEFAULT_EVAL(~, 0,
                                    expr)(BOOST_PROTO_DEFAULT_EVAL(~, 1, expr));
  }
